...
import {
  type RemarkRemoveNodesOptions,
  remarkRouterFilter,
} from './remark-remove-node';
  ...
  return (node, file) => {
    ...
    visit(node, (element) => {
      ...
      if (element.type === 'mdxJsxFlowElement' && element.name) {
        console.log('mdxJsxFlowElement', element.name);
        data.contents.push(
...
export async function structure(
  ...
  removeOptions: RemarkRemoveNodesOptions = { nodeNames: [] },
): Promise<StructuredData> {
  const result = remark()
    .use(remarkGfm)
    .use(remarkRouterFilter, removeOptions)
    .use(remarkStructure, options)
    .process(content);

  return (await result).data.structuredData as StructuredData;
}
