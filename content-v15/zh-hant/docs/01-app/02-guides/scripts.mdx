...
使用 `worker` 策略的腳本會透過 [Partytown](https://partytown.builder.io/) 卸載並在網頁工作者中執行。這可以透過將主要執行緒專用於應用程式的其他程式碼來改善網站效能。
...
接著，執行 `next`（通常為 `npm run dev` 或 `yarn dev`），Next.js 將引導您安裝必要的套件以完成設定：

```bash filename="Terminal"
npm run dev
```

您將看到類似以下的指示：請執行 `npm install @builder.io/partytown` 來安裝 Partytown。
...
在網頁工作者中載入第三方腳本時，需考量許多權衡取捨。請參閱 Partytown 的 [權衡取捨](https://partytown.builder.io/trade-offs) 文件以獲取更多資訊。
...
請查看 Partytown 的 [設定選項](https://partytown.builder.io/configuration) 以查看可新增的其他屬性完整清單。
