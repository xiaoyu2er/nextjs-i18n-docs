...
**草稿模式** 讓您可以預覽來自您的無頭 CMS 的草稿內容，這些內容位於 Next.js 應用程式中。這對於在建置時生成的靜態頁面特別有用，因為它允許您切換至 [動態渲染](/docs/app/getting-started/partial-prerendering#dynamic-rendering)，並查看草稿變更，而無需重新建置整個網站。
