...
- [API 路由请求助手 (Request Helpers)](https://github.com/vercel/next.js/tree/canary/examples/api-routes-middleware)
- [使用 GraphQL 的 API 路由 (API Routes with GraphQL)](https://github.com/vercel/next.js/tree/canary/examples/api-routes-graphql)
- [使用 REST 的 API 路由 (API Routes with REST)](https://github.com/vercel/next.js/tree/canary/examples/api-routes-rest)
- [使用 CORS 的 API 路由 (API Routes with CORS)](https://github.com/vercel/next.js/tree/canary/examples/api-routes-cors)
...
> **小知识**：
>
> - API 路由 [不指定 CORS 请求头 (CORS headers)](https://developer.mozilla.org/docs/Web/HTTP/CORS)，这意味着它们默认**仅限同源 (same-origin only)**。您可以通过使用 [CORS 请求助手 (CORS request helpers)](https://github.com/vercel/next.js/tree/canary/examples/api-routes-cors) 包装请求处理程序来定制这种行为。

- API 路由无法与 [静态导出 (static exports)](/docs/pages/guides/static-exports) 一起使用。但是，应用路由 (App Router) 中的 [路由处理器 (Route Handlers)](/docs/app/api-reference/file-conventions/route) 可以。
  > - API 路由将受到 `next.config.js` 中 [`pageExtensions` 配置](/docs/pages/api-reference/config/next-config-js/pageExtensions) 的影响。
