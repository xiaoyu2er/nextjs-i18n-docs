...
| [`public`](/docs/app/building-your-application/optimizing/static-assets) | 配信される静的アセット             |
...
| [`instrumentation.ts`](/docs/app/building-your-application/optimizing/instrumentation)      | OpenTelemetryとインストルメンテーションファイル |
...
以下のファイル命名規則は、[`app` ルーター](/docs/app)でルートを定義し、メタデータを処理するために使用されます。
...
以下のファイル命名規則は、[`pages` ルーター](/docs/pages)でルートを定義するために使用されます。
