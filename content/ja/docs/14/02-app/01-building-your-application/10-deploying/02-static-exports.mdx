...
[`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props)と[`getStaticPaths`](/docs/pages/building-your-application/data-fetching/get-static-paths)を使用して、`pages`ディレクトリ内の各ページ（または[ダイナミックルート](/docs/app/building-your-application/routing/dynamic-routes)の場合はさらに多くのページ）のHTMLファイルを生成できます。
...
- [`getStaticPaths`使用時のダイナミックルート](/docs/app/building-your-application/routing/dynamic-routes)
- `next/link`によるプリフェッチ
- JavaScriptのプリロード
...
`next/image`による[画像最適化](/docs/app/building-your-application/optimizing/images)は、`next.config.js`でカスタム画像ローダーを定義することで静的エクスポートと併用できます。例えば、Cloudinaryのようなサービスで画像を最適化できます:
...
- `dynamicParams: true`の[ダイナミックルート](/docs/app/building-your-application/routing/dynamic-routes)
- `generateStaticParams()`なしの[ダイナミックルート](/docs/app/building-your-application/routing/dynamic-routes)
