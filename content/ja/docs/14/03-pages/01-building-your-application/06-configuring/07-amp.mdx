...
ページでAMPサポートを有効にする方法や、さまざまなAMP設定について詳しく知るには、[`next/amp`のAPIドキュメント](/docs/pages/building-your-application/configuring/amp)を参照してください。

## 注意点

- CSS-in-JSのみサポートされています。[CSS Modules](/docs/pages/building-your-application/styling)は現時点ではAMPページでサポートされていません。[Next.jsへのCSS Modulesサポートの貢献](https://github.com/vercel/next.js/issues/10549)も可能です。
...
[静的HTMLエクスポート](/docs/pages/building-your-application/deploying/static-exports)時にもページが検証され、警告やエラーがターミナルに出力されます。AMPエラーがあると、エクスポートが有効なAMPではないため、エクスポートはステータスコード`1`で終了します。
...
[`trailingSlash`](/docs/pages/api-reference/next-config-js/trailingSlash)が有効な場合、`pages/about.js`のエクスポートページは以下のようになります：
