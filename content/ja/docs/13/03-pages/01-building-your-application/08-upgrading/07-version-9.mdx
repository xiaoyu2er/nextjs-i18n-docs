...
translation-updated-at: 2025-06-06T17:06:59.799Z
...
以前に動的ルートのために`vercel.json`ファイルで`routes`を設定していた場合、Next.js 9の新しい[動的ルーティング機能](/docs/pages/building-your-application/routing/dynamic-routes)を利用することでこれらのルールを削除できます。
...
[`fork-ts-checker-webpack-plugin`](https://github.com/Realytics/fork-ts-checker-webpack-plugin/issues)の使用も`next.config.js`から削除してください。

TypeScript定義は`next`パッケージとともに公開されるようになったため、競合を避けるために`@types/next`をアンインストールする必要があります。
...
動的コンポーネントは、デフォルトではロード中に何も表示しなくなりました。`loading`プロパティを設定することでこの動作をカスタマイズできます:
...
### `withAmp`が削除され、設定オブジェクトのエクスポートに置き換えられました
...
以前は`pages/about.js`をエクスポートすると`out/about/index.html`が生成されましたが、この動作は`out/about.html`を生成するように変更されました。
...
この動作に依存しているコードを更新するのは比較的簡単です！移行を支援するために、変更前後の例を提供します:
