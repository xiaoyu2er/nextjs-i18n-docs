...
translation-updated-at: 2025-06-06T17:34:49.630Z
...
> **Bon à savoir** :
>
> - `useSearchParams` est un hook de [Composant Client](/docs/app/building-your-application/rendering/client-components) et **n'est pas supporté** dans les [Composants Serveur](/docs/app/building-your-application/rendering/server-components) pour éviter des valeurs obsolètes lors du [rendu partiel](/docs/app/building-your-application/routing/linking-and-navigating#3-partial-rendering).
> - Si une application inclut le répertoire `/pages`, `useSearchParams` retournera `ReadonlyURLSearchParams | null`. La valeur `null` est pour la compatibilité lors de la migration car les paramètres de recherche ne peuvent pas être connus pendant le pré-rendu d'une page qui n'utilise pas `getServerSideProps`.

## Comportement

### Rendu statique

Si une route est [rendue statiquement](/docs/app/building-your-application/rendering/server-components#static-rendering-default), l'appel à `useSearchParams()` entraînera le rendu côté client de l'arborescence jusqu'à la limite [`Suspense`](/docs/app/building-your-application/routing/loading-ui-and-streaming#example) la plus proche.

Cela permet qu'une partie de la page soit rendue statiquement tandis que la partie dynamique utilisant `searchParams` est rendue côté client.

Vous pouvez réduire la portion de la route rendue côté client en encapsulant le composant utilisant `useSearchParams` dans une limite `Suspense`. Par exemple :
...
Si une route est [rendue dynamiquement](/docs/app/building-your-application/rendering/server-components#dynamic-rendering), `useSearchParams` sera disponible sur le serveur pendant le rendu serveur initial du Composant Client.

> **Bon à savoir** : Définir l'option [`dynamic` de configuration des segments de route](/docs/app/api-reference/file-conventions/route-segment-config#dynamic) à `force-dynamic` peut être utilisé pour forcer un rendu dynamique.
