...
translation-updated-at: 2025-06-06T17:40:32.350Z
title: MDX
...
Créez un fichier `mdx-components.tsx` à la racine de votre application (le dossier parent de `app/` ou `src/`) :
...
Si vos fichiers ou contenu markdown/MDX se trouvent _ailleurs_, vous pouvez les récupérer dynamiquement sur le serveur. Ceci est utile pour le contenu stocké dans un dossier local séparé, un CMS, une base de données ou tout autre endroit.
...
export default async function PageMdxDistante() {
...
```tsx filename="components/mise-en-page-mdx.tsx" switcher
export default function MiseEnPageMdx({ children }: { children: React.ReactNode }) {
  // Créez ici toute mise en page ou style partagé
  return <div style={{ color: 'blue' }}>{children}</div>
}
...
```jsx filename="components/mise-en-page-mdx.js" switcher
export default function MiseEnPageMdx({ children }) {
  // Créez ici toute mise en page ou style partagé
  return <div style={{ color: 'blue' }}>{children}</div>
}
...
L'écosystème `remark` et `rehype` contient des plugins pour [la coloration syntaxique](https://github.com/atomiks/rehype-pretty-code), [les liens vers les titres](https://github.com/rehypejs/rehype-autolink-headings), [la génération d'une table des matières](https://github.com/remarkjs/remark-toc), et plus encore.
