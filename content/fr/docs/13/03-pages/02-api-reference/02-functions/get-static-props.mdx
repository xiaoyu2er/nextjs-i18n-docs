...
translation-updated-at: 2025-06-06T17:25:18.116Z
...
Vous pouvez importer des modules dans la portée de niveau supérieur pour les utiliser dans `getStaticProps`. Les imports utilisés **ne seront pas regroupés pour le côté client**. Cela signifie que vous pouvez écrire **du code côté serveur directement dans `getStaticProps`**, y compris la récupération de données depuis votre base de données.
...
Comme Next.js compile votre code dans un répertoire séparé, vous ne pouvez pas utiliser `__dirname` car le chemin qu'il retourne sera différent du routeur Pages.
...
| `v12.2.0` | [Régénération Statique Incrémentielle à la Demande](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) est stable.    |
| `v12.1.0` | [Régénération Statique Incrémentielle à la Demande](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) ajoutée (bêta). |
