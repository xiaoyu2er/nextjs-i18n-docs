...
translation-updated-at: 2025-06-06T17:27:03.565Z
title: useRouter
...
- `basePath` : `String` - Le [basePath](/docs/app/api-reference/next-config-js/basePath) actif (si activé).
...
- `isPreview` : `boolean` - Indique si l'application est actuellement en [mode prévisualisation](/docs/pages/building-your-application/configuring/preview-mode).

> L'utilisation du champ `asPath` peut entraîner une incohérence entre le client et le serveur si la page est rendue en utilisant le rendu côté serveur ou [l'optimisation statique automatique](/docs/pages/building-your-application/rendering/automatic-static-optimization). Évitez d'utiliser `asPath` tant que le champ `isReady` n'est pas `true`.
...
- [Utilisation du Routeur](https://github.com/vercel/next.js/tree/canary/examples/using-router)

</details>

Gère les transitions côté client. Cette méthode est utile pour les cas où [`next/link`](/docs/pages/api-reference/components/link) ne suffit pas.
...
Redirection de l'utilisateur vers `pages/login.js`, utile pour les pages derrière une [authentification](/docs/pages/building-your-application/routing/authenticating) :
...
- Utiliser une `key` React pour [indiquer à React de remonter le composant](https://react.dev/learn/rendering-lists#keeping-list-items-in-order-with-key). Pour le faire pour toutes les pages, vous pouvez utiliser une application personnalisée :
...
Vous pouvez utiliser un objet URL de la même manière que pour [`next/link`](/docs/pages/api-reference/components/link#with-url-object). Fonctionne pour les paramètres `url` et `as` :
...
Similaire à la prop `replace` dans [`next/link`](/docs/pages/api-reference/components/link), `router.replace` empêchera l'ajout d'une nouvelle entrée d'URL dans la pile `history`.

```js
router.replace(url, as, options)
```

- L'API pour `router.replace` est exactement la même que pour [`router.push`](#routerpush).
...
Dans certains cas (par exemple, si vous utilisez un [serveur personnalisé](/docs/pages/building-your-application/configuring/custom-server)), vous pouvez souhaiter écouter les événements [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) et faire quelque chose avant que le routeur n'agisse dessus.
...
Si `cb` retourne `false`, le routeur Next.js ne gérera pas le `popstate`, et vous serez responsable de le gérer dans ce cas. Voir [Désactivation du routage par système de fichiers](/docs/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
- [Avec un indicateur de chargement de page](https://github.com/vercel/next.js/tree/canary/examples/with-loading)
...
> **Bon à savoir** : Ici, `url` est l'URL affichée dans le navigateur, incluant le [`basePath`](/docs/app/api-reference/next-config-js/basePath).
...
> Nous utilisons une [Application Personnalisée](/docs/pages/building-your-application/routing/custom-app) (`pages/_app.js`) pour cet exemple afin de nous abonner à l'événement car elle n'est pas démontée lors des navigations de page, mais vous pouvez vous abonner aux événements du routeur dans n'importe quel composant de votre application.
...
Si [`useRouter`](#router-object) ne convient pas à votre cas, `withRouter` peut aussi ajouter le même objet [`router`](#router-object) à n'importe quel composant.
