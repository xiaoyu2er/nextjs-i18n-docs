...
translation-updated-at: 2025-06-06T17:30:38.774Z
...
- Le composant `<Head />` utilisé dans `_document` n'est pas le même que [`next/head`](/docs/pages/api-reference/components/head). Le `<Head />` utilisé ici doit uniquement contenir le code `<head>` commun à toutes les pages. Pour tous les autres cas, comme les balises `<title>`, nous recommandons d'utiliser [`next/head`](/docs/pages/api-reference/components/head) dans vos pages ou composants.
