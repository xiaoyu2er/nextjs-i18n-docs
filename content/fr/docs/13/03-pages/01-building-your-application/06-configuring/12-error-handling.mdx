...
translation-updated-at: 2025-06-06T17:26:51.471Z
title: Gestion des erreurs
description: Gestion des erreurs dans votre application Next.js.
---
...
Les [Error Boundaries (limites d'erreur)](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary) de React offrent une manière élégante de gérer les erreurs JavaScript côté client afin que les autres parties de l'application continuent à fonctionner. En plus d'éviter le plantage de la page, cela vous permet de fournir un composant de repli personnalisé et même de journaliser les informations d'erreur.
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    // Vous pouvez utiliser votre propre service de journalisation des erreurs ici
...
Le composant `ErrorBoundary` suit un état `hasError`. La valeur de cette variable d'état est un booléen. Lorsque `hasError` est `true`, le composant `ErrorBoundary` affichera une interface de repli. Sinon, il affichera les composants enfants.
...
Pour surveiller les erreurs client, utilisez un service comme [Sentry](https://github.com/vercel/next.js/tree/canary/examples/with-sentry), Bugsnag ou Datadog.