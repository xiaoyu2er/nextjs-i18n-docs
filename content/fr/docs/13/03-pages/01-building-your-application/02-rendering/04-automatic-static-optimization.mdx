...
translation-updated-at: 2025-06-06T17:29:14.712Z
...
Cette fonctionnalité permet à Next.js de produire des applications hybrides qui contiennent **à la fois des pages rendues côté serveur et des pages générées statiquement**.
...
- Des [réécritures (rewrites)](/docs/pages/api-reference/next-config-js/rewrites) sont configurées dans votre `next.config.js` car elles peuvent avoir des paramètres qui doivent être analysés et fournis dans la `query`.

Pour pouvoir déterminer si la `query` est entièrement mise à jour et prête à l'emploi, vous pouvez utiliser le champ `isReady` de [`next/router`](/docs/pages/api-reference/functions/use-router#router-object).
...
- Si vous avez une [application personnalisée (custom `App`)](/docs/pages/building-your-application/routing/custom-app) avec `getInitialProps`, alors cette optimisation sera désactivée pour les pages sans [Génération statique (Static Generation)](/docs/pages/building-your-application/data-fetching/get-static-props).
