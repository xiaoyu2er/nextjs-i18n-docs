...
translation-updated-at: 2025-06-06T16:52:46.491Z
...
Bienvenue dans le Guide de contribution à la documentation Next.js ! Nous sommes ravis de vous accueillir.

Cette page fournit des instructions sur la manière de modifier la documentation Next.js. Notre objectif est de permettre à chaque membre de la communauté de contribuer et d'améliorer nos documents.

## Pourquoi contribuer ?

Le travail open-source n'est jamais terminé, et la documentation non plus. Contribuer à la documentation est une excellente façon pour les débutants de s'impliquer dans l'open-source et pour les développeurs expérimentés de clarifier des sujets complexes tout en partageant leurs connaissances avec la communauté.

En contribuant à la documentation Next.js, vous nous aidez à construire une ressource d'apprentissage plus robuste pour tous les développeurs. Que vous ayez trouvé une faute de frappe, une section confuse ou que vous ayez remarqué l'absence d'un sujet particulier, vos contributions sont les bienvenues et appréciées.

## Comment contribuer

Le contenu de la documentation se trouve dans le [dépôt Next.js](https://github.com/vercel/next.js/tree/canary/docs). Pour contribuer, vous pouvez modifier les fichiers directement sur GitHub ou cloner le dépôt et modifier les fichiers localement.

### Workflow GitHub

Si vous débutez avec GitHub, nous vous recommandons de lire le [Guide Open Source de GitHub](https://opensource.guide/how-to-contribute/#opening-a-pull-request) pour apprendre comment forker un dépôt, créer une branche et soumettre une pull request.

> **Bon à savoir** : Le code sous-jacent de la documentation se trouve dans une base de code privée qui est synchronisée avec le dépôt public Next.js. Cela signifie que vous ne pouvez pas prévisualiser la documentation localement. Cependant, vous verrez vos modifications sur [nextjs.org](https://nextjs.org/docs) après la fusion d'une pull request.

### Écrire en MDX

La documentation est écrite en [MDX](https://mdxjs.com/), un format markdown qui prend en charge la syntaxe JSX. Cela nous permet d'intégrer des composants React dans la documentation. Consultez le [Guide Markdown de GitHub](https://docs.github.com/fr/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax) pour un aperçu rapide de la syntaxe markdown.
...
- [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) : Formater les fichiers MDX lors de l'enregistrement.
...
N'hésitez pas à nous faire savoir si vous avez des questions ou besoin d'une assistance supplémentaire dans les commentaires de votre PR. Merci de contribuer à la documentation Next.js et de faire partie de notre communauté !

> **Astuce** : Exécutez `pnpm prettier-fix` pour lancer Prettier avant de soumettre votre PR.

## Structure des fichiers

La documentation utilise le **routage par système de fichiers**. Chaque dossier et fichier à l'intérieur de [`/docs`](https://github.com/vercel/next.js/tree/canary/docs) représente un segment de route. Ces segments sont utilisés pour générer les chemins d'URL, la navigation et les fil d'Ariane.

La structure des fichiers reflète la navigation que vous voyez sur le site, et par défaut, les éléments de navigation sont triés par ordre alphabétique. Cependant, nous pouvons changer l'ordre des éléments en ajoutant un nombre à deux chiffres (`00-`) au nom du dossier ou du fichier.

Par exemple, dans la [référence API des fonctions](/docs/app/api-reference/functions), les pages sont triées par ordre alphabétique car cela facilite la recherche d'une fonction spécifique pour les développeurs :
...
Mais, dans la [section routage](/docs/app/building-your-application/routing), les fichiers sont préfixés par un nombre à deux chiffres, triés dans l'ordre où les développeurs devraient apprendre ces concepts :
...
| `source`    | Récupère le contenu dans une page partagée. Voir [Pages partagées](#pages-partagées).                                                              |
| `related`   | Une liste de pages connexes en bas du document. Celles-ci seront automatiquement transformées en cartes. Voir [Liens connexes](#liens-connexes).   |
...
Étant donné que la plupart des fonctionnalités du **Routeur App** et du **Routeur Pages** sont complètement différentes, leur documentation est conservée dans des sections distinctes (`02-app` et `03-pages`). Cependant, il existe quelques fonctionnalités qui sont partagées entre eux.

### Pages partagées

Pour éviter la duplication de contenu et le risque que le contenu devienne désynchronisé, nous utilisons le champ `source` pour récupérer le contenu d'une page dans une autre. Par exemple, le composant `<Link>` se comporte _principalement_ de la même manière dans **App** et **Pages**. Au lieu de dupliquer le contenu, nous pouvons récupérer le contenu de `app/.../link.mdx` dans `pages/.../link.mdx` :
...
Nous pouvons donc modifier le contenu à un seul endroit et le voir reflété dans les deux sections.

### Contenu partagé

Dans les pages partagées, il peut parfois y avoir du contenu spécifique au **Routeur App** ou au **Routeur Pages**. Par exemple, le composant `<Link>` a une prop `shallow` qui n'est disponible que dans **Pages** mais pas dans **App**.
...
Vous utiliserez probablement ces composants pour des exemples et des blocs de code.
...
Toujours exécuter les exemples localement avant de les valider. Cela garantira que le code est à jour et fonctionnel.
...
La plupart des exemples dans la documentation sont écrits en `tsx` et `jsx`, et quelques-uns en `bash`. Cependant, vous pouvez utiliser n'importe quel langage pris en charge, voici la [liste complète](https://github.com/shikijs/shiki/blob/main/docs/languages.md#all-languages).
...
> **Bon à savoir** : Nous prévoyons de compiler automatiquement les extraits TypeScript en JavaScript à l'avenir. En attendant, vous pouvez utiliser [transform.tools](https://transform.tools/typescript-to-javascript).
...
> **Bon à savoir** :
>
> - Nous utilisons également ce format pour les notes multi-lignes.
...
- Les liens seront générés automatiquement pour les pages qui ont des pages enfants. Par exemple, la section [Optimisation](/docs/app/building-your-application/optimizing) a des liens vers toutes ses pages enfants.
...
Les diagrammes sont un excellent moyen d'expliquer des concepts complexes. Nous utilisons [Figma](https://www.figma.com/) pour créer des diagrammes, en suivant le guide de conception de Vercel.

Les diagrammes se trouvent actuellement dans un dossier `/public` dans notre site Next.js privé. Si vous souhaitez mettre à jour ou ajouter un diagramme, veuillez ouvrir une [issue GitHub](https://github.com/vercel/next.js/issues/new?assignees=&labels=template%3A+documentation&projects=&template=4.docs_request.yml&title=Docs%3A+) avec vos idées.
...
Cette section contient des directives pour rédiger de la documentation pour ceux qui débutent dans l'écriture technique.

### Modèles de page

Bien que nous n'ayons pas de modèle strict pour les pages, il existe des sections de page que vous verrez répétées dans la documentation :

- **Vue d'ensemble :** Le premier paragraphe d'une page doit indiquer à l'utilisateur ce qu'est la fonctionnalité et à quoi elle sert. Suivi d'un exemple minimal fonctionnel ou de sa référence API.
...
- **Tableaux API** : Les pages API doivent avoir un tableau de synthèse en bas de la page avec des liens vers les sections (si possible).
...
Voici quelques directives pour maintenir un style et un ton cohérents dans toute la documentation :
...
- Soyez attentif à l'utilisation du mot _ceci_. Il peut être ambigu et source de confusion, n'hésitez pas à répéter le sujet de la phrase si nécessaire.
...
- Évitez les mots négatifs comme _ne pas_, _impossible_, _ne pas vouloir_, etc. Cela peut décourager les lecteurs.
...
{/* À faire : Composant des derniers contributeurs */}
