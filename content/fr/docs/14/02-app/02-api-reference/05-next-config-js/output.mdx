...
> **Bon à savoir** :
>
> - `next.config.js` est lu pendant `next build` et sérialisé dans le fichier de sortie `server.js`. Si les options obsolètes [`serverRuntimeConfig` ou `publicRuntimeConfig`](/docs/pages/api-reference/next-config-js/runtime-configuration) sont utilisées, les valeurs seront spécifiques aux valeurs au moment de la compilation.
> - Si votre projet doit écouter un port ou un nom d'hôte spécifique, vous pouvez définir les variables d'environnement `PORT` ou `HOSTNAME` avant d'exécuter `server.js`. Par exemple, exécutez `PORT=8080 HOSTNAME=0.0.0.0 node server.js` pour démarrer le serveur sur `http://0.0.0.0:8080`.
