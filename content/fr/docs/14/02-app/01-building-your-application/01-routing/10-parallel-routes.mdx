...
translation-updated-at: 2025-06-06T17:22:19.331Z
...
Cependant, les emplacements ne sont **pas** des [segments de route](/docs/app/building-your-application/routing#route-segments) et n'affectent pas la structure de l'URL. Par exemple, pour `/@analytics/views`, l'URL sera `/views` car `@analytics` est un emplacement.
...
Par défaut, Next.js suit l'état _actif_ (ou la sous-page) pour chaque emplacement. Cependant, le contenu affiché dans un emplacement dépendra du type de navigation :

- [**Navigation douce**](/docs/app/building-your-application/routing/linking-and-navigating#5-soft-navigation) : Lors de la navigation côté client, Next.js effectuera un [rendu partiel](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering), changeant la sous-page dans l'emplacement, tout en conservant les sous-pages actives des autres emplacements, même si elles ne correspondent pas à l'URL actuelle.
...
Les fonctions [`useSelectedLayoutSegment`](/docs/app/api-reference/functions/use-selected-layout-segment) et [`useSelectedLayoutSegments`](/docs/app/api-reference/functions/use-selected-layout-segments) acceptent un paramètre `parallelRoutesKey`, qui vous permet de lire le segment de route actif dans un emplacement.
...
Dans `@analytics`, créez un fichier [`layout`](/docs/app/building-your-application/routing/pages-and-layouts) pour partager les onglets entre les deux pages :
...
Les routes parallèles peuvent être utilisées avec les [routes d'interception](/docs/app/building-your-application/routing/intercepting-routes) pour créer des modales. Cela vous permet de résoudre des défis courants lors de la création de modales, tels que :
...
> **Bon à savoir :**
>
> - La convention utilisée pour intercepter la route, par exemple `(.)`, dépend de votre structure de fichiers. Voir [Convention des routes d'interception](/docs/app/building-your-application/routing/intercepting-routes#convention).
> - En séparant la fonctionnalité `<Modal>` du contenu de la modale (`<Login>`), vous pouvez vous assurer que tout contenu à l'intérieur de la modale, comme les [formulaires](/docs/app/building-your-application/data-fetching/server-actions-and-mutations#forms), sont des composants serveur. Voir [Entrelacement des composants client et serveur](/docs/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props) pour plus d'informations.
...
Voir la documentation [UI de chargement](/docs/app/building-your-application/routing/loading-ui-and-streaming) et [Gestion des erreurs](/docs/app/building-your-application/routing/error-handling) pour plus d'informations.