...
translation-updated-at: 2025-06-06T17:21:34.940Z
...
En dehors des [conventions de dossiers et fichiers pour le routage](/docs/getting-started/project-structure#app-routing-conventions), Next.js est **non-opinionated** concernant la manière d'organiser et de colocaliser les fichiers de votre projet.
...
- [Colocalisation sécurisée par défaut](#safe-colocation-by-default)
- [Fonctionnalités d'organisation de projet](#project-organization-features)
- [Stratégies d'organisation de projet](#project-organization-strategies)

## Colocalisation sécurisée par défaut

Dans le répertoire `app`, [la hiérarchie des dossiers imbriqués](/docs/app/building-your-application/routing#route-segments) définit la structure des routes.
...
Et, même lorsqu'une route est rendue accessible publiquement, seul le **contenu retourné** par `page.js` ou `route.js` est envoyé au client.
...
Cela signifie que les **fichiers de projet** peuvent être **colocalisés en toute sécurité** dans les segments de route du répertoire `app` sans devenir accidentellement accessibles.

<Image
  alt="Un diagramme montrant que les fichiers colocalisés ne sont pas accessibles même lorsqu'un segment contient un fichier page.js ou route.js."
...
> **Bon à savoir** :
>
> - Ceci est différent du répertoire `pages`, où tout fichier dans `pages` est considéré comme une route.
> - Bien que vous **puissiez** colocaliser vos fichiers de projet dans `app`, vous n'êtes **pas obligé**. Si vous préférez, vous pouvez [les conserver en dehors du répertoire `app`](#store-project-files-outside-of-app).
...
Comme les fichiers du répertoire `app` peuvent être [colocalisés en toute sécurité par défaut](#safe-colocation-by-default), les dossiers privés ne sont pas nécessaires pour la colocalisation. Cependant, ils peuvent être utiles pour :
...
> - Si vous n'utilisez pas de dossiers privés, il serait utile de connaître les [conventions de fichiers spéciales](/docs/getting-started/project-structure#routing-files) de Next.js pour éviter des conflits de noms inattendus.
...
<Image
  alt="Un exemple de structure de dossiers avec les fichiers de projet en dehors de app"
...
<Image
  alt="Un exemple de structure de dossiers avec les fichiers de projet à l'intérieur de app"
...
<Image
  alt="Un exemple de structure de dossiers avec les fichiers de projet divisés par fonctionnalité ou route"
