...
Dans la [documentation sur les Pages](/docs/pages/building-your-application/routing/pages-and-layouts) et la [documentation sur la Récupération de Données](/docs/pages/building-your-application/data-fetching), nous avons expliqué comment pré-rendre une page au moment du build (**Génération Statique**) en utilisant `getStaticProps` et `getStaticPaths`.
...
Ensuite, envoyez une requête à `/api/clear-preview-mode-cookies` pour invoquer la Route API. Si vous appelez cette route en utilisant [`next/link`](/docs/pages/api-reference/components/link), vous devez passer `prefetch={false}` pour éviter d'appeler `clearPreviewData` pendant le préchargement des liens.
...
> **Bon à savoir** : Vous ne devriez pas définir l'en-tête `Cache-Control` lors de l'utilisation du Mode Prévisualisation car il ne peut pas être contourné. Nous recommandons plutôt d'utiliser [ISR](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration).
