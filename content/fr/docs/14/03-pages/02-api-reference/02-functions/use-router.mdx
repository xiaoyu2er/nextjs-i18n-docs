...
- `isFallback`: `boolean` - Indique si la page actuelle est en [mode de secours (fallback)](/docs/pages/api-reference/functions/get-static-paths#fallback-true).
...
> L'utilisation du champ `asPath` peut entraîner une incohérence entre le client et le serveur si la page est rendue côté serveur ou avec [l'optimisation statique automatique](/docs/pages/building-your-application/rendering/automatic-static-optimization). Évitez d'utiliser `asPath` tant que le champ `isReady` n'est pas `true`.
...
Redirection de l'utilisateur vers `pages/login.js`, utile pour les pages protégées par [authentification](/docs/pages/building-your-application/authentication) :
...
Similaire à la prop `replace` de [`next/link`](/docs/pages/api-reference/components/link), `router.replace` empêche l'ajout d'une nouvelle entrée d'URL dans la pile d'historique.
...
Dans certains cas (par exemple, si vous utilisez un [serveur personnalisé](/docs/pages/building-your-application/configuring/custom-server)), vous pouvez souhaiter écouter les événements [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) et faire quelque chose avant que le routeur n'agisse.
