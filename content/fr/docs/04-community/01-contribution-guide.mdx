...
translation-updated-at: 2025-06-02T19:59:11.109Z
...
Le contenu de la documentation se trouve dans le [dépôt Next.js](https://github.com/vercel/next.js/tree/canary/docs). Pour contribuer, vous pouvez modifier les fichiers directement sur GitHub ou cloner le dépôt et éditer les fichiers localement.

### Workflow GitHub

Si vous êtes nouveau sur GitHub, nous vous recommandons de lire le [Guide Open Source de GitHub](https://opensource.guide/how-to-contribute/#opening-a-pull-request) pour apprendre comment forker un dépôt, créer une branche et soumettre une pull request.
...
- [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) : Formater les fichiers MDX lors de la sauvegarde.
...
Par exemple, dans la [référence API des fonctions](/docs/app/api-reference/functions), les pages sont triées par ordre alphabétique car cela facilite la recherche d'une fonction spécifique par les développeurs :
...
Mais, dans la [section routage](/docs/app/building-your-application/routing), les fichiers sont préfixés par un nombre à deux chiffres, triés dans l'ordre dans lequel les développeurs devraient apprendre ces concepts :
...
| `source`    | Récupère le contenu dans une page partagée. Voir [Pages partagées](#shared-pages).                                                                 |
| `related`   | Une liste de pages connexes en bas du document. Elles seront automatiquement transformées en cartes. Voir [Liens connexes](#related-links).         |
...
Les diagrammes se trouvent actuellement dans le dossier `/public` de notre site Next.js privé. Si vous souhaitez mettre à jour ou ajouter un diagramme, veuillez ouvrir une [issue GitHub](https://github.com/vercel/next.js/issues/new?assignees=&labels=template%3A+documentation&projects=&template=4.docs_request.yml&title=Docs%3A+) avec vos idées.
...
- Évitez les mots comme _facile_, _rapide_, _simple_, _juste_, etc. Ces termes sont subjectifs et peuvent décourager les utilisateurs.
...
Bien que ces directives ne soient pas exhaustives, elles devraient vous aider à démarrer. Si vous souhaitez approfondir vos connaissances en rédaction technique, consultez le [Cours de rédaction technique de Google](https://developers.google.com/tech-writing/overview).
