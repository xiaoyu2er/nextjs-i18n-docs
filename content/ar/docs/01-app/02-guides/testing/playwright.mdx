...
translation-updated-at: 2025-06-02T20:11:18.864Z
title: كيفية إعداد Playwright مع Next.js
nav_title: Playwright
description: تعلم كيفية إعداد Playwright مع Next.js لاختبارات End-to-End (E2E).
---
...
أسرع طريقة للبدء هي استخدام `create-next-app` مع [مثال with-playwright](https://github.com/vercel/next.js/tree/canary/examples/with-playwright). سيؤدي هذا إلى إنشاء مشروع Next.js مكتمل مع تكوين Playwright.
...
سيأخذك هذا عبر سلسلة من الخطوات لإعداد وتكوين Playwright لمشروعك، بما في ذلك إضافة ملف `playwright.config.ts`. يُرجى الرجوع إلى [دليل تثبيت Playwright](https://playwright.dev/docs/intro#installation) للحصول على الدليل خطوة بخطوة.
...
> **معلومة مفيدة**: يمكنك استخدام `page.goto("/")` بدلاً من `page.goto("http://localhost:3000/")`، إذا أضفت [`"baseURL": "http://localhost:3000"`](https://playwright.dev/docs/api/class-testoptions#test-options-base-url) إلى ملف التكوين `playwright.config.ts` [ملف التكوين](https://playwright.dev/docs/test-configuration).
