...
translation-updated-at: 2025-06-08T22:16:10.703Z
title: هيكل المشروع وتنظيمه
...
|                                                                                                        |                                  |
| ------------------------------------------------------------------------------------------------------ | -------------------------------- |
| [`[folder]`](/docs/app/api-reference/file-conventions/dynamic-routes#convention)                       | مقطع طريق ديناميكي               |
| [`[...folder]`](/docs/app/api-reference/file-conventions/dynamic-routes#catch-all-segments)            | مقطع طريق شامل (Catch-all)       |
| [`[[...folder]]`](/docs/app/api-reference/file-conventions/dynamic-routes#optional-catch-all-segments) | مقطع طريق شامل اختياري           |
...
|                                                                                             |                            |
| ------------------------------------------------------------------------------------------- | -------------------------- |
| [`@folder`](/docs/app/api-reference/file-conventions/parallel-routes#slots)                 | فتحة مسماة (Named slot)    |
...
| [`[...folder]/index`](/docs/pages/building-your-application/routing/dynamic-routes#catch-all-segments)            | `.js` `.jsx` `.tsx` | مقطع طريق شامل (Catch-all)       |
...
| [`[...file]`](/docs/pages/building-your-application/routing/dynamic-routes#catch-all-segments)                    | `.js` `.jsx` `.tsx` | مقطع طريق شامل (Catch-all)       |
...
<Image
  alt="مخطط يوضح أن ملفات المشروع المنسقة مكانيًا ليست قابلة للتوجيه حتى عندما يحتوي الجزء على ملف page.js أو route.js."
...
<Image
  alt="مثال على هيكل مجلدات باستخدام المجلدات الخاصة"
...
بما أن الملفات في دليل `app` يمكن [تنسيقها بأمان افتراضيًا](#colocation)، فإن المجلدات الخاصة ليست مطلوبة للتنسيق المكاني. ومع ذلك، يمكن أن تكون مفيدة لـ:
...
> - إذا كنت لا تستخدم المجلدات الخاصة، سيكون من المفهم معرفة [اصطلاحات الملفات الخاصة بـ Next.js](/docs/app/getting-started/project-structure#routing-files) لتجنب تعارضات التسمية غير المتوقعة.
...
يدعم Next.js تخزين كود التطبيق (بما في ذلك `app`) داخل مجلد [`src` اختياري](/docs/app/api-reference/file-conventions/src-folder). هذا يفصل كود التطبيق عن ملفات تكوين المشروع التي تعيش غالبًا في جذر المشروع.
...
لتطبيق [هيكل تحميل](/docs/app/api-reference/file-conventions/loading) عبر ملف `loading.js` على مسار معين، أنشئ مجموعة مسار جديدة (مثل `/(overview)`) ثم انقل ملف `loading.tsx` داخل مجموعة المسار هذه.

<Image
  alt="هيكل مجلدات يوضح ملف loading.tsx و page.tsx داخل مجموعة المسار"
...
لإنشاء عدة [تخطيطات جذر](/docs/app/api-reference/file-conventions/layout#root-layout)، احذف ملف `layout.js` الأعلى مستوى، وأضف ملف `layout.js` داخل كل مجموعة مسار. هذا مفيد لتقسيم التطبيق إلى أقسام لها واجهة مستخدم أو تجربة مختلفة تمامًا. يجب إضافة العلامات `<html>` و `<body>` إلى كل تخطيط جذر.
