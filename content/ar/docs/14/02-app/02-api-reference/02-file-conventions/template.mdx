...
ملف **القالب (template)** يشبه [التخطيط (layout)](/docs/app/building-your-application/routing/pages-and-layouts#layouts) من حيث أنه يغلف كل تخطيط فرعي أو صفحة. ولكن على عكس التخطيطات التي تظل ثابتة عبر المسارات وتحافظ على الحالة، فإن القوالب تنشئ نسخة جديدة لكل عنصر فرعي عند التنقل.
...
> **معلومة مفيدة**:
>
> - افتراضيًا، `template` هو [مكون خادم (Server Component)](/docs/app/building-your-application/rendering/server-components)، ولكن يمكن أيضًا استخدامه كـ [مكون عميل (Client Component)](/docs/app/building-your-application/rendering/client-components) عبر توجيه `"use client"`.
