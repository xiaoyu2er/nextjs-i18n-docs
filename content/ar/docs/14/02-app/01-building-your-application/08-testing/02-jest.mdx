...
> **معلومة مفيدة:** لاختبار متغيرات البيئة مباشرة، قم بتحميلها يدويًا في سيناريو إعداد منفصل أو في ملف `jest.config.ts`. لمزيد من المعلومات، يرجى الاطلاع على [متغيرات بيئة الاختبار](/docs/pages/building-your-application/configuring/environment-variables#test-environment-variables).
...
إذا كان مشروعك يستخدم [أسماء مسار الوحدات (Module Path Aliases)](/docs/pages/building-your-application/configuring/absolute-imports-and-module-aliases)، فستحتاج إلى تكوين Jest لحل عمليات الاستيراد عن طريق مطابقة خيار paths في ملف `jsconfig.json` مع خيار `moduleNameMapper` في ملف `jest.config.js`. على سبيل المثال:
