...
| `previewData`      | (مهمل لصالح `draftMode`) بيانات [المعاينة](/docs/pages/building-your-application/configuring/preview-mode) المحددة بواسطة `setPreviewData`.                                                                                                                                                                                                                                                                                                          |
...
| `revalidateReason` | يقدم سببًا لاستدعاء الدالة. يمكن أن يكون أحد: "build" (يعمل أثناء البناء)، "stale" (انتهت فترة إعادة التحقق، أو يعمل في [وضع التطوير](/docs/pages/building-your-application/data-fetching/get-static-props#runs-on-every-request-in-development))، "on-demand" (يتم تشغيله عبر [إعادة التحقق عند الطلب](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)) |
...
القيمة المنطقية `notFound` تسمح للصفحة بإرجاع حالة `404` و[صفحة 404](/docs/pages/building-your-application/routing/custom-error#404-page). مع `notFound: true`، سترجع الصفحة `404` حتى لو كان هناك صفحة تم إنشاؤها بنجاح من قبل. هذا مخصص لدعم حالات الاستخدام مثل المحتوى الذي ينشئه المستخدم ويتم إزالته من قبل المؤلف. ملاحظة، `notFound` يتبع نفس سلوك `revalidate` [الموضح هنا](#revalidate).
...
> **جيد للمعرفة**: `notFound` غير مطلوب لوضع [`fallback: false`](/docs/pages/api-reference/functions/get-static-paths#fallback-false) حيث سيتم فقط عرض المسارات التي تم إرجاعها من `getStaticPaths` مسبقًا.
...
إذا كانت عمليات إعادة التوجيه معروفة أثناء البناء، فيجب إضافتها في [`next.config.js`](/docs/pages/api-reference/next-config-js/redirects) بدلاً من ذلك.
...
| الإصدار   | التغييرات                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/app/building-your-application/data-fetching) أصبح الآن مستقرًا مع جلب بيانات مبسط                                                           |
