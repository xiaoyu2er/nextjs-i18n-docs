...
translation-updated-at: 2025-06-02T20:06:16.177Z
title: كيفية الترقية إلى الإصدار 12
nav_title: الإصدار 12
...
[البرمجيات الوسيطة (Middleware)](/docs/messages/middleware-upgrade-guide) - إذا كنت تستخدم البرمجيات الوسيطة قبل الإصدار `12.2`، يرجى الاطلاع على [دليل الترقية](/docs/messages/middleware-upgrade-guide) لمزيد من المعلومات.
...
يستخدم Next.js الآن المترجم المعتمد على Rust [SWC](https://swc.rs/) لترجمة JavaScript/TypeScript. هذا المترجم الجديد أسرع بمقدار 17 مرة من Babel عند ترجمة الملفات الفردية وأسرع بمقدار 5 مرات في التحديث السريع (Fast Refresh).

يوفر Next.js توافقًا كاملاً مع الإصدارات السابقة للتطبيقات التي تحتوي على [تكوين Babel مخصص](/docs/pages/guides/babel). جميع التحويلات التي يتعامل معها Next.js افتراضيًا مثل styled-jsx وإزالة الأكواد غير المستخدمة (tree-shaking) لـ `getStaticProps` / `getStaticPaths` / `getServerSideProps` تم نقلها إلى Rust.
...
يعد تصغير الأكواد باستخدام SWC علامة اختيارية لضمان إمكانية اختبارها ضد المزيد من تطبيقات Next.js الواقعية قبل أن تصبح الافتراضية في Next.js 12.1. إذا كان لديك ملاحظات حول التصغير، يرجى مشاركتها في [موضوع الملاحظات هذا](https://github.com/vercel/next.js/discussions/30237).
...
راجع [الوثائق](/docs/pages/api-reference/components/image#styling-images) لمزيد من المعلومات.
...
اعتمد Next.js webpack 5 كافتراضي للترجمة في Next.js 11. كما تم الإبلاغ في [وثائق ترقية webpack 5](/docs/messages/webpack5) فإن Next.js 12 يزيل دعم webpack 4.

إذا كان تطبيقك لا يزال يستخدم webpack 4 باستخدام علامة الإلغاء، فسترى الآن خطأ يشير إلى [وثائق ترقية webpack 5](/docs/messages/webpack5).
...
إذا كنت تستخدم حاليًا خيار `target` مضبوطًا على `serverless`، يرجى قراءة [الوثائق حول كيفية الاستفادة من الإخراج الجديد](/docs/pages/api-reference/config/next-config-js/output).