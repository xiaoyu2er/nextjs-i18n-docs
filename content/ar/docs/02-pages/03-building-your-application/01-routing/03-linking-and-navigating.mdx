...
translation-updated-at: 2025-06-02T20:05:34.676Z
title: الربط والتنقل
...
أي `<Link />` في نطاق الرؤية (مبدئيًا أو عن طريق التمرير) سيتم جلبها مسبقًا افتراضيًا (بما في ذلك البيانات المقابلة) للصفحات التي تستخدم [التوليد الثابت (Static Generation)](/docs/pages/building-your-application/data-fetching/get-static-props). يتم جلب البيانات المقابلة لطرق [العرض من جانب الخادم (server-rendered)](/docs/pages/building-your-application/data-fetching/get-server-side-props) _فقط عند_ النقر على `<Link />`.

## الربط بمسارات ديناميكية

يمكنك أيضًا استخدام الاستيفاء لإنشاء المسار، وهو مفيد لـ [أجزاء المسار الديناميكية (dynamic route segments)](/docs/pages/building-your-application/routing/dynamic-routes). على سبيل المثال، لعرض قائمة بالمقالات التي تم تمريرها إلى المكون كخاصية:
...
يجب أن يكون [`next/link`](/docs/pages/api-reference/components/link) قادرًا على تغطية معظم احتياجاتك للتنقل، ولكن يمكنك أيضًا تنفيذ التنقل من جانب العميل بدونه، راجع [توثيق `next/router`](/docs/pages/api-reference/functions/use-router).

يوضح المثال التالي كيفية تنفيذ التنقل الأساسي بين الصفحات باستخدام [`useRouter`](/docs/pages/api-reference/functions/use-router):
...
- [التنقل السطحي](https://github.com/vercel/next.js/tree/canary/examples/with-shallow-routing)

</details>

يسمح لك التنقل السطحي بتغيير عنوان URL دون إعادة تشغيل طرق جلب البيانات مرة أخرى، وهذا يشمل [`getServerSideProps`](/docs/pages/building-your-application/data-fetching/get-server-side-props)، و[`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props)، و[`getInitialProps`](/docs/pages/api-reference/functions/get-initial-props).

ستتلقى `pathname` و `query` المحدثين عبر [كائن الموجه (`router`)](/docs/pages/api-reference/functions/use-router#router-object) (الذي يتم إضافته بواسطة [`useRouter`](/docs/pages/api-reference/functions/use-router) أو [`withRouter`](/docs/pages/api-reference/functions/use-router#withrouter))، دون فقدان الحالة.

لتمكين التنقل السطحي، اضبط خيار `shallow` على `true`. ضع في اعتبارك المثال التالي:
...
يعمل التنقل السطحي **فقط** لتغييرات عنوان URL في الصفحة الحالية. على سبيل المثال، لنفترض أن لدينا صفحة أخرى تسمى `pages/about.js`، وقمت بتنفيذ هذا:
