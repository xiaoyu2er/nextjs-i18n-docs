...
translation-updated-at: 2025-06-06T16:50:33.932Z
title: getStaticProps
description: Получение данных и генерация статических страниц с помощью `getStaticProps`. Узнайте больше об этом API для загрузки данных в Next.js.
---
...
> Обратите внимание, что независимо от типа рендеринга, любые `props` будут переданы в компонент страницы и могут быть просмотрены на стороне клиента в исходном HTML. Это необходимо для корректной [гидратации (hydration)](https://react.dev/reference/react-dom/hydrate) страницы. Убедитесь, что вы не передаете в `props` никакой конфиденциальной информации, которая не должна быть доступна на клиенте.
...
- `getStaticProps` вызывается перед первым рендером при использовании [`fallback: blocking`](/docs/pages/api-reference/functions/get-static-paths#fallback-blocking)
- `getStaticProps` выполняется в фоновом режиме при использовании `revalidate`
- `getStaticProps` выполняется по запросу в фоновом режиме при использовании [`revalidate()`](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)

При использовании [Инкрементальной статической регенерации (ISR)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration), `getStaticProps` будет выполняться в фоновом режиме, пока устаревшая страница проходит ревалидацию, а свежая страница отправляется в браузер.
...
Альтернативно, если вы **не** используете API routes для получения данных, можно использовать API [`fetch()`](https://developer.mozilla.org/docs/Web/API/Fetch_API) напрямую в `getStaticProps`.
...
> **Полезно знать**: если вы создали [кастомное приложение (custom app)](/docs/pages/building-your-application/routing/custom-app), убедитесь, что передаете `pageProps` в компонент страницы, как показано в документации, иначе пропсы будут пустыми.
...
Можно временно обойти статическую генерацию и рендерить страницу во **время запроса** вместо сборки, используя [**Режим предпросмотра (Preview Mode)**](/docs/pages/building-your-application/configuring/preview-mode). Например, если вы используете headless CMS и хотите просматривать черновики перед их публикацией.