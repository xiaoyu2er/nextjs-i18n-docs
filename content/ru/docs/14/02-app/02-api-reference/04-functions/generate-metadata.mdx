...
translation-updated-at: 2025-06-06T17:01:49.381Z
title: Объект Metadata и параметры generateMetadata
nav_title: generateMetadata
description: Узнайте, как добавить метаданные в приложение Next.js для улучшения поисковой оптимизации (SEO) и возможности публикации в интернете.
related:
  title: Дальнейшие шаги
  description: Просмотрите все параметры API Metadata.
  links:
...
Динамические метаданные, зависящие от **динамической информации**, такой как параметры текущего маршрута, внешние данные или `metadata` в родительских сегментах, могут быть заданы с помощью функции `generateMetadata`, которая возвращает объект [`Metadata`](#metadata-fields).
...
> - Запросы `fetch` автоматически [мемоизируются](/docs/app/building-your-application/caching#request-memoization) для одних и тех же данных в `generateMetadata`, `generateStaticParams`, Layouts, Pages и Server Components. React [`cache` можно использовать](/docs/app/building-your-application/caching#request-memoization), если `fetch` недоступен.
...
Атрибут `title` используется для установки заголовка документа. Он может быть определён как простая [строка](#string) или необязательный [шаблонный объект](#template-object).
...
| `<style>`                     | Подробнее о [стилизации в Next.js](/docs/app/building-your-application/styling/css-modules).                                                                                                                                                    |
| `<script>`                    | Подробнее о [использовании скриптов](/docs/app/building-your-application/optimizing/scripts).                                                                                                                                                          |
...
Вы можете добавить проверку типов к своим метаданным, используя тип `Metadata`. Если вы используете [встроенный плагин TypeScript](/docs/app/building-your-application/configuring/typescript) в своей IDE, вам не нужно вручную добавлять тип, но вы всё равно можете явно указать его при желании.
