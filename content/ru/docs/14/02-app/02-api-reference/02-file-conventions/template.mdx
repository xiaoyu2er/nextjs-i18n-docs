...
translation-updated-at: 2025-06-06T16:53:53.516Z
title: template.js
...
Компоненты шаблона должны принимать и использовать пропс `children`. `template` рендерится между [layout](/docs/app/api-reference/file-conventions/layout) и его дочерними элементами. Например:
...
> **Полезно знать**:
>
> - По умолчанию `template` является [серверным компонентом](/docs/app/building-your-application/rendering/server-components), но также может использоваться как [клиентский компонент](/docs/app/building-your-application/rendering/client-components) с помощью директивы `"use client"`.
