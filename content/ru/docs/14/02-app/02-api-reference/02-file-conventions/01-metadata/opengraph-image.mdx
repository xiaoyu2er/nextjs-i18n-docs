...
translation-updated-at: 2025-06-06T17:39:33.670Z
...
Добавьте сопровождающий файл `opengraph-image.alt.txt` в том же сегменте маршрута, что и изображение `opengraph-image.(jpg|jpeg|png|gif)`, чтобы указать альтернативный текст.
...
Добавьте сопровождающий файл `twitter-image.alt.txt` в том же сегменте маршрута, что и изображение `twitter-image.(jpg|jpeg|png|gif)`, чтобы указать альтернативный текст.
...
В дополнение к использованию [готовых файлов изображений](#image-files-jpg-png-gif), вы можете программно **генерировать** изображения с помощью кода.

Создайте изображение для сегмента маршрута, добавив файл `opengraph-image` или `twitter-image`, который экспортирует функцию по умолчанию.
...
export default async function Image() {
  ...
  return new ImageResponse(
    ...
    // Опции ImageResponse
    {
      // Для удобства можно повторно использовать экспортированные
      // настройки размера opengraph-image для установки ширины и высоты ImageResponse.
      ...size,
      fonts: [
        {
          name: 'Inter',
...
`opengraph-image` и `twitter-image` являются специализированными [обработчиками маршрутов](/docs/app/building-your-application/routing/route-handlers), которые могут использовать те же [настройки сегмента маршрута](/docs/app/api-reference/file-conventions/route-segment-config), что и страницы и макеты.
...
> **Полезно знать**:
> По умолчанию это сгенерированное изображение будет [статически оптимизировано](/docs/app/building-your-application/rendering/server-components#static-rendering-default). Вы можете настроить отдельные [`опции`](/docs/app/api-reference/functions/fetch) `fetch` или [настройки сегмента маршрута](/docs/app/api-reference/file-conventions/route-segment-config#revalidate), чтобы изменить это поведение.
