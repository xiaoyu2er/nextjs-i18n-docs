...
translation-updated-at: 2025-06-06T17:16:45.878Z
title: Отладка
description: Узнайте, как отлаживать ваше приложение Next.js с помощью VS Code или Chrome DevTools.
---

В этой документации объясняется, как можно отлаживать фронтенд и бэкенд код Next.js с полной поддержкой карт источников (source maps), используя либо [отладчик VS Code](https://code.visualstudio.com/docs/editor/debugging), либо [Chrome DevTools](https://developers.google.com/web/tools/chrome-devtools).

Любой отладчик, который может подключаться к Node.js, также может использоваться для отладки приложения Next.js. Подробнее об этом можно узнать в [Руководстве по отладке Node.js](https://nodejs.org/en/docs/guides/debugging-getting-started/).
...
`npm run dev` можно заменить на `yarn dev`, если вы используете Yarn. Если вы [изменяете номер порта](/docs/pages/api-reference/next-cli#development), на котором запускается ваше приложение, замените `3000` в `http://localhost:3000` на используемый вами порт.
...
> **Полезно знать**: Убедитесь, что Windows Defender отключен на вашем компьютере. Эта внешняя служба проверяет _каждый читаемый файл_, что, как сообщается, значительно увеличивает время Fast Refresh при использовании `next dev`. Это известная проблема, не связанная напрямую с Next.js, но она влияет на разработку в Next.js.
