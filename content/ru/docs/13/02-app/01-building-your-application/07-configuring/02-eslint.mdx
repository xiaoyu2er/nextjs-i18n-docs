...
translation-updated-at: 2025-06-06T17:31:49.549Z
title: ESLint
description: Next.js предоставляет встроенную поддержку ESLint по умолчанию. Эти правила помогают оптимально использовать Next.js.
---

{/* Содержание этого документа используется как для роутера app, так и для pages. Для добавления контента, специфичного для роутера Pages, используйте компонент `<PagesOnly>Контент</PagesOnly>`. Общий контент не должен быть обёрнут в компонент. */}

Next.js предоставляет встроенную поддержку [ESLint](https://eslint.org/) из коробки. Добавьте `next lint` как скрипт в `package.json`:
...
Next.js предоставляет плагин ESLint [`eslint-plugin-next`](https://www.npmjs.com/package/@next/eslint-plugin-next), уже включённый в базовую конфигурацию, который помогает выявлять распространённые проблемы в приложениях Next.js. Полный список правил:
...
| <Check size={18} /> | [@next/next/no-img-element](/docs/messages/no-img-element)                                                               | Запрещает использование элемента `<img>` из-за замедления LCP и увеличения трафика.                                         |
| <Check size={18} /> | [@next/next/no-page-custom-font](/docs/messages/no-page-custom-font)                                                     | Запрещает кастомные шрифты только для страниц.                                                                                  |
...
Если ESLint уже настроен в вашем приложении, рекомендуем расширять конфигурацию непосредственно из этого плагина, а не включать `eslint-config-next`, за исключением определённых случаев. Подробнее в разделе [Рекомендуемый набор правил плагина](#recommended-plugin-ruleset).
...
Для повышения производительности информация о проверенных ESLint файлах кэшируется по умолчанию. Кэш хранится в `.next/cache` или в указанной [директории сборки](/docs/app/api-reference/next-config-js/distDir). Если вы используете правила ESLint, зависящие от содержимого нескольких файлов, и нужно отключить кэш, используйте флаг `--no-cache` с `next lint`.

</AppOnly>

<PagesOnly>

Для повышения производительности информация о проверенных ESLint файлах кэшируется по умолчанию. Кэш хранится в `.next/cache` или в указанной [директории сборки](/docs/pages/api-reference/next-config-js/distDir). Если вы используете правила ESLint, зависящие от содержимого нескольких файлов, и нужно отключить кэш, используйте флаг `--no-cache` с `next lint`.
...
`next/core-web-vitals` обновляет `eslint-plugin-next`, преобразуя в ошибки некоторые правила, которые по умолчанию являются предупреждениями, если они влияют на [Core Web Vitals](https://web.dev/vitals/).

> Точка входа `next/core-web-vitals` автоматически включается для новых приложений, созданных с помощью [Create Next App](/docs/app/api-reference/create-next-app).

## Использование с другими инструментами

### Prettier

ESLint также содержит правила форматирования кода, которые могут конфликтовать с вашей текущей настройкой [Prettier](https://prettier.io/). Рекомендуем включить [eslint-config-prettier](https://github.com/prettier/eslint-config-prettier) в конфигурацию ESLint для совместной работы ESLint и Prettier.
...
Затем добавьте `prettier` в существующую конфигурацию ESLint:
...
- У вас уже установлен один или несколько следующих плагинов (отдельно или через другую конфигурацию, такую как `airbnb` или `react-app`):
  ...
  - `jsx-a11y`
  - `import`
- Вы определили специфические `parserOptions`, отличающиеся от конфигурации Babel в Next.js (это не рекомендуется, если только вы не [настроили Babel вручную](/docs/pages/building-your-application/configuring/babel))
...
Тогда рекомендуем либо удалить эти настройки, если вы предпочитаете конфигурацию из [`eslint-config-next`](https://github.com/vercel/next.js/blob/canary/packages/eslint-config-next/index.js), либо расширить конфигурацию непосредственно из плагина Next.js ESLint:
...
Это исключает риск конфликтов или ошибок из-за импорта одного и того же плагина или парсера в нескольких конфигурациях.
