...
translation-updated-at: 2025-06-02T20:03:53.817Z
title: getStaticPaths
description: Получение данных и генерация статических страниц с помощью `getStaticPaths`. Узнайте больше об этом API для загрузки данных в Next.js.
---
...
[Справочник по API `getStaticPaths`](/docs/pages/api-reference/functions/get-static-paths) охватывает все параметры и свойства, которые можно использовать с `getStaticPaths`.
...
- Страница должна быть предварительно отрендерена (для SEO) и быть очень быстрой — `getStaticProps` генерирует `HTML` и `JSON` файлы, которые могут кэшироваться CDN для повышения производительности

## Когда выполняется getStaticPaths

`getStaticPaths` выполняется только во время сборки в production и не вызывается во время выполнения. Вы можете проверить, что код внутри `getStaticPaths` удален из клиентского бандла, [с помощью этого инструмента](https://next-code-elimination.vercel.app/).

### Как выполняется getStaticProps относительно getStaticPaths

- `getStaticProps` выполняется во время `next build` для всех путей, возвращенных при сборке
...
- `getStaticPaths` **должен** использоваться вместе с `getStaticProps`
...
Вы можете отложить генерацию всех страниц по требованию, вернув пустой массив для `paths`. Это особенно полезно при развертывании Next.js приложения в нескольких средах. Например, можно ускорить сборки, генерируя все страницы по требованию для превью (но не для production сборок). Это полезно для сайтов с сотнями/тысячами статических страниц.

```jsx filename="pages/posts/[id].js"
export async function getStaticPaths() {
  // Когда это true (в средах превью), не
  // предварительно рендерить статические страницы
  // (быстрее сборки, но медленнее первоначальная загрузка)
  ...
  // (медленнее сборки, но быстрее первоначальная загрузка)
