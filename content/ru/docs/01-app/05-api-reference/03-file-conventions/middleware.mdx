...
translation-updated-at: 2025-06-02T20:08:45.524Z
title: middleware.js
description: API-справочник для файла middleware.js.
related:
...
Опционально можно экспортировать объект конфигурации вместе с функцией Middleware. Этот объект включает [matcher](#matcher) для указания путей, к которым применяется Middleware.
...
> **Полезно знать**:
>
> - `NextRequest` — это тип, представляющий входящие HTTP-запросы в Middleware Next.js, тогда как [`NextResponse`](#nextresponse) — это класс, используемый для манипуляции и отправки HTTP-ответов.
...
По умолчанию Middleware использует [Edge runtime](/docs/app/api-reference/edge). Если вы не хотите этого, вы можете использовать [полную среду выполнения Node.js](/blog/next-15-2#nodejs-middleware-experimental) для запуска Middleware.
