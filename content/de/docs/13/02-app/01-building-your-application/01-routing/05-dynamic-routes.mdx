...
translation-updated-at: 2025-06-06T18:10:29.630Z
...
Wenn Sie die genauen Segmentnamen im Voraus nicht kennen und Routen aus dynamischen Daten erstellen möchten, können Sie dynamische Segmente verwenden, die zur Laufzeit gefüllt oder [vorgerendert](#generating-static-params) werden.
...
> **Wissenswert**: Dynamische Segmente entsprechen [dynamischen Routen](/docs/app/building-your-application/routing/dynamic-routes) im `pages`-Verzeichnis.

## Statische Parameter generieren

Die Funktion `generateStaticParams` kann in Kombination mit [dynamischen Routensegmenten](/docs/app/building-your-application/routing/dynamic-routes) verwendet werden, um Routen [**statisch zu generieren**](/docs/app/building-your-application/rendering/server-components#static-rendering-default) und nicht erst bei Bedarf zur Laufzeit.
...
Der Hauptvorteil der Funktion `generateStaticParams` ist die intelligente Abfrage von Daten. Wenn Inhalte innerhalb der Funktion `generateStaticParams` über eine `fetch`-Anfrage abgerufen werden, werden die Anfragen [automatisch zwischengespeichert](/docs/app/building-your-application/caching#request-memoization). Das bedeutet, dass eine `fetch`-Anfrage mit denselben Argumenten über mehrere `generateStaticParams`, Layouts und Seiten hinweg nur einmal durchgeführt wird, was die Build-Zeiten verkürzt.
...
Der Unterschied zwischen **Catch-all-** und **optionalen Catch-all-Segmenten** besteht darin, dass bei optionalen Segmenten die Route ohne den Parameter ebenfalls passt (`/shop` im obigen Beispiel).
...
> **Wissenswert**: Dies könnte in Zukunft automatisch durch das [TypeScript-Plugin](/docs/app/building-your-application/configuring/typescript#typescript-plugin) erfolgen.