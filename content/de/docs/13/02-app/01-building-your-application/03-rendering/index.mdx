...
translation-updated-at: 2025-06-06T18:07:29.026Z
title: Rendering
...
- Der [Request-Response-Lebenszyklus](#request-response-lifecycle), der initiiert wird, wenn ein Benutzer Ihre Anwendung besucht oder mit ihr interagiert.
- Die [Netzwerkgrenze](#network-boundary), die Server- und Client-Code trennt.

### Rendering-Umgebungen

Es gibt zwei Umgebungen, in denen Webanwendungen gerendert werden können: der Client und der Server.
...
Historisch gesehen mussten Entwickler verschiedene Sprachen (z.B. JavaScript, PHP) und Frameworks verwenden, wenn sie Code für den Server und den Client schrieben. Mit React können Entwickler die **gleiche Sprache** (JavaScript) und das **gleiche Framework** (z.B. Next.js oder ein Framework Ihrer Wahl) verwenden. Diese Flexibilität ermöglicht es Ihnen, nahtlos Code für beide Umgebungen zu schreiben, ohne den Kontext wechseln zu müssen.

Jede Umgebung hat jedoch ihre eigenen Fähigkeiten und Einschränkungen. Daher ist der Code, den Sie für den Server und den Client schreiben, nicht immer derselbe. Bestimmte Operationen (z.B. Datenabruf oder Verwaltung des Benutzerzustands) sind besser für die eine oder die andere Umgebung geeignet.
...
Grob gesagt folgen alle Websites dem gleichen **Request-Response-Lebenszyklus**:

1. **Benutzeraktion:** Der Benutzer interagiert mit einer Webanwendung. Dies könnte das Anklicken eines Links, das Absenden eines Formulars oder das direkte Eingeben einer URL in die Adressleiste des Browsers sein.
...
In der Webentwicklung ist die **Netzwerkgrenze** eine konzeptionelle Linie, die die verschiedenen Umgebungen trennt. Zum Beispiel der Client und der Server oder der Server und der Datenspeicher.

{/* Diagramm: Netzwerkgrenze */}

In React können Sie die Client-Server-Netzwerkgrenze dort platzieren, wo es am sinnvollsten ist.
...
Es kann hilfreich sein, sich Modulgraphen als visuelle Darstellung vorzustellen, wie die Dateien in Ihrer Anwendung voneinander abhängig sind.

{/* Beispiel: Wenn Sie eine Datei namens `Page.jsx` haben, die auf dem Server eine Datei namens `Button.jsx` importiert, würde der Modulgraph etwa so aussehen: - Diagramm - */}

Sie können die React-Konvention `"use client"` verwenden, um die Grenze zu definieren. Es gibt auch eine `"use server"`-Konvention, die React anweist, einige Berechnungen auf dem Server durchzuführen, während der Client aktiv ist.
...
Wenn Sie vom Client aus auf den Server zugreifen müssen, senden Sie eine **neue** Anfrage an den Server, anstatt dieselbe Anfrage erneut zu verwenden. Dies erleichtert das Verständnis, wo Sie Ihre Komponenten rendern und wo Sie die Netzwerkgrenze platzieren sollen.
