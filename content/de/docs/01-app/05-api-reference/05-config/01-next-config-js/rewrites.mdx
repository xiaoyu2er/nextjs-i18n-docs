...
translation-updated-at: 2025-06-02T20:16:59.854Z
title: Rewrites
...
Rewrites fungieren als URL-Proxy und maskieren den Zielpfad, sodass es so aussieht, als hätte der Benutzer seinen Standort auf der Website nicht geändert. Im Gegensatz dazu leiten [Redirects](/docs/app/api-reference/config/next-config-js/redirects) zu einer neuen Seite weiter und zeigen die URL-Änderungen an.

</AppOnly>

<PagesOnly>

Rewrites fungieren als URL-Proxy und maskieren den Zielpfad, sodass es so aussieht, als hätte der Benutzer seinen Standort auf der Website nicht geändert. Im Gegensatz dazu leiten [Redirects](/docs/pages/api-reference/config/next-config-js/redirects) zu einer neuen Seite weiter und zeigen die URL-Änderungen an.
...
1. [Headers](/docs/app/api-reference/config/next-config-js/headers) werden überprüft/angewendet
2. [Redirects](/docs/app/api-reference/config/next-config-js/redirects) werden überprüft/angewendet
3. `beforeFiles`-Rewrites werden überprüft/angewendet
4. Statische Dateien aus dem [public-Verzeichnis](/docs/app/api-reference/file-conventions/public-folder), `_next/static`-Dateien und nicht-dynamische Seiten werden überprüft/bereitgestellt
...
1. [Headers](/docs/pages/api-reference/config/next-config-js/headers) werden überprüft/angewendet
2. [Redirects](/docs/pages/api-reference/config/next-config-js/redirects) werden überprüft/angewendet
3. `beforeFiles`-Rewrites werden überprüft/angewendet
4. Statische Dateien aus dem [public-Verzeichnis](/docs/pages/api-reference/file-conventions/public-folder), `_next/static`-Dateien und nicht-dynamische Seiten werden überprüft/bereitgestellt
5. `afterFiles`-Rewrites werden überprüft/angewendet, wenn eines dieser Rewrites matched, werden dynamische Routen/statische Dateien nach jedem Match überprüft
6. `fallback`-Rewrites werden überprüft/angewendet, diese werden vor dem Rendern der 404-Seite und nach der Überprüfung dynamischer Routen/aller statischen Assets angewendet. Wenn Sie [fallback: true/'blocking'](/docs/pages/api-reference/functions/get-static-paths#fallback-true) in `getStaticPaths` verwenden, werden die in Ihrer `next.config.js` definierten `fallback`-Rewrites _nicht_ ausgeführt.
...
> **Gut zu wissen**: Statische Seiten aus [Automatic Static Optimization](/docs/pages/building-your-application/rendering/automatic-static-optimization) oder [Prerendering](/docs/pages/building-your-application/data-fetching/get-static-props) Parameter von Rewrites werden nach der Hydration auf dem Client geparst und in der Query bereitgestellt.
...
Wenn Sie [`basePath`-Unterstützung](/docs/app/api-reference/config/next-config-js/basePath) mit Rewrites verwenden, wird jeder `source`- und `destination`-Pfad automatisch mit dem `basePath` präfixiert, es sei denn, Sie fügen `basePath: false` zum Rewrite hinzu:
...
Wenn Sie [`i18n`-Unterstützung](/docs/pages/guides/internationalization) mit Rewrites verwenden, wird jeder `source`- und `destination`-Pfad automatisch präfixiert, um die konfigurierten `locales` zu handhaben, es sei denn, Sie fügen `locale: false` zum Rewrite hinzu. Wenn `locale: false` verwendet wird, müssen Sie `source` und `destination` mit einem Locale präfixieren, damit sie korrekt gematcht werden können.
