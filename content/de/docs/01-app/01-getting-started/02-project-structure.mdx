...
translation-updated-at: 2025-06-08T21:37:40.242Z
title: Projektstruktur und Organisation
nav_title: Projektstruktur
description: Eine Übersicht über die Ordner- und Dateikonventionen in Next.js sowie Empfehlungen zur Projektorganisation.
---
...
| [`public`](/docs/app/api-reference/file-conventions/public-folder) | Statische Assets für den Einsatz   |
...
| [`.env.production`](/docs/app/guides/environment-variables)                  | Produktions-Umgebungsvariablen          |
| [`.env.development`](/docs/app/guides/environment-variables)                 | Entwicklungs-Umgebungsvariablen         |
...
|                                                                                |                                                  |
| ------------------------------------------------------------------------------ | ------------------------------------------------ |
| [`(folder)`](/docs/app/api-reference/file-conventions/route-groups#convention) | Routen gruppieren ohne Routing zu beeinflussen   |
| [`_folder`](#private-folders)                                                  | Ordner und Kindsegmente vom Routing ausschließen |
...
<Image
  alt="Ein Diagramm, das zeigt, dass zusammen abgelegte Projektdateien nicht routbar sind, selbst wenn ein Segment eine page.js- oder route.js-Datei enthält."
...
> **Gut zu wissen**: Während Sie Ihre Projektdateien in `app` zusammenlegen **können**, müssen Sie dies nicht tun. Wenn Sie es bevorzugen, können Sie [sie außerhalb des `app`-Verzeichnisses belassen](#store-project-files-outside-of-app).
...
Da Dateien im `app`-Verzeichnis standardmäßig [sicher zusammengelegt werden können](#colocation), sind private Ordner für die Zusammenlagerung nicht erforderlich. Sie können jedoch nützlich sein für:
...
Next.js unterstützt die Speicherung von Anwendungscode (einschließlich `app`) in einem optionalen [`src`-Ordner](/docs/app/api-reference/file-conventions/src-folder). Dies trennt den Anwendungscode von Projektkonfigurationsdateien, die meist im Stammverzeichnis eines Projekts liegen.
...
<Image
  alt="Eine Beispielordnerstruktur mit Projektdateien, die nach Funktion oder Route aufgeteilt sind"
...
Um ein [Lade-Skelett](/docs/app/api-reference/file-conventions/loading) über eine `loading.js`-Datei auf eine bestimmte Route anzuwenden, erstellen Sie eine neue Routengruppe (z.B. `/(overview)`) und verschieben Sie Ihre `loading.tsx` in diese Routengruppe.
