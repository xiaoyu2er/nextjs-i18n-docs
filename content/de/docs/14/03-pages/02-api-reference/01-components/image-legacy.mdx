...
Sie können die Time to Live (TTL) in Sekunden für zwischengespeicherte optimierte Bilder konfigurieren. In vielen Fällen ist es besser, einen [statischen Bildimport](/docs/pages/building-your-application/optimizing/images#local-images) zu verwenden, der automatisch den Dateiinhalt hashed und das Bild mit einem `Cache-Control`-Header von `immutable` für immer zwischenspeichert.
...
Wenn Sie das Caching-Verhalten pro Bild ändern müssen, können Sie [`headers`](/docs/pages/api-reference/next-config-js/headers) konfigurieren, um den `Cache-Control`-Header für das Upstream-Bild zu setzen (z.B. `/some-asset.jpg`, nicht `/_next/image` selbst).
