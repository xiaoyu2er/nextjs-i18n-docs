...
- `props` - Ein Objekt mit den Parametern der aktuellen Route:
  ...
  - `params` - Ein Objekt mit den [dynamischen Routenparametern](/docs/app/building-your-application/routing/dynamic-routes) vom Wurzelsegment bis zu dem Segment, aus dem `generateMetadata` aufgerufen wird. Beispiele:
...
> - `fetch`-Anfragen werden automatisch [memoisiert](/docs/app/building-your-application/caching#request-memoization) für dieselben Daten über `generateMetadata`, `generateStaticParams`, Layouts, Seiten und Server Components hinweg. React [`cache` kann verwendet werden](/docs/app/building-your-application/caching#request-memoization), wenn `fetch` nicht verfügbar ist.
...
Sie können Typsicherheit für Ihre Metadaten hinzufügen, indem Sie den `Metadata`-Typ verwenden. Wenn Sie das [integrierte TypeScript-Plugin](/docs/app/building-your-application/configuring/typescript) in Ihrer IDE verwenden, müssen Sie den Typ nicht manuell hinzufügen, können dies aber dennoch explizit tun.
