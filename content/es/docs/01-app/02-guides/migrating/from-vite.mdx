...
translation-updated-at: 2025-06-08T21:27:26.517Z
...
### Sin división automática de código (code splitting)
...
Con soporte integrado para [transmisión mediante React Suspense](/docs/app/getting-started/linking-and-navigating#streaming), puedes ser más intencional sobre qué partes de tu interfaz de usuario deseas cargar primero y en qué orden sin introducir cascadas de red.
...
Dependiendo de tus necesidades, Next.js te permite elegir tu estrategia de obtención de datos por página y componente. Puedes decidir obtener en tiempo de compilación, en tiempo de solicitud en el servidor o en el cliente. Por ejemplo, puedes obtener datos de tu CMS y renderizar tus publicaciones de blog en tiempo de compilación, que luego pueden almacenarse en caché eficientemente en una CDN.
...
6. Finalmente, Next.js puede gestionar tus últimas etiquetas `<head>` con la [API de Metadatos](/docs/app/getting-started/metadata-and-og-images). Mueve tu información de metadatos final a un objeto [`metadata`](/docs/app/api-reference/functions/generate-metadata#metadata-object) exportado:
...
Este directorio es lo que se conoce como un [segmento de ruta opcional catch-all](/docs/app/api-reference/file-conventions/dynamic-routes#optional-catch-all-segments). Next.js utiliza un enrutador basado en el sistema de archivos donde las carpetas definen las rutas. Este directorio especial asegurará que todas las rutas de tu aplicación se dirijan al archivo `page.tsx` que contiene.
...
Este archivo es un [Componente del Servidor (Server Component)](/docs/app/getting-started/server-and-client-components). Cuando ejecutas `next build`, el archivo se prerrenderiza en un recurso estático. No requiere ningún código dinámico.
...
Este archivo es un [Componente del Cliente (Client Component)](/docs/app/getting-started/server-and-client-components), definido por la directiva `'use client'`. Los Componentes del Cliente aún se [prerrenderizan a HTML](/docs/app/getting-started/server-and-client-components#how-do-server-and-client-components-work-in-nextjs) en el servidor antes de enviarse al cliente.

Como queremos comenzar con una aplicación solo del cliente, podemos configurar Next.js para desactivar el prerrenderizado desde el componente `App` hacia abajo.
...
Next.js tiene soporte para [variables de entorno](/docs/app/guides/environment-variables) en `.env`, similar a Vite. La principal diferencia es el prefijo utilizado para exponer variables de entorno en el lado del cliente.
...
2. **Establece [`basePath`](/docs/app/api-reference/config/next-config-js/basePath) en `process.env.NEXT_PUBLIC_BASE_PATH` en tu archivo `next.config.mjs`:**
...
const nextConfig = {
  ...
  distDir: './dist', // Cambia el directorio de salida de la compilación a `./dist/`.
  basePath: process.env.NEXT_PUBLIC_BASE_PATH, // Establece la ruta base en `/some-base-path`.
}
...
- Migrar de React Router al [Enrutador de Aplicación (App Router) de Next.js](/docs/app/building-your-application/routing) para obtener:
  ...
  - [Renderizado del servidor con streaming (Streaming Server-Rendering)](/docs/app/api-reference/file-conventions/loading)
...
- [Actualizar tu configuración de ESLint para admitir las reglas de Next.js](/docs/app/api-reference/config/eslint)
