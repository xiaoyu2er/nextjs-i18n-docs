...
translation-updated-at: 2025-06-08T21:23:30.549Z
title: useSearchParams
description: Referencia de API para el hook useSearchParams.
---

`useSearchParams` es un hook de **Componente Cliente** que permite leer la **cadena de consulta** de la URL actual.
...
`useSearchParams` no recibe ningún parámetro.
...
> **Importante**:
>
> - `useSearchParams` es un hook de [Componente Cliente](/docs/app/getting-started/server-and-client-components) y **no es compatible** con [Componentes Servidor](/docs/app/getting-started/server-and-client-components) para evitar valores obsoletos durante el [renderizado parcial](/docs/app/getting-started/linking-and-navigating#client-side-transitions).
> - Si una aplicación incluye el directorio `/pages`, `useSearchParams` devolverá `ReadonlyURLSearchParams | null`. El valor `null` es para compatibilidad durante la migración ya que los parámetros de búsqueda no pueden conocerse durante el prerenderizado de una página que no use `getServerSideProps`

## Comportamiento

### Renderizado Estático

Si una ruta se [renderiza estáticamente](/docs/app/getting-started/partial-prerendering#static-rendering), llamar a `useSearchParams` hará que el árbol de Componentes Cliente hasta el límite [`Suspense`](/docs/app/api-reference/file-conventions/loading#examples) más cercano se renderice en el lado del cliente.
...
Recomendamos envolver el Componente Cliente que usa `useSearchParams` en un límite `<Suspense/>`. Esto permitirá que cualquier Componente Cliente superior se renderice estáticamente y se envíe como parte del HTML inicial. [Ejemplo](/docs/app/api-reference/functions/use-search-params#static-rendering).
...
export default function SearchBar() {
  ...
  // Esto no se registrará en el servidor al usar renderizado estático
  console.log(search)
...
Si una ruta se [renderiza dinámicamente](/docs/app/getting-started/partial-prerendering#dynamic-rendering), `useSearchParams` estará disponible en el servidor durante el renderizado inicial del Componente Cliente.
...
A diferencia de las Páginas, los [Layouts](/docs/app/api-reference/file-conventions/layout) (Componentes Servidor) **no reciben** la prop `searchParams`. Esto se debe a que un layout compartido [no se vuelve a renderizar durante la navegación](/docs/app/getting-started/linking-and-navigating#client-side-transitions), lo que podría llevar a `searchParams` obsoletos entre navegaciones. Ver [explicación detallada](/docs/app/api-reference/file-conventions/layout#query-params).
...
export default function ExampleClientComponent() {
  ...
  // Obtén una nueva cadena searchParams combinando los
