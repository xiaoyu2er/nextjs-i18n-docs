...
translation-updated-at: 2025-06-06T17:33:05.827Z
...
Use un archivo de imagen para configurar un icono de aplicación colocando un archivo de imagen `favicon`, `icon` o `apple-icon` dentro de su directorio `/app`.
La imagen `favicon` solo puede ubicarse en el nivel superior de `app/`.
...
> - Los favicons solo pueden configurarse en el segmento raíz `/app`. Si necesita mayor granularidad, puede usar [`icon`](#icon).
...
Además de usar [archivos de imagen literales](#archivos-de-imagen-ico-jpg-png), puede **generar** iconos programáticamente usando código.
...
La forma más fácil de generar un icono es usando la API [`ImageResponse`](/docs/app/api-reference/functions/image-response) de `next/server`.
...
> **Dato útil**
>
> - Por defecto, los iconos generados están [**optimizados estáticamente**](/docs/app/building-your-application/rendering/server-components#static-rendering-default) (generados en tiempo de compilación y cacheados) a menos que usen [funciones dinámicas](/docs/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) o datos no cacheados.
> - Puede generar múltiples iconos en el mismo archivo usando [`generateImageMetadata`](/docs/app/api-reference/functions/generate-image-metadata).
> - No puede generar un icono `favicon`. Use [`icon`](#icon) o un archivo [favicon.ico](#favicon) en su lugar.
...
`icon` y `apple-icon` son [Route Handlers](/docs/app/building-your-application/routing/route-handlers) especializados que pueden usar las mismas opciones de [configuración de segmento de ruta](/docs/app/api-reference/file-conventions/route-segment-config) que Páginas y Layouts.
