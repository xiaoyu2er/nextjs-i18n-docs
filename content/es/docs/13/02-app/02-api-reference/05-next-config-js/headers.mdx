...
translation-updated-at: 2025-06-06T17:17:06.593Z
...
- `has`: array de [objetos has](#coincidencia-de-encabezados-cookies-y-consultas) con las propiedades `type`, `key` y `value`.
- `missing`: array de [objetos missing](#coincidencia-de-encabezados-cookies-y-consultas) con las propiedades `type`, `key` y `value`.
...
Cuando se usa el [soporte para `basePath`](/docs/app/api-reference/next-config-js/basePath) con encabezados, cada `source` se prefija automáticamente con el `basePath` a menos que agregue `basePath: false` al encabezado:
...
Cuando se usa el [soporte para `i18n`](/docs/app/building-your-application/routing/internationalization) con encabezados, cada `source` se prefija automáticamente para manejar los `locales` configurados, a menos que agregue `locale: false` al encabezado. Si se usa `locale: false`, debe prefijar el `source` con un locale para que coincida correctamente.

</AppOnly>

<PagesOnly>

Cuando se usa el [soporte para `i18n`](/docs/pages/building-your-application/routing/internationalization) con encabezados, cada `source` se prefija automáticamente para manejar los `locales` configurados, a menos que agregue `locale: false` al encabezado. Si se usa `locale: false`, debe prefijar el `source` con un locale para que coincida correctamente.
...
[Este encabezado](https://developer.mozilla.org/docs/Web/HTTP/Headers/X-Frame-Options) indica si se debe permitir que el sitio se muestre dentro de un `iframe`. Esto puede prevenir ataques de clickjacking.
...
[Este encabezado](https://developer.mozilla.org/docs/Web/HTTP/Headers/Permissions-Policy) le permite controlar qué funciones y APIs se pueden usar en el navegador. Anteriormente se llamaba `Feature-Policy`.
...
Aprenda más sobre cómo agregar una [Política de Seguridad de Contenido (Content Security Policy)](/docs/app/building-your-application/configuring/content-security-policy) a su aplicación.
