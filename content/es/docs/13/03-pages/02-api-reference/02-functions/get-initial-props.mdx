...
translation-updated-at: 2025-06-06T17:06:51.498Z
...
> **Es bueno saberlo**:
>
> - Los datos devueltos por `getInitialProps` se serializan al renderizar en el servidor. Asegúrate de que el objeto devuelto por `getInitialProps` sea un `Object` plano, y no use `Date`, `Map` o `Set`.
...
| `asPath`   | `String` de la ruta real (incluyendo la consulta) mostrada en el navegador                            |
...
- `getInitialProps` solo se puede usar en archivos de nivel superior en `pages/`, y no en componentes anidados. Para tener obtención de datos anidados a nivel de componente, considera explorar el [App Router](/docs/app/building-your-application/data-fetching).
- Independientemente de si tu ruta es estática o dinámica, cualquier dato devuelto por `getInitialProps` como `props` podrá examinarse en el lado del cliente en el HTML inicial. Esto es para permitir que la página se [hidrate](https://react.dev/reference/react-dom/hydrate) correctamente. Asegúrate de no pasar ninguna información sensible que no deba estar disponible en el cliente en `props`.