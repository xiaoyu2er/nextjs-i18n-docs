...
translation-updated-at: 2025-06-06T16:38:40.857Z
...
Si eres nuevo en GitHub, te recomendamos leer la [Guía de Código Abierto de GitHub](https://opensource.guide/how-to-contribute/#opening-a-pull-request) para aprender cómo bifurcar un repositorio, crear una rama y enviar una solicitud de extracción.
...
La documentación está escrita en [MDX](https://mdxjs.com/), un formato de markdown que soporta sintaxis JSX. Esto nos permite incrustar componentes React en la documentación. Consulta la [Guía de Markdown de GitHub](https://docs.github.com/es/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax) para una descripción rápida de la sintaxis de markdown.

### VSCode

#### Previsualizando cambios localmente

VSCode tiene un visor de markdown integrado que puedes usar para ver tus ediciones localmente. Para habilitar el visor para archivos MDX, necesitarás agregar una opción de configuración a tus ajustes de usuario.
...
Pero, en la [sección de enrutamiento](/docs/app/building-your-application/routing), los archivos tienen prefijo con un número de dos dígitos, ordenados en el orden en que los desarrolladores deberían aprender estos conceptos:
...
>
> Consideramos usar un archivo manifiesto (otra forma popular de generar la navegación de la documentación), pero encontramos que un manifiesto rápidamente se desincronizaría con los archivos. El enrutamiento por sistema de archivos nos obliga a pensar en la estructura de la documentación y se siente más nativo para Next.js.
...
| Campo         | Descripción                                                                  |
| ------------- | ---------------------------------------------------------------------------- |
| `title`       | El título `<h1>` de la página, usado para SEO y OG Images.                   |
...
| `source`    | Extrae contenido en una página compartida. Ver [Páginas Compartidas](#páginas-compartidas).                                                                               |
...
---
nav_title: Título del Ítem de Navegación
...
Dado que la mayoría de las características en el **App Router** y **Pages Router** son completamente diferentes, su documentación para cada uno se mantiene en secciones separadas (`02-app` y `03-pages`). Sin embargo, hay algunas características que son compartidas entre ellos.
...
En páginas compartidas, a veces puede haber contenido que es específico del **App Router** o **Pages Router**. Por ejemplo, el componente `<Link>` tiene una prop `shallow` que solo está disponible en **Pages** pero no en **App**.
...
Agrega un conmutador de idioma para alternar entre TypeScript y JavaScript. Los bloques de código deben ser primero TypeScript con una versión en JavaScript para acomodar a los usuarios.
...
- Los enlaces se generarán automáticamente para páginas que tienen páginas secundarias. Por ejemplo, la sección [Optimización](/docs/app/building-your-application/optimizing) tiene enlaces a todas sus páginas secundarias.
...
| `links`       | Requerido  | Una lista de enlaces a otras páginas de documentación. Cada elemento de la lista debe ser una ruta URL relativa (sin una barra diagonal inicial) ej. `app/api-reference/file-conventions/page` |
...
Si tienes ideas para nuevos componentes, por favor abre un [issue en GitHub](https://github.com/vercel/next.js/issues/new/choose).
...
- **Tablas de API**: Las páginas de API deben tener una tabla de resumen al final de la página con enlaces de salto a sección (cuando sea posible).
...
- **Páginas Conceptuales** se utilizan para explicar un concepto o funcionalidad. Suelen ser más extensas y contener más información que las páginas de referencia. En la documentación de Next.js, estas páginas se encuentran en la sección **Construyendo Su Aplicación**.
...
> **Nota importante**: Dependiendo de la página a la que esté contribuyendo, puede necesitar seguir un tono y estilo diferente. Por ejemplo, las páginas conceptuales son más instructivas y utilizan la palabra _usted_ para dirigirse al usuario. Las páginas de referencia son más técnicas, usan palabras más imperativas como "crear, actualizar, aceptar" y tienden a omitir la palabra _usted_.

### Tono

Aquí hay algunas pautas para mantener un estilo y tono consistentes en toda la documentación:

- Escriba oraciones claras y concisas. Evite divagaciones.
  - Si nota que está usando muchas comas, considere dividir la oración en varias o usar una lista.
  - Sustituya palabras complejas por otras más simples. Por ejemplo, _usar_ en lugar de _utilizar_.
...
- Utilice voz activa en lugar de pasiva. Una oración activa es más fácil de leer.
  - Por ejemplo, _Next.js usa React_ en lugar de _React es usado por Next.js_. Si nota que usa palabras como _fue_ o _por_, puede estar usando voz pasiva.
- Evite usar palabras como _fácil_, _rápido_, _simple_, _solo_, etc. Esto es subjetivo y puede desmotivar a los usuarios.
- Evite palabras negativas como _no_, _no puede_, _no hará_, etc. Esto puede desanimar a los lectores.
...
- Utilice lenguaje neutro en cuanto al género. Use _desarrolladores_, _usuarios_ o _lectores_ al referirse a la audiencia.
- Si añade ejemplos de código, asegúrese de que estén correctamente formateados y funcionen.

Aunque estas pautas no son exhaustivas, deberían ayudarle a comenzar. Si desea profundizar en la escritura técnica, consulte el [Curso de Escritura Técnica de Google](https://developers.google.com/tech-writing/overview).
