...
translation-updated-at: 2025-06-06T16:59:08.407Z
title: OpenTelemetry
description: Aprende cómo instrumentar tu aplicación Next.js con OpenTelemetry.
---

{/* El contenido de este documento se comparte entre el enrutador de app y pages. Puedes usar el componente `<PagesOnly>Contenido</PagesOnly>` para añadir contenido específico del enrutador de Pages. Cualquier contenido compartido no debe estar envuelto en un componente. */}
...
Es una forma agnóstica de plataforma para instrumentar aplicaciones que te permite cambiar tu proveedor de observabilidad sin modificar tu código.
Lee la [documentación oficial de OpenTelemetry](https://opentelemetry.io/docs/) para más información sobre OpenTelemetry y cómo funciona.

Esta documentación utiliza términos como _Span_ (Intervalo), _Trace_ (Traza) o _Exporter_ (Exportador) a lo largo del documento, todos los cuales puedes encontrar en [el manual de observabilidad de OpenTelemetry](https://opentelemetry.io/docs/concepts/observability-primer/).
...
Cuando activas OpenTelemetry, automáticamente envolvemos todo tu código como `getStaticProps` en _spans_ con atributos útiles.

## Primeros pasos

OpenTelemetry es extensible pero configurarlo correctamente puede ser bastante verboso.
...
A continuación, crea un archivo personalizado [`instrumentation.ts`](/docs/app/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):

</AppOnly>

<PagesOnly>

A continuación, crea un archivo personalizado [`instrumentation.ts`](/docs/pages/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):
...
Consulta la [documentación de `@vercel/otel`](https://www.npmjs.com/package/@vercel/otel) para opciones de configuración adicionales.
...
> - Si usas la opción de configuración [`pageExtensions`](/docs/app/api-reference/next-config-js/pageExtensions) para añadir un sufijo, también necesitarás actualizar el nombre del archivo `instrumentation` para que coincida.
...
>
> - El archivo `instrumentation` debe estar en la raíz de tu proyecto y no dentro de los directorios `app` o `pages`. Si usas la carpeta `src`, coloca el archivo dentro de `src` junto a `pages` y `app`.
> - Si usas la opción de configuración [`pageExtensions`](/docs/pages/api-reference/next-config-js/pageExtensions) para añadir un sufijo, también necesitarás actualizar el nombre del archivo `instrumentation` para que coincida.
> - Hemos creado un ejemplo básico [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry) que puedes usar.
...
A diferencia de `@vercel/otel`, `NodeSDK` no es compatible con el entorno de ejecución edge, por lo que debes asegurarte de que solo los importas cuando `process.env.NEXT_RUNTIME === 'nodejs'`. Recomendamos crear un nuevo archivo `instrumentation.node.ts` que importes condicionalmente solo cuando uses node:
...
Cuando te despliegas con OpenTelemetry Collector, puedes usar `@vercel/otel`.
...
Para hacer esto, sigue la [guía de inicio de OpenTelemetry Collector](https://opentelemetry.io/docs/collector/getting-started/), que te guiará a través de la configuración del recolector para recibir datos de tu aplicación Next.js.
...
OpenTelemetry Collector no es necesario. Puedes usar un exportador OpenTelemetry personalizado con [`@vercel/otel`](#usando-vercelotel) o [configuración manual de OpenTelemetry](/docs/pages/building-your-application/optimizing/open-telemetry#configuración-manual-de-opentelemetry).

## Spans personalizados

Puedes añadir un span personalizado con las [APIs de OpenTelemetry](https://opentelemetry.io/docs/instrumentation/js/instrumentation).
...
Puedes comenzar a crear nuevos spans, y deberían añadirse correctamente a la traza exportada.
...
Los atributos en los spans siguen las [convenciones semánticas de OpenTelemetry](https://opentelemetry.io/docs/reference/specification/trace/semantic_conventions/). También añadimos algunos atributos personalizados bajo el espacio de nombres `next`:
