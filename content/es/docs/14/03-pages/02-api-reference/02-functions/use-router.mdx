...
Redirigiendo al usuario a `pages/login.js`, útil para páginas detrás de [autenticación](/docs/pages/building-your-application/authentication):
...
Puedes usar un objeto URL de la misma manera que lo harías con [`next/link`](/docs/pages/api-reference/components/link#with-url-object). Funciona tanto para el parámetro `url` como para `as`:
...
Similar a la propiedad `replace` en [`next/link`](/docs/pages/api-reference/components/link), `router.replace` evitará agregar una nueva entrada de URL en la pila del `history`.
...
En algunos casos (por ejemplo, si usas un [Servidor Personalizado](/docs/pages/building-your-application/configuring/custom-server)), puedes querer escuchar [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) y hacer algo antes de que el router actúe sobre él.
...
Si `cb` devuelve `false`, el router de Next.js no manejará el `popstate`, y serás responsable de manejarlo en ese caso. Consulta [Deshabilitar enrutamiento del sistema de archivos](/docs/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
> Usamos una [Aplicación Personalizada](/docs/pages/building-your-application/routing/custom-app) (`pages/_app.js`) para este ejemplo para suscribirnos al evento porque no se desmonta en navegaciones de página, pero puedes suscribirte a eventos del router en cualquier componente de tu aplicación.
