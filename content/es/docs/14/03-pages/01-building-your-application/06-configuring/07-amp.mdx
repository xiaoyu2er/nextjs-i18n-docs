...
Para habilitar el soporte de AMP en una página y aprender sobre las diferentes configuraciones de AMP, lea la [documentación de la API para `next/amp`](/docs/pages/building-your-application/configuring/amp).

## Consideraciones

- Solo se admite CSS-in-JS. Los [Módulos CSS](/docs/pages/building-your-application/styling) no son compatibles con páginas AMP actualmente. Puede [contribuir al soporte de Módulos CSS en Next.js](https://github.com/vercel/next.js/issues/10549).
...
Las páginas también se validan durante la [exportación HTML estática](/docs/pages/building-your-application/deploying/static-exports) y cualquier advertencia/error se imprimirá en la terminal. Cualquier error de AMP hará que la exportación termine con código de estado `1` porque la exportación no es AMP válido.
...
Al usar la [exportación HTML estática](/docs/pages/building-your-application/deploying/static-exports) para prerrenderizar páginas estáticamente, Next.js detectará si la página admite AMP y cambiará el comportamiento de exportación en consecuencia.
...
Cuando [`trailingSlash`](/docs/pages/api-reference/next-config-js/trailingSlash) está habilitado, las páginas exportadas para `pages/about.js` serían:
