...
description: 学习如何为你的 Next.js 应用添加元数据
image: >-
...
*   **基于配置**：导出一个 [静态 `metadata` 对象](/docs/app/api-reference/functions/generate-metadata#metadata-object) 或一个动态的 [`generateMetadata` 函数](/docs/app/api-reference/functions/generate-metadata#generatemetadata-function) 在 `layout.js` 或 `page.js` 文件中。
*   **基于文件**：Next.js 有一系列特殊文件，专门用于元数据用途：
    *   `favicon.ico`, `apple-icon.jpg` 以及 `icon.jpg`：用于网站图标和图标
