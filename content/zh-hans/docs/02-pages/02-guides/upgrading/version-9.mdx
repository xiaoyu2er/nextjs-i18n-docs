...
translation-updated-at: 2025-05-20T22:22:03.880Z
title: 如何升级到版本 9
nav_title: 版本 9
...
从 `pages/_app.js` 中移除 `getInitialProps`（在可能的情况下）对于利用 Next.js 新功能非常重要！
...
从您的自定义 `.babelrc` 中移除对 `@zeit/next-typescript/babel` 的引用（如果存在）。
...
TypeScript 定义已随 `next` 包发布，因此您需要卸载 `@types/next`，因为它们会产生冲突。

以下类型已变更：

> 此列表由社区创建以帮助您升级，如果您发现其他差异，请提交拉取请求更新此列表以帮助其他用户。
...
您不能再从页面导出一个名为 `config` 的自定义变量（即 `export { config }` / `export const config ...`）。
此导出变量现在用于指定页面级 Next.js 配置，如 Opt-in AMP 和 API 路由功能。
...
Next.js 现在支持页面级配置的概念，因此为了保持一致性，移除了 `withAmp` 高阶组件。

此变更可以通过 **在您的 Next.js 项目根目录运行以下命令自动迁移：**

```bash filename="终端"
curl -L https://github.com/vercel/next-codemod/archive/master.tar.gz | tar -xz --strip=2 next-codemod-master/transforms/withamp-to-config.js npx jscodeshift -t ./withamp-to-config.js pages/**/*.js
```

如需手动执行此迁移，或查看代码修改器将生成的内容，请参考以下示例：
...
`pages/api/` 中的页面现在被视为 [API 路由](https://nextjs.org/blog/next-9#api-routes)。
此目录中的页面将不再包含客户端包。

## 已弃用的功能

### `next/dynamic` 已弃用同时加载多个模块

`next/dynamic` 中同时加载多个模块的功能已被弃用，以更接近 React 的实现方式（`React.lazy` 和 `Suspense`）。

更新依赖此行为的代码相对简单！我们提供了一个迁移前后的示例来帮助您迁移应用：
