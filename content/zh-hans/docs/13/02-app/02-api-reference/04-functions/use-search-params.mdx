...
translation-updated-at: 2025-05-21T20:58:05.647Z
...
> - 如果应用程序包含 `/pages` 目录，`useSearchParams` 将返回 `ReadonlyURLSearchParams | null`。`null` 值是为了在迁移期间保持兼容性，因为在不使用 `getServerSideProps` 的页面预渲染期间无法获知搜索参数。

## 行为

### 静态渲染 (Static Rendering)

如果路由是[静态渲染 (statically rendered)](/docs/app/building-your-application/rendering/server-components#static-rendering-default)，调用 `useSearchParams()` 将导致最近的 [`Suspense` 边界 (boundary)](/docs/app/building-your-application/routing/loading-ui-and-streaming#example) 之前的组件树在客户端渲染。

这使得页面的静态渲染部分与使用 `searchParams` 的动态部分可以同时在客户端渲染。

您可以通过将使用 `useSearchParams` 的组件包裹在 `Suspense` 边界中来减少客户端渲染的路由部分。例如：
...
如果路由是[动态渲染 (dynamically rendered)](/docs/app/building-your-application/rendering/server-components#dynamic-rendering)，`useSearchParams` 将在客户端组件的初始服务器渲染期间在服务器端可用。
...
与页面不同，[布局 (Layouts)](/docs/app/api-reference/file-conventions/layout)（服务端组件）**不会**接收 `searchParams` 属性。这是因为共享布局在[导航期间不会重新渲染](/docs/app/building-your-application/routing/linking-and-navigating#3-partial-rendering)，这可能导致导航之间的 `searchParams` 陈旧。查看[详细解释](/docs/app/api-reference/file-conventions/layout#layouts-do-not-receive-searchparams)。

相反，可以在客户端组件中使用页面的 [`searchParams`](/docs/app/api-reference/file-conventions/page) 属性或 [`useSearchParams`](/docs/app/api-reference/functions/use-search-params) 钩子，这些组件会在客户端重新渲染并获取最新的 `searchParams`。
