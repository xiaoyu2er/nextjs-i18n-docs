...
translation-updated-at: 2025-05-21T21:10:41.152Z
...
当用户在社交网络和消息应用中分享您网站的链接时，这些图片将显示出来。
...
在与 `opengraph-image.(jpg|jpeg|png|gif)` 图片相同的路由段中添加 `opengraph-image.alt.txt` 文件，作为其替代文本。
...
在与 `twitter-image.(jpg|jpeg|png|gif)` 图片相同的路由段中添加 `twitter-image.alt.txt` 文件，作为其替代文本。
...
通过创建 `opengraph-image` 或 `twitter-image` 路由并默认导出一个函数，生成路由段的共享图片。
...
生成图片最简单的方法是使用 `next/server` 中的 [ImageResponse](/docs/app/api-reference/functions/image-response) API。
...
包含从根段到 `opengraph-image` 或 `twitter-image` 所在段的[动态路由参数](/docs/app/building-your-application/routing/dynamic-routes)对象。
...
> **须知**：
> 默认情况下，此生成的图片会进行[静态优化](/docs/app/building-your-application/rendering/server-components#static-rendering-default)。您可以配置单个 `fetch` 的 [`options`](/docs/app/api-reference/functions/fetch) 或路由段的 [options](/docs/app/api-reference/file-conventions/route-segment-config#revalidate) 来更改此行为。
