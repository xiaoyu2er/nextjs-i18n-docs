...
translation-updated-at: 2025-05-21T20:58:54.275Z
title: route.js
...
`request` 对象是一个 [NextRequest](/docs/app/api-reference/functions/next-request) 对象，它扩展了 Web [Request](https://developer.mozilla.org/docs/Web/API/Request) API。`NextRequest` 让您可以进一步控制传入的请求，包括轻松访问 `cookies` 和一个扩展的解析后的 URL 对象 `nextUrl`。
