...
当为 `blur` 时，将使用 [`blurDataURL`](#blurdataurl) 属性作为占位符。如果 `src` 是[静态导入](/docs/pages/building-your-application/optimizing/images#local-images)的对象且导入的图片是 `.jpg`、`.png`、`.webp` 或 `.avif` 格式，则 `blurDataURL` 会自动填充。
...
> 使用 [`output: 'export'`](/docs/pages/building-your-application/deploying/static-exports) 时，图像无法在构建时优化，只能按需优化。要将 `next/legacy/image` 与 `output: 'export'` 一起使用，您需要使用非默认的加载器。[在讨论中了解更多。](https://github.com/vercel/next.js/discussions/19065)
...
您可以配置缓存优化图像的生存时间 (TTL)（以秒为单位）。在许多情况下，最好使用 [静态图像导入 (Static Image Import)](/docs/pages/building-your-application/optimizing/images#local-images)，它会自动哈希文件内容，并使用 `Cache-Control` 标头 `immutable` 永久缓存图像。
...
如果需要按图像更改缓存行为，可以配置 [`headers`](/docs/pages/api-reference/next-config-js/headers) 在上游图像（例如 `/some-asset.jpg`，而不是 `/_next/image` 本身）上设置 `Cache-Control` 标头。
...
默认的 [加载器 (loader)](#loader) 出于几个原因不优化 SVG 图像。首先，SVG 是一种矢量格式，可以无损调整大小。其次，SVG 具有许多与 HTML/CSS 相同的功能，如果没有适当的 [内容安全策略 (CSP) 标头](/docs/app/api-reference/next-config-js/headers#content-security-policy)，可能会导致漏洞。
