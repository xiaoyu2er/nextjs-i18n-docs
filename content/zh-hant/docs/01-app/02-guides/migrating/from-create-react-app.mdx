---
title: 如何從 Create React App 遷移到 Next.js
nav_title: Create React App
description: 學習如何將現有的 React 應用程式從 Create React App 遷移到 Next.js。
---
...
[Next.js 代理](/docs/app/api-reference/file-conventions/proxy) 允許您在請求完成前於伺服器上執行程式碼。例如，您可以透過將使用者重新導向至僅限驗證頁面的代理登入頁面，來避免未驗證內容的閃爍。您也可以用於 A/B 測試、實驗和 [國際化](/docs/app/guides/internationalization) 等功能。
...
> **注意**：這需要透過在您的 `dev` 腳本中加入 `--webpack` 來使用 Webpack。
...
Create React App 使用 Webpack 進行封裝。Next.js 現在預設使用 [Turbopack](/docs/app/api-reference/config/next-config-js/turbopack) 以實現更快的本地開發：

```bash
next dev  # Uses Turbopack by default
```

若要改用 Webpack（類似於 CRA）:
...
- **將 React Router 遷移至** [Next.js App Router](/docs/app)，原因如下：
...
- **優化影像** 使用 [`<Image>` 元件](/docs/app/api-reference/components/image)
- **優化字型** 使用 [`next/font`](/docs/app/api-reference/components/font)
- **優化第三方腳本** 使用 [`<Script>` 元件](/docs/app/guides/scripts)
- **啟用 ESLint** 使用 Next.js [建議規則](/docs/app/api-reference/config/eslint#setup-eslint)
