...
translation-updated-at: 2025-05-25T20:51:09.033Z
title: Image (舊版)
...
檢視**新版** [`next/image` API 參考文件](/docs/pages/api-reference/components/image)

## 比較

與 `next/legacy/image` 相比，新版 `next/image` 元件有以下變更：

- 移除 `<img>` 周圍的 `<span>` 包裝，改用[原生的計算寬高比](https://caniuse.com/mdn-html_elements_img_aspect_ratio_computed_from_attributes)
...
- 移除 `loader` 配置，改用 [`loader`](#loader) prop
- 將 `alt` prop 從可選改為必填
...
- [靜態匯入](/docs/pages/api-reference/components/image#src)的圖片檔案
- 路徑字串。可以是絕對外部 URL 或內部路徑，取決於 [loader](#loader) prop 或 [loader 配置](#loader-configuration)。

使用預設 [loader](#loader) 時，還需考慮以下來源圖片情況：

- 當 src 是外部 URL 時，必須配置 [remotePatterns](#remote-patterns)
- 當 src 是[動畫圖片](#animated-images)或非已知格式 (JPEG、PNG、WebP、AVIF、GIF、TIFF) 時，圖片將按原樣提供
- 當 src 是 SVG 格式時，除非啟用 `unoptimized` 或 `dangerouslyAllowSVG`，否則將被阻止

### width

`width` 屬性可表示 _渲染_ 寬度或 _原始_ 寬度（像素），取決於 [`layout`](#layout) 和 [`sizes`](#sizes) 屬性。
...
`width` 屬性是必填的，除非是[靜態匯入的圖片](/docs/pages/api-reference/components/image#src)或使用 `layout="fill"` 的情況。

### height

`height` 屬性可表示 _渲染_ 高度或 _原始_ 高度（像素），取決於 [`layout`](#layout) 和 [`sizes`](#sizes) 屬性。
...
`height` 屬性是必填的，除非是[靜態匯入的圖片](/docs/pages/api-reference/components/image#src)或使用 `layout="fill"` 的情況。
...
| --------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------- | --------------------- |
| `intrinsic` (預設) | 縮小以適應容器寬度，最大不超過圖片原始尺寸 | `1x`, `2x` (基於 [imageSizes](#image-sizes))                                                            | N/A     | 是                   |
...
用於解析 URL 的自訂函式。在 Image 元件上設定 loader prop 會覆蓋 `next.config.js` 的 [`images` 區段](#loader-configuration)中定義的預設 loader。
...
一個字串，提供有關圖片在不同斷點下寬度的資訊。`sizes` 的值會極大影響使用 `layout="responsive"` 或 `layout="fill"` 的圖片效能。對於使用 `layout="intrinsic"` 或 `layout="fixed"` 的圖片，此屬性會被忽略。
...
當設為 true 時，圖片將被視為高優先級並進行[預載入](https://web.dev/preload-responsive-images/)。對於使用 `priority` 的圖片，延遲載入會自動停用。

您應該對檢測為[最大內容繪製 (LCP)](https://nextjs.org/learn/seo/web-performance/lcp) 元素的任何圖片使用 `priority` 屬性。根據不同視口大小，可能需要為多個圖片設定 priority。
...
當設為 `blur` 時，將使用 [`blurDataURL`](#blurdataurl) 屬性作為佔位符。如果 `src` 是[靜態匯入](/docs/pages/api-reference/components/image#src)的物件且匯入的圖片是 `.jpg`、`.png`、`.webp` 或 `.avif`，則 `blurDataURL` 會自動填充。
...
- [演示帶有 `blurDataURL` prop 的微光效果](https://image-legacy-component.nextjs.gallery/shimmer)
- [演示帶有 `blurDataURL` prop 的顏色效果](https://image-legacy-component.nextjs.gallery/color)
...
還要注意，必要的 `width` 和 `height` props 可能會與您的樣式互動。如果您使用樣式修改圖片的 `width`，則必須同時設定 `height="auto"` 樣式，否則圖片會變形。
...
一個回調函式，在圖片完全載入且[佔位符](#placeholder)被移除後呼叫。
...
一個[資料 URL](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs)，在 `src` 圖片成功載入前用作佔位圖片。僅在與 [`placeholder="blur"`](#placeholder) 結合使用時生效。
...
您也可以[生成純色資料 URL](https://png-pixel.com) 來匹配圖片。

### lazyBoundary

一個字串（語法類似於 margin 屬性），用作檢測視口與圖片交集並觸發延遲[載入](#loading)的邊界框。預設為 `"200px"`。

如果圖片嵌套在根文件以外的可滾動父元素中，您還需要指定 [lazyRoot](#lazyroot) prop。

[了解更多](https://developer.mozilla.org/docs/Web/API/IntersectionObserver/rootMargin)

### lazyRoot

一個指向可滾動父元素的 React [Ref](https://react.dev/learn/referencing-values-with-refs)。預設為 `null`（文件視口）。

Ref 必須指向 DOM 元素或將 Ref [轉發](https://react.dev/reference/react/forwardRef)到底層 DOM 元素的 React 元件。
...
- `srcSet`。請改用 [裝置尺寸 (Device Sizes)](#device-sizes)。
...
> **須知**：上述範例將確保 `next/legacy/image` 的 `src` 屬性必須以 `https://example.com/account123/` 開頭，且不能有查詢字串。任何其他協定、主機名稱、連接埠或不匹配的路徑將回傳 400 Bad Request。
...
> **須知**：上述範例將確保 `next/legacy/image` 的 `src` 屬性必須以 `https://img1.example.com` 或 `https://me.avatar.example.com` 或任何數量的子網域開頭。它不能有連接埠或查詢字串。任何其他協定或不匹配的主機名稱將回傳 400 Bad Request。
...
> **須知**：當省略 `protocol`、`port`、`pathname` 或 `search` 時，隱含使用萬用字元 `**`。這不建議使用，因為它可能允許惡意行為者優化您無意允許的 URL。
...
> **須知**：上述範例將確保 `next/legacy/image` 的 `src` 屬性必須以 `https://assets.example.com` 開頭，且必須具有確切的查詢字串 `?v=1727111025337`。任何其他協定或查詢字串將回傳 400 Bad Request。

### 網域 (Domains)

> **警告**：自 Next.js 14 起已棄用，轉而使用嚴格的 [`remotePatterns`](#remote-patterns) 以保護您的應用程式免受惡意使用者攻擊。僅在您擁有該網域提供的所有內容時才使用 `domains`。
...
如果您想使用雲端供應商來優化圖片，而不是使用 Next.js 內建的圖片優化 API，您可以在 `next.config.js` 檔案中配置 `loader` 和 `path` 前綴。這允許您為圖片的 [`src`](#src) 使用相對 URL，並自動為您的供應商生成正確的絕對 URL。
...
> 使用 [`output: 'export'`](/docs/pages/guides/static-exports) 時，圖片無法在構建時優化，僅能按需優化。要將 `next/legacy/image` 與 `output: 'export'` 一起使用，您需要使用非預設的載入器。[在討論中了解更多。](https://github.com/vercel/next.js/discussions/19065)
...
如果您知道使用者的預期裝置寬度，可以在 `next.config.js` 中使用 `deviceSizes` 屬性指定裝置寬度斷點列表。當 `next/legacy/image` 元件使用 `layout="responsive"` 或 `layout="fill"` 時，這些寬度用於確保為使用者的裝置提供正確的圖片。
...
您可以在 `next.config.js` 檔案中使用 `images.imageSizes` 屬性指定圖片寬度列表。這些寬度與 [裝置尺寸 (device sizes)](#device-sizes) 陣列連接，形成用於生成圖片 [srcset](https://developer.mozilla.org/docs/Web/API/HTMLImageElement/srcset) 的完整尺寸陣列。
...
預設的 [圖片優化 API](#loader-configuration) 會透過請求的 `Accept` 標頭自動檢測瀏覽器支援的圖片格式，以確定最佳輸出格式。

如果 `Accept` 標頭匹配多個配置的格式，則使用陣列中的第一個匹配項。因此，陣列順序很重要。如果沒有匹配項（或來源圖片是 [動畫圖片](#animated-images)），圖片優化 API 將回退到原始圖片的格式。
...
您可以啟用 AVIF 支援，如果瀏覽器 [不支援 AVIF](https://caniuse.com/avif)，則會回退到 `src` 圖片的原始格式：
...
以下描述預設 [載入器 (loader)](#loader) 的快取演算法。對於所有其他載入器，請參考您的雲端供應商文件。
...
過期時間（或稱最大壽命 (Max Age)）由 [`minimumCacheTTL`](#minimum-cache-ttl) 配置或上游圖片的 `Cache-Control` 標頭中的較大者定義。具體來說，使用 `Cache-Control` 標頭的 `max-age` 值。如果同時找到 `s-maxage` 和 `max-age`，則優先使用 `s-maxage`。`max-age` 也會傳遞給任何下游客戶端，包括 CDN 和瀏覽器。
...
- 您可以配置 [格式 (formats)](#acceptable-formats) 以停用多種格式，僅使用單一圖片格式。

### 最小快取 TTL (Minimum Cache TTL)

您可以為快取的優化圖片配置存留時間 (TTL)（以秒為單位）。在許多情況下，最好使用 [靜態圖片導入 (Static Image Import)](/docs/pages/api-reference/components/image#src)，它會自動對檔案內容進行雜湊處理，並使用 `Cache-Control` 標頭 `immutable` 永久快取圖片。
...
如果需要按圖片更改快取行為，可以配置 [`headers`](/docs/pages/api-reference/config/next-config-js/headers) 在上游圖片（例如 `/some-asset.jpg`，而不是 `/_next/image` 本身）上設定 `Cache-Control` 標頭。

目前沒有機制可以手動使快取失效，因此最好保持 `minimumCacheTTL` 較低。否則，您可能需要手動更改 [`src`](#src) 屬性或刪除 `<distDir>/cache/images`。
...
預設的 [載入器 (loader)](#loader) 不會優化 SVG 圖片，原因有幾個。首先，SVG 是一種向量格式，意味著它可以無損縮放。其次，SVG 具有許多與 HTML/CSS 相同的功能，如果沒有適當的 [內容安全政策 (Content Security Policy, CSP) 標頭](/docs/app/api-reference/config/next-config-js/headers#content-security-policy)，可能導致漏洞。

因此，我們建議在 [`src`](#src) 屬性已知為 SVG 時使用 [`unoptimized`](#unoptimized) 屬性。當 `src` 以 `".svg"` 結尾時，這會自動發生。
...
預設的 [載入器 (loader)](#loader) 將 [`Content-Disposition`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition#as_a_response_header_for_the_main_body) 標頭設定為 `attachment` 以增加保護，因為 API 可以提供任意的遠端圖片。

預設值為 `attachment`，這會強制瀏覽器在直接訪問時下載圖片。這在 [`dangerouslyAllowSVG`](#dangerously-allow-svg) 為 `true` 時尤其重要。
...
預設的 [載入器 (loader)](#loader) 會自動繞過動畫圖片的優化，並直接提供原始圖片。

對動畫檔案的自動檢測是盡力而為的，支援 GIF、APNG 和 WebP。如果您想明確繞過特定動畫圖片的優化，請使用 [unoptimized](#unoptimized) 屬性。
