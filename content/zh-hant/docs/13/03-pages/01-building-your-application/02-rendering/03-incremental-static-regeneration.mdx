...
translation-updated-at: 2025-05-25T21:37:41.557Z
title: 增量靜態再生 (ISR)
description: 了解如何使用增量靜態再生在運行時建立或更新靜態頁面。
---
...
> **須知事項**：目前 [`edge` 運行時](/docs/app/api-reference/edge) 與 ISR 不相容，但您可以透過手動設定 `cache-control` 標頭來利用 `stale-while-revalidate` 機制。
...
當請求一個尚未生成的路徑時，Next.js 會在首次請求時進行伺服器端渲染。後續請求將從快取中提供靜態檔案。在 Vercel 上，ISR [會全域持久化快取並處理回滾](https://vercel.com/docs/concepts/next.js/incremental-static-regeneration?utm_source=next-site&utm_medium=docs&utm_campaign=next-website)。
...
從 `v12.2.0` 開始，Next.js 支援按需增量靜態再生，可以手動清除特定頁面的 Next.js 快取。這讓您在以下情況更容易更新網站：

- 您的無頭 CMS 內容被建立或更新時
- 電子商務元資料變更時（價格、描述、類別、評論等）

在 `getStaticProps` 中，您不需要指定 `revalidate` 來使用按需重新驗證。如果省略 `revalidate`，Next.js 將使用預設值 `false`（不重新驗證），僅在呼叫 `revalidate()` 時按需重新驗證頁面。

> **須知事項**：[中介軟體](/docs/app/building-your-application/routing/middleware) 不會針對按需 ISR 請求執行。相反，您需要在想要重新驗證的 _確切_ 路徑上呼叫 `revalidate()`。例如，如果您有 `pages/blog/[slug].js` 和一個從 `/post-1` 重寫到 `/blog/post-1` 的路由，您需要呼叫 `res.revalidate('/blog/post-1')`。
...
接著，將該秘密新增為應用程式的 [環境變數](/docs/pages/building-your-application/configuring/environment-variables)。最後，建立重新驗證 API 路由：
...
當使用 `next dev` 在本機運行時，`getStaticProps` 會在每個請求上被呼叫。要驗證您的按需 ISR 配置是否正確，您需要建立一個 [生產版本](/docs/pages/api-reference/next-cli#build) 並啟動 [生產伺服器](/docs/pages/api-reference/next-cli#production)：
...
增量靜態再生 (ISR) 在 [自託管 Next.js 網站](/docs/pages/building-your-application/deploying#self-hosting) 上開箱即用，當您使用 `next start` 時即可使用。

當部署到容器編排系統如 [Kubernetes](https://kubernetes.io/) 或 [HashiCorp Nomad](https://www.nomadproject.io/) 時，可以使用此方法。預設情況下，生成的資源將儲存在每個 pod 的記憶體中。這意味著每個 pod 都有自己的靜態檔案副本。在該特定 pod 收到請求之前，可能會顯示過期資料。
...
| `v12.0.0` | 新增 [Bot-aware ISR fallback](https://nextjs.org/blog/next-12#bot-aware-isr-fallback)。 |
| `v9.5.0`  | 新增 Base Path 支援。                                                                |