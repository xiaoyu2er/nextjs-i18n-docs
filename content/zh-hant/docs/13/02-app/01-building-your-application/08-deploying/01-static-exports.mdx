...
translation-updated-at: 2025-05-25T21:46:25.178Z
title: 靜態匯出 (Static Exports)
description: Next.js 允許您從靜態網站或單頁應用程式 (SPA) 開始，之後可選擇性地升級使用需要伺服器的功能。
---
...
生成的元件將被渲染為初始頁面載入的靜態 HTML 和客戶端路由導航的靜態負載。使用靜態匯出時，除非伺服器元件使用了[動態伺服器功能](#不支援的功能)，否則無需對其進行任何更改。
...
如果您需要在客戶端進行資料獲取，可以使用客戶端元件搭配 [SWR](https://github.com/vercel/swr) 來快取請求。
...
- [使用 `getStaticPaths` 時的動態路由](/docs/app/building-your-application/routing/dynamic-routes)
- 使用 `next/link` 進行預取
- 預載入 JavaScript
- [動態導入](/docs/pages/building-your-application/optimizing/lazy-loading)
...
透過 `next/image` 進行的[圖片最佳化](/docs/app/building-your-application/optimizing/images)可以與靜態匯出一起使用，只需在 `next.config.js` 中定義自訂圖片載入器。例如，您可以使用 Cloudinary 等服務最佳化圖片：
...
客戶端元件在 `next build` 期間會被預渲染為 HTML。由於 [Web API](https://developer.mozilla.org/docs/Web/API) 如 `window`、`localStorage` 和 `navigator` 在伺服器上不可用，您需要確保僅在瀏覽器環境中安全地存取這些 API。例如：
...
需要 Node.js 伺服器或在構建過程中無法計算的動態邏輯的功能**不**支援：

<AppOnly>

- 使用 `dynamicParams: true` 的[動態路由](/docs/app/building-your-application/routing/dynamic-routes)
- 未使用 `generateStaticParams()` 的[動態路由](/docs/app/building-your-application/routing/dynamic-routes)
- 依賴 Request 的[路由處理器](/docs/app/building-your-application/routing/route-handlers)
- [Cookies](/docs/app/api-reference/functions/cookies)
- [重寫 (Rewrites)](/docs/app/api-reference/next-config-js/rewrites)
- [重新導向 (Redirects)](/docs/app/api-reference/next-config-js/redirects)
- [標頭 (Headers)](/docs/app/api-reference/next-config-js/headers)
...
- 使用預設 `loader` 的[圖片最佳化](/docs/app/building-your-application/optimizing/images)
...
- [重寫 (Rewrites)](/docs/pages/api-reference/next-config-js/rewrites)
- [重新導向 (Redirects)](/docs/pages/api-reference/next-config-js/redirects)
- [標頭 (Headers)](/docs/pages/api-reference/next-config-js/headers)
...
- 使用預設 `loader` 的[圖片最佳化](/docs/pages/building-your-application/optimizing/images)
