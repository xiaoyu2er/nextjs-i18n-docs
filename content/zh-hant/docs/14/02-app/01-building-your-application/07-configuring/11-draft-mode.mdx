...
當您的頁面從無頭式 CMS (headless CMS) 獲取資料時，靜態渲染 (static rendering) 非常有用。然而，當您在無頭式 CMS 上撰寫草稿並希望立即在頁面上預覽時，這種方式就不太理想。您會希望 Next.js 在**請求時** (request time) 而非建置時 (build time) 渲染這些頁面，並獲取草稿內容而非已發布的內容。您會希望 Next.js 僅針對這種特定情況切換到[動態渲染 (dynamic rendering)](/docs/app/building-your-application/rendering/server-components#dynamic-rendering)。
...
然後，發送請求到 `/api/disable-draft` 以呼叫路由處理器。如果使用 [`next/link`](/docs/app/api-reference/components/link) 呼叫此路由，您必須傳遞 `prefetch={false}` 以防止在預取時意外刪除 cookie。
