...
呼叫 `notFound()` 函式會拋出 `NEXT_NOT_FOUND` 錯誤，並終止所在路由區段的渲染。透過設定 [**not-found** 檔案](/docs/app/api-reference/file-conventions/not-found)，您可以在該區段內優雅地處理此類錯誤，並顯示「找不到頁面」的使用者介面。
