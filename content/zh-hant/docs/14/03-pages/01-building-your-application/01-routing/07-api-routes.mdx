...
> **須知**：如果您使用 App Router，可以使用 [伺服器元件 (Server Components)](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating) 或 [路由處理器 (Route Handlers)](/docs/app/building-your-application/routing/route-handlers) 來替代 API 路由。
...
- `res.revalidate(urlPath)` - [按需重新驗證頁面](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) 使用 `getStaticProps`。`urlPath` 必須是 `string`。
