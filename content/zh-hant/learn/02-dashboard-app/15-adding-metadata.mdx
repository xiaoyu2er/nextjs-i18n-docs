...
description: 學習如何在您的 Next.js 應用程式中添加元數據。
image: >-
  https://nextjs.org/api/learn-og?title=Adding%20Metadata&amp;chapter=15&amp;sig=db074432aa9780d5
---
...
元數據為什麼很重要？[](#why-is-metadata-important)
--------------------------------------------------------
...
*   **基於配置的 (Config-based)**: 在 `layout.js` 或 `page.js` 檔案中匯出 [靜態 `metadata` 物件](/docs/app/api-reference/functions/generate-metadata#metadata-object) 或 [動態 `generateMetadata` 函數](/docs/app/api-reference/functions/generate-metadata#generatemetadata-function)。
*   **基於檔案的 (File-based)**: Next.js 有多種專門用於元數據目的的特殊檔案：
    *   `favicon.ico`, `apple-icon.jpg` 和 `icon.jpg`: 用於別圖標和圖標
    *   `opengraph-image.jpg` 和 `twitter-image.jpg`: 用於社群媒體圖片
    *   `robots.txt`: 提供搜索引擎爬取說明
    *   `sitemap.xml`: 提供網站結構資訊

您可以靈活使用這些檔案來設定靜態元數據，也可以在專案中動態生成它們。

無論選擇哪種方式，Next.js 都會自動為您的頁面生成相關的 `<head>` 元素。

### 別圖標 (Favicon) 與 Open Graph 圖片 [](#favicon-and-open-graph-image)
...
### 頁面標題與描述 [](#page-title-and-descriptions)
...
練習：添加元數據 [](#practice-adding-metadata)
------------------------------------------------------
