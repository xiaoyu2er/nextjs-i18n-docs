...
description: 使用 React Server Actions 變更資料，並重新驗證 Next.js 快取。
image: >-
...
什麼是伺服器操作 (Server Actions)?[](#what-are-server-actions)
----------------------------------------------------
...
使用表單與伺服器操作 (Server Actions)[](#using-forms-with-server-actions)
-------------------------------------------------------------------
...
Next.js 搭配伺服器操作 (Server Actions)[](#nextjs-with-server-actions)
----------------------------------------------------------
...
### 1\. 建立新路由與表單[](#1-create-a-new-route-and-form)

首先，在 `/invoices` 資料夾中，新增一個名為 `/create` 的路由區段，並在其中建立 `page.tsx` 檔案：

![包含內層 create 資料夾與其中 page.tsx 檔案的 Invoices 資料夾](https://h8DxKfmAPhn8O0p3.public.blob.vercel-storage.com/learn/light/create-invoice-route.png)![包含內層 create 資料夾與其中 page.tsx 檔案的 Invoices 資料夾](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Flearn%2Fdark%2Fcreate-invoice-route.png&w=3840&q=75)
...
![包含麵包屑導航與表單的建立發票頁面](https://h8DxKfmAPhn8O0p3.public.blob.vercel-storage.com/learn/light/create-invoice-page.png)![包含麵包屑導航與表單的建立發票頁面](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Flearn%2Fdark%2Fcreate-invoice-page.png&w=1920&q=75)

### 2\. 建立伺服器操作 (Server Action)[](#2-create-a-server-action)
...
### 4\. 驗證與準備資料[](#4-validate-and-prepare-the-data)
...
#### 以美分為單位儲存數值[](#storing-values-in-cents)
...
### 1\. 建立包含發票 id 的動態路由區段[](#1-create-a-dynamic-route-segment-with-the-invoice-id)
...
![包含內層 [id] 資料夾與其中 edit 資料夾的 Invoices 資料夾](https://h8DxKfmAPhn8O0p3.public.blob.vercel-storage.com/learn/light/edit-invoice-route.png)![包含內層 [id] 資料夾與其中 edit 資料夾的 Invoices 資料夾](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Flearn%2Fdark%2Fedit-invoice-route.png&w=3840&q=75)
...
### 2\. 從頁面 params 中讀取發票 id[](#2-read-the-invoice-id-from-page-params)
...
![包含麵包屑導航與表單的更新發票頁面](https://h8DxKfmAPhn8O0p3.public.blob.vercel-storage.com/learn/light/edit-invoice-page.png)![包含麵包屑導航與表單的更新發票頁面](/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Flearn%2Fdark%2Fedit-invoice-page.png&w=1920&q=75)
...
### 4\. 將 id 傳遞給伺服器操作 (Server Action)[](#4-pass-the-id-to-the-server-action)
