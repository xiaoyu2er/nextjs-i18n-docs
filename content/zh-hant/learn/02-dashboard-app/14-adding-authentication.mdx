...
description: >-
  使用 NextAuth.js、Server Actions（伺服器動作）和 Proxy 保護您的儀表板路線。
image: >-
...
## 什麼是身分驗證？[](#what-is-authentication)
--------------------------------------------------
...
### 身分驗證 (Authentication) VS. 權限管理 (Authorization)[](#authentication-vs-authorization)
...
建立登入 (login) 路線 [](#creating-the-login-route)
-----------------------------------------------------
...
使用 Next.js Proxy 保護您的路線 [](#protecting-your-routes-with-nextjs-proxy)
--------------------------------------------------------------------------------------
...
`authorized` 回呼用於驗證請求是否有權使用 [Next.js Proxy](/docs/app/api-reference/file-conventions/proxy) 存取頁面。它在請求完成前被呼叫，接收一個包含 `auth` 和 `request` 屬性的物件。`auth` 屬性包含使用者的工作階段，`request` 屬性包含請求。
...
接下來，您需要將 `authConfig` 物件匯入 Proxy 檔案。在專案根目錄建立名為 `proxy.ts` 的檔案，並貼上以下程式碼：
...
此處您使用 `authConfig` 物件初始化 NextAuth.js 並匯出 `auth` 屬性。您也正在使用 Proxy 的 `matcher` 選項指定它应在特定路徑上執行。

使用 Proxy 執行此任務的優點是，受保護的路線甚至不會開始渲染，直到 Proxy 驗證身分驗證，增強您應用程式的安全性和效能。

### 密碼雜湊 (Password hashing)[](#password-hashing)

在將密碼儲存到資料庫之前進行 **雜湊** 是一個好習慣。雜湊將密碼轉換為固定長度的字元字串，看起來像隨機的，即使使用者的資料被洩露也能提供一層安全性。

當種子化您的資料庫時，您使用了一種名為 `bcrypt` 的套件來雜湊使用者密碼，然後儲存在資料庫中。您將在本章節稍後使用它來比較使用者輸入的密碼是否與資料庫中的密碼匹配。不過，您必須為 `bcrypt` 套件建立獨立檔案。這是因為 `bcrypt` 依賴的 Node.js API 在 Next.js Proxy 中無法使用。
...
### 新增 Credentials provider [](#adding-the-credentials-provider)
...
### 新增登入 (sign in) 功能 [](#adding-the-sign-in-functionality)
...
新增登出 (logout) 功能 [](#adding-the-logout-functionality)
-------------------------------------------------------------------
