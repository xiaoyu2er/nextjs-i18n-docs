...
`revalidatePath` permite purgar los [datos en caché](/docs/14/app/building-your-application/caching) bajo demanda para una ruta específica.

> **Importante**:
>
> - `revalidatePath` está disponible tanto en los [entornos de ejecución Node.js y Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> - Actualmente, `revalidatePath` invalida todas las rutas en la [caché del enrutador del lado del cliente (client-side Router Cache)](/docs/14/app/building-your-application/caching#router-cache). Este comportamiento es temporal y se actualizará en el futuro para aplicarse solo a la ruta específica.
> - Usar `revalidatePath` invalida **solo la ruta específica** en la [caché de ruta del lado del servidor (server-side Route Cache)](/docs/14/app/building-your-application/caching#full-route-cache).
