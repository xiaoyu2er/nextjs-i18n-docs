...
> **Es bueno saberlo**: `getInitialProps` es una API heredada. Recomendamos usar [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) o [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) en su lugar.
...
> - Para la carga inicial de la página, `getInitialProps` se ejecutará solo en el servidor. `getInitialProps` también se ejecutará en el cliente al navegar a una ruta diferente con el componente [`next/link`](/docs/14/pages/api-reference/components/link) o usando [`next/router`](/docs/14/pages/api-reference/functions/use-router).
...
- `getInitialProps` solo se puede usar en archivos de nivel superior en `pages/`, y no en componentes anidados. Para obtener datos anidados a nivel de componente, considera explorar el [App Router](/docs/14/app/building-your-application/data-fetching).
