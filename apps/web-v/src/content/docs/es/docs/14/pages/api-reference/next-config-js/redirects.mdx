...
translation-updated-at: 2025-06-06T16:39:49.530Z
title: Redirecciones
...
Cuando se usa el Enrutador de Páginas, las redirecciones no se aplican al enrutamiento del lado del cliente (`Link`, `router.push`) a menos que esté presente un [Middleware](/docs/14/app/building-your-application/routing/middleware) y coincida con la ruta.
...
Cuando se usa [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) con redirecciones, cada `source` y `destination` se prefija automáticamente con el `basePath` a menos que agregues `basePath: false` a la redirección:
...
Cuando se usa [`i18n`](/docs/14/app/building-your-application/routing/internationalization) con redirecciones, cada `source` y `destination` se prefija automáticamente para manejar los `locales` configurados a menos que agregues `locale: false` a la redirección. Si se usa `locale: false`, debes prefijar `source` y `destination` con un locale para que coincida correctamente.

</AppOnly>

<PagesOnly>

Cuando se usa [`i18n`](/docs/14/pages/building-your-application/routing/internationalization) con redirecciones, cada `source` y `destination` se prefija automáticamente para manejar los `locales` configurados a menos que agregues `locale: false` a la redirección. Si se usa `locale: false`, debes prefijar `source` y `destination` con un locale para que coincida correctamente.
...
- Dentro de [API Routes](/docs/14/pages/building-your-application/routing/api-routes) y [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers), puedes redirigir según la solicitud entrante.
- Dentro de [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) y [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props), puedes redirigir páginas específicas en tiempo de solicitud.
