...
translation-updated-at: 2025-06-06T17:32:20.345Z
title: opengraph-image y twitter-image
description: Referencia de API para las convenciones de archivos Open Graph Image y Twitter Image.
sidebar:
  order: 2020201
---
...
> **Nota importante**
>
> - Por defecto, las imágenes generadas están [**optimizadas estáticamente**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (generadas en tiempo de compilación y almacenadas en caché) a menos que usen [funciones dinámicas](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) o datos no cacheados.
> - Puede generar múltiples imágenes en el mismo archivo usando [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata).

La forma más fácil de generar una imagen es usando la API [ImageResponse](/docs/14/app/api-reference/functions/image-response) de `next/og`.
...
Un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) desde el segmento raíz hasta el segmento donde `opengraph-image` o `twitter-image` está ubicado.
...
`opengraph-image` y `twitter-image` son [Manejadores de Ruta](/docs/14/app/building-your-application/routing/route-handlers) especializados que pueden usar las mismas [opciones de configuración de segmento de ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) que las Páginas y Layouts.
...
> **Nota importante**:
> Por defecto, esta imagen generada estará [optimizada estáticamente](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). Puede configurar las [`opciones`](/docs/14/app/api-reference/functions/fetch) individuales de `fetch` o las [opciones](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) de segmentos de ruta para cambiar este comportamiento.
