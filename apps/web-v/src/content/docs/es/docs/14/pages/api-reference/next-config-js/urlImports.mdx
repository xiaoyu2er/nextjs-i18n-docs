...
Esta característica se está diseñando con **la seguridad como máxima prioridad**. Para empezar, añadimos una bandera experimental que te obliga a permitir explícitamente los dominios desde los que aceptas importaciones. Estamos trabajando para llevar esto más lejos limitando las importaciones desde URL para que se ejecuten en el sandbox del navegador usando el [Edge Runtime](/docs/14/app/api-reference/edge).

## Archivo de bloqueo (lockfile)

Cuando usas importaciones desde URL, Next.js creará un directorio `next.lock` que contiene un archivo de bloqueo y los recursos descargados.
Este directorio **debe ser confirmado en Git**, no ignorado por `.gitignore`.

- Al ejecutar `next dev`, Next.js descargará y añadirá todas las nuevas importaciones desde URL descubiertas a tu archivo de bloqueo
- Al ejecutar `next build`, Next.js usará solo el archivo de bloqueo para construir la aplicación para producción
