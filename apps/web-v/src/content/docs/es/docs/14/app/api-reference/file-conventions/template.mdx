...
Los componentes de plantilla deben aceptar y usar una prop `children`. El `template` se renderiza entre un [layout](/docs/14/app/api-reference/file-conventions/layout) y sus hijos. Por ejemplo:
...
> **Es bueno saber**:
>
> - Por defecto, `template` es un [Componente de Servidor (Server Component)](/docs/14/app/building-your-application/rendering/server-components), pero también puede usarse como [Componente de Cliente (Client Component)](/docs/14/app/building-your-application/rendering/client-components) mediante la directiva `"use client"`.
