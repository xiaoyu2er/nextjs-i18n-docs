...
- `props` - Un objeto que contiene los parámetros de la ruta actual:
  ...
  - `params` - Un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) desde el segmento raíz hasta el segmento donde se llama a `generateMetadata`. Ejemplos:
...
> - Las solicitudes `fetch` se [memorizan](/docs/14/app/building-your-application/caching#request-memoization) automáticamente para los mismos datos en `generateMetadata`, `generateStaticParams`, Layouts, Pages y Server Components. Se puede usar [`cache` de React](/docs/14/app/building-your-application/caching#request-memoization) si `fetch` no está disponible.
> - `searchParams` solo está disponible en segmentos `page.js`.
> - Los métodos [`redirect()`](/docs/14/app/api-reference/functions/redirect) y [`notFound()`](/docs/14/app/api-reference/functions/not-found) de Next.js también se pueden usar dentro de `generateMetadata`.
...
> **Obsoleto**: La opción `themeColor` en `metadata` está obsoleta a partir de Next.js 14. Por favor, use la [configuración `viewport`](/docs/14/app/api-reference/functions/generate-viewport) en su lugar.
...
> **Obsoleto**: La opción `viewport` en `metadata` está obsoleta a partir de Next.js 14. Por favor, use la [configuración `viewport`](/docs/14/app/api-reference/functions/generate-viewport) en su lugar.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | Use encabezados HTTP apropiados mediante [`redirect()`](/docs/14/app/api-reference/functions/redirect), [Middleware](/docs/14/app/building-your-application/routing/middleware#nextresponse), o [Encabezados de Seguridad](/docs/14/app/api-reference/next-config-js/headers) |
| `<base>`                      | Renderice la etiqueta en el layout o la página misma.                                                                                                                                                                                                       |
| `<noscript>`                  | Renderice la etiqueta en el layout o la página misma.                                                                                                                                                                                                       |
| `<style>`                     | Más información sobre [estilos en Next.js](/docs/14/app/building-your-application/styling/css-modules).                                                                                                                                                    |
| `<script>`                    | Más información sobre [uso de scripts](/docs/14/app/building-your-application/optimizing/scripts).                                                                                                                                                          |
...
Puede agregar seguridad de tipos a sus metadatos usando el tipo `Metadata`. Si está utilizando el [plugin de TypeScript incorporado](/docs/14/app/building-your-application/configuring/typescript) en su IDE, no necesita agregar manualmente el tipo, pero aún puede hacerlo explícitamente si lo desea.
...
| Versión   | Cambios                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.2.0` | `viewport`, `themeColor`, y `colorScheme` obsoletos en favor de la [configuración `viewport`](/docs/14/app/api-reference/functions/generate-viewport). |
