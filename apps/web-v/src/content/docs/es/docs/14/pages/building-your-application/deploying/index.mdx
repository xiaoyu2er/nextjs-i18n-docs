...
Dado que Next.js soporta esta [exportación estática](/docs/14/app/building-your-application/deploying/static-exports), puede desplegarse y alojarse en cualquier servidor web que pueda servir activos estáticos HTML/CSS/JS. Esto incluye herramientas como AWS S3, Nginx o Apache.

Ejecutarse como una [exportación estática](/docs/14/app/building-your-application/deploying/static-exports) no soporta características de Next.js que requieran un servidor. [Aprenda más](/docs/14/app/building-your-application/deploying/static-exports#unsupported-features).

> **Nota importante:**
>
> - [Componentes del Servidor](/docs/14/app/building-your-application/rendering/server-components) son compatibles con exportaciones estáticas.

## Características

### Optimización de Imágenes

[Optimización de Imágenes](/docs/14/app/building-your-application/optimizing/images) a través de `next/image` funciona autoalojado sin configuración al desplegar usando `next start`. Si prefiere tener un servicio separado para optimizar imágenes, puede [configurar un cargador de imágenes](/docs/14/app/building-your-application/optimizing/images#loaders).

La optimización de imágenes puede usarse con una [exportación estática](/docs/14/app/building-your-application/deploying/static-exports#image-optimization) definiendo un cargador de imágenes personalizado en `next.config.js`. Tenga en cuenta que las imágenes se optimizan en tiempo de ejecución, no durante la construcción.

> **Nota importante:**
>
> - Al autoalojar, considere instalar `sharp` para una [Optimización de Imágenes](/docs/14/pages/building-your-application/optimizing/images) más eficiente en su entorno de producción ejecutando `npm install sharp` en su directorio de proyecto. En plataformas Linux, `sharp` puede requerir [configuración adicional](https://sharp.pixelplumbing.com/install#linux-memory-allocator) para evitar uso excesivo de memoria.
> - Aprenda más sobre el [comportamiento de caché de imágenes optimizadas](/docs/14/app/api-reference/components/image#caching-behavior) y cómo configurar el TTL.
> - También puede [desactivar la Optimización de Imágenes](/docs/14/app/api-reference/components/image#unoptimized) y conservar otros beneficios de usar `next/image` si lo prefiere. Por ejemplo, si está optimizando imágenes por separado.

### Middleware

[Middleware](/docs/14/app/building-your-application/routing/middleware) funciona autoalojado sin configuración al desplegar usando `next start`. Dado que requiere acceso a la solicitud entrante, no es compatible cuando se usa una [exportación estática](/docs/14/app/building-your-application/deploying/static-exports).

Middleware usa un [tiempo de ejecución](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes) que es un subconjunto de todas las API disponibles de Node.js para ayudar a garantizar baja latencia, ya que puede ejecutarse frente a cada ruta o activo en su aplicación. Este tiempo de ejecución no requiere ejecutarse "en el borde" y funciona en un servidor de una sola región. Se requiere configuración e infraestructura adicionales para ejecutar Middleware en múltiples regiones.

Si busca agregar lógica (o usar un paquete externo) que requiera todas las API de Node.js, podría mover esta lógica a un [layout](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) como un [Componente del Servidor](/docs/14/app/building-your-application/rendering/server-components). Por ejemplo, verificando [encabezados](/docs/14/app/api-reference/functions/headers) y [redireccionando](/docs/14/app/api-reference/functions/redirect). También puede usar encabezados, cookies o parámetros de consulta para [redireccionar](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching) o [reescribir](/docs/14/app/api-reference/next-config-js/rewrites#header-cookie-and-query-matching) a través de `next.config.js`. Si eso no funciona, también puede usar un [servidor personalizado](/docs/14/pages/building-your-application/configuring/custom-server).
...
> **Nota importante:**
>
> - Puede ejecutar código al iniciar el servidor usando la [función `register`](/docs/14/app/building-your-application/optimizing/instrumentation).
> - No recomendamos usar la opción [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration), ya que no funciona con el modo de salida independiente. En su lugar, recomendamos [adoptar incrementalmente](/docs/14/app/building-your-application/upgrading/app-router-migration) el Enrutador de Aplicaciones.
...
El Enrutador de Aplicaciones de Next.js soporta [respuestas de streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) cuando se autoalojan. Si está usando Nginx o un proxy similar, necesitará configurarlo para desactivar el buffering y habilitar el streaming.
