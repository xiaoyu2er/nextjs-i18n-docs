...
sidebar:
  order: 2010201
  label: "Obtención, caché y revalidación"
---
...
Next.js extiende la [API nativa `fetch` de Web](https://developer.mozilla.org/docs/Web/API/Fetch_API) para permitirte configurar el comportamiento de [almacenamiento en caché](#almacenamiento-en-caché-de-datos) y [revalidación](#revalidación-de-datos) para cada solicitud fetch en el servidor. React extiende `fetch` para [memorizar automáticamente](/docs/14/app/building-your-application/data-fetching/patterns#fetching-data-where-its-needed) las solicitudes fetch al renderizar un árbol de componentes de React.

Puedes usar `fetch` con `async`/`await` en Componentes de Servidor, en [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) y en [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
> **Nota importante**:
>
> - Next.js proporciona funciones útiles que puedes necesitar al obtener datos en Componentes de Servidor, como [`cookies`](/docs/14/app/api-reference/functions/cookies) y [`headers`](/docs/14/app/api-reference/functions/headers). Estas harán que la ruta se renderice dinámicamente ya que dependen de información en tiempo de solicitud.
...
> - En [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations), las solicitudes fetch no se almacenan en caché (valor predeterminado `cache: no-store`).
...
Por defecto, Next.js almacena automáticamente en caché los valores devueltos por `fetch` en la [Caché de Datos](/docs/14/app/building-your-application/caching#data-cache) del servidor. Esto significa que los datos se pueden obtener en tiempo de compilación o solicitud, almacenar en caché y reutilizar en cada solicitud de datos.
...
- Se usan dentro de una [Server Action](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
- Se usan dentro de un [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) que utiliza el método `POST`.
...
>
> Aprende más sobre la [Caché de Datos](/docs/14/app/building-your-application/caching#data-cache).
...
Alternativamente, para revalidar todas las solicitudes `fetch` en un segmento de ruta, puedes usar las [Opciones de Configuración de Segmento](/docs/14/app/api-reference/file-conventions/route-segment-config).
...
Aprende más sobre la [revalidación basada en tiempo](/docs/14/app/building-your-application/caching#time-based-revalidation).

#### Revalidación bajo demanda

Los datos se pueden revalidar bajo demanda por ruta ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) o por etiqueta de caché ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)) dentro de una [Server Action](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) o [Route Handler](/docs/14/app/building-your-application/routing/route-handlers).
...
Aprende más sobre la [revalidación bajo demanda](/docs/14/app/building-your-application/caching#on-demand-revalidation).
...
Consulta todas las opciones disponibles de `cache` en la [referencia de la API `fetch`](/docs/14/app/api-reference/functions/fetch).

#### Múltiples solicitudes `fetch`

Si tienes múltiples solicitudes `fetch` en un segmento de ruta (por ejemplo, un Layout o Page), puedes configurar el comportamiento de almacenamiento en caché de todas las solicitudes de datos en el segmento usando las [Opciones de Configuración de Segmento](/docs/14/app/api-reference/file-conventions/route-segment-config).
...
En casos donde uses una biblioteca de terceros que no soporte o exponga `fetch` (por ejemplo, un cliente de base de datos, CMS u ORM), puedes configurar el comportamiento de almacenamiento en caché y revalidación de esas solicitudes usando la [Opción de Configuración de Segmento de Ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) y la función `cache` de React.

Si los datos se almacenan en caché o no dependerá de si el segmento de ruta se [renderiza estática o dinámicamente](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies). Si el segmento es estático (predeterminado), la salida de la solicitud se almacenará en caché y se revalidará como parte del segmento de ruta. Si el segmento es dinámico, la salida de la solicitud _no_ se almacenará en caché y se volverá a obtener en cada solicitud cuando se renderice el segmento.

También puedes usar la API experimental [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache).

### Ejemplo

En el siguiente ejemplo:

- La función `cache` de React se usa para [memorizar](/docs/14/app/building-your-application/caching#request-memoization) solicitudes de datos.
...
Si necesitas obtener datos en un componente cliente, puedes llamar a un [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) desde el cliente. Los Route Handlers se ejecutan en el servidor y devuelven los datos al cliente. Esto es útil cuando no quieres exponer información sensible al cliente, como tokens de API.

Consulta la documentación de [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) para ver ejemplos.
