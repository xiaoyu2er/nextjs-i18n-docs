...
Next.js 13 introdujo el nuevo [App Router](/docs/14/app/building-your-application/routing) con nuevas características y convenciones. El nuevo Router está disponible en el directorio `app` y coexiste con el directorio `pages`.

Actualizar a Next.js 13 **no** requiere usar el nuevo [App Router](/docs/14/app/building-your-application/routing#the-app-router). Puedes seguir usando `pages` con nuevas características que funcionan en ambos directorios, como el componente [Image](#image-component) actualizado, el componente [Link](#link-component), el componente [Script](#script-component) y la [optimización de fuentes](#font-optimization).
...
- [**Codemod `next-image-to-legacy-image`**](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image): Renombra de forma segura y automática las importaciones de `next/image` a `next/legacy/image`. Los componentes existentes mantendrán el mismo comportamiento.
- [**Codemod `next-image-experimental`**](/docs/14/app/building-your-application/upgrading/codemods#next-image-experimental): Agrega peligrosamente estilos en línea y elimina props no utilizados. Esto cambiará el comportamiento de los componentes existentes para que coincidan con los nuevos valores predeterminados. Para usar este codemod, primero debes ejecutar `next-image-to-legacy-image`.

### Componente `<Link>`

El [componente `<Link>`](/docs/14/app/building-your-application/routing/linking-and-navigating#link-component) ya no requiere agregar manualmente una etiqueta `<a>` como hijo. Este comportamiento se agregó como opción experimental en la [versión 12.2](https://nextjs.org/blog/next-12-2) y ahora es el predeterminado. En Next.js 13, `<Link>` siempre renderiza `<a>` y te permite pasar props a la etiqueta subyacente.
...
Para actualizar tus enlaces a Next.js 13, puedes usar el [codemod `new-link`](/docs/14/app/building-your-application/upgrading/codemods#new-link).

### Componente `<Script>`

El comportamiento de [`next/script`](/docs/14/app/api-reference/components/script) se ha actualizado para soportar tanto `pages` como `app`, pero se necesitan algunos cambios para garantizar una migración fluida:
...
- Los manejadores `onLoad`, `onReady` y `onError` no funcionarán en Componentes del Servidor, así que asegúrate de moverlos a un [Componente del Cliente](/docs/14/app/building-your-application/rendering/server-components) o eliminarlos por completo.

### Optimización de fuentes

Anteriormente, Next.js te ayudaba a optimizar fuentes [incrustando CSS de fuentes](/docs/14/app/building-your-application/optimizing/fonts). La versión 13 introduce el nuevo módulo [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) que te permite personalizar tu experiencia de carga de fuentes mientras garantiza un gran rendimiento y privacidad. `next/font` es compatible tanto con los directorios `pages` como `app`.

Si bien [incrustar CSS](/docs/14/app/building-your-application/optimizing/fonts) todavía funciona en `pages`, no funciona en `app`. Debes usar [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) en su lugar.

Consulta la página [Optimización de fuentes](/docs/14/app/building-your-application/optimizing/fonts) para aprender a usar `next/font`.
...
Mudarse al App Router puede ser la primera vez que uses características de React en las que Next.js se basa, como Componentes del Servidor, Suspense y más. Cuando se combinan con nuevas características de Next.js como [archivos especiales](/docs/14/app/building-your-application/routing#file-conventions) y [diseños](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts), la migración implica nuevos conceptos, modelos mentales y cambios de comportamiento que aprender.
...
- El directorio `app` admite rutas anidadas _y_ diseños. [Aprende más](/docs/14/app/building-your-application/routing).
- Usa carpetas anidadas para [definir rutas](/docs/14/app/building-your-application/routing/defining-routes) y un archivo especial `page.js` para hacer que un segmento de ruta sea accesible públicamente. [Aprende más](#step-4-migrating-pages).
- Se usan [convenciones de archivos especiales](/docs/14/app/building-your-application/routing#file-conventions) para crear la interfaz de usuario para cada segmento de ruta. Los archivos especiales más comunes son `page.js` y `layout.js`.
...
- Puedes colocar otros archivos dentro del directorio `app`, como componentes, estilos, pruebas y más. [Aprende más](/docs/14/app/building-your-application/routing).
- Las funciones de obtención de datos como `getServerSideProps` y `getStaticProps` han sido reemplazadas por [una nueva API](/docs/14/app/building-your-application/data-fetching) dentro de `app`. `getStaticPaths` ha sido reemplazado por [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params).
- `pages/_app.js` y `pages/_document.js` han sido reemplazados por un único diseño raíz `app/layout.js`. [Aprende más](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required).
- `pages/_error.js` ha sido reemplazado por archivos especiales `error.js` más granulares. [Aprende más](/docs/14/app/building-your-application/routing/error-handling).
- `pages/404.js` ha sido reemplazado por el archivo [`not-found.js`](/docs/14/app/api-reference/file-conventions/not-found).
- Las Rutas API `pages/api/*` han sido reemplazadas por el archivo especial [`route.js`](/docs/14/app/api-reference/file-conventions/route) (Manejador de Ruta).
...
Crea un nuevo archivo `app/layout.tsx` dentro del directorio `app`. Este es un [diseño raíz](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required) que se aplicará a todas las rutas dentro de `app`.
...
Para manejar elementos HTML `<head>`, puedes usar el [soporte SEO incorporado](/docs/14/app/building-your-application/optimizing/metadata):
...
Si estás usando proveedores de contexto de React, deberán moverse a un [Componente del Cliente](/docs/14/app/building-your-application/rendering/client-components).

#### Migrando el patrón `getLayout()` a Diseños (Opcional)

Next.js recomendaba agregar una [propiedad a los componentes de página](/docs/14/pages/building-your-application/routing/pages-and-layouts#layout-pattern#per-page-layouts) para lograr diseños por página en el directorio `pages`. Este patrón puede reemplazarse con el soporte nativo para [diseños anidados](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) en el directorio `app`.
...
- Mueve el contenido de `DashboardLayout` a un nuevo [Componente del Cliente](/docs/14/app/building-your-application/rendering/client-components) para conservar el comportamiento del directorio `pages`.
...
En el directorio `pages`, el componente React `next/head` se usa para manejar elementos HTML `<head>` como `title` y `meta`. En el directorio `app`, `next/head` se reemplaza con el nuevo [soporte SEO incorporado](/docs/14/app/building-your-application/optimizing/metadata).
...
[Consulta todas las opciones de metadata](/docs/14/app/api-reference/functions/generate-metadata).

### Paso 4: Migración de páginas

- Las páginas en el [directorio `app`](/docs/14/app/building-your-application/routing) son [Componentes del Servidor (Server Components)](/docs/14/app/building-your-application/rendering/server-components) por defecto. Esto es diferente del directorio `pages`, donde las páginas son [Componentes del Cliente (Client Components)](/docs/14/app/building-your-application/rendering/client-components).
- La [obtención de datos (Data fetching)](/docs/14/app/building-your-application/data-fetching) ha cambiado en `app`. `getServerSideProps`, `getStaticProps` y `getInitialProps` han sido reemplazados por una API más simple.
- El directorio `app` utiliza carpetas anidadas para [definir rutas (defining routes)](/docs/14/app/building-your-application/routing/defining-routes) y un archivo especial `page.js` para hacer accesible un segmento de ruta públicamente.
...
- Crea un nuevo archivo separado dentro del directorio `app` (ej. `app/home-page.tsx` o similar) que exporte un Componente del Cliente. Para definir Componentes del Cliente, añade la directiva `'use client'` al inicio del archivo (antes de cualquier importación).
  - Similar al Enrutador de Páginas (Pages Router), hay un [paso de optimización](/docs/14/app/building-your-application/rendering/client-components#full-page-load) para prerenderizar Componentes del Cliente a HTML estático en la carga inicial de la página.
...
- Si estabas obteniendo datos en `pages/index.js`, mueve la lógica de obtención de datos directamente al Componente del Servidor usando las nuevas [APIs de obtención de datos (data fetching APIs)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating). Consulta la [guía de actualización de obtención de datos](#step-6-migrating-data-fetching-methods) para más detalles.
...
- Si tu página anterior usaba `useRouter`, necesitarás actualizar a los nuevos hooks de enrutamiento. [Aprende más](/docs/14/app/api-reference/functions/use-router).
...
En `app`, deberías usar los tres nuevos hooks importados desde `next/navigation`: [`useRouter()`](/docs/14/app/api-reference/functions/use-router), [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname), y [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params).
...
- El nuevo hook `useRouter` se importa desde `next/navigation` y tiene un comportamiento diferente al hook `useRouter` en `pages`, que se importa desde `next/router`.
  - El [hook `useRouter` importado desde `next/router`](/docs/14/pages/api-reference/functions/use-router) no es compatible con el directorio `app` pero puede seguir usándose en el directorio `pages`.
...
- Puedes usar `useSearchParams` y `usePathname` juntos para escuchar cambios de página. Consulta la sección [Eventos del Enrutador (Router Events)](/docs/14/app/api-reference/functions/use-router#router-events) para más detalles.
...
- Los valores `locale`, `locales`, `defaultLocales`, `domainLocales` han sido eliminados porque las características integradas de i18n de Next.js ya no son necesarias en el directorio `app`. [Aprende más sobre i18n](/docs/14/app/building-your-application/routing/internationalization).
...
- `isReady` ha sido eliminado porque ya no es necesario. Durante el [renderizado estático (static rendering)](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default), cualquier componente que use el hook [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) omitirá el paso de prerenderizado y en su lugar se renderizará en el cliente en tiempo de ejecución.

[Consulta la referencia de la API de `useRouter()`](/docs/14/app/api-reference/functions/use-router).

### Paso 6: Migración de Métodos de Obtención de Datos

El directorio `pages` usa `getServerSideProps` y `getStaticProps` para obtener datos para las páginas. Dentro del directorio `app`, estas funciones anteriores de obtención de datos son reemplazadas por una [API más simple](/docs/14/app/building-your-application/data-fetching) construida sobre `fetch()` y Componentes del Servidor React asíncronos.
...
En el directorio `app`, podemos colocar nuestra obtención de datos dentro de nuestros componentes React usando [Componentes del Servidor (Server Components)](/docs/14/app/building-your-application/rendering/server-components). Esto nos permite enviar menos JavaScript al cliente, manteniendo el HTML renderizado desde el servidor.

Al establecer la opción `cache` en `no-store`, podemos indicar que los datos obtenidos [nunca deben almacenarse en caché](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating). Esto es similar a `getServerSideProps` en el directorio `pages`.
...
- [`headers()`](/docs/14/app/api-reference/functions/headers): Basado en la API Web Headers, y puede usarse dentro de [Componentes del Servidor (Server Components)](/docs/14/app/building-your-application/rendering/server-components) para recuperar cabeceras de solicitud.
- [`cookies()`](/docs/14/app/api-reference/functions/cookies): Basado en la API Web Cookies, y puede usarse dentro de [Componentes del Servidor (Server Components)](/docs/14/app/building-your-application/rendering/server-components) para recuperar cookies.
...
En el directorio `app`, la obtención de datos con [`fetch()`](/docs/14/app/api-reference/functions/fetch) tendrá por defecto `cache: 'force-cache'`, lo que almacenará en caché los datos de solicitud hasta que se invaliden manualmente. Esto es similar a `getStaticProps` en el directorio `pages`.
...
En el directorio `app`, `getStaticPaths` se reemplaza con [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params).

[`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) se comporta de manera similar a `getStaticPaths`, pero tiene una API simplificada para devolver parámetros de ruta y puede usarse dentro de [layouts](/docs/14/app/building-your-application/routing/pages-and-layouts). La forma de retorno de `generateStaticParams` es un array de segmentos en lugar de un array de objetos `param` anidados o una cadena de rutas resueltas.
...
En el directorio `app`, la propiedad [`config.dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) controla cómo se manejan los parámetros fuera de [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params):
...
Con [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) establecido en `true` (el valor predeterminado), cuando se solicita un segmento de ruta que no ha sido generado, se renderizará en el servidor y se almacenará en caché.
...
En el directorio `app`, la obtención de datos con [`fetch()`](/docs/14/app/api-reference/functions/fetch) puede usar `revalidate`, que almacenará en caché la solicitud durante la cantidad de segundos especificada.
...
Las Rutas API continúan funcionando en el directorio `pages/api` sin cambios. Sin embargo, han sido reemplazadas por [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) en el directorio `app`.
...
> **Nota importante**: Si anteriormente usaba rutas API para llamar a una API externa desde el cliente, ahora puede usar [Server Components](/docs/14/app/building-your-application/rendering/server-components) en su lugar para obtener datos de forma segura. Más información sobre [obtención de datos](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating).
...
- [Módulos CSS](/docs/14/app/building-your-application/styling/css-modules)
- [Tailwind CSS](/docs/14/app/building-your-application/styling/tailwind-css)
- [Estilos globales](/docs/14/app/building-your-application/styling/css-modules#global-styles)
- [CSS-in-JS](/docs/14/app/building-your-application/styling/css-in-js)
- [Hojas de estilo externas](/docs/14/app/building-your-application/styling/css-modules#external-stylesheets)
...
Más información sobre [estilos con Tailwind CSS](/docs/14/app/building-your-application/styling/tailwind-css)

## Codemods

Next.js proporciona transformaciones Codemod para ayudar a actualizar su base de código cuando una función queda obsoleta. Consulte [Codemods](/docs/14/app/building-your-application/upgrading/codemods) para más información.
