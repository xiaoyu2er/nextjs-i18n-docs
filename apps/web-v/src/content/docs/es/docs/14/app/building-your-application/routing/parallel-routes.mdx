...
translation-updated-at: 2025-06-06T17:04:36.222Z
...
<Image
  alt="Estructura de archivos de Rutas Paralelas"
...
Sin embargo, los slots **no** son [segmentos de ruta](/docs/14/app/building-your-application/routing#route-segments) y no afectan la estructura de la URL. Por ejemplo, para `/@analytics/views`, la URL será `/views` ya que `@analytics` es un slot.
...
Por defecto, Next.js realiza un seguimiento del _estado_ activo (o subpágina) para cada slot. Sin embargo, el contenido renderizado dentro de un slot dependerá del tipo de navegación:

- [**Navegación suave**](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation): Durante la navegación del lado del cliente, Next.js realizará un [renderizado parcial](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering), cambiando la subpágina dentro del slot, mientras mantiene las subpáginas activas de los otros slots, incluso si no coinciden con la URL actual.
- **Navegación dura**: Después de una carga completa de página (actualización del navegador), Next.js no puede determinar el estado activo para los slots que no coinciden con la URL actual. En su lugar, renderizará un archivo [`default.js`](#defaultjs) para los slots no coincidentes, o `404` si `default.js` no existe.
...
Tanto [`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) como [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) aceptan un parámetro `parallelRoutesKey`, que te permite leer el segmento de ruta activo dentro de un slot.
...
<Image
  alt="Slot analytics con dos subpáginas y un layout"
...
Dentro de `@analytics`, crea un archivo [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts) para compartir las pestañas entre las dos páginas:
...
Las rutas paralelas pueden usarse junto con [Rutas de interceptación](/docs/14/app/building-your-application/routing/intercepting-routes) para crear modales. Esto te permite resolver desafíos comunes al construir modales, como:
...
Luego, dentro del slot `@auth`, agrega un archivo [`default.js`](/docs/14/app/api-reference/file-conventions/default) que devuelva `null`. Esto asegura que el modal no se renderice cuando no está activo.
...
> **Nota importante:**
>
> - La convención usada para interceptar la ruta, ej. `(.)`, depende de la estructura de tu sistema de archivos. Consulta [Convención de Rutas de Interceptación](/docs/14/app/building-your-application/routing/intercepting-routes#convention).
> - Al separar la funcionalidad del `<Modal>` del contenido del modal (`<Login>`), puedes asegurar que cualquier contenido dentro del modal, ej. [formularios](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations#forms), sean Componentes de Servidor. Consulta [Intercalar Componentes de Cliente y Servidor](/docs/14/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props) para más información.
...
Consulta la documentación de [UI de carga](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) y [Manejo de errores](/docs/14/app/building-your-application/routing/error-handling) para más información.