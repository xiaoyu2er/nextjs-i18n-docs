...
Next.js proporciona compatibilidad total con versiones anteriores para aplicaciones que tienen [configuración personalizada de Babel](/docs/14/pages/building-your-application/configuring/babel). Todas las transformaciones que Next.js maneja por defecto, como styled-jsx y la eliminación de código muerto (tree-shaking) de `getStaticProps` / `getStaticPaths` / `getServerSideProps`, se han portado a Rust.
...
Consulte la [documentación](/docs/14/pages/building-your-application/optimizing/images#styling) para más información.
...
Si actualmente está utilizando la opción `target` configurada como `serverless`, lea la [documentación sobre cómo aprovechar el nuevo output](/docs/14/pages/api-reference/next-config-js/output).