...
Consulte la **nueva** [Referencia de API de `next/image`](/docs/14/pages/api-reference/components/image)
...
- Un archivo de imagen [importado estáticamente](/docs/14/pages/building-your-application/optimizing/images#local-images)
...
La propiedad `width` es requerida, excepto para [imágenes importadas estáticamente](/docs/14/pages/building-your-application/optimizing/images#local-images), o aquellas con `layout="fill"`.
...
La propiedad `height` es requerida, excepto para [imágenes importadas estáticamente](/docs/14/pages/building-your-application/optimizing/images#local-images), o aquellas con `layout="fill"`.
...
Cuando es `blur`, se usará la propiedad [`blurDataURL`](#blurdataurl) como marcador de posición. Si `src` es un objeto de una [importación estática](/docs/14/pages/building-your-application/optimizing/images#local-images) y la imagen importada es `.jpg`, `.png`, `.webp`, o `.avif`, entonces `blurDataURL` se llenará automáticamente.
...
> Las imágenes no pueden optimizarse en tiempo de compilación usando [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports), solo bajo demanda. Para usar `next/legacy/image` con `output: 'export'`, necesitará usar un loader diferente al predeterminado. [Más información en la discusión.](https://github.com/vercel/next.js/discussions/19065)
...
Puede configurar el Time to Live (TTL) en segundos para imágenes optimizadas en caché. En muchos casos, es mejor usar una [Importación de Imagen Estática](/docs/14/pages/building-your-application/optimizing/images#local-images) que automáticamente hashear los contenidos del archivo y almacenar la imagen para siempre con una cabecera `Cache-Control` de `immutable`.
...
Si necesita cambiar el comportamiento de caché por imagen, puede configurar [`headers`](/docs/14/pages/api-reference/next-config-js/headers) para establecer la cabecera `Cache-Control` en la imagen upstream (ej. `/some-asset.jpg`, no en `/_next/image` mismo).
...
El [loader](#loader) predeterminado no optimiza imágenes SVG por varias razones. Primero, SVG es un formato vectorial lo que significa que puede redimensionarse sin pérdida. Segundo, SVG tiene muchas de las mismas características que HTML/CSS, lo que puede llevar a vulnerabilidades sin las adecuadas [cabeceras de Content Security Policy (CSP)](/docs/14/app/api-reference/next-config-js/headers#content-security-policy).
