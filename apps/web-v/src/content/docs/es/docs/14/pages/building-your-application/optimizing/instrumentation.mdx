...
Para configurar la instrumentación, crea un archivo `instrumentation.ts|js` en el **directorio raíz** de tu proyecto (o dentro de la carpeta [`src`](/docs/14/app/building-your-application/configuring/src-directory) si estás usando una).
...
> **Importante**
>
> - Esta característica es **experimental**. Para usarla, debes activarla explícitamente definiendo [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) en tu `next.config.js`.
...
> - Si usas la opción de configuración [`pageExtensions`](/docs/14/app/api-reference/next-config-js/pageExtensions) para agregar un sufijo, también necesitarás actualizar el nombre del archivo `instrumentation` para que coincida.
...
Next.js llama a `register` en todos los entornos, por lo que es importante importar condicionalmente cualquier código que no sea compatible con entornos específicos (como [Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime) o [Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime)). Puedes usar la variable de entorno `NEXT_RUNTIME` para obtener el entorno actual:
