...
translation-updated-at: 2025-06-06T16:46:53.397Z
title: Enrutamiento de internacionalización (i18n)
nav_title: Internacionalización
description: Next.js tiene soporte integrado para enrutamiento internacionalizado y detección de idiomas. Aprende más aquí.
sidebar:
  order: 3010110
...
module.exports = {
  i18n: {
    ...
    // Este es el locale predeterminado que se usará al visitar
    // una ruta sin prefijo de locale, ej. `/hello`
    defaultLocale: 'en-US',
    // Esta es una lista de dominios de locale y el locale predeterminado que
    // deben manejar (solo son necesarios al configurar enrutamiento por dominio)
...
Al usar el enrutamiento por dominio, puedes configurar locales para que se sirvan desde diferentes dominios:
...
Al usar Enrutamiento por Dominio, si un usuario con el encabezado `Accept-Language` `fr;q=0.9` visita `example.com`, será redirigido a `example.fr` ya que ese dominio maneja el locale `fr` por defecto.

Al usar Enrutamiento por Sub-ruta, el usuario sería redirigido a `/fr`.

### Prefijar el Locale Predeterminado

Con Next.js 12 y [Middleware](/docs/14/pages/building-your-application/routing/middleware), podemos agregar un prefijo al locale predeterminado con una [solución alternativa](https://github.com/vercel/next.js/discussions/18419).
...
Luego, podemos usar [Middleware](/docs/14/pages/building-your-application/routing/middleware) para agregar reglas de enrutamiento personalizadas:
...
Este [Middleware](/docs/14/pages/building-your-application/routing/middleware) omite agregar el prefijo predeterminado a [API Routes](/docs/14/pages/building-your-application/routing/api-routes) y archivos [públicos](/docs/14/pages/building-your-application/optimizing/static-assets) como fuentes o imágenes. Si se hace una solicitud al locale predeterminado, redirigimos a nuestro prefijo `/en`.
...
Cuando `localeDetection` está configurado como `false`, Next.js ya no redirigirá automáticamente según el locale preferido del usuario y solo proporcionará información de locale detectada desde el dominio basado en locale o la ruta de locale como se describió anteriormente.

## Accediendo a la información del locale

Puedes acceder a la información del locale a través del enrutador de Next.js. Por ejemplo, usando el hook [`useRouter()`](/docs/14/pages/api-reference/functions/use-router) están disponibles las siguientes propiedades:
...
Al [pre-renderizar](/docs/14/pages/building-your-application/rendering/static-site-generation) páginas con `getStaticProps` o `getServerSideProps`, la información del locale se proporciona en [el contexto](/docs/14/pages/building-your-application/data-fetching/get-static-props) proporcionado a la función.
...
Al usar los métodos de `next/router` directamente, puedes especificar el `locale` que debe usarse a través de las opciones de transición. Por ejemplo:
...
Nota que para manejar el cambio solo del `locale` manteniendo toda la información de enrutamiento como valores de consulta de [rutas dinámicas](/docs/14/pages/building-your-application/routing/dynamic-routes) o valores de consulta ocultos de href, puedes proporcionar el parámetro `href` como un objeto:
...
Consulta [aquí](/docs/14/pages/api-reference/functions/use-router#with-url-object) para más información sobre la estructura del objeto para `router.push`.
...
Next.js admite anular el encabezado accept-language con una cookie `NEXT_LOCALE=el-locale`. Esta cookie se puede configurar usando un selector de idioma y luego, cuando un usuario regresa al sitio, aprovechará el locale especificado en la cookie al redirigir desde `/` a la ubicación correcta del locale.

Por ejemplo, si un usuario prefiere el locale `fr` en su encabezado accept-language pero hay una cookie `NEXT_LOCALE=en` configurada, al visitar `/` el usuario será redirigido a la ubicación del locale `en` hasta que la cookie se elimine o expire.
...
Next.js no conoce las variantes de una página, por lo que depende de ti agregar las etiquetas meta `hreflang` usando [`next/head`](/docs/14/pages/api-reference/components/head). Puedes aprender más sobre `hreflang` en la [documentación de Google Webmasters](https://support.google.com/webmasters/answer/189077).

## ¿Cómo funciona esto con Generación Estática?

> Nota que el Enrutamiento Internacionalizado no se integra con [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) ya que no aprovecha la capa de enrutamiento de Next.js. Las aplicaciones híbridas de Next.js que no usan `output: 'export'` tienen soporte completo.

### Rutas Dinámicas y Páginas con `getStaticProps`

Para páginas que usan `getStaticProps` con [Rutas Dinámicas](/docs/14/pages/building-your-application/routing/dynamic-routes), todas las variantes de locale de la página que se desean pre-renderizar deben devolverse desde [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths). Junto con el objeto `params` devuelto para `paths`, también puedes devolver un campo `locale` especificando qué locale deseas renderizar. Por ejemplo:
...
Para páginas [Optimizadas Estáticamente de Forma Automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) y páginas no dinámicas con `getStaticProps`, **se generará una versión de la página para cada locale**. Esto es importante considerarlo porque puede aumentar los tiempos de construcción dependiendo de cuántos locales estén configurados dentro de `getStaticProps`.
...
Para disminuir el tiempo de construcción de páginas dinámicas con `getStaticProps`, usa un [modo `fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true). Esto te permite devolver solo las rutas y locales más populares desde `getStaticPaths` para pre-renderizar durante la construcción. Luego, Next.js construirá las páginas restantes en tiempo de ejecución según se soliciten.

### Páginas Optimizadas Estáticamente de Forma Automática

Para páginas que están [optimizadas estáticamente de forma automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization), se generará una versión de la página para cada locale.

### Páginas No Dinámicas con `getStaticProps`

Para páginas no dinámicas con `getStaticProps`, se genera una versión para cada locale como se mencionó anteriormente. `getStaticProps` se llama con cada `locale` que se está renderizando. Si deseas excluir un cierto locale de ser pre-renderizado, puedes devolver `notFound: true` desde `getStaticProps` y no se generará esa variante de la página.
...
> **Nota importante**: Estos límites se agregaron inicialmente para evitar posibles [problemas de rendimiento en tiempo de construcción](#rutas-dinámicas-y-páginas-con-getstaticprops). Puedes solucionar estos límites con enrutamiento personalizado usando [Middleware](/docs/14/pages/building-your-application/routing/middleware) en Next.js 12.