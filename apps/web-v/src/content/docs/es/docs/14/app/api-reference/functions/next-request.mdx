...
translation-updated-at: 2025-06-06T16:49:56.360Z
title: NextRequest
...
Extiende la API nativa de [`URL`](https://developer.mozilla.org/docs/Web/API/URL) con métodos adicionales de conveniencia, incluyendo propiedades específicas de Next.js.
...
| ---------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `basePath`       | `string`                  | La [ruta base](/docs/14/pages/api-reference/next-config-js/basePath) de la URL.                                                  |
| `buildId`        | `string` \| `undefined`   | El identificador de compilación de la aplicación Next.js. Puede [personalizarse](/docs/14/pages/api-reference/next-config-js/generateBuildId). |
| `defaultLocale`  | `string` \| `undefined`   | La configuración regional predeterminada para [internacionalización](/docs/14/pages/building-your-application/routing/internationalization).              |
| `domainLocale`   |                           |                                                                                                                                 |
...
| Propiedad      | Tipo                    | Descripción                                                                                                                   |
| -------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `basePath`     | `string`                | La [ruta base](/docs/14/app/api-reference/next-config-js/basePath) de la URL.                                                  |
| `buildId`      | `string` \| `undefined` | El identificador de compilación de la aplicación Next.js. Puede [personalizarse](/docs/14/app/api-reference/next-config-js/generateBuildId). |
| `pathname`     | `string`                | El pathname de la URL.                                                                                                      |
| `searchParams` | `Object`                | Los parámetros de búsqueda de la URL.                                                                                             |

> **Nota:** Las propiedades de internacionalización del Enrutador de Páginas no están disponibles para usar en el Enrutador de Aplicaciones. Más información sobre [internacionalización con el Enrutador de Aplicaciones](/docs/14/app/building-your-application/routing/internationalization).
...
> **Bueno saber:** En [Vercel](https://vercel.com/docs/frameworks/nextjs?utm_source=next-site&utm_medium=docs&utm_campaign=next-website), este valor se proporciona por defecto. En otras plataformas, puedes usar el encabezado [`X-Forwarded-For`](https://developer.mozilla.org/docs/Web/HTTP/Headers/X-Forwarded-For) para proporcionar la dirección IP.
...
> **Bueno saber:** En [Vercel](https://vercel.com/docs/frameworks/nextjs?utm_source=next-site&utm_medium=docs&utm_campaign=next-website), este valor se proporciona por defecto. En otras plataformas, puedes usar el encabezado [`X-Forwarded-For`](https://developer.mozilla.org/docs/Web/HTTP/Headers/X-Forwarded-For) para obtener la dirección IP, luego usar un [servicio de terceros](https://ip-api.com/) para buscar la información geográfica.
