...
translation-updated-at: 2025-06-06T16:41:15.904Z
...
[El middleware de Next.js](/docs/14/app/building-your-application/routing/middleware) te permite ejecutar código en el servidor antes de que se complete una solicitud. Esto es especialmente útil para evitar mostrar contenido no autenticado cuando el usuario visita una página solo para autenticados redirigiéndolo a una página de inicio de sesión. El middleware también es útil para experimentación y [internacionalización](/docs/14/app/building-your-application/routing/internationalization).
...
5. Cualquier [archivo de metadatos](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) como `favicon.ico`, `icon.png`, `robots.txt` se agrega automáticamente a la etiqueta `<head>` de la aplicación siempre que los coloques en el nivel superior del directorio `app`. Después de mover [todos los archivos admitidos](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) al directorio `app`, puedes eliminar sus etiquetas `<link>`:
...
6. Finalmente, Next.js puede gestionar tus últimas etiquetas `<head>` con la [API de Metadatos](/docs/14/app/building-your-application/optimizing/metadata). Mueve tu información de metadatos final a un objeto [`metadata` exportado](/docs/14/app/api-reference/functions/generate-metadata#metadata-object):
...
Este directorio es lo que se conoce como un [segmento de ruta opcional catch-all](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments). Next.js utiliza un enrutador basado en sistema de archivos donde [los directorios se usan para definir rutas](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). Este directorio especial asegurará que todas las rutas de tu aplicación se dirijan al archivo `page.tsx` que contiene.
...
Este archivo es un [Componente de Servidor (Server Component)](/docs/14/app/building-your-application/rendering/server-components). Cuando ejecutas `next build`, el archivo se prerrenderiza en un recurso estático. _No_ requiere código dinámico.

Este archivo importa nuestro CSS global y le indica a [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) que solo generaremos una ruta, la ruta de índice en `/`.
...
Este archivo es un [Componente de Cliente (Client Component)](/docs/14/app/building-your-application/rendering/client-components), definido por la directiva `'use client'`. Los Componentes de Cliente aún se [prerrenderizan a HTML](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered) en el servidor antes de enviarse al cliente.
...
Next.js tiene soporte para [variables de entorno](/docs/14/app/building-your-application/configuring/environment-variables) `.env` similar a Vite. La principal diferencia es el prefijo utilizado para exponer variables de entorno en el lado del cliente.
...
2. **Establece [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) como `process.env.NEXT_PUBLIC_BASE_PATH` en tu archivo `next.config.mjs`:**
...
- Migrar de React Router al [Enrutador de Aplicación (App Router) de Next.js](/docs/14/app/building-your-application/routing) para obtener:
  - División de código automática
  - [Renderizado del lado del servidor con streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - [Componentes de Servidor de React](/docs/14/app/building-your-application/rendering/server-components)
...
- [Actualizar tu configuración de ESLint para soportar las reglas de Next.js](/docs/14/app/building-your-application/configuring/eslint)
