...
A continuación, crea un archivo personalizado [`instrumentation.ts`](/docs/14/app/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):

</AppOnly>

<PagesOnly>

A continuación, crea un archivo personalizado [`instrumentation.ts`](/docs/14/pages/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):
...
> - Si usas la opción de configuración [`pageExtensions`](/docs/14/app/api-reference/next-config-js/pageExtensions) para añadir un sufijo, también necesitarás actualizar el nombre del archivo `instrumentation` para que coincida.
...
>
> - El archivo `instrumentation` debe estar en la raíz de tu proyecto y no dentro de los directorios `app` o `pages`. Si usas la carpeta `src`, coloca el archivo dentro de `src` junto a `pages` y `app`.
> - Si usas la opción de configuración [`pageExtensions`](/docs/14/pages/api-reference/next-config-js/pageExtensions) para añadir un sufijo, también necesitarás actualizar el nombre del archivo `instrumentation` para que coincida.
> - Hemos creado un ejemplo básico [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry) que puedes usar.
...
OpenTelemetry Collector no es necesario. Puedes usar un exportador OpenTelemetry personalizado con [`@vercel/otel`](#usando-vercelotel) o [configuración manual de OpenTelemetry](/docs/14/pages/building-your-application/optimizing/open-telemetry#configuración-manual-de-opentelemetry).
