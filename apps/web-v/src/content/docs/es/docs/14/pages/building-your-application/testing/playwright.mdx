...
Esto te guiará a través de una serie de prompts para configurar Playwright en tu proyecto, incluyendo la creación de un archivo `playwright.config.ts`. Consulta la [guía de instalación de Playwright](https://playwright.dev/docs/intro#installation) para ver los pasos detallados.
...
> **Bueno saber**:
>
> Puedes usar `page.goto("/")` en lugar de `page.goto("http://localhost:3000/")`, si añades [`"baseURL": "http://localhost:3000"`](https://playwright.dev/docs/api/class-testoptions#test-options-base-url) al archivo de configuración [`playwright.config.ts`](https://playwright.dev/docs/test-configuration).
...
> **Bueno saber**: Alternativamente, puedes usar la función [`webServer`](https://playwright.dev/docs/test-webserver/) para que Playwright inicie el servidor de desarrollo y espere hasta que esté completamente disponible.

### Ejecutando Playwright en Integración Continua (CI)

Playwright ejecutará tus pruebas en [modo headless](https://playwright.dev/docs/ci#running-headed) por defecto. Para instalar todas las dependencias de Playwright, ejecuta `npx playwright install-deps`.
...
- [Playwright en tu proveedor de CI](https://playwright.dev/docs/ci)
