...
translation-updated-at: 2025-06-06T16:40:37.442Z
title: exportPathMap
...
`exportPathMap` te permite especificar un mapeo de rutas de solicitud a destinos de página, para ser usado durante la exportación. Las rutas definidas en `exportPathMap` también estarán disponibles al usar [`next dev`](/docs/14/app/api-reference/cli/next#next-dev-options).
...
> **Importante**: el campo `query` en `exportPathMap` no puede usarse con [páginas optimizadas estáticamente de forma automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) o [páginas con `getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) ya que se renderizan como archivos HTML en el momento de construcción y no se puede proporcionar información adicional de consulta durante `next export`.
...
- `distDir` - Ruta absoluta al directorio `.next/` (configurable con [`distDir`](/docs/14/pages/api-reference/next-config-js/distDir))
...
[`next export`](/docs/14/app/building-your-application/deploying/static-exports) usará `out` como directorio de salida predeterminado, puedes personalizarlo usando el argumento `-o`, así:

</AppOnly>

<PagesOnly>

[`next export`](/docs/14/pages/building-your-application/deploying/static-exports) usará `out` como directorio de salida predeterminado, puedes personalizarlo usando el argumento `-o`, así:
