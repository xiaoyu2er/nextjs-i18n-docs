...
Actualmente, el único valor de `context` es `params`, que es un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) para la ruta actual.
...
Los Route Handlers pueden extender la API Response web devolviendo un objeto `NextResponse`. Esto permite configurar fácilmente cookies, encabezados, redirecciones y reescrituras. [Ver la referencia de API](/docs/14/app/api-reference/functions/next-response).
