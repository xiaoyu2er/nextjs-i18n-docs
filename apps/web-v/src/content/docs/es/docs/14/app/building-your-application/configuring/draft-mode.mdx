...
Primero, crea un [Route Handler](/docs/14/app/building-your-application/routing/route-handlers). Puede tener cualquier nombre, por ejemplo, `app/api/draft/route.ts`.
...
Luego, envía una solicitud a `/api/disable-draft` para invocar el Route Handler. Si llamas a esta ruta usando [`next/link`](/docs/14/app/api-reference/components/link), debes pasar `prefetch={false}` para evitar eliminar accidentalmente la cookie durante el prefetch.
