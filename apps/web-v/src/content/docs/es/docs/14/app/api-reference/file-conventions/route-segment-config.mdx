...
Las opciones de Segmento de Ruta te permiten configurar el comportamiento de una [Página](/docs/14/app/building-your-application/routing/pages-and-layouts), [Layout](/docs/14/app/building-your-application/routing/pages-and-layouts) o [Manejador de Ruta (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) exportando directamente las siguientes variables:
...
- **`'force-dynamic'`**: Fuerza el [renderizado dinámico (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering), lo que hará que las rutas se rendericen para cada usuario en el momento de la solicitud. Esta opción es equivalente a `getServerSideProps()` en el directorio `pages`.

- **`'error'`**: Fuerza el renderizado estático y almacena en caché los datos de un layout o página causando un error si algún componente usa [funciones dinámicas (dynamic functions)](/docs/14/app/building-your-application/rendering/server-components#dynamic-functions) o datos no almacenados en caché. Esta opción es equivalente a:
...
- **`'force-static'`**: Fuerza el renderizado estático y almacena en caché los datos de un layout o página haciendo que [`cookies()`](/docs/14/app/api-reference/functions/cookies), [`headers()`](/docs/14/app/api-reference/functions/headers) y [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) devuelvan valores vacíos.

> **Es bueno saber**:
>
> - Puedes encontrar instrucciones sobre [cómo migrar](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods) desde `getServerSideProps` y `getStaticProps` a `dynamic: 'force-dynamic'` y `dynamic: 'error'` en la [guía de actualización](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods).

### `dynamicParams`

Controla qué sucede cuando se visita un segmento dinámico que no fue generado con [generateStaticParams](/docs/14/app/api-reference/functions/generate-static-params).
...
> - Cuando `dynamicParams = true`, el segmento utiliza [Renderizado de Servidor con Streaming (Streaming Server Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
- **`false`** (por defecto): La heurística predeterminada para almacenar en caché cualquier solicitud `fetch` que establezca su opción `cache` en `'force-cache'` o que se descubra antes de usar una [función dinámica (dynamic function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions). Semánticamente equivalente a `revalidate: Infinity`, lo que significa que el recurso debe almacenarse en caché indefinidamente. Todavía es posible que solicitudes `fetch` individuales usen `cache: 'no-store'` o `revalidate: 0` para evitar el almacenamiento en caché y hacer que la ruta se renderice dinámicamente. O establecer `revalidate` en un número positivo menor que el predeterminado de la ruta para aumentar la frecuencia de revalidación.
- **`0`**: Asegura que un layout o página siempre se [renderice dinámicamente (dynamically rendered)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering), incluso si no se descubren funciones dinámicas o recuperaciones de datos no almacenadas en caché. Esta opción cambia el valor predeterminado de las solicitudes `fetch` que no establecen una opción `cache` a `'no-store'`, pero deja las solicitudes `fetch` que optan por `'force-cache'` o usan un `revalidate` positivo como están.
- **`number`**: (en segundos) Establece la frecuencia de revalidación predeterminada de un layout o página a `n` segundos.

> **Es bueno saber**: La opción `revalidate` solo está disponible cuando se usa el [Runtime de Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime). Esto significa que usar la opción `revalidate` con `runtime = 'edge'` no funcionará.
...
Aprende más sobre los [runtimes Edge y Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> **Es bueno saber**:
>
> - Si usas [Acciones de Servidor (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations), establece `maxDuration` a nivel de página para cambiar el tiempo de espera predeterminado de todas las Acciones de Servidor usadas en la página.

### `generateStaticParams`

La función `generateStaticParams` se puede usar en combinación con [segmentos de ruta dinámicos](/docs/14/app/building-your-application/routing/dynamic-routes) para definir la lista de parámetros de segmento de ruta que se generarán estáticamente en el momento de la compilación en lugar de bajo demanda en el momento de la solicitud.

Consulta la [referencia de API](/docs/14/app/api-reference/functions/generate-static-params) para más detalles.