...
translation-updated-at: 2025-06-06T17:01:26.176Z
...
related:
  description: Conozca cómo Next.js almacena en caché los datos y el resultado del renderizado estático.
  links:
...
Por defecto, Next.js utiliza Componentes del Servidor. Esto le permite implementar automáticamente el renderizado en el servidor sin configuración adicional, y puede optar por usar Componentes del Cliente cuando sea necesario, consulte [Componentes del Cliente](/docs/14/app/building-your-application/rendering/client-components).
...
2. Next.js utiliza el RSC Payload y las instrucciones de JavaScript de los Componentes del Cliente para renderizar **HTML** en el servidor.
...
Con el Renderizado Estático, las rutas se renderizan en **tiempo de compilación**, o en segundo plano después de [revalidación de datos](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data). El resultado se almacena en caché y puede enviarse a una [Red de Entrega de Contenido (CDN)](https://developer.mozilla.org/docs/Glossary/CDN). Esta optimización permite compartir el resultado del trabajo de renderizado entre usuarios y solicitudes del servidor.
...
>
> Obtenga más información sobre la [caché de ruta completa](/docs/14/app/building-your-application/caching#full-route-cache) y [Caché de Datos](/docs/14/app/building-your-application/caching#data-cache).

#### Cambio a Renderizado Dinámico

Durante el renderizado, si se descubre una [función dinámica](#dynamic-functions) o una [solicitud de datos no almacenados en caché](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#opting-out-of-data-caching), Next.js cambiará a renderizar dinámicamente toda la ruta. Esta tabla resume cómo las funciones dinámicas y el almacenamiento en caché de datos afectan si una ruta se renderiza estática o dinámicamente:
...
- **[`cookies()`](/docs/14/app/api-reference/functions/cookies) y [`headers()`](/docs/14/app/api-reference/functions/headers)**: Usar estas en un Componente del Servidor hará que toda la ruta opte por el renderizado dinámico en el momento de la solicitud.
- **[`searchParams`](/docs/14/app/api-reference/file-conventions/page#searchparams-optional)**: Usar la prop `searchParams` en una [Página](/docs/14/app/api-reference/file-conventions/page) hará que la página opte por el renderizado dinámico en el momento de la solicitud.
...
<Image
  alt="Diagrama que muestra la paralelización de segmentos de ruta durante el streaming, mostrando la obtención de datos, el renderizado y la hidratación de fragmentos individuales."
...
<Image
  alt="Diagrama que muestra una página parcialmente renderizada en el cliente, con UI de carga para los fragmentos que se están transmitiendo."
...
Puede comenzar a transmitir segmentos de ruta usando `loading.js` y componentes de UI con [React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming). Consulte la sección [UI de Carga y Streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) para obtener más información.