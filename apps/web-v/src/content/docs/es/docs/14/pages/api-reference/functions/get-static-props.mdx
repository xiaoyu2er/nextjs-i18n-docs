...
| Nombre               | Descripción                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`             | Contiene los parámetros de ruta para páginas que usan [rutas dinámicas](/docs/14/pages/building-your-application/routing/dynamic-routes). Por ejemplo, si el nombre de la página es `[id].js`, entonces `params` se verá como `{ id: ... }`. Debes usar esto junto con `getStaticPaths`, que explicaremos más adelante.                                                                                                                              |
| `preview`            | (Obsoleto para `draftMode`) `preview` es `true` si la página está en [Modo de Vista Previa](/docs/14/pages/building-your-application/configuring/preview-mode) y `false` en caso contrario.                                                                                                                                                                                                                                                          |
| `previewData`        | (Obsoleto para `draftMode`) Los datos de [vista previa](/docs/14/pages/building-your-application/configuring/preview-mode) establecidos por `setPreviewData`.                                                                                                                                                                                                                                                                                        |
| `draftMode`          | `draftMode` es `true` si la página está en [Modo de Borrador](/docs/14/pages/building-your-application/configuring/draft-mode) y `false` en caso contrario.                                                                                                                                                                                                                                                                                          |
...
| `revalidateReason`   | Proporciona un motivo por el cual se llamó a la función. Puede ser uno de: "build" (ejecutado en tiempo de construcción), "stale" (período de revalidación expirado, o ejecutándose en [modo desarrollo](/docs/14/pages/building-your-application/data-fetching/get-static-props#runs-on-every-request-in-development)), "on-demand" (activado mediante [revalidación bajo demanda](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)) |
...
Aprende más sobre [Regeneración Estática Incremental](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
El booleano `notFound` permite que la página devuelva un estado `404` y una [Página 404](/docs/14/pages/building-your-application/routing/custom-error#404-page). Con `notFound: true`, la página devolverá un `404` incluso si hubo una página generada exitosamente antes. Esto está diseñado para soportar casos de uso como contenido generado por usuarios que es eliminado por su autor. Nota: `notFound` sigue el mismo comportamiento de `revalidate` [descrito aquí](#revalidate).
...
> **Nota importante**: `notFound` no es necesario para el modo [`fallback: false`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-false) ya que solo las rutas devueltas por `getStaticPaths` serán pre-renderizadas.
...
Si las redirecciones son conocidas en el momento de construcción, deben agregarse en [`next.config.js`](/docs/14/pages/api-reference/next-config-js/redirects) en su lugar.
...
| Versión   | Cambios                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/14/app/building-your-application/data-fetching) ahora es estable con obtención de datos simplificada                                                  |
...
| `v12.1.0` | Se añadió [Regeneración Estática Incremental bajo demanda](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) (beta). |
