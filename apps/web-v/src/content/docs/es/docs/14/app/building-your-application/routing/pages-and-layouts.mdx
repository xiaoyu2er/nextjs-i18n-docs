...
> Recomendamos leer las páginas [Fundamentos de Enrutamiento](/docs/14/app/building-your-application/routing) y [Definiendo Rutas](/docs/14/app/building-your-application/routing/defining-routes) antes de continuar.

Los archivos especiales [layout.js](#layouts), [page.js](#pages), y [template.js](#templates) te permiten crear UI para una [ruta](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). Esta página te guiará sobre cómo y cuándo usar estos archivos especiales.
...
> - Una página siempre es la [hoja](/docs/14/app/building-your-application/routing#terminology) del [subárbol de rutas](/docs/14/app/building-your-application/routing#terminology).
> - Se requiere un archivo `page.js` para hacer un segmento de ruta accesible públicamente.
> - Las páginas son [Componentes de Servidor](/docs/14/app/building-your-application/rendering/server-components) por defecto, pero pueden configurarse como [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components).
> - Las páginas pueden obtener datos. Consulta la sección [Obtención de Datos](/docs/14/app/building-your-application/data-fetching) para más información.
...
> - Los layouts son [Componentes de Servidor](/docs/14/app/building-your-application/rendering/server-components) por defecto pero pueden configurarse como [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components).
> - Los layouts pueden obtener datos. Consulta la sección [Obtención de Datos](/docs/14/app/building-your-application/data-fetching) para más información.
> - No es posible pasar datos entre un layout padre y sus hijos. Sin embargo, puedes obtener los mismos datos en una ruta más de una vez, y React [deduplicará automáticamente las solicitudes](/docs/14/app/building-your-application/caching#request-memoization) sin afectar el rendimiento.
> - Los layouts no tienen acceso a los segmentos de ruta debajo de sí mismos. Para acceder a todos los segmentos de ruta, puedes usar [`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) o [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) en un Componente de Cliente.
> - Puedes usar [Grupos de Rutas](/docs/14/app/building-your-application/routing/route-groups) para incluir o excluir segmentos de ruta específicos de layouts compartidos.
> - Puedes usar [Grupos de Rutas](/docs/14/app/building-your-application/routing/route-groups) para crear múltiples root layouts. Consulta un [ejemplo aquí](/docs/14/app/building-your-application/routing/route-groups#creating-multiple-root-layouts).
> - **Migrando desde el directorio `pages`:** El root layout reemplaza los archivos [`_app.js`](/docs/14/pages/building-your-application/routing/custom-app) y [`_document.js`](/docs/14/pages/building-your-application/routing/custom-document). [Consulta la guía de migración](/docs/14/app/building-your-application/upgrading/app-router-migration#migrating-_documentjs-and-_appjs).
...
En el directorio `app`, puedes modificar los elementos HTML `<head>` como `title` y `meta` usando las [APIs de Metadata](/docs/14/app/building-your-application/optimizing/metadata).

La metadata puede definirse exportando un [objeto `metadata`](/docs/14/app/api-reference/functions/generate-metadata#the-metadata-object) o una [función `generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata#generatemetadata-function) en un archivo [`layout.js`](/docs/14/app/api-reference/file-conventions/layout) o [`page.js`](/docs/14/app/api-reference/file-conventions/page).
...
> **Bueno saber**: No debes añadir manualmente etiquetas `<head>` como `<title>` y `<meta>` a root layouts. En su lugar, debes usar la [API de Metadata](/docs/14/app/api-reference/functions/generate-metadata) que maneja automáticamente requisitos avanzados como streaming y deduplicación de elementos `<head>`.

Aprende más sobre las opciones de metadata disponibles en la [referencia de API](/docs/14/app/api-reference/functions/generate-metadata)