...
`usePathname` requiere intencionalmente el uso de un [Componente Cliente](/docs/14/app/building-your-application/rendering/client-components). Es importante destacar que los Componentes Cliente no son una desoptimización. Son parte integral de la arquitectura de [Componentes Servidor](/docs/14/app/building-your-application/rendering/server-components).
...
> **Dato importante**:
>
> - No se admite leer la URL actual desde un [Componente Servidor](/docs/14/app/building-your-application/rendering/server-components). Este diseño es intencional para mantener el estado del diseño entre navegaciones de página.
> - Modo de compatibilidad:
>   - `usePathname` puede devolver `null` cuando se está renderizando una [ruta de respaldo (fallback route)](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) o cuando una página del directorio `pages` ha sido [optimizada estáticamente de forma automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) por Next.js y el enrutador no está listo.
