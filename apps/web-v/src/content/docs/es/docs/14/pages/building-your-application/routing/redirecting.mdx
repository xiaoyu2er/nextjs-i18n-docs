...
La función `redirect` permite redirigir al usuario a otra URL. Puede llamar a `redirect` en [Componentes del Servidor](/docs/14/app/building-your-application/rendering/server-components), [Manejadores de Ruta](/docs/14/app/building-your-application/routing/route-handlers) y [Acciones del Servidor](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
Consulte la [referencia de API de `redirect`](/docs/14/app/api-reference/functions/redirect) para más información.
...
Consulte la [referencia de API de `permanentRedirect`](/docs/14/app/api-reference/functions/permanentRedirect) para más información.
...
> **Nota importante**:
>
> - Si no necesita navegar programáticamente a un usuario, debería usar un componente [`<Link>`](/docs/14/app/api-reference/components/link).

<AppOnly>

Consulte la [referencia de API de `useRouter`](/docs/14/app/api-reference/functions/use-router) para más información.

</AppOnly>

<PagesOnly>

Consulte la [referencia de API de `useRouter`](/docs/14/pages/api-reference/functions/use-router) para más información.
...
`redirects` soporta [coincidencia de rutas](/docs/14/app/api-reference/next-config-js/redirects#path-matching), [cabeceras, cookies y consultas](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching), brindando flexibilidad para redirigir usuarios basado en solicitudes entrantes.
...
Consulte la [referencia de API de `redirects`](/docs/14/app/api-reference/next-config-js/redirects) para más información.

> **Nota importante**:
>
> - `redirects` puede retornar códigos 307 (Redirección Temporal) o 308 (Redirección Permanente) con la opción `permanent`.
> - `redirects` puede tener límites en plataformas. Por ejemplo, en Vercel hay un límite de 1,024 redirecciones. Para manejar grandes cantidades (1000+), considere crear una solución personalizada con [Middleware](/docs/14/app/building-your-application/routing/middleware). Vea [gestión de redirecciones a escala](#managing-redirections-at-scale-advanced) para más.
...
Consulte la documentación de [Middleware](/docs/14/app/building-your-application/routing/middleware) para más información.
...
Si existe, reenvíe la solicitud a un <AppOnly>[Manejador de Ruta (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers)</AppOnly> <PagesOnly>[Rutas API (API Routes)](/docs/14/pages/building-your-application/routing/api-routes)</PagesOnly> que verificará el archivo real y redirigirá al usuario a la URL correspondiente. Esto evita importar un archivo grande de redirecciones en el Middleware, lo cual puede ralentizar cada solicitud entrante.
