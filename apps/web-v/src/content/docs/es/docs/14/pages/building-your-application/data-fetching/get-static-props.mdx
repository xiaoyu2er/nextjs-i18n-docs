...
La [referencia de la API de `getStaticProps`](/docs/14/pages/api-reference/functions/get-static-props) cubre todos los parámetros y props que pueden usarse con `getStaticProps`.
...
- `getStaticProps` se ejecuta en segundo plano cuando se usa [`fallback: true`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)
- `getStaticProps` se llama antes del renderizado inicial cuando se usa [`fallback: blocking`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking)
- `getStaticProps` se ejecuta en segundo plano cuando se usa `revalidate`
- `getStaticProps` se ejecuta bajo demanda en segundo plano cuando se usa [`revalidate()`](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)

Cuando se combina con [Regeneración Estática Incremental](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration), `getStaticProps` se ejecutará en segundo plano mientras se revalida la página obsoleta, y la página fresca se sirve al navegador.

`getStaticProps` no tiene acceso a la solicitud entrante (como parámetros de consulta o cabeceras HTTP) ya que genera HTML estático. Si necesitas acceso a la solicitud para tu página, considera usar [Middleware](/docs/14/pages/building-your-application/routing/middleware) además de `getStaticProps`.
...
Este archivo JSON se usará en el enrutamiento del lado del cliente a través de [`next/link`](/docs/14/pages/api-reference/components/link) o [`next/router`](/docs/14/pages/api-reference/functions/use-router). Cuando navegas a una página que se ha pre-renderizado usando `getStaticProps`, Next.js obtiene este archivo JSON (precalculado en el momento de construcción) y lo usa como props para el componente de la página. Esto significa que las transiciones de página del lado del cliente **no** llamarán a `getStaticProps`, ya que solo se usa el JSON exportado.
...
> **Nota importante**: si has creado una [app personalizada](/docs/14/pages/building-your-application/routing/custom-app), asegúrate de pasar las `pageProps` al componente de la página como se muestra en el documento enlazado, de lo contrario las props estarán vacías.
...
Puedes omitir temporalmente la generación estática y renderizar la página en **tiempo de solicitud** en lugar de en tiempo de construcción usando el [**Modo de vista previa**](/docs/14/pages/building-your-application/configuring/preview-mode). Por ejemplo, podrías estar usando un CMS headless y querer previsualizar borradores antes de que se publiquen.