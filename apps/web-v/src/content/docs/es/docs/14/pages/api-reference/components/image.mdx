...
> **Es bueno saberlo**: Si estás usando una versión de Next.js anterior a la 13, deberás consultar la documentación de [next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) ya que el componente fue renombrado.

</PagesOnly>

Esta referencia de API te ayudará a entender cómo usar las [props](#props) y [opciones de configuración](#configuration-options) disponibles para el Componente de Imagen. Para características y uso, consulta la página del [Componente de Imagen](/docs/14/app/building-your-application/optimizing/images).
...
- Un archivo de imagen [importado estáticamente](/docs/14/app/building-your-application/optimizing/images#local-images)
...
Requerido, excepto para imágenes [importadas estáticamente](/docs/14/app/building-your-application/optimizing/images#local-images) o imágenes con la propiedad [`fill`](#fill).
...
> **Es bueno saberlo**: Usar props como `loader`, que aceptan una función, requiere usar [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components) para serializar la función proporcionada.
...
Cuando es `blur`, se usará la propiedad [`blurDataURL`](#blurdataurl) como marcador de posición. Si `src` es un objeto de una [importación estática](/docs/14/app/building-your-application/optimizing/images#local-images) y la imagen importada es `.jpg`, `.png`, `.webp` o `.avif`, entonces `blurDataURL` se llenará automáticamente, excepto cuando la imagen se detecte como animada.
...
> **Es bueno saberlo**: Usar props como `onLoadingComplete`, que aceptan una función, requiere usar [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components) para serializar la función proporcionada.
...
> **Es bueno saberlo**: Usar props como `onLoad`, que aceptan una función, requiere usar [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components) para serializar la función proporcionada.
...
> **Es bueno saberlo**: Usar props como `onError`, que aceptan una función, requiere usar [Componentes de Cliente](/docs/14/app/building-your-application/rendering/client-components) para serializar la función proporcionada.
...
- [Configuración de Cargador de Imagen Personalizado](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **Bueno saber**: Personalizar el archivo del cargador de imágenes, que acepta una función, requiere usar [Client Components](/docs/14/app/building-your-application/rendering/client-components) para serializar la función proporcionada.
...
| `v13.0.0`  | La importación `next/image` se renombró a `next/legacy/image`. La importación `next/future/image` se renombró a `next/image`. Hay disponible un [codemod](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) para renombrar las importaciones de forma segura y automática. Se eliminó el envoltorio `<span>`. Se eliminaron las propiedades `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot`. `alt` es obligatorio. `onLoadingComplete` recibe referencia al elemento `img`. Se eliminó la configuración del loader integrado. |
