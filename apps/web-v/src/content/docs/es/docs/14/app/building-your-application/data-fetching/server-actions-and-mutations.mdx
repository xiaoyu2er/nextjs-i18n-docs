...
sidebar:
  order: 2010202
  label: "Acciones de servidor y mutaciones"
---
...
- Las Acciones de Servidor se integran con la arquitectura de [caché y revalidación](/docs/14/app/building-your-application/caching) de Next.js. Cuando se invoca una acción, Next.js puede devolver tanto la UI actualizada como nuevos datos en un solo viaje al servidor.
...
- Las Acciones de Servidor heredan el [entorno de ejecución](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes) de la página o layout donde se usan.
- Las Acciones de Servidor heredan la [Configuración del Segmento de Ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) de la página o layout donde se usan, incluyendo campos como `maxDuration`.
...
Cuando se lanza un error, será capturado por el límite [`error.js`](/docs/14/app/building-your-application/routing/error-handling) más cercano o `<Suspense>` en el cliente. Recomendamos usar `try/catch` para devolver errores que puedan ser manejados por su interfaz de usuario.
...
Puede revalidar la [Caché de Next.js](/docs/14/app/building-your-application/caching) dentro de sus Acciones del Servidor con la API [`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath):
...
O invalidar una obtención de datos específica con una etiqueta de caché usando [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag):
...
Si desea redirigir al usuario a una ruta diferente después de completar una Acción del Servidor, puede usar la API [`redirect`](/docs/14/app/api-reference/functions/redirect). `redirect` debe llamarse fuera del bloque `try/catch`:
...
Puede `obtener`, `establecer` y `eliminar` cookies dentro de una Acción del Servidor usando la API [`cookies`](/docs/14/app/api-reference/functions/cookies):
...
Consulte [ejemplos adicionales](/docs/14/app/api-reference/functions/cookies#deleting-cookies) para eliminar cookies desde Acciones del Servidor.
