...
> **A tener en cuenta**: Si prefiere devolver una redirección HTTP 307 (Temporal) en lugar de 308 (Permanente), puede usar la función [`redirect`](/docs/14/app/api-reference/functions/redirect) en su lugar.
...
Por defecto, `permanentRedirect` usará `push` (añade una nueva entrada al historial del navegador) en [Acciones del Servidor (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) y `replace` (reemplaza la URL actual en el historial del navegador) en otros casos. Puede sobrescribir este comportamiento especificando el parámetro `type`.
