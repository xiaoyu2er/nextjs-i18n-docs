...
Si una página tiene [Rutas Dinámicas](/docs/14/pages/building-your-application/routing/dynamic-routes) y usa `getStaticProps`, necesita definir una lista de rutas para ser generadas estáticamente.
...
La [referencia de la API `getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) cubre todos los parámetros y props que pueden usarse con `getStaticPaths`.
...
- **No puedes** usar `getStaticPaths` con [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)
- Puedes exportar `getStaticPaths` desde una [Ruta Dinámica](/docs/14/pages/building-your-application/routing/dynamic-routes) que también use `getStaticProps`
...
`getStaticPaths` te permite controlar qué páginas se generan durante la compilación en lugar de hacerlo bajo demanda con [`fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking). Generar más páginas durante la compilación hará que las compilaciones sean más lentas.
