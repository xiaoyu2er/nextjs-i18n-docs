...
El archivo `default.js` se utiliza para renderizar un respaldo dentro de [Rutas Paralelas](/docs/14/app/building-your-application/routing/parallel-routes) cuando Next.js no puede recuperar el estado activo de un [slot](/docs/14/app/building-your-application/routing/parallel-routes#slots) después de una carga completa de página.

Durante la [navegación suave (soft navigation)](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation), Next.js mantiene un registro del estado activo (subpágina) para cada slot. Sin embargo, para navegaciones duras (carga completa de página), Next.js no puede recuperar el estado activo. En este caso, se puede renderizar un archivo `default.js` para subpáginas que no coincidan con la URL actual.
...
Un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) desde el segmento raíz hasta las subpáginas del slot. Por ejemplo:
