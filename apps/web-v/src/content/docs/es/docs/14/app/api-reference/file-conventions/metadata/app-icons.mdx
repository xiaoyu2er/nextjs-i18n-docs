...
La forma más fácil de generar un icono es usando la API [`ImageResponse`](/docs/14/app/api-reference/functions/image-response) de `next/og`.
...
> **Dato importante**
>
> - Por defecto, los iconos generados son [**optimizados estáticamente**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (generados en tiempo de compilación y cacheados) a menos que usen [funciones dinámicas](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) o datos no cacheados.
> - Puedes generar múltiples iconos en el mismo archivo usando [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata).
...
Un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) desde el segmento raíz hasta el segmento donde `icon` o `apple-icon` está ubicado.
...
`icon` y `apple-icon` son [Manejadores de Ruta](/docs/14/app/building-your-application/routing/route-handlers) especializados que pueden usar las mismas opciones de [configuración de segmento de ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) que Páginas y Layouts.
