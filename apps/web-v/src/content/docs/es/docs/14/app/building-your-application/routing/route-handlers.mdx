...
> **Es bueno saberlo**: Los Manejadores de Ruta solo están disponibles dentro del directorio `app`. Son equivalentes a las [Rutas API](/docs/14/pages/building-your-application/routing/api-routes) dentro del directorio `pages`, lo que significa que **no necesitas** usar Rutas API y Manejadores de Ruta juntos.

## Convención

Los Manejadores de Ruta se definen en un archivo [`route.js|ts`](/docs/14/app/api-reference/file-conventions/route) dentro del directorio `app`:
...
> **Advertencia de TypeScript:** `Response.json()` solo es válido a partir de TypeScript 5.2. Si usas una versión inferior de TypeScript, puedes usar [`NextResponse.json()`](/docs/14/app/api-reference/functions/next-response#json) para respuestas tipadas.
...
Puedes [revalidar datos en caché](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) usando la opción [`next.revalidate`](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data):
...
Alternativamente, puedes usar la [opción de configuración de segmento `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate):
...
Los Manejadores de Ruta pueden usarse con funciones dinámicas de Next.js, como [`cookies`](/docs/14/app/api-reference/functions/cookies) y [`headers`](/docs/14/app/api-reference/functions/headers).

#### Cookies

Puedes leer o establecer cookies con [`cookies`](/docs/14/app/api-reference/functions/cookies) de `next/headers`. Esta función de servidor puede llamarse directamente en un Manejador de Ruta o anidarse dentro de otra función.
...
También puedes usar las APIs Web subyacentes para leer cookies de la solicitud ([`NextRequest`](/docs/14/app/api-reference/functions/next-request)):
...
Puedes leer cabeceras con [`headers`](/docs/14/app/api-reference/functions/headers) de `next/headers`. Esta función de servidor puede llamarse directamente en un Manejador de Ruta o anidarse dentro de otra función.
...
También puedes usar las APIs Web subyacentes para leer cabeceras de la solicitud ([`NextRequest`](/docs/14/app/api-reference/functions/next-request)):
...
> Recomendamos leer la página [Definiendo Rutas](/docs/14/app/building-your-application/routing/defining-routes) antes de continuar.

Los Manejadores de Ruta pueden usar [Segmentos Dinámicos](/docs/14/app/building-your-application/routing/dynamic-routes) para crear manejadores de solicitud a partir de datos dinámicos.
...
El objeto de solicitud pasado al Manejador de Ruta es una instancia de `NextRequest`, que tiene [algunos métodos adicionales convenientes](/docs/14/app/api-reference/functions/next-request#nexturl), incluyendo para manejar más fácilmente los parámetros de consulta.
...
> **Nota importante**:
>
> - Para agregar encabezados CORS a múltiples Manejadores de Ruta, puedes usar [Middleware](/docs/14/app/building-your-application/routing/middleware#cors) o el archivo [`next.config.js`](/docs/14/app/api-reference/next-config-js/headers#cors).
...
Los Manejadores de Ruta tienen una Web API isomórfica para soportar tanto entornos Edge como Node.js sin problemas, incluyendo soporte para streaming. Dado que los Manejadores de Ruta usan la misma [configuración de segmento de ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) que las páginas y layouts, soportan características muy esperadas como los Manejadores de Ruta [regenerados estáticamente](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) de propósito general.
...
Los Manejadores de Ruta usan la misma [configuración de segmento de ruta](/docs/14/app/api-reference/file-conventions/route-segment-config) que las páginas y layouts.
...
Consulta la [referencia de API](/docs/14/app/api-reference/file-conventions/route-segment-config) para más detalles.
