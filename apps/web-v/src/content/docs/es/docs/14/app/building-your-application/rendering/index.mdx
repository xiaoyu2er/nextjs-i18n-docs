...
translation-updated-at: 2025-06-06T16:59:46.128Z
title: Renderizado
description: Aprende las diferencias entre los entornos, estrategias y tiempos de ejecución (runtimes) de renderizado en Next.js.
sidebar:
  order: 20103
  label: "Renderizado"
---
...
El renderizado convierte el código que escribes en interfaces de usuario. React y Next.js te permiten crear aplicaciones web híbridas donde partes de tu código pueden renderizarse en el servidor o en el cliente. Esta sección te ayudará a comprender las diferencias entre estos entornos de renderizado, estrategias y tiempos de ejecución.

## Fundamentos

Para comenzar, es útil familiarizarse con tres conceptos web fundamentales:

- Los [Entornos](#entornos-de-renderizado) donde se puede ejecutar el código de tu aplicación: el servidor y el cliente.
- El [Ciclo de Vida Solicitud-Respuesta](#ciclo-solicitud-respuesta) que se inicia cuando un usuario visita o interactúa con tu aplicación.
- El [Límite de Red](#limite-de-red) que separa el código del servidor y del cliente.

### Entornos de Renderizado

Existen dos entornos donde se pueden renderizar aplicaciones web: el cliente y el servidor.
...
- El **servidor** se refiere a la computadora en un centro de datos que almacena tu código de aplicación, recibe solicitudes de un cliente y envía una respuesta adecuada.
...
Comprender estas diferencias es clave para usar React y Next.js efectivamente. Cubriremos las diferencias y casos de uso con más detalle en las páginas de [Componentes del Servidor](/docs/14/app/building-your-application/rendering/server-components) y [Componentes del Cliente](/docs/14/app/building-your-application/rendering/client-components). Por ahora, continuemos construyendo nuestra base.
...
4. **Respuesta HTTP:** Después de procesar la solicitud, el servidor envía una respuesta HTTP al cliente. Esta respuesta contiene un código de estado (que indica si la solicitud fue exitosa o no) y los recursos solicitados (ej. HTML, CSS, JavaScript, recursos estáticos, etc).
...
Puedes usar la convención `"use client"` de React para definir el límite. También existe una convención `"use server"`, que le indica a React que realice algún trabajo computacional en el servidor.

## Construyendo Aplicaciones Híbridas

Al trabajar en estos entornos, es útil pensar en el flujo del código en tu aplicación como **unidireccional**. En otras palabras, durante una respuesta, tu código de aplicación fluye en una dirección: del servidor al cliente.
...
Este concepto se volverá más claro cuando veamos cómo puedes [entrelazar componentes del cliente y del servidor](/docs/14/app/building-your-application/rendering/composition-patterns) en el mismo árbol de componentes.