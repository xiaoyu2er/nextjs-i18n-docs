...
Un objeto que contiene los [parámetros de ruta dinámica](/docs/14/app/building-your-application/routing/dynamic-routes) desde el segmento raíz hasta el segmento desde donde se llama a `generateImageMetadata`.
...
Este ejemplo utiliza el objeto `params` y datos externos para generar múltiples [imágenes Open Graph](/docs/14/app/api-reference/file-conventions/metadata/opengraph-image) para un segmento de ruta.
