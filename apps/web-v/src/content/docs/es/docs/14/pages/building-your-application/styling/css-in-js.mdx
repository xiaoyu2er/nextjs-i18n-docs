...
Las siguientes bibliotecas son compatibles en Componentes del Cliente en el directorio `app` (orden alfabético):
...
Si deseas estilizar Componentes del Servidor, recomendamos usar [Módulos CSS](/docs/14/app/building-your-application/styling/css-modules) u otras soluciones que generen archivos CSS, como PostCSS o [Tailwind CSS](/docs/14/app/building-your-application/styling/tailwind-css).
...
3. Un Componente del Cliente que envuelve tu aplicación con el registro de estilos durante el renderizado inicial del lado del servidor.

### `styled-jsx`

Usar `styled-jsx` en Componentes del Cliente requiere usar `v5.1.0`. Primero, crea un nuevo registro:
...
Luego, envuelve tu [layout raíz](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required) con el registro:
...
A continuación se muestra un ejemplo de cómo configurar `styled-components@6` o más reciente:
...
> - Durante el streaming, los estilos de cada fragmento se recolectarán y agregarán a los estilos existentes. Después de que la hidratación del lado del cliente se complete, `styled-components` tomará el control como de costumbre e inyectará cualquier estilo dinámico adicional.
> - Usamos específicamente un Componente del Cliente en el nivel superior del árbol para el registro de estilos porque es más eficiente extraer reglas CSS de esta manera. Evita regenerar estilos en renderizados posteriores del servidor y evita que se envíen en la carga útil del Componente del Servidor.
> - Para casos de uso avanzados donde necesites configurar propiedades individuales de la compilación de styled-components, puedes leer nuestra [referencia de API de styled-components para Next.js](/docs/architecture/nextjs-compiler#styled-components) para aprender más.
...
Incluimos [styled-jsx](https://github.com/vercel/styled-jsx) para proporcionar soporte para CSS con ámbito aislado.
El objetivo es admitir "CSS shadow" similar a los Web Components, que desafortunadamente [no admiten renderizado del servidor y son solo JS](https://github.com/w3c/webcomponents/issues/71).
