...
translation-updated-at: 2025-06-06T16:43:20.191Z
...
Next.js [determinará automáticamente](#image-sizing) el `width` y `height` de tu imagen basándose en el archivo importado. Estos valores se usan para prevenir [Cambio Acumulativo de Diseño (CLS)](https://nextjs.org/learn/seo/web-performance/cls) mientras tu imagen se carga.
...
Puedes configurar opcionalmente `localPatterns` en tu archivo `next.config.js` para permitir imágenes específicas y bloquear todas las demás.
...
Como Next.js no tiene acceso a archivos remotos durante el proceso de compilación, deberás proporcionar manualmente las propiedades [`width`](/docs/14/app/api-reference/components/image#width), [`height`](/docs/14/app/api-reference/components/image#height) y opcionalmente [`blurDataURL`](/docs/14/app/api-reference/components/image#blurdataurl).

Los atributos `width` y `height` se usan para inferir la relación de aspecto correcta de la imagen y evitar cambios de diseño mientras la imagen se carga. El `width` y `height` no determinan el tamaño renderizado del archivo de imagen. Aprende más sobre [Dimensionamiento de imágenes](#image-sizing).
...
Para permitir de forma segura la optimización de imágenes, define una lista de patrones URL admitidos en `next.config.js`. Sé lo más específico posible para evitar usos maliciosos. Por ejemplo, la siguiente configuración solo permitirá imágenes de un bucket específico de AWS S3:
...
Aprende más sobre la configuración de [`remotePatterns`](/docs/14/app/api-reference/components/image#remotepatterns). Si deseas usar URLs relativas para el `src` de la imagen, usa un [`loader`](/docs/14/app/api-reference/components/image#loader).
...
> Aprende más sobre la configuración de [`remotePatterns`](/docs/14/app/api-reference/components/image#remotepatterns).
...
Puedes definir un loader por imagen con la [propiedad `loader`](/docs/14/app/api-reference/components/image#loader), o a nivel de aplicación con la configuración [`loaderFile`](/docs/14/app/api-reference/components/image#loaderfile).

## Prioridad

Debes agregar la propiedad `priority` a la imagen que será el elemento [Largest Contentful Paint (LCP)](https://web.dev/lcp/#what-elements-are-considered) para cada página. Esto permite a Next.js priorizar especialmente la imagen para su carga (por ejemplo, mediante etiquetas de precarga o hints de prioridad), lo que genera una mejora significativa en LCP.
...
2. Explícitamente, incluyendo una propiedad [`width`](/docs/14/app/api-reference/components/image#width) y [`height`](/docs/14/app/api-reference/components/image#height)
3. Implícitamente, usando [fill](/docs/14/app/api-reference/components/image#fill) que hace que la imagen se expanda para llenar su elemento padre.
...
> **Usa `fill`**
>
> La propiedad [`fill`](/docs/14/app/api-reference/components/image#fill) permite que tu imagen se dimensione por su elemento padre. Considera usar CSS para dar espacio al elemento padre de la imagen en la página junto con la propiedad [`sizes`](/docs/14/app/api-reference/components/image#sizes) para que coincida con cualquier punto de interrupción de media query. También puedes usar [`object-fit`](https://developer.mozilla.org/docs/Web/CSS/object-fit) con `fill`, `contain` o `cover`, y [`object-position`](https://developer.mozilla.org/docs/Web/CSS/object-position) para definir cómo debe ocupar ese espacio la imagen.
>
...
- Usa `className` o `style`, no `styled-jsx`.
  - En la mayoría de los casos, recomendamos usar la propiedad `className`. Esto puede ser un [CSS Module](/docs/14/app/building-your-application/styling/css-modules) importado, una [hoja de estilos global](/docs/14/app/building-your-application/styling/css-modules#global-styles), etc.
  - También puedes usar la propiedad `style` para asignar estilos en línea.
  - No puedes usar [styled-jsx](/docs/14/app/building-your-application/styling/css-in-js) porque está limitado al componente actual (a menos que marques el estilo como `global`).
...
[**Ver todas las propiedades disponibles para el componente `next/image`.**](/docs/14/app/api-reference/components/image)

## Configuración

El componente `next/image` y la API de Optimización de Imágenes de Next.js se pueden configurar en el archivo [`next.config.js`](/docs/14/app/api-reference/next-config-js). Estas configuraciones te permiten [habilitar imágenes remotas](/docs/14/app/api-reference/components/image#remotepatterns), [definir breakpoints de imagen personalizados](/docs/14/app/api-reference/components/image#devicesizes), [cambiar el comportamiento de caché](/docs/14/app/api-reference/components/image#caching-behavior) y más.

[**Lee la documentación completa de configuración de imágenes para más información.**](/docs/14/app/api-reference/components/image#configuration-options)