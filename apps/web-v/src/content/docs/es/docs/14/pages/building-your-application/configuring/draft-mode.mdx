...
En la [documentación de páginas](/docs/14/pages/building-your-application/routing/pages-and-layouts) y la [documentación de obtención de datos](/docs/14/pages/building-your-application/data-fetching), explicamos cómo prerrenderizar una página en el momento de construcción (**Generación estática**) usando `getStaticProps` y `getStaticPaths`.
...
> Consulte primero la [documentación de rutas API](/docs/14/pages/building-your-application/routing/api-routes) si no está familiarizado con las rutas API de Next.js.
...
Luego, envíe una solicitud a `/api/disable-draft` para invocar la ruta API. Si usa [`next/link`](/docs/14/pages/api-reference/components/link) para llamar a esta ruta, debe pasar `prefetch={false}` para evitar eliminar accidentalmente la cookie durante el prefetch.

### Funciona con `getServerSideProps`

El Modo borrador funciona con `getServerSideProps`, y está disponible como una clave `draftMode` en el objeto [`context`](/docs/14/pages/api-reference/functions/get-server-side-props#context-parameter).

> **Nota importante**: No debe establecer el encabezado `Cache-Control` cuando use el Modo borrador, ya que no se puede omitir. En su lugar, recomendamos usar [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
