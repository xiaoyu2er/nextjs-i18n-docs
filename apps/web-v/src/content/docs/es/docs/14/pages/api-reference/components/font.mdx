...
translation-updated-at: 2025-06-06T16:40:58.777Z
...
Esta referencia de API te ayudará a entender cómo usar [`next/font/google`](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) y [`next/font/local`](/docs/14/app/building-your-application/optimizing/fonts#local-fonts). Para características y uso, consulta la página [Optimización de Fuentes](/docs/14/app/building-your-application/optimizing/fonts).

### Argumentos de la Función de Fuente

Para su uso, revisa [Fuentes de Google](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) y [Fuentes Locales](/docs/14/app/building-your-application/optimizing/fonts#local-fonts).
...
- Un array de valores de peso si la fuente no es una [fuente variable de Google](https://fonts.google.com/variablefonts). Aplica solo a `next/font/google`.
...
- Un array de valores de estilo si la fuente no es una [fuente variable de Google](https://fonts.google.com/variablefonts). Aplica solo a `next/font/google`.
...
Los [`subconjuntos`](https://fonts.google.com/knowledge/glossary/subsetting) de la fuente definidos por un array de strings con los nombres de cada subconjunto que deseas que sean [precargados](/docs/14/app/building-your-application/optimizing/fonts#specifying-a-subset). Las fuentes especificadas mediante `subsets` tendrán una etiqueta de precarga inyectada en el head cuando la opción [`preload`](#preload) sea true, que es el valor predeterminado.
...
El [`display`](https://developer.mozilla.org/docs/Web/CSS/@font-face/font-display) de la fuente con posibles [valores](https://developer.mozilla.org/docs/Web/CSS/@font-face/font-display#values) de string `'auto'`, `'block'`, `'swap'`, `'fallback'` o `'optional'` con valor predeterminado de `'swap'`.
...
Un valor booleano que especifica si la fuente debe ser [precargada](/docs/14/app/building-your-application/optimizing/fonts#preloading) o no. El valor predeterminado es `true`.
...
Un valor string para definir el nombre de la variable CSS que se usará si el estilo se aplica con el [método de variables CSS](#variables-css).
