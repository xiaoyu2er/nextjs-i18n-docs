...
Con soporte integrado para [transmisión mediante React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense), puedes ser más intencional sobre qué partes de tu interfaz de usuario deseas cargar primero y en qué orden sin introducir cascadas de red.
...
[Next.js Middleware](/docs/14/app/building-your-application/routing/middleware) te permite ejecutar código en el servidor antes de que se complete una solicitud. Esto es especialmente útil para evitar que aparezca contenido no autenticado cuando el usuario visita una página que requiere autenticación, redirigiendo al usuario a una página de inicio de sesión. El middleware también es útil para experimentación y [internacionalización](/docs/14/app/building-your-application/routing/internationalization).

### Optimizaciones integradas

[Imágenes](/docs/14/app/building-your-application/optimizing/images), [fuentes](/docs/14/app/building-your-application/optimizing/fonts) y [scripts de terceros](/docs/14/app/building-your-application/optimizing/scripts) suelen tener un impacto significativo en el rendimiento de una aplicación. Next.js viene con componentes integrados que los optimizan automáticamente.
...
Crea un archivo `next.config.mjs` en la raíz de tu proyecto. Este archivo contendrá tus [opciones de configuración de Next.js](/docs/14/app/api-reference/next-config-js).
...
Puedes encontrar más información sobre la configuración de TypeScript en la [documentación de Next.js](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin).

### Paso 4: Crear el diseño raíz (Root Layout)

Una aplicación de [App Router](/docs/14/app) de Next.js debe incluir un archivo de [diseño raíz](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required), que es un [componente de servidor de React](/docs/14/app/building-your-application/rendering/server-components) que envolverá todas las páginas de tu aplicación. Este archivo se define en el nivel superior del directorio `app`.
...
> **Nota importante**: Ignoraremos el [archivo de manifiesto](/docs/14/app/api-reference/file-conventions/metadata), iconografía adicional aparte del favicon y [configuración de pruebas](/docs/14/app/building-your-application/testing), pero si estos son requisitos, Next.js también admite estas opciones.
...
Cualquier [archivo de metadatos](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) como `favicon.ico`, `icon.png`, `robots.txt` se agrega automáticamente a la etiqueta `<head>` de la aplicación siempre que los hayas colocado en el nivel superior del directorio `app`. Después de mover [todos los archivos admitidos](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) al directorio `app`, puedes eliminar de manera segura sus etiquetas `<link>`:
...
Finalmente, Next.js puede gestionar tus últimas etiquetas `<head>` con la [API de Metadatos](/docs/14/app/building-your-application/optimizing/metadata). Mueve tu información de metadatos final a un objeto [`metadata` exportado](/docs/14/app/api-reference/functions/generate-metadata#metadata-object):
...
Con los cambios anteriores, pasaste de declarar todo en tu `index.html` a utilizar el enfoque basado en convenciones de Next.js integrado en el marco ([API de Metadatos](/docs/14/app/building-your-application/optimizing/metadata)). Este enfoque te permite mejorar más fácilmente el SEO y la capacidad de compartir tus páginas en la web.

### Paso 6: Estilos

Al igual que Create React App, Next.js tiene soporte integrado para [Módulos CSS](/docs/14/app/building-your-application/styling/css-modules).
...
Este directorio es lo que se llama un [segmento de ruta opcional catch-all](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments). Next.js utiliza un enrutador basado en sistema de archivos donde [los directorios se utilizan para definir rutas](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). Este directorio especial se asegurará de que todas las rutas de tu aplicación se dirijan a su archivo `page.tsx` contenido.
...
Este archivo es un [Componente de Servidor](/docs/14/app/building-your-application/rendering/server-components). Cuando ejecutes `next build`, el archivo se prerrenderiza en un activo estático. No requiere ningún código dinámico.

Este archivo importa nuestro CSS global y le dice a [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) que solo vamos a generar una ruta, la ruta de índice en `/`.
...
Este archivo es un [Componente de Cliente](/docs/14/app/building-your-application/rendering/client-components), definido por la directiva `'use client'`. Los Componentes de Cliente todavía se [prerrenderizan a HTML](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered) en el servidor antes de enviarse al cliente.
...
Next.js tiene soporte para [variables de entorno](/docs/14/app/building-your-application/configuring/environment-variables) `.env` similar a CRA.
...
Al migrar tu aplicación de CRA a Next.js, podrías tener una configuración personalizada de webpack que deseas migrar. Next.js soporta proporcionar una [configuración personalizada de webpack](/docs/14/app/api-reference/next-config-js/webpack).

Además, Next.js tiene soporte para [Turbopack](/docs/14/app/api-reference/next-config-js/turbo) a través de `next dev --turbo` para mejorar el rendimiento de desarrollo local. Turbopack también soporta algunos [loaders de webpack](/docs/14/app/api-reference/next-config-js/turbo) para compatibilidad y adopción incremental.
...
- Migrar de React Router al [App Router de Next.js](/docs/14/app/building-your-application/routing) para obtener:
  - División de código automática
  - [Renderizado del lado del servidor con streaming (Streaming Server-Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - [Componentes del lado del servidor (Server Components)](/docs/14/app/building-your-application/rendering/server-components)
- [Optimizar imágenes con el componente `<Image>`](/docs/14/app/building-your-application/optimizing/images)
- [Optimizar fuentes con `next/font`](/docs/14/app/building-your-application/optimizing/fonts)
- [Optimizar scripts de terceros con el componente `<Script>`](/docs/14/app/building-your-application/optimizing/scripts)
- [Actualizar tu configuración de ESLint para soportar reglas de Next.js](/docs/14/app/building-your-application/configuring/eslint)
