...
Cuando se usa [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) con encabezados, cada `source` se prefija automáticamente con el `basePath` a menos que agregue `basePath: false` al encabezado:
...
Cuando se usa [`i18n`](/docs/14/app/building-your-application/routing/internationalization) con encabezados, cada `source` se prefija automáticamente para manejar los `locales` configurados a menos que agregue `locale: false` al encabezado. Si se usa `locale: false`, debe prefijar el `source` con un locale para que coincida correctamente.

</AppOnly>

<PagesOnly>

Cuando se usa [`i18n`](/docs/14/pages/building-your-application/routing/internationalization) con encabezados, cada `source` se prefija automáticamente para manejar los `locales` configurados a menos que agregue `locale: false` al encabezado. Si se usa `locale: false`, debe prefijar el `source` con un locale para que coincida correctamente.
...
Aprenda más sobre [caché](/docs/14/app/building-your-application/caching) con el Enrutador de Aplicación.

</AppOnly>

<PagesOnly>

Si necesita revalidar la caché de una página que ha sido [generada estáticamente](/docs/14/pages/building-your-application/rendering/static-site-generation), puede hacerlo estableciendo la prop `revalidate` en la función [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) de la página.

Puede establecer el encabezado `Cache-Control` en sus [Rutas API](/docs/14/pages/building-your-application/routing/api-routes) usando el método `res.setHeader`:
...
**Este encabezado ha sido reemplazado por la opción `frame-ancestors` de CSP**, que tiene mejor soporte en navegadores modernos (ver [Content Security Policy](/docs/14/app/building-your-application/configuring/content-security-policy) para detalles de configuración).
...
Aprenda más sobre cómo agregar una [Content Security Policy](/docs/14/app/building-your-application/configuring/content-security-policy) a su aplicación.
