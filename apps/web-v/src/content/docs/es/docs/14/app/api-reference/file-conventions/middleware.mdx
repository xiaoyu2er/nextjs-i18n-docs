...
translation-updated-at: 2025-06-06T16:51:13.961Z
title: middleware.js
...
El archivo `middleware.js|ts` se utiliza para escribir [Middleware](/docs/14/app/building-your-application/routing/middleware) y ejecutar código en el servidor antes de que se complete una solicitud. Luego, basándose en la solicitud entrante, puede modificar la respuesta reescribiendo, redirigiendo, modificando los encabezados de solicitud o respuesta, o respondiendo directamente.
...
Opcionalmente, se puede exportar un objeto de configuración junto con la función Middleware. Este objeto incluye el [matcher](#matcher) para especificar las rutas donde aplica el Middleware.
...
El Middleware puede utilizar el objeto [`NextResponse`](/docs/14/app/building-your-application/routing/middleware#nextresponse) que extiende la [API de Respuesta Web](https://developer.mozilla.org/en-US/docs/Web/API/Response). Al devolver un objeto `NextResponse`, puede manipular directamente cookies, establecer encabezados, implementar redirecciones y reescribir rutas.

> **Nota importante**: Para redirecciones, también puede usar `Response.redirect` en lugar de `NextResponse.redirect`.

## Entorno de ejecución

El Middleware solo soporta el [entorno de ejecución Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes). No se puede utilizar el entorno de ejecución Node.js.
...
| `v12.0.9` | Se aplican URLs absolutas en el entorno Edge ([PR](https://github.com/vercel/next.js/pull/33410))    |
