...
Consulta la documentación de referencia de la API [`next/script`](/docs/14/app/api-reference/components/script#strategy) para aprender más sobre cada estrategia y sus casos de uso.

### Descarga de Scripts a un Web Worker (Experimental)

> **Advertencia:** La estrategia `worker` aún no es estable y no funciona con el directorio [`app`](/docs/14/app/building-your-application/routing/defining-routes). Úsala con precaución.
...
Estos manejadores solo funcionarán cuando `next/script` se importe y use dentro de un [Componente Cliente](/docs/14/app/building-your-application/rendering/client-components) donde `"use client"` esté definido como la primera línea de código:
...
Consulta la referencia de la API [`next/script`](/docs/14/app/api-reference/components/script#onload) para aprender más sobre cada manejador de eventos y ver ejemplos.
...
Consulta la referencia de la API [`next/script`](/docs/14/pages/api-reference/components/script#onload) para aprender más sobre cada manejador de eventos y ver ejemplos.
