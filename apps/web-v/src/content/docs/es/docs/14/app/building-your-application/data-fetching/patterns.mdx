...
sidebar:
  order: 2010203
  label: "Patrones y mejores prácticas para la obtención de datos"
---
...
Luego, puedes mutar o actualizar datos con [Acciones de Servidor (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
Esto es posible porque las solicitudes `fetch` se memorizan automáticamente. Más información sobre [memorización de solicitudes](/docs/14/app/building-your-application/caching#request-memoization)
...
Con Componentes de Servidor y [layouts anidados](/docs/14/app/building-your-application/routing/pages-and-layouts), puedes renderizar instantáneamente partes de la página que no requieren datos específicos, y mostrar un [estado de carga](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) para las partes que están obteniendo datos. Esto significa que el usuario no tiene que esperar a que cargue toda la página para comenzar a interactuar con ella.
...
Para más información sobre Streaming y Suspense, consulta las páginas de [UI de carga](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) y [Streaming con Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
Si tienes componentes anidados y cada uno obtiene sus propios datos, la obtención ocurrirá secuencialmente si esas solicitudes son diferentes (esto no aplica para solicitudes de los mismos datos, ya que se [memorizan](/docs/14/app/building-your-application/caching#request-memoization) automáticamente).
...
En casos como este, puedes usar [`loading.js`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) (para segmentos de ruta) o [React `<Suspense>`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense) (para componentes anidados) para mostrar un estado de carga mientras React transmite el resultado.
...
Para mejorar la experiencia de usuario, puedes añadir un [Límite de Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) para dividir el trabajo de renderizado y mostrar parte del resultado lo antes posible.
...
Con este enfoque, puedes obtener datos anticipadamente, cachear respuestas y garantizar que esta obtención de datos [solo ocurra en el servidor](/docs/14/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment).
...
> **Nota importante:**
>
> - Recomendamos usar el paquete [`server-only`](/docs/14/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment) para asegurar que las funciones de obtención de datos del servidor nunca se usen en el cliente.
