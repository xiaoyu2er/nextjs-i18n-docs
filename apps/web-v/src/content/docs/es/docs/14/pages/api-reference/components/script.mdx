...
Esta referencia de API te ayudará a entender cómo usar las [props](#props) disponibles para el componente Script. Para características y uso, consulta la página [Optimización de Scripts](/docs/14/app/building-your-application/optimizing/scripts).
...
Los scripts que se cargan con la estrategia `beforeInteractive` se inyectan en el HTML inicial desde el servidor, se descargan antes de cualquier módulo de Next.js y se ejecutan en el orden en que se colocan antes de que ocurra _cualquier_ hidratación en la página.
...
> **Advertencia:** La estrategia `worker` aún no es estable y aún no funciona con el directorio [`app`](/docs/14/app/building-your-application/routing/defining-routes). Usar con precaución.
