...
translation-updated-at: 2025-06-06T16:45:56.778Z
...
Cualquier `<Link />` en el viewport (inicialmente o mediante scroll) será prefetchado por defecto (incluyendo los datos correspondientes) para páginas que usen [Generación Estática](/docs/14/pages/building-your-application/data-fetching/get-static-props). Los datos correspondientes para rutas [renderizadas en el servidor](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) se obtienen _solo cuando_ se hace clic en el `<Link />`.

## Enlaces a rutas dinámicas

También puedes usar interpolación para crear la ruta, lo cual es útil para [segmentos de ruta dinámicos](/docs/14/pages/building-your-application/routing/dynamic-routes). Por ejemplo, para mostrar una lista de posts que se pasan al componente como prop:
...
Para acceder al [objeto `router`](/docs/14/pages/api-reference/functions/use-router#router-object) en un componente React puedes usar [`useRouter`](/docs/14/pages/api-reference/functions/use-router) o [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter).

En general recomendamos usar [`useRouter`](/docs/14/pages/api-reference/functions/use-router).

## Navegación Imperativa

[`next/link`](/docs/14/pages/api-reference/components/link) debería cubrir la mayoría de tus necesidades de enrutamiento, pero también puedes hacer navegaciones del lado del cliente sin él, revisa la [documentación de `next/router`](/docs/14/pages/api-reference/functions/use-router).

El siguiente ejemplo muestra cómo hacer navegaciones básicas de página con [`useRouter`](/docs/14/pages/api-reference/functions/use-router):
...
El enrutamiento superficial te permite cambiar la URL sin volver a ejecutar los métodos de obtención de datos, eso incluye [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props), [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props), y [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props).

Recibirás el `pathname` actualizado y el `query` a través del [objeto `router`](/docs/14/pages/api-reference/functions/use-router#router-object) (agregado por [`useRouter`](/docs/14/pages/api-reference/functions/use-router) o [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter)), sin perder el estado.
