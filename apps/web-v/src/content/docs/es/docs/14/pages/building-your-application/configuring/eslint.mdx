...
> Si no deseas que ESLint se ejecute durante `next build`, consulta la documentación para [Ignorar ESLint](/docs/14/app/api-reference/next-config-js/eslint).

</AppOnly>

<PagesOnly>

> Si no deseas que ESLint se ejecute durante `next build`, consulta la documentación para [Ignorar ESLint](/docs/14/pages/api-reference/next-config-js/eslint).
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | Previene errores comunes en [funciones de obtención de datos de Next.js](/docs/14/pages/building-your-application/data-fetching) |
...
Para mejorar el rendimiento, la información de los archivos procesados por ESLint se almacena en caché por defecto. Esto se guarda en `.next/cache` o en tu [directorio de compilación](/docs/14/pages/api-reference/next-config-js/distDir) definido. Si incluyes reglas de ESLint que dependen de más que el contenido de un solo archivo fuente y necesitas deshabilitar la caché, usa el flag `--no-cache` con `next lint`.
...
> El punto de entrada `next/core-web-vitals` se incluye automáticamente para nuevas aplicaciones creadas con [Create Next App](/docs/14/app/api-reference/cli/create-next-app).
...
- Tienes uno o más de los siguientes plugins ya instalados (ya sea por separado o a través de una configuración diferente como `airbnb` o `react-app`):
  ...
  - `jsx-a11y`
  - `import`
- Has definido `parserOptions` específicos que son diferentes a cómo está configurado Babel en Next.js (esto no se recomienda a menos que hayas [personalizado tu configuración de Babel](/docs/14/pages/building-your-application/configuring/babel))
