...
Los rewrites actúan como un proxy de URL y enmascaran la ruta de destino, haciendo parecer que el usuario no ha cambiado su ubicación en el sitio. En contraste, los [redirects](/docs/14/app/api-reference/next-config-js/redirects) redirigirán a una nueva página y mostrarán los cambios en la URL.

</AppOnly>

<PagesOnly>

Los rewrites actúan como un proxy de URL y enmascaran la ruta de destino, haciendo parecer que el usuario no ha cambiado su ubicación en el sitio. En contraste, los [redirects](/docs/14/pages/api-reference/next-config-js/redirects) redirigirán a una nueva página y mostrarán los cambios en la URL.
...
1. Se verifican/aplican los [headers](/docs/14/app/api-reference/next-config-js/headers)
2. Se verifican/aplican los [redirects](/docs/14/app/api-reference/next-config-js/redirects)
3. Se verifican/aplican los rewrites `beforeFiles`
4. Se verifican/sirven archivos estáticos del [directorio public](/docs/14/app/building-your-application/optimizing/static-assets), archivos `_next/static` y páginas no dinámicas
...
6. Se verifican/aplican los rewrites `fallback`, estos se aplican antes de renderizar la página 404 y después de verificar rutas dinámicas/todos los recursos estáticos. Si usas [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) en `getStaticPaths`, los rewrites `fallback` definidos en tu `next.config.js` _no_ se ejecutarán.

</AppOnly>

<PagesOnly>

1. Se verifican/aplican los [headers](/docs/14/pages/api-reference/next-config-js/headers)
2. Se verifican/aplican los [redirects](/docs/14/pages/api-reference/next-config-js/redirects)
3. Se verifican/aplican los rewrites `beforeFiles`
4. Se verifican/sirven archivos estáticos del [directorio public](/docs/14/pages/building-your-application/optimizing/static-assets), archivos `_next/static` y páginas no dinámicas
5. Se verifican/aplican los rewrites `afterFiles`, si uno de estos rewrites coincide, verificamos rutas dinámicas/archivos estáticos después de cada coincidencia
6. Se verifican/aplican los rewrites `fallback`, estos se aplican antes de renderizar la página 404 y después de verificar rutas dinámicas/todos los recursos estáticos. Si usas [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) en `getStaticPaths`, los rewrites `fallback` definidos en tu `next.config.js` _no_ se ejecutarán.
...
> **Bueno saber**: Las páginas estáticas de [Optimización Estática Automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) o [prerenderizado](/docs/14/pages/building-your-application/data-fetching/get-static-props) con parámetros de rewrites se analizarán en el cliente después de la hidratación y se proporcionarán en la query.
...
Cuando aprovechas el soporte de [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) con rewrites, cada `source` y `destination` se prefija automáticamente con el `basePath` a menos que agregues `basePath: false` al rewrite:
...
Cuando aprovechas el soporte de [`i18n`](/docs/14/app/building-your-application/routing/internationalization) con rewrites, cada `source` y `destination` se prefija automáticamente para manejar los `locales` configurados a menos que agregues `locale: false` al rewrite. Si usas `locale: false`, debes prefijar el `source` y `destination` con un locale para que coincida correctamente.

</AppOnly>

<PagesOnly>

Cuando aprovechas el soporte de [`i18n`](/docs/14/pages/building-your-application/routing/internationalization) con rewrites, cada `source` y `destination` se prefija automáticamente para manejar los `locales` configurados a menos que agregues `locale: false` al rewrite. Si usas `locale: false`, debes prefijar el `source` y `destination` con un locale para que coincida correctamente.
