...
> **Es bueno saberlo**:
>
> - Antes de decidir usar un servidor personalizado, tenga en cuenta que solo debe usarse cuando el enrutador integrado de Next.js no pueda satisfacer los requisitos de su aplicación. Un servidor personalizado eliminará optimizaciones de rendimiento importantes, como **funciones serverless** y **[Optimización estática automática](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)**.
...
| -------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| `conf`         | `Object`           | El mismo objeto que usaría en [next.config.js](/docs/14/pages/api-reference/next-config-js). Por defecto `{}`    |
...
> También puede configurar el enrutador del lado del cliente para prohibir redirecciones a rutas por nombre de archivo; para eso consulte [`router.beforePopState`](/docs/14/pages/api-reference/functions/use-router#routerbeforepopstate).