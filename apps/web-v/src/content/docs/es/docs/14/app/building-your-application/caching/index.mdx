...
>
> Para aprender más, consulta la documentación de [Server Components](/docs/14/app/building-your-application/rendering/server-components).
...
Aprende más sobre [renderizado estático y dinámico](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies).
...
- **[Revalidación de Datos](/docs/14/app/building-your-application/caching#revalidating)**: Revalidar la [Caché de Datos](#data-cache) invalidará a su vez la Caché del Enrutador al volver a renderizar los componentes en el servidor y almacenar en caché el nuevo resultado del renderizado.
...
A medida que un usuario navega entre rutas, Next.js almacena en caché los segmentos de ruta visitados y [precarga](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) las rutas a las que es probable que el usuario navegue (basándose en componentes `<Link>` en su viewport).

Esto resulta en una experiencia de navegación mejorada para el usuario:

- Navegación instantánea hacia atrás/adelante porque las rutas visitadas están almacenadas en caché y navegación rápida a nuevas rutas debido al prefetching y al [renderizado parcial](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering).
...
- **Período de Invalidación Automática**: La caché de diseños (layouts) y estados de carga se invalida automáticamente después de un tiempo específico. La duración depende de cómo se [precargó](/docs/14/app/api-reference/components/link#prefetch) el recurso y si el recurso fue [generado estáticamente](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default):
...
> **Bueno saber**: La opción experimental [`staleTimes`](/docs/14/app/api-reference/next-config-js/staleTimes) se puede utilizar para ajustar los tiempos de invalidación automática mencionados anteriormente.
...
- En una **Acción del Servidor**:
  - Revalidar datos bajo demanda por ruta con ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) o por etiqueta de caché con ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag))
  - Usar [`cookies.set`](/docs/14/app/api-reference/functions/cookies#cookiessetname-value-options) o [`cookies.delete`](/docs/14/app/api-reference/functions/cookies#deleting-cookies) invalida la Caché del Enrutador para evitar que las rutas que usan cookies se vuelvan obsoletas (por ejemplo, autenticación).
- Llamar a [`router.refresh`](/docs/14/app/api-reference/functions/use-router) invalidará la Caché del Enrutador y hará una nueva solicitud al servidor para la ruta actual.

### Exclusión

No es posible excluirse de la Caché del Enrutador. Sin embargo, puedes invalidarla llamando a [`router.refresh`](/docs/14/app/api-reference/functions/use-router), [`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) o [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag) (ver arriba). Esto borrará la caché y hará una nueva solicitud al servidor, asegurando que se muestren los datos más recientes.
...
- Revalidar la Caché de Datos en un [Manejador de Ruta (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) **no invalidará** inmediatamente la Caché del Enrutador, ya que el Manejador de Ruta no está vinculado a una ruta específica. Esto significa que la Caché del Enrutador seguirá sirviendo la carga útil anterior hasta que se realice una actualización dura o haya transcurrido el período de invalidación automática.
- Para invalidar inmediatamente la Caché de Datos y la Caché del Enrutador, puedes usar [`revalidatePath`](#revalidatepath) o [`revalidateTag`](#fetch-optionsnexttags-and-revalidatetag) en una [Acción del Servidor](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
Aprende más sobre el [componente `<Link>`](/docs/14/app/api-reference/components/link).
...
Consulta la referencia de la API del [hook `useRouter`](/docs/14/app/api-reference/functions/use-router).
...
Consulta la [Referencia de la API de `fetch`](/docs/14/app/api-reference/functions/fetch) para más opciones.
...
1. Al usar `fetch` o [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache), tiene la opción de etiquetar entradas de caché con una o más etiquetas.
...
1. [Manejadores de ruta (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) - para revalidar datos en respuesta a un evento de terceros (ej. un webhook). Esto no invalidará la caché del enrutador (Router Cache) inmediatamente ya que el Manejador de Ruta no está vinculado a una ruta específica.
2. [Acciones de servidor (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - para revalidar datos después de una acción del usuario (ej. envío de un formulario). Esto invalidará la caché del enrutador (Router Cache) para la ruta asociada.
...
1. [Manejadores de ruta (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) - para revalidar datos en respuesta a un evento de terceros (ej. un webhook).
2. [Acciones de servidor (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - para revalidar datos después de una interacción del usuario (ej. envío de un formulario, clic en un botón).

Consulte la [referencia de la API `revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) para obtener más información.
...
Consulte la referencia de la API [`cookies`](/docs/14/app/api-reference/functions/cookies).
...
Consulte la documentación de [Configuración de segmento de ruta (Route Segment Config)](/docs/14/app/api-reference/file-conventions/route-segment-config) para ver más opciones.

### `generateStaticParams`

Para [segmentos dinámicos](/docs/14/app/building-your-application/routing/dynamic-routes) (ej. `app/blog/[slug]/page.js`), las rutas proporcionadas por `generateStaticParams` se almacenan en la caché de ruta completa (Full Route Cache) en tiempo de compilación. En tiempo de solicitud, Next.js también almacenará en caché las rutas que no se conocían en tiempo de compilación la primera vez que se visitan.

Puede deshabilitar el almacenamiento en caché en tiempo de solicitud usando la opción `export const dynamicParams = false` en un segmento de ruta. Cuando se usa esta opción de configuración, solo se servirán las rutas proporcionadas por `generateStaticParams`, y otras rutas devolverán un error 404 o coincidirán (en el caso de [rutas comodín (catch-all routes)](/docs/14/app/building-your-application/routing/dynamic-routes#catch-all-segments)).

Consulte la [referencia de la API `generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params).
