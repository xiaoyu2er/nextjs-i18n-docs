...
El hook `useRouter` permite cambiar rutas programáticamente dentro de [Componentes del Cliente](/docs/14/app/building-your-application/rendering/client-components).

> **Recomendación:** Utilice el [componente `<Link>`](/docs/14/app/building-your-application/routing/linking-and-navigating#link-component) para navegación a menos que tenga un requisito específico para usar `useRouter`.
...
- `router.prefetch(href: string)`: [Prefetch](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) de la ruta proporcionada para transiciones más rápidas del lado del cliente.
...
- La cadena `pathname` ha sido eliminada y reemplazada por [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname)
- El objeto `query` ha sido eliminado y reemplazado por [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params)
- `router.events` ha sido reemplazado. [Ver abajo.](#router-events)

[Ver la guía completa de migración](/docs/14/app/building-your-application/upgrading/app-router-migration).
...
> **Es bueno saber**: `<NavigationEvents>` está envuelto en un límite [`Suspense`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#example) porque [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) causa renderizado del lado del cliente hasta el límite `Suspense` más cercano durante [renderizado estático](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). [Aprenda más](/docs/14/app/api-reference/functions/use-search-params#behavior).
