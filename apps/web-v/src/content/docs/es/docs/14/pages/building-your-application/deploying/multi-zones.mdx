...
No hay APIs especiales para definir una nueva zona. Una zona es una aplicación Next.js normal donde también configuras un [basePath](/docs/14/app/api-reference/next-config-js/basePath) para evitar conflictos con páginas y archivos estáticos en otras zonas.
...
Para enrutar a la zona correcta usando una aplicación Next.js, puedes usar [`rewrites`](/docs/14/app/api-reference/next-config-js/rewrites). Para cada ruta servida por una zona diferente, añadirías una regla de reescritura para enviar esa ruta al dominio de la otra zona. Por ejemplo:
...
Los enlaces a rutas en una zona diferente deben usar una etiqueta `a` en lugar del componente [`<Link>`](/docs/14/pages/api-reference/components/link) de Next.js. Esto es porque Next.js intentará precargar y navegar suavemente a cualquier ruta relativa en el componente `<Link>`, lo cual no funcionará entre zonas.
