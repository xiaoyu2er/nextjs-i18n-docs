...
Esto carga `process.env.DB_HOST`, `process.env.DB_USER` y `process.env.DB_PASS` en el entorno Node.js automáticamente, permitiéndote usarlos en [métodos de obtención de datos de Next.js](/docs/14/pages/building-your-application/data-fetching) y [rutas API](/docs/14/pages/building-your-application/routing/api-routes).

Por ejemplo, usando [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props):
...
> Esto carga `process.env.DB_HOST`, `process.env.DB_USER` y `process.env.DB_PASS` en el entorno Node.js automáticamente, permitiéndote usarlos en [Manejadores de Ruta](/docs/14/app/building-your-application/routing/route-handlers).
...
- Puedes ejecutar código al iniciar el servidor usando la [función `register`](/docs/14/app/building-your-application/optimizing/instrumentation).
...
- Si estás usando un [directorio `/src`](/docs/14/app/building-your-application/configuring/src-directory), los archivos `.env.*` deben permanecer en la raíz de tu proyecto.
