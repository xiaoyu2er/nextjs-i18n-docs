...
Turbopack en Next.js no requiere configuración para la mayoría de usuarios y puede extenderse para casos de uso más avanzados. Para conocer las funcionalidades actualmente soportadas por Turbopack, consulte la [Referencia de API](/docs/14/app/api-reference/next-config-js/turbo).
...
- Configuración de [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) en `next.config.js`
  - Turbopack reemplaza Webpack, lo que significa que la configuración de webpack no está soportada.
  - Para configurar Turbopack, [consulte la documentación](/docs/14/app/api-reference/next-config-js/turbo).
  - Un subconjunto de [loaders de Webpack](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) está soportado en Turbopack.
- Babel (`.babelrc`)
  ...
  - Si aún necesita usar Babel después de verificar que su caso de uso no está cubierto, puede aprovechar el [soporte de Turbopack para loaders personalizados de webpack](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) para incluir `babel-loader`.
...
- `@next/font` (soporte heredado para fuentes).
  - `@next/font` está obsoleto en favor de `next/font`. [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) está totalmente soportado con Turbopack.
