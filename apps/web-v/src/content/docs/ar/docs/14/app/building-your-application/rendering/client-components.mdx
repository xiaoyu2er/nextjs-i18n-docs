...
يستخدم `"use client"` لتعريف [حد فاصل](/docs/14/app/building-your-application/rendering#network-boundary) بين وحدات مكون الخادم ومكون العميل. هذا يعني أنه من خلال تعريف `"use client"` في ملف، فإن جميع الوحدات الأخرى المستوردة إليه، بما في ذلك المكونات الفرعية، تعتبر جزءًا من حزمة العميل.
...
يمكنك الاحتفاظ بالشفرة على الخادم حتى لو كانت نظريًا متداخلة داخل مكونات العميل عن طريق دمج مكونات العميل والخادم و[أفعال الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations). راجع صفحة [أنماط التكوين (Composition Patterns)](/docs/14/app/building-your-application/rendering/composition-patterns) لمزيد من المعلومات.