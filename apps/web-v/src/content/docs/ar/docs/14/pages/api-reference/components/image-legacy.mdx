...
عرض **الجديد** [مرجع API لـ `next/image`](/docs/14/pages/api-reference/components/image)
...
- ملف صورة [مستورد بشكل ثابت](/docs/14/pages/building-your-application/optimizing/images#local-images)
...
خاصية `width` مطلوبة، باستثناء [الصور المستوردة بشكل ثابت](/docs/14/pages/building-your-application/optimizing/images#local-images)، أو تلك التي تحتوي على `layout="fill"`.
...
خاصية `height` مطلوبة، باستثناء [الصور المستوردة بشكل ثابت](/docs/14/pages/building-your-application/optimizing/images#local-images)، أو تلك التي تحتوي على `layout="fill"`.
...
عند `blur`، سيتم استخدام خاصية [`blurDataURL`](#blurdataurl) كعنصر نائب. إذا كان `src` كائنًا من [استيراد ثابت](/docs/14/pages/building-your-application/optimizing/images#local-images) وكانت الصورة المستوردة من نوع `.jpg`، `.png`، `.webp`، أو `.avif`، فسيتم ملء `blurDataURL` تلقائيًا.
...
> لا يمكن تحسين الصور في وقت البناء باستخدام [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports)، فقط عند الطلب. لاستخدام `next/legacy/image` مع `output: 'export'`، ستحتاج إلى استخدام محمل مختلف عن الافتراضي. [اقرأ المزيد في المناقشة.](https://github.com/vercel/next.js/discussions/19065)
...
يمكنك تكوين وقت البقاء (TTL) بالثواني للصور المحسنة المخزنة مؤقتًا. في كثير من الحالات، من الأفضل استخدام [استيراد صورة ثابت (Static Image Import)](/docs/14/pages/building-your-application/optimizing/images#local-images) والذي سيقوم تلقائيًا بتجزئة محتويات الملف وتخزين الصورة مؤقتًا إلى الأبد مع رأس `Cache-Control` بقيمة `immutable`.
...
إذا كنت بحاجة إلى تغيير سلوك التخزين المؤقت لكل صورة، يمكنك تكوين [`headers`](/docs/14/pages/api-reference/next-config-js/headers) لتعيين رأس `Cache-Control` على الصورة الأصلية (مثل `/some-asset.jpg`، وليس `/_next/image` نفسه).
