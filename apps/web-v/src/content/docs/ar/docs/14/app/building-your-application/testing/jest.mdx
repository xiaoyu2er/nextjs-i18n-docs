...
translation-updated-at: 2025-06-06T17:40:39.671Z
title: إعداد Jest مع Next.js
nav_title: Jest
description: تعلم كيفية إعداد Jest مع Next.js لاختبار الوحدات واختبار اللقطات (Snapshot Testing).
sidebar:
  order: 2010802
  label: "Jest"
---
...
يتم استخدام Jest ومكتبة اختبار React (React Testing Library) معًا بشكل متكرر لـ **اختبار الوحدات (Unit Testing)** و**اختبار اللقطات (Snapshot Testing)**. سيوضح لك هذا الدليل كيفية إعداد Jest مع Next.js وكتابة أول اختباراتك.
...
- إعداد `transform` باستخدام [مترجم Next.js](/docs/architecture/nextjs-compiler)
- المحاكاة التلقائية لملفات الأنماط (`.css`, `.module.css`، ومتغيرات scss الخاصة بها)، واستيراد الصور و[`next/font`](/docs/14/pages/building-your-application/optimizing/fonts)
...
> **معلومة مفيدة:** لاختبار متغيرات البيئة مباشرة، قم بتحميلها يدويًا في سيناريو إعداد منفصل أو في ملف `jest.config.ts`. لمزيد من المعلومات، يرجى الاطلاع على [متغيرات بيئة الاختبار](/docs/14/pages/building-your-application/configuring/environment-variables#test-environment-variables).

<PagesOnly>

## إعداد Jest (مع Babel)

إذا اخترت عدم استخدام [مترجم Next.js](/docs/architecture/nextjs-compiler) واستخدمت Babel بدلاً من ذلك، فستحتاج إلى تكوين Jest يدويًا وتثبيت `babel-jest` و`identity-obj-proxy` بالإضافة إلى الحزم المذكورة أعلاه.
...
يمكنك معرفة المزيد حول كل خيار تكوين في [وثائق Jest](https://jestjs.io/docs/configuration). نوصي أيضًا بمراجعة [تكوين `next/jest`](https://github.com/vercel/next.js/blob/e02fe314dcd0ae614c65b505c6daafbdeebb920e/packages/next/src/build/jest/jest.ts) لمعرفة كيفية تكوين Next.js لـ Jest.
...
لمزيد من المعلومات حول التعامل مع الأصول الثابتة، يرجى الرجوع إلى [وثائق Jest](https://jestjs.io/docs/webpack#handling-static-assets).
...
إذا كان مشروعك يستخدم [أسماء مسار الوحدات (Module Path Aliases)](/docs/14/pages/building-your-application/configuring/absolute-imports-and-module-aliases)، فستحتاج إلى تكوين Jest لحل عمليات الاستيراد عن طريق مطابقة خيار paths في ملف `jsconfig.json` مع خيار `moduleNameMapper` في ملف `jest.config.js`. على سبيل المثال:
...
سيقوم `jest --watch` بإعادة تشغيل الاختبارات عند تغيير ملف. لمزيد من خيارات سطر أوامر Jest، يرجى الرجوع إلى [وثائق Jest](https://jestjs.io/docs/cli#reference).
...
على سبيل المثال، يمكننا إضافة اختبار للتحقق مما إذا كان المكون `<Page />` يقوم بعرض عنوان بنجاح:
...
اختياريًا، أضف [اختبار لقطة (snapshot test)](https://jestjs.io/docs/snapshot-testing) لتتبع أي تغييرات غير متوقعة في المكون الخاص بك:
