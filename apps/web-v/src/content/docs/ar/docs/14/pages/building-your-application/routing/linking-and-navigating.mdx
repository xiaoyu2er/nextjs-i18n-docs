...
سيتم جلب أي `<Link />` في نطاق الرؤية (مبدئيًا أو عن طريق التمرير) مسبقًا افتراضيًا (بما في ذلك البيانات المقابلة) للصفحات التي تستخدم [التوليد الثابت](/docs/14/pages/building-your-application/data-fetching/get-static-props). بينما يتم جلب البيانات المقابلة للطرق [المقدمة من الخادم](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) فقط عند النقر على `<Link />`.

## الربط بمسارات ديناميكية

يمكنك أيضًا استخدام الاستيفاء لإنشاء المسار، وهو مفيد لـ [شرائح المسار الديناميكية](/docs/14/pages/building-your-application/routing/dynamic-routes). على سبيل المثال، لعرض قائمة بالمقالات التي تم تمريرها إلى المكون كخاصية:
...
للوصول إلى [كائن الموجه](/docs/14/pages/api-reference/functions/use-router#router-object) في مكون React، يمكنك استخدام [`useRouter`](/docs/14/pages/api-reference/functions/use-router) أو [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter).

بشكل عام، نوصي باستخدام [`useRouter`](/docs/14/pages/api-reference/functions/use-router).

## التوجيه الأمرى

يجب أن يكون [`next/link`](/docs/14/pages/api-reference/components/link) قادرًا على تغطية معظم احتياجاتك من التوجيه، ولكن يمكنك أيضًا إجراء التنقل من جانب العميل بدونه، راجع [توثيق `next/router`](/docs/14/pages/api-reference/functions/use-router).

يوضح المثال التالي كيفية إجراء التنقل الأساسي بين الصفحات باستخدام [`useRouter`](/docs/14/pages/api-reference/functions/use-router):
...
يسمح التوجيه السطحي بتغيير عنوان URL دون إعادة تشغيل طرق جلب البيانات مرة أخرى، بما في ذلك [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)، و[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)، و[`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props).

ستتلقى `pathname` و`query` المحدثين عبر [كائن الموجه](/docs/14/pages/api-reference/functions/use-router#router-object) (المضاف بواسطة [`useRouter`](/docs/14/pages/api-reference/functions/use-router) أو [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter))، دون فقدان الحالة.
