...
تكوين كيفية تفاعل الطلب مع [ذاكرة التخزين المؤقت للبيانات](/docs/14/app/building-your-application/caching#data-cache) في Next.js.
...
> **معلومة مفيدة**:
>
> - إذا لم تقم بتوفير خيار `cache`، سيقوم Next.js بالافتراضي إلى `force-cache`، ما لم يتم استخدام [وظيفة ديناميكية](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) مثل `cookies()`، وفي هذه الحالة سيكون الافتراضي هو `no-store`.
...
> **معلومة مفيدة**:
>
> - إذا قام طلب `fetch()` فردي بتعيين رقم `revalidate` أقل من [الافتراضي `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) لمسار، فسيتم تقليل فاصل إعادة التحقق بالكامل للمسار.
