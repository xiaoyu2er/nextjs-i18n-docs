...
sidebar:
  order: 2010201
  label: "الجلب، التخزين المؤقت، وإعادة التحقق"
---
...
يمتد Next.js [واجهة برمجة تطبيقات `fetch` الأصلية](https://developer.mozilla.org/docs/Web/API/Fetch_API) للسماح لك بتكوين سلوك [التخزين المؤقت](#caching-data) و[إعادة التحقق](#revalidating-data) لكل طلب جلب بيانات على الخادم. يمتد React `fetch` لتلقائيًا [تذكر](/docs/14/app/building-your-application/data-fetching/patterns#fetching-data-where-its-needed) طلبات الجلب أثناء عرض شجرة مكونات React.

يمكنك استخدام `fetch` مع `async`/`await` في مكونات الخادم، [معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers)، و[إجراءات الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
> **معلومة مفيدة**:
>
> - يوفر Next.js دوال مساعدة قد تحتاجها عند جلب البيانات في مكونات الخادم مثل [`cookies`](/docs/14/app/api-reference/functions/cookies) و[`headers`](/docs/14/app/api-reference/functions/headers). سيؤدي هذا إلى عرض المسار بشكل ديناميكي حيث يعتمد على معلومات وقت الطلب.
> - في معالجات المسار (Route Handlers)، لا يتم تذكر طلبات `fetch` لأن معالجات المسار ليست جزءًا من شجرة مكونات React.
> - في [إجراءات الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)، لا يتم تخزين طلبات `fetch` مؤقتًا (الإعداد الافتراضي `cache: no-store`).
...
افتراضيًا، يقوم Next.js تلقائيًا بتخزين القيم المرجعة من `fetch` في [ذاكرة التخزين المؤقت للبيانات (Data Cache)](/docs/14/app/building-your-application/caching#data-cache) على الخادم. هذا يعني أنه يمكن جلب البيانات في وقت البناء أو وقت الطلب، تخزينها مؤقتًا، وإعادة استخدامها في كل طلب بيانات.
...
- تستخدم داخل [إجراء خادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
- تستخدم داخل [معالج مسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) يستخدم طريقة `POST`.
...
>
> تعلم المزيد عن [ذاكرة التخزين المؤقت للبيانات](/docs/14/app/building-your-application/caching#data-cache).
...
بدلاً من ذلك، لإعادة التحقق من جميع طلبات `fetch` في جزء المسار، يمكنك استخدام [خيارات تكوين الجزء (Segment Config Options)](/docs/14/app/api-reference/file-conventions/route-segment-config).
...
تعلم المزيد عن [إعادة التحقق المبنية على الوقت](/docs/14/app/building-your-application/caching#time-based-revalidation).

#### إعادة التحقق عند الطلب

يمكن إعادة التحقق من البيانات عند الطلب عن طريق المسار ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) أو عن طريق علامة التخزين المؤقت ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)) داخل [إجراء خادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) أو [معالج مسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
...
تعلم المزيد عن [إعادة التحقق عند الطلب](/docs/14/app/building-your-application/caching#on-demand-revalidation).
...
عرض جميع خيارات `cache` المتاحة في [مرجع واجهة برمجة تطبيقات `fetch`](/docs/14/app/api-reference/functions/fetch).

#### طلبات `fetch` المتعددة

إذا كان لديك عدة طلبات `fetch` في جزء مسار (مثل تخطيط أو صفحة)، يمكنك تكوين سلوك التخزين المؤقت لجميع طلبات البيانات في الجزء باستخدام [خيارات تكوين الجزء (Segment Config Options)](/docs/14/app/api-reference/file-conventions/route-segment-config).
...
في الحالات التي تستخدم فيها مكتبة طرف ثالث لا تدعم أو تعرض `fetch` (مثل قاعدة بيانات، نظام إدارة محتوى، أو عميل ORM)، يمكنك تكوين سلوك التخزين المؤقت وإعادة التحقق من تلك الطلبات باستخدام [خيار تكوين جزء المسار (Route Segment Config Option)](/docs/14/app/api-reference/file-conventions/route-segment-config) ودالة `cache` في React.

سيعتمد ما إذا كانت البيانات مخزنة مؤقتًا أم لا على ما إذا كان جزء المسار [معروضًا بشكل ثابت أو ديناميكي](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies). إذا كان الجزء ثابتًا (افتراضيًا)، سيتم تخزين مخرجات الطلب مؤقتًا وإعادة التحقق منها كجزء من جزء المسار. إذا كان الجزء ديناميكيًا، لن يتم تخزين مخرجات الطلب مؤقتًا وسيتم إعادة جلبها في كل طلب عند عرض الجزء.

يمكنك أيضًا استخدام واجهة برمجة التطبيقات التجريبية [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache).

### مثال

في المثال أدناه:

- يتم استخدام دالة React `cache` [لتذكر](/docs/14/app/building-your-application/caching#request-memoization) طلبات البيانات.
...
إذا كنت بحاجة إلى جلب البيانات في مكون عميل، يمكنك استدعاء [معالج مسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) من العميل. تعمل معالجات المسار على الخادم وتعيد البيانات إلى العميل. هذا مفيد عندما لا تريد الكشف عن معلومات حساسة للعميل، مثل رموز API.

راجع وثائق [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) للحصول على أمثلة.
