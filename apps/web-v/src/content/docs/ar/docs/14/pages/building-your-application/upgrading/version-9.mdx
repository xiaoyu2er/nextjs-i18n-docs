...
إذا كنت قد قمت بتكوين `routes` في ملف `vercel.json` سابقًا للطرق الديناميكية، يمكن إزالة هذه القواعد عند استخدام ميزة [التوجيه الديناميكي](/docs/14/pages/building-your-application/routing/dynamic-routes) الجديدة في Next.js 9.

طرق Next.js 9 الديناميكية **مكونة تلقائيًا على [Vercel](https://vercel.com/)** ولا تتطلب أي تخصيص في `vercel.json`.

يمكنك قراءة المزيد عن [التوجيه الديناميكي هنا](/docs/14/pages/building-your-application/routing/dynamic-routes).

## تحقق من ملف التطبيق المخصص (`pages/_app.js`)

إذا كنت قد نسخت مثال [التطبيق المخصص `<App>`](/docs/14/pages/building-your-application/routing/custom-app) سابقًا، فقد تتمكن من إزالة `getInitialProps`.
