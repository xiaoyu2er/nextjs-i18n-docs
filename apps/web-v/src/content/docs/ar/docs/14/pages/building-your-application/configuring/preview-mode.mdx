...
translation-updated-at: 2025-06-06T17:25:28.898Z
title: وضع المعاينة
...
> **ملاحظة**: تم استبدال هذه الميزة بـ [وضع المسودة](/docs/14/pages/building-your-application/configuring/draft-mode).
...
في [توثيق الصفحات](/docs/14/pages/building-your-application/routing/pages-and-layouts) و[توثيق جلب البيانات](/docs/14/pages/building-your-application/data-fetching)، تحدثنا عن كيفية تقديم صفحة مسبقًا في وقت البناء (**التوليد الثابت**) باستخدام `getStaticProps` و `getStaticPaths`.
...
> ألق نظرة على [توثيق مسارات API](/docs/14/pages/building-your-application/routing/api-routes) أولاً إذا لم تكن معتادًا على مسارات API في Next.js.
...
في الواقع، سترغب في استدعاء مسار API هذا _بأمان_ من نظام إدارة المحتوى الخاص بك. الخطوات المحددة ستختلف اعتمادًا على نظام إدارة المحتوى الذي تستخدمه، ولكن إليك بعض الخطوات الشائعة التي يمكنك اتخاذها.
...
> **من الجيد معرفة**: أثناء العرض، يعرض `next/router` علامة `isPreview`، راجع [توثيق كائن الموجه](/docs/14/pages/api-reference/functions/use-router#router-object) لمزيد من المعلومات.
...
ثم، أرسل طلبًا إلى `/api/clear-preview-mode-cookies` لاستدعاء مسار API. إذا كنت تستدعي هذا المسار باستخدام [`next/link`](/docs/14/pages/api-reference/components/link)، فيجب عليك تمرير `prefetch={false}` لمنع استدعاء `clearPreviewData` أثناء الجلب المسبق للرابط.
...
> **من الجيد معرفة**: لا يجب عليك تعيين رأس `Cache-Control` عند استخدام وضع المعاينة لأنه لا يمكن تجاوزه. بدلاً من ذلك، نوصي باستخدام [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
