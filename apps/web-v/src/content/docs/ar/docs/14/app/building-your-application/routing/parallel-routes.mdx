...
ومع ذلك، الفتحات **ليست** [أجزاء مسار (route segments)](/docs/14/app/building-your-application/routing#route-segments) ولا تؤثر على بنية URL. على سبيل المثال، بالنسبة لـ `/@analytics/views`، سيكون URL هو `/views` لأن `@analytics` فتحة.
...
- [**التنقل اللين (Soft Navigation)**](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation): أثناء التنقل من جانب العميل، سيقوم Next.js بعمل [عرض جزئي (partial render)](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)، تغيير الصفحة الفرعية داخل الفتحة، مع الحفاظ على الصفحات الفرعية النشطة للفتحات الأخرى، حتى لو لم تتطابق مع URL الحالي.
...
كل من [`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) و [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) يقبلان معلمة `parallelRoutesKey`، مما يسمح لك بقراءة جزء المسار النشط داخل فتحة.
...
داخل `@analytics`، أنشئ ملف [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts) لمشاركة علامات التبويب بين الصفحتين:
...
يمكن استخدام المسارات المتوازية مع [مسارات الاعتراض (Intercepting Routes)](/docs/14/app/building-your-application/routing/intercepting-routes) لإنشاء نوافذ منبثقة. هذا يسمح لك بحل التحديات الشائعة عند بناء النوافذ المنبثقة، مثل:
...
ثم، داخل فتحة `@auth`، أضف ملف [`default.js`](/docs/14/app/api-reference/file-conventions/default) الذي يُرجع `null`. هذا يضمن عدم عرض النافذة المنبثقة عندما لا تكون نشطة.
...
> **معلومة مفيدة:**
>
> - الاصطلاح المستخدم لاعتراض المسار، مثل `(.)`، يعتمد على بنية نظام الملفات لديك. راجع [اصطلاح مسارات الاعتراض (Intercepting Routes convention)](/docs/14/app/building-your-application/routing/intercepting-routes#convention).
> - من خلال فصل وظيفة `<Modal>` عن محتوى النافذة المنبثقة (`<Login>`)، يمكنك التأكد من أن أي محتوى داخل النافذة المنبثقة، مثل [النماذج (forms)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations#forms)، هي مكونات خادم (Server Components). راجع [دمج مكونات العميل والخادم (Interleaving Client and Server Components)](/docs/14/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props) لمزيد من المعلومات.
...
راجع وثائق [واجهة التحميل (Loading UI)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) و [معالجة الأخطاء (Error Handling)](/docs/14/app/building-your-application/routing/error-handling) لمزيد من المعلومات.