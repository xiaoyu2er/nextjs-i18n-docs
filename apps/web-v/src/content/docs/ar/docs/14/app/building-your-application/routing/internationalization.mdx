...
يمكن تدويل التوجيه إما عن طريق المسار الفرعي (`/fr/products`) أو النطاق (`my-site.fr/products`). باستخدام هذه المعلومات، يمكنك الآن إعادة توجيه المستخدم بناءً على المحلية داخل [الوسيط (Middleware)](/docs/14/app/building-your-application/routing/middleware).
...
لأن جميع التخطيطات والصفحات في دليل `app/` تعمل افتراضيًا كمكونات خادم [Server Components](/docs/14/app/building-your-application/rendering/server-components)، لا داعي للقلق بشأن حجم ملفات الترجمة الذي يؤثر على حجم حزمة JavaScript على جانب العميل. هذا الكود **سيعمل فقط على الخادم**، وسيتم إرسال HTML الناتج فقط إلى المتصفح.
