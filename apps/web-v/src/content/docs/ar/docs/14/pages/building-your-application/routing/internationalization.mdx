...
مع Next.js 12 و[البرمجية الوسيطة (Middleware)](/docs/14/pages/building-your-application/routing/middleware)، يمكننا إضافة بادئة للغة الافتراضية باستخدام [حل بديل](https://github.com/vercel/next.js/discussions/18419).
...
بعد ذلك، يمكننا استخدام [البرمجية الوسيطة (Middleware)](/docs/14/pages/building-your-application/routing/middleware) لإضافة قواعد توجيه مخصصة:
...
تتخطى هذه [البرمجية الوسيطة (Middleware)](/docs/14/pages/building-your-application/routing/middleware) إضافة البادئة الافتراضية إلى [مسارات API](/docs/14/pages/building-your-application/routing/api-routes) وملفات [العامة](/docs/14/pages/building-your-application/optimizing/static-assets) مثل الخطوط أو الصور. إذا تم تقديم طلب للغة الافتراضية، نقوم بإعادة التوجيه إلى البادئة `/en`.
...
يمكنك الوصول إلى معلومات اللغة عبر موجه Next.js. على سبيل المثال، باستخدام خطاف [`useRouter()`](/docs/14/pages/api-reference/functions/use-router) تكون الخصائص التالية متاحة:
...
عند [التقديم المسبق](/docs/14/pages/building-your-application/rendering/static-site-generation) للصفحات باستخدام `getStaticProps` أو `getServerSideProps`، يتم توفير معلومات اللغة في [السياق](/docs/14/pages/building-your-application/data-fetching/get-static-props) المقدم للدالة.
...
لاحظ أنه للتعامل مع تبديل اللغة `locale` فقط مع الحفاظ على جميع معلومات التوجيه مثل قيم استعلام [المسار الديناميكي](/docs/14/pages/building-your-application/routing/dynamic-routes) أو قيم استعلام href المخفية، يمكنك توفير معلمة `href` ككائن:
...
راجع [هنا](/docs/14/pages/api-reference/functions/use-router#with-url-object) لمزيد من المعلومات حول هيكل الكائن لـ `router.push`.
...
لا يعرف Next.js عن متغيرات الصفحة، لذا فإن الأمر يعود إليك لإضافة علامات `hreflang` باستخدام [`next/head`](/docs/14/pages/api-reference/components/head). يمكنك معرفة المزيد عن `hreflang` في [توثيق Google Webmasters](https://support.google.com/webmasters/answer/189077).

## كيف يعمل هذا مع التوليد الثابت؟

> لاحظ أن التوجيه الدولي لا يتكامل مع [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) لأنه لا يستفيد من طبقة توجيه Next.js. يتم دعم تطبيقات Next.js الهجينة التي لا تستخدم `output: 'export'` بالكامل.

### المسارات الديناميكية وصفحات `getStaticProps`

بالنسبة للصفحات التي تستخدم `getStaticProps` مع [المسارات الديناميكية](/docs/14/pages/building-your-application/routing/dynamic-routes)، يجب إرجاع جميع متغيرات اللغة للصفحة المراد تقديمها مسبقًا من [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths). إلى جانب كائن `params` الذي تم إرجاعه لـ `paths`، يمكنك أيضًا إرجاع حقل `locale` يحدد اللغة التي تريد تقديمها. على سبيل المثال:
...
بالنسبة لصفحات [التحسين الثابت التلقائي](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) وصفحات `getStaticProps` غير الديناميكية، **سيتم إنشاء نسخة من الصفحة لكل لغة**. هذا مهم للنظر فيه لأنه يمكن أن يزيد من أوقات البناء اعتمادًا على عدد اللغات المكونة داخل `getStaticProps`.
...
لتقليل وقت بناء الصفحات الديناميكية مع `getStaticProps`، استخدم [وضع `fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true). هذا يسمح لك بإرجاع المسارات واللغات الأكثر شيوعًا فقط من `getStaticPaths` للتقديم المسبق أثناء البناء. ثم سيقوم Next.js ببناء الصفحات المتبقية في وقت التشغيل عند طلبها.

### صفحات التحسين الثابت التلقائي

بالنسبة للصفحات التي يتم [تحسينها ثابتًا تلقائيًا](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)، سيتم إنشاء نسخة من الصفحة لكل لغة.
...
> **من الجيد معرفة**: تمت إضافة هذه الحدود في البداية لمنع [مشكلات الأداء المحتملة في وقت البناء](#dynamic-routes-and-getstaticprops-pages). يمكنك تجاوز هذه الحدود باستخدام التوجيه المخصص عبر [البرمجية الوسيطة (Middleware)](/docs/14/pages/building-your-application/routing/middleware) في Next.js 12.