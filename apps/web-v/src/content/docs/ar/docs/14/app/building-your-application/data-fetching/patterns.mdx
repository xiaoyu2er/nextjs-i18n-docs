...
sidebar:
  order: 2010203
  label: "أنماط وأفضل ممارسات جلب البيانات"
---
...
ثم يمكنك تعديل أو تحديث البيانات باستخدام [الإجراءات الخادمية (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
هذا ممكن لأن طلبات `fetch` يتم تذكرها تلقائيًا. تعلم المزيد عن [تذكر الطلبات (request memoization)](/docs/14/app/building-your-application/caching#request-memoization)
...
مع مكونات الخادم و [التخطيطات المتداخلة (nested layouts)](/docs/14/app/building-your-application/routing/pages-and-layouts)، يمكنك تصيير أجزاء الصفحة التي لا تتطلب بيانات على الفور، وعرض [حالة التحميل (loading state)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) للأجزاء التي تقوم بجلب البيانات. هذا يعني أن المستخدم لا يضطر إلى انتظار تحميل الصفحة بالكامل قبل أن يتمكن من التفاعل معها.
...
للمزيد عن البث والتعليق، راجع صفحات [واجهة التحميل (Loading UI)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) و [البث مع التعليق (Streaming with Suspense)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
إذا كان لديك مكونات متداخلة، وكل مكون يجلب بياناته الخاصة، فسيحدث جلب البيانات بشكل تسلسلي إذا كانت هذه الطلبات مختلفة (هذا لا ينطبق على الطلبات لنفس البيانات لأنها يتم تذكرها تلقائيًا [memoized](/docs/14/app/building-your-application/caching#request-memoization)).
...
في مثل هذه الحالات، يمكنك استخدام [`loading.js`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) (لقطاعات المسار) أو [React `<Suspense>`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense) (للمكونات المتداخلة) لعرض حالة تحميل فورية أثناء قيام React ببث النتيجة.
...
لتحسين تجربة المستخدم، يمكنك إضافة [حدود التعليق (Suspense Boundary)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) لتقسيم عمل التصيير وعرض جزء من النتيجة في أسرع وقت ممكن.
...
مع هذا النهج، يمكنك جلب البيانات بفارغ الصبر، وتخزين الردود، وضمان أن جلب البيانات هذا [يحدث فقط على الخادم](/docs/14/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment).

يمكن استخدام صادرات `utils/get-item` بواسطة التخطيطات أو الصفحات أو المكونات الأخرى لمنحها التحكم في وقت جلب بيانات العنصر.

> **معلومة مفيدة:**
>
> - نوصي باستخدام حزمة [`server-only`](/docs/14/app/building-your-application/rendering/composition-patterns#keeping-server-only-code-out-of-the-client-environment) للتأكد من عدم استخدام دوال جلب بيانات الخادم على العميل أبدًا.
