...
translation-updated-at: 2025-06-06T17:19:30.978Z
title: رؤوس (headers)
...
عند استخدام [دعم `basePath`](/docs/14/app/api-reference/next-config-js/basePath) مع الرؤوس، يتم تلقائيًا إضافة بادئة `basePath` إلى كل `source` ما لم تقم بإضافة `basePath: false` إلى الرأس:
...
عند استخدام [دعم `i18n`](/docs/14/app/building-your-application/routing/internationalization) مع الرؤوس، يتم تلقائيًا إضافة بادئة إلى كل `source` للتعامل مع `locales` المكونة ما لم تقم بإضافة `locale: false` إلى الرأس. إذا تم استخدام `locale: false` فيجب إضافة بادئة اللغة إلى `source` لمطابقتها بشكل صحيح.

</AppOnly>

<PagesOnly>

عند استخدام [دعم `i18n`](/docs/14/pages/building-your-application/routing/internationalization) مع الرؤوس، يتم تلقائيًا إضافة بادئة إلى كل `source` للتعامل مع `locales` المكونة ما لم تقم بإضافة `locale: false` إلى الرأس. إذا تم استخدام `locale: false` فيجب إضافة بادئة اللغة إلى `source` لمطابقتها بشكل صحيح.
...
تعرف المزيد عن [التخزين المؤقت](/docs/14/app/building-your-application/caching) مع موجه التطبيق.

</AppOnly>

<PagesOnly>

إذا كنت بحاجة إلى إعادة التحقق من التخزين المؤقت لصفحة تم [إنشاؤها بشكل ثابت](/docs/14/pages/building-your-application/rendering/static-site-generation)، يمكنك القيام بذلك عن طريق تعيين خاصية `revalidate` في دالة [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) للصفحة.

يمكنك تعيين رأس `Cache-Control` في [مسارات API](/docs/14/pages/building-your-application/routing/api-routes) باستخدام طريقة `res.setHeader`:
...
**تم استبدال هذا الرأس بخيار `frame-ancestors` في CSP**، والذي يتمتع بدعم أفضل في المتصفحات الحديثة (راجع [سياسة أمان المحتوى](/docs/14/app/building-your-application/configuring/content-security-policy) للحصول على تفاصيل التكوين).
...
تعرف على المزيد حول إضافة [سياسة أمان المحتوى](/docs/14/app/building-your-application/configuring/content-security-policy) إلى تطبيقك.
