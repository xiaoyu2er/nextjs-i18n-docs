...
يُستخدم ملف `default.js` لعرض بديل احتياطي داخل [المسارات المتوازية (Parallel Routes)](/docs/14/app/building-your-application/routing/parallel-routes) عندما لا يتمكن Next.js من استعادة الحالة النشطة [للفتحة (slot)](/docs/14/app/building-your-application/routing/parallel-routes#slots) بعد تحميل الصفحة بالكامل.

أثناء [التنقل الناعم (soft navigation)](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation)، يحتفظ Next.js بتتبع الحالة _النشطة_ (الصفحة الفرعية) لكل فتحة. ومع ذلك، بالنسبة للتنقلات الصلبة (تحميل الصفحة بالكامل)، لا يمكن لـ Next.js استعادة الحالة النشطة. في هذه الحالة، يمكن عرض ملف `default.js` للصفحات الفرعية التي لا تتطابق مع عنوان URL الحالي.
...
كائن يحتوي على [معلمات المسار الديناميكي (dynamic route parameters)](/docs/14/app/building-your-application/routing/dynamic-routes) من الجزء الجذري وصولاً إلى الصفحات الفرعية للفتحة. على سبيل المثال:
