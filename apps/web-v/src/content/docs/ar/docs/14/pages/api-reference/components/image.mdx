...
> **معلومة مفيدة**: إذا كنت تستخدم إصدارًا من Next.js قبل الإصدار 13، فستحتاج إلى استخدام [وثائق next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) حيث تم تغيير اسم المكون.

</PagesOnly>

سيساعدك مرجع API هذا على فهم كيفية استخدام [الخصائص (props)](#props) وخيارات [التكوين](#configuration-options) المتاحة لمكون الصورة. لميزات الاستخدام، يرجى زيارة صفحة [مكون الصورة](/docs/14/app/building-your-application/optimizing/images).
...
- ملف صورة [مستورد بشكل ثابت](/docs/14/app/building-your-application/optimizing/images#local-images)
...
مطلوبة، باستثناء [الصور المستوردة بشكل ثابت](/docs/14/app/building-your-application/optimizing/images#local-images) أو الصور ذات خاصية [`fill`](#fill).
...
> **معلومة مفيدة**: استخدام خصائص مثل `loader`، التي تقبل دالة، يتطلب استخدام [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components) لتحويل الدالة المقدمة إلى سلسلة.
...
عند `blur`، سيتم استخدام خاصية [`blurDataURL`](#blurdataurl) كعنصر نائب. إذا كان `src` كائنًا من [استيراد ثابت](/docs/14/app/building-your-application/optimizing/images#local-images) وكانت الصورة المستوردة `.jpg`، `.png`، `.webp`، أو `.avif`، فسيتم ملء `blurDataURL` تلقائيًا، إلا إذا تم اكتشاف أن الصورة متحركة.
...
> **معلومة مفيدة**: استخدام خصائص مثل `onLoadingComplete`، التي تقبل دالة، يتطلب استخدام [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components) لتحويل الدالة المقدمة إلى سلسلة.
...
> **معلومة مفيدة**: استخدام خصائص مثل `onLoad`، التي تقبل دالة، يتطلب استخدام [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components) لتحويل الدالة المقدمة إلى سلسلة.
...
> **معلومة مفيدة**: استخدام خصائص مثل `onError`، التي تقبل دالة، يتطلب استخدام [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components) لتحويل الدالة المقدمة إلى سلسلة.
...
- [تكوين محمل الصور المخصص](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **جيد للمعرفة**: تخصيص ملف محمل الصور، الذي يقبل دالة، يتطلب استخدام [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components) لتحويل الدالة المقدمة إلى سلسلة.
...
يمكنك تكوين وقت البقاء (TTL) بالثواني للصور المحسنة المخزنة مؤقتًا. في كثير من الحالات، من الأفضل استخدام [استيراد صورة ثابتة (Static Image Import)](/docs/14/app/building-your-application/optimizing/images#local-images) والتي ستقوم تلقائيًا بتجزئة محتويات الملف وتخزين الصورة مؤقتًا إلى الأبد مع رأس `Cache-Control` بقيمة `immutable`.
...
إذا كنت بحاجة إلى تغيير سلوك التخزين المؤقت لكل صورة، يمكنك تكوين [`headers`](/docs/14/app/api-reference/next-config-js/headers) لتعيين رأس `Cache-Control` على الصورة الأصلية (مثل `/some-asset.jpg`، وليس `/_next/image` نفسها).
...
| `v13.0.0`  | تم تغيير اسم استيراد `next/image` إلى `next/legacy/image`. تم تغيير اسم استيراد `next/future/image` إلى `next/image`. يتوفر [أداة تحويل تلقائي](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) لإعادة تسمية الاستيرادات بشكل آلي وآمن. تمت إزالة غلاف `<span>`. تمت إزالة الخصائص `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot`. أصبحت `alt` إلزامية. تستقبل `onLoadingComplete` مرجعًا لعنصر `img`. تمت إزالة تكوين المحمل المدمج. |
