...
sidebar:
  order: 3010303
  label: "النماذج والتحويلات (Forms and Mutations)"
---
...
> **معلومة مفيدة:**
>
> - سنوصي قريبًا [باعتماد تدريجي](/docs/14/app/building-your-application/upgrading/app-router-migration) لموجه التطبيق (App Router) واستخدام [إجراءات الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) للتعامل مع إرسال النماذج وتحويل البيانات. تسمح إجراءات الخادم بتعريف دوال غير متزامنة على الخادم يمكن استدعاؤها مباشرة من مكوناتك، دون الحاجة إلى إنشاء مسار API يدويًا.
...
> - نظرًا لأن مسارات API تعمل على الخادم، يمكننا استخدام قيم حساسة (مثل مفاتيح API) عبر [متغيرات البيئة (Environment Variables)](/docs/14/pages/building-your-application/configuring/environment-variables) دون الكشف عنها للعميل. هذا أمر بالغ الأهمية لأمان تطبيقك.
...
إذا كنت ترغب في توجيه المستخدم إلى مسار مختلف بعد التحويل، يمكنك استخدام [`redirect`](/docs/14/pages/building-your-application/routing/api-routes#response-helpers) لتوجيهه إلى أي عنوان URL مطلق أو نسبي:
...
يمكنك قراءة ملفات تعريف الارتباط داخل مسار API باستخدام مساعد الطلب [`cookies`](/docs/14/pages/building-your-application/routing/api-routes#request-helpers):
