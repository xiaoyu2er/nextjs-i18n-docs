...
[مرجع واجهة برمجة التطبيقات `getStaticProps`](/docs/14/pages/api-reference/functions/get-static-props) يغطي جميع المعاملات والخصائص التي يمكن استخدامها مع `getStaticProps`.
...
- `getStaticProps` يعمل في الخلفية عند استخدام [`fallback: true`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)
- `getStaticProps` يتم استدعاؤه قبل العرض الأولي عند استخدام [`fallback: blocking`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking)
- `getStaticProps` يعمل في الخلفية عند استخدام `revalidate`
- `getStaticProps` يعمل عند الطلب في الخلفية عند استخدام [`revalidate()`](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)

عند استخدامه مع [التجديد الثابت التدريجي (Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration)، سيعمل `getStaticProps` في الخلفية أثناء إعادة التحقق من الصفحة القديمة، وسيتم تقديم الصفحة الجديدة للمتصفح.

لا يمكن لـ `getStaticProps` الوصول إلى الطلب الوارد (مثل معلمات الاستعلام أو رؤوس HTTP) لأنه يولد HTML ثابتًا. إذا كنت بحاجة إلى الوصول إلى الطلب لصفحتك، ففكر في استخدام [الوسيط (Middleware)](/docs/14/pages/building-your-application/routing/middleware) بالإضافة إلى `getStaticProps`.
...
سيتم استخدام ملف JSON هذا في توجيه جانب العميل من خلال [`next/link`](/docs/14/pages/api-reference/components/link) أو [`next/router`](/docs/14/pages/api-reference/functions/use-router). عند الانتقال إلى صفحة تم عرضها مسبقًا باستخدام `getStaticProps`، يقوم Next.js بجلب ملف JSON هذا (المحسوب مسبقًا في وقت البناء) واستخدامه كخصائص لمكون الصفحة. هذا يعني أن انتقالات الصفحة على جانب العميل **لن** تستدعي `getStaticProps` حيث يتم استخدام ملف JSON المصدر فقط.
...
> **ملاحظة جيدة**: إذا كنت قد أنشأت [تطبيقًا مخصصًا](/docs/14/pages/building-your-application/routing/custom-app)، فتأكد من تمرير `pageProps` إلى مكون الصفحة كما هو موضح في المستند المرتبط، وإلا ستكون الخصائص فارغة.
...
يمكنك تجاوز التوليد الثابت مؤقتًا وعرض الصفحة في **وقت الطلب** بدلاً من وقت البناء باستخدام [**وضع المعاينة (Preview Mode)**](/docs/14/pages/building-your-application/configuring/preview-mode). على سبيل المثال، قد تستخدم نظام إدارة محتوى بدون واجهة أمامية وتريد معاينة المسودات قبل نشرها.