...
في الإصدار 13، قدمت Next.js **موجه تطبيق (App Router)** جديد مبني على [مكونات الخادم في React](/docs/14/app/building-your-application/rendering/server-components)، والذي يدعم التخطيطات المشتركة، التوجيه المتداخل، حالات التحميل، معالجة الأخطاء، والمزيد.

يعمل موجه التطبيق في مجلد جديد باسم `app`. يعمل مجلد `app` جنبًا إلى جنب مع مجلد `pages` للسماح بالتبني التدريجي. هذا يسمح لك باختيار بعض مسارات تطبيقك للسلوك الجديد مع الاحتفاظ بمسارات أخرى في مجلد `pages` للسلوك السابق. إذا كان تطبيقك يستخدم مجلد `pages`، يرجى أيضًا الاطلاع على وثائق [موجه الصفحات (Pages Router)](/docs/14/pages/building-your-application/routing).
...
بشكل افتراضي، المكونات داخل `app` هي [مكونات خادم React](/docs/14/app/building-your-application/rendering/server-components). هذا تحسين للأداء ويسمح لك بتبنيها بسهولة، ويمكنك أيضًا استخدام [مكونات العميل](/docs/14/app/building-your-application/rendering/client-components).

> **التوصية:** تحقق من صفحة [الخادم](/docs/14/app/building-your-application/rendering/server-components) إذا كنت جديدًا على مكونات الخادم.

## أدوار المجلدات والملفات

يستخدم Next.js موجهًا يعتمد على نظام الملفات حيث:

- **المجلدات** تُستخدم لتعريف المسارات. المسار هو مسار واحد من المجلدات المتداخلة، يتبع التسلسل الهرمي لنظام الملفات من **مجلد الجذر** وصولاً إلى **مجلد الورقة** النهائي الذي يتضمن ملف `page.js`. انظر [تحديد المسارات](/docs/14/app/building-your-application/routing/defining-routes).
...
|                                                                                       |                                                                                                |
| ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts)     | واجهة مستخدم مشتركة لقطاع وأطفاله                                                              |
| [`page`](/docs/14/app/building-your-application/routing/pages-and-layouts#pages)         | واجهة مستخدم فريدة لمسار وجعل المسارات قابلة للوصول العام                                       |
| [`loading`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)     | واجهة مستخدم التحميل لقطاع وأطفاله                                                             |
| [`not-found`](/docs/14/app/api-reference/file-conventions/not-found)                     | واجهة مستخدم غير موجود لقطاع وأطفاله                                                           |
| [`error`](/docs/14/app/building-your-application/routing/error-handling)                 | واجهة مستخدم الخطأ لقطاع وأطفاله                                                               |
| [`global-error`](/docs/14/app/building-your-application/routing/error-handling)          | واجهة مستخدم الخطأ العام                                                                       |
| [`route`](/docs/14/app/building-your-application/routing/route-handlers)                 | نقطة نهاية API من جانب الخادم                                                                  |
| [`template`](/docs/14/app/building-your-application/routing/pages-and-layouts#templates) | واجهة مستخدم تخطيط معاد عرضها بشكل متخصص                                                       |
| [`default`](/docs/14/app/api-reference/file-conventions/default)                         | واجهة مستخدم احتياطية لـ [المسارات المتوازية](/docs/14/app/building-your-application/routing/parallel-routes) |
...
تعلم المزيد حول [تنظيم المشروع والتواجد المشترك](/docs/14/app/building-your-application/routing/colocation).

## أنماط التوجيه المتقدمة

يوفر موجه التطبيق أيضًا مجموعة من الاصطلاحات لمساعدتك في تنفيذ أنماط توجيه أكثر تقدمًا. وتشمل:

- [المسارات المتوازية](/docs/14/app/building-your-application/routing/parallel-routes): تسمح لك بعرض صفحتين أو أكثر في نفس العرض يمكن التنقل بينهما بشكل مستقل. يمكنك استخدامها لعروض مقسمة لها تنقل فرعي خاص بها. مثل لوحات التحكم.
- [مسارات الاعتراض](/docs/14/app/building-your-application/routing/intercepting-routes): تسمح لك باعتراض مسار وعرضه في سياق مسار آخر. يمكنك استخدام هذه عندما يكون الحفاظ على سياق الصفحة الحالية مهمًا. مثل رؤية جميع المهام أثناء تحرير مهمة واحدة أو توسيع صورة في موجز.
