...
يسمح لك خطاف `useRouter` بتغيير المسارات برمجيًا داخل [مكونات العميل (Client Components)](/docs/14/app/building-your-application/rendering/client-components).

> **توصية:** استخدم [مكون `<Link>`](/docs/14/app/building-your-application/routing/linking-and-navigating#link-component) للتنقل إلا إذا كان لديك متطلب محدد لاستخدام `useRouter`.
...
- `router.prefetch(href: string)`: [الجلب المسبق (Prefetch)](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) للمسار المحدد لتحقيق انتقالات أسرع من جانب العميل.
...
- تمت إزالة سلسلة `pathname` واستبدالها بـ [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname)
- تمت إزالة كائن `query` واستبداله بـ [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params)
- تم استبدال `router.events`. [انظر أدناه.](#router-events)

[عرض دليل الانتقال الكامل](/docs/14/app/building-your-application/upgrading/app-router-migration).
...
> **معلومة مفيدة**: تم تغليف `<NavigationEvents>` في [حدود `Suspense`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#example) لأن [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) يتسبب في العرض من جانب العميل حتى حدود `Suspense` الأقرب أثناء [العرض الثابت (static rendering)](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). [معرفة المزيد](/docs/14/app/api-reference/functions/use-search-params#behavior).
