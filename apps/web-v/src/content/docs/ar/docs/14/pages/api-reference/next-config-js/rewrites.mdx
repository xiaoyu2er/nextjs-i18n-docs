...
translation-updated-at: 2025-06-06T17:19:01.344Z
...
تعمل إعادة التوجيه كوكيل URL وتخفي مسار الوجهة، مما يجعل المستخدم يبدو وكأنه لم يغير موقعه على الموقع. على العكس من ذلك، فإن [إعادة التوجيه (redirects)](/docs/14/app/api-reference/next-config-js/redirects) ستعيد توجيه إلى صفحة جديدة وتظهر تغييرات URL.

</AppOnly>

<PagesOnly>

تعمل إعادة التوجيه كوكيل URL وتخفي مسار الوجهة، مما يجعل المستخدم يبدو وكأنه لم يغير موقعه على الموقع. على العكس من ذلك، فإن [إعادة التوجيه (redirects)](/docs/14/pages/api-reference/next-config-js/redirects) ستعيد توجيه إلى صفحة جديدة وتظهر تغييرات URL.
...
1. يتم التحقق من [الرؤوس](/docs/14/app/api-reference/next-config-js/headers) وتطبيقها
2. يتم التحقق من [إعادة التوجيه](/docs/14/app/api-reference/next-config-js/redirects) وتطبيقها
3. يتم التحقق من إعادة التوجيه `beforeFiles` وتطبيقها
4. يتم التحقق من الملفات الثابتة من [الدليل العام](/docs/14/app/building-your-application/optimizing/static-assets)، وملفات `_next/static`، والصفحات غير الديناميكية وتقديمها
...
6. يتم التحقق من إعادة التوجيه `fallback` وتطبيقها، يتم تطبيق هذه قبل عرض صفحة 404 وبعد التحقق من جميع المسارات الديناميكية/الأصول الثابتة. إذا كنت تستخدم [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) في `getStaticPaths`، فلن يتم تشغيل إعادة التوجيه `fallback` المحددة في ملف `next.config.js`.

</AppOnly>

<PagesOnly>

1. يتم التحقق من [الرؤوس](/docs/14/pages/api-reference/next-config-js/headers) وتطبيقها
2. يتم التحقق من [إعادة التوجيه](/docs/14/pages/api-reference/next-config-js/redirects) وتطبيقها
3. يتم التحقق من إعادة التوجيه `beforeFiles` وتطبيقها
4. يتم التحقق من الملفات الثابتة من [الدليل العام](/docs/14/pages/building-your-application/optimizing/static-assets)، وملفات `_next/static`، والصفحات غير الديناميكية وتقديمها
5. يتم التحقق من إعادة التوجيه `afterFiles` وتطبيقها، إذا تمت مطابقة إحدى إعادة التوجيه هذه، نتحقق من المسارات الديناميكية/الملفات الثابتة بعد كل مطابقة
6. يتم التحقق من إعادة التوجيه `fallback` وتطبيقها، يتم تطبيق هذه قبل عرض صفحة 404 وبعد التحقق من جميع المسارات الديناميكية/الأصول الثابتة. إذا كنت تستخدم [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) في `getStaticPaths`، فلن يتم تشغيل إعادة التوجيه `fallback` المحددة في ملف `next.config.js`.
...
> **معلومة مفيدة**: سيتم تحليل معلمات الصفحات الثابتة من [التحسين الثابت التلقائي](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) أو [التجهيز المسبق](/docs/14/pages/building-your-application/data-fetching/get-static-props) من إعادة التوجيه على العميل بعد التميؤ وتوفيرها في الاستعلام.
...
عند استخدام [دعم `basePath`](/docs/14/app/api-reference/next-config-js/basePath) مع إعادة التوجيه، يتم إضافة `basePath` تلقائيًا إلى كل `source` و `destination` ما لم تضيف `basePath: false` إلى إعادة التوجيه:
...
عند استخدام [دعم `i18n`](/docs/14/app/building-your-application/routing/internationalization) مع إعادة التوجيه، يتم إضافة البادئات تلقائيًا إلى `source` و `destination` للتعامل مع `locales` المكونة ما لم تضيف `locale: false` إلى إعادة التوجيه. إذا تم استخدام `locale: false`، يجب إضافة بادئة اللغة إلى `source` و `destination` لمطابقتها بشكل صحيح.

</AppOnly>

<PagesOnly>

عند استخدام [دعم `i18n`](/docs/14/pages/building-your-application/routing/internationalization) مع إعادة التوجيه، يتم إضافة البادئات تلقائيًا إلى `source` و `destination` للتعامل مع `locales` المكونة ما لم تضيف `locale: false` إلى إعادة التوجيه. إذا تم استخدام `locale: false`، يجب إضافة بادئة اللغة إلى `source` و `destination` لمطابقتها بشكل صحيح.
