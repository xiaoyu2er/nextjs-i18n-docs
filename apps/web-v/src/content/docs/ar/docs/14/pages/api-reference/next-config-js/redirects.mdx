...
عند استخدام موجه الصفحات (Pages Router)، لا يتم تطبيق إعادة التوجيه على التوجيه من جانب العميل (`Link`، `router.push`) إلا إذا كان [البرنامج الوسيط (Middleware)](/docs/14/app/building-your-application/routing/middleware) موجودًا ويتطابق مع المسار.
...
عند استخدام [دعم `basePath`](/docs/14/app/api-reference/next-config-js/basePath) مع إعادة التوجيه، يتم تلقائيًا إضافة `basePath` إلى كل `source` و `destination` ما لم تقم بإضافة `basePath: false` إلى إعادة التوجيه:
...
عند استخدام [دعم `i18n`](/docs/14/app/building-your-application/routing/internationalization) مع إعادة التوجيه، يتم تلقائيًا إضافة البادئات إلى `source` و `destination` للتعامل مع `locales` المكونة ما لم تقم بإضافة `locale: false` إلى إعادة التوجيه. إذا تم استخدام `locale: false`، يجب إضافة بادئة اللغة إلى `source` و `destination` لمطابقتها بشكل صحيح.

</AppOnly>

<PagesOnly>

عند استخدام [دعم `i18n`](/docs/14/pages/building-your-application/routing/internationalization) مع إعادة التوجيه، يتم تلقائيًا إضافة البادئات إلى `source` و `destination` للتعامل مع `locales` المكونة ما لم تقم بإضافة `locale: false` إلى إعادة التوجيه. إذا تم استخدام `locale: false`، يجب إضافة بادئة اللغة إلى `source` و `destination` لمطابقتها بشكل صحيح.
...
- داخل [مسارات API](/docs/14/pages/building-your-application/routing/api-routes) و [معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers)، يمكنك إعادة التوجيه بناءً على الطلب الوارد.
- داخل [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) و [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)، يمكنك إعادة توجيه صفحات محددة في وقت الطلب.
