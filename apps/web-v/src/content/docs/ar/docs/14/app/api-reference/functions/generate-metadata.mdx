...
- `props` - كائن يحتوي على معلمات المسار الحالي:
  ...
  - `params` - كائن يحتوي على كائن [معلمات المسار الديناميكي](/docs/14/app/building-your-application/routing/dynamic-routes) من المقطع الجذري وصولاً إلى المقطع الذي يتم استدعاء `generateMetadata` منه. أمثلة:
...
> - طلبات `fetch` يتم [تخزينها مؤقتًا](/docs/14/app/building-your-application/caching#request-memoization) تلقائيًا لنفس البيانات عبر `generateMetadata`، و`generateStaticParams`، والتخطيطات، والصفحات، ومكونات الخادم. يمكن استخدام [`cache` في React](/docs/14/app/building-your-application/caching#request-memoization) إذا لم يكن `fetch` متاحًا.
> - `searchParams` متاحة فقط في مقاطع `page.js`.
> - يمكن أيضًا استخدام طرق Next.js [`redirect()`](/docs/14/app/api-reference/functions/redirect) و[`notFound()`](/docs/14/app/api-reference/functions/not-found) داخل `generateMetadata`.
...
> **مهمل**: خيار `themeColor` في `metadata` مهمل اعتبارًا من Next.js 14. يرجى استخدام [تكوين `viewport`](/docs/14/app/api-reference/functions/generate-viewport) بدلاً من ذلك.
...
> **مهمل**: خيار `viewport` في `metadata` مهمل اعتبارًا من Next.js 14. يرجى استخدام [تكوين `viewport`](/docs/14/app/api-reference/functions/generate-viewport) بدلاً من ذلك.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | استخدم رؤوس HTTP المناسبة عبر [`redirect()`](/docs/14/app/api-reference/functions/redirect)، [البرمجية الوسيطة (Middleware)](/docs/14/app/building-your-application/routing/middleware#nextresponse)، [رؤوس الأمان](/docs/14/app/api-reference/next-config-js/headers) |
| `<base>`                      | عرض الوسم في التنسيق أو الصفحة نفسها.                                                                                                                                                                                                       |
| `<noscript>`                  | عرض الوسم في التنسيق أو الصفحة نفسها.                                                                                                                                                                                                       |
| `<style>`                     | تعرف على المزيد حول [التنسيق في Next.js](/docs/14/app/building-your-application/styling/css-modules).                                                                                                                                                    |
| `<script>`                    | تعرف على المزيد حول [استخدام النصوص البرمجية (scripts)](/docs/14/app/building-your-application/optimizing/scripts).                                                                                                                                                          |
...
يمكنك إضافة أمان النوع إلى بياناتك الوصفية باستخدام نوع `Metadata`. إذا كنت تستخدم [الملحق المدمج TypeScript](/docs/14/app/building-your-application/configuring/typescript) في بيئة التطوير المتكاملة (IDE)، فلن تحتاج إلى إضافة النوع يدويًا، ولكن لا يزال بإمكانك إضافته صراحة إذا أردت.
...
| الإصدار   | التغييرات                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.2.0` | `viewport` و`themeColor` و`colorScheme` أصبحت قديمة لصالح [تكوين `viewport`](/docs/14/app/api-reference/functions/generate-viewport). |
