...
يمكن استخدام دالة `generateStaticParams` بالاشتراك مع [شرائح المسار الديناميكية](/docs/14/app/building-your-application/routing/dynamic-routes) ل[**إنشاء مسارات بشكل ثابت**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) أثناء وقت البناء بدلاً من الطلب عند وقت الطلب.
...
> **معلومة مفيدة**
>
> - يمكنك استخدام خيار تكوين شريحة [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) للتحكم فيما يحدث عند زيارة جزء ديناميكي لم يتم إنشاؤه باستخدام `generateStaticParams`.
...
> - `generateStaticParams` يحل محل دالة [`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) في موجه الصفحات.
...
> **معلومة مفيدة**: طلبات `fetch` يتم [تخزينها مؤقتًا](/docs/14/app/building-your-application/caching#request-memoization) تلقائيًا لنفس البيانات عبر جميع الدوال ذات البادئة `generate`، والتخطيطات، والصفحات، ومكونات الخادم. يمكن استخدام [`cache` في React](/docs/14/app/building-your-application/caching#request-memoization) إذا لم يكن `fetch` متاحًا.

### إنشاء مجموعة فرعية فقط من params

يمكنك إنشاء مجموعة فرعية من params لمسار عن طريق إرجاع مصفوفة من الكائنات تحتوي فقط على الشرائح الديناميكية التي تريد إنشاؤها. ثم باستخدام خيار تكوين شريحة [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams)، يمكنك التحكم فيما يحدث عند زيارة جزء ديناميكي لم يتم إنشاؤه باستخدام `generateStaticParams`.
