...
عند تصدير دالة تسمى `getStaticPaths` من صفحة تستخدم [المسارات الديناميكية (Dynamic Routes)](/docs/14/pages/building-your-application/routing/dynamic-routes)، سيقوم Next.js بعرض جميع المسارات المحددة بواسطة `getStaticPaths` مسبقًا بشكل ثابت.
...
يحدد مفتاح `paths` المسارات التي سيتم عرضها مسبقًا. على سبيل المثال، لنفترض أن لديك صفحة تستخدم [المسارات الديناميكية (Dynamic Routes)](/docs/14/pages/building-your-application/routing/dynamic-routes) باسم `pages/posts/[id].js`. إذا قمت بتصدير `getStaticPaths` من هذه الصفحة وأعدت ما يلي لـ `paths`:
...
- إذا كان اسم الصفحة يستخدم [مسارات التقاط الكل (catch-all routes)](/docs/14/pages/building-your-application/routing/dynamic-routes#catch-all-segments) مثل `pages/[...slug]`، فيجب أن يحتوي `params` على `slug` (وهو مصفوفة). إذا كانت هذه المصفوفة هي `['hello', 'world']`، فسيقوم Next.js بإنشاء الصفحة بشكل ثابت في `/hello/world`.
- إذا كانت الصفحة تستخدم [مسار التقاط اختياري (optional catch-all route)](/docs/14/pages/building-your-application/routing/dynamic-routes#optional-catch-all-segments)، استخدم `null` أو `[]` أو `undefined` أو `false` لعرض المسار الجذري. على سبيل المثال، إذا قمت بتوفير `slug: false` لـ `pages/[[...slug]]`، فسيقوم Next.js بإنشاء الصفحة `/` بشكل ثابت.
...
بشكل منفصل عن كائن `params`، يمكن إرجاع حقل `locale` عند [تكوين i18n](/docs/14/pages/building-your-application/routing/internationalization)، والذي يحدد اللغة للمسار الذي يتم إنشاؤه.
...
يقدم المثال التالي عرضًا مسبقًا لمقالة مدونة واحدة لكل صفحة تسمى `pages/posts/[id].js`. سيتم جلب قائمة مقالات المدونة من نظام إدارة المحتوى (CMS) وإعادتها بواسطة `getStaticPaths`. ثم، لكل صفحة، يتم جلب بيانات المقالة من نظام إدارة المحتوى باستخدام [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props).
...
> **معلومة جيدة**: لا يتم دعم `fallback: true` عند استخدام [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports).
...
لن تقوم `fallback: true` بـ _تحديث_ الصفحات التي تم إنشاؤها، لذلك يمكنك الاطلاع على [التجديد الثابت التدريجي (Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
لن تقوم `fallback: 'blocking'` بـ _تحديث_ الصفحات التي تم إنشاؤها افتراضيًا. لتحديث الصفحات التي تم إنشاؤها، استخدم [التجديد الثابت التدريجي (Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) بالتزامن مع `fallback: 'blocking'`.

> **معلومة جيدة**: لا يتم دعم `fallback: 'blocking'` عند استخدام [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports).
...
- باستخدام [الموجه (router)](/docs/14/pages/api-reference/functions/use-router)، يمكنك الكشف عما إذا كان يتم عرض fallback، حيث سيكون `router.isFallback` `true`.
...
| الإصدار   | التغييرات                                                                                                                                                                                                     |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | أصبح [موجه التطبيق (App Router)](/docs/14/app/building-your-application/data-fetching) مستقرًا مع جلب بيانات مبسط، بما في ذلك [`generateStaticParams()`](/docs/14/app/api-reference/functions/generate-static-params) |
| `v12.2.0` | أصبح [التجديد الثابت التدريجي عند الطلب (On-Demand Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) مستقرًا.                                          |
| `v12.1.0` | تمت إضافة [التجديد الثابت التدريجي عند الطلب (On-Demand Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) (بيتا).                                       |
| `v9.5.0`  | أصبح [التجديد الثابت التدريجي (Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) مستقرًا                                                                               |
