...
translation-updated-at: 2025-06-06T17:25:23.892Z
...
- **نظام تشغيل Edge** مبني على [واجهات برمجة الويب (Web APIs)](/docs/14/app/api-reference/edge).
...
| [بث HTTP](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)                                                       | نعم    | نعم        | نعم              |
...
| [التصيير الثابت](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)                            | نعم    | نعم        | لا               |
| [التصيير الديناميكي](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)                               | نعم    | نعم        | نعم              |
| [إعادة التحقق من البيانات مع `fetch`](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) | نعم    | نعم        | نعم              |
...
يمكنك تحديد نظام تشغيل لقطعات المسار الفردية في تطبيق Next.js الخاص بك. للقيام بذلك، [قم بتعريف متغير يسمى `runtime` وقم بتصديره](/docs/14/app/api-reference/file-conventions/route-segment-config). يجب أن يكون المتغير سلسلة نصية، ويجب أن تكون قيمته إما `'nodejs'` أو `'edge'`.
...
> يرجى الرجوع إلى [وثائق Node.js](https://nodejs.org/docs/latest/api/) و[وثائق Edge](/docs/14/app/api-reference/edge) للحصول على القائمة الكاملة لواجهات البرمجة المتاحة. يمكن لكلا نظامي التشغيل أيضًا دعم [البث (streaming)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) اعتمادًا على بنية النشر الخاصة بك.