...
يستخدم ملف **not-found** لعرض واجهة المستخدم عند استدعاء دالة [`notFound`](/docs/14/app/api-reference/functions/not-found) داخل مقطع مسار. بالإضافة إلى تقديم واجهة مستخدم مخصصة، سيقوم Next.js بإرجاع رمز حالة HTTP `200` للاستجابات المُجراة (streamed)، و`404` للاستجابات غير المُجراة.
