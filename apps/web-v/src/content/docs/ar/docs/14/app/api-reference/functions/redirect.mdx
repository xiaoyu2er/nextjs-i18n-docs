...
تسمح دالة `redirect` بإعادة توجيه المستخدم إلى عنوان URL آخر. يمكن استخدام `redirect` في [مكونات الخادم (Server Components)](/docs/14/app/building-your-application/rendering/server-components)، [معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers)، و [أفعال الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).

عند استخدامها في [سياق البث (streaming context)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#what-is-streaming)، تقوم بإدراج وسم meta لإصدار إعادة التوجيه على جانب العميل. عند استخدامها في فعل خادم، تقدم استجابة إعادة توجيه HTTP 303 للمتصل. وإلا، تقدم استجابة إعادة توجيه HTTP 307 للمتصل.

إذا كان المورد غير موجود، يمكنك استخدام [دالة `notFound`](/docs/14/app/api-reference/functions/not-found) بدلاً من ذلك.

> **معلومة مفيدة**:
>
> - في أفعال الخادم ومعالجات المسار، يجب استدعاء `redirect` بعد كتلة `try/catch`.
> - إذا كنت تفضل إرجاع إعادة توجيه HTTP 308 (دائم) بدلاً من 307 (مؤقت)، يمكنك استخدام [دالة `permanentRedirect`](/docs/14/app/api-reference/functions/permanentRedirect) بدلاً من ذلك.
...
بشكل افتراضي، تستخدم `redirect` `push` (إضافة إدخال جديد إلى سجل المتصفح) في [أفعال الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) و `replace` (استبدال عنوان URL الحالي في سجل المتصفح) في كل مكان آخر. يمكنك تجاوز هذا السلوك عن طريق تحديد معامل `type`.
...
يمكن استخدام `redirect` في مكون العميل من خلال فعل خادم. إذا كنت بحاجة إلى استخدام معالج حدث لإعادة توجيه المستخدم، يمكنك استخدام خطاف [`useRouter`](/docs/14/app/api-reference/functions/use-router).
