...
في Next.js، يستخدم [معالج التخزين المؤقت الافتراضي](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) لموجه الصفحات (Pages Router) وموجه التطبيق (App Router) تخزين نظام الملفات. لا يتطلب هذا أي تكوين، ولكن يمكنك تخصيص معالج التخزين المؤقت باستخدام حقل `cacheHandler` في ملف `next.config.js`.
...
عرض مثال لـ [معالج التخزين المؤقت المخصص](/docs/14/app/building-your-application/deploying#configuring-caching) ومعرفة المزيد حول التنفيذ.
...
تُرجع `Promise<void>`. تعرف على المزيد حول [إعادة التحقق من البيانات](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) أو دالة [`revalidateTag()`](/docs/14/app/api-reference/functions/revalidateTag).
