...
يتم تمرير المقاطع الديناميكية كخاصية `params` إلى دوال [`layout`](/docs/14/app/api-reference/file-conventions/layout)، و[`page`](/docs/14/app/api-reference/file-conventions/page)، و[`route`](/docs/14/app/building-your-application/routing/route-handlers)، و[`generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata#generatemetadata-function).
...
> **ملاحظة جيدة**: المقاطع الديناميكية تعادل [المسارات الديناميكية](/docs/14/app/building-your-application/routing/dynamic-routes) في دليل `pages`.

## إنشاء معلمات ثابتة

يمكن استخدام دالة `generateStaticParams` بالاشتراك مع [مقاطع المسار الديناميكية](/docs/14/app/building-your-application/routing/dynamic-routes) ل[إنشاء مسارات **ثابتة**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) وقت البناء بدلاً من الطلب عند الطلب.
...
الفائدة الأساسية من دالة `generateStaticParams` هي استرجاعها الذكي للبيانات. إذا تم جلب المحتوى داخل دالة `generateStaticParams` باستخدام طلب `fetch`، فإن الطلبات يتم [تخزينها مؤقتًا تلقائيًا](/docs/14/app/building-your-application/caching#request-memoization). هذا يعني أن طلب `fetch` بنفس الوسائط عبر عدة `generateStaticParams`، وتخطيطات، وصفحات سيتم تنفيذه مرة واحدة فقط، مما يقلل أوقات البناء.

استخدم [دليل الترحيل](/docs/14/app/building-your-application/upgrading/app-router-migration#dynamic-paths-getstaticpaths) إذا كنت تقوم بالترحيل من دليل `pages`.

راجع [توثيق دالة الخادم `generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) لمزيد من المعلومات وحالات الاستخدام المتقدمة.
...
> **ملاحظة جيدة**: قد يتم ذلك تلقائيًا بواسطة [ملحق TypeScript](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin) في المستقبل.