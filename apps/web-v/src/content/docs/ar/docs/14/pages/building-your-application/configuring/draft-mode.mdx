...
ثم أرسل طلبًا إلى `/api/disable-draft` لاستدعاء مسار API. إذا كنت تستدعي هذا المسار باستخدام [`next/link`](/docs/14/pages/api-reference/components/link)، يجب عليك تمرير `prefetch={false}` لمنع حذف الكوكي عن طريق الخطأ أثناء الجلب المسبق.

### يعمل مع `getServerSideProps`

يعمل وضع المسودة مع `getServerSideProps`، وهو متاح كمفتاح `draftMode` في كائن [`context`](/docs/14/pages/api-reference/functions/get-server-side-props#context-parameter).

> **ملاحظة جيدة**: لا يجب عليك تعيين رأس `Cache-Control` عند استخدام وضع المسودة لأنه لا يمكن تجاوزه. بدلاً من ذلك، نوصي باستخدام [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
