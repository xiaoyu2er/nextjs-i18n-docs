...
- التحذير عند تمرير قيم غير صالحة لـ [خيارات تكوين المقاطع](/docs/14/app/api-reference/file-conventions/route-segment-config).
...
2. **تدفق بيانات مبسط بين المكونات**: مع إزالة `_app` لصالح التخطيطات الجذرية، أصبح من الأسهل تصور تدفق البيانات بين المكونات والصفحات. سابقًا، كان تدفق البيانات بين `pages` الفردية و `_app` صعب التحديد ويمكن أن يتسبب في أخطاء مربكة. مع [جلب البيانات في نفس المكان](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) في موجه التطبيق، لم يعد هذا مشكلة.

[جلب البيانات في Next.js](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) يوفر الآن سلامة أنواع قريبة من البداية إلى النهاية قدر الإمكان دون أن يكون تقييديًا بشأن اختيار قاعدة البيانات أو مزود المحتوى.
...
لـ [`getStaticProps`](/docs/14/pages/api-reference/functions/get-static-props) و [`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) و [`getServerSideProps`](/docs/14/pages/api-reference/functions/get-server-side-props)، يمكنك استخدام الأنواع `GetStaticProps` و `GetStaticPaths` و `GetServerSideProps` على التوالي:
...
إذا كان لديك [تطبيق `App` مخصص](/docs/14/pages/building-your-application/routing/custom-app)، يمكنك استخدام النوع المدمج `AppProps` وتغيير اسم الملف إلى `./pages/_app.tsx` كما يلي:
...
يمكنك معرفة المزيد عن هذه الميزة في [وثائق أسماء مسارات الوحدات](/docs/14/app/building-your-application/configuring/absolute-imports-and-module-aliases).

</AppOnly>

<PagesOnly>

يمكنك معرفة المزيد عن هذه الميزة في [وثائق أسماء مسارات الوحدات](/docs/14/pages/building-your-application/configuring/absolute-imports-and-module-aliases).
