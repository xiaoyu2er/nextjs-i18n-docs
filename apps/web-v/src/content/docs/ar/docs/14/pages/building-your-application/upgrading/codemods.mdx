...
هذا الكودمود ينقل استيرادات من `next/server` إلى `next/og` لاستخدام [توليد صور OG ديناميكية](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation).
...
قم بإزالة علامات `<a>` داخل [مكونات Link](/docs/14/app/api-reference/components/link)، أو أضف خاصية `legacyBehavior` إلى الروابط التي لا يمكن إصلاحها تلقائيًا.

</AppOnly>

<PagesOnly>

قم بإزالة علامات `<a>` داخل [مكونات Link](/docs/14/pages/api-reference/components/link)، أو أضف خاصية `legacyBehavior` إلى الروابط التي لا يمكن إصلاحها تلقائيًا.
