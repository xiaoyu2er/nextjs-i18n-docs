...
مع الدعم المدمج [للبث عبر React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense)، يمكنك أن تكون أكثر تحديدًا بشأن الأجزاء التي تريد تحميلها أولاً وبأي ترتيب دون إدخال شلالات شبكة.
...
يجب أن يتضمن تطبيق [جهاز توجيه التطبيق](/docs/14/app) في Next.js ملف [تخطيط جذر](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required)، وهو [مكون خادم React](/docs/14/app/building-your-application/rendering/server-components) سيلف كل الصفحات في تطبيقك. يتم تعريف هذا الملف في المستوى الأعلى من دليل `app`.
...
5. يتم إضافة أي [ملفات بيانات وصفية](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) مثل `favicon.ico` أو `icon.png` أو `robots.txt` تلقائيًا إلى علامة `<head>` للتطبيق طالما قمت بوضعها في المستوى الأعلى من دليل `app`. بعد نقل [جميع الملفات المدعومة](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) إلى دليل `app`، يمكنك حذف علامات `<link>` الخاصة بها بأمان:
...
6. أخيرًا، يمكن لـ Next.js إدارة علامات `<head>` الأخيرة الخاصة بك باستخدام [واجهة برمجة تطبيقات البيانات الوصفية](/docs/14/app/building-your-application/optimizing/metadata). انقل معلومات البيانات الوصفية النهائية الخاصة بك إلى كائن [`metadata` مُصدَّر](/docs/14/app/api-reference/functions/generate-metadata#metadata-object):
...
مع التغييرات المذكورة أعلاه، انتقلت من الإعلان عن كل شيء في `index.html` الخاص بك إلى استخدام النهج القائم على الاتفاقيات في Next.js المدمج في الإطار ([واجهة برمجة تطبيقات البيانات الوصفية](/docs/14/app/building-your-application/optimizing/metadata)). يتيح لك هذا النهج تحسين تحسين محركات البحث وقابلية مشاركة صفحاتك على الويب بسهولة أكبر.
...
[بقطاع مسار اختياري شامل](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments).
يستخدم Next.js جهاز توجيه يعتمد على نظام الملفات حيث 
[تستخدم الأدلة لتحديد المسارات](/docs/14/app/building-your-application/routing/defining-routes#creating-routes).
...
هذا الملف هو [مكون خادم (Server Component)](/docs/14/app/building-your-application/rendering/server-components). عند تشغيل `next build`، يتم تقديم الملف مسبقًا إلى أصل ثابت. لا يتطلب أي كود ديناميكي.

يستورد هذا الملف CSS العام الخاص بنا ويخبر [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) أننا سننشئ مسارًا واحدًا فقط، وهو مسار الفهرس `/`.
...
هذا الملف هو [مكون عميل (Client Component)](/docs/14/app/building-your-application/rendering/client-components)، يتم تعريفه باستخدام التوجيه `'use client'`. لا يزال يتم تقديم مكونات العميل مسبقًا إلى HTML على الخادم قبل إرسالها إلى العميل.
...
مع Next.js، يُرجع استيراد الصور الثابتة كائنًا. يمكن بعد ذلك استخدام هذا الكائن مباشرة مع مكون Next.js [`<Image>`](/docs/14/app/api-reference/components/image)، أو يمكنك استخدام خاصية `src` للكائن مع وسم `<img>` الحالي.

يتمتع مكون `<Image>` بمزايا إضافية مثل 
[تحسين الصور تلقائيًا](/docs/14/app/building-your-application/optimizing/images). يضبط مكون `<Image>` تلقائيًا سمات `width` و `height` لوسم `<img>` الناتج بناءً على أبعاد الصورة. هذا يمنع حدوث تغييرات في التخطيط عند تحميل الصورة. ومع ذلك، يمكن أن يسبب هذا مشاكل إذا كان تطبيقك يحتوي على صور بأبعاد غير متساوية دون ضبط الآخر على `auto`. عندما لا يتم ضبطه على `auto`، سيعتمد البعد على قيمة سمة البعد لوسم `<img>`، مما قد يجعل الصورة تظهر مشوهة.

سيؤدي الاحتفاظ بوسم `<img>` إلى تقليل كمية التغييرات في تطبيقك ومنع المشاكل المذكورة أعلاه. يمكنك بعد ذلك الانتقال لاحقًا إلى مكون `<Image>` للاستفادة من تحسين الصور عن طريق [تكوين محمل (loader)](/docs/14/app/building-your-application/optimizing/images#loaders)، أو الانتقال إلى خادم Next.js الافتراضي الذي يحتوي على تحسين تلقائي للصور.
...
[المشابهة لـ Vite](/docs/14/app/building-your-application/configuring/environment-variables).
...
2. **قم بتعيين [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) إلى `process.env.NEXT_PUBLIC_BASE_PATH` في ملف `next.config.mjs`:**
...
- قم بالانتقال من React Router إلى [موجه تطبيق Next.js](/docs/14/app/building-your-application/routing) للحصول على:
  - تقسيم الشفرة تلقائيًا
  - [عرض الخادم المتدفق (Streaming Server-Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - [مكونات خادم React (React Server Components)](/docs/14/app/building-your-application/rendering/server-components)
- [قم بتحسين الصور باستخدام مكون `<Image>`](/docs/14/app/building-your-application/optimizing/images)
- [قم بتحسين الخطوط باستخدام `next/font`](/docs/14/app/building-your-application/optimizing/fonts)
- [قم بتحسين البرامج النصية لجهات خارجية باستخدام مكون `<Script>`](/docs/14/app/building-your-application/optimizing/scripts)
- [قم بتحديث تكوين ESLint لدعم قواعد Next.js](/docs/14/app/building-your-application/configuring/eslint)
