...
translation-updated-at: 2025-06-06T17:19:33.586Z
title: exportPathMap
...
`exportPathMap` تسمح لك بتحديد تعيين مسارات الطلبات إلى وجهات الصفحات، لاستخدامها أثناء التصدير. المسارات المعرفة في `exportPathMap` ستكون متاحة أيضاً عند استخدام [`next dev`](/docs/14/app/api-reference/cli/next#next-dev-options).
...
> **من الجيد معرفة**: حقل `query` في `exportPathMap` لا يمكن استخدامه مع [الصفحات المحسنة ثابتاً تلقائياً](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) أو صفحات [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) لأنها تُصدر إلى ملفات HTML أثناء وقت البناء ولا يمكن توفير معلومات استعلام إضافية أثناء `next export`.
...
- `distDir` - المسار المطلق لمجلد `.next/` (قابل للتخصيص باستخدام تكوين [`distDir`](/docs/14/pages/api-reference/next-config-js/distDir))
...
[`next export`](/docs/14/app/building-your-application/deploying/static-exports) سيستخدم `out` كمجلد إخراج افتراضي، يمكنك تخصيص هذا باستخدام وسيط `-o`، كما يلي:

</AppOnly>

<PagesOnly>

[`next export`](/docs/14/pages/building-your-application/deploying/static-exports) سيستخدم `out` كمجلد إخراج افتراضي، يمكنك تخصيص هذا باستخدام وسيط `-o`، كما يلي:
