...
مع الدعم المدمج لـ [البث عبر React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense)، يمكنك أن تكون أكثر تحديدًا لأي أجزاء من واجهة المستخدم تريد تحميلها أولاً وبأي ترتيب دون إدخال شلالات الشبكة.
...
[Next.js Middleware](/docs/14/app/building-your-application/routing/middleware) يسمح لك بتشغيل الكود على الخادم قبل اكتمال الطلب. هذا مفيد بشكل خاص لتجنب ظهور محتوى غير مصادق عليه عندما يزور المستخدم صفحة تتطلب مصادقة عن طريق توجيه المستخدم إلى صفحة تسجيل الدخول. كما أن Middleware مفيد للتجارب و[التدويل](/docs/14/app/building-your-application/routing/internationalization).

### تحسينات مدمجة

غالبًا ما يكون لـ [الصور](/docs/14/app/building-your-application/optimizing/images)، [الخطوط](/docs/14/app/building-your-application/optimizing/fonts)، و[نصوص الطرف الثالث](/docs/14/app/building-your-application/optimizing/scripts) تأثير كبير على أداء التطبيق. يأتي Next.js مع مكونات مدمجة تقوم بتحسينها تلقائيًا لك.
...
قم بإنشاء ملف `next.config.mjs` في جذر مشروعك. سيحتوي هذا الملف على [خيارات تكوين Next.js](/docs/14/app/api-reference/next-config-js).
...
يمكنك العثور على مزيد من المعلومات حول تكوين TypeScript في [وثائق Next.js](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin).

### الخطوة 4: إنشاء التخطيط الجذري

يجب أن يتضمن تطبيق [موجه التطبيق (App Router)](/docs/14/app) في Next.js ملف [تخطيط جذري](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required)، وهو [مكون خادم React](/docs/14/app/building-your-application/rendering/server-components) سيضم جميع الصفحات في تطبيقك. يتم تعريف هذا الملف في المستوى الأعلى من دليل `app`.
...
> **جيد أن تعرف**: سنتجاهل [ملف manifest](/docs/14/app/api-reference/file-conventions/metadata)، والأيقونات الإضافية بخلاف favicon، و[تكوين الاختبار](/docs/14/app/building-your-application/testing)، ولكن إذا كانت هذه متطلبات، فإن Next.js يدعم هذه الخيارات أيضًا.
...
يتم إضافة أي [ملفات بيانات وصفية](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) مثل `favicon.ico`، `icon.png`، `robots.txt` تلقائيًا إلى علامة `<head>` للتطبيق طالما أنك وضعتها في المستوى الأعلى من دليل `app`. بعد نقل [جميع الملفات المدعومة](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) إلى دليل `app`، يمكنك حذف علامات `<link>` الخاصة بها بأمان:
...
أخيرًا، يمكن لـ Next.js إدارة علامات `<head>` الأخيرة الخاصة بك باستخدام [واجهة برمجة تطبيقات البيانات الوصفية (Metadata API)](/docs/14/app/building-your-application/optimizing/metadata). انقل معلومات البيانات الوصفية النهائية الخاصة بك إلى كائن [`metadata` مُصدر](/docs/14/app/api-reference/functions/generate-metadata#metadata-object):
...
مع التغييرات أعلاه، انتقلت من الإعلان عن كل شيء في ملف `index.html` إلى استخدام النهج القائم على الاتفاقية المدمج في Next.js ([واجهة برمجة تطبيقات البيانات الوصفية](/docs/14/app/building-your-application/optimizing/metadata)). يمكّنك هذا النهج من تحسين تحسين محركات البحث (SEO) وإمكانية مشاركة صفحاتك على الويب بسهولة أكبر.

### الخطوة 6: الأنماط

مثل Create React App، يحتوي Next.js على دعم مدمج لـ [وحدات CSS](/docs/14/app/building-your-application/styling/css-modules).
...
هذا الدليل هو ما يسمى [بقطاع المسار الاختياري الشامل](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments). يستخدم Next.js جهاز توجيه قائم على نظام الملفات حيث [تستخدم الأدلة لتحديد المسارات](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). سيضمن هذا الدليل الخاص أن جميع مسارات تطبيقك سيتم توجيهها إلى ملف `page.tsx` الموجود فيه.
...
هذا الملف هو [مكون خادم](/docs/14/app/building-your-application/rendering/server-components). عند تشغيل `next build`، يتم تقديم الملف مسبقًا إلى أصل ثابت. لا يتطلب أي كود ديناميكي.

يستورد هذا الملف CSS العام الخاص بنا ويخبر [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) أننا سنقوم فقط بإنشاء مسار واحد، وهو مسار الفهرس عند `/`.
...
هذا الملف هو [مكون عميل](/docs/14/app/building-your-application/rendering/client-components)، معرف بتوجيه `'use client'`. لا يزال يتم تقديم مكونات العميل مسبقًا إلى HTML على الخادم قبل إرسالها إلى العميل.
...
مع Next.js، استيراد الصور الثابتة يعيد كائنًا. يمكن بعد ذلك استخدام هذا الكائن مباشرة مع مكون [`<Image>` في Next.js](/docs/14/app/api-reference/components/image)، أو يمكنك استخدام خاصية `src` من الكائن مع وسم `<img>` الحالي لديك.

يتمتع مكون `<Image>` بفوائد إضافية مثل [تحسين الصور تلقائيًا](/docs/14/app/building-your-application/optimizing/images). يضبط مكون `<Image>` تلقائيًا سمات `width` و `height` لوسم `<img>` الناتج بناءً على أبعاد الصورة. هذا يمنع حدوث تغييرات في التنسيق عند تحميل الصورة. ومع ذلك، قد يسبب هذا مشاكل إذا كان تطبيقك يحتوي على صور بأحد أبعادها فقط مع تنسيق دون الآخر مع تنسيق إلى `auto`. عندما لا يتم تنسيق البعد إلى `auto`، سيعتمد البعد على قيمة سمة البعد في وسم `<img>`، مما قد يجعل الصورة تظهر مشوهة.

سيؤدي الاحتفاظ بوسم `<img>` إلى تقليل عدد التغييرات في تطبيقك ومنع المشاكل المذكورة أعلاه. يمكنك بعد ذلك الانتقال لاحقًا إلى مكون `<Image>` للاستفادة من تحسين الصور عن طريق [تكوين محمل](/docs/14/app/building-your-application/optimizing/images#loaders)، أو الانتقال إلى خادم Next.js الافتراضي الذي يحتوي على تحسين تلقائي للصور.
...
يدعم Next.js متغيرات البيئة [`.env`](/docs/14/app/building-your-application/configuring/environment-variables) بشكل مشابه لـ CRA.
...
عند ترحيل تطبيق CRA إلى Next.js، قد يكون لديك تكوين مخصص لـ webpack تريد ترحيله. يدعم Next.js توفير [تكوين مخصص لـ webpack](/docs/14/app/api-reference/next-config-js/webpack).

علاوة على ذلك، يدعم Next.js [Turbopack](/docs/14/app/api-reference/next-config-js/turbo) عبر `next dev --turbo` لتحسين أداء التطوير المحلي. يدعم Turbopack بعض [محملات webpack](/docs/14/app/api-reference/next-config-js/turbo) أيضًا من أجل التوافق والتبني التدريجي.
...
- الانتقال من React Router إلى [موجه تطبيق Next.js](/docs/14/app/building-your-application/routing) للحصول على:
  - تقسيم الشفرة تلقائيًا
  - [عرض من جانب الخادم مع دفق](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - [مكونات الخادم في React](/docs/14/app/building-your-application/rendering/server-components)
- [تحسين الصور مع مكون `<Image>`](/docs/14/app/building-your-application/optimizing/images)
- [تحسين الخطوط مع `next/font`](/docs/14/app/building-your-application/optimizing/fonts)
- [تحسين نصوص الطرف الثالث مع مكون `<Script>`](/docs/14/app/building-your-application/optimizing/scripts)
- [تحديث تكوين ESLint لدعم قواعد Next.js](/docs/14/app/building-your-application/configuring/eslint)
