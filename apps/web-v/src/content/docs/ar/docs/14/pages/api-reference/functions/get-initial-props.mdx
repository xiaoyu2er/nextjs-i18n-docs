...
> **معلومة مفيدة**: `getInitialProps` هو واجهة برمجة تطبيقات قديمة. نوصي باستخدام [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) أو [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) بدلاً منها.
...
> - بالنسبة لتحميل الصفحة الأولي، ستعمل `getInitialProps` على الخادم فقط. ثم ستعمل `getInitialProps` أيضًا على العميل عند الانتقال إلى مسار مختلف باستخدام مكون [`next/link`](/docs/14/pages/api-reference/components/link) أو باستخدام [`next/router`](/docs/14/pages/api-reference/functions/use-router).
...
- يمكن استخدام `getInitialProps` فقط في ملفات المستوى الأعلى في `pages/`، وليس في المكونات المتداخلة. لجلب بيانات متداخلة على مستوى المكون، يمكنك استكشاف [موجه التطبيق (App Router)](/docs/14/app/building-your-application/data-fetching).
