...
يكون التصيير الثابت مفيدًا عندما تستخدم صفحاتك بيانات من نظام إدارة المحتوى (CMS). ومع ذلك، ليس مثاليًا عندما تكتب مسودة على نظام إدارة المحتوى الخاص بك وتريد معاينة المسودة فورًا على صفحتك. سترغب في أن يقوم Next.js بتصيير هذه الصفحات في **وقت الطلب** بدلاً من وقت البناء وجلب محتوى المسودة بدلاً من المحتوى المنشور. سترغب في أن يتحول Next.js إلى [التصيير الديناميكي](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering) فقط لهذه الحالة الخاصة.
...
أولاً، قم بإنشاء [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers). يمكن أن يكون له أي اسم - مثلاً `app/api/draft/route.ts`
...
ثم، أرسل طلبًا إلى `/api/disable-draft` لاستدعاء معالج المسار. إذا كنت تستدعي هذا المسار باستخدام [`next/link`](/docs/14/app/api-reference/components/link)، فيجب عليك تمرير `prefetch={false}` لمنع حذف الكوكي عن طريق الخطأ أثناء الجلب المسبق.
