...
> - إذا كان مشروعك يستخدم [تحسين الصور](/docs/14/app/building-your-application/optimizing/images) مع محمل `loader` الافتراضي، يجب تثبيت `sharp` كتبعية:
...
> **معلومة مفيدة**:
>
> - يتم قراءة `next.config.js` أثناء `next build` وتسلسله في ملف الإخراج `server.js`. إذا كانت خيارات [`serverRuntimeConfig` أو `publicRuntimeConfig` القديمة](/docs/14/pages/api-reference/next-config-js/runtime-configuration) مستخدمة، فإن القيم ستكون محددة بالقيم في وقت البناء.
> - إذا كان مشروعك بحاجة إلى الاستماع إلى منفذ أو اسم مضيف محدد، يمكنك تعريف متغيرات البيئة `PORT` أو `HOSTNAME` قبل تشغيل `server.js`. على سبيل المثال، قم بتشغيل `PORT=8080 HOSTNAME=0.0.0.0 node server.js` لبدء الخادم على `http://0.0.0.0:8080`.
> - إذا كان مشروعك يستخدم [تحسين الصور](/docs/14/pages/building-your-application/optimizing/images) مع محمل `loader` الافتراضي، يجب تثبيت `sharp` كتبعية:
