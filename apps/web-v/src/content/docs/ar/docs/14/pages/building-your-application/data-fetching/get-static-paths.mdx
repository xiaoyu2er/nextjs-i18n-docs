...
translation-updated-at: 2025-06-06T17:25:39.683Z
...
إذا كانت الصفحة تحتوي على [مسارات ديناميكية](/docs/14/pages/building-your-application/routing/dynamic-routes) وتستخدم `getStaticProps`، فإنها تحتاج إلى تحديد قائمة بالمسارات التي سيتم إنشاؤها بشكل ثابت.

عندما تصدر دالة تسمى `getStaticPaths` (إنشاء موقع ثابت) من صفحة تستخدم مسارات ديناميكية، فإن Next.js سيقوم بعرض مسبقًا جميع المسارات المحددة بواسطة `getStaticPaths` بشكل ثابت.
...
[مرجع واجهة برمجة التطبيقات `getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) يغطي جميع المعلمات والخصائص التي يمكن استخدامها مع `getStaticPaths`.
...
- يجب عرض الصفحة مسبقًا (لتحسين محركات البحث SEO) وتكون سريعة جدًا — حيث يقوم `getStaticProps` بإنشاء ملفات `HTML` و `JSON`، وكلاهما يمكن تخزينه مؤقتًا بواسطة CDN لتحسين الأداء

## متى يتم تشغيل getStaticPaths

سيتم تشغيل `getStaticPaths` فقط أثناء البناء في بيئة الإنتاج، ولن يتم استدعاؤه أثناء وقت التشغيل. يمكنك التحقق من إزالة الكود المكتوب داخل `getStaticPaths` من حزمة جانب العميل [باستخدام هذه الأداة](https://next-code-elimination.vercel.app/).
...
- **لا يمكن** استخدام `getStaticPaths` مع [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)
- يمكنك تصدير `getStaticPaths` من [مسار ديناميكي](/docs/14/pages/building-your-application/routing/dynamic-routes) يستخدم أيضًا `getStaticProps`
...
يسمح `getStaticPaths` لك بالتحكم في الصفحات التي يتم إنشاؤها أثناء البناء بدلاً من إنشائها عند الطلب باستخدام [`fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking). إنشاء المزيد من الصفحات أثناء البناء سيؤدي إلى عمليات بناء أبطأ.
