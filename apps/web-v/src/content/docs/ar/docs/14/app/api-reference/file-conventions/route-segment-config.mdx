...
تتيح لك خيارات مقطع المسار تكوين سلوك [الصفحة](/docs/14/app/building-your-application/routing/pages-and-layouts)، أو [التخطيط](/docs/14/app/building-your-application/routing/pages-and-layouts)، أو [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) من خلال تصدير المتغيرات التالية مباشرة:
...
- **`'force-dynamic'`**: إجبار [التصيير الديناميكي (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)، مما سيؤدي إلى تصيير المسارات لكل مستخدم عند وقت الطلب. هذا الخيار يعادل `getServerSideProps()` في دليل `pages`.

- **`'error'`**: إجبار التصيير الثابت وتخزين بيانات التخطيط أو الصفحة عن طريق التسبب في خطأ إذا استخدمت أي مكونات [وظائف ديناميكية (dynamic functions)](/docs/14/app/building-your-application/rendering/server-components#dynamic-functions) أو بيانات غير مخزنة مؤقتاً. هذا الخيار يعادل:
...
- **`'force-static'`**: إجبار التصيير الثابت وتخزين بيانات التخطيط أو الصفحة عن طريق إجبار [`cookies()`](/docs/14/app/api-reference/functions/cookies)، و [`headers()`](/docs/14/app/api-reference/functions/headers) و [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) على إرجاع قيم فارغة.

> **معلومة مفيدة**:
>
> - يمكن العثور على إرشادات حول [كيفية الترقية](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods) من `getServerSideProps` و `getStaticProps` إلى `dynamic: 'force-dynamic'` و `dynamic: 'error'` في [دليل الترقية](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods).

### `dynamicParams`

التحكم فيما يحدث عند زيارة مقطع ديناميكي لم يتم إنشاؤه باستخدام [generateStaticParams](/docs/14/app/api-reference/functions/generate-static-params).
...
> - عندما يكون `dynamicParams = true`، يستخدم المقطع [تصيير الخادم المتدفق (Streaming Server Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
- **`false`** (الافتراضي): الاستدلال الافتراضي لتخزين أي طلبات `fetch` التي تعين خيار `cache` الخاص بها إلى `'force-cache'` أو التي يتم اكتشافها قبل استخدام [وظيفة ديناميكية (dynamic function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions). يعادل دلالياً `revalidate: Infinity` مما يعني بشكل فعال أنه يجب تخزين المورد إلى أجل غير مسمى. لا يزال بإمكان طلبات `fetch` الفردية استخدام `cache: 'no-store'` أو `revalidate: 0` لتجنب التخزين المؤقت وجعل المسار يتم تصييره ديناميكياً. أو تعيين `revalidate` إلى رقم موجب أقل من الافتراضي للمسار لزيادة تواتر إعادة التحقق للمسار.
- **`0`**: التأكد من أن التخطيط أو الصفحة يتم [تصييره ديناميكياً (dynamically rendered)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering) دائماً حتى إذا لم يتم اكتشاف أي وظائف ديناميكية أو جلب بيانات غير مخزنة مؤقتاً. يغير هذا الخيار الافتراضي لطلبات `fetch` التي لا تعين خيار `cache` إلى `'no-store'` ولكن يترك طلبات `fetch` التي تختار `'force-cache'` أو تستخدم `revalidate` موجباً كما هي.
- **`number`**: (بالثواني) تعيين تواتر إعادة التحقق الافتراضي للتخطيط أو الصفحة إلى `n` ثانية.

> **معلومة مفيدة**: خيار `revalidate` متاح فقط عند استخدام [وقت تشغيل Node.js (Node.js Runtime)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime). هذا يعني أن استخدام خيار `revalidate` مع `runtime = 'edge'` لن يعمل.
...
بشكل افتراضي، سيخزن Next.js **أي** طلبات `fetch()` يمكن الوصول إليها **قبل** استخدام أي [وظائف ديناميكية (dynamic functions)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) **ولن يخزن** طلبات `fetch` التي يتم اكتشافها **بعد** استخدام الوظائف الديناميكية.
...
تعلم المزيد عن [وقت تشغيل Edge و Node.js (Edge and Node.js runtimes)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> **معلومة مفيدة**:
>
> - إذا كنت تستخدم [إجراءات الخادم (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)، فقم بتعيين `maxDuration` على مستوى الصفحة لتغيير المهلة الافتراضية لجميع إجراءات الخادم المستخدمة في الصفحة.

### `generateStaticParams`

يمكن استخدام الدالة `generateStaticParams` بالاشتراك مع [مقاطع المسار الديناميكية (dynamic route segments)](/docs/14/app/building-your-application/routing/dynamic-routes) لتحديد قائمة معلمات مقطع المسار التي سيتم إنشاؤها ثابتاً في وقت البناء بدلاً من عند الطلب في وقت الطلب.

راجع [مرجع API](/docs/14/app/api-reference/functions/generate-static-params) لمزيد من التفاصيل.