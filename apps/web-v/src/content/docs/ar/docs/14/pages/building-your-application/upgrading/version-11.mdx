...
translation-updated-at: 2025-06-06T17:22:27.924Z
title: الإصدار 11
...
أصبح Webpack 5 الآن الخيار الافتراضي لجميع تطبيقات Next.js. إذا لم يكن لديك تكوين مخصص لـ webpack، فإن تطبيقك يستخدم بالفعل webpack 5. إذا كان لديك تكوين مخصص، يمكنك الرجوع إلى [توثيق Next.js لـ webpack 5](/docs/messages/webpack5) للحصول على إرشادات الترقية.

### تنظيف `distDir` أصبح افتراضيًا

دليل إخراج البناء (الافتراضي هو `.next`) يتم تنظيفه الآن افتراضيًا باستثناء ذاكرة التخزين المؤقت لـ Next.js. يمكنك الرجوع إلى [RFC تنظيف `distDir`](https://github.com/vercel/next.js/discussions/6009) لمزيد من المعلومات.
...
إذا كان ملف `pages/_app.js` يستورد `Container` من `next/app`، يمكنك إزالته حيث تمت إزالته. تعلم المزيد في [التوثيق](/docs/messages/app-container-deprecated).
...
أدخل React 17 [تحويل JSX جديد](https://reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) الذي يجلب ميزة طويلة الأمد لـ Next.js إلى نظام React الأوسع: عدم الحاجة إلى `import React from 'react'` عند استخدام JSX. عند استخدام React 17 سيقوم Next.js تلقائيًا باستخدام التحويل الجديد. هذا التحويل لا يجعل المتغير `React` عامًا، والذي كان تأثيرًا جانبيًا غير مقصود للتنفيذ السابق لـ Next.js. يتوفر [أداة تحويل أكواد](/docs/14/pages/building-your-application/upgrading/codemods#add-missing-react-import) لإصلاح الحالات التي استخدمت فيها `React` دون استيراده عن طريق الخطأ.
