...
تتيح لك `revalidatePath` إزالة [البيانات المخزنة مؤقتًا](/docs/14/app/building-your-application/caching) عند الطلب لمسار معين.

> **معلومة مفيدة**:
>
> - `revalidatePath` متاحة في كل من [بيئات تشغيل Node.js و Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> - حاليًا، `revalidatePath` تبطل صلاحية جميع المسارات في [ذاكرة التخزين المؤقت لموجه العميل](/docs/14/app/building-your-application/caching#router-cache). هذا السلوك مؤقت وسيتم تحديثه في المستقبل ليتم تطبيقه فقط على المسار المحدد.
> - استخدام `revalidatePath` يبطل صلاحية **المسار المحدد فقط** في [ذاكرة التخزين المؤقت للمسار من جانب الخادم](/docs/14/app/building-your-application/caching#full-route-cache).
