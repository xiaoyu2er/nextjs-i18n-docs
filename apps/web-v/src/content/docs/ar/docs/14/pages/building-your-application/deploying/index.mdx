...
بما أن Next.js يدعم هذا [التصدير الثابت](/docs/14/app/building-your-application/deploying/static-exports)، يمكن نشره واستضافته على أي خادم ويب يمكنه تقديم أصول ثابتة HTML/CSS/JS. يتضمن ذلك أدوات مثل AWS S3 أو Nginx أو Apache.

التشغيل كـ [تصدير ثابت](/docs/14/app/building-your-application/deploying/static-exports) لا يدعم ميزات Next.js التي تتطلب خادمًا. [تعلم المزيد](/docs/14/app/building-your-application/deploying/static-exports#unsupported-features).

> **معلومة مفيدة:**
>
> - [مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components) مدعومة مع التصدير الثابت.

## الميزات

### تحسين الصور

[تحسين الصور](/docs/14/app/building-your-application/optimizing/images) عبر `next/image` يعمل مع الاستضافة الذاتية دون أي إعدادات عند النشر باستخدام `next start`. إذا كنت تفضل أن يكون لديك خدمة منفصلة لتحسين الصور، يمكنك [تكوين محمل صور](/docs/14/app/building-your-application/optimizing/images#loaders).

يمكن استخدام تحسين الصور مع [تصدير ثابت](/docs/14/app/building-your-application/deploying/static-exports#image-optimization) عن طريق تعريف محمل صور مخصص في `next.config.js`. لاحظ أن الصور يتم تحسينها أثناء التشغيل، وليس أثناء البناء.

> **معلومة مفيدة:**
>
> - عند الاستضافة الذاتية، فكر في تثبيت `sharp` لتحسين أداء [تحسين الصور](/docs/14/pages/building-your-application/optimizing/images) في بيئة الإنتاج عن طريق تشغيل `npm install sharp` في دليل مشروعك. على منصات Linux، قد يحتاج `sharp` إلى [إعدادات إضافية](https://sharp.pixelplumbing.com/install#linux-memory-allocator) لمنع استخدام الذاكرة المفرط.
> - تعلم المزيد عن [سلوك التخزين المؤقت للصور المحسنة](/docs/14/app/api-reference/components/image#caching-behavior) وكيفية تكوين TTL.
> - يمكنك أيضًا [تعطيل تحسين الصور](/docs/14/app/api-reference/components/image#unoptimized) والاستفادة من المزايا الأخرى لاستخدام `next/image` إذا كنت تفضل ذلك. على سبيل المثال، إذا كنت تقوم بتحسين الصور بنفسك بشكل منفصل.

### Middleware

[Middleware](/docs/14/app/building-your-application/routing/middleware) يعمل مع الاستضافة الذاتية دون أي إعدادات عند النشر باستخدام `next start`. نظرًا لأنه يتطلب الوصول إلى الطلب الوارد، فهو غير مدعوم عند استخدام [تصدير ثابت](/docs/14/app/building-your-application/deploying/static-exports).

يستخدم Middleware [وقت تشغيل](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes) وهو مجموعة فرعية من جميع واجهات برمجة تطبيقات Node.js المتاحة للمساعدة في ضمان زمن انتقال منخفض، حيث قد يعمل أمام كل مسار أو أصل في تطبيقك. لا يتطلب هذا وقت التشغيل تشغيل "على الحافة" ويعمل في خادم أحادي المنطقة. هناك حاجة إلى إعدادات إضافية وبنية تحتية لتشغيل Middleware في مناطق متعددة.

إذا كنت تبحث عن إضافة منطق (أو استخدام حزمة خارجية) تتطلب جميع واجهات برمجة تطبيقات Node.js، فقد تتمكن من نقل هذا المنطق إلى [تخطيط](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) كمكون [خادم](/docs/14/app/building-your-application/rendering/server-components). على سبيل المثال، التحقق من [الرؤوس](/docs/14/app/api-reference/functions/headers) و[إعادة التوجيه](/docs/14/app/api-reference/functions/redirect). يمكنك أيضًا استخدام الرؤوس أو ملفات تعريف الارتباط أو معلمات الاستعلام لـ [إعادة التوجيه](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching) أو [إعادة الكتابة](/docs/14/app/api-reference/next-config-js/rewrites#header-cookie-and-query-matching) عبر `next.config.js`. إذا لم ينجح ذلك، يمكنك أيضًا استخدام [خادم مخصص](/docs/14/pages/building-your-application/configuring/custom-server).
...
لقراءة متغيرات البيئة في وقت التشغيل، نوصي باستخدام `getServerSideProps` أو [الترقية التدريجية لموجه التطبيق](/docs/14/app/building-your-application/upgrading/app-router-migration). مع موجه التطبيق، يمكننا قراءة متغيرات البيئة بأمان على الخادم أثناء التقديم الديناميكي. هذا يسمح لك باستخدام صورة Docker واحدة يمكن ترقيتها عبر بيئات متعددة بقيم مختلفة.
...
> **معلومة مفيدة:**
>
> - يمكنك تشغيل التعليمات البرمجية عند بدء تشغيل الخادم باستخدام [وظيفة `register`](/docs/14/app/building-your-application/optimizing/instrumentation).
> - لا نوصي باستخدام خيار [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration)، لأنه لا يعمل مع وضع الإخراج المستقل. بدلاً من ذلك، نوصي [بالترقية التدريجية](/docs/14/app/building-your-application/upgrading/app-router-migration) لموجه التطبيق.
...
- يضبط Next.js رأس `Cache-Control` على `public, max-age=31536000, immutable` للأصول الثابتة حقًا. لا يمكن تجاوزه. تحتوي هذه الملفات الثابتة على تجزئة SHA في اسم الملف، لذا يمكن تخزينها مؤقتًا إلى أجل غير مسمى بأمان. على سبيل المثال، [استيراد الصور الثابتة](/docs/14/app/building-your-application/optimizing/images#local-images). يمكنك [تكوين TTL](/docs/14/app/api-reference/components/image#caching-behavior) للصور.
- يضبط التوليد الثابت التدريجي (ISR) رأس `Cache-Control` على `s-maxage: <revalidate in getStaticProps>, stale-while-revalidate`. يتم تعريف وقت إعادة التحقق هذا في [وظيفة `getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) بالثواني. إذا قمت بتعيين `revalidate: false`، فسيتم تعيين مدة التخزين المؤقت الافتراضية إلى سنة واحدة.
- تعيين الصفحات المقدمة ديناميكيًا رأس `Cache-Control` على `private, no-cache, no-store, max-age=0, must-revalidate` لمنع تخزين البيانات الخاصة بالمستخدم مؤقتًا. ينطبق هذا على كل من موجه التطبيق وموجه الصفحات. يتضمن هذا أيضًا [وضع المسودة](/docs/14/app/building-your-application/configuring/draft-mode).

#### الأصول الثابتة

إذا كنت تريد استضافة أصول ثابتة على نطاق أو CDN مختلف، يمكنك استخدام [تكوين `assetPrefix`](/docs/14/app/api-reference/next-config-js/assetPrefix) في `next.config.js`. سيستخدم Next.js هذه البادئة عند استرداد ملفات JavaScript أو CSS. فصل أصولك إلى نطاق مختلف له عيب الوقت الإضافي المستغرق في تحليل DNS و TLS.

[تعلم المزيد عن `assetPrefix`](/docs/14/app/api-reference/next-config-js/assetPrefix).
...
يدعم موجه تطبيق Next.js [استجابات البث](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) عند الاستضافة الذاتية. إذا كنت تستخدم Nginx أو وكيلًا مشابهًا، فستحتاج إلى تكوينه لتعطيل التخزين المؤقت لتمكين البث.
