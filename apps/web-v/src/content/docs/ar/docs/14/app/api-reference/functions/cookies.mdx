...
تتيح لك وظيفة `cookies` قراءة ملفات تعريف الارتباط (cookies) الواردة في طلب HTTP من [مكون الخادم (Server Component)](/docs/14/app/building-your-application/rendering/server-components) أو كتابة ملفات تعريف الارتباط الصادرة في [إجراء الخادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) أو [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).

> **معلومة مفيدة**: `cookies()` هي [وظيفة ديناميكية (Dynamic Function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) لا يمكن معرفة قيمها مسبقًا. استخدامها في تخطيط أو صفحة سيؤدي إلى تحويل المسار إلى [عرض ديناميكي (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering) عند وقت الطلب.
...
> **معلومة مفيدة**: لا يسمح HTTP بتعيين ملفات تعريف الارتباط بعد بدء البث، لذا يجب استخدام `.set()` في [إجراء الخادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) أو [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
...
> **معلومة مفيدة**: يمكنك فقط حذف ملفات تعريف الارتباط في [إجراء الخادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) أو [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
...
> **معلومة مفيدة**: `.set()` متاح فقط في [إجراء الخادم (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) أو [معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
