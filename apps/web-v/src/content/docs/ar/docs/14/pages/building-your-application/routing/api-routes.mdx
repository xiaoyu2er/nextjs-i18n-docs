...
translation-updated-at: 2025-06-06T17:29:18.258Z
title: مسارات API
description: يدعم Next.js مسارات API التي تتيح لك بناء واجهة برمجة التطبيقات (API) دون الحاجة إلى مغادرة تطبيق Next.js. تعرف على كيفية عملها هنا.
sidebar:
  order: 3010107
---
...
> **معلومة مفيدة**: إذا كنت تستخدم App Router، يمكنك استخدام [مكونات الخادم (Server Components)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) أو [معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) بدلاً من مسارات API.
...
> **معلومة مفيدة**:
>
> - مسارات API [لا تحدد رؤوس CORS](https://developer.mozilla.org/docs/Web/HTTP/CORS)، مما يعني أنها **تعمل فقط من نفس المصدر (same-origin only)** افتراضيًا. يمكنك تخصيص هذا السلوك عن طريق تغليف معالج الطلب بـ[مساعدات طلب CORS](https://github.com/vercel/next.js/tree/canary/examples/api-routes-cors).
> - لا يمكن استخدام مسارات API مع [التصدير الثابت (static exports)](/docs/14/pages/building-your-application/deploying/static-exports). ومع ذلك، يمكن لـ[معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) في App Router ذلك.
> - ستتأثر مسارات API بـ[إعدادات `pageExtensions`](/docs/14/pages/api-reference/next-config-js/pageExtensions) في `next.config.js`.
...
`bodyParser.sizeLimit` هو الحد الأقصى للحجم المسموح به للجسم المحلل، بأي تنسيق مدعوم من [bytes](https://github.com/visionmedia/bytes.js)، كما يلي:
...
- `res.revalidate(urlPath)` - [إعادة التحقق من صفحة عند الطلب](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) باستخدام `getStaticProps`. يجب أن يكون `urlPath` `سلسلة`.
...
تدعم مسارات API [المسارات الديناميكية](/docs/14/pages/building-your-application/routing/dynamic-routes)، وتتبع نفس قواعد تسمية الملفات المستخدمة لـ `pages/`.
...
إذا كنت ترغب في استخدام مسارات API مع Edge Runtime، نوصي بتبني App Router تدريجيًا واستخدام [معالجات المسار (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) بدلاً من مسارات API.
