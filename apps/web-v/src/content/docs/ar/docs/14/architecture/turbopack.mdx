...
لا يحتاج Turbopack في Next.js إلى أي تكوين لمعظم المستخدمين ويمكن توسيعه لحالات استخدام أكثر تقدمًا. لمعرفة المزيد عن الميزات المدعومة حاليًا في Turbopack، راجع [مرجع API](/docs/14/app/api-reference/next-config-js/turbo).
...
- تكوين [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) في `next.config.js`
  - يحل Turbopack محل Webpack، مما يعني أن تكوين Webpack غير مدعوم.
  - لتكوين Turbopack، [راجع الوثائق](/docs/14/app/api-reference/next-config-js/turbo).
  - مجموعة فرعية من [حَمّالات Webpack](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) مدعومة في Turbopack.
- Babel (`.babelrc`)
  ...
  - إذا كنت لا تزال بحاجة إلى استخدام Babel بعد التحقق من أن حالة استخدامك غير مغطاة، يمكنك الاستفادة من [دعم Turbopack لحَمّالات Webpack المخصصة](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) لتضمين `babel-loader`.
...
- `@next/font` (دعم الخطوط القديم).
  - `@next/font` لم يعد مدعومًا ويُفضل استخدام `next/font`. [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) مدعوم بالكامل مع Turbopack.
