...
راجع وثائق مرجع [`next/script`](/docs/14/app/api-reference/components/script#strategy) API لمعرفة المزيد حول كل استراتيجية وحالات استخدامها.

### تفريغ النصوص البرمجية إلى عامل ويب (تجريبي)

> **تحذير:** استراتيجية `worker` ليست مستقرة بعد ولا تعمل مع دليل [`app`](/docs/14/app/building-your-application/routing/defining-routes). استخدم بحذر.
...
ستعمل هذه المعالجات فقط عند استيراد `next/script` واستخدامها داخل [مكون عميل](/docs/14/app/building-your-application/rendering/client-components) حيث يتم تعريف `"use client"` كأول سطر من الكود:
...
راجع مرجع [`next/script`](/docs/14/app/api-reference/components/script#onload) API لمعرفة المزيد حول كل معالج حدث وعرض الأمثلة.
...
راجع مرجع [`next/script`](/docs/14/pages/api-reference/components/script#onload) API لمعرفة المزيد حول كل معالج حدث وعرض الأمثلة.
