...
تسمح لك دالة `redirect` بإعادة توجيه المستخدم إلى عنوان URL آخر. يمكنك استدعاء `redirect` في [مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components)، [معالجات المسار](/docs/14/app/building-your-application/routing/route-handlers)، و[إجراءات الخادم](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
راجع [مرجع API لـ `redirect`](/docs/14/app/api-reference/functions/redirect) لمزيد من المعلومات.

## دالة `permanentRedirect`

تسمح لك دالة `permanentRedirect` بإعادة توجيه المستخدم **بشكل دائم** إلى عنوان URL آخر. يمكنك استدعاء `permanentRedirect` في [مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components)، [معالجات المسار](/docs/14/app/building-your-application/routing/route-handlers)، و[إجراءات الخادم](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
راجع [مرجع API لـ `permanentRedirect`](/docs/14/app/api-reference/functions/permanentRedirect) لمزيد من المعلومات.
...
> **معلومة مفيدة**:
>
> - إذا لم تكن بحاجة إلى تنقل المستخدم برمجيًا، يجب استخدام مكون [`<Link>`](/docs/14/app/api-reference/components/link).

<AppOnly>

راجع [مرجع API لـ `useRouter`](/docs/14/app/api-reference/functions/use-router) لمزيد من المعلومات.

</AppOnly>

<PagesOnly>

راجع [مرجع API لـ `useRouter`](/docs/14/pages/api-reference/functions/use-router) لمزيد من المعلومات.
...
يدعم `redirects` [مطابقة المسار](/docs/14/app/api-reference/next-config-js/redirects#path-matching)، [مطابقة الرأس، الكوكيز، والاستعلام](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching)، مما يمنحك المرونة لإعادة توجيه المستخدمين بناءً على طلب وارد.
...
راجع [مرجع API لـ `redirects`](/docs/14/app/api-reference/next-config-js/redirects) لمزيد من المعلومات.

> **معلومة مفيدة**:
>
> - يمكن أن تُرجع `redirects` رمز حالة 307 (إعادة توجيه مؤقتة) أو 308 (إعادة توجيه دائمة) مع خيار `permanent`.
> - قد يكون لـ `redirects` حد على بعض المنصات. على سبيل المثال، في Vercel، هناك حد لـ 1,024 إعادة توجيه. لإدارة عدد كبير من عمليات إعادة التوجيه (1000+)، فكر في إنشاء حل مخصص باستخدام [Middleware](/docs/14/app/building-your-application/routing/middleware). راجع [إدارة عمليات إعادة التوجيه على نطاق واسع](#managing-redirects-at-scale-advanced) للمزيد.
...
راجع وثائق [Middleware](/docs/14/app/building-your-application/routing/middleware) لمزيد من المعلومات.
...
إذا كان موجودًا، يتم توجيه الطلب إلى <AppOnly>[معالج المسار (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers)</AppOnly> <PagesOnly>[مسارات API (API Routes)](/docs/14/pages/building-your-application/routing/api-routes)</PagesOnly> والذي سيتحقق من الملف الفعلي ويقوم بإعادة توجيه المستخدم إلى العنوان المناسب. هذا يتجنب استيراد ملف إعادة توجيه كبير إلى Middleware، مما يمكن أن يبطئ كل طلب وارد.
