...
`<Link>` هو مكون React يمتد عنصر HTML `<a>` لتوفير [الجلب المسبق (Prefetching)](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) والتنقل من جانب العميل بين المسارات. وهو الطريقة الأساسية للتنقل بين المسارات في Next.js.
...
> **معلومة مفيدة**:
>
> - سيؤدي Next.js إلى التمرير إلى [الصفحة](/docs/14/app/building-your-application/routing/pages-and-layouts#pages) إذا لم تكن مرئية في نافذة العرض عند التنقل.
...
- `false`: لن يحدث الجلب المسبق عند الدخول إلى نافذة العرض، ولكن سيحدث عند التمرير. إذا كنت ترغب في إزالة الجلب تمامًا عند التمرير أيضًا، ففكر في استخدام علامة `<a>` أو [التبديل التدريجي](/docs/14/app/building-your-application/upgrading/app-router-migration) إلى App Router، والذي يتيح تعطيل الجلب المسبق عند التمرير أيضًا.
...
لم يعد عنصر `<a>` مطلوبًا كطفل لـ `<Link>`. أضف خاصية `legacyBehavior` لاستخدام السلوك القديم أو قم بإزالة `<a>` للترقية. يتوفر [أداة تحويل تلقائي (codemod)](/docs/14/app/building-your-application/upgrading/codemods#new-link) لترقية الكود تلقائيًا.
...
تحديث مسار الصفحة الحالية دون إعادة تشغيل [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)، [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) أو [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props). القيمة الافتراضية `false`.
...
إذا كان طفل `Link` هو مكون مخصص يلف علامة `<a>`، فيجب عليك إضافة `passHref` إلى `Link`. هذا ضروري إذا كنت تستخدم مكتبات مثل [styled-components](https://styled-components.com/). بدون ذلك، لن تحتوي علامة `<a>` على سمة `href`، مما يؤثر على إمكانية الوصول إلى موقعك وقد يؤثر على SEO. إذا كنت تستخدم [ESLint](/docs/14/app/building-your-application/configuring/eslint#eslint-plugin)، فهناك قاعدة مدمجة `next/link-passhref` لضمان الاستخدام الصحيح لـ `passHref`.

</AppOnly>

<PagesOnly>

إذا كان طفل `Link` هو مكون مخصص يلف علامة `<a>`، فيجب عليك إضافة `passHref` إلى `Link`. هذا ضروري إذا كنت تستخدم مكتبات مثل [styled-components](https://styled-components.com/). بدون ذلك، لن تحتوي علامة `<a>` على سمة `href`، مما يؤثر على إمكانية الوصول إلى موقعك وقد يؤثر على SEO. إذا كنت تستخدم [ESLint](/docs/14/pages/building-your-application/configuring/eslint#eslint-plugin)، فهناك قاعدة مدمجة `next/link-passhref` لضمان الاستخدام الصحيح لـ `passHref`.
...
- [مسار ديناميكي](/docs/14/app/building-your-application/routing/dynamic-routes): `/blog/my-post`
...
من الشائع استخدام [Middleware](/docs/14/app/building-your-application/routing/middleware) للمصادقة أو لأغراض أخرى تتضمن إعادة كتابة المستخدم إلى صفحة مختلفة. لكي يعمل مكون `<Link />` بشكل صحيح في جلب الروابط مسبقًا مع إعادة الكتابة عبر Middleware، تحتاج إلى إخبار Next.js بكل من URL المعروض وURL المطلوب جلبها مسبقًا. هذا مطلوب لتجنب عمليات الجلب غير الضرورية إلى middleware لمعرفة المسار الصحيح للجلب المسبق.
...
> **معلومة مفيدة**: إذا كنت تستخدم [المسارات الديناميكية](/docs/14/app/building-your-application/routing/dynamic-routes)، فستحتاج إلى تكييف خصائص `as` و `href`. على سبيل المثال، إذا كان لديك مسار ديناميكي مثل `/dashboard/authed/[user]` تريد عرضه بشكل مختلف عبر middleware، فستكتب: `<Link href={{ pathname: '/dashboard/authed/[user]', query: { user: username } }} as="/dashboard/[user]">Profile</Link>`.
...
| الإصدار   | التغييرات                                                                                                                                                                                         |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.0.0` | لم يعد يتطلب وسم `<a>` فرعي. تم توفير [أداة تحويل الشفرات](/docs/14/app/building-your-application/upgrading/codemods#remove-a-tags-from-link-components) لتحديث قاعدة الشفرة تلقائيًا. |
