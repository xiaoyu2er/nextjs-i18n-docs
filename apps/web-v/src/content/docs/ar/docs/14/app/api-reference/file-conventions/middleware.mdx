...
يُستخدم ملف `middleware.js|ts` لكتابة [Middleware](/docs/14/app/building-your-application/routing/middleware) وتنفيذ الكود على الخادم قبل اكتمال الطلب. ثم بناءً على الطلب الوارد، يمكنك تعديل الاستجابة عن طريق إعادة الكتابة، التوجيه، تعديل رؤوس الطلب أو الاستجابة، أو الرد مباشرة.
...
يمكن لـ Middleware استخدام كائن [`NextResponse`](/docs/14/app/building-your-application/routing/middleware#nextresponse) الذي يمتد من [واجهة برمجة تطبيقات Web Response](https://developer.mozilla.org/en-US/docs/Web/API/Response). عن طريق إرجاع كائن `NextResponse`، يمكنك معالجة ملفات تعريف الارتباط مباشرة، تعيين الرؤوس، تنفيذ عمليات التوجيه، وإعادة كتابة المسارات.

> **معلومة مفيدة**: بالنسبة لعمليات التوجيه، يمكنك أيضًا استخدام `Response.redirect` بدلاً من `NextResponse.redirect`.

## وقت التشغيل

يدعم Middleware فقط [وقت تشغيل Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes). لا يمكن استخدام وقت تشغيل Node.js.
