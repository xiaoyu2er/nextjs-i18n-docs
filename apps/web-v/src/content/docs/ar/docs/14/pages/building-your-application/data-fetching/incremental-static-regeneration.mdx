...
translation-updated-at: 2025-06-06T17:26:19.966Z
...
> **معلومة مفيدة**: وقت التشغيل [`edge`](/docs/14/pages/api-reference/edge) غير متوافق حاليًا مع ISR، على الرغم من أنه يمكنك الاستفادة من `stale-while-revalidate` عن طريق تعيين رأس `cache-control` يدويًا.
...
> **معلومة مفيدة**: لن يتم تنفيذ [البرمجيات الوسيطة](/docs/14/pages/building-your-application/routing/middleware) لطلبات ISR عند الطلب. بدلاً من ذلك، استدعِ `revalidate()` على المسار _الدقيق_ الذي تريد إعادة التحقق منه. على سبيل المثال، إذا كان لديك `pages/blog/[slug].js` وإعادة توجيه من `/post-1` -> `/blog/post-1`، فستحتاج إلى استدعاء `res.revalidate('/blog/post-1')`.
...
بعد ذلك، أضف الرمز السري كـ [متغير بيئة](/docs/14/pages/building-your-application/configuring/environment-variables) إلى تطبيقك. أخيرًا، أنشئ مسار API إعادة التحقق:
...
[شاهد العرض التوضيحي لدينا](https://on-demand-isr.vercel.app) لرؤية إعادة التحقق عند الطلب في العمل وتقديم ملاحظاتك.

### اختبار ISR عند الطلب أثناء التطوير

عند التشغيل محليًا باستخدام `next dev`، يتم استدعاء `getStaticProps` عند كل طلب. للتحقق من صحة تكوين ISR عند الطلب، ستحتاج إلى إنشاء [بناء إنتاج](/docs/14/pages/api-reference/cli/next#next-build-options) وبدء [خادم الإنتاج](/docs/14/pages/api-reference/cli/next#next-start-options):
...
يعمل التحديث التدريجي للصفحات الثابتة (ISR) على [مواقع Next.js المخصصة للاستضافة الذاتية](/docs/14/pages/building-your-application/deploying#self-hosting) مباشرة عند استخدام `next start`.

تعلم المزيد عن [استضافة Next.js ذاتيًا](/docs/14/pages/building-your-application/deploying#self-hosting).
...
| `v12.0.0` | تمت إضافة [التراجع المعتمد على الروبوتات لـ ISR](https://nextjs.org/blog/next-12#bot-aware-isr-fallback). |
