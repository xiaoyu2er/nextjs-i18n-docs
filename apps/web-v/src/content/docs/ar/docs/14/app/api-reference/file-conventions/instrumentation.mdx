...
لاستخدامه، ضع الملف في **الجذر** لتطبيقك أو داخل مجلد [`src`](/docs/14/app/building-your-application/configuring/src-directory) إذا كنت تستخدم واحدًا.

## خيار التكوين

الأدوات (Instrumentation) حاليًا ميزة تجريبية، لاستخدام ملف `instrumentation`، يجب عليك الموافقة صراحةً عن طريق تعريف [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) في ملف `next.config.js` الخاص بك:
