...
أسهل طريقة لإنشاء أيقونة هي استخدام واجهة برمجة التطبيقات [`ImageResponse`](/docs/14/app/api-reference/functions/image-response) من `next/og`.
...
> **معلومة مفيدة**
>
> - بشكل افتراضي، يتم [**تحسين الأيقونات المولدة بشكل ثابت**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (يتم إنشاؤها وقت البناء وتخزينها مؤقتًا) ما لم تستخدم [وظائف ديناميكية](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) أو بيانات غير مخزنة مؤقتًا.
> - يمكنك إنشاء أيقونات متعددة في نفس الملف باستخدام [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata).
...
كائن يحتوي على [كائن معلمات المسار الديناميكي](/docs/14/app/building-your-application/routing/dynamic-routes) من المقطع الجذري وصولاً إلى المقطع الذي يوجد فيه `icon` أو `apple-icon`.
...
`icon` و `apple-icon` هما معالجان متخصصان [للمسارات (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) يمكنهما استخدام نفس [خيارات تكوين مقطع المسار](/docs/14/app/api-reference/file-conventions/route-segment-config) مثل الصفحات والتخطيطات.
