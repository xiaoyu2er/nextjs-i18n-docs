...
translation-updated-at: 2025-06-06T17:23:13.791Z
title: useRouter
...
إذا كنت ترغب في الوصول إلى [كائن `router`](#router-object) داخل أي مكون وظيفي في تطبيقك، يمكنك استخدام خطاف `useRouter`. إليك المثال التالي:
...
> `useRouter` هو [خطاف React](https://react.dev/learn#using-hooks)، مما يعني أنه لا يمكن استخدامه مع الفئات. يمكنك إما استخدام [withRouter](#withrouter) أو تغليف فئتك داخل مكون وظيفي.
...
- `query`: `Object` - سلسلة الاستعلام المحللة إلى كائن، بما في ذلك معلمات [المسار الديناميكي](/docs/14/pages/building-your-application/routing/dynamic-routes). سيكون كائنًا فارغًا أثناء التصيير المسبق إذا لم تستخدم الصفحة [التصيير من جانب الخادم (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props). القيمة الافتراضية هي `{}`.
...
- `isFallback`: `boolean` - ما إذا كانت الصفحة الحالية في [وضع الاحتياطي (fallback mode)](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true).
- `basePath`: `String` - [basePath](/docs/14/app/api-reference/next-config-js/basePath) النشط (إذا تم تمكينه).
...
- `isReady`: `boolean` - ما إذا كانت حقول الموجه محدثة من جانب العميل وجاهزة للاستخدام. يجب استخدامها فقط داخل طرق `useEffect` وليس للتصيير الشرطي على الخادم. راجع الوثائق ذات الصلة لحالة الاستخدام مع [الصفحات المحسنة تلقائيًا بشكل ثابت](/docs/14/pages/building-your-application/rendering/automatic-static-optimization).
- `isPreview`: `boolean` - ما إذا كان التطبيق حاليًا في [وضع المعاينة (preview mode)](/docs/14/pages/building-your-application/configuring/preview-mode).

> استخدام حقل `asPath` قد يؤدي إلى عدم تطابق بين العميل والخادم إذا تم تصيير الصفحة باستخدام التصيير من جانب الخادم أو [التحسين الثابت التلقائي](/docs/14/pages/building-your-application/rendering/automatic-static-optimization). تجنب استخدام `asPath` حتى يصبح حقل `isReady` `true`.

تتضمن الطرق التالية داخل `router`:

### router.push

يتحكم في الانتقالات من جانب العميل، هذه الطريقة مفيدة للحالات التي لا يكون فيها [`next/link`](/docs/14/pages/api-reference/components/link) كافيًا.
...
- `options` - كائن اختياري مع خيارات التهيئة التالية:
  ...
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): تحديث مسار الصفحة الحالية دون إعادة تشغيل [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) أو [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) أو [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props). القيمة الافتراضية هي `false`.
...
تحويل المستخدم إلى `pages/login.js`، مفيد للصفحات خلف [المصادقة](/docs/14/pages/building-your-application/authentication):
...
- استخدم `key` في React [لإخبار React بإعادة تحميل المكون](https://react.dev/learn/rendering-lists#keeping-list-items-in-order-with-key). للقيام بذلك لجميع الصفحات، يمكنك استخدام تطبيق مخصص:
...
يمكنك استخدام كائن URL بنفس الطريقة التي يمكنك استخدامها مع [`next/link`](/docs/14/pages/api-reference/components/link#with-url-object). يعمل لكل من معلمتي `url` و `as`:
...
مشابه لخاصية `replace` في [`next/link`](/docs/14/pages/api-reference/components/link)، `router.replace` سيمنع إضافة إدخال URL جديد إلى سجل `history`.
...
جلب الصفحات مسبقًا لتحقيق انتقالات أسرع من جانب العميل. هذه الطريقة مفيدة فقط للتنقلات بدون [`next/link`](/docs/14/pages/api-reference/components/link)، حيث يتولى `next/link` جلب الصفحات مسبقًا تلقائيًا.
...
في بعض الحالات (على سبيل المثال، إذا كنت تستخدم [خادم مخصص](/docs/14/pages/building-your-application/configuring/custom-server))، قد ترغب في الاستماع إلى [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) والقيام بشيء ما قبل أن يتصرف الموجه بناءً عليه.
...
إذا أعادت `cb` `false`، فلن يتعامل موجه Next.js مع `popstate`، وسيكون مسؤوليتك التعامل معه في تلك الحالة. راجع [تعطيل توجيه نظام الملفات](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
> **ملاحظة مهمة**: هنا `url` هو عنوان URL الذي يظهر في المتصفح، بما في ذلك [`basePath`](/docs/14/app/api-reference/next-config-js/basePath).
...
> نستخدم [تطبيقًا مخصصًا](/docs/14/pages/building-your-application/routing/custom-app) (`pages/_app.js`) لهذا المثال للاشتراك في الحدث لأنه لا يتم فك تحميله عند تنقلات الصفحة، ولكن يمكنك الاشتراك في أحداث الموجه في أي مكون في تطبيقك.

يجب تسجيل أحداث الموجه عندما يتم تحميل مكون ([useEffect](https://react.dev/reference/react/useEffect) أو [componentDidMount](https://react.dev/reference/react/Component#componentdidmount) / [componentWillUnmount](https://react.dev/reference/react/Component#componentwillunmount)) أو بشكل إلزامي عند حدوث حدث.
...
بعض الطرق المتاحة على كائن `router` تُرجع Promise. إذا كان لديك قاعدة ESLint [no-floating-promises](https://typescript-eslint.io/rules/no-floating-promises) مفعّلة، يُنظر بتعطيلها إما بشكل عام، أو للأسطر المتأثرة.
