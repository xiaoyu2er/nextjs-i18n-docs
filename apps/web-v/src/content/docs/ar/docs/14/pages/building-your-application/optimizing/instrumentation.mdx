...
translation-updated-at: 2025-06-06T17:24:16.247Z
title: التجهيز (Instrumentation)
description: تعلم كيفية استخدام التجهيز لتشغيل التعليمات البرمجية عند بدء تشغيل الخادم في تطبيق Next.js الخاص بك
...
لإعداد التجهيز الآلي، قم بإنشاء ملف `instrumentation.ts|js` في **الدليل الجذري** لمشروعك (أو داخل مجلد [`src`](/docs/14/app/building-your-application/configuring/src-directory) إذا كنت تستخدم واحدًا).
...
> **معلومة مفيدة**
>
> - هذه الميزة **تجريبية**. لاستخدامها، يجب عليك الموافقة صراحةً عن طريق تعريف [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) في ملف `next.config.js`.
...
> - إذا كنت تستخدم خيار [`pageExtensions`](/docs/14/app/api-reference/next-config-js/pageExtensions) لإضافة لاحقة، فستحتاج أيضًا إلى تحديث اسم ملف `instrumentation` ليتطابق.
...
يستدعي Next.js دالة `register` في جميع البيئات، لذلك من المهم استيراد أي تعليمات برمجية لا تدعم بيئات تشغيل محددة (مثل [Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime) أو [Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime)) بشكل مشروط. يمكنك استخدام متغير البيئة `NEXT_RUNTIME` للحصول على البيئة الحالية:
