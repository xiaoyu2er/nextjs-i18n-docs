...
- استخدام [خطاف `useRouter`](#userouter-hook) ([مكونات العميل](/docs/14/app/building-your-application/rendering/client-components))
- استخدام [دالة `redirect`](#redirect-function) ([مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components))
...
هناك خصائص اختيارية أخرى يمكنك تمريرها إلى `<Link>`. راجع [مرجع API](/docs/14/app/api-reference/components/link) للمزيد.

### أمثلة

#### الربط إلى الأجزاء الديناميكية

عند الربط إلى [الأجزاء الديناميكية](/docs/14/app/building-your-application/routing/dynamic-routes)، يمكنك استخدام [القوالب الحرفية والاستيفاء](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals) لإنشاء قائمة من الروابط. على سبيل المثال، لإنشاء قائمة من منشورات المدونة:
...
يمكنك استخدام [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname) لتحديد ما إذا كان الرابط نشطًا. على سبيل المثال، لإضافة فئة إلى الرابط النشط، يمكنك التحقق مما إذا كان `pathname` الحالي يتطابق مع `href` الرابط:
...
> **جيد أن تعرف**:
>
> - سيقوم Next.js بالتمرير إلى [الصفحة](/docs/14/app/building-your-application/routing/pages-and-layouts#pages) إذا لم تكن مرئية في نافذة العرض عند التنقل.
...
يسمح لك خطاف `useRouter` بتغيير المسارات برمجيًا من [مكونات العميل](/docs/14/app/building-your-application/rendering/client-components).
...
للحصول على قائمة كاملة بطرق `useRouter`، راجع [مرجع API](/docs/14/app/api-reference/functions/use-router).

> **توصية:** استخدم مكون `<Link>` للتنقل بين المسارات ما لم يكن لديك متطلب محدد لاستخدام `useRouter`.

## دالة `redirect`

بالنسبة لـ [مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components)، استخدم دالة `redirect` بدلاً من ذلك.
...
> - إذا كنت ترغب في إعادة التوجيه قبل عملية العرض، استخدم [`next.config.js`](/docs/14/app/building-your-application/routing/redirecting#redirects-in-nextconfigjs) أو [البرنامج الوسيط](/docs/14/app/building-your-application/routing/redirecting#nextresponseredirect-in-middleware).

راجع [مرجع API لـ `redirect`](/docs/14/app/api-reference/functions/redirect) للمزيد من المعلومات.
...
تتكامل استدعاءات `pushState` و `replaceState` مع موجه Next.js، مما يسمح لك بالمزامنة مع [`usePathname`](/docs/14/app/api-reference/functions/use-pathname) و [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params).
...
تسمح لك [مكونات الخادم](/docs/14/app/building-your-application/rendering/server-components) بتقسيم كود التطبيق تلقائيًا حسب أجزاء المسار. هذا يعني أنه يتم تحميل الكود المطلوب للمسار الحالي فقط عند التنقل.
...
يختلف سلوك الجلب المسبق الافتراضي لـ `<Link>` (أي عندما تكون الخاصية `prefetch` غير محددة أو مضبوطة على `null`) اعتمادًا على استخدامك لـ [`loading.js`](/docs/14/app/api-reference/file-conventions/loading). يتم جلب التخطيط المشترك فقط، وصولاً إلى "شجرة" المكونات المعروضة حتى ملف `loading.js` الأول، وتخزينه مؤقتًا لمدة `30s`. هذا يقلل من تكلفة جلب مسار ديناميكي كامل، ويعني أنه يمكنك عرض [حالة تحميل فورية](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states) لردود فعل مرئية أفضل للمستخدمين.
...
راجع [مرجع API لـ `<Link>`](/docs/14/app/api-reference/components/link) للمزيد من المعلومات.
...
يحتوي Next.js على **تخزين مؤقت من جانب العميل في الذاكرة** يسمى [تخزين الموجه المؤقت](/docs/14/app/building-your-application/caching#router-cache). أثناء تنقل المستخدمين في التطبيق، يتم تخزين حمولة مكون خادم React لأجزاء المسار المسبقة الجلب والمسارات التي تمت زيارتها في التخزين المؤقت.
...
تعرف على المزيد حول كيفية عمل [تخزين الموجه المؤقت](/docs/14/app/building-your-application/caching#router-cache) وكيفية تكوينه.
...
افتراضيًا، سوف يحافظ Next.js على موضع التمرير للتنقل للخلف وللأمام، ويعيد استخدام أجزاء المسار في [تخزين الموجه المؤقت](/docs/14/app/building-your-application/caching#router-cache).
