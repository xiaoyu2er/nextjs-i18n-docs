...
هذا يحمل `process.env.DB_HOST`، `process.env.DB_USER`، و `process.env.DB_PASS` إلى بيئة Node.js تلقائيًا مما يسمح لك باستخدامها في [طرق جلب البيانات في Next.js](/docs/14/pages/building-your-application/data-fetching) و [مسارات API](/docs/14/pages/building-your-application/routing/api-routes).

على سبيل المثال، باستخدام [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props):
...
> هذا يحمل `process.env.DB_HOST`، `process.env.DB_USER`، و `process.env.DB_PASS` إلى بيئة Node.js تلقائيًا مما يسمح لك باستخدامها في [معالجات المسار](/docs/14/app/building-your-application/routing/route-handlers).
...
لقراءة متغيرات بيئة التشغيل، نوصي باستخدام `getServerSideProps` أو [اعتماد موجه التطبيق تدريجيًا](/docs/14/app/building-your-application/upgrading/app-router-migration). مع موجه التطبيق، يمكننا قراءة متغيرات البيئة بأمان على الخادم أثناء التصيير الديناميكي. هذا يسمح لك باستخدام صورة Docker واحدة يمكن ترقيتها عبر بيئات متعددة بقيم مختلفة.
...
- يمكنك تشغيل الكود عند بدء تشغيل الخادم باستخدام [دالة `register`](/docs/14/app/building-your-application/optimizing/instrumentation).
- لا نوصي باستخدام خيار [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration)، لأنه لا يعمل مع وضع الإخراج المستقل. بدلاً من ذلك، نوصي [باعتماد موجه التطبيق تدريجيًا](/docs/14/app/building-your-application/upgrading/app-router-migration).
...
- إذا كنت تستخدم [مجلد `/src`](/docs/14/app/building-your-application/configuring/src-directory)، يجب أن تبقى ملفات `.env.*` في جذر مشروعك.
