...
translation-updated-at: 2025-06-06T17:32:40.494Z
title: headers
...
تتيح لك وظيفة `headers` قراءة رؤوس طلب HTTP الواردة من [مكون الخادم (Server Component)](/docs/14/app/building-your-application/rendering/server-components).

## `headers()`

هذه الواجهة تمتد من [واجهة Web Headers API](https://developer.mozilla.org/docs/Web/API/Headers). وهي **للقراءة فقط**، مما يعني أنه لا يمكنك `تعيين` أو `حذف` رؤوس الطلب الصادرة.
...
> **معلومة مفيدة**:
>
> - `headers()` هي [وظيفة ديناميكية (Dynamic Function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) لا يمكن معرفة قيمها المُرجعة مسبقًا. استخدامها في تخطيط أو صفحة سيؤدي إلى تحويل المسار إلى [عرض ديناميكي (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering) وقت الطلب.
...
تُرجع `headers` كائن [Web Headers](https://developer.mozilla.org/docs/Web/API/Headers) **للقراءة فقط**.
...
يمكن استخدام `headers()` مع [Suspense لجلب البيانات (Suspense for Data Fetching)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating).
