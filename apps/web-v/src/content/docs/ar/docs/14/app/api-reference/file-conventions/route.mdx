...
كائن `request` هو كائن [NextRequest](/docs/14/app/api-reference/functions/next-request)، وهو امتداد لواجهة برمجة التطبيقات [Request](https://developer.mozilla.org/docs/Web/API/Request) الخاصة بالويب. يمنحك `NextRequest` تحكمًا أكبر في الطلب الوارد، بما في ذلك الوصول السهل إلى `cookies` وكائن URL ممتد ومحلل `nextUrl`.
...
حاليًا، القيمة الوحيدة لـ `context` هي `params`، وهو كائن يحتوي على [معاملات المسار الديناميكي](/docs/14/app/building-your-application/routing/dynamic-routes) للمسار الحالي.
...
يمكن لمعالجات المسارات توسيع واجهة برمجة التطبيقات Response الخاصة بالويب عن طريق إرجاع كائن `NextResponse`. هذا يسمح لك بسهولة تعيين ملفات تعريف الارتباط (cookies) والعناوين وإعادة التوجيه وإعادة الكتابة. [عرض مرجع API](/docs/14/app/api-reference/functions/next-response).
