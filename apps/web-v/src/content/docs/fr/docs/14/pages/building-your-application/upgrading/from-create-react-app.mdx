...
translation-updated-at: 2025-06-06T16:55:18.468Z
title: Migration depuis Create React App
description: Apprenez comment migrer votre application React existante de Create React App vers Next.js.
...
Avec la prise en charge intégrée du [streaming via React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense), vous pouvez mieux contrôler quelles parties de votre interface utilisateur charger en premier et dans quel ordre, sans introduire de cascades réseau.
...
Le [Middleware Next.js](/docs/14/app/building-your-application/routing/middleware) vous permet d'exécuter du code côté serveur avant qu'une requête ne soit terminée. C'est particulièrement utile pour éviter d'afficher brièvement du contenu non authentifié lorsque l'utilisateur visite une page réservée aux utilisateurs connectés, en le redirigeant vers une page de connexion. Le middleware est aussi utile pour les tests et l'[internationalisation](/docs/14/app/building-your-application/routing/internationalization).

### Optimisations intégrées

Les [images](/docs/14/app/building-your-application/optimizing/images), [polices](/docs/14/app/building-your-application/optimizing/fonts) et [scripts tiers](/docs/14/app/building-your-application/optimizing/scripts) ont souvent un impact significatif sur les performances d'une application. Next.js inclut des composants qui les optimisent automatiquement pour vous.
...
Créez un fichier `next.config.mjs` à la racine de votre projet. Ce fichier contiendra vos [options de configuration Next.js](/docs/14/app/api-reference/next-config-js).
...
Vous trouverez plus d'informations sur la configuration de TypeScript dans la [documentation Next.js](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin).

### Étape 4 : Créer la mise en page racine

Une application utilisant le [routeur App](/docs/14/app) de Next.js doit inclure un fichier de [mise en page racine](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required), qui est un [composant serveur React](/docs/14/app/building-your-application/rendering/server-components) englobant toutes les pages de votre application. Ce fichier est défini au niveau supérieur du répertoire `app`.
...
> **Bon à savoir** : Nous ignorerons le [fichier manifest](/docs/14/app/api-reference/file-conventions/metadata), les icônes supplémentaires autres que le favicon, et la [configuration des tests](/docs/14/app/building-your-application/testing), mais si ces éléments sont nécessaires, Next.js les prend également en charge.
...
Tous les [fichiers de métadonnées](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) tels que `favicon.ico`, `icon.png`, `robots.txt` sont automatiquement ajoutés à la balise `<head>` de l'application tant qu'ils sont placés à la racine du répertoire `app`. Après avoir déplacé [tous les fichiers pris en charge](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) dans le répertoire `app`, vous pouvez supprimer leurs balises `<link>` :
...
Enfin, Next.js peut gérer vos dernières balises `<head>` avec l'[API Metadata](/docs/14/app/building-your-application/optimizing/metadata). Déplacez vos dernières informations de métadonnées dans un objet [`metadata` exporté](/docs/14/app/api-reference/functions/generate-metadata#metadata-object) :
...
Avec ces modifications, vous êtes passé d'une déclaration manuelle dans votre `index.html` à une approche basée sur les conventions de Next.js intégrée au framework ([API Metadata](/docs/14/app/building-your-application/optimizing/metadata)). Cette approche vous permet d'améliorer plus facilement votre SEO et la partageabilité web de vos pages.

### Étape 6 : Styles

Comme Create React App, Next.js prend en charge nativement les [CSS Modules](/docs/14/app/building-your-application/styling/css-modules).
...
Ce répertoire est ce qu'on appelle un [segment de route attrape-tout optionnel](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments). Next.js utilise un routeur basé sur le système de fichiers où [les répertoires définissent les routes](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). Ce répertoire spécial garantira que toutes les routes de votre application seront dirigées vers son fichier `page.tsx`.
...
Ce fichier est un [composant serveur](/docs/14/app/building-your-application/rendering/server-components). Lorsque vous exécutez `next build`, le fichier est prérendu en un asset statique. Il ne nécessite _aucun_ code dynamique.

Ce fichier importe notre CSS global et indique à [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) que nous allons uniquement générer une route, la route index à `/`.
...
Ce fichier est un [composant client](/docs/14/app/building-your-application/rendering/client-components), défini par la directive `'use client'`. Les composants clients sont toujours [prérendus en HTML](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered) côté serveur avant d'être envoyés au client.
...
Avec Next.js, les imports d'images statiques renvoient un objet. Cet objet peut ensuite être utilisé directement avec le [composant `<Image>` de Next.js](/docs/14/app/api-reference/components/image), ou vous pouvez utiliser la propriété `src` de l'objet avec votre balise `<img>` existante.

Le composant `<Image>` offre des avantages supplémentaires comme [l'optimisation automatique des images](/docs/14/app/building-your-application/optimizing/images). Le composant `<Image>` définit automatiquement les attributs `width` et `height` de la balise `<img>` résultante en fonction des dimensions de l'image. Cela évite les décalages de mise en page lors du chargement de l'image. Cependant, cela peut causer des problèmes si votre application contient des images dont une seule dimension est stylée sans que l'autre ne soit définie sur `auto`. Lorsqu'elle n'est pas stylée sur `auto`, la dimension prendra par défaut la valeur de l'attribut de dimension de la balise `<img>`, ce qui peut entraîner une distorsion de l'image.

Conserver la balise `<img>` réduira le nombre de modifications dans votre application et évitera les problèmes mentionnés ci-dessus. Vous pourrez ensuite éventuellement migrer vers le composant `<Image>` pour profiter de l'optimisation des images en [configurant un loader](/docs/14/app/building-your-application/optimizing/images#loaders), ou en passant au serveur Next.js par défaut qui offre une optimisation automatique des images.
...
Next.js prend en charge les [variables d'environnement](/docs/14/app/building-your-application/configuring/environment-variables) `.env` de manière similaire à CRA.
...
Lors de la migration de votre application CRA vers Next.js, vous pourriez avoir une configuration webpack personnalisée que vous souhaitez migrer. Next.js prend en charge la fourniture d'une [configuration webpack personnalisée](/docs/14/app/api-reference/next-config-js/webpack).

De plus, Next.js prend en charge [Turbopack](/docs/14/app/api-reference/next-config-js/turbo) via `next dev --turbo` pour améliorer les performances de développement local. Turbopack prend également en charge certains [loaders webpack](/docs/14/app/api-reference/next-config-js/turbo) pour la compatibilité et l'adoption progressive.
...
- Migrer de React Router vers le [Next.js App Router](/docs/14/app/building-your-application/routing) pour obtenir :
  - Le découpage de code automatique
  - Le [rendu côté serveur en streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - Les [composants serveur React](/docs/14/app/building-your-application/rendering/server-components)
- [Optimiser les images avec le composant `<Image>`](/docs/14/app/building-your-application/optimizing/images)
- [Optimiser les polices avec `next/font`](/docs/14/app/building-your-application/optimizing/fonts)
- [Optimiser les scripts tiers avec le composant `<Script>`](/docs/14/app/building-your-application/optimizing/scripts)
- [Mettre à jour votre configuration ESLint pour prendre en charge les règles Next.js](/docs/14/app/building-your-application/configuring/eslint)
