...
Le hook `useRouter` permet de changer programmatiquement les routes dans les [composants clients](/docs/14/app/building-your-application/rendering/client-components).

> **Recommandation** : Utilisez le [composant `<Link>`](/docs/14/app/building-your-application/routing/linking-and-navigating#link-component) pour la navigation, sauf si vous avez un besoin spécifique nécessitant `useRouter`.
...
- `router.prefetch(href: string)` : [Précharge](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) la route fournie pour des transitions côté client plus rapides.
...
- La chaîne `pathname` a été supprimée et remplacée par [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname).
- L'objet `query` a été supprimé et remplacé par [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params).
- `router.events` a été remplacé. [Voir ci-dessous.](#router-events)

[Consultez le guide complet de migration](/docs/14/app/building-your-application/upgrading/app-router-migration).
...
> **Bon à savoir** : `<NavigationEvents>` est encapsulé dans une limite [`Suspense`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#example) car [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) provoque un rendu côté client jusqu'à la limite `Suspense` la plus proche pendant le [rendu statique](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). [En savoir plus](/docs/14/app/api-reference/functions/use-search-params#behavior).
