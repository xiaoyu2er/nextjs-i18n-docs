...
Le fichier `default.js` est utilisé pour afficher un contenu de repli dans les [routes parallèles (Parallel Routes)](/docs/14/app/building-your-application/routing/parallel-routes) lorsque Next.js ne peut pas récupérer l'état actif d'un [emplacement (slot)](/docs/14/app/building-your-application/routing/parallel-routes#slots) après un chargement complet de page.

Lors d'une [navigation douce (soft navigation)](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation), Next.js garde trace de l'état actif (_state_) pour chaque emplacement. Cependant, pour les navigations complètes (hard navigation), Next.js ne peut pas récupérer cet état. Dans ce cas, un fichier `default.js` peut être rendu pour les sous-pages qui ne correspondent pas à l'URL actuelle.
...
Un objet contenant les [paramètres de route dynamique](/docs/14/app/building-your-application/routing/dynamic-routes) depuis le segment racine jusqu'aux sous-pages de l'emplacement. Par exemple :
