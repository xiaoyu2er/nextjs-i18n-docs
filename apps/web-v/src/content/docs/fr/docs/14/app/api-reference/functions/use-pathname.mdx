...
`usePathname` nécessite intentionnellement l'utilisation d'un [Composant Client](/docs/14/app/building-your-application/rendering/client-components). Il est important de noter que les Composants Client ne sont pas une désoptimisation. Ils font partie intégrante de l'architecture des [Composants Serveur](/docs/14/app/building-your-application/rendering/server-components).
...
> **Bon à savoir** :
>
> - La lecture de l'URL actuelle depuis un [Composant Serveur](/docs/14/app/building-your-application/rendering/server-components) n'est pas prise en charge. Cette conception est intentionnelle pour préserver l'état du layout lors des navigations entre pages.
> - Mode de compatibilité :
>   - `usePathname` peut retourner `null` lorsqu'une [route de fallback](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) est en cours de rendu ou lorsqu'une page du répertoire `pages` a été [optimisée statiquement automatiquement](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) par Next.js et que le routeur n'est pas prêt.
