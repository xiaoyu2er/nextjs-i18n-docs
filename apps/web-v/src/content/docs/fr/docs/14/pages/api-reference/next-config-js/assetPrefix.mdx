...
translation-updated-at: 2025-06-06T16:53:41.763Z
title: assetPrefix
...
> **Attention** : [Déployer sur Vercel](/docs/14/app/building-your-application/deploying) configure automatiquement un CDN global pour votre projet Next.js.
...
> **Attention** : [Déployer sur Vercel](/docs/14/pages/building-your-application/deploying) configure automatiquement un CDN global pour votre projet Next.js.
> Vous n'avez pas besoin de configurer manuellement un préfixe d'asset (Asset Prefix).

</PagesOnly>

> **Bon à savoir** : Next.js 9.5+ a ajouté la prise en charge d'un [chemin de base (Base Path)](/docs/14/app/api-reference/next-config-js/basePath) personnalisable, qui est plus adapté pour héberger votre application sur un sous-chemin comme `/docs`.
...
Ouvrez `next.config.mjs` et ajoutez la configuration `assetPrefix` en fonction de la [phase](/docs/14/app/api-reference/next-config-js#async-configuration) :
...
- Les fichiers dans le dossier [public](/docs/14/app/building-your-application/optimizing/static-assets) ; si vous souhaitez servir ces assets via un CDN, vous devrez introduire le préfixe vous-même

</AppOnly>

<PagesOnly>

- Les fichiers dans le dossier [public](/docs/14/pages/building-your-application/optimizing/static-assets) ; si vous souhaitez servir ces assets via un CDN, vous devrez introduire le préfixe vous-même
...
- Les requêtes `/_next/data/` pour les pages `getStaticProps`. Ces requêtes seront toujours faites contre le domaine principal pour prendre en charge la [Génération Statique Incrémentielle (Incremental Static Generation)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration), même si vous ne l'utilisez pas (pour la cohérence).
