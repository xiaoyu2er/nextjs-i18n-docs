...
translation-updated-at: 2025-06-06T16:54:33.924Z
title: Module Font
nav_title: Font
description: Référence API pour le module Font
...
Cette référence API vous aidera à comprendre comment utiliser [`next/font/google`](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) et [`next/font/local`](/docs/14/app/building-your-application/optimizing/fonts#local-fonts). Pour les fonctionnalités et l'utilisation, veuillez consulter la page [Optimisation des polices](/docs/14/app/building-your-application/optimizing/fonts).

### Arguments de la fonction de police

Pour l'utilisation, consultez [Google Fonts](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) et [Polices locales](/docs/14/app/building-your-application/optimizing/fonts#local-fonts).

| Clé                                         | `font/google`       | `font/local`        | Type                       | Requis            |
| ------------------------------------------- | ------------------- | ------------------- | -------------------------- | ----------------- |
| [`src`](#src)                               | <Cross size={18} /> | <Check size={18} /> | Chaîne ou tableau d'objets | Oui               |
| [`weight`](#weight)                         | <Check size={18} /> | <Check size={18} /> | Chaîne ou tableau          | Requis/Optionnel  |
| [`style`](#style)                           | <Check size={18} /> | <Check size={18} /> | Chaîne ou tableau          | -                 |
| [`subsets`](#subsets)                       | <Check size={18} /> | <Cross size={18} /> | Tableau de chaînes          | -                 |
| [`axes`](#axes)                             | <Check size={18} /> | <Cross size={18} /> | Tableau de chaînes          | -                 |
| [`display`](#display)                       | <Check size={18} /> | <Check size={18} /> | Chaîne                     | -                 |
| [`preload`](#preload)                       | <Check size={18} /> | <Check size={18} /> | Booléen                    | -                 |
| [`fallback`](#fallback)                     | <Check size={18} /> | <Check size={18} /> | Tableau de chaînes          | -                 |
| [`adjustFontFallback`](#adjustfontfallback) | <Check size={18} /> | <Check size={18} /> | Booléen ou chaîne          | -                 |
| [`variable`](#variable)                     | <Check size={18} /> | <Check size={18} /> | Chaîne                     | -                 |
| [`declarations`](#declarations)             | <Cross size={18} /> | <Check size={18} /> | Tableau d'objets           | -                 |
...
- Un tableau de valeurs de poids si la police n'est pas une [police Google variable](https://fonts.google.com/variablefonts). S'applique uniquement à `next/font/google`.
...
- Une chaîne de [valeur](https://developer.mozilla.org/docs/Web/CSS/font-style#values) avec la valeur par défaut de `'normal'`
- Un tableau de valeurs de style si la police n'est pas une [police Google variable](https://fonts.google.com/variablefonts). S'applique uniquement à `next/font/google`.
...
- `axes: ['slnt']` : Un tableau avec la valeur `slnt` pour la police variable `Inter` qui a `slnt` comme `axes` supplémentaire comme indiqué [ici](https://fonts.google.com/variablefonts?vfquery=inter#font-families). Vous pouvez trouver les valeurs possibles de `axes` pour votre police en utilisant le filtre sur la [page des polices variables de Google](https://fonts.google.com/variablefonts#font-families) et en recherchant les axes autres que `wght`

### `display`

Le [`display`](https://developer.mozilla.org/docs/Web/CSS/@font-face/font-display) de la police avec les valeurs de chaîne possibles de `'auto'`, `'block'`, `'swap'`, `'fallback'` ou `'optional'` avec la valeur par défaut de `'swap'`.
...
Une valeur booléenne qui spécifie si la police doit être [préchargée](/docs/14/app/building-your-application/optimizing/fonts#preloading) ou non. La valeur par défaut est `true`.
...
- Pour `next/font/local` : Une valeur de chaîne ou booléenne `false` qui définit si une police de secours automatique doit être utilisée pour réduire le [Cumulative Layout Shift](https://web.dev/cls/). Les valeurs possibles sont `'Arial'`, `'Times New Roman'` ou `false`. La valeur par défaut est `'Arial'`.
