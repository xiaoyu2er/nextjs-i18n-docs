...
translation-updated-at: 2025-06-06T16:55:29.984Z
title: Codemods
...
Ce codemod transfère les imports de `next/server` vers `next/og` pour l'utilisation de la [Génération d'images OG dynamiques](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation).
...
Supprime les balises `<a>` à l'intérieur des [composants Link](/docs/14/app/api-reference/components/link), ou ajoute une prop `legacyBehavior` aux Links qui ne peuvent pas être auto-corrigés.

</AppOnly>

<PagesOnly>

Supprime les balises `<a>` à l'intérieur des [composants Link](/docs/14/pages/api-reference/components/link), ou ajoute une prop `legacyBehavior` aux Links qui ne peuvent pas être auto-corrigés.
