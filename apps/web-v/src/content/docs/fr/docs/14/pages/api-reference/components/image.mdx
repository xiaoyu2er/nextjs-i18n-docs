...
translation-updated-at: 2025-06-06T16:54:15.893Z
...
> **Bon à savoir** : Si vous utilisez une version de Next.js antérieure à la 13, vous devrez consulter la documentation [next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) car le composant a été renommé.

</PagesOnly>

Cette référence API vous aidera à comprendre comment utiliser les [props](#props) et les [options de configuration](#configuration-options) disponibles pour le composant Image. Pour les fonctionnalités et l'utilisation, veuillez consulter la page [Composant Image](/docs/14/app/building-your-application/optimizing/images).
...
- Un fichier image [importé statiquement](/docs/14/app/building-your-application/optimizing/images#local-images)
...
Requis, sauf pour les images [importées statiquement](/docs/14/app/building-your-application/optimizing/images#local-images) ou les images avec la propriété [`fill`](#fill).
...
> **Bon à savoir** : L'utilisation de props comme `loader`, qui acceptent une fonction, nécessite d'utiliser des [Composants Client](/docs/14/app/building-your-application/rendering/client-components) pour sérialiser la fonction fournie.
...
Lorsque `blur`, la propriété [`blurDataURL`](#blurdataurl) sera utilisée comme espace réservé. Si `src` est un objet provenant d'une [importation statique](/docs/14/app/building-your-application/optimizing/images#local-images) et que l'image importée est `.jpg`, `.png`, `.webp`, ou `.avif`, alors `blurDataURL` sera automatiquement remplie, sauf si l'image est détectée comme animée.
...
> **Bon à savoir** : L'utilisation de props comme `onLoadingComplete`, qui acceptent une fonction, nécessite d'utiliser des [Composants Client](/docs/14/app/building-your-application/rendering/client-components) pour sérialiser la fonction fournie.
...
> **Bon à savoir** : L'utilisation de props comme `onLoad`, qui acceptent une fonction, nécessite d'utiliser des [Composants Client](/docs/14/app/building-your-application/rendering/client-components) pour sérialiser la fonction fournie.
...
> **Bon à savoir** : L'utilisation de props comme `onError`, qui acceptent une fonction, nécessite d'utiliser des [Composants Client](/docs/14/app/building-your-application/rendering/client-components) pour sérialiser la fonction fournie.
...
- [Configuration personnalisée du chargeur d'images](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **Bon à savoir** : Personnaliser le fichier du chargeur d'images, qui accepte une fonction, nécessite d'utiliser des [Composants Client](/docs/14/app/building-your-application/rendering/client-components) pour sérialiser la fonction fournie.
...
Vous pouvez configurer le Time to Live (TTL) en secondes pour les images optimisées mises en cache. Dans de nombreux cas, il est préférable d'utiliser une [Importation d'Image Statique](/docs/14/app/building-your-application/optimizing/images#local-images) qui hachera automatiquement le contenu du fichier et mettra l'image en cache indéfiniment avec un en-tête `Cache-Control` de `immutable`.
...
Si vous devez modifier le comportement de mise en cache par image, vous pouvez configurer [`headers`](/docs/14/app/api-reference/next-config-js/headers) pour définir l'en-tête `Cache-Control` sur l'image en amont (par exemple `/some-asset.jpg`, et non `/_next/image` elle-même).
...
| `v13.0.0`  | L'import `next/image` a été renommé en `next/legacy/image`. L'import `next/future/image` a été renommé en `next/image`. Un [codemod est disponible](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) pour renommer automatiquement et en toute sécurité vos imports. Suppression du wrapper `<span>`. Suppression des propriétés `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot`. `alt` est désormais obligatoire. `onLoadingComplete` reçoit une référence à l'élément `img`. Suppression de la configuration du loader intégré. |
