...
translation-updated-at: 2025-06-06T16:57:06.203Z
title: ESLint
...
> Si vous ne souhaitez pas qu'ESLint s'exécute pendant `next build`, consultez la documentation pour [Ignorer ESLint](/docs/14/app/api-reference/next-config-js/eslint).

</AppOnly>

<PagesOnly>

> Si vous ne souhaitez pas qu'ESLint s'exécute pendant `next build`, consultez la documentation pour [Ignorer ESLint](/docs/14/pages/api-reference/next-config-js/eslint).
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | Empêche les fautes de frappe courantes dans les [fonctions de récupération de données de Next.js](/docs/14/pages/building-your-application/data-fetching) |
...
Pour améliorer les performances, les informations des fichiers traités par ESLint sont mises en cache par défaut. Ce cache est stocké dans `.next/cache` ou dans votre [répertoire de build](/docs/14/app/api-reference/next-config-js/distDir) défini. Si vous incluez des règles ESLint qui dépendent de plus que le contenu d'un seul fichier source et que vous devez désactiver le cache, utilisez le flag `--no-cache` avec `next lint`.

</AppOnly>

<PagesOnly>

Pour améliorer les performances, les informations des fichiers traités par ESLint sont mises en cache par défaut. Ce cache est stocké dans `.next/cache` ou dans votre [répertoire de build](/docs/14/pages/api-reference/next-config-js/distDir) défini. Si vous incluez des règles ESLint qui dépendent de plus que le contenu d'un seul fichier source et que vous devez désactiver le cache, utilisez le flag `--no-cache` avec `next lint`.
...
> Le point d'entrée `next/core-web-vitals` est automatiquement inclus pour les nouvelles applications créées avec [Create Next App](/docs/14/app/api-reference/cli/create-next-app).
...
- Vous avez déjà installé un ou plusieurs des plugins suivants (soit séparément, soit via une configuration différente comme `airbnb` ou `react-app`) :
  ...
  - `jsx-a11y`
  - `import`
- Vous avez défini des `parserOptions` spécifiques qui diffèrent de la configuration Babel dans Next.js (ce n'est pas recommandé sauf si vous avez [personnalisé votre configuration Babel](/docs/14/pages/building-your-application/configuring/babel))
