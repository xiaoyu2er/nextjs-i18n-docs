...
translation-updated-at: 2025-06-06T16:50:05.666Z
title: Fast Refresh
description: Fast Refresh est une expérience de rechargement à chaud qui fournit un retour instantané sur les modifications apportées à vos composants React.
sidebar:
  order: 4
---


Fast Refresh est une fonctionnalité de Next.js qui fournit un retour instantané sur les modifications apportées à vos composants React. Fast Refresh est activé par défaut dans toutes les applications Next.js **en version 9.4 ou ultérieure**. Avec Fast Refresh activé, la plupart des modifications devraient être visibles en moins d'une seconde, **sans perte de l'état du composant**.
...
- Si vous modifiez un fichier avec des exports qui ne sont _pas_ des composants React, Fast Refresh relancera à la fois ce fichier et les autres fichiers qui l'importent. Par exemple, si `Button.js` et `Modal.js` importent `theme.js`, la modification de `theme.js` mettra à jour les deux composants.
...
- L'état local n'est pas préservé pour les composants classiques (seuls les composants fonction et les Hooks préservent l'état).
...
- Les fonctions fléchées anonymes comme `export default () => <div />;` empêchent Fast Refresh de préserver l'état local du composant. Pour les grandes bases de code, vous pouvez utiliser notre [codemod `name-default-component`](/docs/14/pages/building-your-application/upgrading/codemods#name-default-component).
...
Par exemple, lorsque vous modifiez `useMemo(() => x * 2, [x])` en `useMemo(() => x * 10, [x])`, il sera relancé même si `x` (la dépendance) n'a pas changé. Si React ne faisait pas cela, votre modification ne serait pas reflétée à l'écran !

Parfois, cela peut conduire à des résultats inattendus. Par exemple, même un `useEffect` avec un tableau de dépendances vide serait relancé une fois pendant Fast Refresh.

Cependant, écrire du code résistant aux relances occasionnelles de `useEffect` est une bonne pratique, même sans Fast Refresh. Cela facilitera l'ajout de nouvelles dépendances plus tard et est imposé par le [Mode Strict React](/docs/14/pages/api-reference/next-config-js/reactStrictMode), que nous recommandons fortement d'activer.