...
translation-updated-at: 2025-06-06T17:01:23.362Z
title: CLI next
...
| [`build`](#next-build-options)        | Crée une version optimisée pour la production de votre application. Affiche des informations sur chaque route.                                                                                                                                |
...
| `[directory]`                            | Un répertoire dans lequel construire l'application. Si non fourni, le répertoire courant est utilisé.                                                           |
| `--turbo`                                | Démarre le mode développement en utilisant [Turbopack](https://nextjs.org/docs/architecture/turbopack).                                                           |
...
| `--experimental-https-key <path>`        | Chemin vers un fichier de clé HTTPS.                                                                                                                            |
| `--experimental-https-cert <path>`       | Chemin vers un fichier de certificat HTTPS.                                                                                                                    |
| `--experimental-https-ca <path>`         | Chemin vers un fichier d'autorité de certification HTTPS.                                                                                                          |
...
`next build` crée une version optimisée pour la production de votre application. La sortie affiche des informations sur chaque route. Par exemple :
...
Ces deux valeurs sont [**compressées avec gzip**](/docs/14/app/api-reference/next-config-js/compress). Le premier chargement est indiqué par du vert, jaune ou rouge. Visez le vert pour des applications performantes.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `-h, --help`                       | Affiche toutes les options disponibles.                                                                                                                   |
| `[directory]`                      | Un répertoire sur lequel construire l'application. Si non fourni, le répertoire courant sera utilisé.                                           |
| `-d` ou `--debug`                  | Active une sortie de construction plus verbeuse. Avec ce flag activé, des informations supplémentaires comme les réécritures, redirections et en-têtes seront affichées.      |
|                                    |
...
| `--no-mangling`                    | Désactive le [mangling](https://fr.wikipedia.org/wiki/Name_mangling). Cela peut affecter les performances et ne devrait être utilisé qu'à des fins de débogage. |
...
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `-h` ou `--help`                        | Affiche toutes les options disponibles.                                                                                     |
| `[directory]`                           | Un répertoire sur lequel démarrer l'application. Si aucun répertoire n'est fourni, le répertoire courant sera utilisé. |
| `-p` ou `--port <port>`                 | Spécifie un numéro de port sur lequel démarrer l'application. (par défaut : 3000, env : PORT)                             |
...
| Option                                                | Description                                                                                                   |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `[directory]`                                         | Un répertoire de base sur lequel linter l'application. Si non fourni, le répertoire courant sera utilisé.       |
...
| `--error-on-unmatched-pattern`                        | Signale des erreurs lorsque des motifs de fichiers ne correspondent à rien.                                                          |
| `-h, --help`                                          | Affiche ce message.                                                                                        |

### Options de `next telemetry`

Next.js collecte des données de télémétrie **complètement anonymes** sur l'utilisation générale. La participation à ce programme anonyme est optionnelle, et vous pouvez vous désinscrire si vous préférez ne pas partager d'informations.
...
En savoir plus sur la [Télémétrie](/telemetry).
...
### Utilisation de HTTPS pendant le développement

Pour certains cas d'usage comme les webhooks ou l'authentification, il peut être nécessaire d'utiliser HTTPS pour avoir un environnement sécurisé sur `localhost`. Next.js peut générer un certificat auto-signé avec `next dev` en utilisant le flag `--experimental-https` :
...
`next dev --experimental-https` est uniquement destiné au développement et crée un certificat localement fiable avec [`mkcert`](https://github.com/FiloSottile/mkcert). En production, utilisez des certificats correctement émis par des autorités de confiance.

> **Bon à savoir** : Lors du déploiement sur Vercel, HTTPS est [configuré automatiquement](https://vercel.com/docs/security/encryption) pour votre application Next.js.

### Configuration d'un délai d'expiration pour les proxies en aval

Lors du déploiement de Next.js derrière un proxy en aval (par exemple un répartiteur de charge comme AWS ELB/ALB), il est important de configurer le serveur HTTP sous-jacent de Next avec des [délais d'expiration keep-alive](https://nodejs.org/api/http.html#http_server_keepalivetimeout) qui sont _supérieurs_ à ceux du proxy en aval. Sinon, une fois le délai keep-alive atteint pour une connexion TCP donnée, Node.js terminera immédiatement cette connexion sans avertir le proxy en aval. Cela entraîne une erreur de proxy chaque fois qu'il tente de réutiliser une connexion que Node.js a déjà terminée.

Pour configurer les valeurs de délai d'expiration pour le serveur Next.js en production, passez `--keepAliveTimeout` (en millisecondes) à `next start`, comme ceci :
