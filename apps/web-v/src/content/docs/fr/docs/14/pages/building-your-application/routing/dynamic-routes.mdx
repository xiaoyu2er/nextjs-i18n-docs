...
Lorsque vous ne connaissez pas à l'avance les noms exacts des segments et que vous souhaitez créer des routes à partir de données dynamiques, vous pouvez utiliser des segments dynamiques qui sont remplis au moment de la requête ou [prérendus](/docs/14/pages/building-your-application/data-fetching/get-static-paths) au moment de la construction.
...
Les segments dynamiques sont accessibles via [`useRouter`](/docs/14/pages/api-reference/functions/use-router).
