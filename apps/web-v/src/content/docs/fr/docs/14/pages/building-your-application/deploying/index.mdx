...
Puisque Next.js supporte cette [exportation statique](/docs/14/app/building-your-application/deploying/static-exports), elle peut être déployée et hébergée sur n'importe quel serveur web capable de servir des assets statiques HTML/CSS/JS. Cela inclut des outils comme AWS S3, Nginx ou Apache.

L'exécution en tant qu'[exportation statique](/docs/14/app/building-your-application/deploying/static-exports) ne supporte pas les fonctionnalités de Next.js nécessitant un serveur. [En savoir plus](/docs/14/app/building-your-application/deploying/static-exports#unsupported-features).

> **Bon à savoir :**
>
> - Les [composants serveur (Server Components)](/docs/14/app/building-your-application/rendering/server-components) sont supportés avec les exports statiques.

## Fonctionnalités

### Optimisation d'images

L'[optimisation d'images](/docs/14/app/building-your-application/optimizing/images) via `next/image` fonctionne en auto-hébergement sans configuration lors d'un déploiement avec `next start`. Si vous préférez avoir un service séparé pour optimiser les images, vous pouvez [configurer un loader d'images](/docs/14/app/building-your-application/optimizing/images#loaders).

L'optimisation d'images peut être utilisée avec une [exportation statique](/docs/14/app/building-your-application/deploying/static-exports#image-optimization) en définissant un loader d'images personnalisé dans `next.config.js`. Notez que les images sont optimisées à l'exécution, pas pendant la construction.

> **Bon à savoir :**
>
> - En auto-hébergement, envisagez d'installer `sharp` pour une [optimisation d'images](/docs/14/pages/building-your-application/optimizing/images) plus performante dans votre environnement de production en exécutant `npm install sharp` dans votre répertoire de projet. Sur les plateformes Linux, `sharp` peut nécessiter une [configuration supplémentaire](https://sharp.pixelplumbing.com/install#linux-memory-allocator) pour éviter une utilisation excessive de la mémoire.
> - En savoir plus sur le [comportement de cache des images optimisées](/docs/14/app/api-reference/components/image#caching-behavior) et comment configurer le TTL.
> - Vous pouvez aussi [désactiver l'optimisation d'images](/docs/14/app/api-reference/components/image#unoptimized) tout en conservant les autres avantages de `next/image` si vous préférez. Par exemple, si vous optimisez vous-même les images séparément.

### Middleware

Le [middleware](/docs/14/app/building-your-application/routing/middleware) fonctionne en auto-hébergement sans configuration lors d'un déploiement avec `next start`. Puisqu'il nécessite un accès à la requête entrante, il n'est pas supporté lors d'une [exportation statique](/docs/14/app/building-your-application/deploying/static-exports).

Le middleware utilise un [runtime](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes) qui est un sous-ensemble de toutes les API Node.js disponibles pour garantir une faible latence, puisqu'il peut s'exécuter devant chaque route ou asset de votre application. Ce runtime ne nécessite pas de fonctionner "à la périphérie" et fonctionne sur un serveur mono-région. Une configuration et une infrastructure supplémentaires sont nécessaires pour exécuter le middleware dans plusieurs régions.

Si vous souhaitez ajouter une logique (ou utiliser un package externe) nécessitant toutes les API Node.js, vous pourriez déplacer cette logique dans un [layout](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) en tant que [composant serveur (Server Component)](/docs/14/app/building-your-application/rendering/server-components). Par exemple, vérifier les [en-têtes](/docs/14/app/api-reference/functions/headers) et [rediriger](/docs/14/app/api-reference/functions/redirect). Vous pouvez aussi utiliser des en-têtes, cookies ou paramètres de requête pour [rediriger](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching) ou [réécrire](/docs/14/app/api-reference/next-config-js/rewrites#header-cookie-and-query-matching) via `next.config.js`. Si cela ne fonctionne pas, vous pouvez aussi utiliser un [serveur personnalisé](/docs/14/pages/building-your-application/configuring/custom-server).
...
> **Bon à savoir :**
>
> - Vous pouvez exécuter du code au démarrage du serveur en utilisant la [fonction `register`](/docs/14/app/building-your-application/optimizing/instrumentation).
> - Nous ne recommandons pas d'utiliser l'option [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration), car elle ne fonctionne pas avec le mode de sortie standalone. Nous recommandons plutôt d'[adopter progressivement](/docs/14/app/building-your-application/upgrading/app-router-migration) le routeur App.
...
Le routeur App de Next.js supporte les [réponses en streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) en auto-hébergement. Si vous utilisez Nginx ou un proxy similaire, vous devrez le configurer pour désactiver la mise en mémoire tampon afin d'activer le streaming.
