...
Si vous n'avez pas besoin de récupérer les données au moment de la requête, ou si vous préférez mettre en cache les données et le HTML pré-rendu, nous recommandons d'utiliser [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props).
...
- Quand un utilisateur visite la page via [`next/link`](/docs/14/pages/api-reference/components/link) ou [`next/router`](/docs/14/pages/api-reference/functions/use-router), Next.js envoie une requête API au serveur, qui exécute `getServerSideProps`.
- Vous n'avez pas besoin d'appeler une [Route API](/docs/14/pages/building-your-application/routing/api-routes) Next.js pour récupérer des données lorsque vous utilisez `getServerSideProps` puisque la fonction s'exécute sur le serveur. À la place, vous pouvez appeler directement un CMS, une base de données ou d'autres API tierces depuis `getServerSideProps`.

> **Bon à savoir :**
>
> - Consultez la [référence API de `getServerSideProps`](/docs/14/pages/api-reference/functions/get-server-side-props) pour les paramètres et props utilisables avec `getServerSideProps`.
...
Si une erreur est levée dans `getServerSideProps`, la page `pages/500.js` sera affichée. Consultez la documentation de la [page 500](/docs/14/pages/building-your-application/routing/custom-error#500-page) pour en savoir plus sur sa création. Pendant le développement, cette page ne sera pas utilisée et l'overlay d'erreur de développement sera affiché à la place.

## Cas particuliers

### Runtime Edge

`getServerSideProps` peut être utilisé avec les [Runtimes Serverless et Edge](/docs/14/pages/building-your-application/rendering/edge-and-nodejs-runtimes), et vous pouvez définir des props dans les deux.
...
Cependant, avant d'utiliser `cache-control`, nous recommandons de voir si [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) avec [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) est plus adapté à votre cas d'utilisation.