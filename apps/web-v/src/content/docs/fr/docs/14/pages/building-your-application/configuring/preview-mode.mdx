...
translation-updated-at: 2025-06-06T16:58:09.883Z
title: Mode Prévisualisation
description: Next.js propose un mode prévisualisation pour les pages générées statiquement. Découvrez son fonctionnement ici.
sidebar:
  order: 3010614
---


> **Remarque** : Cette fonctionnalité est remplacée par le [Mode Brouillon](/docs/14/pages/building-your-application/configuring/draft-mode).
...
Dans la [documentation sur les Pages](/docs/14/pages/building-your-application/routing/pages-and-layouts) et la [documentation sur la Récupération de Données](/docs/14/pages/building-your-application/data-fetching), nous avons expliqué comment pré-rendre une page au moment du build (**Génération Statique**) en utilisant `getStaticProps` et `getStaticPaths`.

La Génération Statique est utile lorsque vos pages récupèrent des données depuis un CMS headless. Cependant, elle n'est pas idéale lorsque vous écrivez un brouillon sur votre CMS headless et que vous souhaitez **prévisualiser** immédiatement ce brouillon sur votre page. Vous voudriez que Next.js rende ces pages au **moment de la requête** plutôt qu'au moment du build et qu'il récupère le contenu du brouillon plutôt que le contenu publié. Vous voudriez que Next.js contourne la Génération Statique uniquement pour ce cas spécifique.

Next.js propose une fonctionnalité appelée **Mode Prévisualisation** qui résout ce problème. Voici comment l'utiliser.

## Étape 1 : Créer et accéder à une route API de prévisualisation

> Consultez d'abord la [documentation sur les Routes API](/docs/14/pages/building-your-application/routing/api-routes) si vous n'êtes pas familier avec les Routes API de Next.js.
...
Dans cette route API, vous devez appeler `setPreviewData` sur l'objet de réponse. L'argument de `setPreviewData` doit être un objet, et il peut être utilisé par `getStaticProps` (nous y reviendrons plus tard). Pour l'instant, nous utiliserons `{}`.
...
`res.setPreviewData` définit des **cookies** sur le navigateur qui activent le mode prévisualisation. Toutes les requêtes vers Next.js contenant ces cookies seront considérées comme étant en **mode prévisualisation**, et le comportement pour les pages générées statiquement changera (nous y reviendrons plus tard).
...
En pratique, vous voudriez appeler cette route API de manière _sécurisée_ depuis votre CMS headless. Les étapes spécifiques varieront selon le CMS headless que vous utilisez, mais voici quelques étapes communes que vous pourriez suivre.
...
Si vous demandez une page qui a `getStaticProps` avec les cookies de mode prévisualisation définis (via `res.setPreviewData`), alors `getStaticProps` sera appelé au **moment de la requête** (au lieu du moment du build).
...
Nous avons utilisé `res.setPreviewData({})` dans la route API de prévisualisation, donc `context.previewData` sera `{}`. Vous pouvez utiliser cela pour passer des informations de session depuis la route API de prévisualisation vers `getStaticProps` si nécessaire.
...
Par exemple, votre CMS headless pourrait avoir un point de terminaison API différent pour les brouillons de posts. Si c'est le cas, vous pouvez utiliser `context.preview` pour modifier l'URL du point de terminaison API comme ci-dessous :

```js
export async function getStaticProps(context) {
  // Si context.preview est true, ajoutez "/preview" au point de terminaison API
  // pour demander des données de brouillon au lieu de données publiées. Cela variera
...
> **Bon à savoir** : pendant le rendu, `next/router` expose un indicateur `isPreview`, consultez la [documentation sur l'objet router](/docs/14/pages/api-reference/functions/use-router#router-object) pour plus d'informations.
...
Ensuite, envoyez une requête à `/api/clear-preview-mode-cookies` pour invoquer la Route API. Si vous appelez cette route en utilisant [`next/link`](/docs/14/pages/api-reference/components/link), vous devez passer `prefetch={false}` pour éviter d'appeler `clearPreviewData` pendant le préchargement des liens.
...
Vous pouvez passer un objet à `setPreviewData` et le rendre disponible dans `getStaticProps`. Cependant, comme les données seront stockées dans un cookie, il y a une limitation de taille. Actuellement, les données de prévisualisation sont limitées à 2 Ko.
...
> **Bon à savoir** : Vous ne devriez pas définir l'en-tête `Cache-Control` lors de l'utilisation du Mode Prévisualisation car il ne peut pas être contourné. Nous recommandons plutôt d'utiliser [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
