...
La fonction `headers` permet de lire les en-têtes de requête HTTP entrants depuis un [Composant Serveur](/docs/14/app/building-your-application/rendering/server-components).
...
> **Bon à savoir** :
>
> - `headers()` est une **[Fonction Dynamique](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)** dont les valeurs retournées ne peuvent pas être connues à l'avance. Son utilisation dans un layout ou une page activera le **[rendu dynamique](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)** au moment de la requête.
...
`headers()` peut être utilisé en combinaison avec [Suspense pour la récupération de données](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating).
