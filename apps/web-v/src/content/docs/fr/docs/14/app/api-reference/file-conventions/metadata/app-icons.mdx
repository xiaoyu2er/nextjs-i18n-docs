...
translation-updated-at: 2025-06-06T17:48:21.068Z
...
- [Utilisation de fichiers image (.ico, .jpg, .png)](#fichiers-image-ico-jpg-png)
- [Utilisation de code pour générer une icône (.js, .ts, .tsx)](#générer-des-icônes-avec-du-code-js-ts-tsx)
...
En plus d'utiliser [des fichiers image littéraux](#fichiers-image-ico-jpg-png), vous pouvez **générer** programmatiquement des icônes avec du code.
...
La façon la plus simple de générer une icône est d'utiliser l'API [`ImageResponse`](/docs/14/app/api-reference/functions/image-response) de `next/og`.
...
> **Bon à savoir**
>
> - Par défaut, les icônes générées sont [**optimisées statiquement**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (générées au moment de la construction et mises en cache) sauf si elles utilisent des [fonctions dynamiques](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) ou des données non mises en cache.
> - Vous pouvez générer plusieurs icônes dans le même fichier en utilisant [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata).
...
Un objet contenant les [paramètres de route dynamique](/docs/14/app/building-your-application/routing/dynamic-routes) depuis le segment racine jusqu'au segment où `icon` ou `apple-icon` est colocalisé.
...
| [`contentType`](#contenttype) | `string` - [type MIME de l'image](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/MIME_types#image_types) |
...
`icon` et `apple-icon` sont des [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) spécialisés qui peuvent utiliser les mêmes options de [configuration de segment de route](/docs/14/app/api-reference/file-conventions/route-segment-config) que les Pages et Layouts.

| Option                                                                                             | Type                                                     | Default    |
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | ---------- |
| [`dynamic`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamic)                 | `'auto' \| 'force-dynamic' \| 'error' \| 'force-static'` | `'auto'`   |
| [`revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate)           | `false \| 'force-cache' \| 0 \| number`                  | `false`    |
| [`runtime`](/docs/14/app/api-reference/file-conventions/route-segment-config#runtime)                 | `'nodejs' \| 'edge'`                                     | `'nodejs'` |
| [`preferredRegion`](/docs/14/app/api-reference/file-conventions/route-segment-config#preferredregion) | `'auto' \| 'global' \| 'home' \| string \| string[]`     | `'auto'`   |
...
| Version   | Changements                                      |
| --------- | ------------------------------------------------ |
| `v13.3.0` | Introduction de `favicon`, `icon` et `apple-icon` |