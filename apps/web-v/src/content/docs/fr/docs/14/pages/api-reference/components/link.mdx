...
translation-updated-at: 2025-06-06T16:54:07.717Z
title: <Link>
...
`<Link>` est un composant React qui étend l'élément HTML `<a>` pour fournir un [préchargement](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) et une navigation côté client entre les routes. C'est le moyen principal de naviguer entre les routes dans Next.js.
...
> **Bon à savoir** :
>
> - Next.js fera défiler vers la [Page](/docs/14/app/building-your-application/routing/pages-and-layouts#pages) si elle n'est pas visible dans la fenêtre d'affichage lors de la navigation.
...
Une balise `<a>` n'est plus requise comme enfant de `<Link>`. Ajoutez la prop `legacyBehavior` pour utiliser le comportement hérité ou supprimez la balise `<a>` pour mettre à niveau. Un [codemod est disponible](/docs/14/app/building-your-application/upgrading/codemods#new-link) pour mettre à niveau automatiquement votre code.
...
- Une [route dynamique](/docs/14/app/building-your-application/routing/dynamic-routes) : `/blog/my-post`
...
Il est courant d'utiliser [Middleware](/docs/14/app/building-your-application/routing/middleware) pour l'authentification ou d'autres fins impliquant la réécriture de l'utilisateur vers une page différente. Pour que le composant `<Link />` précharge correctement les liens avec des réécritures via Middleware, vous devez indiquer à Next.js à la fois l'URL à afficher et l'URL à précharger. Ceci est nécessaire pour éviter des requêtes inutiles vers le middleware pour connaître la route correcte à précharger.
...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `v13.0.0` | Ne nécessite plus une balise enfant `<a>`. Un [codemod](/docs/14/app/building-your-application/upgrading/codemods#remove-a-tags-from-link-components) est fourni pour mettre à jour automatiquement votre base de code. |
