...
translation-updated-at: 2025-06-06T16:57:11.083Z
title: AMP
...
Pour activer le support AMP pour une page et en savoir plus sur les différentes configurations AMP, consultez la [documentation de l'API `next/amp`](/docs/14/pages/building-your-application/configuring/amp).

## Limitations

- Seul le CSS-in-JS est pris en charge. Les [CSS Modules](/docs/14/pages/building-your-application/styling) ne sont pas supportés par les pages AMP pour le moment. Vous pouvez [contribuer au support des CSS Modules dans Next.js](https://github.com/vercel/next.js/issues/10549).
...
Les pages sont également validées lors de l'[export HTML statique](/docs/14/pages/building-your-application/deploying/static-exports) et tous les avertissements/erreurs seront affichés dans le terminal. Toute erreur AMP entraînera la sortie de l'export avec le code d'état `1` car l'export n'est pas un AMP valide.
...
Lors de l'utilisation de l'[export HTML statique](/docs/14/pages/building-your-application/deploying/static-exports) pour pré-rendre statiquement les pages, Next.js détectera si la page prend en charge AMP et modifiera le comportement d'export en conséquence.
...
Lorsque [`trailingSlash`](/docs/14/pages/api-reference/next-config-js/trailingSlash) est activé, les pages exportées pour `pages/about.js` seraient :
...
Comme solution de contournement, vous pouvez créer manuellement un fichier appelé `amp.d.ts` dans votre projet et y ajouter ces [types personnalisés](https://stackoverflow.com/a/50601125).