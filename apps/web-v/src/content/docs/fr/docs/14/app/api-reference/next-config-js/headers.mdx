...
translation-updated-at: 2025-06-06T17:04:23.118Z
title: headers
...
Lorsque vous utilisez le [support de `basePath`](/docs/14/app/api-reference/next-config-js/basePath) avec des en-têtes, chaque `source` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à l'en-tête :
...
Lorsque vous utilisez le [support `i18n`](/docs/14/app/building-your-application/routing/internationalization) avec des en-têtes, chaque `source` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à l'en-tête. Si `locale: false` est utilisé, vous devez préfixer la `source` avec une locale pour qu'elle corresponde correctement.

</AppOnly>

<PagesOnly>

Lorsque vous utilisez le [support `i18n`](/docs/14/pages/building-your-application/routing/internationalization) avec des en-têtes, chaque `source` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à l'en-tête. Si `locale: false` est utilisé, vous devez préfixer la `source` avec une locale pour qu'elle corresponde correctement.
...
Apprenez-en plus sur la [mise en cache](/docs/14/app/building-your-application/caching) avec le routeur App.

</AppOnly>

<PagesOnly>

Si vous avez besoin de revalider le cache d'une page qui a été [générée statiquement](/docs/14/pages/building-your-application/rendering/static-site-generation), vous pouvez le faire en définissant la prop `revalidate` dans la fonction [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) de la page.

Vous pouvez définir l'en-tête `Cache-Control` dans vos [routes API](/docs/14/pages/building-your-application/routing/api-routes) en utilisant la méthode `res.setHeader` :
...
**Cet en-tête a été remplacé par l'option `frame-ancestors` de CSP**, qui a un meilleur support dans les navigateurs modernes (voir [Content Security Policy](/docs/14/app/building-your-application/configuring/content-security-policy) pour les détails de configuration).
...
[Cet en-tête](https://developer.mozilla.org/docs/Web/HTTP/Headers/X-Content-Type-Options) empêche le navigateur de tenter de deviner le type de contenu si l'en-tête `Content-Type` n'est pas explicitement défini. Cela peut prévenir les exploits XSS pour les sites qui permettent aux utilisateurs de télécharger et partager des fichiers.
...
Apprenez-en plus sur l'ajout d'une [Content Security Policy](/docs/14/app/building-your-application/configuring/content-security-policy) à votre application.
