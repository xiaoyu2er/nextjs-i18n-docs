...
translation-updated-at: 2025-06-06T17:05:15.547Z
title: template.js
...
Un fichier **template** (modèle) est similaire à un [layout](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) en ce qu'il englobe chaque layout enfant ou page. Contrairement aux layouts qui persistent entre les routes et maintiennent leur état, les templates créent une nouvelle instance pour chacun de leurs enfants lors de la navigation.
...
Les composants Template doivent accepter et utiliser une prop `children`. Le `template` est rendu entre un [layout](/docs/14/app/api-reference/file-conventions/layout) et ses enfants. Par exemple :
...
> **Bon à savoir** :
>
> - Par défaut, `template` est un [Composant Serveur](/docs/14/app/building-your-application/rendering/server-components), mais peut aussi être utilisé comme [Composant Client](/docs/14/app/building-your-application/rendering/client-components) via la directive `"use client"`.
