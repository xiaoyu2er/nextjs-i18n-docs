...
- L'utilisation de `unstable_noStore` à l'intérieur de [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache) ne désactivera pas la génération statique. À la place, il se basera sur la configuration du cache pour déterminer s'il faut mettre en cache le résultat ou non.
