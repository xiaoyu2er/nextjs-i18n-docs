...
1. Utiliser le hook `useEffect()` de React dans vos pages au lieu des méthodes de rendu côté serveur ([`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) et [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)).
...
> **Bon à savoir** :
>
> Gardez à l'esprit que le CSR peut impacter le SEO. Certains robots de moteurs de recherche pourraient ne pas exécuter JavaScript et ne verraient donc que l'état initial vide ou de chargement de votre application. Cela peut aussi causer des problèmes de performance pour les utilisateurs avec des connexions internet lentes ou des appareils peu puissants, car ils doivent attendre que tout le JavaScript soit chargé et exécuté avant de voir la page complète. Next.js promeut une approche hybride qui vous permet d'utiliser une combinaison de [rendu côté serveur](/docs/14/pages/building-your-application/rendering/server-side-rendering), [génération de site statique](/docs/14/pages/building-your-application/rendering/static-site-generation) et rendu côté client, **selon les besoins de chaque page** de votre application. Dans le routeur App, vous pouvez aussi utiliser [l'interface de chargement avec Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) pour afficher un indicateur pendant le rendu de la page.