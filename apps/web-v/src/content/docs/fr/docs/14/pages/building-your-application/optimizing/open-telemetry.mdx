...
À mesure que les applications deviennent plus complexes, il devient de plus en plus difficile d'identifier et de diagnostiquer les problèmes qui peuvent survenir. En utilisant des outils d'observabilité tels que les logs et les métriques, les développeurs peuvent obtenir des insights sur le comportement de leur application et identifier des axes d'optimisation. Avec l'observabilité, les développeurs peuvent résoudre les problèmes de manière proactive avant qu'ils ne deviennent majeurs et offrir une meilleure expérience utilisateur. Par conséquent, il est fortement recommandé d'utiliser l'observabilité dans vos applications Next.js pour améliorer les performances, optimiser les ressources et enrichir l'expérience utilisateur.
...
Consultez la [documentation officielle d'OpenTelemetry](https://opentelemetry.io/docs/) pour plus d'informations sur OpenTelemetry et son fonctionnement.
...
Lorsque vous activez OpenTelemetry, nous encapsulons automatiquement tout votre code comme `getStaticProps` dans des _spans_ avec des attributs utiles.
...
Ensuite, créez un fichier personnalisé [`instrumentation.ts`](/docs/14/app/building-your-application/optimizing/instrumentation) (ou `.js`) dans le **répertoire racine** du projet (ou dans le dossier `src` si vous en utilisez un) :

</AppOnly>

<PagesOnly>

Ensuite, créez un fichier personnalisé [`instrumentation.ts`](/docs/14/pages/building-your-application/optimizing/instrumentation) (ou `.js`) dans le **répertoire racine** du projet (ou dans le dossier `src` si vous en utilisez un) :
...
> - Si vous utilisez l'option de configuration [`pageExtensions`](/docs/14/app/api-reference/next-config-js/pageExtensions) pour ajouter un suffixe, vous devrez également mettre à jour le nom du fichier `instrumentation` pour correspondre.
...
>
> - Le fichier `instrumentation` doit être à la racine de votre projet et non dans les répertoires `app` ou `pages`. Si vous utilisez le dossier `src`, placez le fichier dans `src` à côté de `pages` et `app`.
> - Si vous utilisez l'option de configuration [`pageExtensions`](/docs/14/pages/api-reference/next-config-js/pageExtensions) pour ajouter un suffixe, vous devrez également mettre à jour le nom du fichier `instrumentation` pour correspondre.
> - Nous avons créé un exemple basique [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry) que vous pouvez utiliser.
...
Le package `@vercel/otel` fournit de nombreuses options de configuration et devrait couvrir la plupart des cas d'usage courants. Mais s'il ne répond pas à vos besoins, vous pouvez configurer OpenTelemetry manuellement.

D'abord, vous devez installer les packages OpenTelemetry :
...
Faire cela équivaut à utiliser `@vercel/otel`, mais il est possible de modifier et étendre certaines fonctionnalités qui ne sont pas exposées par `@vercel/otel`. Si le support du runtime edge est nécessaire, vous devrez utiliser `@vercel/otel`.
...
Si tout fonctionne correctement, vous devriez pouvoir voir le span serveur racine étiqueté comme `GET /requested/pathname`.
...
Une fois votre collecteur opérationnel, vous pouvez déployer votre application Next.js sur la plateforme de votre choix en suivant leurs guides de déploiement respectifs.

### Exportateurs personnalisés

OpenTelemetry Collector n'est pas obligatoire. Vous pouvez utiliser un exportateur OpenTelemetry personnalisé avec [`@vercel/otel`](#using-vercelotel) ou [la configuration manuelle d'OpenTelemetry](/docs/14/pages/building-your-application/optimizing/open-telemetry#manual-opentelemetry-configuration).
...
La fonction `register` s'exécutera avant votre code dans un nouvel environnement.
...
Les attributs sur les spans suivent les [conventions sémantiques d'OpenTelemetry](https://opentelemetry.io/docs/reference/specification/trace/semantic_conventions/). Nous ajoutons également certains attributs personnalisés sous l'espace de noms `next` :
...
- `next.rsc` (true/false) - Si la requête est une requête RSC, comme un préchargement.
- `next.page`
...
- [Attributs HTTP client](https://opentelemetry.io/docs/reference/specification/trace/semantic_conventions/http/#http-client)
  - `http.url`
  - `net.peer.name`
  - `net.peer.port` (seulement si spécifié)
- `next.span_name`
- `next.span_type`
