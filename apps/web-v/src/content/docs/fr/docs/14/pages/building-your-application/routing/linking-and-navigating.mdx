...
Tout `<Link />` dans la zone visible (initialement ou via défilement) sera préchargé par défaut (y compris les données correspondantes) pour les pages utilisant la [Génération statique](/docs/14/pages/building-your-application/data-fetching/get-static-props). Les données correspondantes pour les routes [rendues côté serveur](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) sont récupérées uniquement lorsque le `<Link />` est cliqué.

## Liaison vers des chemins dynamiques

Vous pouvez également utiliser l'interpolation pour créer le chemin, ce qui est pratique pour les [segments de route dynamiques](/docs/14/pages/building-your-application/routing/dynamic-routes). Par exemple, pour afficher une liste d'articles passés au composant en tant que prop :
...
Pour accéder à l'[objet `router`](/docs/14/pages/api-reference/functions/use-router#router-object) dans un composant React, vous pouvez utiliser [`useRouter`](/docs/14/pages/api-reference/functions/use-router) ou [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter).

En général, nous recommandons d'utiliser [`useRouter`](/docs/14/pages/api-reference/functions/use-router).

## Routage impératif

[`next/link`](/docs/14/pages/api-reference/components/link) devrait couvrir la plupart de vos besoins en matière de routage, mais vous pouvez également effectuer des navigations côté client sans lui, consultez la [documentation pour `next/router`](/docs/14/pages/api-reference/functions/use-router).

L'exemple suivant montre comment effectuer des navigations de page de base avec [`useRouter`](/docs/14/pages/api-reference/functions/use-router):
...
Le routage superficiel permet de changer l'URL sans exécuter à nouveau les méthodes de récupération de données, y compris [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props), [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props), et [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props).

Vous recevrez le `pathname` mis à jour et la `query` via l'[objet `router`](/docs/14/pages/api-reference/functions/use-router#router-object) (ajouté par [`useRouter`](/docs/14/pages/api-reference/functions/use-router) ou [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter)), sans perdre l'état.
