...
translation-updated-at: 2025-06-06T16:59:24.391Z
...
> **Bon à savoir** : Vous pouvez utiliser [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) dans cette page si vous avez besoin de récupérer des données lors de la construction.
...
- `Error` ne prend pas actuellement en charge les méthodes de récupération de données (Data Fetching) de Next.js comme [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) ou [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props).
- `_error`, comme `_app`, est un nom de chemin réservé. `_error` est utilisé pour définir les mises en page et comportements personnalisés des pages d'erreur. `/_error` affichera une 404 lorsqu'il est accédé directement via le [routage](/docs/14/pages/building-your-application/routing) ou le rendu dans un [serveur personnalisé](/docs/14/pages/building-your-application/configuring/custom-server).