...
`useParams` est un hook pour les **Composants Client** qui permet de lire les [paramètres dynamiques](/docs/14/app/building-your-application/routing/dynamic-routes) d'une route remplis par l'URL actuelle.
...
`useParams` retourne un objet contenant les [paramètres dynamiques](/docs/14/app/building-your-application/routing/dynamic-routes) remplis de la route actuelle.
...
- La valeur de la propriété sera soit une `string` soit un tableau de `string` selon le [type de segment dynamique](/docs/14/app/building-your-application/routing/dynamic-routes).
