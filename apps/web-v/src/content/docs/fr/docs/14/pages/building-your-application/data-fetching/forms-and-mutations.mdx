...
sidebar:
  order: 3010303
  label: "Formulaires et mutations"
---
...
> - Comme les routes API s'exécutent sur le serveur, nous pouvons utiliser des valeurs sensibles (comme des clés API) via les [variables d'environnement](/docs/14/pages/building-your-application/configuring/environment-variables) sans les exposer au client. Ceci est crucial pour la sécurité de votre application.
...
Si vous souhaitez rediriger l'utilisateur vers une route différente après une mutation, vous pouvez utiliser [`redirect`](/docs/14/pages/building-your-application/routing/api-routes#response-helpers) vers n'importe quelle URL absolue ou relative :
...
Vous pouvez lire les cookies dans une route API en utilisant l'aide de requête [`cookies`](/docs/14/pages/building-your-application/routing/api-routes#request-helpers) :
