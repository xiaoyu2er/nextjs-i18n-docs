...
La fonction `permanentRedirect` permet de rediriger l'utilisateur vers une autre URL. `permanentRedirect` peut être utilisée dans les composants serveur, les composants client, les [gestionnaires de route (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) et les [actions serveur (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations).
...
> **Bon à savoir** : Si vous préférez renvoyer une redirection HTTP 307 (Temporaire) au lieu de 308 (Permanente), vous pouvez utiliser la fonction [`redirect`](/docs/14/app/api-reference/functions/redirect) à la place.
...
Par défaut, `permanentRedirect` utilisera `push` (ajoute une nouvelle entrée dans la pile d'historique du navigateur) dans les [actions serveur (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) et `replace` (remplace l'URL actuelle dans la pile d'historique du navigateur) partout ailleurs. Vous pouvez modifier ce comportement en spécifiant le paramètre `type`.
