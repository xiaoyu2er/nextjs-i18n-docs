...
Par exemple, dans la [référence API des fonctions](/docs/14/app/api-reference/functions), les pages sont triées alphabétiquement car cela facilite la recherche d'une fonction spécifique pour les développeurs :
...
Mais, dans la [section routage](/docs/14/app/building-your-application/routing), les fichiers sont préfixés avec un nombre à deux chiffres, triés dans l'ordre où les développeurs devraient apprendre ces concepts :
...
- Les liens seront automatiquement générés pour les pages qui ont des pages enfants. Par exemple, la section [Optimisation](/docs/14/app/building-your-application/optimizing) a des liens vers toutes ses pages enfants.
