...
Le [Middleware](/docs/14/app/building-your-application/routing/middleware) vous permet d'ajouter des en-têtes et de générer des nonces avant le rendu de la page.
...
Par défaut, le Middleware s'exécute sur toutes les requêtes. Vous pouvez filtrer le Middleware pour qu'il s'exécute sur des chemins spécifiques en utilisant un [`matcher`](/docs/14/app/building-your-application/routing/middleware#matcher).
...
Vous pouvez maintenant lire le nonce depuis un [Composant Serveur](/docs/14/app/building-your-application/rendering/server-components) en utilisant [`headers`](/docs/14/app/api-reference/functions/headers) :
...
Pour les applications qui ne nécessitent pas de nonces, vous pouvez définir l'en-tête CSP directement dans votre fichier [`next.config.js`](/docs/14/app/api-reference/next-config-js) :
