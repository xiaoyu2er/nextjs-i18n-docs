...
Avec Next.js 12 et le [Middleware](/docs/14/pages/building-your-application/routing/middleware), nous pouvons ajouter un préfixe à la locale par défaut avec une [solution de contournement](https://github.com/vercel/next.js/discussions/18419).
...
Ensuite, nous pouvons utiliser le [Middleware](/docs/14/pages/building-your-application/routing/middleware) pour ajouter des règles de routage personnalisées :
...
Ce [Middleware](/docs/14/pages/building-your-application/routing/middleware) ignore l'ajout du préfixe par défaut aux [API Routes](/docs/14/pages/building-your-application/routing/api-routes) et aux fichiers [publics](/docs/14/pages/building-your-application/optimizing/static-assets) comme les polices ou images. Si une requête est faite vers la locale par défaut, nous redirigeons vers notre préfixe `/en`.
...
Vous pouvez accéder aux informations de locale via le routeur Next.js. Par exemple, en utilisant le hook [`useRouter()`](/docs/14/pages/api-reference/functions/use-router), les propriétés suivantes sont disponibles :
...
Lors de la [pré-rendu](/docs/14/pages/building-your-application/rendering/static-site-generation) de pages avec `getStaticProps` ou `getServerSideProps`, les informations de locale sont fournies dans [le contexte](/docs/14/pages/building-your-application/data-fetching/get-static-props) fourni à la fonction.
...
Voir [ici](/docs/14/pages/api-reference/functions/use-router#with-url-object) pour plus d'informations sur la structure d'objet pour `router.push`.
...
Next.js ne connaît pas les variantes d'une page, c'est donc à vous d'ajouter les balises meta `hreflang` en utilisant [`next/head`](/docs/14/pages/api-reference/components/head). Vous pouvez en apprendre plus sur `hreflang` dans la [documentation Google pour les webmasters](https://support.google.com/webmasters/answer/189077).

## Comment cela fonctionne-t-il avec la Génération Statique ?

> Notez que le Routage Internationalisé ne s'intègre pas avec [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) car il n'utilise pas la couche de routage de Next.js. Les applications hybrides Next.js qui n'utilisent pas `output: 'export'` sont entièrement supportées.

### Routes Dynamiques et Pages `getStaticProps`

Pour les pages utilisant `getStaticProps` avec des [Routes Dynamiques](/docs/14/pages/building-your-application/routing/dynamic-routes), toutes les variantes de locale de la page que vous souhaitez pré-rendre doivent être retournées par [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths). Avec l'objet `params` retourné pour `paths`, vous pouvez aussi retourner un champ `locale` spécifiant quelle locale vous souhaitez rendre. Par exemple :
...
Pour les pages [Optimisées Statiquement de manière Automatique](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) et les pages non dynamiques avec `getStaticProps`, **une version de la page sera générée pour chaque locale**. Ceci est important à considérer car cela peut augmenter les temps de build en fonction du nombre de locales configurées dans `getStaticProps`.
