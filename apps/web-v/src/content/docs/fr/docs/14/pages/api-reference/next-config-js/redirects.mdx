...
translation-updated-at: 2025-06-06T16:53:02.236Z
title: Redirections
...
- `source` : le modèle de chemin de requête entrant.
- `destination` : le chemin vers lequel vous souhaitez rediriger.
- `permanent` : `true` ou `false` - si `true`, utilise le code de statut 308 qui indique aux clients/moteurs de recherche de mettre en cache la redirection indéfiniment. Si `false`, utilise le code de statut 307 qui est temporaire et non mis en cache.
...
Lorsque vous utilisez le routeur Pages, les redirections ne sont pas appliquées au routage côté client (`Link`, `router.push`) sauf si un [Middleware](/docs/14/app/building-your-application/routing/middleware) est présent et correspond au chemin.
...
Pour correspondre à un chemin regex, vous pouvez encapsuler la regex entre parenthèses après un paramètre, par exemple `/post/:slug(\\d{1,})` correspondra à `/post/123` mais pas à `/post/abc` :
...
Lorsque vous utilisez le support [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) avec des redirections, chaque `source` et `destination` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à la redirection :
...
Lorsque vous utilisez le support [`i18n`](/docs/14/app/building-your-application/routing/internationalization) avec des redirections, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées sauf si vous ajoutez `locale: false` à la redirection. Si `locale: false` est utilisé, vous devez préfixer `source` et `destination` avec une locale pour qu'elle soit correctement correspondante.

</AppOnly>

<PagesOnly>

Lorsque vous utilisez le support [`i18n`](/docs/14/pages/building-your-application/routing/internationalization) avec des redirections, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées sauf si vous ajoutez `locale: false` à la redirection. Si `locale: false` est utilisé, vous devez préfixer `source` et `destination` avec une locale pour qu'elle soit correctement correspondante.
...
- Dans les [Routes API](/docs/14/pages/building-your-application/routing/api-routes) et les [Gestionnaires de route](/docs/14/app/building-your-application/routing/route-handlers), vous pouvez rediriger en fonction de la requête entrante.
- Dans [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) et [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props), vous pouvez rediriger des pages spécifiques au moment de la requête.
