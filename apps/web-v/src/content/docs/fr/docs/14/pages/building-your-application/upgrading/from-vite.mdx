...
translation-updated-at: 2025-06-06T16:55:21.678Z
title: Migration depuis Vite
description: Apprenez comment migrer votre application React existante de Vite vers Next.js.
...
Avec le support intégré pour [le streaming via React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense), vous pouvez être plus intentionnel sur les parties de votre interface que vous souhaitez charger en premier et dans quel ordre, sans introduire de cascades réseau.
...
Le [Middleware Next.js](/docs/14/app/building-your-application/routing/middleware) vous permet d'exécuter du code côté serveur avant qu'une requête ne soit terminée. Ceci est particulièrement utile pour éviter d'afficher brièvement du contenu non authentifié lorsque l'utilisateur visite une page réservée aux utilisateurs authentifiés en redirigeant l'utilisateur vers une page de connexion. Le middleware est également utile pour l'expérimentation et l'[internationalisation](/docs/14/app/building-your-application/routing/internationalization).
...
Vous pouvez trouver plus d'informations sur la configuration de TypeScript dans la [documentation Next.js](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin).

### Étape 4 : Créer la mise en page racine

Une application Next.js utilisant le [routeur App](/docs/14/app) doit inclure un fichier de [mise en page racine](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required), qui est un [composant serveur React](/docs/14/app/building-your-application/rendering/server-components) qui englobera toutes les pages de votre application. Ce fichier est défini au niveau supérieur du répertoire `app`.
...
5. Tous les [fichiers de métadonnées](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) tels que `favicon.ico`, `icon.png`, `robots.txt` sont automatiquement ajoutés à la balise `<head>` de l'application tant qu'ils sont placés au niveau supérieur du répertoire `app`. Après avoir déplacé [tous les fichiers supportés](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) dans le répertoire `app`, vous pouvez supprimer en toute sécurité leurs balises `<link>` :
...
6. Enfin, Next.js peut gérer vos dernières balises `<head>` avec l'[API Metadata](/docs/14/app/building-your-application/optimizing/metadata). Déplacez vos dernières informations de métadonnées dans un objet [`metadata` exporté](/docs/14/app/api-reference/functions/generate-metadata#metadata-object) :
...
[segment de route attrape-tout optionnel](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments).
Next.js utilise un routeur basé sur le système de fichiers où
[les répertoires sont utilisés pour définir les routes](/docs/14/app/building-your-application/routing/defining-routes#creating-routes).
...
Ce fichier est un [Composant Serveur (Server Component)](/docs/14/app/building-your-application/rendering/server-components). Lorsque vous exécutez `next build`, le fichier est prérendu en un asset statique. Il ne nécessite _aucun_ code dynamique.

Ce fichier importe notre CSS global et indique à [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) que nous allons générer une seule route, la route d'index à `/`.
...
Ce fichier est un [Composant Client (Client Component)](/docs/14/app/building-your-application/rendering/client-components), défini par la directive `'use client'`.
Les Composants Client sont toujours [prérendus en HTML](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered) sur le serveur avant d'être envoyés au client.
...
Avec Next.js, les imports d'images statiques retournent un objet. Cet objet peut ensuite être utilisé directement avec le composant [`<Image>`](/docs/14/app/api-reference/components/image) de Next.js, ou vous pouvez utiliser la propriété `src` de l'objet avec votre balise `<img>` existante.

Le composant `<Image>` offre des avantages supplémentaires comme
[l'optimisation automatique des images](/docs/14/app/building-your-application/optimizing/images). Le composant `<Image>`
...
[variables d'environnement](/docs/14/app/building-your-application/configuring/environment-variables) `.env`
...
2. **Définissez [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) à `process.env.NEXT_PUBLIC_BASE_PATH` dans votre fichier `next.config.mjs` :**
...
- Migrer de React Router vers le [Routeur App de Next.js](/docs/14/app/building-your-application/routing) pour obtenir :
  - Le découpage de code automatique
  - [Le rendu côté serveur en streaming (Streaming Server-Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)
  - [Les Composants Serveur React (React Server Components)](/docs/14/app/building-your-application/rendering/server-components)
