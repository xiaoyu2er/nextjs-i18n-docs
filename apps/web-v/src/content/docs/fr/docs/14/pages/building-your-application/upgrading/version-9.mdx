...
Si vous avez précédemment configuré des `routes` dans votre fichier `vercel.json` pour des routes dynamiques, ces règles peuvent être supprimées en tirant parti de la nouvelle [fonctionnalité de routage dynamique](/docs/14/pages/building-your-application/routing/dynamic-routes) de Next.js 9.
...
Vous pouvez en savoir plus sur le [routage dynamique ici](/docs/14/pages/building-your-application/routing/dynamic-routes).

## Vérifiez votre fichier d'application personnalisée (`pages/_app.js`)

Si vous avez précédemment copié l'exemple de [l'application personnalisée (`<App>`)](/docs/14/pages/building-your-application/routing/custom-app), vous pourrez peut-être supprimer votre `getInitialProps`.
