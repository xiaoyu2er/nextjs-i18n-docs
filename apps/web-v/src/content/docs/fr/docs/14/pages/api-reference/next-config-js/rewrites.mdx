...
translation-updated-at: 2025-06-06T16:52:59.459Z
title: Réécritures (rewrites)
description: Ajoutez des réécritures (rewrites) à votre application Next.js.
...
Les réécritures agissent comme un proxy d'URL et masquent le chemin de destination, donnant l'impression que l'utilisateur n'a pas changé d'emplacement sur le site. En revanche, les [redirections](/docs/14/app/api-reference/next-config-js/redirects) redirigeront vers une nouvelle page et montreront les changements d'URL.

</AppOnly>

<PagesOnly>

Les réécritures agissent comme un proxy d'URL et masquent le chemin de destination, donnant l'impression que l'utilisateur n'a pas changé d'emplacement sur le site. En revanche, les [redirections](/docs/14/pages/api-reference/next-config-js/redirects) redirigeront vers une nouvelle page et montreront les changements d'URL.
...
- `source` : `String` - le motif de chemin de requête entrant.
...
- `has` est un tableau d'[objets has](#correspondance-dentete-de-cookie-et-de-requete) avec les propriétés `type`, `key` et `value`.
- `missing` est un tableau d'[objets missing](#correspondance-dentete-de-cookie-et-de-requete) avec les propriétés `type`, `key` et `value`.
...
> **Bon à savoir** : les réécritures dans `beforeFiles` ne vérifient pas immédiatement le système de fichiers/routes dynamiques après avoir trouvé une correspondance, elles continuent jusqu'à ce que toutes les `beforeFiles` aient été vérifiées.

L'ordre dans lequel Next.js vérifie les routes est :

<AppOnly>

1. Les [en-têtes](/docs/14/app/api-reference/next-config-js/headers) sont vérifiés/appliqués
2. Les [redirections](/docs/14/app/api-reference/next-config-js/redirects) sont vérifiées/appliquées
3. Les réécritures `beforeFiles` sont vérifiées/appliquées
4. Les fichiers statiques du [répertoire public](/docs/14/app/building-your-application/optimizing/static-assets), les fichiers `_next/static` et les pages non dynamiques sont vérifiés/servis
...
6. Les réécritures `fallback` sont vérifiées/appliquées. Elles sont appliquées avant le rendu de la page 404 et après la vérification des routes dynamiques/tous les assets statiques. Si vous utilisez [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) dans `getStaticPaths`, les réécritures `fallback` définies dans votre `next.config.js` ne seront _pas_ exécutées.

</AppOnly>

<PagesOnly>

1. Les [en-têtes](/docs/14/pages/api-reference/next-config-js/headers) sont vérifiés/appliqués
2. Les [redirections](/docs/14/pages/api-reference/next-config-js/redirects) sont vérifiées/appliquées
3. Les réécritures `beforeFiles` sont vérifiées/appliquées
4. Les fichiers statiques du [répertoire public](/docs/14/pages/building-your-application/optimizing/static-assets), les fichiers `_next/static` et les pages non dynamiques sont vérifiés/servis
5. Les réécritures `afterFiles` sont vérifiées/appliquées. Si l'une de ces réécritures correspond, nous vérifions les routes dynamiques/fichiers statiques après chaque correspondance
6. Les réécritures `fallback` sont vérifiées/appliquées. Elles sont appliquées avant le rendu de la page 404 et après la vérification des routes dynamiques/tous les assets statiques. Si vous utilisez [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) dans `getStaticPaths`, les réécritures `fallback` définies dans votre `next.config.js` ne seront _pas_ exécutées.
...
> **Bon à savoir** : Les pages statiques de [l'optimisation statique automatique](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) ou les paramètres de prérendu des réécritures seront analysés côté client après l'hydratation et fournis dans la requête.
...
Pour correspondre à un chemin regex, vous pouvez encapsuler la regex entre parenthèses après un paramètre, par exemple `/blog/:slug(\\d{1,})` correspondra à `/blog/123` mais pas à `/blog/abc` :
...
Pour ne correspondre à une réécriture que lorsque les valeurs d'en-tête, de cookie ou de requête correspondent également, le champ `has` peut être utilisé, ou le champ `missing` pour ne pas correspondre. À la fois la `source` et tous les éléments `has` doivent correspondre, et tous les éléments `missing` ne doivent pas correspondre pour que la réécriture soit appliquée.
...
- [Adoption progressive de Next.js](https://github.com/vercel/next.js/tree/canary/examples/custom-routes-proxying)
- [Utilisation de zones multiples](https://github.com/vercel/next.js/tree/canary/examples/with-zones)

</details>

Les réécritures permettent de réécrire vers une URL externe. Cela est particulièrement utile pour l'adoption progressive de Next.js. Voici un exemple de réécriture pour rediriger la route `/blog` de votre application principale vers un site externe.
...
Si vous utilisez `trailingSlash: true`, vous devez également insérer une barre oblique finale dans le paramètre `source`. Si le serveur de destination attend également une barre oblique finale, elle doit être incluse dans le paramètre `destination`.
...
Lorsque vous utilisez le support de [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) avec les réécritures, chaque `source` et `destination` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à la réécriture :
...
Lorsque vous utilisez le support [`i18n`](/docs/14/app/building-your-application/routing/internationalization) avec les réécritures, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à la réécriture. Si `locale: false` est utilisé, vous devez préfixer la `source` et la `destination` avec une locale pour qu'elle corresponde correctement.

</AppOnly>

<PagesOnly>

Lorsque vous utilisez le support [`i18n`](/docs/14/pages/building-your-application/routing/internationalization) avec les réécritures, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à la réécriture. Si `locale: false` est utilisé, vous devez préfixer la `source` et la `destination` avec une locale pour qu'elle corresponde correctement.
