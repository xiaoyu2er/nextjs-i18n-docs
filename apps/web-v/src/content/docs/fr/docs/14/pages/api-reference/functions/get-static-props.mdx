...
| Nom                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`           | Contient les paramètres de route pour les pages utilisant des [routes dynamiques](/docs/14/pages/building-your-application/routing/dynamic-routes). Par exemple, si le nom de la page est `[id].js`, alors `params` ressemblera à `{ id: ... }`. Vous devriez utiliser cela avec `getStaticPaths`, que nous expliquerons plus tard.                                                                                                                  |
...
| `revalidateReason` | Fournit une raison pour laquelle la fonction a été appelée. Peut être l'une des suivantes : "build" (exécuté au moment de la construction), "stale" (période de revalidation expirée, ou exécution en [mode développement](/docs/14/pages/building-your-application/data-fetching/get-static-props#runs-on-every-request-in-development)), "on-demand" (déclenché via [revalidation à la demande](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)) |
...
Apprenez-en plus sur la [Régénération Statique Incrémentielle](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
> **Bon à savoir** : `notFound` n'est pas nécessaire pour le mode [`fallback: false`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-false) car seuls les chemins retournés par `getStaticPaths` seront pré-rendus.
...
Si les redirections sont connues au moment de la construction, elles devraient être ajoutées dans [`next.config.js`](/docs/14/pages/api-reference/next-config-js/redirects) à la place.
...
| Version   | Changements                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | Le [Routeur d'application](/docs/14/app/building-your-application/data-fetching) est maintenant stable avec une récupération de données simplifiée                                                           |
| `v12.2.0` | La [Régénération Statique Incrémentielle à la demande](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) est stable.    |
| `v12.1.0` | La [Régénération Statique Incrémentielle à la demande](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) ajoutée (bêta). |
| `v10.0.0` | Options `locale`, `locales`, `defaultLocale`, et `notFound` ajoutées.                                                                                                   |
| `v10.0.0` | Option de retour `fallback: 'blocking'` ajoutée.                                                                                                                           |
| `v9.5.0`  | [Régénération Statique Incrémentielle](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) stable                                         |
