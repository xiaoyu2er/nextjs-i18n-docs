...
Actuellement, la seule valeur de `context` est `params`, qui est un objet contenant les [paramètres de route dynamique](/docs/14/app/building-your-application/routing/dynamic-routes) pour la route actuelle.
...
Les gestionnaires de route peuvent étendre l'API Web Response en retournant un objet `NextResponse`. Cela vous permet de facilement définir des cookies, des en-têtes, de rediriger et de réécrire. [Voir la référence API](/docs/14/app/api-reference/functions/next-response).
