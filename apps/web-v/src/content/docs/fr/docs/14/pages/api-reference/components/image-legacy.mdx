...
Consultez la **nouvelle** [Référence API de `next/image`](/docs/14/pages/api-reference/components/image)
...
- Un fichier image [importé statiquement](/docs/14/pages/building-your-application/optimizing/images#local-images)
...
La propriété `width` est obligatoire, sauf pour les [images importées statiquement](/docs/14/pages/building-your-application/optimizing/images#local-images), ou celles avec `layout="fill"`.
...
La propriété `height` est obligatoire, sauf pour les [images importées statiquement](/docs/14/pages/building-your-application/optimizing/images#local-images), ou celles avec `layout="fill"`.
...
Avec `blur`, la propriété [`blurDataURL`](#blurdataurl) sera utilisée comme placeholder. Si `src` est un objet provenant d'une [importation statique](/docs/14/pages/building-your-application/optimizing/images#local-images) et que l'image importée est `.jpg`, `.png`, `.webp`, ou `.avif`, alors `blurDataURL` sera automatiquement remplie.
...
> Les images ne peuvent pas être optimisées au moment de la construction en utilisant [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports), uniquement à la demande. Pour utiliser `next/legacy/image` avec `output: 'export'`, vous devrez utiliser un chargeur différent de celui par défaut. [Lire la discussion.](https://github.com/vercel/next.js/discussions/19065)
...
Vous pouvez configurer le Time to Live (TTL) en secondes pour les images optimisées en cache. Dans de nombreux cas, il est préférable d'utiliser une [importation d'image statique](/docs/14/pages/building-your-application/optimizing/images#local-images) qui hachera automatiquement le contenu du fichier et mettra l'image en cache indéfiniment avec un en-tête `Cache-Control` défini sur `immutable`.
...
Si vous devez modifier le comportement de mise en cache par image, vous pouvez configurer [`headers`](/docs/14/pages/api-reference/next-config-js/headers) pour définir l'en-tête `Cache-Control` sur l'image en amont (par exemple `/some-asset.jpg`, et non `/_next/image` lui-même).
...
Le [chargeur](#loader) par défaut n'optimise pas les images SVG pour plusieurs raisons. Premièrement, SVG est un format vectoriel, ce qui signifie qu'il peut être redimensionné sans perte. Deuxièmement, SVG a de nombreuses fonctionnalités similaires à HTML/CSS, ce qui peut entraîner des vulnérabilités sans des [en-têtes Content Security Policy (CSP)](/docs/14/app/api-reference/next-config-js/headers#content-security-policy) appropriés.
