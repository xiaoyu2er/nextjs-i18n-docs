...
La fonction `notFound` vous permet d'afficher le [fichier `not-found`](/docs/14/app/api-reference/file-conventions/not-found) dans un segment de route ainsi que d'injecter une balise `<meta name="robots" content="noindex" />`.

## `notFound()`

L'appel de la fonction `notFound()` génère une erreur `NEXT_NOT_FOUND` et interrompt le rendu du segment de route où elle a été déclenchée. La spécification d'un [fichier **not-found**](/docs/14/app/api-reference/file-conventions/not-found) vous permet de gérer ces erreurs de manière élégante en affichant une interface "Non trouvé" dans le segment.
