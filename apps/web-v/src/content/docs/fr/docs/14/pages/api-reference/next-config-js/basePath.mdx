...
translation-updated-at: 2025-06-06T16:53:38.325Z
title: basePath
...
> **Bon à savoir** : Cette valeur doit être définie au moment de la construction et ne peut pas être modifiée sans recompilation car la valeur est intégrée dans les bundles côté client.

### Liens

Lors de la création de liens vers d'autres pages avec `next/link` et `next/router`, le `basePath` sera automatiquement appliqué.
...
Lorsque vous utilisez le composant [`next/image`](/docs/14/app/api-reference/components/image), vous devrez ajouter le `basePath` devant `src`.

</AppOnly>

<PagesOnly>

Lorsque vous utilisez le composant [`next/image`](/docs/14/pages/api-reference/components/image), vous devrez ajouter le `basePath` devant `src`.
