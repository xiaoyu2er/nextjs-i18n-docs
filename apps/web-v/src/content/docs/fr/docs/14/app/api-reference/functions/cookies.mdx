...
translation-updated-at: 2025-06-06T17:05:35.669Z
title: cookies
...
La fonction `cookies` permet de lire les cookies des requêtes HTTP entrantes dans un [Composant Serveur (Server Component)](/docs/14/app/building-your-application/rendering/server-components) ou d'écrire des cookies dans les requêtes sortantes via une [Action Serveur (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) ou un [Gestionnaire de Route (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).

> **Bon à savoir** : `cookies()` est une **[Fonction Dynamique (Dynamic Function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)** dont les valeurs retournées ne peuvent pas être connues à l'avance. Son utilisation dans une mise en page (layout) ou une page activera le **[rendu dynamique (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)** au moment de la requête.
...
> **Bon à savoir** : HTTP n'autorise pas la définition de cookies après le début du streaming, vous devez donc utiliser `.set()` dans une [Action Serveur (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) ou un [Gestionnaire de Route (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
...
> **Bon à savoir** : Vous ne pouvez supprimer des cookies que dans une [Action Serveur (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) ou un [Gestionnaire de Route (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
...
> **Bon à savoir** : `.set()` n'est disponible que dans une [Action Serveur (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) ou un [Gestionnaire de Route (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers).
