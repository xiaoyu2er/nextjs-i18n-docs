...
Pour l'utiliser, placez le fichier à la **racine** de votre application ou dans un dossier [`src`](/docs/14/app/building-your-application/configuring/src-directory) si vous en utilisez un.

## Option de configuration

L'instrumentation est actuellement une fonctionnalité expérimentale. Pour utiliser le fichier `instrumentation`, vous devez explicitement l'activer en définissant [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) dans votre `next.config.js` :
