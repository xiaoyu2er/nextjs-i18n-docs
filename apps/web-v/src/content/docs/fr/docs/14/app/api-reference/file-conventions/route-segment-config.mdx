...
Les options de segment de route vous permettent de configurer le comportement d'une [Page](/docs/14/app/building-your-application/routing/pages-and-layouts), d'un [Layout](/docs/14/app/building-your-application/routing/pages-and-layouts) ou d'un [Gestionnaire de route (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) en exportant directement les variables suivantes :
...
- **`'force-dynamic'`** : Force le [rendu dynamique (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering), ce qui entraîne le rendu des routes pour chaque utilisateur au moment de la requête. Cette option équivaut à `getServerSideProps()` dans le répertoire `pages`.

- **`'error'`** : Force le rendu statique et met en cache les données d'un layout ou d'une page en provoquant une erreur si des composants utilisent des [fonctions dynamiques (dynamic functions)](/docs/14/app/building-your-application/rendering/server-components#dynamic-functions) ou des données non mises en cache. Cette option équivaut à :
...
- **`'force-static'`** : Force le rendu statique et met en cache les données d'un layout ou d'une page en forçant [`cookies()`](/docs/14/app/api-reference/functions/cookies), [`headers()`](/docs/14/app/api-reference/functions/headers) et [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) à retourner des valeurs vides.

> **Bon à savoir** :
>
> - Des instructions sur [comment migrer](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods) depuis `getServerSideProps` et `getStaticProps` vers `dynamic: 'force-dynamic'` et `dynamic: 'error'` sont disponibles dans le [guide de mise à niveau](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods).

### `dynamicParams`

Contrôle ce qui se passe lorsqu'un segment dynamique est visité et qu'il n'a pas été généré avec [generateStaticParams](/docs/14/app/api-reference/functions/generate-static-params).
...
> - Lorsque `dynamicParams = true`, le segment utilise le [Streaming Server Rendering](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
En savoir plus sur les [runtimes Edge et Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> **Bon à savoir** :
>
> - Si vous utilisez [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations), définissez `maxDuration` au niveau de la page pour modifier le timeout par défaut de toutes les Server Actions utilisées sur la page.

### `generateStaticParams`

La fonction `generateStaticParams` peut être utilisée en combinaison avec des [segments de route dynamiques](/docs/14/app/building-your-application/routing/dynamic-routes) pour définir la liste des paramètres de segment de route qui seront générés statiquement au moment du build plutôt qu'à la demande au moment de la requête.

Voir la [référence API](/docs/14/app/api-reference/functions/generate-static-params) pour plus de détails.