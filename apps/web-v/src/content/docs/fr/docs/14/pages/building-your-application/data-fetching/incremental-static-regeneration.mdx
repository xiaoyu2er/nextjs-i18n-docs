...
> **Bon à savoir** : Le runtime [`edge`](/docs/14/pages/api-reference/edge) n'est actuellement pas compatible avec l'ISR, bien que vous puissiez exploiter `stale-while-revalidate` en définissant manuellement l'en-tête `cache-control`.
...
Ensuite, ajoutez le secret comme [variable d'environnement](/docs/14/pages/building-your-application/configuring/environment-variables) à votre application. Enfin, créez la route API de revalidation :
...
Lors de l'exécution locale avec `next dev`, `getStaticProps` est appelé à chaque requête. Pour vérifier que votre configuration ISR à la demande est correcte, vous devrez créer une [version de production](/docs/14/pages/api-reference/cli/next#next-build-options) et démarrer le [serveur de production](/docs/14/pages/api-reference/cli/next#next-start-options) :
...
La régénération statique incrémentielle (ISR) fonctionne sur [les sites Next.js auto-hébergés](/docs/14/pages/building-your-application/deploying#self-hosting) dès que vous utilisez `next start`.

Apprenez-en plus sur [l'auto-hébergement de Next.js](/docs/14/pages/building-your-application/deploying#self-hosting).
