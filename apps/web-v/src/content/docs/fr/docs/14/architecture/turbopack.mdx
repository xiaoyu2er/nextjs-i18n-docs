...
Turbopack dans Next.js ne nécessite aucune configuration pour la plupart des utilisateurs et peut être étendu pour des cas d'utilisation plus avancés. Pour en savoir plus sur les fonctionnalités actuellement prises en charge par Turbopack, consultez la [Référence API](/docs/14/app/api-reference/next-config-js/turbo).
...
- Configuration [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) dans `next.config.js`
  - Turbopack remplace Webpack, ce qui signifie que la configuration Webpack n'est pas prise en charge.
  - Pour configurer Turbopack, [consultez la documentation](/docs/14/app/api-reference/next-config-js/turbo).
  - Un sous-ensemble de [chargeurs Webpack](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) est pris en charge dans Turbopack.
- Babel (`.babelrc`)
  ...
  - Si vous avez toujours besoin d'utiliser Babel après avoir vérifié que votre cas d'utilisation spécifique n'est pas couvert, vous pouvez utiliser le [support des chargeurs Webpack personnalisés](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) de Turbopack pour inclure `babel-loader`.
...
- `@next/font` (support hérité des polices).
  - `@next/font` est déprécié au profit de `next/font`. [`next/font`](/docs/14/app/building-your-application/optimizing/fonts) est entièrement pris en charge avec Turbopack.
