...
Configure comment la requête doit interagir avec le [cache de données (Data Cache)](/docs/14/app/building-your-application/caching#data-cache) de Next.js.
...
> **Bon à savoir** :
>
> - Si vous ne fournissez pas d'option `cache`, Next.js utilisera `force-cache` par défaut, sauf si une [fonction dynamique](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) comme `cookies()` est utilisée, auquel cas il utilisera `no-store` par défaut.
...
> **Bon à savoir** :
>
> - Si une requête `fetch()` individuelle définit un `revalidate` inférieur à la valeur par défaut [`revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) d'une route, l'intervalle de revalidation de toute la route sera réduit.
