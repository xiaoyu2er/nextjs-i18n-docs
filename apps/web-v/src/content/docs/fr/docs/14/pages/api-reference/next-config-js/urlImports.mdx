...
translation-updated-at: 2025-06-06T16:52:50.705Z
title: Importations depuis des URLs
description: Configurer Next.js pour autoriser l'importation de modules depuis des URLs externes (expérimental).
...
Lors de l'utilisation d'importations depuis des URL, Next.js créera un répertoire `next.lock` contenant un fichier de verrouillage et les ressources téléchargées.
Ce répertoire **doit être commité dans Git**, et ne doit pas être ignoré par `.gitignore`.

- Lors de l'exécution de `next dev`, Next.js téléchargera et ajoutera toutes les nouvelles importations depuis des URL découvertes à votre fichier de verrouillage
