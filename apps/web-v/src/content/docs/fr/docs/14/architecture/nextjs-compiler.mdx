...
Cette option a été remplacée par [`optimizePackageImports`](/docs/14/app/api-reference/next-config-js/optimizePackageImports) dans Next.js 13.5. Nous recommandons de passer à la nouvelle option qui ne nécessite pas de configuration manuelle des chemins d'import.
