...
> **Bon à savoir**
>
> - Par défaut, les images générées sont [**optimisées statiquement**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (générées au moment de la construction et mises en cache) sauf si elles utilisent des [fonctions dynamiques](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) ou des données non mises en cache.
> - Vous pouvez générer plusieurs images dans le même fichier en utilisant [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata).

La façon la plus simple de générer une image est d'utiliser l'API [ImageResponse](/docs/14/app/api-reference/functions/image-response) de `next/og`.
...
Un objet contenant les [paramètres de route dynamique](/docs/14/app/building-your-application/routing/dynamic-routes) du segment racine jusqu'au segment où `opengraph-image` ou `twitter-image` est colocalisé.
...
`opengraph-image` et `twitter-image` sont des [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) spécialisés qui peuvent utiliser les mêmes [options de configuration de segment de route](/docs/14/app/api-reference/file-conventions/route-segment-config) que les Pages et Layouts.
...
> **Bon à savoir** :
> Par défaut, cette image générée sera [optimisée statiquement](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). Vous pouvez configurer les [`options`](/docs/14/app/api-reference/functions/fetch) individuelles de `fetch` ou les [options](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) des segments de route pour modifier ce comportement.
