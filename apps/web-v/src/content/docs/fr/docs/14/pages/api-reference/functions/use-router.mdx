...
translation-updated-at: 2025-06-06T16:56:24.658Z
title: useRouter
...
Voici la définition de l'objet `router` retourné à la fois par [`useRouter`](#top) et [`withRouter`](#withrouter) :
...
- `query`: `Object` - La chaîne de requête analysée en objet, incluant les paramètres des [routes dynamiques](/docs/14/pages/building-your-application/routing/dynamic-routes). Ce sera un objet vide pendant le prérendu si la page n'utilise pas le [rendu côté serveur (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props). Par défaut : `{}`
...
- `isFallback`: `boolean` - Indique si la page actuelle est en [mode de secours (fallback)](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true).
- `basePath`: `String` - Le [basePath](/docs/14/app/api-reference/next-config-js/basePath) actif (si activé).
...
- `isReady`: `boolean` - Indique si les champs du routeur sont mis à jour côté client et prêts à être utilisés. Ne doit être utilisé qu'à l'intérieur de méthodes `useEffect` et pas pour un rendu conditionnel côté serveur. Voir la documentation associée pour les cas d'utilisation avec [les pages optimisées statiquement automatiquement](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)
- `isPreview`: `boolean` - Indique si l'application est actuellement en [mode prévisualisation](/docs/14/pages/building-your-application/configuring/preview-mode).

> L'utilisation du champ `asPath` peut entraîner une incohérence entre le client et le serveur si la page est rendue côté serveur ou avec [l'optimisation statique automatique](/docs/14/pages/building-your-application/rendering/automatic-static-optimization). Évitez d'utiliser `asPath` tant que le champ `isReady` n'est pas `true`.

Les méthodes suivantes sont incluses dans `router` :

### router.push

Gère les transitions côté client. Cette méthode est utile lorsque [`next/link`](/docs/14/pages/api-reference/components/link) ne suffit pas.
...
- `options` - Objet optionnel avec les options de configuration suivantes :
  ...
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): Met à jour le chemin de la page actuelle sans relancer [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props), [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) ou [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props). Par défaut : `false`
...
Redirection de l'utilisateur vers `pages/login.js`, utile pour les pages protégées par [authentification](/docs/14/pages/building-your-application/authentication) :
...
Vous pouvez utiliser un objet URL de la même manière que pour [`next/link`](/docs/14/pages/api-reference/components/link#with-url-object). Fonctionne pour les paramètres `url` et `as` :
...
Similaire à la prop `replace` de [`next/link`](/docs/14/pages/api-reference/components/link), `router.replace` empêche l'ajout d'une nouvelle entrée d'URL dans la pile d'historique.
...
Précharge les pages pour des transitions côté client plus rapides. Cette méthode n'est utile que pour les navigations sans [`next/link`](/docs/14/pages/api-reference/components/link), car `next/link` s'occupe de précharger les pages automatiquement.
...
Dans certains cas (par exemple, si vous utilisez un [serveur personnalisé](/docs/14/pages/building-your-application/configuring/custom-server)), vous pouvez souhaiter écouter les événements [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) et faire quelque chose avant que le routeur n'agisse.
...
Si `cb` retourne `false`, le routeur Next.js ne gérera pas le `popstate`, et vous serez responsable de le gérer dans ce cas. Voir [Désactivation du routage par système de fichiers](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
> **Bon à savoir** : Ici, `url` est l'URL affichée dans le navigateur, incluant le [`basePath`](/docs/14/app/api-reference/next-config-js/basePath).
...
> Nous utilisons une [Application personnalisée](/docs/14/pages/building-your-application/routing/custom-app) (`pages/_app.js`) pour cet exemple afin de nous abonner à l'événement car elle n'est pas démontée lors des navigations de page, mais vous pouvez vous abonner aux événements du routeur dans n'importe quel composant de votre application.
...
Certaines méthodes accessibles sur l'objet `router` retournent une Promise. Si vous avez activé la règle ESLint [no-floating-promises](https://typescript-eslint.io/rules/no-floating-promises), envisagez de la désactiver soit globalement, soit pour la ligne concernée.
...
Si [`useRouter`](#router-object) ne convient pas à votre cas, `withRouter` peut également ajouter le même objet [`router`](#router-object) à n'importe quel composant.
