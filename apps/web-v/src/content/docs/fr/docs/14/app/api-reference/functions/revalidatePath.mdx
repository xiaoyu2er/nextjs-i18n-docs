...
`revalidatePath` vous permet de purger les [données en cache](/docs/14/app/building-your-application/caching) à la demande pour un chemin spécifique.

> **Bon à savoir** :
>
> - `revalidatePath` est disponible dans les environnements d'exécution [Node.js et Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> - Actuellement, `revalidatePath` invalide toutes les routes dans le [cache du routeur côté client](/docs/14/app/building-your-application/caching#router-cache). Ce comportement est temporaire et sera mis à jour à l'avenir pour s'appliquer uniquement au chemin spécifique.
> - L'utilisation de `revalidatePath` invalide **uniquement le chemin spécifique** dans le [cache de route côté serveur](/docs/14/app/building-your-application/caching#full-route-cache).
