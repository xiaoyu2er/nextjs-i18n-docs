...
Standardmäßig werden Server-Komponenten (Server Components) automatisch [code-split](https://developer.mozilla.org/docs/Glossary/Code_splitting), und Sie können [Streaming](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) verwenden, um Teile der UI schrittweise vom Server an den Client zu senden. Lazy Loading gilt für Client-Komponenten.
