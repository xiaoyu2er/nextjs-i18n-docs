...
Mit integrierter Unterstützung für [Streaming durch React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense) können Sie gezielter festlegen, welche Teile Ihrer Benutzeroberfläche zuerst und in welcher Reihenfolge geladen werden sollen, ohne Netzwerk-Wasserfälle einzuführen.
...
[Next.js Middleware](/docs/14/app/building-your-application/routing/middleware) ermöglicht es Ihnen, Code auf dem Server auszuführen, bevor eine Anfrage abgeschlossen ist. Dies ist besonders nützlich, um ein Aufblitzen von nicht authentifizierten Inhalten zu vermeiden, wenn der Benutzer eine nur für authentifizierte Benutzer bestimmte Seite besucht, indem der Benutzer zu einer Login-Seite weitergeleitet wird. Die Middleware ist auch nützlich für Experimente und [Internationalisierung](/docs/14/app/building-your-application/routing/internationalization).

### Integrierte Optimierungen

[Bilder](/docs/14/app/building-your-application/optimizing/images), [Schriftarten](/docs/14/app/building-your-application/optimizing/fonts) und [Skripte von Drittanbietern](/docs/14/app/building-your-application/optimizing/scripts) haben oft einen erheblichen Einfluss auf die Performance einer Anwendung. Next.js kommt mit integrierten Komponenten, die diese automatisch für Sie optimieren.
...
Erstellen Sie eine `next.config.mjs` im Stammverzeichnis Ihres Projekts. Diese Datei enthält Ihre [Next.js-Konfigurationsoptionen](/docs/14/app/api-reference/next-config-js).
...
Weitere Informationen zur Konfiguration von TypeScript finden Sie in den [Next.js-Dokumenten](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin).

### Schritt 4: Root-Layout erstellen

Eine Next.js-[App Router](/docs/14/app)-Anwendung muss eine [Root-Layout](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required)-Datei enthalten, die eine [React Server Component](/docs/14/app/building-your-application/rendering/server-components) ist und alle Seiten Ihrer Anwendung umschließt. Diese Datei wird auf der obersten Ebene des `app`-Verzeichnisses definiert.
...
> **Gut zu wissen**: Wir werden die [Manifest-Datei](/docs/14/app/api-reference/file-conventions/metadata), zusätzliche Icons außer dem Favicon und die [Testkonfiguration](/docs/14/app/building-your-application/testing) ignorieren, aber wenn diese erforderlich sind, unterstützt Next.js auch diese Optionen.
...
Alle [Metadaten-Dateien](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) wie `favicon.ico`, `icon.png`, `robots.txt` werden automatisch zum `<head>`-Tag der Anwendung hinzugefügt, solange Sie sie auf der obersten Ebene des `app`-Verzeichnisses platziert haben. Nachdem Sie [alle unterstützten Dateien](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) in das `app`-Verzeichnis verschoben haben, können Sie deren `<link>`-Tags sicher löschen:
...
Schließlich kann Next.js Ihre letzten `<head>`-Tags mit der [Metadata API](/docs/14/app/building-your-application/optimizing/metadata) verwalten. Verschieben Sie Ihre letzten Metadaten-Informationen in ein exportiertes [`metadata`-Objekt](/docs/14/app/api-reference/functions/generate-metadata#metadata-object):
...
Dieses Verzeichnis wird als [optionaler Catch-All-Route-Segment](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments) bezeichnet. Next.js verwendet einen dateisystembasierten Router, bei dem [Verzeichnisse verwendet werden, um Routen zu definieren](/docs/14/app/building-your-application/routing/defining-routes#creating-routes). Dieses spezielle Verzeichnis stellt sicher, dass alle Routen Ihrer Anwendung zu der darin enthaltenen `page.tsx`-Datei geleitet werden.
...
Diese Datei ist eine [Server Component](/docs/14/app/building-your-application/rendering/server-components). Wenn Sie `next build` ausführen, wird die Datei in ein statisches Asset vorgerendert. Sie benötigt _keinen_ dynamischen Code.

Diese Datei importiert unser globales CSS und teilt [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) mit, dass wir nur eine Route generieren werden, die Index-Route unter `/`.
...
Diese Datei ist eine [Client Component](/docs/14/app/building-your-application/rendering/client-components), definiert durch die `'use client'`-Direktive. Client-Komponenten werden dennoch [auf dem Server zu HTML vorgerendert](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered), bevor sie an den Client gesendet werden.
...
Next.js unterstützt `.env`-[Umgebungsvariablen](/docs/14/app/building-your-application/configuring/environment-variables) ähnlich wie CRA.
...
Bei der Migration Ihrer CRA-Anwendung zu Next.js haben Sie möglicherweise eine benutzerdefinierte webpack-Konfiguration, die Sie migrieren möchten. Next.js unterstützt die Bereitstellung einer [benutzerdefinierten webpack-Konfiguration](/docs/14/app/api-reference/next-config-js/webpack).

Darüber hinaus unterstützt Next.js [Turbopack](/docs/14/app/api-reference/next-config-js/turbo) über `next dev --turbo`, um die lokale Entwicklungsleistung zu verbessern. Turbopack unterstützt auch einige [webpack-Loader](/docs/14/app/api-reference/next-config-js/turbo) für Kompatibilität und schrittweise Einführung.
...
- Migrieren Sie von React Router zum [Next.js App Router](/docs/14/app/building-your-application/routing), um folgendes zu erhalten:
...
- [Optimieren Sie Bilder mit der `<Image>`-Komponente](/docs/14/app/building-your-application/optimizing/images)
- [Optimieren Sie Schriftarten mit `next/font`](/docs/14/app/building-your-application/optimizing/fonts)
- [Optimieren Sie Drittanbieter-Skripte mit der `<Script>`-Komponente](/docs/14/app/building-your-application/optimizing/scripts)
- [Aktualisieren Sie Ihre ESLint-Konfiguration, um Next.js-Regeln zu unterstützen](/docs/14/app/building-your-application/configuring/eslint)
