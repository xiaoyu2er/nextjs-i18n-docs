...
sidebar:
  order: 2010201
  label: "Abrufen, Cachen und Revalidieren"
---
...
Next.js erweitert die native [`fetch` Web API](https://developer.mozilla.org/docs/Web/API/Fetch_API), um das [Caching](#caching-von-daten) und die [Revalidierung](#revalidierung-von-daten) für jede Fetch-Anfrage auf dem Server konfigurieren zu können. React erweitert `fetch`, um Fetch-Anfragen automatisch zu [memoizen](/docs/14/app/building-your-application/data-fetching/patterns#fetching-data-where-its-needed), während ein React-Komponentenbaum gerendert wird.

Sie können `fetch` mit `async`/`await` in Server Components, [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) und [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) verwenden.
...
> **Wichtig zu wissen**:
>
> - Next.js bietet hilfreiche Funktionen wie [`cookies`](/docs/14/app/api-reference/functions/cookies) und [`headers`](/docs/14/app/api-reference/functions/headers) für den Datenabruf in Server Components. Diese führen zu dynamischem Rendering der Route, da sie auf Informationen zur Anfragezeit angewiesen sind.
> - In Route Handlern werden `fetch`-Anfragen nicht memoized, da Route Handlers nicht Teil des React-Komponentenbaums sind.
> - In [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) werden `fetch`-Anfragen nicht gecached (Standardwert `cache: no-store`).
...
Standardmäßig cached Next.js die Rückgabewerte von `fetch` automatisch im [Data Cache](/docs/14/app/building-your-application/caching#data-cache) auf dem Server. Dies bedeutet, dass die Daten zum Build-Zeitpunkt oder zur Anfragezeit abgerufen, gecached und bei jeder Datenanfrage wiederverwendet werden können.
...
- Sie in einer [Server Action](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) verwendet werden.
- Sie in einem [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) mit der `POST`-Methode verwendet werden.
...
>
> Erfahren Sie mehr über den [Data Cache](/docs/14/app/building-your-application/caching#data-cache).
...
Alternativ können Sie die [Segment Config Options](/docs/14/app/api-reference/file-conventions/route-segment-config) verwenden, um alle `fetch`-Anfragen in einem Routensegment zu revalidieren.
...
Erfahren Sie mehr über [zeitbasierte Revalidierung](/docs/14/app/building-your-application/caching#time-based-revalidation).

#### On-Demand-Revalidierung

Daten können on-demand pfadbasiert ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) oder cache-tag-basiert ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)) in einer [Server Action](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) oder einem [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) revalidiert werden.
...
Erfahren Sie mehr über [On-Demand-Revalidierung](/docs/14/app/building-your-application/caching#on-demand-revalidation).
...
Alle verfügbaren `cache`-Optionen finden Sie in der [`fetch` API-Referenz](/docs/14/app/api-reference/functions/fetch).

#### Mehrere `fetch`-Anfragen

Wenn Sie mehrere `fetch`-Anfragen in einem Routensegment (z.B. einem Layout oder einer Page) haben, können Sie das Caching-Verhalten aller Datenanfragen im Segment mit den [Segment Config Options](/docs/14/app/api-reference/file-conventions/route-segment-config) konfigurieren.
...
Falls Sie eine Drittanbieter-Bibliothek verwenden, die `fetch` nicht unterstützt oder verfügbar macht (z.B. eine Datenbank, ein CMS oder ein ORM-Client), können Sie das Caching- und Revalidierungsverhalten dieser Anfragen mit den [Route Segment Config Options](/docs/14/app/api-reference/file-conventions/route-segment-config) und Reacts `cache`-Funktion konfigurieren.

Ob die Daten gecached werden oder nicht, hängt davon ab, ob das Routensegment [statisch oder dynamisch gerendert](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies) wird. Wenn das Segment statisch ist (Standard), wird die Ausgabe der Anfrage als Teil des Routensegments gecached und revalidiert. Wenn das Segment dynamisch ist, wird die Ausgabe der Anfrage _nicht_ gecached und bei jeder Anfrage neu abgerufen, wenn das Segment gerendert wird.

Sie können auch die experimentelle [`unstable_cache` API](/docs/14/app/api-reference/functions/unstable_cache) verwenden.

### Beispiel

Im folgenden Beispiel:

- Die React `cache`-Funktion wird verwendet, um Datenanfragen zu [memoizen](/docs/14/app/building-your-application/caching#request-memoization).
...
Wenn Sie Daten in einer Client-Komponente abrufen müssen, können Sie einen [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) vom Client aus aufrufen. Route Handler werden auf dem Server ausgeführt und geben die Daten an den Client zurück. Dies ist nützlich, wenn Sie sensible Informationen wie API-Tokens nicht dem Client preisgeben möchten.

Beispiele finden Sie in der [Route Handler](/docs/14/app/building-your-application/routing/route-handlers)-Dokumentation.
