...
Der `useRouter`-Hook ermöglicht es Ihnen, Routen programmatisch in [Client Components](/docs/14/app/building-your-application/rendering/client-components) zu ändern.

> **Empfehlung:** Verwenden Sie die [`<Link>`-Komponente](/docs/14/app/building-your-application/routing/linking-and-navigating#link-component) für die Navigation, es sei denn, Sie haben einen spezifischen Grund, `useRouter` zu verwenden.
...
- `router.prefetch(href: string)`: [Prefetch](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) der angegebenen Route für schnellere clientseitige Übergänge.
...
- Die `pathname`-Zeichenkette wurde entfernt und durch [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname) ersetzt.
- Das `query`-Objekt wurde entfernt und durch [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) ersetzt.
- `router.events` wurde ersetzt. [Siehe unten.](#router-events)

[Vollständige Migrationsanleitung anzeigen](/docs/14/app/building-your-application/upgrading/app-router-migration).
...
> **Wissenswert**: `<NavigationEvents>` ist in eine [`Suspense`-Boundary](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#example) eingeschlossen, weil [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) während des [statischen Renderings](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) clientseitiges Rendering bis zur nächsten `Suspense`-Boundary verursacht. [Mehr erfahren](/docs/14/app/api-reference/functions/use-search-params#behavior).
