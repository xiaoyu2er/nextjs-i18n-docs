...
Um sie zu verwenden, platzieren Sie die Datei im **Stammverzeichnis** Ihrer Anwendung oder in einem [`src`-Ordner](/docs/14/app/building-your-application/configuring/src-directory), falls Sie einen verwenden.

## Konfigurationsoption

Instrumentation ist derzeit ein experimentelles Feature. Um die `instrumentation`-Datei zu verwenden, müssen Sie sich explizit dafür entscheiden, indem Sie [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) in Ihrer `next.config.js` definieren:
