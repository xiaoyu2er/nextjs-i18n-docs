...
| -------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| `conf`         | `Object`           | Dasselbe Objekt, das Sie in [next.config.js](/docs/14/pages/api-reference/next-config-js) verwenden würden. Standardmäßig `{}` |
...
> Möglicherweise möchten Sie auch den clientseitigen Router so konfigurieren, dass clientseitige Weiterleitungen zu Dateinamen-Routen nicht erlaubt sind; siehe dazu [`router.beforePopState`](/docs/14/pages/api-reference/functions/use-router#routerbeforepopstate).