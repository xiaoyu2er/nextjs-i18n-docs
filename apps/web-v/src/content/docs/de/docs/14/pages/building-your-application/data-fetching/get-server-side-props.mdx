...
Wenn Sie die Daten nicht zur Request-Zeit abrufen müssen oder die Daten und das vorgerenderte HTML zwischengespeichert werden sollen, empfehlen wir die Verwendung von [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props).
...
- Wenn ein Benutzer die Seite über [`next/link`](/docs/14/pages/api-reference/components/link) oder [`next/router`](/docs/14/pages/api-reference/functions/use-router) aufruft, sendet Next.js einen API-Request an den Server, der `getServerSideProps` ausführt.
- Sie müssen keine Next.js-[API-Route](/docs/14/pages/building-your-application/routing/api-routes) aufrufen, um Daten abzurufen, wenn Sie `getServerSideProps` verwenden, da die Funktion auf dem Server läuft. Stattdessen können Sie direkt aus `getServerSideProps` heraus ein CMS, eine Datenbank oder andere externe APIs ansprechen.

> **Gut zu wissen:**
>
> - Siehe die [`getServerSideProps`-API-Referenz](/docs/14/pages/api-reference/functions/get-server-side-props) für Parameter und Props, die mit `getServerSideProps` verwendet werden können.
...
Wenn in `getServerSideProps` ein Fehler auftritt, wird die Datei `pages/500.js` angezeigt. Lesen Sie die Dokumentation zur [500-Seite](/docs/14/pages/building-your-application/routing/custom-error#500-page), um mehr über deren Erstellung zu erfahren. Während der Entwicklung wird diese Datei nicht verwendet und stattdessen das Entwicklungs-Fehler-Overlay angezeigt.

## Besondere Fälle

### Edge Runtime

`getServerSideProps` kann sowohl mit [Serverless- als auch Edge-Runtimes](/docs/14/pages/building-your-application/rendering/edge-and-nodejs-runtimes) verwendet werden, und Sie können in beiden Props setzen.
...
Bevor Sie jedoch `cache-control` verwenden, empfehlen wir zu prüfen, ob [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) mit [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) besser für Ihren Anwendungsfall geeignet ist.