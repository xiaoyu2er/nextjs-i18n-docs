...
Wenn `getServerSideProps` oder `getInitialProps` auf einer Seite vorhanden ist, wird Next.js auf bedarfsgerechtes Rendering pro Anfrage umschalten (d.h. [Server-Side Rendering](/docs/14/pages/building-your-application/rendering/server-side-rendering)).
...
- Die Seite ist eine [dynamische Route](/docs/14/pages/building-your-application/routing/dynamic-routes).
- Die Seite hat Query-Werte in der URL.
- In Ihrer `next.config.js` sind [Rewrites](/docs/14/pages/api-reference/next-config-js/rewrites) konfiguriert, da diese Parameter enthalten können, die geparst und in der `query` bereitgestellt werden müssen.

Um festzustellen, ob die Query vollständig aktualisiert und einsatzbereit ist, können Sie das `isReady`-Feld von [`next/router`](/docs/14/pages/api-reference/functions/use-router#router-object) nutzen.

> **Wissenswert**: Parameter, die mit [dynamischen Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) zu einer Seite hinzugefügt werden, die [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) verwendet, sind immer im `query`-Objekt verfügbar.
...
- Wenn Sie eine [benutzerdefinierte `App`](/docs/14/pages/building-your-application/routing/custom-app) mit `getInitialProps` haben, wird diese Optimierung für Seiten ohne [Static Generation](/docs/14/pages/building-your-application/data-fetching/get-static-props) deaktiviert.
- Wenn Sie eine [benutzerdefinierte `Document`](/docs/14/pages/building-your-application/routing/custom-document) mit `getInitialProps` haben, stellen Sie sicher, dass Sie prüfen, ob `ctx.req` definiert ist, bevor Sie davon ausgehen, dass die Seite server-seitig gerendert wird. `ctx.req` ist `undefined` für vorgerenderte Seiten.
- Vermeiden Sie die Verwendung des `asPath`-Werts von [`next/router`](/docs/14/pages/api-reference/functions/use-router#router-object) im Rendering-Baum, bis das `isReady`-Feld des Routers `true` ist. Statisch optimierte Seiten kennen `asPath` nur auf dem Client und nicht auf dem Server, daher kann seine Verwendung als Prop zu Fehlern führen. Das [`active-class-name`-Beispiel](https://github.com/vercel/next.js/tree/canary/examples/active-class-name) zeigt eine Möglichkeit, `asPath` als Prop zu verwenden.