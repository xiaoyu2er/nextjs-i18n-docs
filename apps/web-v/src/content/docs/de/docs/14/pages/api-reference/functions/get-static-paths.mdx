...
Wenn Sie eine Funktion namens `getStaticPaths` aus einer Seite exportieren, die [Dynamische Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) verwendet, wird Next.js alle von `getStaticPaths` angegebenen Pfade statisch vor-rendern.
...
Der `paths`-Schlüssel bestimmt, welche Pfade vorgerendert werden. Angenommen, Sie haben eine Seite mit [Dynamischen Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) namens `pages/posts/[id].js`. Wenn Sie `getStaticPaths` von dieser Seite exportieren und Folgendes für `paths` zurückgeben:
...
- Wenn der Seitenname [Catch-all-Routen](/docs/14/pages/building-your-application/routing/dynamic-routes#catch-all-segments) wie `pages/[...slug]` verwendet, sollte `params` `slug` enthalten (was ein Array ist). Wenn dieses Array `['hello', 'world']` ist, generiert Next.js die Seite statisch unter `/hello/world`.
- Wenn die Seite eine [optionale Catch-all-Route](/docs/14/pages/building-your-application/routing/dynamic-routes#optional-catch-all-segments) verwendet, verwenden Sie `null`, `[]`, `undefined` oder `false`, um die Root-Route zu rendern. Wenn Sie beispielsweise `slug: false` für `pages/[[...slug]]` angeben, generiert Next.js die Seite `/` statisch.
...
Unabhängig vom `params`-Objekt kann ein `locale`-Feld zurückgegeben werden, wenn [i18n konfiguriert](/docs/14/pages/building-your-application/routing/internationalization) ist, wodurch das Locale für den generierten Pfad konfiguriert wird.
...
> **Gut zu wissen**: `fallback: true` wird nicht unterstützt, wenn [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) verwendet wird.
...
`fallback: true` wird generierte Seiten nicht _aktualisieren_. Dafür siehe [Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
`fallback: 'blocking'` aktualisiert generierte Seiten standardmäßig nicht. Um generierte Seiten zu aktualisieren, verwenden Sie [Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) in Verbindung mit `fallback: 'blocking'`.

> **Gut zu wissen**: `fallback: 'blocking'` wird nicht unterstützt, wenn [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) verwendet wird.
...
- Mit dem [Router](/docs/14/pages/api-reference/functions/use-router) können Sie erkennen, ob der Fallback gerendert wird: `router.isFallback` wird `true` sein.
...
| Version   | Änderungen                                                                                                                                                                                                 |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/14/app/building-your-application/data-fetching) ist jetzt stabil mit vereinfachtem Data Fetching, einschließlich [`generateStaticParams()`](/docs/14/app/api-reference/functions/generate-static-params) |
| `v12.2.0` | [On-Demand Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) ist stabil.                                    |
| `v12.1.0` | [On-Demand Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) hinzugefügt (beta).                            |
| `v9.5.0`  | Stabile [Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration)                                                                        |
