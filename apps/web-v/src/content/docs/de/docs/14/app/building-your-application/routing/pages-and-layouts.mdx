...
> Wir empfehlen, die Seiten [Routing-Grundlagen](/docs/14/app/building-your-application/routing) und [Routen definieren](/docs/14/app/building-your-application/routing/defining-routes) zu lesen, bevor Sie fortfahren.

Die speziellen Dateien [layout.js](#layouts), [page.js](#pages) und [template.js](#templates) ermöglichen es Ihnen, Benutzeroberflächen für eine [Route](/docs/14/app/building-your-application/routing/defining-routes#creating-routes) zu erstellen. Diese Seite führt Sie durch die Verwendung dieser speziellen Dateien.
...
> - Eine Seite ist immer das [Blatt](/docs/14/app/building-your-application/routing#terminology) des [Route-Unterbaums](/docs/14/app/building-your-application/routing#terminology).
> - Eine `page.js`-Datei ist erforderlich, um ein Routensegment öffentlich zugänglich zu machen.
> - Seiten sind standardmäßig [Server Components](/docs/14/app/building-your-application/rendering/server-components), können aber als [Client Component](/docs/14/app/building-your-application/rendering/client-components) festgelegt werden.
> - Seiten können Daten abrufen. Weitere Informationen finden Sie im Abschnitt [Datenabruf](/docs/14/app/building-your-application/data-fetching).
...
> - Layouts sind standardmäßig [Server Components](/docs/14/app/building-your-application/rendering/server-components), können aber als [Client Component](/docs/14/app/building-your-application/rendering/client-components) festgelegt werden.
> - Layouts können Daten abrufen. Weitere Informationen finden Sie im Abschnitt [Datenabruf](/docs/14/app/building-your-application/data-fetching).
> - Die Weitergabe von Daten zwischen einem übergeordneten Layout und seinen untergeordneten Elementen ist nicht möglich. Sie können jedoch dieselben Daten in einer Route mehrmals abrufen, und React wird die Anfragen [automatisch deduplizieren](/docs/14/app/building-your-application/caching#request-memoization), ohne die Leistung zu beeinträchtigen.
> - Layouts haben keinen Zugriff auf die darunter liegenden Routensegmente. Um auf alle Routensegmente zuzugreifen, können Sie [`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) oder [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) in einer Client-Komponente verwenden.
> - Sie können [Route Groups](/docs/14/app/building-your-application/routing/route-groups) verwenden, um bestimmte Routensegmente in gemeinsame Layouts einzubeziehen oder auszuschließen.
> - Sie können [Route Groups](/docs/14/app/building-your-application/routing/route-groups) verwenden, um mehrere Root-Layouts zu erstellen. Siehe ein [Beispiel hier](/docs/14/app/building-your-application/routing/route-groups#creating-multiple-root-layouts).
> - **Migration vom `pages`-Verzeichnis:** Das Root-Layout ersetzt die Dateien [`_app.js`](/docs/14/pages/building-your-application/routing/custom-app) und [`_document.js`](/docs/14/pages/building-your-application/routing/custom-document). [Siehe Migrationsleitfaden](/docs/14/app/building-your-application/upgrading/app-router-migration#migrating-_documentjs-and-_appjs).
...
Im `app`-Verzeichnis können Sie HTML-Elemente wie `title` und `meta` im `<head>` mit den [Metadaten-APIs](/docs/14/app/building-your-application/optimizing/metadata) ändern.

Metadaten können definiert werden, indem ein [`metadata`-Objekt](/docs/14/app/api-reference/functions/generate-metadata#the-metadata-object) oder eine [`generateMetadata`-Funktion](/docs/14/app/api-reference/functions/generate-metadata#generatemetadata-function) in einer [`layout.js`](/docs/14/app/api-reference/file-conventions/layout)- oder [`page.js`](/docs/14/app/api-reference/file-conventions/page)-Datei exportiert wird.
...
> **Wissenswert**: Sie sollten `<head>`-Tags wie `<title>` und `<meta>` nicht manuell zu Root-Layouts hinzufügen. Verwenden Sie stattdessen die [Metadaten-API](/docs/14/app/api-reference/functions/generate-metadata), die erweiterte Anforderungen wie Streaming und Deduplizierung von `<head>`-Elementen automatisch handhabt.

Erfahren Sie mehr über verfügbare Metadatenoptionen in der [API-Referenz](/docs/14/app/api-reference/functions/generate-metadata)