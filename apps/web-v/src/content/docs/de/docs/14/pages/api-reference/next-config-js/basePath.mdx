...
Bei Verwendung der [`next/image`](/docs/14/app/api-reference/components/image)-Komponente müssen Sie den `basePath` vor das `src`-Attribut setzen.

</AppOnly>

<PagesOnly>

Bei Verwendung der [`next/image`](/docs/14/pages/api-reference/components/image)-Komponente müssen Sie den `basePath` vor das `src`-Attribut setzen.
