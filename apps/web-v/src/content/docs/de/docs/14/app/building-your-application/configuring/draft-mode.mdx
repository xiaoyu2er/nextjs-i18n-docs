...
Erstellen Sie zunächst einen [Route Handler](/docs/14/app/building-your-application/routing/route-handlers). Er kann beliebig benannt werden, z.B. `app/api/draft/route.ts`.
...
Rufen Sie dann `/api/disable-draft` auf, um den Route Handler auszuführen. Wenn Sie diese Route mit [`next/link`](/docs/14/app/api-reference/components/link) aufrufen, müssen Sie `prefetch={false}` übergeben, um ein versehentliches Löschen des Cookies beim Prefetch zu verhindern.
