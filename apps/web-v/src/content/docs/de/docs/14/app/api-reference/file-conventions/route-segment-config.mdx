...
translation-updated-at: 2025-06-06T17:35:09.738Z
...
Die Routensegment-Optionen ermöglichen es Ihnen, das Verhalten einer [Seite](/docs/14/app/building-your-application/routing/pages-and-layouts), eines [Layouts](/docs/14/app/building-your-application/routing/pages-and-layouts) oder eines [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) zu konfigurieren, indem Sie die folgenden Variablen direkt exportieren:
...
- **`'force-dynamic'`**: Erzwingt [dynamisches Rendering](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering), wodurch Routen bei jeder Anfrage für jeden Benutzer gerendert werden. Diese Option entspricht `getServerSideProps()` im `pages`-Verzeichnis.

- **`'error'`**: Erzwingt statisches Rendering und cacht die Daten eines Layouts oder einer Seite, indem ein Fehler ausgelöst wird, wenn Komponenten [dynamische Funktionen](/docs/14/app/building-your-application/rendering/server-components#dynamic-functions) oder ungecachte Daten verwenden. Diese Option entspricht:
...
- **`'force-static'`**: Erzwingt statisches Rendering und cacht die Daten eines Layouts oder einer Seite, indem [`cookies()`](/docs/14/app/api-reference/functions/cookies), [`headers()`](/docs/14/app/api-reference/functions/headers) und [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) gezwungen werden, leere Werte zurückzugeben.

> **Wichtig zu wissen**:
>
> - Anleitungen zur [Migration](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods) von `getServerSideProps` und `getStaticProps` zu `dynamic: 'force-dynamic'` und `dynamic: 'error'` finden Sie im [Upgrade-Guide](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods).

### `dynamicParams`

Steuert, was passiert, wenn ein dynamisches Segment aufgerufen wird, das nicht mit [generateStaticParams](/docs/14/app/api-reference/functions/generate-static-params) generiert wurde.
...
> - Wenn `dynamicParams = true` ist, verwendet das Segment [Streaming Server Rendering](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense).
...
Erfahren Sie mehr über die [Edge- und Node.js-Runtimes](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
> **Wichtig zu wissen**:
>
> - Wenn Sie [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) verwenden, setzen Sie `maxDuration` auf Seitenebene, um das Standard-Timeout aller auf der Seite verwendeten Server Actions zu ändern.

### `generateStaticParams`

Die Funktion `generateStaticParams` kann in Kombination mit [dynamischen Routensegmenten](/docs/14/app/building-your-application/routing/dynamic-routes) verwendet werden, um die Liste der Routensegmentparameter zu definieren, die statisch zur Build-Zeit statt bei Bedarf zur Anfragezeit generiert werden.

Weitere Details finden Sie in der [API-Referenz](/docs/14/app/api-reference/functions/generate-static-params).