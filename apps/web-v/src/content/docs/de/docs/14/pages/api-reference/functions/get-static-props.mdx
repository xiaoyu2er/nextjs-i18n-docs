...
| Name               | Beschreibung                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`           | Enthält die Routenparameter für Seiten mit [dynamischen Routen](/docs/14/pages/building-your-application/routing/dynamic-routes). Wenn der Seitenname beispielsweise `[id].js` ist, sieht `params` wie `{ id: ... }` aus. Dies sollte zusammen mit `getStaticPaths` verwendet werden, was wir später erklären werden.                                                                                                                                  |
| `preview`          | (Veraltet für `draftMode`) `preview` ist `true`, wenn die Seite sich im [Preview Mode](/docs/14/pages/building-your-application/configuring/preview-mode) befindet, andernfalls `false`.                                                                                                                                                                                                                                                              |
| `previewData`      | (Veraltet für `draftMode`) Die durch `setPreviewData` gesetzten [Preview](/docs/14/pages/building-your-application/configuring/preview-mode)-Daten.                                                                                                                                                                                                                                                                                                   |
| `draftMode`        | `draftMode` ist `true`, wenn die Seite sich im [Draft Mode](/docs/14/pages/building-your-application/configuring/draft-mode) befindet, andernfalls `false`.                                                                                                                                                                                                                                                                                           |
...
Erfahren Sie mehr über [Inkrementelle Statische Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
Der `notFound`-Boolean ermöglicht es der Seite, einen `404`-Status und eine [404-Seite](/docs/14/pages/building-your-application/routing/custom-error#404-page) zurückzugeben. Mit `notFound: true` gibt die Seite einen `404` zurück, selbst wenn zuvor eine erfolgreich generierte Seite vorhanden war. Dies dient zur Unterstützung von Anwendungsfällen wie benutzergenerierten Inhalten, die vom Autor entfernt wurden. Beachten Sie, dass `notFound` demselben `revalidate`-Verhalten folgt, [das hier beschrieben ist](#revalidate).
...
> **Gut zu wissen**: `notFound` ist nicht erforderlich für den [`fallback: false`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-false)-Modus, da nur Pfade, die von `getStaticPaths` zurückgegeben werden, vorgerendert werden.
...
Wenn die Weiterleitungen zur Build-Zeit bekannt sind, sollten sie stattdessen in [`next.config.js`](/docs/14/pages/api-reference/next-config-js/redirects) hinzugefügt werden.
...
| Version   | Änderungen                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/14/app/building-your-application/data-fetching) ist jetzt stabil mit vereinfachtem Data Fetching                                                           |
| `v12.2.0` | [On-Demand Incremental Static Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) ist stabil.    |
| `v12.1.0` | [On-Demand Incremental Static Regeneration](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) hinzugefügt (Beta). |
