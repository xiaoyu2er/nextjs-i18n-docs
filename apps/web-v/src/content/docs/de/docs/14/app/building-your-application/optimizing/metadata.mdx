...
- **Konfigurationsbasierte Metadaten**: Exportieren Sie ein [statisches `metadata`-Objekt](/docs/14/app/api-reference/functions/generate-metadata#metadata-object) oder eine dynamische [`generateMetadata`-Funktion](/docs/14/app/api-reference/functions/generate-metadata#generatemetadata-function) in einer `layout.js`- oder `page.js`-Datei.
- **Dateibasierte Metadaten**: Fügen Sie statische oder dynamisch generierte spezielle Dateien zu Routensegmenten hinzu.

Mit beiden Optionen generiert Next.js automatisch die relevanten `<head>`-Elemente für Ihre Seiten. Sie können auch dynamische OG-Bilder mit dem [`ImageResponse`](/docs/14/app/api-reference/functions/image-response)-Konstruktor erstellen.

## Statische Metadaten

Um statische Metadaten zu definieren, exportieren Sie ein [`Metadata`-Objekt](/docs/14/app/api-reference/functions/generate-metadata#metadata-object) aus einer `layout.js`- oder statischen `page.js`-Datei.
...
Alle verfügbaren Optionen finden Sie in der [API-Referenz](/docs/14/app/api-reference/functions/generate-metadata).
...
Alle verfügbaren Parameter finden Sie in der [API-Referenz](/docs/14/app/api-reference/functions/generate-metadata).

> **Gut zu wissen**:
>
> - Sowohl statische als auch dynamische Metadaten über `generateMetadata` werden **nur in Server Components** unterstützt.
> - `fetch`-Anfragen werden automatisch [memoisiert](/docs/14/app/building-your-application/caching#request-memoization) für dieselben Daten über `generateMetadata`, `generateStaticParams`, Layouts, Pages und Server Components hinweg. React [`cache` kann verwendet werden](/docs/14/app/building-your-application/caching#request-memoization), falls `fetch` nicht verfügbar ist.
> - Next.js wartet, bis das Datenabrufen innerhalb von `generateMetadata` abgeschlossen ist, bevor die Benutzeroberfläche an den Client gestreamt wird. Dies stellt sicher, dass der erste Teil einer [gestreamten Antwort](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) die `<head>`-Tags enthält.

## Dateibasierte Metadaten

Diese speziellen Dateien sind für Metadaten verfügbar:

- [favicon.ico, apple-icon.jpg und icon.jpg](/docs/14/app/api-reference/file-conventions/metadata/app-icons)
- [opengraph-image.jpg und twitter-image.jpg](/docs/14/app/api-reference/file-conventions/metadata/opengraph-image)
...
Für Implementierung und Beispiele siehe die [Metadata Files](/docs/14/app/api-reference/file-conventions/metadata) API-Referenz und [Dynamische Bildgenerierung](#dynamische-bildgenerierung).
...
> **Gut zu wissen**: Sie können das standardmäßige [`viewport`](/docs/14/app/api-reference/functions/generate-metadata#viewport)-Meta-Tag überschreiben.
...
Das bedeutet, dass Metadaten mit verschachtelten Feldern wie [`openGraph`](/docs/14/app/api-reference/functions/generate-metadata#opengraph) und [`robots`](/docs/14/app/api-reference/functions/generate-metadata#robots), die in einem früheren Segment definiert wurden, durch das letzte Segment, das sie definiert, **überschrieben** werden.
...
`ImageResponse` verwendet die [Edge Runtime](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime), und Next.js fügt automatisch die richtigen Header für zwischengespeicherte Bilder am Edge hinzu, was die Leistung verbessert und Neuberechnungen reduziert.
...
`ImageResponse` integriert sich gut mit anderen Next.js-APIs, einschließlich [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) und dateibasierten Metadaten. Beispielsweise können Sie `ImageResponse` in einer `opengraph-image.tsx`-Datei verwenden, um Open-Graph-Bilder zur Build-Zeit oder dynamisch zur Anfragezeit zu generieren.

`ImageResponse` unterstützt gängige CSS-Eigenschaften, einschließlich Flexbox und absolute Positionierung, benutzerdefinierte Schriftarten, Textumbruch, Zentrierung und verschachtelte Bilder. [Siehe die vollständige Liste der unterstützten CSS-Eigenschaften](/docs/14/app/api-reference/functions/image-response).
