...
Die `default.js`-Datei wird verwendet, um einen Fallback innerhalb von [Parallel Routes](/docs/14/app/building-your-application/routing/parallel-routes) zu rendern, wenn Next.js den aktiven [Slot-Zustand](/docs/14/app/building-your-application/routing/parallel-routes#slots) nach einem vollständigen Seitenladen nicht wiederherstellen kann.

Während der [Soft Navigation](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation) behält Next.js den aktiven _Zustand_ (Unterseite) für jeden Slot im Auge. Bei Hard Navigations (vollständiger Seitenladen) kann Next.js den aktiven Zustand jedoch nicht wiederherstellen. In diesem Fall kann eine `default.js`-Datei für Unterseiten gerendert werden, die nicht mit der aktuellen URL übereinstimmen.
...
Ein Objekt, das die [dynamischen Routenparameter](/docs/14/app/building-your-application/routing/dynamic-routes) vom Wurzel-Segment bis zu den Unterseiten des Slots enthält. Zum Beispiel:
