...
- Der Import `next/image` wurde in `next/legacy/image` umbenannt. Der Import `next/future/image` wurde in `next/image` umbenannt. Ein [Codemod ist verfügbar](/docs/14/pages/building-your-application/upgrading/codemods#next-image-to-legacy-image), um Ihre Importe sicher und automatisch umzubenennen.
- Das Kindelement von `next/link` kann nicht mehr `<a>` sein. Fügen Sie die Eigenschaft `legacyBehavior` hinzu, um das alte Verhalten zu verwenden, oder entfernen Sie `<a>`, um zu aktualisieren. Ein [Codemod ist verfügbar](/docs/14/pages/building-your-application/upgrading/codemods#new-link), um Ihren Code automatisch zu aktualisieren.
- Die Konfigurationseigenschaft `target` wurde entfernt und durch [Output File Tracing](/docs/14/pages/api-reference/next-config-js/output) ersetzt.

## Migration gemeinsamer Funktionen

Next.js 13 führt ein neues [`app`-Verzeichnis](/docs/14/app/building-your-application/routing) mit neuen Funktionen und Konventionen ein. Die Aktualisierung auf Next.js 13 erfordert jedoch **nicht** die Verwendung des neuen [`app`-Verzeichnisses](/docs/14/app/building-your-application/routing#the-app-router).
...
Um Ihre Links auf Next.js 13 zu aktualisieren, können Sie den [`new-link`-Codemod](/docs/14/pages/building-your-application/upgrading/codemods#new-link) verwenden.

### `<Script>`-Komponente

Das Verhalten von [`next/script`](/docs/14/pages/api-reference/components/script) wurde aktualisiert, um sowohl `pages` als auch `app` zu unterstützen. Wenn Sie `app` schrittweise einführen, lesen Sie die [Upgrade-Anleitung](/docs/14/pages/building-your-application/upgrading).

### Schriftoptimierung

Bisher half Next.js Ihnen, Schriften zu optimieren, indem CSS für Schriften inline eingebettet wurde. Version 13 führt das neue [`next/font`](/docs/14/pages/building-your-application/optimizing/fonts)-Modul ein, das Ihnen die Möglichkeit gibt, Ihre Schriftladeerfahrung anzupassen und gleichzeitig eine hervorragende Leistung und Privatsphäre zu gewährleisten.

Weitere Informationen zur Verwendung von `next/font` finden Sie unter [Optimierung von Schriften](/docs/14/pages/building-your-application/optimizing/fonts).