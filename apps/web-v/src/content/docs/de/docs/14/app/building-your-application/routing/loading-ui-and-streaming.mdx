...
> **Wissenswert**:
>
> - Die Navigation ist sofort, auch mit [serverzentriertem Routing](/docs/14/app/building-your-application/routing/linking-and-navigating#how-routing-and-navigation-works).
...
Zusätzlich zu `loading.js` können Sie auch manuell Suspense-Grenzen für Ihre eigenen UI-Komponenten erstellen. Der App Router unterstützt Streaming mit [Suspense](https://react.dev/reference/react/Suspense) für beide [Node.js- und Edge-Runtimes](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes).
...
- Next.js wartet auf den Abschluss der Datenabfrage in [`generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata), bevor UI an den Client gestreamt wird. Dies stellt sicher, dass der erste Teil einer gestreamten Antwort `<head>`-Tags enthält.
