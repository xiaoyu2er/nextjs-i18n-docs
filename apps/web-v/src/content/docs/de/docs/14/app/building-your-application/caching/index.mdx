...
>
> Weitere Informationen finden Sie in der [Server Components](/docs/14/app/building-your-application/rendering/server-components)-Dokumentation.
...
Erfahren Sie mehr über [statisches und dynamisches Rendering](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies).
...
- **[Daten-Revalidierung](/docs/14/app/building-your-application/caching#revalidating)**: Die Revalidierung des [Data Cache](#data-cache) invalidiert wiederum den Router Cache, indem Komponenten auf dem Server neu gerendert und das neue Render-Ergebnis gecached wird.
...
Wenn ein Benutzer zwischen Routen navigiert, cached Next.js besuchte Routensegmente und [lädt Routen vorab](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching), zu denen der Benutzer wahrscheinlich navigieren wird (basierend auf `<Link>`-Komponenten in seinem Viewport).

Dies führt zu einem verbesserten Navigationserlebnis für den Benutzer:

- Sofortige Vorwärts-/Rückwärtsnavigation, weil besuchte Routen gecached sind, und schnelle Navigation zu neuen Routen aufgrund von Prefetching und [partiellem Rendering](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering).
...
- **Automatischer Invalidierungszeitraum**: Der Cache von Layouts und Ladezuständen wird automatisch nach einem bestimmten Zeitraum invalidiert. Die Dauer hängt davon ab, wie die Ressource [vorab geladen](/docs/14/app/api-reference/components/link#prefetch) wurde und ob die Ressource [statisch generiert](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) wurde:
...
> **Gut zu wissen**: Die experimentelle [`staleTimes`](/docs/14/app/api-reference/next-config-js/staleTimes)-Konfigurationsoption kann verwendet werden, um die oben genannten automatischen Invalidierungszeiten anzupassen.
...
- In einer **Server-Aktion**:
  - Daten-On-Demand-Revalidierung nach Pfad mit ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) oder nach Cache-Tag mit ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag))
  - Die Verwendung von [`cookies.set`](/docs/14/app/api-reference/functions/cookies#cookiessetname-value-options) oder [`cookies.delete`](/docs/14/app/api-reference/functions/cookies#deleting-cookies) invalidiert den Router Cache, um zu verhindern, dass Routen, die Cookies verwenden, veraltet werden (z.B. Authentifizierung).
- Der Aufruf von [`router.refresh`](/docs/14/app/api-reference/functions/use-router) invalidiert den Router Cache und stellt eine neue Anfrage an den Server für die aktuelle Route.

### Opt-out

Es ist nicht möglich, den Router Cache zu deaktivieren. Sie können ihn jedoch durch Aufruf von [`router.refresh`](/docs/14/app/api-reference/functions/use-router), [`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) oder [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag) invalidieren (siehe oben). Dadurch wird der Cache gelöscht und eine neue Anfrage an den Server gestellt, um sicherzustellen, dass die neuesten Daten angezeigt werden.
...
- Die Revalidierung des Data Cache in einem [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) **wird** den Router Cache nicht sofort invalidieren, da der Route Handler nicht an eine bestimmte Route gebunden ist. Das bedeutet, der Router Cache wird weiterhin das vorherige Payload bereitstellen, bis ein Hard Refresh durchgeführt wird oder der automatische Invalidierungszeitraum abgelaufen ist.
- Um den Data Cache und Router Cache sofort zu invalidieren, können Sie [`revalidatePath`](#revalidatepath) oder [`revalidateTag`](#fetch-optionsnexttags-and-revalidatetag) in einer [Server-Aktion](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) verwenden.
...
Erfahren Sie mehr über die [`<Link>`-Komponente](/docs/14/app/api-reference/components/link).
...
Siehe die [`useRouter`-Hook](/docs/14/app/api-reference/functions/use-router)-API-Referenz.
...
Siehe die [`fetch`-API-Referenz](/docs/14/app/api-reference/functions/fetch) für weitere Optionen.
...
1. Bei der Verwendung von `fetch` oder [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache) können Sie Cache-Einträge mit einem oder mehreren Tags versehen.
...
1. [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) - um Daten als Reaktion auf ein externes Ereignis (z.B. Webhook) zu revalidieren. Dies wird den Router Cache nicht sofort ungültig machen, da der Route Handler nicht an eine bestimmte Route gebunden ist.
2. [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - um Daten nach einer Benutzeraktion (z.B. Formularübermittlung) zu revalidieren. Dies wird den Router Cache für die zugehörige Route ungültig machen.
...
1. [Route Handler](/docs/14/app/building-your-application/routing/route-handlers) - um Daten als Reaktion auf ein externes Ereignis (z.B. Webhook) zu revalidieren.
2. [Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - um Daten nach einer Benutzerinteraktion (z.B. Formularübermittlung, Klicken eines Buttons) zu revalidieren.

Weitere Informationen finden Sie in der [`revalidatePath` API-Referenz](/docs/14/app/api-reference/functions/revalidatePath).
...
Weitere Informationen finden Sie in der [`cookies`](/docs/14/app/api-reference/functions/cookies) API-Referenz.
...
Weitere Optionen finden Sie in der [Route Segment Config](/docs/14/app/api-reference/file-conventions/route-segment-config)-Dokumentation.

### `generateStaticParams`

Für [dynamische Segmente](/docs/14/app/building-your-application/routing/dynamic-routes) (z.B. `app/blog/[slug]/page.js`) werden Pfade, die von `generateStaticParams` bereitgestellt werden, zur Build-Zeit im Full Route Cache gespeichert. Zur Laufzeit werden auch Pfade, die zur Build-Zeit nicht bekannt waren, beim ersten Aufruf im Cache gespeichert.

Sie können das Caching zur Laufzeit deaktivieren, indem Sie die Option `export const dynamicParams = false` in einem Routen-Segment verwenden. Wenn diese Konfigurationsoption verwendet wird, werden nur die von `generateStaticParams` bereitgestellten Pfade bedient, andere Routen führen zu einem 404-Fehler oder werden gematcht (im Fall von [Catch-all-Routen](/docs/14/app/building-your-application/routing/dynamic-routes#catch-all-segments)).

Weitere Informationen finden Sie in der [`generateStaticParams` API-Referenz](/docs/14/app/api-reference/functions/generate-static-params).
