...
Es gibt keine speziellen APIs, um eine neue Zone zu definieren. Eine Zone ist eine normale Next.js-Anwendung, in der Sie auch einen [basePath](/docs/14/app/api-reference/next-config-js/basePath) konfigurieren, um Konflikte mit Seiten und statischen Dateien in anderen Zonen zu vermeiden.
