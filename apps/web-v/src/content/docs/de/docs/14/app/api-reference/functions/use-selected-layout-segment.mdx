...
> **Wissenswert**:
>
> - Da `useSelectedLayoutSegment` ein [Client Component](/docs/14/app/building-your-application/rendering/client-components)-Hook ist und Layouts standardmäßig [Server Components](/docs/14/app/building-your-application/rendering/server-components) sind, wird `useSelectedLayoutSegment` normalerweise über eine Client Component aufgerufen, die in ein Layout importiert wird.
> - `useSelectedLayoutSegment` gibt nur das Segment eine Ebene darunter zurück. Um alle aktiven Segmente zurückzugeben, siehe [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments)
...
`useSelectedLayoutSegment` akzeptiert optional einen [`parallelRoutesKey`](/docs/14/app/building-your-application/routing/parallel-routes#useselectedlayoutsegments), der es Ihnen ermöglicht, das aktive Routensegment innerhalb dieses Slots zu lesen.
