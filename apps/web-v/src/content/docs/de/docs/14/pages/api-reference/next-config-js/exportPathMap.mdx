...
translation-updated-at: 2025-06-06T17:19:36.459Z
title: exportPathMap
description: Anpassung der Seiten, die beim Einsatz von `next export` als HTML-Dateien exportiert werden.
...
`exportPathMap` ermöglicht es Ihnen, eine Zuordnung von Anfragepfaden zu Seitenzielen anzugeben, die während des Exports verwendet wird. Pfade, die in `exportPathMap` definiert sind, sind auch verfügbar, wenn [`next dev`](/docs/14/app/api-reference/cli/next#next-dev-options) verwendet wird.
...
> **Wissenswert**: Das `query`-Feld in `exportPathMap` kann nicht mit [automatisch statisch optimierten Seiten](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) oder [`getStaticProps`-Seiten](/docs/14/pages/building-your-application/data-fetching/get-static-props) verwendet werden, da diese zur Build-Zeit in HTML-Dateien gerendert werden und zusätzliche Abfrageinformationen während `next export` nicht bereitgestellt werden können.
...
- `distDir` - Absoluter Pfad zum `.next/`-Verzeichnis (konfigurierbar mit der [`distDir`](/docs/14/pages/api-reference/next-config-js/distDir)-Konfiguration)
...
[`next export`](/docs/14/app/building-your-application/deploying/static-exports) verwendet standardmäßig `out` als Ausgabeverzeichnis. Sie können dies mit dem `-o`-Argument anpassen, wie folgt:

</AppOnly>

<PagesOnly>

[`next export`](/docs/14/pages/building-your-application/deploying/static-exports) verwendet standardmäßig `out` als Ausgabeverzeichnis. Sie können dies mit dem `-o`-Argument anpassen, wie folgt:
