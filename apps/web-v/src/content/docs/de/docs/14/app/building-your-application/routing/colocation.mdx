...
Abgesehen von den [Routing-Ordner- und Dateikonventionen](/docs/14/getting-started/project-structure#app-routing-conventions) ist Next.js **nicht vorschreibend** darüber, wie Sie Ihre Projektdateien organisieren und kolokalisieren.
...
Im `app`-Verzeichnis definiert die [verschachtelte Ordnerhierarchie](/docs/14/app/building-your-application/routing#route-segments) die Routenstruktur.
...
> - Wenn Sie keine privaten Ordner verwenden, ist es hilfreich, die [speziellen Dateikonventionen](/docs/14/getting-started/project-structure#routing-files) von Next.js zu kennen, um unerwartete Namenskonflikte zu vermeiden.
...
- [Organisation von Routen in Gruppen](/docs/14/app/building-your-application/routing/route-groups#organize-routes-without-affecting-the-url-path), z.B. nach Website-Bereich, Absicht oder Team.
- Ermöglichen verschachtelter Layouts auf demselben Routensegment-Level:
  - [Erstellen mehrerer verschachtelter Layouts im selben Segment, einschließlich mehrerer Root-Layouts](/docs/14/app/building-your-application/routing/route-groups#creating-multiple-root-layouts)
  - [Hinzufügen eines Layouts zu einer Teilmenge von Routen in einem gemeinsamen Segment](/docs/14/app/building-your-application/routing/route-groups#opting-specific-segments-into-a-layout)

### `src`-Verzeichnis

Next.js unterstützt die Speicherung von Anwendungscode (einschließlich `app`) in einem optionalen [`src`-Verzeichnis](/docs/14/app/building-your-application/configuring/src-directory). Dies trennt Anwendungscode von Projektkonfigurationsdateien, die meist im Stammverzeichnis eines Projekts liegen.
...
Next.js unterstützt [Modulpfad-Aliase](/docs/14/app/building-your-application/configuring/absolute-imports-and-module-aliases), die das Lesen und Warten von Importen über tief verschachtelte Projektdateien hinweg erleichtern.
