...
Dynamische Segmente werden als `params`-Prop an die Funktionen [`layout`](/docs/14/app/api-reference/file-conventions/layout), [`page`](/docs/14/app/api-reference/file-conventions/page), [`route`](/docs/14/app/building-your-application/routing/route-handlers) und [`generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata#generatemetadata-function) übergeben.
...
> **Gut zu wissen**: Dynamische Segmente entsprechen [dynamischen Routen](/docs/14/app/building-your-application/routing/dynamic-routes) im `pages`-Verzeichnis.

## Statische Parameter generieren

Die Funktion `generateStaticParams` kann in Kombination mit [dynamischen Routensegmenten](/docs/14/app/building-your-application/routing/dynamic-routes) verwendet werden, um Routen [**statisch**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) zur Build-Zeit statt bei Bedarf zur Laufzeit zu generieren.
...
Der Hauptvorteil der `generateStaticParams`-Funktion ist die intelligente Abfrage von Daten. Wenn Inhalte innerhalb der `generateStaticParams`-Funktion mit einer `fetch`-Anfrage abgerufen werden, werden die Anfragen [automatisch zwischengespeichert](/docs/14/app/building-your-application/caching#request-memoization). Das bedeutet, dass eine `fetch`-Anfrage mit denselben Argumenten über mehrere `generateStaticParams`, Layouts und Seiten hinweg nur einmal durchgeführt wird, was die Build-Zeiten verkürzt.

Verwenden Sie den [Migrationsleitfaden](/docs/14/app/building-your-application/upgrading/app-router-migration#dynamic-paths-getstaticpaths), wenn Sie vom `pages`-Verzeichnis migrieren.

Weitere Informationen und erweiterte Anwendungsfälle finden Sie in der [Dokumentation zur `generateStaticParams`-Serverfunktion](/docs/14/app/api-reference/functions/generate-static-params).
...
> **Gut zu wissen**: Dies könnte in Zukunft automatisch durch das [TypeScript-Plugin](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin) erfolgen.