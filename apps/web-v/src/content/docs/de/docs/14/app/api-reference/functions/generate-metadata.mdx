...
translation-updated-at: 2025-06-06T17:38:19.542Z
...
Diese Seite behandelt alle **konfigurationsbasierten Metadaten**-Optionen mit `generateMetadata` und dem statischen Metadata-Objekt.
...
> - Sie können nicht gleichzeitig das `metadata`-Objekt und die `generateMetadata`-Funktion aus demselben Routensegment exportieren.

## Das `metadata`-Objekt

Um statische Metadaten zu definieren, exportieren Sie ein [`Metadata`-Objekt](#metadata-felder) aus einer `layout.js`- oder `page.js`-Datei.
...
Eine vollständige Liste der unterstützten Optionen finden Sie unter [Metadata-Felder](#metadata-felder).

## `generateMetadata`-Funktion

Dynamische Metadaten, die von **dynamischen Informationen** abhängen, wie z.B. den aktuellen Routenparametern, externen Daten oder `metadata` in übergeordneten Segmenten, können durch Exportieren einer `generateMetadata`-Funktion festgelegt werden, die ein [`Metadata`-Objekt](#metadata-felder) zurückgibt.
...
- `props` - Ein Objekt mit den Parametern der aktuellen Route:
  ...
  - `params` - Ein Objekt mit den [dynamischen Routenparametern](/docs/14/app/building-your-application/routing/dynamic-routes) vom Wurzelsegment bis zu dem Segment, aus dem `generateMetadata` aufgerufen wird. Beispiele:
...
`generateMetadata` sollte ein [`Metadata`-Objekt](#metadata-felder) zurückgeben, das ein oder mehrere Metadatenfelder enthält.

> **Gut zu wissen**:
>
> - Wenn Metadaten nicht von Laufzeitinformationen abhängen, sollten sie mit dem statischen [`metadata`-Objekt](#das-metadata-objekt) definiert werden, nicht mit `generateMetadata`.
> - `fetch`-Anfragen werden automatisch [memoisiert](/docs/14/app/building-your-application/caching#request-memoization) für dieselben Daten über `generateMetadata`, `generateStaticParams`, Layouts, Seiten und Server Components hinweg. React [`cache` kann verwendet werden](/docs/14/app/building-your-application/caching#request-memoization), wenn `fetch` nicht verfügbar ist.
> - `searchParams` sind nur in `page.js`-Segmenten verfügbar.
> - Die Next.js-Methoden [`redirect()`](/docs/14/app/api-reference/functions/redirect) und [`notFound()`](/docs/14/app/api-reference/functions/not-found) können auch innerhalb von `generateMetadata` verwendet werden.

## Metadata-Felder

### `title`

Das `title`-Attribut wird verwendet, um den Titel des Dokuments festzulegen. Es kann als einfache [Zeichenkette](#zeichenkette) oder als optionales [Template-Objekt](#template-objekt) definiert werden.
...
`title.template` kann verwendet werden, um `titles` in **untergeordneten** Routensegmenten ein Präfix oder Suffix hinzuzufügen.
...
> - `layout.js`
>
>   - `title` (Zeichenkette) und `title.default` definieren den Standardtitel für untergeordnete Segmente (die keinen eigenen `title` definieren). Es wird `title.template` vom nächsten übergeordneten Segment ergänzen, falls vorhanden.
...
>
> - `page.js`
>   - Wenn eine Seite keinen eigenen Titel definiert, wird der aufgelöste Titel des nächsten übergeordneten Segments verwendet.
>   - `title` (Zeichenkette) definiert den Titel der Route. Es wird `title.template` vom nächsten übergeordneten Segment ergänzen, falls vorhanden.
...
- Wenn nicht konfiguriert, wird `metadataBase` **automatisch** mit einem [Standardwert](#standardwert) gefüllt.
...
- Wenn [`VERCEL_URL`](https://vercel.com/docs/concepts/projects/environment-variables/system-environment-variables#:~:text=.-,VERCEL_URL,-The%20domain%20name) erkannt wird: `https://${process.env.VERCEL_URL`, andernfalls wird auf `http://localhost:${process.env.PORT || 3000}` zurückgegriffen.
- Beim Überschreiben des Standardwerts empfehlen wir die Verwendung von Umgebungsvariablen, um die URL zu berechnen. Dies ermöglicht die Konfiguration einer URL für lokale Entwicklung, Staging und Produktionsumgebungen.

#### URL-Zusammensetzung

Die URL-Zusammensetzung bevorzugt die Absicht des Entwicklers gegenüber der Standardverzeichnistraversierungssemantik.
...
Beispielsweise, gegeben den folgenden `metadataBase`:
...
Alle `metadata`-Felder, die den obigen `metadataBase` erben und ihren eigenen Wert setzen, werden wie folgt aufgelöst:
...
> **Veraltet**: Die `themeColor`-Option in `metadata` ist seit Next.js 14 veraltet. Bitte verwenden Sie stattdessen die [`viewport`-Konfiguration](/docs/14/app/api-reference/functions/generate-viewport).
...
Mehr Informationen finden Sie in der [Twitter Card Markup Referenz](https://developer.twitter.com/en/docs/twitter-for-websites/cards/overview/markup).
...
> **Veraltet**: Die `viewport`-Option in `metadata` ist seit Next.js 14 veraltet. Bitte verwenden Sie stattdessen die [`viewport`-Konfiguration](/docs/14/app/api-reference/functions/generate-viewport).
...
Alle Metadaten-Optionen sollten durch die integrierte Unterstützung abgedeckt sein. Es kann jedoch sein, dass es benutzerdefinierte Metadaten-Tags gibt, die spezifisch für Ihre Seite sind, oder brandneue Metadaten-Tags, die gerade veröffentlicht wurden. Sie können die `other`-Option verwenden, um beliebige benutzerdefinierte Metadaten-Tags zu rendern.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | Verwenden Sie geeignete HTTP-Header über [`redirect()`](/docs/14/app/api-reference/functions/redirect), [Middleware](/docs/14/app/building-your-application/routing/middleware#nextresponse) oder [Sicherheits-Header](/docs/14/app/api-reference/next-config-js/headers) |
| `<base>`                      | Rendern Sie das Tag direkt im Layout oder auf der Seite.                                                                                                                                                                                                       |
| `<noscript>`                  | Rendern Sie das Tag direkt im Layout oder auf der Seite.                                                                                                                                                                                                       |
| `<style>`                     | Erfahren Sie mehr über [Styling in Next.js](/docs/14/app/building-your-application/styling/css-modules).                                                                                                                                                    |
| `<script>`                    | Erfahren Sie mehr über die [Verwendung von Skripten](/docs/14/app/building-your-application/optimizing/scripts).                                                                                                                                                          |
| `<link rel="stylesheet" />`   | Importieren Sie Stylesheets direkt im Layout oder auf der Seite.                                                                                                                                                                                        |
...
Das `<link>`-Element bietet eine Reihe von `rel`-Schlüsselwörtern, die dem Browser signalisieren, dass eine externe Ressource wahrscheinlich benötigt wird. Der Browser nutzt diese Informationen, um Vorab-Ladeoptimierungen anzuwenden.
...
Initiiert präemptiv eine Verbindung zu einem Origin. [MDN Docs](https://developer.mozilla.org/docs/Web/HTML/Attributes/rel/preconnect).
...
Sie können Typsicherheit für Ihre Metadaten hinzufügen, indem Sie den `Metadata`-Typ verwenden. Wenn Sie das [integrierte TypeScript-Plugin](/docs/14/app/building-your-application/configuring/typescript) in Ihrer IDE verwenden, müssen Sie den Typ nicht manuell hinzufügen, können dies aber dennoch explizit tun.
...
| Version   | Änderungen                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.2.0` | `viewport`, `themeColor` und `colorScheme` wurden zugunsten der [`viewport`-Konfiguration](/docs/14/app/api-reference/functions/generate-viewport) als veraltet markiert. |
| `v13.2.0` | `metadata` und `generateMetadata` wurden eingeführt.                                                                                                           |
