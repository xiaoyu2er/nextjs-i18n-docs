...
Da Next.js diesen [statischen Export](/docs/14/app/building-your-application/deploying/static-exports) unterstützt, kann er auf jedem Webserver bereitgestellt und gehostet werden, der statische Assets wie HTML/CSS/JS bereitstellen kann. Dazu gehören Tools wie AWS S3, Nginx oder Apache.

Der Betrieb als [statischer Export](/docs/14/app/building-your-application/deploying/static-exports) unterstützt keine Next.js-Funktionen, die einen Server erfordern. [Mehr erfahren](/docs/14/app/building-your-application/deploying/static-exports#unsupported-features).

> **Gut zu wissen:**
>
> - [Server Components](/docs/14/app/building-your-application/rendering/server-components) werden mit statischen Exporten unterstützt.

## Funktionen

### Bildoptimierung

[Bildoptimierung](/docs/14/app/building-your-application/optimizing/images) über `next/image` funktioniert beim Self-Hosting ohne Konfiguration, wenn Sie `next start` verwenden. Wenn Sie einen separaten Dienst zur Bildoptimierung bevorzugen, können Sie einen [Image Loader konfigurieren](/docs/14/app/building-your-application/optimizing/images#loaders).

Bildoptimierung kann mit einem [statischen Export](/docs/14/app/building-your-application/deploying/static-exports#image-optimization) verwendet werden, indem ein benutzerdefinierter Image Loader in `next.config.js` definiert wird. Beachten Sie, dass Bilder zur Laufzeit optimiert werden, nicht während des Builds.

> **Gut zu wissen:**
>
> - Beim Self-Hosting sollten Sie `sharp` installieren, um eine leistungsfähigere [Bildoptimierung](/docs/14/pages/building-your-application/optimizing/images) in Ihrer Produktionsumgebung zu erreichen. Führen Sie dazu `npm install sharp` in Ihrem Projektverzeichnis aus. Auf Linux-Plattformen erfordert `sharp` möglicherweise [zusätzliche Konfiguration](https://sharp.pixelplumbing.com/install#linux-memory-allocator), um übermäßigen Speicherverbrauch zu verhindern.
> - Erfahren Sie mehr über das [Caching-Verhalten optimierter Bilder](/docs/14/app/api-reference/components/image#caching-behavior) und wie Sie die TTL konfigurieren.
> - Sie können auch die [Bildoptimierung deaktivieren](/docs/14/app/api-reference/components/image#unoptimized) und dennoch die anderen Vorteile von `next/image` nutzen, wenn Sie dies bevorzugen. Zum Beispiel, wenn Sie Bilder separat selbst optimieren.

### Middleware

[Middleware](/docs/14/app/building-your-application/routing/middleware) funktioniert beim Self-Hosting ohne Konfiguration, wenn Sie `next start` verwenden. Da sie Zugriff auf die eingehende Anfrage benötigt, wird sie bei einem [statischen Export](/docs/14/app/building-your-application/deploying/static-exports) nicht unterstützt.

Middleware verwendet eine [Runtime](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes), die eine Teilmenge aller verfügbaren Node.js-APIs darstellt, um eine niedrige Latenz zu gewährleisten, da sie vor jeder Route oder jedem Asset Ihrer Anwendung ausgeführt werden kann. Diese Runtime erfordert nicht den Betrieb "am Edge" und funktioniert auf einem Single-Region-Server. Zusätzliche Konfiguration und Infrastruktur sind erforderlich, um Middleware in mehreren Regionen auszuführen.

Wenn Sie Logik hinzufügen möchten (oder ein externes Paket verwenden), das alle Node.js-APIs erfordert, können Sie diese Logik möglicherweise in ein [Layout](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts) als [Server Component](/docs/14/app/building-your-application/rendering/server-components) verschieben. Zum Beispiel das Überprüfen von [Headers](/docs/14/app/api-reference/functions/headers) und [Umleitungen](/docs/14/app/api-reference/functions/redirect). Sie können auch Header, Cookies oder Abfrageparameter verwenden, um über `next.config.js` [Umleitungen](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching) oder [Rewrites](/docs/14/app/api-reference/next-config-js/rewrites#header-cookie-and-query-matching) durchzuführen. Falls das nicht funktioniert, können Sie auch einen [Custom Server](/docs/14/pages/building-your-application/configuring/custom-server) verwenden.
...
Um Laufzeit-Umgebungsvariablen zu lesen, empfehlen wir die Verwendung von `getServerSideProps` oder die [schrittweise Migration zum App Router](/docs/14/app/building-your-application/upgrading/app-router-migration). Mit dem App Router können Sie Umgebungsvariablen sicher auf dem Server während des dynamischen Renderings lesen. Dies ermöglicht die Verwendung eines einzigen Docker-Images, das durch mehrere Umgebungen mit unterschiedlichen Werten befördert werden kann.
...
> **Gut zu wissen:**
>
> - Sie können Code beim Serverstart mit der [`register`-Funktion](/docs/14/app/building-your-application/optimizing/instrumentation) ausführen.
> - Wir empfehlen nicht die Verwendung der [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration)-Option, da diese nicht mit dem Standalone-Output-Modus funktioniert. Stattdessen empfehlen wir die [schrittweise Migration](/docs/14/app/building-your-application/upgrading/app-router-migration) zum App Router.
...
- Next.js setzt den `Cache-Control`-Header von `public, max-age=31536000, immutable` für wirklich unveränderliche Assets. Dies kann nicht überschrieben werden. Diese unveränderlichen Dateien enthalten einen SHA-Hash im Dateinamen und können daher unbegrenzt zwischengespeichert werden. Zum Beispiel [Statische Bildimporte](/docs/14/app/building-your-application/optimizing/images#local-images). Sie können die [TTL für Bilder konfigurieren](/docs/14/app/api-reference/components/image#caching-behavior).
- Incremental Static Regeneration (ISR) setzt den `Cache-Control`-Header auf `s-maxage: <revalidate in getStaticProps>, stale-while-revalidate`. Diese Revalidierungszeit wird in Ihrer [`getStaticProps`-Funktion](/docs/14/pages/building-your-application/data-fetching/get-static-props) in Sekunden definiert. Wenn Sie `revalidate: false` setzen, wird standardmäßig eine einjährige Cache-Dauer verwendet.
- Dynamisch gerenderte Seiten setzen einen `Cache-Control`-Header von `private, no-cache, no-store, max-age=0, must-revalidate`, um die Zwischenspeicherung benutzerspezifischer Daten zu verhindern. Dies gilt sowohl für den App- als auch den Pages-Router. Dazu gehört auch der [Draft Mode](/docs/14/app/building-your-application/configuring/draft-mode).

#### Statische Assets

Wenn Sie statische Assets auf einer anderen Domain oder einem CDN hosten möchten, können Sie die `assetPrefix`-[Konfiguration](/docs/14/app/api-reference/next-config-js/assetPrefix) in `next.config.js` verwenden. Next.js verwendet dieses Asset-Präfix beim Abrufen von JavaScript- oder CSS-Dateien. Die Trennung Ihrer Assets auf eine andere Domain hat den Nachteil einer zusätzlichen Zeit für DNS- und TLS-Auflösung.

[Mehr über `assetPrefix` erfahren](/docs/14/app/api-reference/next-config-js/assetPrefix).
...
Der Next.js App Router unterstützt [Streaming-Antworten](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) beim Self-Hosting. Wenn Sie Nginx oder einen ähnlichen Proxy verwenden, müssen Sie diesen so konfigurieren, dass das Buffering deaktiviert wird, um Streaming zu ermöglichen.
