...
translation-updated-at: 2025-06-06T18:13:02.541Z
...
Die `opengraph-image` und `twitter-image` Dateikonventionen ermöglichen es Ihnen, Open Graph- und Twitter-Bilder für ein Routensegment festzulegen.
...
> **Gut zu wissen**
>
> - Standardmäßig werden generierte Bilder [**statisch optimiert**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (zum Build-Zeitpunkt generiert und zwischengespeichert), es sei denn, sie verwenden [dynamische Funktionen](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) oder ungecachte Daten.
> - Sie können mehrere Bilder in derselben Datei mit [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata) generieren.

Der einfachste Weg, ein Bild zu generieren, ist die Verwendung der [ImageResponse](/docs/14/app/api-reference/functions/image-response) API aus `next/og`.
...
Ein Objekt, das das [dynamische Routenparameter](/docs/14/app/building-your-application/routing/dynamic-routes)-Objekt vom Root-Segment bis zu dem Segment enthält, in dem sich `opengraph-image` oder `twitter-image` befindet.
...
`opengraph-image` und `twitter-image` sind spezialisierte [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers), die dieselben [Routensegment-Konfigurationsoptionen](/docs/14/app/api-reference/file-conventions/route-segment-config) wie Pages und Layouts verwenden können.
...
> **Gut zu wissen**:
> Standardmäßig wird dieses generierte Bild [statisch optimiert](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default). Sie können die individuellen `fetch`-[`Optionen`](/docs/14/app/api-reference/functions/fetch) oder Routensegment-[Optionen](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) konfigurieren, um dieses Verhalten zu ändern.
