...
translation-updated-at: 2025-06-06T17:23:08.533Z
title: useRouter
...
Wenn Sie auf das [`router`-Objekt](#router-object) in einer beliebigen Funktionskomponente Ihrer Anwendung zugreifen möchten, können Sie den `useRouter`-Hook verwenden. Sehen Sie sich das folgende Beispiel an:
...
- `pathname`: `String` - Der Pfad der aktuellen Routendatei nach `/pages`. Daher sind `basePath`, `locale` und nachgestellter Schrägstrich (`trailingSlash: true`) nicht enthalten.
- `query`: `Object` - Die Query-String als geparstes Objekt, einschließlich [dynamischer Routen](/docs/14/pages/building-your-application/routing/dynamic-routes)-Parameter. Während des Prerenderings ist es ein leeres Objekt, wenn die Seite kein [Server-seitiges Rendering (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) verwendet. Standardwert: `{}`
- `asPath`: `String` - Der im Browser angezeigte Pfad inklusive Suchparametern und unter Berücksichtigung der `trailingSlash`-Konfiguration. `basePath` und `locale` sind nicht enthalten.
- `isFallback`: `boolean` - Gibt an, ob sich die aktuelle Seite im [Fallback-Modus](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) befindet.
- `basePath`: `String` - Der aktive [basePath](/docs/14/app/api-reference/next-config-js/basePath) (falls aktiviert).
...
- `isReady`: `boolean` - Gibt an, ob die Router-Felder clientseitig aktualisiert und einsatzbereit sind. Sollte nur innerhalb von `useEffect`-Methoden verwendet werden und nicht für bedingtes Rendering auf dem Server. Siehe verwandte Dokumentation für den Anwendungsfall mit [automatisch statisch optimierten Seiten](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)
- `isPreview`: `boolean` - Gibt an, ob sich die Anwendung im [Preview-Modus](/docs/14/pages/building-your-application/configuring/preview-mode) befindet.

> Die Verwendung des `asPath`-Felds kann zu einer Diskrepanz zwischen Client und Server führen, wenn die Seite mit Server-seitigem Rendering oder [automatischer statischer Optimierung](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) gerendert wird. Vermeiden Sie die Verwendung von `asPath`, bis das `isReady`-Feld `true` ist.

Folgende Methoden sind im `router`-Objekt enthalten:

### router.push

Handhabt clientseitige Übergänge. Diese Methode ist nützlich, wenn [`next/link`](/docs/14/pages/api-reference/components/link) nicht ausreicht.
...
- `options` - Optionales Objekt mit folgenden Konfigurationsoptionen:
  - `scroll` - Optionales Boolean, steuert das Scrollen zum Seitenanfang nach der Navigation. Standardwert: `true`
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): Aktualisiert den Pfad der aktuellen Seite ohne erneutes Ausführen von [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props), [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) oder [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props). Standardwert: `false`
...
Weiterleitung des Benutzers zu `pages/login.js`, nützlich für Seiten hinter einer [Authentifizierung](/docs/14/pages/building-your-application/authentication):
...
Im obigen Beispiel wird beim Navigieren zwischen `/one` und `/two` der Zähler **nicht** zurückgesetzt. Der `useState`-Wert bleibt zwischen den Renderings erhalten, weil die übergeordnete React-Komponente `Page` dieselbe ist.
...
- Verwenden Sie einen React `key`, um [React mitzuteilen, dass die Komponente neu eingehängt werden soll](https://react.dev/learn/rendering-lists#keeping-list-items-in-order-with-key). Für alle Seiten können Sie eine benutzerdefinierte App verwenden:
...
Sie können ein URL-Objekt auf die gleiche Weise verwenden wie für [`next/link`](/docs/14/pages/api-reference/components/link#with-url-object). Funktioniert sowohl für den `url`- als auch den `as`-Parameter:
...
Ähnlich wie die `replace`-Prop in [`next/link`](/docs/14/pages/api-reference/components/link) verhindert `router.replace`, dass ein neuer URL-Eintrag in den `history`-Stack hinzugefügt wird.
...
Seiten vorab laden für schnellere clientseitige Übergänge. Diese Methode ist nur für Navigationen ohne [`next/link`](/docs/14/pages/api-reference/components/link) nützlich, da `next/link` das Vorabladen von Seiten automatisch übernimmt.
...
- `as` - Optionaler Dekorator für `url`. Vor Next.js 9.5.3 wurde dies zum Vorabladen dynamischer Routen verwendet.
...
In einigen Fällen (z.B. bei Verwendung eines [Custom Servers](/docs/14/pages/building-your-application/configuring/custom-server)) möchten Sie möglicherweise auf [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) hören und etwas tun, bevor der Router darauf reagiert.

```js
router.beforePopState(cb)
```

- `cb` - Die Funktion, die bei eingehenden `popstate`-Ereignissen ausgeführt wird. Die Funktion erhält den Zustand des Ereignisses als Objekt mit folgenden Props:
...
Wenn `cb` `false` zurückgibt, wird der Next.js-Router `popstate` nicht behandeln, und Sie sind dafür verantwortlich. Siehe [Deaktivierung des Dateisystem-Routings](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).

Sie könnten `beforePopState` verwenden, um die Anfrage zu manipulieren oder ein SSR-Refresh zu erzwingen, wie im folgenden Beispiel:
...
> **Wichtig**: Hier ist `url` die im Browser angezeigte URL, einschließlich des [`basePath`](/docs/14/app/api-reference/next-config-js/basePath).
...
> Wir verwenden eine [Custom App](/docs/14/pages/building-your-application/routing/custom-app) (`pages/_app.js`) für dieses Beispiel, um das Ereignis zu abonnieren, weil sie bei Seitenwechseln nicht ausgehängt wird. Sie können Router-Ereignisse jedoch in jeder Komponente Ihrer Anwendung abonnieren.
...
Bestimmte Methoden des `router`-Objekts geben ein Promise zurück. Wenn die ESLint-Regel [no-floating-promises](https://typescript-eslint.io/rules/no-floating-promises) aktiviert ist, sollten Sie diese entweder global oder für die betroffene Zeile deaktivieren.
...
Falls [`useRouter`](#router-object) nicht die beste Option für Sie ist, kann `withRouter` das gleiche [`router`-Objekt](#router-object) zu jeder Komponente hinzufügen.
