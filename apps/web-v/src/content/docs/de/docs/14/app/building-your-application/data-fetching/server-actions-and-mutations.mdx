...
sidebar:
  order: 2010202
  label: "Server Actions und Mutationen"
---
...
- Server Actions integrieren sich in die Next.js [Caching- und Revalidierungsarchitektur](/docs/14/app/building-your-application/caching). Wenn eine Action aufgerufen wird, kann Next.js sowohl die aktualisierte UI als auch neue Daten in einem einzigen Server-Roundtrip zurückgeben.
...
- Server Actions erben die [Laufzeitumgebung](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes) von der Seite oder dem Layout, in dem sie verwendet werden.
- Server Actions erben die [Route Segment Config](/docs/14/app/api-reference/file-conventions/route-segment-config) von der Seite oder dem Layout, in dem sie verwendet werden, einschließlich Feldern wie `maxDuration`.
...
Wenn ein Fehler auftritt, wird er vom nächsten [`error.js`](/docs/14/app/building-your-application/routing/error-handling) oder `<Suspense>`-Boundary auf dem Client abgefangen. Wir empfehlen die Verwendung von `try/catch`, um Fehler zurückzugeben, die von Ihrer Benutzeroberfläche behandelt werden können.
...
Sie können den [Next.js Cache](/docs/14/app/building-your-application/caching) innerhalb Ihrer Server-Aktionen mit der [`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)-API neu validieren:
...
Oder Sie können einen bestimmten Datenabruf mit einem Cache-Tag über [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag) ungültig machen:
...
Wenn Sie den Benutzer nach Abschluss einer Server-Aktion zu einer anderen Route weiterleiten möchten, können Sie die [`redirect`](/docs/14/app/api-reference/functions/redirect)-API verwenden. `redirect` muss außerhalb des `try/catch`-Blocks aufgerufen werden:
...
Sie können Cookies innerhalb einer Server-Aktion mit der [`cookies`](/docs/14/app/api-reference/functions/cookies)-API `get`, `set` und `delete`:
...
Siehe [weitere Beispiele](/docs/14/app/api-reference/functions/cookies#deleting-cookies) zum Löschen von Cookies aus Server-Aktionen.
...
> **Gut zu wissen:** Wir empfehlen nicht, sich allein auf die Verschlüsselung zu verlassen, um zu verhindern, dass sensible Werte dem Client preisgegeben werden. Stattdessen sollten Sie die [React taint APIs](/docs/14/app/building-your-application/data-fetching/patterns#preventing-sensitive-data-from-being-exposed-to-the-client) verwenden, um proaktiv zu verhindern, dass bestimmte Daten an den Client gesendet werden.
