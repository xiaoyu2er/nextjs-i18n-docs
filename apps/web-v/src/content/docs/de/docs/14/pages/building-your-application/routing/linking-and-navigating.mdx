...
translation-updated-at: 2025-06-06T17:27:33.210Z
...
Jeder `<Link />` im sichtbaren Bereich (initial oder durch Scrollen) wird standardmäßig vorab geladen (inklusive der entsprechenden Daten) für Seiten, die [Static Generation](/docs/14/pages/building-your-application/data-fetching/get-static-props) verwenden. Die entsprechenden Daten für [server-seitig gerenderte](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) Routen werden _erst dann_ abgerufen, wenn auf den `<Link />` geklickt wird.

## Verlinkung zu dynamischen Pfaden

Sie können auch Interpolation verwenden, um den Pfad zu erstellen, was besonders nützlich für [dynamische Routensegmente](/docs/14/pages/building-your-application/routing/dynamic-routes) ist. Zum Beispiel, um eine Liste von Beiträgen anzuzeigen, die der Komponente als Prop übergeben wurden:
...
Um auf das [`router`-Objekt](/docs/14/pages/api-reference/functions/use-router#router-object) in einer React-Komponente zuzugreifen, können Sie [`useRouter`](/docs/14/pages/api-reference/functions/use-router) oder [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter) verwenden.

Generell empfehlen wir die Verwendung von [`useRouter`](/docs/14/pages/api-reference/functions/use-router).

## Imperative Navigation

[`next/link`](/docs/14/pages/api-reference/components/link) sollte die meisten Ihrer Routing-Anforderungen abdecken, aber Sie können auch clientseitige Navigationen ohne diese Komponente durchführen. Siehe die [Dokumentation für `next/router`](/docs/14/pages/api-reference/functions/use-router).

Das folgende Beispiel zeigt grundlegende Seitennavigation mit [`useRouter`](/docs/14/pages/api-reference/functions/use-router):
...
Shallow Routing ermöglicht es, die URL zu ändern, ohne die Datenabrufmethoden erneut auszuführen. Dazu gehören [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props), [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) und [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props).

Sie erhalten den aktualisierten `pathname` und das `query`-Objekt über das [`router`-Objekt](/docs/14/pages/api-reference/functions/use-router#router-object) (bereitgestellt durch [`useRouter`](/docs/14/pages/api-reference/functions/use-router) oder [`withRouter`](/docs/14/pages/api-reference/functions/use-router#withrouter)), ohne den Zustand zu verlieren.
