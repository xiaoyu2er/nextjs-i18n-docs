...
Konfiguriert, wie die Anfrage mit dem [Data Cache](/docs/14/app/building-your-application/caching#data-cache) von Next.js interagieren soll.
...
> **Wissenswert**:
>
> - Wenn Sie keine `cache`-Option angeben, verwendet Next.js standardmäßig `force-cache`, es sei denn, eine [dynamische Funktion](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) wie `cookies()` wird verwendet, in diesem Fall wird standardmäßig `no-store` verwendet.
...
> **Wissenswert**:
>
> - Wenn eine individuelle `fetch()`-Anfrage einen `revalidate`-Wert festlegt, der niedriger ist als die [standardmäßige `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) einer Route, wird das gesamte Revalidierungsintervall der Route verringert.
