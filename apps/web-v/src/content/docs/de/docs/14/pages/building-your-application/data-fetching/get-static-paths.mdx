...
translation-updated-at: 2025-06-06T17:25:33.656Z
...
Wenn eine Seite [Dynamische Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) verwendet und `getStaticProps` einsetzt, muss sie eine Liste von Pfaden definieren, die statisch generiert werden sollen.

Wenn Sie eine Funktion namens `getStaticPaths` (Static Site Generation) aus einer Seite mit dynamischen Routen exportieren, wird Next.js alle von `getStaticPaths` angegebenen Pfade statisch vorab rendern.
...
Die [`getStaticPaths` API-Referenz](/docs/14/pages/api-reference/functions/get-static-paths) deckt alle Parameter und Props ab, die mit `getStaticPaths` verwendet werden können.
...
`getStaticPaths` wird nur während des Builds in der Produktionsumgebung ausgeführt und nicht zur Laufzeit. Sie können überprüfen, dass der Code innerhalb von `getStaticPaths` aus dem Client-seitigen Bundle entfernt wurde, [mit diesem Tool](https://next-code-elimination.vercel.app/).
...
- Sie können `getStaticPaths` nicht mit [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) verwenden
- Sie können `getStaticPaths` aus einer [Dynamischen Route](/docs/14/pages/building-your-application/routing/dynamic-routes) exportieren, die auch `getStaticProps` verwendet
...
`getStaticPaths` ermöglicht es Ihnen, zu steuern, welche Seiten während des Builds generiert werden, anstatt bei Bedarf mit [`fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking). Die Generierung mehrerer Seiten während eines Builds führt zu langsameren Builds.

Sie können die Generierung aller Seiten bei Bedarf verzögern, indem Sie ein leeres Array für `paths` zurückgeben. Dies kann besonders hilfreich sein, wenn Sie Ihre Next.js-Anwendung in mehreren Umgebungen bereitstellen. Zum Beispiel können Sie schnellere Builds erreichen, indem Sie alle Seiten für Vorschauen bei Bedarf generieren (aber nicht für Produktions-Builds). Dies ist hilfreich für Websites mit Hunderten/Tausenden von statischen Seiten.
...
export async function getStaticPaths() {
  ...
  // Pfade basierend auf Posts bestimmen, die vorab gerendert werden sollen
