...
Der einfachste Weg, ein Icon zu generieren, ist die Verwendung der [`ImageResponse`](/docs/14/app/api-reference/functions/image-response)-API von `next/og`.
...
> **Gut zu wissen**
>
> - Standardmäßig sind generierte Icons [**statisch optimiert**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default) (während des Builds generiert und zwischengespeichert), es sei denn, sie verwenden [dynamische Funktionen](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) oder ungecachte Daten.
> - Sie können mehrere Icons in derselben Datei mit [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata) generieren.
...
Ein Objekt, das das [dynamische Route-Parameter](/docs/14/app/building-your-application/routing/dynamic-routes)-Objekt vom Root-Segment bis zum Segment enthält, in dem sich `icon` oder `apple-icon` befindet.
...
`icon` und `apple-icon` sind spezialisierte [Route Handler](/docs/14/app/building-your-application/routing/route-handlers), die dieselben [Route-Segment-Konfigurationsoptionen](/docs/14/app/api-reference/file-conventions/route-segment-config) wie Pages und Layouts verwenden können.
