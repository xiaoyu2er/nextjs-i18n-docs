...
Dieser Codemod verschiebt Imports von `next/server` zu `next/og` für die Verwendung von [Dynamischer OG-Bildgenerierung](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation).
...
Entfernt `<a>`-Tags innerhalb von [Link-Komponenten](/docs/14/app/api-reference/components/link) oder fügt ein `legacyBehavior`-Prop zu Links hinzu, die nicht automatisch behoben werden können.

</AppOnly>

<PagesOnly>

Entfernt `<a>`-Tags innerhalb von [Link-Komponenten](/docs/14/pages/api-reference/components/link) oder fügt ein `legacyBehavior`-Prop zu Links hinzu, die nicht automatisch behoben werden können.
