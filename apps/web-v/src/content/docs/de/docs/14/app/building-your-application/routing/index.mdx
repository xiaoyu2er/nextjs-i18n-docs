...
In Version 13 hat Next.js einen neuen **App Router** eingeführt, der auf [React Server Components](/docs/14/app/building-your-application/rendering/server-components) basiert und gemeinsame Layouts, verschachteltes Routing, Ladezustände, Fehlerbehandlung und mehr unterstützt.

Der App Router arbeitet in einem neuen Verzeichnis namens `app`. Das `app`-Verzeichnis funktioniert parallel zum `pages`-Verzeichnis, um eine schrittweise Einführung zu ermöglichen. Dadurch können Sie einige Routen Ihrer Anwendung für das neue Verhalten aktivieren, während andere Routen im `pages`-Verzeichnis für das bisherige Verhalten bleiben. Wenn Ihre Anwendung das `pages`-Verzeichnis verwendet, lesen Sie bitte auch die Dokumentation zum [Pages Router](/docs/14/pages/building-your-application/routing).
...
Standardmäßig sind Komponenten innerhalb von `app` [React Server Components](/docs/14/app/building-your-application/rendering/server-components). Dies ist eine Leistungsoptimierung und ermöglicht eine einfache Übernahme. Sie können auch [Client Components](/docs/14/app/building-your-application/rendering/client-components) verwenden.

> **Empfehlung:** Schauen Sie sich die [Server](/docs/14/app/building-your-application/rendering/server-components)-Seite an, wenn Sie neu bei Server Components sind.

## Rollen von Ordnern und Dateien

Next.js verwendet einen dateisystembasierten Router, bei dem:

- **Ordner** verwendet werden, um Routen zu definieren. Eine Route ist ein einzelner Pfad verschachtelter Ordner, der der Dateisystemhierarchie vom **Root-Ordner** bis zu einem finalen **Blatt-Ordner** folgt, der eine `page.js`-Datei enthält. Siehe [Defining Routes](/docs/14/app/building-your-application/routing/defining-routes).
...
|                                                                                       |                                                                                                |
| ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts#layouts)     | Gemeinsame UI für ein Segment und seine Kinder                                                |
| [`page`](/docs/14/app/building-your-application/routing/pages-and-layouts#pages)         | Einzigartige UI einer Route und macht Routen öffentlich zugänglich                            |
| [`loading`](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)     | Lade-UI für ein Segment und seine Kinder                                                       |
| [`not-found`](/docs/14/app/api-reference/file-conventions/not-found)                     | Nicht-gefunden-UI für ein Segment und seine Kinder                                            |
| [`error`](/docs/14/app/building-your-application/routing/error-handling)                 | Fehler-UI für ein Segment und seine Kinder                                                     |
| [`global-error`](/docs/14/app/building-your-application/routing/error-handling)          | Globale Fehler-UI                                                                              |
| [`route`](/docs/14/app/building-your-application/routing/route-handlers)                 | Server-seitiger API-Endpunkt                                                                   |
| [`template`](/docs/14/app/building-your-application/routing/pages-and-layouts#templates) | Spezialisiertes neu gerendertes Layout-UI                                                      |
| [`default`](/docs/14/app/api-reference/file-conventions/default)                         | Fallback-UI für [Parallel Routes](/docs/14/app/building-your-application/routing/parallel-routes) |
...
Erfahren Sie mehr über [Projektorganisation und Kollokation](/docs/14/app/building-your-application/routing/colocation).
