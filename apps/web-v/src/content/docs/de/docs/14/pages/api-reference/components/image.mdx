...
translation-updated-at: 2025-06-06T17:20:32.493Z
title: <Image>
...
> **Wissenswert**: Wenn Sie eine Version von Next.js vor 13 verwenden, sollten Sie die Dokumentation für [next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) verwenden, da die Komponente umbenannt wurde.

</PagesOnly>

Diese API-Referenz hilft Ihnen zu verstehen, wie Sie die verfügbaren [Props](#props) und [Konfigurationsoptionen](#configuration-options) für die Image-Komponente verwenden. Für Funktionen und Verwendung siehe die Seite [Image-Komponente](/docs/14/app/building-your-application/optimizing/images).
...
- Eine [statisch importierte](/docs/14/app/building-your-application/optimizing/images#local-images) Bilddatei
...
Erforderlich, außer für [statisch importierte Bilder](/docs/14/app/building-your-application/optimizing/images#local-images) oder Bilder mit der [`fill`](#fill)-Eigenschaft.
...
> **Wissenswert**: Die Verwendung von Props wie `loader`, die eine Funktion akzeptieren, erfordert die Verwendung von [Client Components](/docs/14/app/building-your-application/rendering/client-components), um die bereitgestellte Funktion zu serialisieren.
...
> **Wissenswert**: Die Verwendung von Props wie `onLoadingComplete`, die eine Funktion akzeptieren, erfordert die Verwendung von [Client Components](/docs/14/app/building-your-application/rendering/client-components), um die bereitgestellte Funktion zu serialisieren.
...
> **Wissenswert**: Die Verwendung von Props wie `onLoad`, die eine Funktion akzeptieren, erfordert die Verwendung von [Client Components](/docs/14/app/building-your-application/rendering/client-components), um die bereitgestellte Funktion zu serialisieren.
...
> **Wissenswert**: Die Verwendung von Props wie `onError`, die eine Funktion akzeptieren, erfordert die Verwendung von [Client Components](/docs/14/app/building-your-application/rendering/client-components), um die bereitgestellte Funktion zu serialisieren.
...
- [Benutzerdefinierte Image Loader-Konfiguration](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **Gut zu wissen**: Die Anpassung der Image Loader-Datei, die eine Funktion akzeptiert, erfordert die Verwendung von [Client Components](/docs/14/app/building-your-application/rendering/client-components), um die bereitgestellte Funktion zu serialisieren.
...
Sie können die Time to Live (TTL) in Sekunden für zwischengespeicherte optimierte Bilder konfigurieren. In vielen Fällen ist es besser, einen [statischen Bildimport](/docs/14/app/building-your-application/optimizing/images#local-images) zu verwenden, der den Dateiinhalt automatisch hasht und das Bild mit einem `Cache-Control`-Header von `immutable` dauerhaft zwischenspeichert.
...
Wenn Sie das Caching-Verhalten pro Bild ändern müssen, können Sie [`headers`](/docs/14/app/api-reference/next-config-js/headers) konfigurieren, um den `Cache-Control`-Header auf dem Upstream-Bild zu setzen (z.B. `/some-asset.jpg`, nicht `/_next/image` selbst).
...
| `v13.0.0`  | Der Import `next/image` wurde in `next/legacy/image` umbenannt. Der Import `next/future/image` wurde in `next/image` umbenannt. Ein [Codemod ist verfügbar](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) zum sicheren und automatischen Umbenennen Ihrer Imports. `<span>`-Wrapper entfernt. `layout`-, `objectFit`-, `objectPosition`-, `lazyBoundary`-, `lazyRoot`-Props entfernt. `alt` ist jetzt erforderlich. `onLoadingComplete` erhält Referenz zum `img`-Element. Integrierte Loader-Konfiguration entfernt. |
