...
Die `headers`-Funktion ermöglicht das Lesen der eingehenden HTTP-Anfrage-Header in einer [Server-Komponente (Server Component)](/docs/14/app/building-your-application/rendering/server-components).
...
> **Wissenswert**:
>
> - `headers()` ist eine **[Dynamische Funktion (Dynamic Function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)**, deren Rückgabewerte nicht im Voraus bekannt sind. Die Verwendung in einem Layout oder einer Seite führt zu **[dynamischem Rendering (dynamic rendering)](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)** zur Laufzeit der Anfrage.
...
`headers()` kann in Kombination mit [Suspense für Data Fetching](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) verwendet werden.
