...
> **Gut zu wissen**: Wenn Sie den App-Router verwenden, können Sie [Server Components](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) oder [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) anstelle von API-Routen nutzen.
...
> - API-Routen können nicht mit [statischen Exports](/docs/14/pages/building-your-application/deploying/static-exports) verwendet werden. Allerdings können [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) im App-Router dies.
> - API-Routen werden durch die [`pageExtensions`-Konfiguration](/docs/14/pages/api-reference/next-config-js/pageExtensions) in `next.config.js` beeinflusst.
...
- `res.revalidate(urlPath)` - [Revalidiert eine Seite bei Bedarf](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) mit `getStaticProps`. `urlPath` muss ein `string` sein.
...
API-Routen unterstützen [dynamische Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) und folgen denselben Dateibenennungsregeln wie `pages/`.
...
Wenn Sie API-Routen mit der Edge Runtime verwenden möchten, empfehlen wir die schrittweise Einführung des App-Routers und die Verwendung von [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) anstelle von API-Routen.
