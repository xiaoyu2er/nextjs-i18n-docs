...
translation-updated-at: 2025-06-06T17:26:35.351Z
...
Die [`getStaticProps`-API-Referenz](/docs/14/pages/api-reference/functions/get-static-props) deckt alle Parameter und Props ab, die mit `getStaticProps` verwendet werden können.
...
- `getStaticProps` läuft im Hintergrund bei Verwendung von [`fallback: true`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)
- `getStaticProps` wird vor dem initialen Rendering aufgerufen bei Verwendung von [`fallback: blocking`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking)
- `getStaticProps` läuft im Hintergrund bei Verwendung von `revalidate`
- `getStaticProps` läuft bedarfsgesteuert im Hintergrund bei Verwendung von [`revalidate()`](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)

In Kombination mit [Inkrementeller Statischer Regenerierung](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) wird `getStaticProps` im Hintergrund ausgeführt, während die veraltete Seite neu validiert wird, und die aktualisierte Seite an den Browser gesendet wird.

`getStaticProps` hat keinen Zugriff auf die eingehende Anfrage (wie Query-Parameter oder HTTP-Header), da es statisches HTML generiert. Wenn Sie Zugriff auf die Anfrage für Ihre Seite benötigen, sollten Sie zusätzlich zu `getStaticProps` eine [Middleware](/docs/14/pages/building-your-application/routing/middleware) verwenden.
...
Diese JSON-Datei wird beim clientseitigen Routing über [`next/link`](/docs/14/pages/api-reference/components/link) oder [`next/router`](/docs/14/pages/api-reference/functions/use-router) verwendet. Wenn Sie zu einer Seite navigieren, die mit `getStaticProps` vorgerendert wurde, ruft Next.js diese JSON-Datei (die zur Build-Zeit vorberechnet wurde) ab und verwendet sie als Props für die Seitenkomponente. Das bedeutet, dass clientseitige Seitenübergänge **nicht** `getStaticProps` aufrufen, da nur das exportierte JSON verwendet wird.
...
> **Gut zu wissen**: Wenn Sie eine [benutzerdefinierte App](/docs/14/pages/building-your-application/routing/custom-app) erstellt haben, stellen Sie sicher, dass Sie die `pageProps` wie im verlinkten Dokument gezeigt an die Seitenkomponente übergeben, da die Props sonst leer sein werden.
