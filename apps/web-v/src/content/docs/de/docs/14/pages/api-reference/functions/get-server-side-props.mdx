...
| Name            | Beschreibung                                                                                                                                                                                                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`        | Wenn diese Seite eine [dynamische Route](/docs/14/pages/building-your-application/routing/dynamic-routes) verwendet, enthält `params` die Routenparameter. Wenn der Seitenname `[id].js` lautet, sieht `params` wie `{ id: ... }` aus. |
...
| `previewData`   | (Veraltet für `draftMode`) Die [Preview](/docs/14/pages/building-your-application/configuring/preview-mode)-Daten, die von `setPreviewData` gesetzt wurden.                                                                              |
...
Der `notFound`-Boolean ermöglicht es der Seite, einen `404`-Status und eine [404-Seite](/docs/14/pages/building-your-application/routing/custom-error#404-page) zurückzugeben. Mit `notFound: true` gibt die Seite einen `404` zurück, auch wenn zuvor erfolgreich eine Seite generiert wurde. Dies dient zur Unterstützung von Anwendungsfällen wie nutzergenerierten Inhalten, die vom Autor entfernt wurden.
...
| Version   | Änderungen                                                                                                     |
| --------- | ----------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/14/app/building-your-application/data-fetching) ist jetzt stabil mit vereinfachtem Datenabruf |
