...
translation-updated-at: 2025-06-06T17:23:55.893Z
...
Dadurch werden `process.env.DB_HOST`, `process.env.DB_USER` und `process.env.DB_PASS` automatisch in die Node.js-Umgebung geladen, sodass Sie sie in [Next.js Data-Fetching-Methoden](/docs/14/pages/building-your-application/data-fetching) und [API-Routen](/docs/14/pages/building-your-application/routing/api-routes) verwenden können.

Beispielsweise mit [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props):
...
> Dadurch werden `process.env.DB_HOST`, `process.env.DB_USER` und `process.env.DB_PASS` automatisch in die Node.js-Umgebung geladen, sodass Sie sie in [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers) verwenden können.
...
- Wir empfehlen nicht die Verwendung der [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration)-Option, da diese nicht mit dem standalone-Output-Modus funktioniert. Stattdessen empfehlen wir die [schrittweise Einführung](/docs/14/app/building-your-application/upgrading/app-router-migration) des App-Routers.
...
- Wenn Sie ein [`/src`-Verzeichnis](/docs/14/app/building-your-application/configuring/src-directory) verwenden, sollten die `.env.*`-Dateien im Stammverzeichnis Ihres Projekts bleiben.
