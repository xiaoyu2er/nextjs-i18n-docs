...
Mit Next.js 12 und [Middleware](/docs/14/pages/building-your-application/routing/middleware) können wir der Standard-Locale mit einem [Workaround](https://github.com/vercel/next.js/discussions/18419) ein Präfix hinzufügen.
...
Als Nächstes können wir [Middleware](/docs/14/pages/building-your-application/routing/middleware) verwenden, um benutzerdefinierte Routing-Regeln hinzuzufügen:
...
Diese [Middleware](/docs/14/pages/building-your-application/routing/middleware) überspringt das Hinzufügen des Standard-Präfix zu [API-Routen](/docs/14/pages/building-your-application/routing/api-routes) und [öffentlichen](/docs/14/pages/building-your-application/optimizing/static-assets) Dateien wie Schriftarten oder Bildern. Wenn eine Anfrage an die Standard-Locale gestellt wird, leiten wir zu unserem Präfix `/en` weiter.
...
Sie können über den Next.js-Router auf die Locale-Informationen zugreifen. Beispielsweise sind mit dem [`useRouter()`](/docs/14/pages/api-reference/functions/use-router)-Hook folgende Eigenschaften verfügbar:
...
Wenn Seiten mit `getStaticProps` oder `getServerSideProps` [vorgerendert](/docs/14/pages/building-your-application/rendering/static-site-generation) werden, werden die Locale-Informationen im [Kontext](/docs/14/pages/building-your-application/data-fetching/get-static-props) bereitgestellt, der der Funktion übergeben wird.
...
Hinweis: Um nur die `locale` zu ändern und gleichzeitig alle Routing-Informationen wie [dynamische Routen](/docs/14/pages/building-your-application/routing/dynamic-routes)-Query-Werte oder versteckte href-Query-Werte beizubehalten, können Sie den `href`-Parameter als Objekt bereitstellen:
...
Weitere Informationen zur Objektstruktur für `router.push` finden Sie [hier](/docs/14/pages/api-reference/functions/use-router#with-url-object).
...
Next.js kennt keine Varianten einer Seite, daher liegt es an Ihnen, die `hreflang`-Meta-Tags mit [`next/head`](/docs/14/pages/api-reference/components/head) hinzuzufügen. Weitere Informationen zu `hreflang` finden Sie in der [Google Webmasters-Dokumentation](https://support.google.com/webmasters/answer/189077).

## Wie funktioniert dies mit statischer Generierung?

> Hinweis: Internationalisiertes Routing ist nicht mit [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) integriert, da es nicht auf der Next.js-Routing-Ebene basiert. Hybride Next.js-Anwendungen, die `output: 'export'` nicht verwenden, werden vollständig unterstützt.

### Dynamische Routen und `getStaticProps`-Seiten

Für Seiten, die `getStaticProps` mit [dynamischen Routen](/docs/14/pages/building-your-application/routing/dynamic-routes) verwenden, müssen alle gewünschten Locale-Varianten der Seite aus [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths) zurückgegeben werden. Neben dem `params`-Objekt, das für `paths` zurückgegeben wird, können Sie auch ein `locale`-Feld zurückgeben, das angibt, welche Locale gerendert werden soll. Beispiel:
...
Für [automatisch statisch optimierte](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) und nicht-dynamische `getStaticProps`-Seiten wird **eine Version der Seite für jede Locale generiert**. Dies ist wichtig zu beachten, da es die Build-Zeiten erhöhen kann, abhängig davon, wie viele Locales in `getStaticProps` konfiguriert sind.
...
Um die Build-Zeit für dynamische Seiten mit `getStaticProps` zu verringern, verwenden Sie einen [`fallback`-Modus](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true). Dadurch können Sie nur die beliebtesten Pfade und Locales aus `getStaticPaths` für die Vorrendering während des Builds zurückgeben. Next.js generiert dann die restlichen Seiten zur Laufzeit, wenn sie angefordert werden.

### Automatisch statisch optimierte Seiten

Für Seiten, die [automatisch statisch optimiert](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) werden, wird eine Version der Seite für jede Locale generiert.
...
> **Gut zu wissen**: Diese Grenzen wurden zunächst hinzugefügt, um potenzielle [Performance-Probleme beim Build](#dynamische-routen-und-getstaticprops-seiten) zu vermeiden. Sie können diese Grenzen mit benutzerdefiniertem Routing unter Verwendung von [Middleware](/docs/14/pages/building-your-application/routing/middleware) in Next.js 12 umgehen.