...
translation-updated-at: 2025-06-06T17:24:13.735Z
title: OpenTelemetry
...
Erstellen Sie als Nächstes eine benutzerdefinierte [`instrumentation.ts`](/docs/14/app/building-your-application/optimizing/instrumentation)- (oder `.js`-) Datei im **Stammverzeichnis** des Projekts (oder im `src`-Ordner, falls verwendet):

</AppOnly>

<PagesOnly>

Erstellen Sie als Nächstes eine benutzerdefinierte [`instrumentation.ts`](/docs/14/pages/building-your-application/optimizing/instrumentation)- (oder `.js`-) Datei im **Stammverzeichnis** des Projekts (oder im `src`-Ordner, falls verwendet):
...
> - Wenn Sie die [`pageExtensions`-Konfigurationsoption](/docs/14/app/api-reference/next-config-js/pageExtensions) verwenden, um ein Suffix hinzuzufügen, müssen Sie auch den Dateinamen der `instrumentation`-Datei entsprechend anpassen.
...
>
> - Die `instrumentation`-Datei sollte im Stammverzeichnis Ihres Projekts liegen und nicht im `app`- oder `pages`-Verzeichnis. Wenn Sie den `src`-Ordner verwenden, legen Sie die Datei in `src` neben `pages` und `app` ab.
> - Wenn Sie die [`pageExtensions`-Konfigurationsoption](/docs/14/pages/api-reference/next-config-js/pageExtensions) verwenden, um ein Suffix hinzuzufügen, müssen Sie auch den Dateinamen der `instrumentation`-Datei entsprechend anpassen.
> - Wir haben ein einfaches [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry)-Beispiel erstellt, das Sie verwenden können.
