...
Weitere Informationen finden Sie in der [Dokumentation](/docs/14/pages/building-your-application/optimizing/images#styling).
...
Wenn Sie derzeit die `target`-Option auf `serverless` gesetzt haben, lesen Sie bitte die [Dokumentation zur Nutzung der neuen Output-Option](/docs/14/pages/api-reference/next-config-js/output).