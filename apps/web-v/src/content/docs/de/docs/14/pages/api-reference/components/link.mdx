...
translation-updated-at: 2025-06-06T17:20:17.576Z
title: <Link>
...
`<Link>` ist eine React-Komponente, die das HTML-`<a>`-Element erweitert, um [Prefetching](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) und clientseitige Navigation zwischen Routen zu ermöglichen. Es ist die primäre Methode, um zwischen Routen in Next.js zu navigieren.
...
> **Gut zu wissen**:
>
> - Next.js scrollt zur [Seite](/docs/14/app/building-your-application/routing/pages-and-layouts#pages), wenn diese bei der Navigation nicht im Viewport sichtbar ist.
...
- `false`: Prefetching erfolgt nicht beim Eintritt in den Viewport, aber beim Hover. Wenn Sie Prefetching komplett deaktivieren möchten, erwägen Sie die Verwendung eines `<a>`-Tags oder die [schrittweise Migration](/docs/14/app/building-your-application/upgrading/app-router-migration) zum App-Router, der auch das Deaktivieren von Prefetching beim Hover ermöglicht.
...
Ein `<a>`-Element ist nicht mehr als Kind von `<Link>` erforderlich. Fügen Sie die `legacyBehavior`-Prop hinzu, um das Legacy-Verhalten zu verwenden, oder entfernen Sie das `<a>`-Element für ein Upgrade. Ein [Codemod ist verfügbar](/docs/14/app/building-your-application/upgrading/codemods#new-link), um Ihren Code automatisch zu aktualisieren.
...
Aktualisiert den Pfad der aktuellen Seite, ohne [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props), [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) oder [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props) erneut auszuführen. Standardwert: `false`
...
- Einer [dynamischen Route](/docs/14/app/building-your-application/routing/dynamic-routes): `/blog/my-post`
...
> **Gut zu wissen**: Wenn Sie [Dynamische Routen](/docs/14/app/building-your-application/routing/dynamic-routes) verwenden, müssen Sie Ihre `as`- und `href`-Props anpassen. Wenn Sie beispielsweise eine Dynamische Route wie `/dashboard/authed/[user]` haben, die Sie über Middleware anders präsentieren möchten, würden Sie schreiben: `<Link href={{ pathname: '/dashboard/authed/[user]', query: { user: username } }} as="/dashboard/[user]">Profil</Link>`.
...
| Version   | Änderungen                                                                                                                                                                                         |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.0.0` | Benötigt kein untergeordnetes `<a>`-Tag mehr. Ein [Codemod](/docs/14/app/building-your-application/upgrading/codemods#remove-a-tags-from-link-components) wird bereitgestellt, um Ihre Codebasis automatisch zu aktualisieren. |
