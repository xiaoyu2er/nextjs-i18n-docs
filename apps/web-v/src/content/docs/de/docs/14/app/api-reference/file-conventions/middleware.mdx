...
Middleware kann das [`NextResponse`](/docs/14/app/building-your-application/routing/middleware#nextresponse)-Objekt verwenden, das die [Web Response API](https://developer.mozilla.org/en-US/docs/Web/API/Response) erweitert. Durch die Rückgabe eines `NextResponse`-Objekts können Sie direkt Cookies manipulieren, Header setzen, Weiterleitungen implementieren und Pfade umschreiben.

> **Gut zu wissen**: Für Weiterleitungen können Sie auch `Response.redirect` anstelle von `NextResponse.redirect` verwenden.

## Laufzeitumgebung

Middleware unterstützt nur die [Edge-Laufzeitumgebung](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes). Die Node.js-Laufzeitumgebung kann nicht verwendet werden.
