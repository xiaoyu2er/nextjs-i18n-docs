...
> Werfen Sie einen Blick in die [API Routes-Dokumentation](/docs/14/pages/building-your-application/routing/api-routes), falls Sie mit Next.js API-Routen noch nicht vertraut sind.
...
Rufen Sie dann `/api/disable-draft` auf, um die API-Route auszuführen. Wenn Sie diese Route mit [`next/link`](/docs/14/pages/api-reference/components/link) aufrufen, müssen Sie `prefetch={false}` übergeben, um ein versehentliches Löschen des Cookies beim Prefetch zu verhindern.

### Funktioniert mit `getServerSideProps`

Draft Mode funktioniert mit `getServerSideProps` und ist als `draftMode`-Schlüssel im [`context`-Objekt](/docs/14/pages/api-reference/functions/get-server-side-props#context-parameter) verfügbar.

> **Wichtig**: Sie sollten den `Cache-Control`-Header nicht setzen, wenn Sie Draft Mode verwenden, da er nicht umgangen werden kann. Stattdessen empfehlen wir [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).
