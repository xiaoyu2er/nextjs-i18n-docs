...
Wenn Sie zuvor `routes` in Ihrer `vercel.json`-Datei für dynamische Routen konfiguriert haben, können diese Regeln entfernt werden, wenn Sie die neue [Dynamic Routing-Funktion](/docs/14/pages/building-your-application/routing/dynamic-routes) von Next.js 9 nutzen.
...
Weitere Informationen finden Sie unter [Dynamic Routing](/docs/14/pages/building-your-application/routing/dynamic-routes).

## Überprüfen Sie Ihre Custom App-Datei (`pages/_app.js`)

Wenn Sie zuvor das Beispiel für eine [Custom `<App>`](/docs/14/pages/building-your-application/routing/custom-app) kopiert haben, können Sie möglicherweise Ihr `getInitialProps` entfernen.
