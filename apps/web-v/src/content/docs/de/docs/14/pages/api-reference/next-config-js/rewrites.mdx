...
1. [Headers](/docs/14/app/api-reference/next-config-js/headers) werden überprüft/angewendet
2. [Redirects](/docs/14/app/api-reference/next-config-js/redirects) werden überprüft/angewendet
3. `beforeFiles`-Rewrites werden überprüft/angewendet
4. Statische Dateien aus dem [public-Verzeichnis](/docs/14/app/building-your-application/optimizing/static-assets), `_next/static`-Dateien und nicht-dynamische Seiten werden überprüft/bereitgestellt
...
6. `fallback`-Rewrites werden überprüft/angewendet; diese werden vor dem Rendern der 404-Seite und nach der Überprüfung aller dynamischen Routen/statischen Assets angewendet. Wenn Sie [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) in `getStaticPaths` verwenden, werden die in Ihrer `next.config.js` definierten `fallback`-Rewrites _nicht_ ausgeführt.

</AppOnly>

<PagesOnly>

1. [Headers](/docs/14/pages/api-reference/next-config-js/headers) werden überprüft/angewendet
2. [Redirects](/docs/14/pages/api-reference/next-config-js/redirects) werden überprüft/angewendet
3. `beforeFiles`-Rewrites werden überprüft/angewendet
4. Statische Dateien aus dem [public-Verzeichnis](/docs/14/pages/building-your-application/optimizing/static-assets), `_next/static`-Dateien und nicht-dynamische Seiten werden überprüft/bereitgestellt
5. `afterFiles`-Rewrites werden überprüft/angewendet; wenn eines dieser Rewrites matched, werden dynamische Routen/statische Dateien nach jedem Match überprüft
6. `fallback`-Rewrites werden überprüft/angewendet; diese werden vor dem Rendern der 404-Seite und nach der Überprüfung aller dynamischen Routen/statischen Assets angewendet. Wenn Sie [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) in `getStaticPaths` verwenden, werden die in Ihrer `next.config.js` definierten `fallback`-Rewrites _nicht_ ausgeführt.
...
> **Gut zu wissen**: Statische Seiten aus [Automatic Static Optimization](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) oder [Prerendering](/docs/14/pages/building-your-application/data-fetching/get-static-props) werden die Parameter aus Rewrites nach der Hydration auf dem Client geparst und in der Query bereitgestellt.
...
Wenn Sie [`basePath`-Unterstützung](/docs/14/app/api-reference/next-config-js/basePath) mit Rewrites verwenden, wird jeder `source`- und `destination`-Pfad automatisch mit dem `basePath` präfixiert, es sei denn, Sie fügen `basePath: false` zum Rewrite hinzu:
...
Wenn Sie [`i18n`-Unterstützung](/docs/14/app/building-your-application/routing/internationalization) mit Rewrites verwenden, wird jeder `source`- und `destination`-Pfad automatisch präfixiert, um die konfigurierten `locales` zu handhaben, es sei denn, Sie fügen `locale: false` zum Rewrite hinzu. Wenn `locale: false` verwendet wird, müssen Sie `source` und `destination` mit einem Locale präfixieren, damit sie korrekt gematcht werden.

</AppOnly>

<PagesOnly>

Wenn Sie [`i18n`-Unterstützung](/docs/14/pages/building-your-application/routing/internationalization) mit Rewrites verwenden, wird jeder `source`- und `destination`-Pfad automatisch präfixiert, um die konfigurierten `locales` zu handhaben, es sei denn, Sie fügen `locale: false` zum Rewrite hinzu. Wenn `locale: false` verwendet wird, müssen Sie `source` und `destination` mit einem Locale präfixieren, damit sie korrekt gematcht werden.
