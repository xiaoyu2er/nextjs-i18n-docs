...
translation-updated-at: 2025-06-06T17:24:13.687Z
...
Um Instrumentation einzurichten, erstellen Sie eine `instrumentation.ts|js`-Datei im **Stammverzeichnis** Ihres Projekts (oder im [`src`](/docs/14/app/building-your-application/configuring/src-directory)-Ordner, falls verwendet).
...
> **Wichtig zu wissen**
>
> - Diese Funktion ist **experimentell**. Um sie zu nutzen, müssen Sie explizit [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) in Ihrer `next.config.js` definieren.
...
Next.js ruft `register` in allen Umgebungen auf, daher ist es wichtig, Code, der bestimmte Laufzeitumgebungen nicht unterstützt (z.B. [Edge](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime) oder [Node.js](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime)), bedingt zu importieren. Sie können die Umgebungsvariable `NEXT_RUNTIME` verwenden, um die aktuelle Umgebung abzufragen:
