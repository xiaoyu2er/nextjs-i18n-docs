...
- Verwendung des [`useRouter`-Hooks](#userouter-hook) ([Client Components](/docs/14/app/building-your-application/rendering/client-components))
- Verwendung der [`redirect`-Funktion](#redirect-function) ([Server Components](/docs/14/app/building-your-application/rendering/server-components))
...
Es gibt weitere optionale Props, die Sie an `<Link>` übergeben können. Weitere Informationen finden Sie in der [API-Referenz](/docs/14/app/api-reference/components/link).

### Beispiele

#### Verlinkung zu dynamischen Segmenten

Beim Verlinken zu [dynamischen Segmenten](/docs/14/app/building-your-application/routing/dynamic-routes) können Sie [Template-Literale und Interpolation](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals) verwenden, um eine Liste von Links zu generieren. Zum Beispiel, um eine Liste von Blog-Posts zu generieren:
...
Sie können [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname) verwenden, um festzustellen, ob ein Link aktiv ist. Um beispielsweise eine Klasse zum aktiven Link hinzuzufügen, können Sie überprüfen, ob der aktuelle `pathname` mit dem `href` des Links übereinstimmt:
...
> **Gut zu wissen**:
>
> - Next.js scrollt zur [Seite](/docs/14/app/building-your-application/routing/pages-and-layouts#pages), wenn sie bei der Navigation nicht im Viewport sichtbar ist.
...
Der `useRouter`-Hook ermöglicht es Ihnen, Routen programmatisch aus [Client Components](/docs/14/app/building-your-application/rendering/client-components) zu ändern.
...
Eine vollständige Liste der `useRouter`-Methoden finden Sie in der [API-Referenz](/docs/14/app/api-reference/functions/use-router).
...
Für [Server Components](/docs/14/app/building-your-application/rendering/server-components) verwenden Sie stattdessen die `redirect`-Funktion.
...
> - Wenn Sie vor dem Rendering-Prozess weiterleiten möchten, verwenden Sie [`next.config.js`](/docs/14/app/building-your-application/routing/redirecting#redirects-in-nextconfigjs) oder [Middleware](/docs/14/app/building-your-application/routing/redirecting#nextresponseredirect-in-middleware).

Weitere Informationen finden Sie in der [`redirect`-API-Referenz](/docs/14/app/api-reference/functions/redirect).
...
`pushState`- und `replaceState`-Aufrufe integrieren sich in den Next.js Router und ermöglichen die Synchronisation mit [`usePathname`](/docs/14/app/api-reference/functions/use-pathname) und [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params).
...
[Server Components](/docs/14/app/building-your-application/rendering/server-components) ermöglichen es, Ihren Anwendungscode automatisch nach Routensegmenten aufzuteilen. Das bedeutet, dass nur der Code für die aktuelle Route bei der Navigation geladen wird.
...
Weitere Informationen finden Sie in der [`<Link>`-API-Referenz](/docs/14/app/api-reference/components/link).
...
Next.js hat einen **clientseitigen In-Memory-Cache** namens [Router Cache](/docs/14/app/building-your-application/caching#router-cache). Wenn Benutzer durch die App navigieren, werden die React Server Component Payloads von [geprefetchten](#2-prefetching) Routensegmenten und besuchten Routen im Cache gespeichert.
...
Erfahren Sie mehr darüber, wie der [Router Cache](/docs/14/app/building-your-application/caching#router-cache) funktioniert und wie Sie ihn konfigurieren können.
...
Standardmäßig behält Next.js die Scroll-Position für Vor- und Zurück-Navigation bei und verwendet Routensegmente aus dem [Router Cache](/docs/14/app/building-your-application/caching#router-cache) wieder.
