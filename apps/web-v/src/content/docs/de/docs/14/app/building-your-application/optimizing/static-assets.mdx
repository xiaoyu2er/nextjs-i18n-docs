...
translation-updated-at: 2025-06-06T17:40:57.002Z
title: Statische Assets im `public`-Verzeichnis
nav_title: Statische Assets
...
Für statische Metadaten-Dateien wie `robots.txt`, `favicon.ico` usw. sollten Sie [spezielle Metadaten-Dateien](/docs/14/app/api-reference/file-conventions/metadata) im `app`-Ordner verwenden.
...
> - Nur Assets, die sich zum Zeitpunkt des [Builds](/docs/14/app/api-reference/cli/next#next-build-options) im `public`-Verzeichnis befinden, werden von Next.js bereitgestellt. Zur Laufzeit hinzugefügte Dateien sind nicht verfügbar. Für persistente Dateispeicherung empfehlen wir einen Drittanbieterdienst wie [Vercel Blob](https://vercel.com/docs/storage/vercel-blob?utm_source=next-site&utm_medium=docs&utm_campaign=next-website).