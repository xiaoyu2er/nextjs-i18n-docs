...
> **Wichtig**: Die [`edge`-Runtime](/docs/14/pages/api-reference/edge) ist derzeit nicht mit ISR kompatibel, obwohl Sie `stale-while-revalidate` nutzen können, indem Sie den `cache-control`-Header manuell setzen.
...
Fügen Sie das Geheimnis als [Umgebungsvariable](/docs/14/pages/building-your-application/configuring/environment-variables) zu Ihrer Anwendung hinzu. Erstellen Sie dann die Revalidierungs-API-Route:
...
Bei lokaler Ausführung mit `next dev` wird `getStaticProps` bei jeder Anfrage aufgerufen. Um zu überprüfen, ob Ihre On-Demand-ISR-Konfiguration korrekt ist, müssen Sie einen [Produktions-Build](/docs/14/pages/api-reference/cli/next#next-build-options) erstellen und den [Produktions-Server](/docs/14/pages/api-reference/cli/next#next-start-options) starten:
...
Incremental Static Regeneration (ISR) funktioniert bei [selbst gehosteten Next.js-Sites](/docs/14/pages/building-your-application/deploying#self-hosting) ohne zusätzliche Konfiguration, wenn Sie `next start` verwenden.

Erfahren Sie mehr über [Self-hosting mit Next.js](/docs/14/pages/building-your-application/deploying#self-hosting).
