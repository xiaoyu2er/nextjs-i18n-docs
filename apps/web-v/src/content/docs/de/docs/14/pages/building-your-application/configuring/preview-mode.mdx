...
translation-updated-at: 2025-06-06T17:25:17.259Z
title: Vorschaumodus
...
> **Hinweis**: Diese Funktion wurde durch [Draft Mode](/docs/14/pages/building-your-application/configuring/draft-mode) ersetzt.
...
In der [Pages-Dokumentation](/docs/14/pages/building-your-application/routing/pages-and-layouts) und der [Data Fetching-Dokumentation](/docs/14/pages/building-your-application/data-fetching) haben wir besprochen, wie Sie eine Seite zur Build-Zeit (**Static Generation**) mit `getStaticProps` und `getStaticPaths` vorrendern können.

Statische Generierung ist nützlich, wenn Ihre Seiten Daten von einem Headless-CMS abrufen. Sie ist jedoch nicht ideal, wenn Sie einen Entwurf in Ihrem Headless-CMS schreiben und diesen Entwurf sofort auf Ihrer Seite **vorschauen** möchten. In diesem Fall sollten Sie Next.js anweisen, diese Seiten zur **Anfragezeit** statt zur Build-Zeit zu rendern und den Entwurfsinhalt statt des veröffentlichten Inhalts abzurufen. Sie möchten, dass Next.js die statische Generierung nur für diesen speziellen Fall umgeht.

Next.js verfügt über eine Funktion namens **Vorschaumodus (Preview Mode)**, die dieses Problem löst. Hier finden Sie Anleitungen zur Verwendung.

## Schritt 1: Erstellen und Zugreifen auf eine Preview-API-Route

> Werfen Sie zunächst einen Blick in die [API Routes-Dokumentation](/docs/14/pages/building-your-application/routing/api-routes), wenn Sie mit Next.js-API-Routen nicht vertraut sind.
...
In der Praxis möchten Sie diese API-Route _sicher_ von Ihrem Headless-CMS aus aufrufen. Die genauen Schritte hängen davon ab, welches Headless-CMS Sie verwenden, aber hier sind einige allgemeine Schritte, die Sie unternehmen können.

Diese Schritte setzen voraus, dass Ihr Headless-CMS die Einstellung von **benutzerdefinierten Vorschau-URLs** unterstützt. Falls nicht, können Sie diese Methode dennoch verwenden, um Ihre Vorschau-URLs zu sichern, müssen die Vorschau-URL jedoch manuell erstellen und aufrufen.

**Erstens** sollten Sie einen **geheimen Token-String** mit einem Token-Generator Ihrer Wahl erstellen. Dieses Geheimnis sollte nur Ihrer Next.js-App und Ihrem Headless-CMS bekannt sein. Es verhindert, dass Personen ohne Zugriff auf Ihr CMS auf Vorschau-URLs zugreifen können.

**Zweitens**, wenn Ihr Headless-CMS die Einstellung benutzerdefinierter Vorschau-URLs unterstützt, geben Sie Folgendes als Vorschau-URL an. Dabei wird davon ausgegangen, dass sich Ihre Preview-API-Route unter `pages/api/preview.js` befindet.

```bash filename="Terminal"
https://<ihre-website>/api/preview?secret=<token>&slug=<pfad>
```

- `<ihre-website>` sollte Ihre Bereitstellungsdomain sein.
- `<token>` sollte durch den generierten geheimen Token ersetzt werden.
- `<pfad>` sollte der Pfad der Seite sein, die Sie in der Vorschau sehen möchten. Wenn Sie `/posts/foo` in der Vorschau sehen möchten, sollten Sie `&slug=/posts/foo` verwenden.

Ihr Headless-CMS erlaubt möglicherweise die Verwendung einer Variablen in der Vorschau-URL, sodass `<pfad>` dynamisch basierend auf den Daten des CMS gesetzt werden kann, z.B.: `&slug=/posts/{entry.fields.slug}`
...
export default async (req, res) => {
  // Überprüfen des geheimen Tokens und der nächsten Parameter
  ...
  // Das Headless-CMS abfragen, um zu überprüfen, ob der angegebene `slug` existiert
  ...
  // Vorschaumodus durch Setzen der Cookies aktivieren
  res.setPreviewData({})
...
Wenn Sie eine Seite mit `getStaticProps` aufrufen, während die Vorschaumodus-Cookies gesetzt sind (über `res.setPreviewData`), wird `getStaticProps` zur **Anfragezeit** (statt zur Build-Zeit) aufgerufen.
...
export async function getStaticProps(context) {
  // Wenn Sie diese Seite mit gesetzten Vorschaumodus-Cookies aufrufen:
  //
...
Sie können `getStaticProps` so aktualisieren, dass basierend auf `context.preview` und/oder `context.previewData` unterschiedliche Daten abgerufen werden.

Ihr Headless-CMS verfügt möglicherweise über einen anderen API-Endpunkt für Entwurfsposts. In diesem Fall können Sie `context.preview` verwenden, um die API-Endpunkt-URL wie folgt zu modifizieren:
...
Das war's! Wenn Sie die Preview-API-Route (mit `secret` und `slug`) von Ihrem Headless-CMS oder manuell aufrufen, sollten Sie nun den Vorschauinhalt sehen können. Und wenn Sie Ihren Entwurf ohne Veröffentlichung aktualisieren, sollten Sie den Entwurf in der Vorschau sehen können.
...
> **Gut zu wissen**: Während des Renderings stellt `next/router` ein `isPreview`-Flag bereit, siehe die [Router-Objekt-Dokumentation](/docs/14/pages/api-reference/functions/use-router#router-object) für weitere Informationen.
...
Senden Sie dann eine Anfrage an `/api/clear-preview-mode-cookies`, um die API-Route aufzurufen. Wenn Sie diese Route mit [`next/link`](/docs/14/pages/api-reference/components/link) aufrufen, müssen Sie `prefetch={false}` übergeben, um zu verhindern, dass `clearPreviewData` während des Link-Prefetchings aufgerufen wird.
...
Der Vorschaumodus funktioniert auch mit `getServerSideProps`. Er ist ebenfalls im `context`-Objekt mit `preview` und `previewData` verfügbar.

> **Gut zu wissen**: Sie sollten den `Cache-Control`-Header nicht setzen, wenn Sie den Vorschaumodus verwenden, da er nicht umgangen werden kann. Wir empfehlen stattdessen die Verwendung von [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration).

### Funktioniert mit API-Routen

API-Routen haben Zugriff auf `preview` und `previewData` über das Request-Objekt. Zum Beispiel:
...
> **Gut zu wissen**: Um den Vorschaumodus lokal über HTTP zu testen, muss Ihr Browser Cookies von Drittanbietern und den Zugriff auf den lokalen Speicher zulassen.