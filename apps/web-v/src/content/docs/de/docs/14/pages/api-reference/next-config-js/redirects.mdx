...
translation-updated-at: 2025-06-06T17:19:04.902Z
...
Bei Verwendung des Pages-Routers werden Weiterleitungen nicht auf das clientseitige Routing (`Link`, `router.push`) angewendet, es sei denn, [Middleware](/docs/14/app/building-your-application/routing/middleware) ist vorhanden und passt zum Pfad.
...
Bei Verwendung von [`basePath`-Unterstützung](/docs/14/app/api-reference/next-config-js/basePath) mit Weiterleitungen wird jeder `source` und `destination` automatisch mit dem `basePath` präfixiert, es sei denn, Sie fügen `basePath: false` zur Weiterleitung hinzu:
...
Bei Verwendung von [`i18n`-Unterstützung](/docs/14/app/building-your-application/routing/internationalization) mit Weiterleitungen wird jeder `source` und `destination` automatisch präfixiert, um die konfigurierten `locales` zu handhaben, es sei denn, Sie fügen `locale: false` zur Weiterleitung hinzu. Wenn `locale: false` verwendet wird, müssen Sie `source` und `destination` mit einem Locale präfixieren, damit sie korrekt gematcht werden.

</AppOnly>

<PagesOnly>

Bei Verwendung von [`i18n`-Unterstützung](/docs/14/pages/building-your-application/routing/internationalization) mit Weiterleitungen wird jeder `source` und `destination` automatisch präfixiert, um die konfigurierten `locales` zu handhaben, es sei denn, Sie fügen `locale: false` zur Weiterleitung hinzu. Wenn `locale: false` verwendet wird, müssen Sie `source` und `destination` mit einem Locale präfixieren, damit sie korrekt gematcht werden.
...
- Innerhalb von [API-Routen](/docs/14/pages/building-your-application/routing/api-routes) und [Route Handlern](/docs/14/app/building-your-application/routing/route-handlers) können Sie basierend auf der eingehenden Anfrage weiterleiten.
- Innerhalb von [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) und [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) können Sie bestimmte Seiten zur Laufzeit weiterleiten.
