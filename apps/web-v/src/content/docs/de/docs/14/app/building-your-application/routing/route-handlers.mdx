...
> **Wichtig zu wissen**: Route Handler sind nur innerhalb des `app`-Verzeichnisses verfügbar. Sie entsprechen den [API-Routen](/docs/14/pages/building-your-application/routing/api-routes) innerhalb des `pages`-Verzeichnisses, was bedeutet, dass Sie API-Routen und Route Handler **nicht** zusammen verwenden müssen.

## Konvention

Route Handler werden in einer [`route.js|ts`-Datei](/docs/14/app/api-reference/file-conventions/route) innerhalb des `app`-Verzeichnisses definiert:
...
[`NextRequest`](/docs/14/app/api-reference/functions/next-request) und [`NextResponse`](/docs/14/app/api-reference/functions/next-response), um praktische Hilfsmittel für erweiterte Anwendungsfälle bereitzustellen.
...
> **TypeScript-Warnung:** `Response.json()` ist erst ab TypeScript 5.2 gültig. Wenn Sie eine ältere TypeScript-Version verwenden, können Sie stattdessen [`NextResponse.json()`](/docs/14/app/api-reference/functions/next-response#json) für typisierte Antworten verwenden.
...
> **Wichtig zu wissen**: Wie API-Routen können Route Handler für Fälle wie die Verarbeitung von Formulareingaben verwendet werden. Eine neue Abstraktion für die [Verarbeitung von Formularen und Mutationen](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations), die eng mit React integriert ist, wird derzeit entwickelt.
...
Sie können [zwischengespeicherte Daten neu validieren](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) mit der [`next.revalidate`](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data)-Option:
...
Alternativ können Sie die [`revalidate`-Segmentkonfigurationsoption](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) verwenden:
...
Route Handler können mit dynamischen Funktionen von Next.js verwendet werden, wie [`cookies`](/docs/14/app/api-reference/functions/cookies) und [`headers`](/docs/14/app/api-reference/functions/headers).

#### Cookies

Sie können Cookies mit [`cookies`](/docs/14/app/api-reference/functions/cookies) aus `next/headers` lesen oder setzen. Diese Serverfunktion kann direkt in einem Route Handler aufgerufen oder in einer anderen Funktion verschachtelt werden.
...
Sie können auch die zugrunde liegenden Web-APIs verwenden, um Cookies aus der Anfrage ([`NextRequest`](/docs/14/app/api-reference/functions/next-request)) zu lesen:
...
Sie können Header mit [`headers`](/docs/14/app/api-reference/functions/headers) aus `next/headers` lesen. Diese Serverfunktion kann direkt in einem Route Handler aufgerufen oder in einer anderen Funktion verschachtelt werden.
...
Sie können auch die zugrunde liegenden Web-APIs verwenden, um Header aus der Anfrage ([`NextRequest`](/docs/14/app/api-reference/functions/next-request)) zu lesen:
...
> Wir empfehlen, die Seite [Defining Routes](/docs/14/app/building-your-application/routing/defining-routes) zu lesen, bevor Sie fortfahren.

Route Handler können [Dynamische Segmente](/docs/14/app/building-your-application/routing/dynamic-routes) verwenden, um Anfragehandler aus dynamischen Daten zu erstellen.
...
Das an den Route Handler übergebene Anfrageobjekt ist eine `NextRequest`-Instanz, die [einige zusätzliche Hilfsmethoden](/docs/14/app/api-reference/functions/next-request#nexturl) bietet, einschließlich der einfacheren Handhabung von Abfrageparametern.
...
> **Gut zu wissen**:
>
> - Um CORS-Header mehreren Route Handlern hinzuzufügen, können Sie [Middleware](/docs/14/app/building-your-application/routing/middleware#cors) oder die [`next.config.js`-Datei](/docs/14/app/api-reference/next-config-js/headers#cors) verwenden.
...
Route Handler bieten eine isomorphe Web-API, die nahtlos sowohl Edge- als auch Node.js-Runtimes unterstützt, inklusive Streaming. Da Route Handler dieselbe [Route-Segment-Konfiguration](/docs/14/app/api-reference/file-conventions/route-segment-config) wie Seiten und Layouts verwenden, unterstützen sie lang erwartete Funktionen wie allgemein nutzbare [statisch regenerierte](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) Route Handler.
...
Route Handler verwenden dieselbe [Route-Segment-Konfiguration](/docs/14/app/api-reference/file-conventions/route-segment-config) wie Seiten und Layouts.
...
Weitere Details finden Sie in der [API-Referenz](/docs/14/app/api-reference/file-conventions/route-segment-config).
