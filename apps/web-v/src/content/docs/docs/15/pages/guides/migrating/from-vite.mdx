...
With built-in support for [streaming through React Suspense](/docs/15/app/getting-started/linking-and-navigating#streaming), you can be more intentional about which parts of your UI you want to load first and in what order without introducing network waterfalls.
...
[Next.js Middleware](/docs/15/app/api-reference/file-conventions/middleware) allows you to run code on the server before a request is completed. This is especially useful to avoid having a flash of unauthenticated content when the user visits an authenticated-only page by redirecting the user to a login page. The middleware is also useful for experimentation and [internationalization](/docs/15/app/guides/internationalization).

### Built-in Optimizations

[Images](/docs/15/app/api-reference/components/image), [fonts](/docs/15/app/api-reference/components/font), and [third-party scripts](/docs/15/app/guides/scripts) often have significant impact on an application's performance. Next.js comes with built-in components that automatically optimize those for you.
...
[Next.js configuration options](/docs/15/app/api-reference/config/next-config-js).
...
[Next.js docs](/docs/15/app/api-reference/config/typescript#ide-plugin).

### Step 4: Create the Root Layout

A Next.js [App Router](/docs/15/app) application must include a
[root layout](/docs/15/app/api-reference/file-conventions/layout#root-layout)
file, which is a [React Server Component](/docs/15/app/getting-started/server-and-client-components)
...
5. Any [metadata files](/docs/15/app/getting-started/metadata-and-og-images#file-based-metadata)
   ...
   moving
   [all supported files](/docs/15/app/getting-started/metadata-and-og-images#file-based-metadata)
...
6. Finally, Next.js can manage your last `<head>` tags with the
   [Metadata API](/docs/15/app/getting-started/metadata-and-og-images). Move your final metadata
   info into an exported
   [`metadata` object](/docs/15/app/api-reference/functions/generate-metadata#metadata-object):
...
([Metadata API](/docs/15/app/getting-started/metadata-and-og-images)). This approach enables you
...
This file is a [Server Component](/docs/15/app/getting-started/server-and-client-components). When you run `next build`, the file is prerendered into a static asset. It does _not_ require any dynamic code.

This file imports our global CSS and tells [`generateStaticParams`](/docs/15/app/api-reference/functions/generate-static-params) we are only going to generate one route, the index route at `/`.
...
This file is a [Client Component](/docs/15/app/getting-started/server-and-client-components), defined by the `'use client'`
directive. Client Components are still [prerendered to HTML](/docs/15/app/getting-started/server-and-client-components#how-do-server-and-client-components-work-in-nextjs) on the server before being sent to the client.
...
Next.js [`<Image>` component](/docs/15/app/api-reference/components/image), or you can use the object's
`src` property with your existing `<img>` tag.

The `<Image>` component has the added benefits of
[automatic image optimization](/docs/15/app/api-reference/components/image). The `<Image>`
...
issues. You can then optionally later migrate to the `<Image>` component to take advantage of optimizing images by [configuring a loader](/docs/15/app/api-reference/components/image#loader), or moving to the default Next.js server which has automatic image optimization.
...
2. **Set [`basePath`](/docs/15/app/api-reference/config/next-config-js/basePath) to `process.env.NEXT_PUBLIC_BASE_PATH` in your `next.config.mjs` file:**
...
- Migrate from React Router to the [Next.js App Router](/docs/15/app) to get:
...
- [Update your ESLint configuration to support Next.js rules](/docs/15/app/api-reference/config/eslint)
