...
The Next.js [`<Image>`](/docs/15/app/api-reference/components/image) component extends the HTML `<img>` element to provide:
...
You can store static files, like images and fonts, under a folder called [`public`](/docs/15/app/api-reference/file-conventions/public-folder) in the root directory. Files inside `public` can then be referenced by your code starting from the base URL (`/`).
