...
This loads `process.env.DB_HOST`, `process.env.DB_USER`, and `process.env.DB_PASS` into the Node.js environment automatically allowing you to use them in [Next.js data fetching methods](/docs/15/pages/building-your-application/data-fetching) and [API routes](/docs/15/pages/building-your-application/routing/api-routes).

For example, using [`getStaticProps`](/docs/15/pages/building-your-application/data-fetching/get-static-props):
...
> This loads `process.env.DB_HOST`, `process.env.DB_USER`, and `process.env.DB_PASS` into the Node.js environment automatically allowing you to use them in [Route Handlers](/docs/15/app/api-reference/file-conventions/route).
...
- We do not recommend using the [`runtimeConfig`](/docs/15/pages/api-reference/config/next-config-js/runtime-configuration) option, as this does not work with the standalone output mode. Instead, we recommend [incrementally adopting](/docs/15/app/guides/migrating/app-router-migration) the App Router if you need this feature.
...
- If you are using a [`/src` directory](/docs/15/app/api-reference/file-conventions/src-folder), `.env.*` files should remain in the root of your project.
