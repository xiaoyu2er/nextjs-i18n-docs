...
A zone is a normal Next.js application where you also configure an [assetPrefix](/docs/15/app/api-reference/config/next-config-js/assetPrefix) to avoid conflicts with pages and static files in other zones.
...
Links to paths in a different zone should use an `a` tag instead of the Next.js [`<Link>`](/docs/15/pages/api-reference/components/link) component. This is because Next.js will try to prefetch and soft navigate to any relative path in `<Link>` component, which will not work across zones.
...
When using [Server Actions](/docs/15/app/getting-started/updating-data) with Multi-Zones, you must explicitly allow the user-facing origin since your user facing domain may serve multiple applications. In your `next.config.js` file, add the following lines:
...
See [`serverActions.allowedOrigins`](/docs/15/app/api-reference/config/next-config-js/serverActions#allowedorigins) for more information.
