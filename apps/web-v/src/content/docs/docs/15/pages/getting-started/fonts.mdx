...
The [`next/font`](/docs/15/app/api-reference/components/font) module automatically optimizes your fonts and removes external network requests for improved privacy and performance.
...
Fonts are scoped to the component they're used in. To apply a font to your entire application, add it to the [Root Layout](/docs/15/app/api-reference/file-conventions/layout#root-layout).
...
To use a local font, import your font from `next/font/local` and specify the [`src`](/docs/15/app/api-reference/components/font#src) of your local font file. Fonts can be stored in the [`public`](/docs/15/app/api-reference/file-conventions/public-folder) folder or co-located inside the `app` folder. For example:
