...
[Middleware](/docs/15/app/api-reference/file-conventions/middleware) enables you to add headers and generate nonces before the page renders.

Every time a page is viewed, a fresh nonce should be generated. This means that you **must use [dynamic rendering](/docs/15/app/getting-started/partial-prerendering#dynamic-rendering) to add nonces**.
...
By default, Middleware runs on all requests. You can filter Middleware to run on specific paths using a [`matcher`](/docs/15/app/api-reference/file-conventions/middleware#matcher).
...
You can read the nonce from a [Server Component](/docs/15/app/getting-started/server-and-client-components) using [`headers`](/docs/15/app/api-reference/functions/headers):
...
For applications that do not require nonces, you can set the CSP header directly in your [`next.config.js`](/docs/15/app/api-reference/config/next-config-js) file:
