...
In development mode, `fetch` data is [reused for Hot Module Replacement (HMR)](/docs/15/app/api-reference/functions/fetch#fetch-default-auto-no-store-and-cache-no-store-not-showing-fresh-data-in-development), and caching options are ignored for [hard refreshes](/docs/15/app/api-reference/functions/fetch#hard-refresh-and-caching-in-development).
...
>
> To learn more, see the [Server Components](/docs/15/app/getting-started/server-and-client-components) documentation.
...
Learn more about [static and dynamic rendering](/docs/15/app/getting-started/partial-prerendering#static-rendering).
...
- **[Revalidating Data](/docs/15/app/guides/caching#revalidating)**: Revalidating the [Data Cache](#data-cache), will in turn invalidate the Router Cache by re-rendering components on the server and caching the new render output.
...
When a user navigates between routes, Next.js caches the visited route segments and [prefetches](/docs/15/app/getting-started/linking-and-navigating#prefetching) the routes the user is likely to navigate to. This results in instant back/forward navigation, no full-page reload between navigations, and preservation of browser state and React state in shared layouts.

With the Router Cache:

- **Layouts** are cached and reused on navigation ([partial rendering](/docs/15/app/getting-started/linking-and-navigating#client-side-transitions)).
- **Loading states** are cached and reused on navigation for [instant navigation](/docs/15/app/api-reference/file-conventions/loading#instant-loading-states).
- **Pages** are not cached by default, but are reused during browser backward and forward navigation. You can enable caching for page segments by using the experimental [`staleTimes`](/docs/15/app/api-reference/config/next-config-js/staleTimes) config option.
...
> **Good to know**: The experimental [`staleTimes`](/docs/15/app/api-reference/config/next-config-js/staleTimes) config option can be used to adjust the automatic invalidation times mentioned above.
...
- In a **Server Action**:
  - Revalidating data on-demand by path with ([`revalidatePath`](/docs/15/app/api-reference/functions/revalidatePath)) or by cache tag with ([`revalidateTag`](/docs/15/app/api-reference/functions/revalidateTag))
  - Using [`cookies.set`](/docs/15/app/api-reference/functions/cookies#setting-a-cookie) or [`cookies.delete`](/docs/15/app/api-reference/functions/cookies#deleting-cookies) invalidates the Router Cache to prevent routes that use cookies from becoming stale (e.g. authentication).
- Calling [`router.refresh`](/docs/15/app/api-reference/functions/use-router) will invalidate the Router Cache and make a new request to the server for the current route.

### Opting out

As of Next.js 15, page segments are opted out by default.

> **Good to know:** You can also opt out of [prefetching](/docs/15/app/getting-started/linking-and-navigating#prefetching) by setting the `prefetch` prop of the `<Link>` component to `false`.
...
| [`unstable_cache`](/docs/15/app/api-reference/functions/unstable_cache)    |                            |                       | Cache                 |                      |
...
Learn more about the [`<Link>` component](/docs/15/app/api-reference/components/link).
...
See the [`useRouter` hook](/docs/15/app/api-reference/functions/use-router) API reference.
...
See the [`fetch` API Reference](/docs/15/app/api-reference/functions/fetch) for more options.
...
1. When using `fetch` or [`unstable_cache`](/docs/15/app/api-reference/functions/unstable_cache), you have the option to tag cache entries with one or more tags.
...
1. [Route Handlers](/docs/15/app/api-reference/file-conventions/route) - to revalidate data in response of a third party event (e.g. webhook). This will not invalidate the Router Cache immediately as the Router Handler isn't tied to a specific route.
2. [Server Actions](/docs/15/app/getting-started/updating-data) - to revalidate data after a user action (e.g. form submission). This will invalidate the Router Cache for the associated route.
...
1. [Route Handlers](/docs/15/app/api-reference/file-conventions/route) - to revalidate data in response to a third party event (e.g. webhook).
2. [Server Actions](/docs/15/app/getting-started/updating-data) - to revalidate data after a user interaction (e.g. form submission, clicking a button).

See the [`revalidatePath` API reference](/docs/15/app/api-reference/functions/revalidatePath) for more information.
...
See the [`cookies`](/docs/15/app/api-reference/functions/cookies) API reference.
...
See the [`fetchCache`](/docs/15/app/api-reference/file-conventions/route-segment-config#fetchcache) to see more advanced options.

See the [Route Segment Config](/docs/15/app/api-reference/file-conventions/route-segment-config) documentation for more options.

### `generateStaticParams`

For [dynamic segments](/docs/15/app/api-reference/file-conventions/dynamic-routes) (e.g. `app/blog/[slug]/page.js`), paths provided by `generateStaticParams` are cached in the Full Route Cache at build time. At request time, Next.js will also cache paths that weren't known at build time the first time they're visited.
...
To statically render all paths the first time they're visited, return an empty array (no paths will be rendered at build time) or utilize [`export const dynamic = 'force-static'`](/docs/15/app/api-reference/file-conventions/route-segment-config#dynamic):
