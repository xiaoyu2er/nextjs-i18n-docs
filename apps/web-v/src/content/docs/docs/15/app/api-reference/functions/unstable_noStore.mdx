...
**In version 15, we recommend using [`connection`](/docs/15/app/api-reference/functions/connection) instead of `unstable_noStore`.**
...
- Using `unstable_noStore` inside [`unstable_cache`](/docs/15/app/api-reference/functions/unstable_cache) will not opt out of static generation. Instead, it will defer to the cache configuration to determine whether to cache the result or not.
