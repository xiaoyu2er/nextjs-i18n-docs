...
> - Learn more about the [`mdx-components.tsx` file convention](/docs/15/app/api-reference/file-conventions/mdx-components).
...
In App Router apps, that includes being able to use [metadata](/docs/15/app/getting-started/metadata-and-og-images).
...
[`generateStaticParams`](/docs/15/app/api-reference/functions/generate-static-params) can be used to prerender the provided routes. By marking `dynamicParams` as `false`, accessing a route not defined in `generateStaticParams` will 404.
...
> **Good to know**: Ensure you specify the `.mdx` file extension in your import. While it is not required to use [module path aliases](/docs/15/app/getting-started/installation#set-up-absolute-imports-and-module-path-aliases) (e.g., `@/content`), it does simplify your import path.
...
To share a layout across MDX pages, you can use the [built-in layouts support](/docs/15/app/api-reference/file-conventions/layout) with the App Router.
...
To use plugins with [Turbopack](/docs/15/app/api-reference/turbopack), upgrade to the latest `@next/mdx` and specify plugin names using a string:
...
> **Good to know**:
>
> remark and rehype plugins without serializable options cannot be used yet with [Turbopack](/docs/15/app/api-reference/turbopack), because JavaScript functions can't be passed to Rust.
