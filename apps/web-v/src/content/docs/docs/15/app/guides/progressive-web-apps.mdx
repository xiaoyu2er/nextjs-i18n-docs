...
Next.js provides built-in support for creating a [web app manifest](/docs/15/app/api-reference/file-conventions/metadata/manifest) using the App Router. You can create either a static or dynamic manifest file:
...
- You are [running locally with HTTPS](/docs/15/app/api-reference/cli/next#using-https-during-development)
...
Learn more about defining [Content Security Policies](/docs/15/app/guides/content-security-policy) with Next.js.
...
2. **Static Exports:** If your application requires not running a server, and instead using a static export of files, you can update the Next.js configuration to enable this change. Learn more in the [Next.js Static Export documentation](/docs/15/app/guides/static-exports). However, you will need to move from Server Actions to calling an external API, as well as moving your defined headers to your proxy.
