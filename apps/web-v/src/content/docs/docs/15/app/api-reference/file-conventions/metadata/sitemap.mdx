...
> **Good to know**: `sitemap.js` is a special Route Handler that is cached by default unless it uses a [Dynamic API](/docs/15/app/guides/caching#dynamic-apis) or [dynamic config](/docs/15/app/guides/caching#segment-config-options) option.
...
- By using the [`generateSitemaps`](/docs/15/app/api-reference/functions/generate-sitemaps) function.
...
See the [`generateSitemaps` API reference](/docs/15/app/api-reference/functions/generate-sitemaps) for more information.
