...
The first step is to create a [Server Component](/docs/15/app/getting-started/server-and-client-components) that generates the appropriate iframe for embedding the video. This component will fetch the source URL for the video and render the iframe.
...
After creating the Server Component to embed the video, the next step is to [stream](/docs/15/app/api-reference/file-conventions/loading) the component using [React Suspense](https://react.dev/reference/react/Suspense).
