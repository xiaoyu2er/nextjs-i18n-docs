...
The `useRouter` hook allows you to programmatically change routes inside [Client Components](/docs/15/app/getting-started/server-and-client-components).

> **Recommendation:** Use the [`<Link>` component](/docs/15/app/api-reference/components/link) for navigation unless you have a specific requirement for using `useRouter`.
...
- `router.prefetch(href: string, options?: { onInvalidate?: () => void })`: [Prefetch](/docs/15/app/getting-started/linking-and-navigating#prefetching) the provided route for faster client-side transitions. The optional `onInvalidate` callback is called when the [prefetched data becomes stale](/docs/15/app/guides/prefetching#extending-or-ejecting-link).
...
- The `pathname` string has been removed and is replaced by [`usePathname()`](/docs/15/app/api-reference/functions/use-pathname)
- The `query` object has been removed and is replaced by [`useSearchParams()`](/docs/15/app/api-reference/functions/use-search-params)
- `router.events` has been replaced. [See below.](#router-events)

[View the full migration guide](/docs/15/app/guides/migrating/app-router-migration).
...
> **Good to know**: `<NavigationEvents>` is wrapped in a [`Suspense` boundary](/docs/15/app/api-reference/file-conventions/loading#examples) because[`useSearchParams()`](/docs/15/app/api-reference/functions/use-search-params) causes client-side rendering up to the closest `Suspense` boundary during [static rendering](/docs/15/app/getting-started/partial-prerendering#static-rendering). [Learn more](/docs/15/app/api-reference/functions/use-search-params#behavior).
