...
- **`not-found.js`**: Used when you call the [`notFound`](/docs/15/app/api-reference/functions/not-found) function in a route segment.
...
The **not-found** file is used to render UI when the [`notFound`](/docs/15/app/api-reference/functions/not-found) function is thrown within a route segment. Along with serving a custom UI, Next.js will return a `200` HTTP status code for streamed responses, and `404` for non-streamed responses.
...
For `global-not-found.js`, you can export a `metadata` object or a [`generateMetadata`](/docs/15/app/api-reference/functions/generate-metadata) function to customize the `<title>`, `<meta>`, and other head tags for your 404 page:
