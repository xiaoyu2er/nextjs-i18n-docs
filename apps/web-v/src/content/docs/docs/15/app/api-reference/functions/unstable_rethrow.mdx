...
For example, calling the `notFound` function will throw an internal Next.js error and render the [`not-found.js`](/docs/15/app/api-reference/file-conventions/not-found) component. However, if used inside the `try` block of a `try/catch` statement, the error will be caught, preventing `not-found.js` from rendering:
...
- [`notFound()`](/docs/15/app/api-reference/functions/not-found)
- [`redirect()`](/docs/15/app/guides/redirecting#redirect-function)
- [`permanentRedirect()`](/docs/15/app/guides/redirecting#permanentredirect-function)
...
- [`searchParams`](/docs/15/app/api-reference/file-conventions/page#searchparams-optional)
