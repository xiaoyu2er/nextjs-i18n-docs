...
You can use the [`<form>`](https://react.dev/reference/react-dom/components/form) element with React's [Server Actions](/docs/15/app/getting-started/updating-data) and `useActionState` to capture user credentials, validate form fields, and call your Authentication Provider's API or database.
...
1. Generate a secret key, which will be used to sign your session, and store it as an [environment variable](/docs/15/app/guides/environment-variables).
2. Write logic to encrypt/decrypt session data using a session management library.
3. Manage cookies using the Next.js [`cookies`](/docs/15/app/api-reference/functions/cookies) API.
...
This command generates a 32-character random string that you can use as your secret key and store in your [environment variables file](/docs/15/app/guides/environment-variables):
...
To store the session in a cookie, use the Next.js [`cookies`](/docs/15/app/api-reference/functions/cookies) API. The cookie should be set on the server, and include the recommended options:
...
Back in your Server Action, you can invoke the `createSession()` function, and use the [`redirect()`](/docs/15/app/guides/redirecting) API to redirect the user to the appropriate page:
...
You can use [API Routes](/docs/15/pages/building-your-application/routing/api-routes) to set the session as a cookie on the server:
...
There are some cases where you may want to use [Middleware](/docs/15/app/api-reference/file-conventions/middleware) and redirect users based on permissions:
...
However, since Middleware runs on every route, including [prefetched](/docs/15/app/getting-started/linking-and-navigating#prefetching) routes, it's important to only read the session from the cookie (optimistic checks), and avoid database checks to prevent performance issues.
...
> - Middleware uses the [Edge Runtime](/docs/15/app/api-reference/edge), check if your Auth library and session management library are compatible.
...
Auth check in [Server Components](/docs/15/app/getting-started/server-and-client-components) are useful for role-based access. For example, to conditionally render components based on the user's role:
...
Due to [Partial Rendering](/docs/15/app/getting-started/linking-and-navigating#client-side-transitions), be cautious when doing checks in [Layouts](/docs/15/app/api-reference/file-conventions/layout) as these don't re-render on navigation, meaning the user session won't be checked on every route change.
...
Treat [Server Actions](/docs/15/app/getting-started/updating-data) with the same security considerations as public-facing API endpoints, and verify if the user is allowed to perform a mutation.
...
Treat [Route Handlers](/docs/15/app/api-reference/file-conventions/route) with the same security considerations as public-facing API endpoints, and verify if the user is allowed to access the Route Handler.
...
Using context providers for auth works due to [interleaving](/docs/15/app/getting-started/server-and-client-components#examples#interleaving-server-and-client-components). However, React `context` is not supported in Server Components, making them only applicable to Client Components.
