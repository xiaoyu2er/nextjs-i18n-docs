...
It is expected to be used in conjunction with [`use cache`](/docs/15/app/api-reference/directives/use-cache) so that your data fetching happens at runtime by default unless you define specific parts of your application to be cached with `use cache` at the page, function, or component level.
...
- The [`use cache` directive](/docs/15/app/api-reference/directives/use-cache)
- The [`cacheLife` function](/docs/15/app/api-reference/config/next-config-js/cacheLife) with `use cache`
- The [`cacheTag` function](/docs/15/app/api-reference/functions/cacheTag)
