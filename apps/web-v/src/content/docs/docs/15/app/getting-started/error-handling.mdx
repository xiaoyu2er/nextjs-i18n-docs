...
Expected errors are those that can occur during the normal operation of the application, such as those from [server-side form validation](/docs/15/app/guides/forms) or failed requests. These errors should be handled explicitly and returned to the client.
...
When fetching data inside of a Server Component, you can use the response to conditionally render an error message or [`redirect`](/docs/15/app/api-reference/functions/redirect).
...
You can call the [`notFound`](/docs/15/app/api-reference/functions/not-found) function within a route segment and use the [`not-found.js`](/docs/15/app/api-reference/file-conventions/not-found) file to show a 404 UI.
...
Create an error boundary by adding an [`error.js`](/docs/15/app/api-reference/file-conventions/error) file inside a route segment and exporting a React component:
...
Errors will bubble up to the nearest parent error boundary. This allows for granular error handling by placing `error.tsx` files at different levels in the [route hierarchy](/docs/15/app/getting-started/project-structure#component-hierarchy).
...
While less common, you can handle errors in the root layout using the [`global-error.js`](/docs/15/app/api-reference/file-conventions/error#global-error) file, located in the root app directory, even when leveraging [internationalization](/docs/15/app/guides/internationalization). Global error UI must define its own `<html>` and `<body>` tags, since it is replacing the root layout or template when active.
