...
Create React App uses purely client-side rendering. Client-side only applications, also known as [single-page applications (SPAs)](/docs/15/app/guides/single-page-applications), often experience slow initial page loading time. This happens due to a couple of reasons:
...
With built-in support for [streaming through React Suspense](/docs/15/app/getting-started/linking-and-navigating#streaming), you can define which parts of your UI load first and in what order, without creating network waterfalls.
...
[Next.js Middleware](/docs/15/app/api-reference/file-conventions/middleware) allows you to run code on the server before a request is completed. For instance, you can avoid a flash of unauthenticated content by redirecting a user to a login page in the middleware for authenticated-only pages. You can also use it for features like A/B testing, experimentation, and [internationalization](/docs/15/app/guides/internationalization).

### Built-in Optimizations

[Images](/docs/15/app/api-reference/components/image), [fonts](/docs/15/app/api-reference/components/font), and [third-party scripts](/docs/15/app/guides/scripts) often have a large impact on an application’s performance. Next.js includes specialized components and APIs that automatically optimize them for you.

## Migration Steps

Our goal is to get a working Next.js application as quickly as possible so that you can then adopt Next.js features incrementally. To begin with, we’ll treat your application as a purely client-side application ([SPA](/docs/15/app/guides/single-page-applications)) without immediately replacing your existing router. This reduces complexity and merge conflicts.
...
Create a `next.config.ts` at the root of your project (same level as your `package.json`). This file holds your [Next.js configuration options](/docs/15/app/api-reference/config/next-config-js).
...
A Next.js [App Router](/docs/15/app) application must include a [root layout](/docs/15/app/api-reference/file-conventions/layout#root-layout) file, which is a [React Server Component](/docs/15/app/getting-started/server-and-client-components) that will wrap all your pages.
...
> **Good to know**: Next.js ignores CRA’s `public/manifest.json`, additional iconography, and [testing configuration](/docs/15/app/guides/testing) by default. If you need these, Next.js has support with its [Metadata API](/docs/15/app/getting-started/metadata-and-og-images) and [Testing](/docs/15/app/guides/testing) setup.
...
Any [metadata files](/docs/15/app/getting-started/metadata-and-og-images#file-based-metadata) such as `favicon.ico`, `icon.png`, `robots.txt` are automatically added to the application `<head>` tag as long as you have them placed into the top level of the `app` directory. After moving [all supported files](/docs/15/app/getting-started/metadata-and-og-images#file-based-metadata) into the `app` directory you can safely delete their `<link>` tags:
...
Finally, Next.js can manage your last `<head>` tags with the [Metadata API](/docs/15/app/getting-started/metadata-and-og-images). Move your final metadata info into an exported [`metadata` object](/docs/15/app/api-reference/functions/generate-metadata#metadata-object):
...
With the above changes, you shifted from declaring everything in your `index.html` to using Next.js' convention-based approach built into the framework ([Metadata API](/docs/15/app/getting-started/metadata-and-og-images)). This approach enables you to more easily improve your SEO and web shareability of your pages.

### Step 5: Styles

Like CRA, Next.js supports [CSS Modules](/docs/15/app/getting-started/css#css-modules) out of the box. It also supports [global CSS imports](/docs/15/app/getting-started/css#global-css).
...
If you're using Tailwind CSS, see our [installation docs](/docs/15/app/getting-started/css#tailwind-css).
...
Since we want to keep the app as an SPA for now and intercept **all** routes, we’ll use an [optional catch-all route](/docs/15/app/api-reference/file-conventions/dynamic-routes#optional-catch-all-segments).
...
This tells Next.js to generate a single route for the empty slug (`/`), effectively mapping **all** routes to the same page. This page is a [Server Component](/docs/15/app/getting-started/server-and-client-components), prerendered into static HTML.

### Step 7: Add a Client-Only Entrypoint

Next, we’ll embed your CRA’s root App component inside a [Client Component](/docs/15/app/getting-started/server-and-client-components) so that all logic remains client-side. If this is your first time using Next.js, it's worth knowing that clients components (by default) are still prerendered on the server. You can think about them as having the additional capability of running client-side JavaScript.
...
Next.js supports [environment variables](/docs/15/app/guides/environment-variables) similarly to CRA but **requires** a `NEXT_PUBLIC_` prefix for any variable you want to expose in the browser.
...
If you used the `homepage` field in your CRA `package.json` to serve the app under a specific subpath, you can replicate that in Next.js using the [`basePath` configuration](/docs/15/app/api-reference/config/next-config-js/basePath) in `next.config.ts`:
...
If you used CRA’s service worker (e.g., `serviceWorker.js` from `create-react-app`), you can learn how to create [Progressive Web Applications (PWAs)](/docs/15/app/guides/progressive-web-apps) with Next.js.

### Proxying API Requests

If your CRA app used the `proxy` field in `package.json` to forward requests to a backend server, you can replicate this with [Next.js rewrites](/docs/15/app/api-reference/config/next-config-js/rewrites) in `next.config.ts`:
...
Both Create React App and Next.js default to webpack for bundling. Next.js also offers [Turbopack](/docs/15/app/api-reference/config/next-config-js/turbopack) for faster local development with:

```bash
next dev --turbopack
```

You can still provide a [custom webpack configuration](/docs/15/app/api-reference/config/next-config-js/webpack) if you need to migrate advanced webpack settings from CRA.
...
- **Migrate from React Router** to the [Next.js App Router](/docs/15/app) for:
  - Automatic code splitting
  - [Streaming server rendering](/docs/15/app/api-reference/file-conventions/loading)
  - [React Server Components](/docs/15/app/getting-started/server-and-client-components)
- **Optimize images** with the [`<Image>` component](/docs/15/app/api-reference/components/image)
- **Optimize fonts** with [`next/font`](/docs/15/app/api-reference/components/font)
- **Optimize third-party scripts** with the [`<Script>` component](/docs/15/app/guides/scripts)
