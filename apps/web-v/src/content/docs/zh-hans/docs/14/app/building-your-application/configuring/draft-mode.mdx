...
当页面从无头 CMS (headless CMS) 获取数据时，静态渲染非常有用。但若您在无头 CMS 上撰写草稿并希望立即在页面上预览时，静态渲染就不太理想。此时您会希望 Next.js 在**请求时**而非构建时渲染这些页面，并获取草稿内容而非已发布内容。这种情况下，您需要 Next.js 仅针对此特定场景切换至[动态渲染](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)。

Next.js 的**草稿模式 (Draft Mode)**功能正是为此而生。以下是使用指南。

## 步骤一：创建并访问路由处理器 (Route Handler)

首先创建一个[路由处理器](/docs/14/app/building-your-application/routing/route-handlers)，可任意命名（例如 `app/api/draft/route.ts`）。
...
向 `/api/disable-draft` 发送请求即可触发。若使用 [`next/link`](/docs/14/app/api-reference/components/link) 调用此路由，需传递 `prefetch={false}` 以防预取时意外删除 Cookie。
