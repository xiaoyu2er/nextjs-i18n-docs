...
要使用该文件，请将其放置在应用的**根目录**下，或者如果使用了 [`src` 文件夹](/docs/14/app/building-your-application/configuring/src-directory)，则放置在该文件夹内。

## 配置选项

Instrumentation 目前是一项实验性功能。要使用 `instrumentation` 文件，您必须在 `next.config.js` 中显式启用 [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook)：
