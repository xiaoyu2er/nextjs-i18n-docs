...
Next.js 为具有 [自定义 Babel 配置](/docs/14/pages/building-your-application/configuring/babel) 的应用程序提供了完整的向后兼容性。所有 Next.js 默认处理的转换，如 styled-jsx 和对 `getStaticProps` / `getStaticPaths` / `getServerSideProps` 的树摇优化 (tree-shaking)，均已移植至 Rust。
...
更多信息请参阅 [文档](/docs/14/pages/building-your-application/optimizing/images#styling)。
...
如果您当前将 `target` 选项设置为 `serverless`，请阅读 [关于如何利用新输出的文档](/docs/14/pages/api-reference/next-config-js/output)。