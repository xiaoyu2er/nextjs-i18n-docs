...
Next.js 默认使用服务端组件，无需额外配置即可自动实现服务端渲染。您也可以根据需要选择使用[客户端组件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components)。
...
通过静态渲染，路由会在**构建时**或在[数据重新验证](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data)后在后台渲染。结果会被缓存并可推送至[内容分发网络 (CDN)](https://developer.mozilla.org/docs/Glossary/CDN)，使得渲染结果可在用户和服务器请求间共享。
...
>
> 了解更多关于[全路由缓存](/docs/14/app/building-your-application/caching#full-route-cache)和[数据缓存](/docs/14/app/building-your-application/caching#data-cache)的信息。

#### 切换至动态渲染

在渲染过程中，如果检测到[动态函数](#dynamic-functions)或[非缓存数据请求](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#opting-out-of-data-caching)，Next.js 会切换为动态渲染整个路由。下表总结了动态函数和数据缓存如何影响路由渲染方式：
...
开发者无需手动选择静态或动态渲染，Next.js 会根据使用的功能和 API 自动为每个路由选择最佳策略。您只需决定何时[缓存或重新验证特定数据](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)，并可以选择[流式传输](#streaming)部分 UI。

#### 动态函数

动态函数依赖只能在请求时获取的信息，如用户 cookies、当前请求头或 URL 搜索参数。在 Next.js 中，这些动态函数包括：

- **[`cookies()`](/docs/14/app/api-reference/functions/cookies) 和 [`headers()`](/docs/14/app/api-reference/functions/headers)**：在服务端组件中使用会使整个路由在请求时转为动态渲染。
- **[`searchParams`](/docs/14/app/api-reference/file-conventions/page#searchparams-optional)**：在 [Page](/docs/14/app/api-reference/file-conventions/page) 组件上使用 `searchParams` 属性会使页面在请求时转为动态渲染。
...
您可以通过 `loading.js` 和 [React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) 组件实现路由段的流式传输。详见[加载状态与流式传输](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)章节。