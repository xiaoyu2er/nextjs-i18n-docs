...
需要注意的是，插槽**不是**[路由段 (route segments)](/docs/14/app/building-your-application/routing#route-segments)，不会影响 URL 结构。例如，对于 `/@analytics/views`，URL 实际会是 `/views`，因为 `@analytics` 只是一个插槽。
...
- [**软导航 (Soft Navigation)**](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation)：在客户端导航时，Next.js 会执行[部分渲染 (partial render)](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)，仅改变插槽内的子页面，同时保持其他插槽的活动子页面不变，即使它们与当前 URL 不匹配。
...
[`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) 和 [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) 都接受 `parallelRoutesKey` 参数，允许您读取插槽内的活动路由段。
...
在 `@analytics` 内创建 [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts) 文件，在两个页面间共享选项卡：
...
并行路由可与[拦截路由 (Intercepting Routes)](/docs/14/app/building-your-application/routing/intercepting-routes) 结合使用来创建模态框。这解决了构建模态框时的常见挑战：
...
然后在 `@auth` 插槽中添加 [`default.js`](/docs/14/app/api-reference/file-conventions/default) 文件并返回 `null`。这确保模态框在不活动时不会渲染。
...
> **须知**：
>
> - 拦截路由的约定（如 `(.)`）取决于您的文件系统结构。详见[拦截路由约定](/docs/14/app/building-your-application/routing/intercepting-routes#convention)。
> - 通过将 `<Modal>` 功能与模态框内容（`<Login>`）分离，可以确保模态框内的任何内容（如[表单](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations#forms)）都是服务端组件 (Server Components)。详见[混合客户端与服务端组件](/docs/14/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props)。
...
详见[加载 UI](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) 和[错误处理](/docs/14/app/building-your-application/routing/error-handling)文档了解更多信息。