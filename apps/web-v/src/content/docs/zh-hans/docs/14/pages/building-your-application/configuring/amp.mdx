...
要为页面启用 AMP 支持并了解不同的 AMP 配置选项，请阅读 [`next/amp` 的 API 文档](/docs/14/pages/building-your-application/configuring/amp)。

## 注意事项

- 目前仅支持 CSS-in-JS。[CSS 模块](/docs/14/pages/building-your-application/styling) 目前不受 AMP 页面支持。您可以[为 Next.js 贡献 CSS 模块支持](https://github.com/vercel/next.js/issues/10549)。
...
在[静态 HTML 导出](/docs/14/pages/building-your-application/deploying/static-exports)时也会进行验证，任何警告/错误将打印到终端。任何 AMP 错误都会导致导出以状态码 `1` 退出，因为导出的内容不是有效的 AMP。
...
当使用[静态 HTML 导出](/docs/14/pages/building-your-application/deploying/static-exports)预渲染页面时，Next.js 会检测页面是否支持 AMP 并据此调整导出行为。
...
当启用 [`trailingSlash`](/docs/14/pages/api-reference/next-config-js/trailingSlash) 时，`pages/about.js` 导出的页面将为：
