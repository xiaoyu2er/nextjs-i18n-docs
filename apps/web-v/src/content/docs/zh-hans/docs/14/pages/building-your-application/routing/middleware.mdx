...
translation-updated-at: 2025-05-21T20:30:44.932Z
...
1. `rewrite` 到生成响应的路由（[页面](/docs/14/app/building-your-application/routing/pages-and-layouts) 或 [路由处理器](/docs/14/app/building-your-application/routing/route-handlers)）
...
1. `rewrite` 到生成响应的路由（[页面](/docs/14/pages/building-your-application/routing/pages-and-layouts) 或 [Edge API路由](/docs/14/pages/building-your-application/routing/api-routes)）
2. 直接返回 `NextResponse`。参见[生成响应](#producing-a-response)
...
> **须知：** 您可以在[路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers#cors)中为单个路由配置 CORS 头信息。
...
中间件目前仅支持 [Edge 运行时](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)，无法使用 Node.js 运行时。
