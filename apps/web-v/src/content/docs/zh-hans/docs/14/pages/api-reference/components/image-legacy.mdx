...
translation-updated-at: 2025-05-21T20:31:19.955Z
title: <Image>（旧版）
...
查看**新版** [`next/image` API 参考文档](/docs/14/pages/api-reference/components/image)
...
- [静态导入](/docs/14/pages/building-your-application/optimizing/images#local-images)的图片文件
...
`width` 属性可以表示像素级的_渲染_宽度或_原始_宽度，具体取决于 [`layout`](#layout) 和 [`sizes`](#sizes) 属性。

当使用 `layout="intrinsic"` 或 `layout="fixed"` 时，`width` 属性表示像素级的_渲染_宽度，因此会影响图片显示的大小。

当使用 `layout="responsive"` 或 `layout="fill"` 时，`width` 属性表示像素级的_原始_宽度，因此仅影响宽高比。

`width` 属性是必填的，除非是[静态导入的图片](/docs/14/pages/building-your-application/optimizing/images#local-images)或使用 `layout="fill"` 的情况。

### height

`height` 属性可以表示像素级的_渲染_高度或_原始_高度，具体取决于 [`layout`](#layout) 和 [`sizes`](#sizes) 属性。

当使用 `layout="intrinsic"` 或 `layout="fixed"` 时，`height` 属性表示像素级的_渲染_高度，因此会影响图片显示的大小。

当使用 `layout="responsive"` 或 `layout="fill"` 时，`height` 属性表示像素级的_原始_高度，因此仅影响宽高比。

`height` 属性是必填的，除非是[静态导入的图片](/docs/14/pages/building-your-application/optimizing/images#local-images)或使用 `layout="fill"` 的情况。
...
| --------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------- | ----------------------- |
| `intrinsic` (默认值)  | 缩放以适应容器宽度，最大不超过图片原始尺寸               | `1x`, `2x` (基于 [imageSizes](#image-sizes))                                                                | 不适用  | 是                      |
| `fixed`               | 严格按 `width` 和 `height` 尺寸显示                      | `1x`, `2x` (基于 [imageSizes](#image-sizes))                                                                | 不适用  | 是                      |
...
一个字符串，提供有关图片在不同断点下的宽度信息。对于使用 `layout="responsive"` 或 `layout="fill"` 的图片，`sizes` 的值会显著影响性能。对于使用 `layout="intrinsic"` 或 `layout="fixed"` 的图片，该属性会被忽略。
...
优化图片的质量，整数范围从 `1` 到 `100`，`100` 为最佳质量。默认为 `75`。

### priority

当为 `true` 时，图片将被视为高优先级并[预加载](https://web.dev/preload-responsive-images/)。使用 `priority` 的图片会自动禁用懒加载。

你应该对检测为[最大内容绘制 (LCP)](https://nextjs.org/learn/seo/web-performance/lcp) 元素的任何图片使用 `priority` 属性。根据不同的视口大小，可能有多个优先级图片是合适的。

仅当图片在首屏可见时使用。默认为 `false`。

### placeholder

图片加载时使用的占位符。可能值为 `blur` 或 `empty`。默认为 `empty`。

当为 `blur` 时，将使用 [`blurDataURL`](#blurdataurl) 属性作为占位符。如果 `src` 是[静态导入](/docs/14/pages/building-your-application/optimizing/images#local-images)的对象且导入的图片是 `.jpg`、`.png`、`.webp` 或 `.avif` 格式，则 `blurDataURL` 会自动填充。

对于动态图片，你必须提供 [`blurDataURL`](#blurdataurl) 属性。可以使用 [Plaiceholder](https://github.com/joe-bell/plaiceholder) 等工具帮助生成 `base64`。
...
- [演示使用 `blurDataURL` 属性的微光效果](https://image-legacy-component.nextjs.gallery/shimmer)
- [演示使用 `blurDataURL` 属性的颜色效果](https://image-legacy-component.nextjs.gallery/color)
...
- [演示默认 `blurDataURL` 属性](https://image-legacy-component.nextjs.gallery/placeholder)
- [演示使用 `blurDataURL` 属性的微光效果](https://image-legacy-component.nextjs.gallery/shimmer)
- [演示使用 `blurDataURL` 属性的颜色效果](https://image-legacy-component.nextjs.gallery/color)
...
一个字符串（语法类似于 margin 属性），用作检测视口与图片交集的边界框，以触发懒[加载](#loading)。默认为 `"200px"`。

如果图片嵌套在根文档之外的可滚动父元素中，你还需要指定 [lazyRoot](#lazyroot) 属性。
...
> **须知**：上述示例将确保 `next/legacy/image` 的 `src` 属性必须以 `https://example.com/account123/` 开头，并且不能包含查询字符串。任何其他协议、主机名、端口或不匹配的路径都将返回 400 Bad Request。
...
> **须知**：上述示例将确保 `next/legacy/image` 的 `src` 属性必须以 `https://img1.example.com`、`https://me.avatar.example.com` 或任意数量的子域名开头。不能包含端口或查询字符串。任何其他协议或不匹配的主机名都将返回 400 Bad Request。
...
> **须知**：如果省略 `protocol`、`port`、`pathname` 或 `search`，则默认为通配符 `**`。这不推荐，因为它可能允许恶意用户优化您未预期的 URL。
...
> **须知**：上述示例将确保 `next/legacy/image` 的 `src` 属性必须以 `https://assets.example.com` 开头，并且必须包含精确的查询字符串 `?v=1727111025337`。任何其他协议或查询字符串都将返回 400 Bad Request。

### 域名 (Domains)

> **警告**：自 Next.js 14 起已弃用，建议使用严格的 [`remotePatterns`](#remote-patterns) 以保护您的应用免受恶意用户攻击。仅当您拥有该域名下的所有内容时才使用 `domains`。
...
如果您希望使用云提供商优化图像，而不是使用 Next.js 内置的图像优化 API，可以在 `next.config.js` 文件中配置 `loader` 和 `path` 前缀。这样您可以为图像的 [`src`](#src) 使用相对 URL，并自动为您的提供商生成正确的绝对 URL。
...
- [Vercel](https://vercel.com)：部署在 Vercel 时自动生效，无需配置。[了解更多](https://vercel.com/docs/concepts/image-optimization?utm_source=next-site&utm_medium=docs&utm_campaign=next-website)
- [Imgix](https://www.imgix.com)：`loader: 'imgix'`
- [Cloudinary](https://cloudinary.com)：`loader: 'cloudinary'`
- [Akamai](https://www.akamai.com)：`loader: 'akamai'`
- 自定义：`loader: 'custom'`，通过为 `next/legacy/image` 组件实现 [`loader`](#loader) 属性使用自定义云提供商

如果您需要其他提供商，可以在 `next/legacy/image` 上使用 [`loader`](#loader) 属性。

> 使用 [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) 时，图像无法在构建时优化，只能按需优化。要将 `next/legacy/image` 与 `output: 'export'` 一起使用，您需要使用非默认的加载器。[在讨论中了解更多。](https://github.com/vercel/next.js/discussions/19065)

> `next/legacy/image` 组件的默认加载器使用 [`squoosh`](https://www.npmjs.com/package/@squoosh/lib)，因为它安装快速且适合开发环境。在生产环境中使用 `next start` 时，强烈建议通过运行 `npm i sharp` 在项目目录中安装 [`sharp`](https://www.npmjs.com/package/sharp)。对于 Vercel 部署，无需手动安装，因为 `sharp` 会自动安装。
...
如果您知道用户的预期设备宽度，可以在 `next.config.js` 中使用 `deviceSizes` 属性指定设备宽度断点列表。当 `next/legacy/image` 组件使用 `layout="responsive"` 或 `layout="fill"` 时，这些宽度用于确保为用户设备提供正确的图像。
...
您可以在 `next.config.js` 文件中使用 `images.imageSizes` 属性指定图像宽度列表。这些宽度与 [设备尺寸 (Device Sizes)](#device-sizes) 数组连接，形成用于生成图像 [srcset](https://developer.mozilla.org/docs/Web/API/HTMLImageElement/srcset) 的完整尺寸数组。
...
默认的 [图像优化 API (Image Optimization API)](#loader-configuration) 会通过请求的 `Accept` 标头自动检测浏览器支持的图像格式。

如果 `Accept` 标头匹配多个配置的格式，则使用数组中的第一个匹配项。因此，数组顺序很重要。如果没有匹配项（或源图像是 [动画图像 (Animated Images)](#animated-images)），图像优化 API 将回退到原始图像的格式。
...
以下描述了默认 [加载器 (loader)](#loader) 的缓存算法。对于所有其他加载器，请参阅您的云提供商的文档。
...
过期时间（或更准确地说，最大寿命）由 [`minimumCacheTTL`](#minimum-cache-ttl) 配置或上游图像的 `Cache-Control` 标头中的较大者定义。具体来说，使用 `Cache-Control` 标头的 `max-age` 值。如果同时找到 `s-maxage` 和 `max-age`，则优先使用 `s-maxage`。`max-age` 也会传递给任何下游客户端，包括 CDN 和浏览器。
...
您可以配置缓存优化图像的生存时间 (TTL)（以秒为单位）。在许多情况下，最好使用 [静态图像导入 (Static Image Import)](/docs/14/pages/building-your-application/optimizing/images#local-images)，它会自动哈希文件内容，并使用 `Cache-Control` 标头 `immutable` 永久缓存图像。
...
如果需要按图像更改缓存行为，可以配置 [`headers`](/docs/14/pages/api-reference/next-config-js/headers) 在上游图像（例如 `/some-asset.jpg`，而不是 `/_next/image` 本身）上设置 `Cache-Control` 标头。

目前没有机制可以手动使缓存失效，因此最好将 `minimumCacheTTL` 设置为较低的值。否则，您可能需要手动更改 [`src`](#src) 属性或删除 `<distDir>/cache/images`。
...
默认的 [加载器 (loader)](#loader) 出于几个原因不优化 SVG 图像。首先，SVG 是一种矢量格式，可以无损调整大小。其次，SVG 具有许多与 HTML/CSS 相同的功能，如果没有适当的 [内容安全策略 (CSP) 标头](/docs/14/app/api-reference/next-config-js/headers#content-security-policy)，可能会导致漏洞。

因此，当 [`src`](#src) 属性已知为 SVG 时，我们建议使用 [`unoptimized`](#unoptimized) 属性。当 `src` 以 `".svg"` 结尾时，会自动启用此属性。
...
对动画文件的自动检测是尽力而为的，支持 GIF、APNG 和 WebP。如果要显式绕过特定动画图像的优化，请使用 [unoptimized](#unoptimized) 属性。
