...
> **须知**：
>
> - 即使采用[以服务端为中心的路由](/docs/14/app/building-your-application/routing/linking-and-navigating#how-routing-and-navigation-works)，导航也是即时的。
...
除了 `loading.js`，您还可以为自己的界面组件手动创建 Suspense 边界。App 路由器支持在 [Node.js 和 Edge 运行时](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)中使用 [Suspense](https://react.dev/reference/react/Suspense) 进行流式传输。
...
- Next.js 会先等待 [`generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata) 内的数据获取完成，再向客户端流式传输界面。这保证流式响应的第一部分包含 `<head>` 标签。
...
服务端仍可通过流式内容本身向客户端传达错误或问题，例如使用 [`redirect`](/docs/14/app/api-reference/functions/redirect) 或 [`notFound`](/docs/14/app/api-reference/functions/not-found) 时。由于响应头已发送给客户端，响应状态码无法更新。这不会影响 SEO。