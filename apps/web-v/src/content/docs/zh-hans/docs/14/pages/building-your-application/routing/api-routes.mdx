...
> **须知**：如果您使用应用路由 (App Router)，可以使用[服务端组件 (Server Components)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) 或[路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) 替代 API 路由。
...
> - API 路由不能与[静态导出 (static exports)](/docs/14/pages/building-your-application/deploying/static-exports) 一起使用。但应用路由中的[路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) 可以。
> - API 路由会受到 `next.config.js` 中的 [`pageExtensions` 配置](/docs/14/pages/api-reference/next-config-js/pageExtensions) 影响。
...
- `res.revalidate(urlPath)` - [按需重新验证页面](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) 使用 `getStaticProps`。`urlPath` 必须是 `string`。
...
API 路由支持[动态路由](/docs/14/pages/building-your-application/routing/dynamic-routes)，并遵循与 `pages/` 相同的文件命名规则。
...
如果您希望在 Edge Runtime 中使用 API 路由，我们建议逐步采用应用路由，并使用[路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) 替代。
