...
使用 [`next/image`](/docs/14/app/api-reference/components/image) 组件时，您需要在 `src` 前添加 `basePath`。

</AppOnly>

<PagesOnly>

使用 [`next/image`](/docs/14/pages/api-reference/components/image) 组件时，您需要在 `src` 前添加 `basePath`。
