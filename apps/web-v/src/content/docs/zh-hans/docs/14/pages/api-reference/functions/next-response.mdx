...
[URL](https://developer.mozilla.org/docs/Web/API/URL) 可在用于 `NextResponse.redirect()` 方法前被创建和修改。例如，您可以使用 `request.nextUrl` 属性获取当前 URL 并修改它以重定向到其他 URL。
...
// 向 /login URL 添加 ?from=/incoming-url
