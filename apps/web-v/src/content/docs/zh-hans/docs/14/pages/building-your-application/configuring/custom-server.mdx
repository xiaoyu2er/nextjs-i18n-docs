...
> **须知**:
>
> - 在决定使用自定义服务器前，请注意仅当 Next.js 内置路由无法满足应用需求时才应使用。自定义服务器将移除重要性能优化，如 **无服务器函数 (serverless functions)** 和 **[自动静态优化 (Automatic Static Optimization)](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)**。
...
| 选项           | 类型               | 描述                                                                                                   |
| -------------- | ------------------ | ----------------------------------------------------------------------------------------------------- |
| `conf`         | `Object`           | 与 [next.config.js](/docs/14/pages/api-reference/next-config-js) 相同的配置对象。默认为 `{}`              |
...
> 您可能还需要配置客户端路由以禁止重定向到文件名路由，具体可参考 [`router.beforePopState`](/docs/14/pages/api-reference/functions/use-router#routerbeforepopstate)。