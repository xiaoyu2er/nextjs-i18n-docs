...
Next.js 中的 Turbopack 对大多数用户来说无需配置，并可扩展以满足更高级的用例。要了解 Turbopack 当前支持的功能，请查看 [API 参考文档](/docs/14/app/api-reference/next-config-js/turbo)。
...
- `next.config.js` 中的 [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) 配置
  - Turbopack 替代了 Webpack，这意味着不支持 Webpack 配置。
  - 要配置 Turbopack，[请参阅文档](/docs/14/app/api-reference/next-config-js/turbo)。
  - Turbopack 支持部分 [Webpack 加载器](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders)。
- Babel (`.babelrc`)
  ...
  - 如果确认您的特定用例未被覆盖后仍需使用 Babel，可以利用 Turbopack 的 [自定义 Webpack 加载器支持](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders) 来包含 `babel-loader`。
- 在 App Router 中自动创建根布局。
  - 目前不支持此行为，因为它会更改输入文件，相反，会显示错误提示您手动在所需位置添加根布局。
- `@next/font` (旧版字体支持)。
  - `@next/font` 已弃用，推荐使用 `next/font`。[`next/font`](/docs/14/app/building-your-application/optimizing/fonts) 在 Turbopack 中完全支持。
