...
translation-updated-at: 2025-05-21T20:31:38.474Z
title: 静态站点生成 (SSG)
...
要了解更多关于 `getStaticProps` 的工作原理，请查阅 [数据获取文档](/docs/14/pages/building-your-application/data-fetching/get-static-props)。

#### 场景 2：页面路径依赖于外部数据

Next.js 允许你创建具有 **动态路由** 的页面。例如，你可以创建一个名为 `pages/posts/[id].js` 的文件来显示基于 `id` 的单篇博客文章。这将允许你在访问 `posts/1` 时显示 `id: 1` 的博客文章。
...
因此，你预渲染的页面 **路径** 依赖于外部数据。为了处理这种情况，Next.js 允许你从动态页面（本例中为 `pages/posts/[id].js`）中 `export` 一个名为 `getStaticPaths` 的 `async` 函数。此函数在构建时被调用，并允许你指定要预渲染的路径。
...
要了解更多关于 `getStaticPaths` 的工作原理，请查阅 [数据获取文档](/docs/14/pages/building-your-application/data-fetching/get-static-paths)。

### 何时应该使用静态生成？

我们建议尽可能使用 **静态生成**（带或不带数据），因为你的页面可以构建一次并通过 CDN 提供服务，这比让服务器在每次请求时渲染页面要快得多。
...
- 使用 **客户端数据获取** 的静态生成：你可以跳过预渲染页面的某些部分，然后使用客户端 JavaScript 填充它们。要了解更多关于这种方法的信息，请查阅 [数据获取文档](/docs/14/pages/building-your-application/data-fetching/client-side)。
- 使用 **服务端渲染**：Next.js 在每次请求时预渲染页面。这会比较慢，因为页面无法被 CDN 缓存，但预渲染的页面将始终是最新的。我们将在下面讨论这种方法。