...
> 如果不希望 ESLint 在 `next build` 期间运行，请参阅 [忽略 ESLint](/docs/14/app/api-reference/next-config-js/eslint) 文档。

</AppOnly>

<PagesOnly>

> 如果不希望 ESLint 在 `next build` 期间运行，请参阅 [忽略 ESLint](/docs/14/pages/api-reference/next-config-js/eslint) 文档。
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | 防止 [Next.js 数据获取函数](/docs/14/pages/building-your-application/data-fetching) 中的常见拼写错误 |
...
为了提高性能，ESLint 处理的文件信息默认会被缓存。这些信息存储在 `.next/cache` 或您定义的 [构建目录](/docs/14/app/api-reference/next-config-js/distDir) 中。如果包含任何依赖于多个源文件内容而不仅仅是单个源文件内容的 ESLint 规则，并需要禁用缓存，请使用 `next lint` 的 `--no-cache` 标志。

</AppOnly>

<PagesOnly>

为了提高性能，ESLint 处理的文件信息默认会被缓存。这些信息存储在 `.next/cache` 或您定义的 [构建目录](/docs/14/pages/api-reference/next-config-js/distDir) 中。如果包含任何依赖于多个源文件内容而不仅仅是单个源文件内容的 ESLint 规则，并需要禁用缓存，请使用 `next lint` 的 `--no-cache` 标志。
...
> 对于使用 [Create Next App](/docs/14/app/api-reference/cli/create-next-app) 构建的新应用，会自动包含 `next/core-web-vitals` 入口点。
...
- 已安装以下任一插件（单独安装或通过其他配置如 `airbnb` 或 `react-app`）：
  ...
  - `jsx-a11y`
  - `import`
- 自定义了与 Next.js 内部 Babel 配置不同的 `parserOptions`（除非您已 [自定义 Babel 配置](/docs/14/pages/building-your-application/configuring/babel)，否则不建议这样做）
