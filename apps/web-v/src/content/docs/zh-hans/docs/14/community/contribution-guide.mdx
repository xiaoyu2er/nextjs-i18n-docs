...
例如，在 [函数 API 参考](/docs/14/app/api-reference/functions) 中，页面按字母顺序排序便于开发者查找特定函数：
...
而在 [路由部分](/docs/14/app/building-your-application/routing)，文件使用两位数编号前缀，按开发者学习概念的顺序排列：
...
- 链接会以卡片形式显示在页面主要内容下方
- 对于有子页面的页面会自动生成链接。例如 [优化](/docs/14/app/building-your-application/optimizing) 部分会显示所有子页面的链接
