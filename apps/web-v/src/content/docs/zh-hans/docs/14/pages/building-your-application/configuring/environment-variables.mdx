...
这会自动将 `process.env.DB_HOST`、`process.env.DB_USER` 和 `process.env.DB_PASS` 加载到 Node.js 环境中，您可以在 [Next.js 数据获取方法](/docs/14/pages/building-your-application/data-fetching) 和 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 中使用它们。
...
> 这会自动将 `process.env.DB_HOST`、`process.env.DB_USER` 和 `process.env.DB_PASS` 加载到 Node.js 环境中，您可以在 [路由处理器](/docs/14/app/building-your-application/routing/route-handlers) 中使用它们。
...
要读取运行时环境变量，我们建议使用 `getServerSideProps` 或 [逐步采用应用路由](/docs/14/app/building-your-application/upgrading/app-router-migration)。通过应用路由，我们可以在动态渲染期间安全地在服务器上读取环境变量。这允许您使用一个单一的 Docker 镜像，可以在多个环境中推广使用不同的值。
...
- 您可以使用 [`register` 函数](/docs/14/app/building-your-application/optimizing/instrumentation) 在服务器启动时运行代码。
- 我们不建议使用 [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration) 选项，因为它不适用于独立输出模式。相反，我们建议 [逐步采用](/docs/14/app/building-your-application/upgrading/app-router-migration) 应用路由。
...
- 如果使用 [`/src` 目录](/docs/14/app/building-your-application/configuring/src-directory)，`.env.*` 文件应保留在项目的根目录中。
