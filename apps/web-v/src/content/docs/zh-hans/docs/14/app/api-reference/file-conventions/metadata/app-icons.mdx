...
生成图标的最简单方法是使用 `next/og` 中的 [`ImageResponse`](/docs/14/app/api-reference/functions/image-response) API。
...
> **须知**
>
> - 默认情况下，生成的图标是[**静态优化**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)的（在构建时生成并缓存），除非它们使用[动态函数](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)或未缓存的数据。
> - 您可以使用 [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata) 在同一个文件中生成多个图标。
...
一个包含从根段到 `icon` 或 `apple-icon` 所在段的[动态路由参数](/docs/14/app/building-your-application/routing/dynamic-routes)的对象。
...
`icon` 和 `apple-icon` 是专用的[路由处理器](/docs/14/app/building-your-application/routing/route-handlers)，可以使用与页面和布局相同的[路由段配置](/docs/14/app/api-reference/file-conventions/route-segment-config)选项。
