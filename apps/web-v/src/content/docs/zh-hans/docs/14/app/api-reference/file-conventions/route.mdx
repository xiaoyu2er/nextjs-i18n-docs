...
`request` 对象是一个 [NextRequest](/docs/14/app/api-reference/functions/next-request) 对象，它扩展了 Web [Request](https://developer.mozilla.org/docs/Web/API/Request) API。`NextRequest` 让您能进一步控制传入请求，包括轻松访问 `cookies` 和扩展的解析 URL 对象 `nextUrl`。
...
目前，`context` 的唯一值是 `params`，它是一个包含当前路由的 [动态路由参数](/docs/14/app/building-your-application/routing/dynamic-routes) 的对象。
...
路由处理器可以通过返回 `NextResponse` 对象来扩展 Web Response API。这使您可以轻松设置 cookies、headers、重定向和重写。[查看 API 参考](/docs/14/app/api-reference/functions/next-response)。
