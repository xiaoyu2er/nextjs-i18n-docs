...
translation-updated-at: 2025-05-21T20:29:10.871Z
title: 错误处理
...
Next.js 默认提供静态 500 页面来处理应用中发生的服务端错误。你也可以通过创建 `pages/500.js` 文件来[自定义这个页面](/docs/14/pages/building-your-application/routing/custom-error#customizing-the-500-page)。

应用中的 500 页面不会向用户显示具体错误信息。

你还可以使用 [404 页面](/docs/14/pages/building-your-application/routing/custom-error#404-page) 来处理特定的运行时错误，例如"文件未找到"。

## 客户端错误处理

React 的[错误边界 (Error Boundaries)](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary) 可以优雅地处理客户端 JavaScript 错误，确保应用的其他部分继续运行。除了防止页面崩溃外，它还允许你提供自定义的回退组件，甚至记录错误信息。
...
`ErrorBoundary` 组件通过 `hasError` 状态变量来追踪错误。当 `hasError` 为 `true` 时，组件会渲染回退 UI；否则渲染子组件。
