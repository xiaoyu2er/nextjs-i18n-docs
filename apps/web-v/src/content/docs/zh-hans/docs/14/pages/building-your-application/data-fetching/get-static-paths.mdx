...
如果页面使用了[动态路由 (Dynamic Routes)](/docs/14/pages/building-your-application/routing/dynamic-routes) 并配合 `getStaticProps`，则需要定义需要静态生成的路径列表。
...
[`getStaticPaths` API 参考文档](/docs/14/pages/api-reference/functions/get-static-paths) 涵盖了可与 `getStaticPaths` 一起使用的所有参数和属性。
...
- **不能** 将 `getStaticPaths` 与 [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 一起使用
- 可以从同时使用 `getStaticProps` 的[动态路由 (Dynamic Route)](/docs/14/pages/building-your-application/routing/dynamic-routes) 中导出 `getStaticPaths`
...
`getStaticPaths` 允许您控制在构建期间生成哪些页面，而不是通过 [`fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking) 按需生成。在构建期间生成更多页面会导致构建速度变慢。
