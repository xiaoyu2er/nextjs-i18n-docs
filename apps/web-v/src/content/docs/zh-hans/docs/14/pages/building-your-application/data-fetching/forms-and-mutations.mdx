...
> **须知：**
>
> - 我们即将推荐 [渐进式采用](/docs/14/app/building-your-application/upgrading/app-router-migration) 应用路由 (App Router) 并使用 [服务端操作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) 来处理表单提交和数据变更。服务端操作允许您定义异步服务端函数，这些函数可以直接从组件中调用，无需手动创建 API 路由。
> - API 路由 [默认不指定 CORS 头信息](https://developer.mozilla.org/docs/Web/HTTP/CORS)，这意味着它们默认仅支持同源请求。
> - 由于 API 路由在服务端运行，我们可以通过 [环境变量 (Environment Variables)](/docs/14/pages/building-your-application/configuring/environment-variables) 使用敏感值（如 API 密钥）而不会暴露给客户端。这对应用程序的安全性至关重要。
...
如果您想在数据变更后将用户重定向到其他路由，可以使用 [`redirect`](/docs/14/pages/building-your-application/routing/api-routes#response-helpers) 跳转到任意绝对或相对 URL：
...
您可以在 API 路由中使用 [`cookies`](/docs/14/pages/building-your-application/routing/api-routes#request-helpers) 请求辅助方法来读取 Cookie：
