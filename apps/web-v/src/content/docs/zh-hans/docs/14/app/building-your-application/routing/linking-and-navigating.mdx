...
- 使用 [`useRouter` 钩子](#userouter-hook) ([客户端组件](/docs/14/app/building-your-application/rendering/client-components))
- 使用 [`redirect` 函数](#redirect-function) ([服务端组件](/docs/14/app/building-your-application/rendering/server-components))
...
`<Link>` 还支持其他可选属性，详见 [API 参考文档](/docs/14/app/api-reference/components/link)。

### 使用示例

#### 链接到动态路由段

当链接到[动态路由段](/docs/14/app/building-your-application/routing/dynamic-routes)时，可以使用[模板字符串和插值](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals)生成链接列表。例如生成博客文章列表：
...
可以使用 [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname) 判断链接是否处于活动状态。例如，为活动链接添加特定类名：
...
> **须知**：
>
> - 如果目标页面在视窗中不可见，Next.js 会在导航时自动滚动到[页面](/docs/14/app/building-your-application/routing/pages-and-layouts#pages)顶部。
...
`useRouter` 钩子允许你在[客户端组件](/docs/14/app/building-your-application/rendering/client-components)中以编程方式更改路由。
...
完整的 `useRouter` 方法列表，请参阅 [API 参考文档](/docs/14/app/api-reference/functions/use-router)。

> **推荐**：除非有特殊需求，否则优先使用 `<Link>` 组件进行路由导航。

## `redirect` 函数

对于[服务端组件](/docs/14/app/building-your-application/rendering/server-components)，请使用 `redirect` 函数。
...
> - 如需在渲染前重定向，可使用 [`next.config.js`](/docs/14/app/building-your-application/routing/redirecting#redirects-in-nextconfigjs) 或 [中间件](/docs/14/app/building-your-application/routing/redirecting#nextresponseredirect-in-middleware)。

更多信息请参阅 [`redirect` API 参考文档](/docs/14/app/api-reference/functions/redirect)。
...
`pushState` 和 `replaceState` 调用会与 Next.js 路由器集成，可与 [`usePathname`](/docs/14/app/api-reference/functions/use-pathname) 和 [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params) 同步使用。
...
[服务端组件](/docs/14/app/building-your-application/rendering/server-components)使得应用代码能按路由段自动分割。这意味着导航时只会加载当前路由所需的代码。
...
`<Link>` 的默认预加载行为（即不指定 `prefetch` 属性或设为 `null` 时）会根据 [`loading.js`](/docs/14/app/api-reference/file-conventions/loading) 的使用情况有所不同。只会预加载共享布局，直到第一个 `loading.js` 文件为止的组件树，并缓存 30 秒。这降低了获取整个动态路由的成本，同时可以显示[即时加载状态](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states)以提供更好的视觉反馈。

通过将 `prefetch` 属性设为 `false` 可禁用预加载。或者，通过设为 `true` 可以预加载超出加载边界的完整页面数据。

更多信息请参阅 [`<Link>` API 参考文档](/docs/14/app/api-reference/components/link)。
...
Next.js 有一个称为[路由缓存](/docs/14/app/building-your-application/caching#router-cache)的**内存客户端缓存**。当用户在应用中导航时，[预加载](#2-prefetching)的路由段和已访问路由的 React 服务端组件负载会被存储在缓存中。

这意味着导航时会尽可能复用缓存，而不是向服务器发起新请求——通过减少请求次数和数据传输量来提升性能。

详细了解[路由缓存](/docs/14/app/building-your-application/caching#router-cache)的工作原理及配置方法。
...
默认情况下，Next.js 会在前进/后退导航时保持滚动位置，并复用[路由缓存](/docs/14/app/building-your-application/caching#router-cache)中的路由段。
