...
配置请求如何与 Next.js [数据缓存 (Data Cache)](/docs/14/app/building-your-application/caching#data-cache) 交互。
...
> **须知**:
>
> - 如果不提供 `cache` 选项，Next.js 会默认使用 `force-cache`，除非使用了 [动态函数 (dynamic function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) 如 `cookies()`，此时会默认使用 `no-store`。
...
> **须知**:
>
> - 如果单个 `fetch()` 请求设置的 `revalidate` 数值小于路由的 [默认 `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate)，整个路由的重新验证间隔会被缩短。
