...
>
> 了解更多，请参阅[服务端组件](/docs/14/app/building-your-application/rendering/server-components)文档。
...
了解更多关于[静态与动态渲染](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies)的内容。
...
- **[重新验证数据 (Revalidating Data)](/docs/14/app/building-your-application/caching#revalidating)**：重新验证[数据缓存 (Data Cache)](#data-cache) 会通过重新渲染服务器上的组件并缓存新的渲染输出，从而使路由缓存失效。
...
当用户在路由间导航时，Next.js 会缓存已访问的路由段，并根据视口中的 `<Link>` 组件[预取 (prefetch)](/docs/14/app/building-your-application/routing/linking-and-navigating#2-prefetching) 用户可能导航到的路由。

这会为用户带来更好的导航体验：

- 即时前进/后退导航，因为已访问的路由被缓存；快速导航到新路由，因为预取和[部分渲染 (partial rendering)](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)。
...
- **自动失效周期 (Automatic Invalidation Period)**：布局和加载状态的缓存会在特定时间后自动失效。持续时间取决于资源的[预取方式](/docs/14/app/api-reference/components/link#prefetch)以及资源是否[静态生成](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)：
...
> **须知**：实验性的 [`staleTimes`](/docs/14/app/api-reference/next-config-js/staleTimes) 配置选项可用于调整上述自动失效时间。
...
- 在**服务器操作 (Server Action)** 中：
  - 按路径 ([`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath)) 或缓存标签 ([`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)) 按需重新验证数据
  - 使用 [`cookies.set`](/docs/14/app/api-reference/functions/cookies#cookiessetname-value-options) 或 [`cookies.delete`](/docs/14/app/api-reference/functions/cookies#deleting-cookies) 会使路由缓存失效，以防止使用 cookie 的路由（如身份验证）过期。
- 调用 [`router.refresh`](/docs/14/app/api-reference/functions/use-router) 会使路由缓存失效，并为当前路由向服务器发起新请求。

### 退出缓存

无法完全退出路由缓存，但可以通过调用 [`router.refresh`](/docs/14/app/api-reference/functions/use-router)、[`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) 或 [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)（见上文）使其失效。这将清除缓存并向服务器发起新请求，确保显示最新数据。
...
- 在[路由处理器 (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers) 中重新验证数据缓存**不会**立即使路由缓存失效，因为路由处理器不与特定路由绑定。这意味着路由缓存会继续提供先前的负载，直到强制刷新或自动失效周期结束。
- 要立即使数据缓存和路由缓存失效，可以在[服务器操作 (Server Action)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) 中使用 [`revalidatePath`](#revalidatepath) 或 [`revalidateTag`](#fetch-optionsnexttags-and-revalidatetag)。
...
了解更多关于 [`<Link>` 组件](/docs/14/app/api-reference/components/link)的内容。

### `router.prefetch`

`useRouter` 钩子的 `prefetch` 选项可用于手动预取路由。这会将 React 服务器组件负载添加到路由缓存中。

参见 [`useRouter` 钩子](/docs/14/app/api-reference/functions/use-router)的 API 参考。
...
更多选项参见 [`fetch` API 参考](/docs/14/app/api-reference/functions/fetch)。
...
1. 使用 `fetch` 或 [`unstable_cache`](/docs/14/app/api-reference/functions/unstable_cache) 时，您可以选择为一个或多个标签标记缓存条目。
...
1. [路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) - 用于响应第三方事件（如 Webhook）时重新验证数据。由于路由处理器不与特定路由绑定，这不会立即使路由缓存失效。
2. [服务器操作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - 在用户操作（如表单提交）后重新验证数据。这将使关联路由的路由缓存失效。
...
1. [路由处理器 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers) - 用于响应第三方事件（如 Webhook）时重新验证数据。
2. [服务器操作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations) - 在用户交互（如表单提交、点击按钮）后重新验证数据。

更多信息请参阅 [`revalidatePath` API 参考文档](/docs/14/app/api-reference/functions/revalidatePath)。
...
更多信息请参阅 [`cookies` API 参考文档](/docs/14/app/api-reference/functions/cookies)。
...
更多选项请参阅 [路由段配置文档](/docs/14/app/api-reference/file-conventions/route-segment-config)。

### `generateStaticParams`

对于[动态路由段 (dynamic segments)](/docs/14/app/building-your-application/routing/dynamic-routes)（如 `app/blog/[slug]/page.js`），由 `generateStaticParams` 提供的路径会在构建时缓存到完整路由缓存 (Full Route Cache) 中。在请求时，Next.js 还会在首次访问时缓存构建时未知的路径。

您可以通过在路由段中使用 `export const dynamicParams = false` 选项来禁用请求时的缓存。使用此配置后，仅会提供由 `generateStaticParams` 生成的路径，其他路由将返回 404 或匹配（对于[全捕获路由 (catch-all routes)](/docs/14/app/building-your-application/routing/dynamic-routes#catch-all-segments)）。

更多信息请参阅 [`generateStaticParams` API 参考文档](/docs/14/app/api-reference/functions/generate-static-params)。
