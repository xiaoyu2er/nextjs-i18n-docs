...
- 服务端操作与 Next.js 的[缓存和重新验证](/docs/14/app/building-your-application/caching)架构集成。当操作被调用时，Next.js 可以在单次服务端往返中返回更新的 UI 和新数据。
...
- 服务端操作继承其所在页面或布局的[运行时](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)。
- 服务端操作继承其所在页面或布局的[路由段配置](/docs/14/app/api-reference/file-conventions/route-segment-config)，包括 `maxDuration` 等字段。
...
当抛出错误时，它会被客户端最近的 [`error.js`](/docs/14/app/building-your-application/routing/error-handling) 或 `<Suspense>` 边界捕获。我们推荐使用 `try/catch` 将错误返回给 UI 处理。
...
您可以在服务端操作中使用 [`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) API 重新验证 [Next.js 缓存](/docs/14/app/building-your-application/caching)：
...
或者使用 [`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag) 通过缓存标签使特定数据获取失效：
...
如果希望在服务端操作完成后将用户重定向到不同路由，可以使用 [`redirect`](/docs/14/app/api-reference/functions/redirect) API。`redirect` 需要在 `try/catch` 块外部调用：
...
您可以在服务端操作中使用 [`cookies`](/docs/14/app/api-reference/functions/cookies) API 获取、设置和删除 cookies：
...
查看[更多示例](/docs/14/app/api-reference/functions/cookies#deleting-cookies)了解如何从服务端操作中删除 cookies。
...
> **须知：** 我们不建议仅依赖加密来防止敏感值暴露在客户端。相反，您应该使用 [React taint APIs](/docs/14/app/building-your-application/data-fetching/patterns#preventing-sensitive-data-from-being-exposed-to-the-client) 主动防止特定数据发送到客户端。
...
对于使用反向代理或多层后端架构（服务器 API 与生产域不同）的大型应用，建议使用配置选项 [`serverActions.allowedOrigins`](/docs/14/app/api-reference/next-config-js/serverActions) 指定安全来源列表。该选项接受字符串数组。
