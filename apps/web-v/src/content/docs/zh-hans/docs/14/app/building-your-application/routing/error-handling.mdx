...
`error.js` 文件约定允许您在[嵌套路由](/docs/14/app/building-your-application/routing#nested-routes)中优雅地处理意外的运行时错误。
...
通过[特殊文件](/docs/14/app/building-your-application/routing#file-conventions)创建的 React 组件会在[特定的嵌套层级结构](/docs/14/app/building-your-application/routing#component-hierarchy)中渲染。
