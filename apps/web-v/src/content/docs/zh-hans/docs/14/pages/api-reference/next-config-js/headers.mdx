...
当结合 [`basePath` 支持](/docs/14/app/api-reference/next-config-js/basePath) 使用头部时，除非在头部设置 `basePath: false`，否则每个 `source` 都会自动添加 `basePath` 前缀：
...
当结合 [`i18n` 支持](/docs/14/app/building-your-application/routing/internationalization) 使用头部时，除非在头部设置 `locale: false`，否则每个 `source` 会自动添加语言环境前缀。若使用 `locale: false`，则必须手动在 `source` 前添加语言环境才能正确匹配。

</AppOnly>

<PagesOnly>

当结合 [`i18n` 支持](/docs/14/pages/building-your-application/routing/internationalization) 使用头部时，除非在头部设置 `locale: false`，否则每个 `source` 会自动添加语言环境前缀。若使用 `locale: false`，则必须手动在 `source` 前添加语言环境才能正确匹配。
...
了解应用路由的 [缓存机制](/docs/14/app/building-your-application/caching)。

</AppOnly>

<PagesOnly>

如果您需要重新验证 [静态生成](/docs/14/pages/building-your-application/rendering/static-site-generation) 页面的缓存，可以通过在页面的 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) 函数中设置 `revalidate` 属性来实现。

您可以在 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 中使用 `res.setHeader` 方法设置 `Cache-Control` 头部：
...
**此头部已被 CSP 的 `frame-ancestors` 选项取代**，现代浏览器对其支持更好（配置详情请参阅 [内容安全策略](/docs/14/app/building-your-application/configuring/content-security-policy)）。
...
了解如何为应用添加 [内容安全策略](/docs/14/app/building-your-application/configuring/content-security-policy)。
