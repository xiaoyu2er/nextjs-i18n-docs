...
> **须知**
>
> - 默认情况下，生成的图片会进行[**静态优化**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)（在构建时生成并缓存），除非它们使用了[动态函数](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)或未缓存的数据。
> - 您可以在同一文件中使用 [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata) 生成多个图片。

生成图片最简单的方法是使用 `next/og` 中的 [ImageResponse](/docs/14/app/api-reference/functions/image-response) API。
...
一个包含从根段到 `opengraph-image` 或 `twitter-image` 所在段的[动态路由参数](/docs/14/app/building-your-application/routing/dynamic-routes)的对象。
...
`opengraph-image` 和 `twitter-image` 是专门的[路由处理器](/docs/14/app/building-your-application/routing/route-handlers)，可以使用与页面和布局相同的[路由段配置](/docs/14/app/api-reference/file-conventions/route-segment-config)选项。
...
> **须知**：
> 默认情况下，此生成的图片将进行[静态优化](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)。您可以配置单个 `fetch` 的[选项](/docs/14/app/api-reference/functions/fetch)或路由段的[选项](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate)来更改此行为。
