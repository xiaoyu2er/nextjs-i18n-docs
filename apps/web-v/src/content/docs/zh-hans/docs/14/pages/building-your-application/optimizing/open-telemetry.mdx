...
接着，在项目**根目录**（若使用 `src` 目录则放在其中）创建自定义 [`instrumentation.ts`](/docs/14/app/building-your-application/optimizing/instrumentation)（或 `.js`）文件：

</AppOnly>

<PagesOnly>

接着，在项目**根目录**（若使用 `src` 目录则放在其中）创建自定义 [`instrumentation.ts`](/docs/14/pages/building-your-application/optimizing/instrumentation)（或 `.js`）文件：
...
> - 若使用 [`pageExtensions` 配置选项](/docs/14/app/api-reference/next-config-js/pageExtensions) 添加后缀，需同步更新 `instrumentation` 文件名以匹配
...
>
> - `instrumentation` 文件应位于项目根目录，而非 `app` 或 `pages` 目录内。若使用 `src` 目录，请将文件置于 `src` 内与 `pages` 和 `app` 同级
> - 若使用 [`pageExtensions` 配置选项](/docs/14/pages/api-reference/next-config-js/pageExtensions) 添加后缀，需同步更新 `instrumentation` 文件名以匹配
> - 我们提供了基础示例 [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry) 供您参考
...
OpenTelemetry 收集器并非必需。您可以通过 [`@vercel/otel`](#使用-vercelotel) 或 [手动 OpenTelemetry 配置](/docs/14/pages/building-your-application/optimizing/open-telemetry#手动配置-opentelemetry) 使用自定义导出器。
