...
translation-updated-at: 2025-05-21T20:30:16.307Z
...
- **Edge 运行时**基于 [Web API](/docs/14/app/api-reference/edge) 实现。
...
| ------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------- | ---------------- |
| 冷启动                                                                                                                               | /      | 常规                | 极低             |
| [HTTP 流式传输](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)                                                | 支持   | 支持                | 支持             |
| IO 操作                                                                                                                              | 全部   | 全部                | 仅限 `fetch`     |
| 可扩展性                                                                                                                             | /      | 高                  | 最高             |
| 安全性                                                                                                                               | 常规   | 高                  | 高               |
| 延迟                                                                                                                                 | 常规   | 低                  | 最低             |
| npm 包支持                                                                                                                           | 全部   | 全部                | 有限子集         |
| [静态渲染](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)                                  | 支持   | 支持                | 不支持           |
| [动态渲染](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)                                        | 支持   | 支持                | 支持             |
| [通过 `fetch` 实现数据重新验证](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) | 支持   | 支持                | 支持             |
...
您可以为 Next.js 应用中的单个路由段指定运行时。具体方法是[声明并导出一个名为 `runtime` 的变量](/docs/14/app/api-reference/file-conventions/route-segment-config)。该变量必须是字符串类型，且值只能为 `'nodejs'` 或 `'edge'` 运行时。
...
> 完整 API 列表请参阅 [Node.js 文档](https://nodejs.org/docs/latest/api/) 和 [Edge 文档](/docs/14/app/api-reference/edge)。根据部署基础设施的不同，两种运行时也都支持[流式传输](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)。