...
`middleware.js|ts` 文件用于编写 [中间件 (Middleware)](/docs/14/app/building-your-application/routing/middleware)，并在请求完成前在服务端运行代码。根据传入的请求，您可以通过重写、重定向、修改请求或响应头，或直接响应来修改响应内容。
...
中间件可以使用 [`NextResponse`](/docs/14/app/building-your-application/routing/middleware#nextresponse) 对象，它扩展了 [Web Response API](https://developer.mozilla.org/en-US/docs/Web/API/Response)。通过返回 `NextResponse` 对象，您可以直接操作 cookies、设置头信息、实现重定向和重写路径。

> **须知**：对于重定向，您也可以使用 `Response.redirect` 替代 `NextResponse.redirect`。

## 运行时 (Runtime)

中间件仅支持 [边缘运行时 (Edge Runtime)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)，不支持 Node.js 运行时。
