...
[中间件](/docs/14/app/building-your-application/routing/middleware) 允许您在页面渲染前添加头部信息并生成 Nonce。
...
默认情况下，中间件会处理所有请求。您可以通过 [`matcher`](/docs/14/app/building-your-application/routing/middleware#matcher) 配置使其仅针对特定路径生效。
...
现在您可以通过 [`headers`](/docs/14/app/api-reference/functions/headers) 在 [服务端组件](/docs/14/app/building-your-application/rendering/server-components) 中读取 Nonce：
...
对于不需要 Nonce 的应用，可以直接在 [`next.config.js`](/docs/14/app/api-reference/next-config-js) 文件中设置 CSP 头：
