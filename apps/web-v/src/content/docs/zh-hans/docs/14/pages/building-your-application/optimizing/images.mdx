...
translation-updated-at: 2025-05-21T20:29:44.787Z
title: 图片优化
nav_title: 图片
...
由于 Next.js 在构建时无法访问远程文件，您需要手动提供 [`width`](/docs/14/app/api-reference/components/image#width)、[`height`](/docs/14/app/api-reference/components/image#height) 和可选的 [`blurDataURL`](/docs/14/app/api-reference/components/image#blurdataurl) 属性。
...
了解更多关于 [`remotePatterns`](/docs/14/app/api-reference/components/image#remotepatterns) 配置。如果想为图片 `src` 使用相对 URL，请使用 [`loader`](/docs/14/app/api-reference/components/image#loader)。
...
> 了解更多关于 [`remotePatterns`](/docs/14/app/api-reference/components/image#remotepatterns) 配置。
...
您可以通过 [`loader` 属性](/docs/14/app/api-reference/components/image#loader)为单个图片定义加载器，或通过 [`loaderFile` 配置](/docs/14/app/api-reference/components/image#loaderfile)在应用级别定义。
...
在 [`next/image` 组件文档](/docs/14/app/api-reference/components/image#priority) 中查看更多关于优先级的信息。
...
2. 显式地包含 [`width`](/docs/14/app/api-reference/components/image#width) 和 [`height`](/docs/14/app/api-reference/components/image#height) 属性
3. 隐式地使用 [fill](/docs/14/app/api-reference/components/image#fill) 使图片扩展以填充其父元素
...
- 使用 `className` 或 `style`，而不是 `styled-jsx`
  - 在大多数情况下，我们推荐使用 `className` 属性。可以是导入的 [CSS 模块](/docs/14/app/building-your-application/styling/css-modules)、[全局样式表](/docs/14/app/building-your-application/styling/css-modules#global-styles) 等
  - 也可以使用 `style` 属性分配内联样式
  - 不能使用 [styled-jsx](/docs/14/app/building-your-application/styling/css-in-js)，因为它的作用域限于当前组件（除非将样式标记为 `global`）
...
[**查看 `next/image` 组件所有可用属性。**](/docs/14/app/api-reference/components/image)

## 配置

`next/image` 组件和 Next.js 图片优化 API 可以在 [`next.config.js` 文件](/docs/14/app/api-reference/next-config-js) 中配置。这些配置允许您 [启用远程图片](/docs/14/app/api-reference/components/image#remotepatterns)、[定义自定义图片断点](/docs/14/app/api-reference/components/image#devicesizes)、[更改缓存行为](/docs/14/app/api-reference/components/image#caching-behavior) 等。

[**阅读完整的图片配置文档了解更多信息。**](/docs/14/app/api-reference/components/image#configuration-options)