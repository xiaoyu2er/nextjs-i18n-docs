...
translation-updated-at: 2025-05-21T20:27:28.185Z
title: 字体模块
nav_title: 字体
description: 字体模块的 API 参考文档
...
本 API 参考将帮助您理解如何使用 [`next/font/google`](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) 和 [`next/font/local`](/docs/14/app/building-your-application/optimizing/fonts#local-fonts)。关于功能特性及用法，请参阅 [优化字体](/docs/14/app/building-your-application/optimizing/fonts) 页面。

### 字体函数参数

具体用法请查阅 [Google 字体](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) 和 [本地字体](/docs/14/app/building-your-application/optimizing/fonts#local-fonts)。

| 参数                                         | `font/google`       | `font/local`        | 类型                       | 是否必填          |
| ------------------------------------------- | ------------------- | ------------------- | -------------------------- | ----------------- |
| [`src`](#src)                               | <Cross size={18} /> | <Check size={18} /> | 字符串或对象数组           | 是               |
| [`weight`](#weight)                         | <Check size={18} /> | <Check size={18} /> | 字符串或数组               | 必填/可选        |
...
- 如果在 `app/page.tsx` 中调用字体加载器函数并使用 `src:'../styles/fonts/my-font.ttf'`，则 `my-font.ttf` 应位于项目根目录的 `styles/fonts` 下

### `weight`

字体的 [`weight`](https://fonts.google.com/knowledge/glossary/weight)，可选值如下：

- 字符串，值为特定字体可用的权重或 [可变字体](https://fonts.google.com/variablefonts) 的权重范围
- 若非 [可变 Google 字体](https://fonts.google.com/variablefonts)，可使用权重值数组。仅适用于 `next/font/google`
...
- `weight: '400'`：单个权重值的字符串 - 对于 [`Inter`](https://fonts.google.com/specimen/Inter?query=inter) 字体，可选值为 `'100'`、`'200'`、`'300'`、`'400'`、`'500'`、`'600'`、`'700'`、`'800'`、`'900'` 或 `'variable'`（`'variable'` 为默认值）
- `weight: '100 900'`：可变字体在 `100` 到 `900` 之间的权重范围字符串
- `weight: ['100','400','900']`：非可变字体的 3 个可能权重值数组

### `style`

字体的 [`style`](https://developer.mozilla.org/docs/Web/CSS/font-style)，可选值如下：

- 字符串 [值](https://developer.mozilla.org/docs/Web/CSS/font-style#values)，默认值为 `'normal'`
- 若非 [可变 Google 字体](https://fonts.google.com/variablefonts)，可使用样式值数组。仅适用于 `next/font/google`
...
- `style: 'oblique'`：字符串 - 对于 `next/font/local` 可以是任何值，但通常来自 [标准字体样式](https://developer.mozilla.org/docs/Web/CSS/font-style)
- `style: ['italic','normal']`：`next/font/google` 的 2 个值数组 - 可选值为 `normal` 和 `italic`

### `subsets`

字体的 [`subsets`](https://fonts.google.com/knowledge/glossary/subsetting)，由字符串数组定义，包含您希望 [预加载](/docs/14/app/building-your-application/optimizing/fonts#specifying-a-subset) 的每个子集名称。当 [`preload`](#preload) 选项为 `true`（默认值）时，通过 `subsets` 指定的字体将在头部注入链接预加载标签。
...
- `axes: ['slnt']`：值为 `slnt` 的数组，适用于 `Inter` 可变字体，该字体具有额外的 `axes` 如 [此处](https://fonts.google.com/variablefonts?vfquery=inter#font-families) 所示。您可以在 [Google 可变字体页面](https://fonts.google.com/variablefonts#font-families) 上使用筛选器查找字体的其他 `axes` 值（除 `wght` 外）
...
布尔值，指定是否应 [预加载](/docs/14/app/building-your-application/optimizing/fonts#preloading) 字体。默认为 `true`。
...
- 对于 `next/font/google`：布尔值，设置是否使用自动回退字体以减少 [累积布局偏移 (CLS)](https://web.dev/cls/)。默认为 `true`。
- 对于 `next/font/local`：字符串或布尔值 `false`，设置是否使用自动回退字体以减少 [累积布局偏移 (CLS)](https://web.dev/cls/)。可选值为 `'Arial'`、`'Times New Roman'` 或 `false`。默认为 `'Arial'`。
