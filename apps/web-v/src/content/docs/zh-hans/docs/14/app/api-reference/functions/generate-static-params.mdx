...
`generateStaticParams` 函数可与[动态路由段](/docs/14/app/building-your-application/routing/dynamic-routes)结合使用，用于在构建时[**静态生成**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)路由，而非在请求时按需生成。
...
> **须知**
>
> - 您可以使用 [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) 段配置选项来控制当访问未通过 `generateStaticParams` 生成的动态段时会发生什么。
...
> - `generateStaticParams` 替代了 Pages Router 中的 [`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) 函数。
...
> **须知**：在所有以 `generate` 为前缀的函数、布局、页面和服务器组件中，`fetch` 请求会自动[记忆化](/docs/14/app/building-your-application/caching#request-memoization)。如果 `fetch` 不可用，可以使用 React 的 [`cache`](/docs/14/app/building-your-application/caching#request-memoization)。

### 仅生成部分参数

您可以通过返回仅包含要生成的动态段的对象数组来为路由生成部分参数。然后，通过使用 [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) 段配置选项，您可以控制当访问未通过 `generateStaticParams` 生成的动态段时会发生什么。
