...
请参阅 [`next/script`](/docs/14/app/api-reference/components/script#strategy) API 参考文档，了解每种策略及其用例的更多信息。

### 将脚本卸载到 Web Worker（实验性）

> **警告**：`worker` 策略尚未稳定，并且尚不适用于 [`app`](/docs/14/app/building-your-application/routing/defining-routes) 目录。请谨慎使用。
...
这些处理程序仅在 `next/script` 被导入并在 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 中使用时才有效，其中 `"use client"` 被定义为代码的第一行：
...
请参阅 [`next/script`](/docs/14/app/api-reference/components/script#onload) API 参考，了解每个事件处理程序的更多信息并查看示例。
...
请参阅 [`next/script`](/docs/14/pages/api-reference/components/script#onload) API 参考，了解每个事件处理程序的更多信息并查看示例。
