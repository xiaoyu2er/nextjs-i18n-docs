...
> - 如果您是从 `metadata` 导出迁移而来，可以使用 [metadata-to-viewport-export 代码修改工具](/docs/14/app/building-your-application/upgrading/codemods#metadata-to-viewport-export) 来更新您的更改。
...
您可以通过使用 `Viewport` 类型为视口对象添加类型安全。如果您在 IDE 中使用 [内置的 TypeScript 插件](/docs/14/app/building-your-application/configuring/typescript)，则无需手动添加类型，但您仍然可以显式添加。
