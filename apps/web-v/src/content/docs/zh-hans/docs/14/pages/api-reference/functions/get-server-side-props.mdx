...
| 名称             | 描述                                                                                                                                                                                                 |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`         | 如果页面使用了 [动态路由](/docs/14/pages/building-your-application/routing/dynamic-routes)，`params` 包含路由参数。如果页面名称为 `[id].js`，则 `params` 会形如 `{ id: ... }`。                          |
...
| `preview`        | （已弃用，改用 `draftMode`）如果页面处于 [预览模式 (Preview Mode)](/docs/14/pages/building-your-application/configuring/preview-mode)，则 `preview` 为 `true`，否则为 `false`。                         |
| `previewData`    | （已弃用，改用 `draftMode`）由 `setPreviewData` 设置的 [预览数据](/docs/14/pages/building-your-application/configuring/preview-mode)。                                                                 |
| `draftMode`      | 如果页面处于 [草稿模式 (Draft Mode)](/docs/14/pages/building-your-application/configuring/draft-mode)，则 `draftMode` 为 `true`，否则为 `false`。                                                      |
...
`notFound` 布尔值允许页面返回 `404` 状态和 [404 页面](/docs/14/pages/building-your-application/routing/custom-error#404-page)。即使之前成功生成了页面，`notFound: true` 也会使页面返回 `404`。这适用于诸如用户生成内容被作者删除等情况。
...
| 版本       | 变更                                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------------------- |
| `v13.4.0`  | [应用路由 (App Router)](/docs/14/app/building-your-application/data-fetching) 现已稳定，数据获取更简化      |
