...
当您无法提前确定确切的路径段名称，并希望基于动态数据创建路由时，可以使用动态段 (Dynamic Segments)，这些动态段会在请求时填充或在构建时进行[预渲染](/docs/14/pages/building-your-application/data-fetching/get-static-paths)。

## 约定

通过将文件名或文件夹名用方括号包裹即可创建动态段：`[segmentName]`。例如 `[id]` 或 `[slug]`。

可以通过 [`useRouter`](/docs/14/pages/api-reference/functions/use-router) 访问动态段。
