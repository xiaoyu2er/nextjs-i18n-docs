...
[打包分析器](/docs/14/app/building-your-application/optimizing/package-bundling#analyzing-javascript-bundles) 可以帮助你排查应用中可能移除的大型依赖项，从而提升性能和减少内存占用。
...
你可以通过添加 [自定义 Webpack 配置](/docs/14/app/api-reference/next-config-js/webpack) 来禁用此行为：
