...
translation-updated-at: 2025-05-21T19:20:43.242Z
title: 重定向 (redirects)
...
使用页面路由时，除非存在 [中间件](/docs/14/app/building-your-application/routing/middleware) 且匹配路径，否则重定向不会应用于客户端路由（`Link`、`router.push`）。
...
当结合 [`basePath` 支持](/docs/14/app/api-reference/next-config-js/basePath) 使用重定向时，每个 `source` 和 `destination` 会自动添加 `basePath` 前缀，除非您在重定向中设置 `basePath: false`：
...
当结合 [`i18n` 国际化支持](/docs/14/app/building-your-application/routing/internationalization) 使用重定向时，每个 `source` 和 `destination` 会自动添加配置的 `locales` 前缀，除非您设置 `locale: false`。若使用 `locale: false` 则必须在 `source` 和 `destination` 中手动添加语言环境前缀才能正确匹配。

</AppOnly>

<PagesOnly>

当结合 [`i18n` 国际化支持](/docs/14/pages/building-your-application/routing/internationalization) 使用重定向时，每个 `source` 和 `destination` 会自动添加配置的 `locales` 前缀，除非您设置 `locale: false`。若使用 `locale: false` 则必须在 `source` 和 `destination` 中手动添加语言环境前缀才能正确匹配。
...
- 在 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 和 [路由处理器](/docs/14/app/building-your-application/routing/route-handlers) 中，可根据传入请求进行重定向
- 在 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) 和 [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 中，可在请求时重定向特定页面
