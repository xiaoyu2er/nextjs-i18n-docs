...
- 自动模拟样式表 (`.css`, `.module.css` 及其 scss 变体)、图片导入和 [`next/font`](/docs/14/pages/building-your-application/optimizing/fonts)
- 将 `.env` (及其所有变体) 加载到 `process.env`
- 从测试解析和转换中忽略 `node_modules`
- 从测试解析中忽略 `.next`
- 加载 `next.config.js` 以启用 SWC 转换的标记

> **须知**：如需直接测试环境变量，需在单独的安装脚本或 `jest.config.ts` 中手动加载。详见[测试环境变量](/docs/14/pages/building-your-application/configuring/environment-variables#test-environment-variables)。
...
如果项目使用[模块路径别名](/docs/14/pages/building-your-application/configuring/absolute-imports-and-module-aliases)，需通过配置 Jest 的 `moduleNameMapper` 选项来匹配 `jsconfig.json` 中的路径设置：
