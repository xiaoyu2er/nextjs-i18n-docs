...
路由可以通过子路径 (`/fr/products`) 或域名 (`my-site.fr/products`) 实现国际化。利用这些信息，您现在可以根据 [中间件](/docs/14/app/building-your-application/routing/middleware) 中的区域设置重定向用户。
...
由于 `app/` 目录中的所有布局和页面默认都是 [服务端组件](/docs/14/app/building-your-application/rendering/server-components)，我们无需担心翻译文件的大小会影响客户端 JavaScript 包的大小。此代码 **仅在服务器端运行**，只有生成的 HTML 会发送到浏览器。
