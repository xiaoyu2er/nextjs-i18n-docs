...
translation-updated-at: 2025-05-21T20:28:06.900Z
title: Codemods
...
此代码修改工具将 [动态 OG 图片生成](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation) 相关的导入从 `next/server` 迁移至 `next/og`。
...
移除 [链接组件](/docs/14/app/api-reference/components/link) 中的 `<a>` 标签，或为无法自动修复的链接添加 `legacyBehavior` 属性。

</AppOnly>

<PagesOnly>

移除 [链接组件](/docs/14/pages/api-reference/components/link) 中的 `<a>` 标签，或为无法自动修复的链接添加 `legacyBehavior` 属性。
