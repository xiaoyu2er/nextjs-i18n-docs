...
- `query`: `Object` - 解析为对象的查询字符串，包括 [动态路由](/docs/14/pages/building-your-application/routing/dynamic-routes) 参数。如果页面未使用 [服务端渲染 (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)，则在预渲染期间为空对象。默认为 `{}`。
- `asPath`: `String` - 浏览器中显示的路径，包括搜索参数并遵循 `trailingSlash` 配置。不包含 `basePath` 和 `locale`。
- `isFallback`: `boolean` - 当前页面是否处于 [回退模式 (fallback mode)](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)。
- `basePath`: `String` - 当前激活的 [basePath](/docs/14/app/api-reference/next-config-js/basePath)（如果启用）。
...
- `isReady`: `boolean` - 路由器字段是否已在客户端更新并准备就绪。应仅在 `useEffect` 方法中使用，不可用于服务端条件渲染。相关用例请参阅 [自动静态优化页面](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) 文档。
- `isPreview`: `boolean` - 应用当前是否处于 [预览模式 (preview mode)](/docs/14/pages/building-your-application/configuring/preview-mode)。

> 如果页面使用服务端渲染或 [自动静态优化 (automatic static optimization)](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)，使用 `asPath` 字段可能导致客户端与服务端不匹配。在 `isReady` 字段为 `true` 之前，避免使用 `asPath`。

`router` 包含以下方法：

### router.push

处理客户端导航，此方法适用于 [`next/link`](/docs/14/pages/api-reference/components/link) 不够用的情况。
...
- `options` - 可选配置对象，包含以下选项：
  ...
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): 更新当前页面路径而不重新运行 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)、[`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 或 [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props)。默认为 `false`。
...
将用户重定向到 `pages/login.js`，适用于 [身份验证 (authentication)](/docs/14/pages/building-your-application/authentication) 后的页面：
...
可以像在 [`next/link`](/docs/14/pages/api-reference/components/link#with-url-object) 中一样使用 URL 对象。适用于 `url` 和 `as` 参数：
...
类似于 [`next/link`](/docs/14/pages/api-reference/components/link) 中的 `replace` 属性，`router.replace` 会阻止将新 URL 添加到 `history` 堆栈中。
...
预取页面以实现更快的客户端导航。此方法仅适用于没有使用 [`next/link`](/docs/14/pages/api-reference/components/link) 的导航，因为 `next/link` 会自动处理页面预取。
...
在某些情况下（例如使用 [自定义服务器 (Custom Server)](/docs/14/pages/building-your-application/configuring/custom-server)），你可能希望监听 [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) 并在路由器处理之前执行某些操作。
...
如果 `cb` 返回 `false`，Next.js 路由器将不会处理 `popstate`，你需要自行处理。请参阅 [禁用文件系统路由 (Disabling file-system routing)](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing)。
...
> **须知**：这里的 `url` 是浏览器中显示的 URL，包含 [`basePath`](/docs/14/app/api-reference/next-config-js/basePath)。
...
> 本例使用 [自定义应用 (Custom App)](/docs/14/pages/building-your-application/routing/custom-app) (`pages/_app.js`) 订阅事件，因为它在页面导航时不会卸载，但你可以在应用中的任何组件订阅路由器事件。
