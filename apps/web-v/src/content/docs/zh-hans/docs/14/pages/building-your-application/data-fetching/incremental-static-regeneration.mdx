...
> **须知**：当前 [`edge` 运行时](/docs/14/pages/api-reference/edge) 与 ISR 不兼容，但您可以通过手动设置 `cache-control` 标头来利用 `stale-while-revalidate` 策略。
...
> **须知**：对于按需 ISR 请求，[中间件](/docs/14/pages/building-your-application/routing/middleware) 不会执行。相反，应在您想要重新验证的 _确切_ 路径上调用 `revalidate()`。例如，如果您有 `pages/blog/[slug].js` 和从 `/post-1` 重写到 `/blog/post-1` 的规则，则需要调用 `res.revalidate('/blog/post-1')`。
...
接下来，将密钥作为[环境变量](/docs/14/pages/building-your-application/configuring/environment-variables)添加到您的应用中。最后，创建重新验证 API 路由：
...
在使用 `next dev` 本地运行时，`getStaticProps` 会在每个请求上调用。要验证您的按需 ISR 配置是否正确，您需要创建[生产构建](/docs/14/pages/api-reference/cli/next#next-build-options)并启动[生产服务器](/docs/14/pages/api-reference/cli/next#next-start-options)：
...
增量静态再生 (ISR) 在[自托管 Next.js 站点](/docs/14/pages/building-your-application/deploying#self-hosting)上开箱即用，只需使用 `next start` 即可。

了解更多关于[自托管 Next.js](/docs/14/pages/building-your-application/deploying#self-hosting) 的信息。
