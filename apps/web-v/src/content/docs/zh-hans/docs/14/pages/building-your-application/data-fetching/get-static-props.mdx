...
[`getStaticProps` API 参考文档](/docs/14/pages/api-reference/functions/get-static-props)涵盖了可与 `getStaticProps` 一起使用的所有参数和 props。
...
- 使用 [`fallback: true`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true) 时，`getStaticProps` 会在后台运行
- 使用 [`fallback: blocking`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking) 时，`getStaticProps` 会在初始渲染前调用
- 使用 `revalidate` 时，`getStaticProps` 会在后台运行
- 使用 [`revalidate()`](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) 时，`getStaticProps` 会根据需求在后台运行

与[增量静态再生 (Incremental Static Regeneration)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) 结合使用时，`getStaticProps` 会在重新验证过期页面的同时在后台运行，并将新页面提供给浏览器。

`getStaticProps` 无法访问传入的请求（如查询参数或 HTTP 标头），因为它生成的是静态 HTML。如果页面需要访问请求，可考虑在使用 `getStaticProps` 的同时配合使用[中间件 (Middleware)](/docs/14/pages/building-your-application/routing/middleware)。
...
此 JSON 文件将通过 [`next/link`](/docs/14/pages/api-reference/components/link) 或 [`next/router`](/docs/14/pages/api-reference/functions/use-router) 用于客户端路由。当导航到使用 `getStaticProps` 预渲染的页面时，Next.js 会获取此 JSON 文件（在构建时预计算）并将其用作页面组件的 props。这意味着客户端页面转换**不会**调用 `getStaticProps`，因为只使用导出的 JSON。
...
> **须知**：如果您创建了[自定义应用 (custom app)](/docs/14/pages/building-your-application/routing/custom-app)，请确保按照链接文档所示将 `pageProps` 传递给页面组件，否则 props 将为空。
...
您可以使用[**预览模式 (Preview Mode)**](/docs/14/pages/building-your-application/configuring/preview-mode) 临时绕过静态生成，在**请求时**而非构建时渲染页面。例如，您可能正在使用无头 CMS 并希望在草稿发布前预览它们。