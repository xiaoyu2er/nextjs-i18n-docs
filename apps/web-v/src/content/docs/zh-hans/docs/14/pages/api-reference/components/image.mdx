...
translation-updated-at: 2025-05-21T20:27:24.626Z
title: <Image>
...
> **须知**：如果您使用的是 Next.js 13 之前的版本，请查阅 [next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) 文档，因为该组件已重命名。

</PagesOnly>

本 API 参考将帮助您理解如何使用图片组件的 [属性](#props) 和 [配置选项](#configuration-options)。关于功能和用法，请参阅 [图片组件](/docs/14/app/building-your-application/optimizing/images) 页面。
...
- [静态导入](/docs/14/app/building-your-application/optimizing/images#local-images) 的图片文件
...
必填，除非是 [静态导入的图片](/docs/14/app/building-your-application/optimizing/images#local-images) 或使用了 [`fill` 属性](#fill) 的图片。
...
> **须知**：使用像 `loader` 这样接受函数的属性时，需要使用 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 来序列化提供的函数。
...
当为 `blur` 时，将使用 [`blurDataURL`](#blurdataurl) 属性作为占位符。如果 `src` 是 [静态导入](/docs/14/app/building-your-application/optimizing/images#local-images) 的对象且导入的图片是 `.jpg`、`.png`、`.webp` 或 `.avif`，则 `blurDataURL` 将自动填充，除非检测到图片是动画的。
...
> **须知**：使用像 `onLoadingComplete` 这样接受函数的属性时，需要使用 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 来序列化提供的函数。
...
> **须知**：使用像 `onLoad` 这样接受函数的属性时，需要使用 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 来序列化提供的函数。
...
> **须知**：使用像 `onError` 这样接受函数的属性时，需要使用 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 来序列化提供的函数。
...
- [自定义图片加载器配置](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **须知**：自定义图片加载器文件（接受函数）需要使用 [客户端组件](/docs/14/app/building-your-application/rendering/client-components) 来序列化提供的函数。
...
可以配置缓存优化图片的生存时间（TTL，秒）。在许多情况下，最好使用 [静态图片导入](/docs/14/app/building-your-application/optimizing/images#local-images)，它会自动哈希文件内容并使用 `Cache-Control` 头 `immutable` 永久缓存图片。
...
如果需要按图片更改缓存行为，可以配置 [`headers`](/docs/14/app/api-reference/next-config-js/headers) 在上游图片（例如 `/some-asset.jpg`，而非 `/_next/image` 本身）上设置 `Cache-Control` 头。
...
默认的 [loader](#loader) 出于几个原因不会优化 SVG 图像。首先，SVG 是一种矢量格式，意味着它可以无损调整大小。其次，SVG 具有许多与 HTML/CSS 相同的特性，若没有正确的 [内容安全策略 (CSP) 标头](/docs/14/app/api-reference/next-config-js/headers#content-security-policy)，可能会导致安全漏洞。
...
| `v13.0.6`  | 新增 `ref` 属性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `v13.0.0`  | `next/image` 导入路径更名为 `next/legacy/image`，`next/future/image` 导入路径更名为 `next/image`。提供 [自动化迁移工具](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) 安全重命名导入路径。移除 `<span>` 包裹层。移除 `layout`、`objectFit`、`objectPosition`、`lazyBoundary`、`lazyRoot` 属性。`alt` 变为必填项。`onLoadingComplete` 回调接收 `img` 元素引用。移除内置加载器配置。 |
