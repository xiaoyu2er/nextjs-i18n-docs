...
`redirect` 函数允许你将用户重定向到另一个 URL。可以在[服务端组件](/docs/14/app/building-your-application/rendering/server-components)、[路由处理器](/docs/14/app/building-your-application/routing/route-handlers)和[服务端操作](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)中调用此函数。
...
查看 [`redirect` API 参考文档](/docs/14/app/api-reference/functions/redirect)获取更多信息。

## `permanentRedirect` 函数

`permanentRedirect` 函数允许你**永久**重定向用户到另一个 URL。可以在[服务端组件](/docs/14/app/building-your-application/rendering/server-components)、[路由处理器](/docs/14/app/building-your-application/routing/route-handlers)和[服务端操作](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)中调用此函数。
...
查看 [`permanentRedirect` API 参考文档](/docs/14/app/api-reference/functions/permanentRedirect)获取更多信息。
...
> **须知**：
>
> - 如果不需要编程式导航，应使用 [`<Link>`](/docs/14/app/api-reference/components/link) 组件。

<AppOnly>

查看 [`useRouter` API 参考文档](/docs/14/app/api-reference/functions/use-router)获取更多信息。

</AppOnly>

<PagesOnly>

查看 [`useRouter` API 参考文档](/docs/14/pages/api-reference/functions/use-router)获取更多信息。
...
`redirects` 支持[路径匹配](/docs/14/app/api-reference/next-config-js/redirects#path-matching)、[请求头、Cookie 和查询参数匹配](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching)，可根据传入请求灵活重定向用户。
...
查看 [`redirects` API 参考文档](/docs/14/app/api-reference/next-config-js/redirects)获取更多信息。

> **须知**：
>
> - `redirects` 可通过 `permanent` 选项返回 307 (临时重定向) 或 308 (永久重定向) 状态码。
> - 不同平台可能对 `redirects` 有限制。例如 Vercel 限制 1,024 条重定向规则。如需管理大量重定向 (1000+)，可考虑使用[中间件](/docs/14/app/building-your-application/routing/middleware)创建自定义解决方案。详见[大规模重定向管理](#managing-redirects-at-scale-advanced)。
...
查看[中间件文档](/docs/14/app/building-your-application/routing/middleware)获取更多信息。
...
如果存在，则将请求转发至 <AppOnly>[路由处理器 (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers)</AppOnly> <PagesOnly>[API 路由 (API Routes)](/docs/14/pages/building-your-application/routing/api-routes)</PagesOnly>，该处理器会检查实际文件并将用户重定向到适当的 URL。这样可以避免将大型重定向文件导入中间件，从而避免拖慢每个传入请求。
