...
translation-updated-at: 2025-05-21T20:28:23.596Z
title: 身份验证
description: 学习如何在 Next.js 中实现身份验证，涵盖最佳实践、路由保护、授权技术和会话管理等内容。
...
Next.js 中的[中间件 (Middleware)](/docs/14/app/building-your-application/routing/middleware) 可帮助控制网站不同部分的访问权限。这对于保护用户仪表盘等区域同时保持营销页面公开非常重要。建议对所有路由应用中间件，并为公开访问指定例外。
...
对待[服务端操作](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)时，应与面向公众的 API 端点一样重视安全性。验证每个操作用户的授权至关重要。在服务端操作中实施权限检查，例如限制某些操作仅限管理员用户执行。
...
Next.js 中的[服务端组件](/docs/14/app/building-your-application/rendering/server-components)专为服务端执行设计，为集成复杂逻辑（如授权）提供了安全环境。它们可直接访问后端资源，优化数据密集型任务的性能，并增强敏感操作的安全性。
