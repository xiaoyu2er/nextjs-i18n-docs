...
- `query`: `Object` - クエリ文字列をオブジェクトにパースしたもの。[動的ルート](/docs/14/pages/building-your-application/routing/dynamic-routes)パラメータを含みます。ページが[サーバーサイドレンダリング (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)を使用していない場合、プリレンダリング中は空のオブジェクトになります。デフォルトは`{}`
- `asPath`: `String` - ブラウザに表示されるパスで、検索パラメータを含み、`trailingSlash`設定を尊重します。`basePath`と`locale`は含まれません。
- `isFallback`: `boolean` - 現在のページが[フォールバックモード](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)かどうか。
- `basePath`: `String` - 有効な[basePath](/docs/14/app/api-reference/next-config-js/basePath)（有効な場合）。
...
- `isReady`: `boolean` - ルーターフィールドがクライアントサイドで更新され、使用準備ができているかどうか。`useEffect`内でのみ使用し、サーバーでの条件付きレンダリングには使用しないでください。[自動静的最適化ページ](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)のユースケースに関連するドキュメントを参照してください。
- `isPreview`: `boolean` - アプリケーションが現在[プレビューモード](/docs/14/pages/building-your-application/configuring/preview-mode)かどうか。

> `asPath`フィールドを使用すると、ページがサーバーサイドレンダリングまたは[自動静的最適化](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)でレンダリングされている場合、クライアントとサーバーの間に不一致が生じる可能性があります。`isReady`フィールドが`true`になるまで`asPath`の使用を避けてください。

`router`には以下のメソッドが含まれています：

### router.push

クライアントサイドの遷移を処理します。[`next/link`](/docs/14/pages/api-reference/components/link)では不十分な場合に便利です。
...
- `options` - 以下の設定オプションを持つオプショナルなオブジェクト：
  ...
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)、[`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)、[`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props)を再実行せずに現在のページのパスを更新します。デフォルトは`false`
...
[認証](/docs/14/pages/building-your-application/authentication)が必要なページの場合、ユーザーを`pages/login.js`にリダイレクト：
...
[`next/link`](/docs/14/pages/api-reference/components/link#with-url-object)と同様にURLオブジェクトを使用できます。`url`と`as`パラメータの両方で動作します：
...
[`next/link`](/docs/14/pages/api-reference/components/link)の`replace`プロパティと同様に、`router.replace`は新しいURLエントリを`history`スタックに追加しません。
...
クライアントサイドの遷移を高速化するためにページをプリフェッチします。このメソッドは[`next/link`](/docs/14/pages/api-reference/components/link)を使用しないナビゲーションにのみ有用です。`next/link`は自動的にページをプリフェッチします。
...
[カスタムサーバー](/docs/14/pages/building-your-application/configuring/custom-server)を使用している場合など、ルーターが処理する前に[popstate](https://developer.mozilla.org/docs/Web/Events/popstate)をリッスンして何かを行いたい場合があります。
...
`cb`が`false`を返す場合、Next.jsルーターは`popstate`を処理せず、その場合は自分で処理する必要があります。[ファイルシステムルーティングの無効化](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing)を参照してください。
...
> **知っておくと良いこと**: ここでの`url`は[`basePath`](/docs/14/app/api-reference/next-config-js/basePath)を含むブラウザに表示されるURLです。
...
> この例では[カスタムApp](/docs/14/pages/building-your-application/routing/custom-app)（`pages/_app.js`）を使用しています。これはページナビゲーション時にアンマウントされないためですが、アプリケーション内の任意のコンポーネントでルーターイベントを購読できます。
