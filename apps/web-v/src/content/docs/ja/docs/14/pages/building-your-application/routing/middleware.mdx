...
1. レスポンスを生成するルート（[ページ](/docs/14/app/building-your-application/routing/pages-and-layouts)または[Route Handler](/docs/14/app/building-your-application/routing/route-handlers)）に`rewrite`
...
1. レスポンスを生成するルート（[ページ](/docs/14/pages/building-your-application/routing/pages-and-layouts)または[Edge API Route](/docs/14/pages/building-your-application/routing/api-routes)）に`rewrite`
2. 直接`NextResponse`を返す。[レスポンスの生成](#producing-a-response)を参照
...
> **補足:** [Route Handlers](/docs/14/app/building-your-application/routing/route-handlers#cors)で個々のルートに対してCORSヘッダーを設定できます。
...
ミドルウェアは現在[Edgeランタイム](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)のみをサポートしています。Node.jsランタイムは使用できません。
