...
translation-updated-at: 2025-06-06T16:36:36.978Z
title: リダイレクト
...
Pages Routerを使用する場合、リダイレクトはクライアントサイドルーティング（`Link`、`router.push`）には適用されません（[Middleware](/docs/14/app/building-your-application/routing/middleware)が存在し、パスに一致する場合を除く）。
...
[`basePath`サポート](/docs/14/app/api-reference/next-config-js/basePath)をリダイレクトで活用する場合、各`source`と`destination`には自動的に`basePath`がプレフィックスとして追加されます（リダイレクトに`basePath: false`を追加しない限り）：
...
[`i18n`サポート](/docs/14/app/building-your-application/routing/internationalization)をリダイレクトで活用する場合、各`source`と`destination`には自動的に設定された`locales`を処理するためのプレフィックスが追加されます（リダイレクトに`locale: false`を追加しない限り）。`locale: false`を使用する場合、`source`と`destination`にロケールをプレフィックスとして付ける必要があります。

</AppOnly>

<PagesOnly>

[`i18n`サポート](/docs/14/pages/building-your-application/routing/internationalization)をリダイレクトで活用する場合、各`source`と`destination`には自動的に設定された`locales`を処理するためのプレフィックスが追加されます（リダイレクトに`locale: false`を追加しない限り）。`locale: false`を使用する場合、`source`と`destination`にロケールをプレフィックスとして付ける必要があります。
...
- [API Routes](/docs/14/pages/building-your-application/routing/api-routes)と[Route Handlers](/docs/14/app/building-your-application/routing/route-handlers)内では、受信リクエストに基づいてリダイレクトできます
- [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)と[`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)内では、リクエスト時に特定のページをリダイレクトできます
