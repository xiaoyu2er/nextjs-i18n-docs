...
> **知っておくと良いこと**: App Router を使用している場合、API ルートの代わりに [サーバーコンポーネント](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) または [ルートハンドラー](/docs/14/app/building-your-application/routing/route-handlers) を利用できます。
...
> - API ルートは [静的エクスポート](/docs/14/pages/building-your-application/deploying/static-exports) と一緒に使用できません。ただし、App Router の [ルートハンドラー](/docs/14/app/building-your-application/routing/route-handlers) は使用可能です。
> - API ルートは `next.config.js` の [`pageExtensions` 設定](/docs/14/pages/api-reference/next-config-js/pageExtensions) の影響を受けます。
...
- `res.revalidate(urlPath)` - `getStaticProps` を使用して [オンデマンドでページを再検証](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)。`urlPath` は `string` でなければなりません
...
API ルートは [動的ルート](/docs/14/pages/building-your-application/routing/dynamic-routes) をサポートしており、`pages/` と同じファイル命名規則に従います。
...
Edge Runtime で API ルートを使用したい場合、App Router を段階的に採用し、[ルートハンドラー](/docs/14/app/building-your-application/routing/route-handlers) を使用することを推奨します。
