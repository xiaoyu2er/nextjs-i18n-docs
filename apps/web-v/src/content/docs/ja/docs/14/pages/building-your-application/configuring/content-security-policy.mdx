...
[ミドルウェア](/docs/14/app/building-your-application/routing/middleware)を使用すると、ページがレンダリングされる前にヘッダーを追加し、ノンスを生成できます。
...
デフォルトでは、ミドルウェアはすべてのリクエストで実行されます。[`matcher`](/docs/14/app/building-your-application/routing/middleware#matcher)を使用して、特定のパスでミドルウェアを実行するようにフィルタリングできます。
...
[`headers`](/docs/14/app/api-reference/functions/headers)を使用して、[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)からノンスを読み取ることができます:
...
ノンスを必要としないアプリケーションでは、[`next.config.js`](/docs/14/app/api-reference/next-config-js)ファイルに直接CSPヘッダーを設定できます:
