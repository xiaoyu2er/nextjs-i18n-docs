...
`redirect` 関数を使用すると、ユーザーを別のURLにリダイレクトできます。[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)、[ルートハンドラー](/docs/14/app/building-your-application/routing/route-handlers)、[サーバーアクション](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)で呼び出すことができます。
...
詳細は [`redirect` APIリファレンス](/docs/14/app/api-reference/functions/redirect) をご覧ください。

## `permanentRedirect` 関数

`permanentRedirect` 関数を使用すると、ユーザーを**永続的**に別のURLにリダイレクトできます。[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)、[ルートハンドラー](/docs/14/app/building-your-application/routing/route-handlers)、[サーバーアクション](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)で呼び出すことができます。
...
詳細は [`permanentRedirect` APIリファレンス](/docs/14/app/api-reference/functions/permanentRedirect) をご覧ください。
...
> **知っておくと良いこと**:
>
> - プログラム的にユーザーをナビゲートする必要がない場合は、[`<Link>`](/docs/14/app/api-reference/components/link) コンポーネントを使用するべきです。

<AppOnly>

詳細は [`useRouter` APIリファレンス](/docs/14/app/api-reference/functions/use-router) をご覧ください。

</AppOnly>

<PagesOnly>

詳細は [`useRouter` APIリファレンス](/docs/14/pages/api-reference/functions/use-router) をご覧ください。
...
`redirects` は[パス](/docs/14/app/api-reference/next-config-js/redirects#path-matching)、[ヘッダー、クッキー、クエリマッチング](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching)をサポートしており、受信リクエストに基づいてユーザーをリダイレクトする柔軟性を提供します。
...
詳細は [`redirects` APIリファレンス](/docs/14/app/api-reference/next-config-js/redirects) をご覧ください。

> **知っておくと良いこと**:
>
> - `redirects` は `permanent` オプションで307 (一時的リダイレクト) または308 (永続的リダイレクト) ステータスコードを返せます。
> - プラットフォームによって `redirects` に制限がある場合があります。例えばVercelでは1,024件のリダイレクト制限があります。大量のリダイレクト(1000件以上)を管理する場合は、[ミドルウェア](/docs/14/app/building-your-application/routing/middleware)を使用したカスタムソリューションの作成を検討してください。詳細は[大規模なリダイレクト管理](#managing-redirects-at-scale-advanced)をご覧ください。
...
詳細は [ミドルウェア](/docs/14/app/building-your-application/routing/middleware) ドキュメントをご覧ください。
...
存在する場合、リクエストを <AppOnly>[Route Handler（ルートハンドラー）](/docs/14/app/building-your-application/routing/route-handlers)</AppOnly> <PagesOnly>[API Routes（APIルート）](/docs/14/pages/building-your-application/routing/api-routes)</PagesOnly> に転送します。これにより実際のファイルをチェックし、ユーザーを適切なURLにリダイレクトします。これにより、すべての受信リクエストを遅くする可能性のある大きなリダイレクトファイルをミドルウェアにインポートすることを回避できます。
