...
**not-found** ファイルは、ルートセグメント内で [`notFound`](/docs/14/app/api-reference/functions/not-found) 関数がスローされたときに UI をレンダリングするために使用されます。カスタム UI を提供するだけでなく、Next.js はストリーミングレスポンスに対して `200` HTTP ステータスコードを、非ストリーミングレスポンスに対して `404` を返します。
