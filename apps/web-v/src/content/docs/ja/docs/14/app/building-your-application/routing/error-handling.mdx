...
`error.js` ファイル規約を使用すると、[ネストされたルート](/docs/14/app/building-your-application/routing#nested-routes)で予期しないランタイムエラーを適切に処理できます。
...
[特別なファイル](/docs/14/app/building-your-application/routing#file-conventions)を通じて作成されたReactコンポーネントは、[特定のネスト階層](/docs/14/app/building-your-application/routing#component-hierarchy)でレンダリングされます。
