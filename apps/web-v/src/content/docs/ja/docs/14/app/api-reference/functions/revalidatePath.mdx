...
`revalidatePath` を使用すると、特定のパスに対してオンデマンドで[キャッシュデータ](/docs/14/app/building-your-application/caching)を削除できます。

> **補足情報**:
>
> - `revalidatePath` は[Node.js と Edge ランタイム](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)の両方で利用可能です。
...
> - 現在、`revalidatePath` は[クライアントサイドのルーターキャッシュ](/docs/14/app/building-your-application/caching#router-cache)内のすべてのルートを無効化します。この動作は一時的なもので、将来は指定されたパスのみに適用されるように更新される予定です。
> - `revalidatePath` を使用すると、[サーバーサイドのルートキャッシュ](/docs/14/app/building-your-application/caching#full-route-cache)内の**特定のパスのみ**が無効化されます。
