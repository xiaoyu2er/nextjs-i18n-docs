...
このAPIリファレンスでは、[`next/font/google`](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) と [`next/font/local`](/docs/14/app/building-your-application/optimizing/fonts#local-fonts) の使用方法を理解するのに役立ちます。機能と使用方法については、[フォントの最適化](/docs/14/app/building-your-application/optimizing/fonts) ページを参照してください。

### フォント関数の引数

使用方法については、[Google Fonts](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) と [ローカルフォント](/docs/14/app/building-your-application/optimizing/fonts#local-fonts) を確認してください。
...
- [可変Googleフォント](https://fonts.google.com/variablefonts) でない場合のウェイト値の配列（`next/font/google` のみに適用）
...
- `weight: '400'`: 単一のウェイト値の文字列 - [`Inter`](https://fonts.google.com/specimen/Inter?query=inter) フォントの場合、可能な値は `'100'`, `'200'`, `'300'`, `'400'`, `'500'`, `'600'`, `'700'`, `'800'`, `'900'` または `'variable'`（デフォルトは `'variable'`）
...
- [可変Googleフォント](https://fonts.google.com/variablefonts) でない場合のスタイル値の配列（`next/font/google` のみに適用）
...
- `style: 'oblique'`: 文字列 - `next/font/local` の場合は任意の値を指定可能ですが、[標準フォントスタイル](https://developer.mozilla.org/docs/Web/CSS/font-style) からの値が期待されます
- `style: ['italic','normal']`: `next/font/google` 用の2つの値の配列 - 値は `normal` と `italic` から選択

### `subsets`

フォントの [`subsets`](https://fonts.google.com/knowledge/glossary/subsetting) を文字列値の配列で定義します。各サブセットの名前を指定すると、[プリロード](/docs/14/app/building-your-application/optimizing/fonts#specifying-a-subset) されます。`subsets` で指定されたフォントは、[`preload`](#preload) オプションが true（デフォルト）の場合、head にリンクプリロードタグが挿入されます。
...
- `axes: ['slnt']`: `Inter` 可変フォントの追加 `axes` として `slnt` を指定する配列（[こちら](https://fonts.google.com/variablefonts?vfquery=inter#font-families) で確認可能）。フォントの可能な `axes` 値は、[Google可変フォントページ](https://fonts.google.com/variablefonts#font-families) で `wght` 以外の軸をフィルタリングして確認できます

### `display`

フォントの [`display`](https://developer.mozilla.org/docs/Web/CSS/@font-face/font-display) を指定します。可能な文字列 [値](https://developer.mozilla.org/docs/Web/CSS/@font-face/font-display#values) は `'auto'`, `'block'`, `'swap'`, `'fallback'`, `'optional'` で、デフォルト値は `'swap'` です。
...
フォントを [プリロード](/docs/14/app/building-your-application/optimizing/fonts#preloading) するかどうかを指定するブール値。デフォルトは `true` です。
...
- `next/font/google` の場合: [累積レイアウトシフト (CLS)](https://web.dev/cls/) を減らすために自動フォールバックフォントを使用するかどうかを指定するブール値。デフォルトは `true`。
- `next/font/local` の場合: [累積レイアウトシフト (CLS)](https://web.dev/cls/) を減らすために自動フォールバックフォントを使用するかどうかを指定する文字列またはブール値 `false`。可能な値は `'Arial'`, `'Times New Roman'` または `false`。デフォルトは `'Arial'`。
