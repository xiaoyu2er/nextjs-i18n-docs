...
translation-updated-at: 2025-06-06T16:41:58.514Z
...
- **Edge ランタイム**: [Web API](/docs/14/app/api-reference/edge) をベースにしています
...
| ------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- | ---------------- |
| コールドブート                                                                                                                       | /      | 通常       | 低               |
| [HTTP ストリーミング](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)                                           | 可     | 可         | 可               |
| I/O                                                                                                                                   | 全     | 全         | `fetch` のみ     |
| スケーラビリティ                                                                                                                     | /      | 高         | 最高             |
| セキュリティ                                                                                                                         | 通常   | 高         | 高               |
| レイテンシ                                                                                                                           | 通常   | 低         | 最低             |
| npm パッケージ                                                                                                                       | 全     | 全         | 限定的なサブセット |
| [静的レンダリング](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)                          | 可     | 可         | 不可             |
| [動的レンダリング](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)                                 | 可     | 可         | 可               |
| [`fetch` を使用したデータ再検証](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) | 可     | 可         | 可               |
...
Next.js アプリケーションでは、個々のルートセグメントに対してランタイムを指定できます。これを行うには、[`runtime` という変数を宣言してエクスポートします](/docs/14/app/api-reference/file-conventions/route-segment-config)。変数は文字列でなければならず、値は `'nodejs'` または `'edge'` ランタイムのいずれかでなければなりません。
...
> 利用可能な API の完全なリストについては、[Node.js ドキュメント](https://nodejs.org/docs/latest/api/) と [Edge ドキュメント](/docs/14/app/api-reference/edge) を参照してください。どちらのランタイムも、デプロイインフラストラクチャに応じて [ストリーミング](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) をサポートできます。