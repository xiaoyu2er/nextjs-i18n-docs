...
- [`useRouter` フック](#userouter-hook)の使用（[クライアントコンポーネント](/docs/14/app/building-your-application/rendering/client-components)）
- [`redirect` 関数](#redirect-function)の使用（[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)）
...
`<Link>` には他にもオプションのプロパティを渡せます。詳細は[APIリファレンス](/docs/14/app/api-reference/components/link)を参照してください。

### 使用例

#### ダイナミックセグメントへのリンク

[ダイナミックセグメント](/docs/14/app/building-your-application/routing/dynamic-routes)にリンクする場合、[テンプレートリテラルと補間](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals)を使用してリンクのリストを生成できます。例えば、ブログ記事のリストを生成する場合:
...
[`usePathname()`](/docs/14/app/api-reference/functions/use-pathname)を使用してリンクがアクティブかどうかを判断できます。例えば、アクティブなリンクにクラスを追加するには、現在の `pathname` がリンクの `href` と一致するかどうかを確認します:
...
> **豆知識**:
>
> - Next.jsは、ナビゲーション時にビューポート内に表示されていない場合、[ページ](/docs/14/app/building-your-application/routing/pages-and-layouts#pages)までスクロールします。
...
`useRouter` フックを使用すると、[クライアントコンポーネント](/docs/14/app/building-your-application/rendering/client-components)からプログラムでルートを変更できます。
...
`useRouter` の全メソッドについては、[APIリファレンス](/docs/14/app/api-reference/functions/use-router)を参照してください。

> **推奨事項**: `useRouter` を使用する特別な要件がない限り、ルート間を移動するには `<Link>` コンポーネントを使用してください。

## `redirect` 関数

[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)では、代わりに `redirect` 関数を使用します。
...
> - レンダリングプロセスの前にリダイレクトしたい場合は、[`next.config.js`](/docs/14/app/building-your-application/routing/redirecting#redirects-in-nextconfigjs) または [ミドルウェア](/docs/14/app/building-your-application/routing/redirecting#nextresponseredirect-in-middleware)を使用します。

詳細については、[`redirect` APIリファレンス](/docs/14/app/api-reference/functions/redirect)を参照してください。
...
`pushState` と `replaceState` の呼び出しはNext.jsルーターと統合され、[`usePathname`](/docs/14/app/api-reference/functions/use-pathname) および [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params) と同期できます。
...
[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)により、アプリケーションコードがルートセグメントごとに自動的にコード分割されます。つまり、ナビゲーション時に現在のルートに必要なコードのみが読み込まれます。
...
`<Link>` のデフォルトのプリフェッチ動作（つまり、`prefetch` プロパティが指定されていないか `null` に設定されている場合）は、[`loading.js`](/docs/14/app/api-reference/file-conventions/loading) の使用状況によって異なります。レンダリングされたコンポーネントの「ツリー」で最初の `loading.js` ファイルまでの共有レイアウトのみがプリフェッチされ、30秒間キャッシュされます。これにより、ダイナミックルート全体をフェッチするコストが削減され、ユーザーに視覚的なフィードバックを提供するための[即時ローディング状態](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states)を表示できます。

`prefetch` プロパティを `false` に設定することでプリフェッチを無効にできます。または、`prefetch` プロパティを `true` に設定することで、ローディング境界を超えた完全なページデータをプリフェッチできます。

詳細については、[`<Link>` APIリファレンス](/docs/14/app/api-reference/components/link)を参照してください。
...
Next.jsには[Router Cache](/docs/14/app/building-your-application/caching#router-cache)と呼ばれる**メモリ内のクライアントサイドキャッシュ**があります。ユーザーがアプリ内を移動すると、[プリフェッチ](#2-prefetching)されたルートセグメントと訪問済みルートのReactサーバーコンポーネントペイロードがキャッシュに保存されます。

つまり、ナビゲーション時にキャッシュが可能な限り再利用され、サーバーへの新しいリクエストが行われないため、リクエスト数と転送データ量が減少し、パフォーマンスが向上します。

[Router Cache](/docs/14/app/building-your-application/caching#router-cache)の仕組みと設定方法について詳しく学びます。
...
デフォルトでは、Next.jsは前後ナビゲーションのスクロール位置を維持し、[Router Cache](/docs/14/app/building-your-application/caching#router-cache)内のルートセグメントを再利用します。
