...
> **知っておくと便利**:
>
> - [サーバー中心のルーティング](/docs/14/app/building-your-application/routing/linking-and-navigating#how-routing-and-navigation-works)でもナビゲーションは即時に行われます。
...
`loading.js` に加えて、独自のUIコンポーネントに対して手動でSuspenseバウンダリを作成できます。App Routerは[Node.jsとEdgeランタイム](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)の両方で[Suspense](https://react.dev/reference/react/Suspense)によるストリーミングをサポートしています。
...
- Next.jsは[`generateMetadata`](/docs/14/app/api-reference/functions/generate-metadata)内のデータフェッチが完了するまで、UIのクライアントへのストリーミングを待機します。これにより、ストリーミングレスポンスの最初の部分に`<head>`タグが含まれることが保証されます。
...
サーバーは、ストリーミングコンテンツ内でエラーや問題をクライアントに伝えることができます（例: [`redirect`](/docs/14/app/api-reference/functions/redirect) や [`notFound`](/docs/14/app/api-reference/functions/not-found) を使用時）。レスポンスヘッダーは既にクライアントに送信されているため、レスポンスのステータスコードを更新することはできません。これはSEOに影響しません。