...
事前に正確なセグメント名がわからず、動的なデータからルートを作成したい場合、リクエスト時に値を設定するか、ビルド時に[プリレンダリング](/docs/14/pages/building-your-application/data-fetching/get-static-paths)するダイナミックセグメントを使用できます。

## 規約

ファイル名またはフォルダ名を角括弧で囲むことでダイナミックセグメントを作成できます: `[segmentName]`。例: `[id]` や `[slug]`。

ダイナミックセグメントは[`useRouter`](/docs/14/pages/api-reference/functions/use-router)からアクセス可能です。
