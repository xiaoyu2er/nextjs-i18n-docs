...
Next.js では、Pages ルーターと App ルーターの[デフォルトキャッシュハンドラ](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)はファイルシステムキャッシュを使用します。これは設定不要ですが、`next.config.js` の `cacheHandler` フィールドを使用してキャッシュハンドラをカスタマイズできます。
...
[カスタムキャッシュハンドラの例](/docs/14/app/building-your-application/deploying#configuring-caching)を参照し、実装の詳細について学びましょう。
...
`Promise<void>` を返します。[データの再検証](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)や[`revalidateTag()` 関数](/docs/14/app/api-reference/functions/revalidateTag)について詳しく学びましょう。
