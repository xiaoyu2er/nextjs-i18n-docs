...
> **補足**: 現在、[`edge`ランタイム](/docs/14/pages/api-reference/edge)はISRと互換性がありませんが、`cache-control`ヘッダーを手動で設定することで`stale-while-revalidate`を活用できます。
...
> **補足**: [ミドルウェア](/docs/14/pages/building-your-application/routing/middleware)はオンデマンドISRリクエストでは実行されません。代わりに、再検証したい正確なパスで`revalidate()`を呼び出してください。例えば、`pages/blog/[slug].js`があり、`/post-1`から`/blog/post-1`へのリライトがある場合、`res.revalidate('/blog/post-1')`を呼び出す必要があります。
...
次に、この秘密をアプリケーションの[環境変数](/docs/14/pages/building-your-application/configuring/environment-variables)として追加します。最後に、再検証APIルートを作成します:
...
`next dev`でローカル実行する場合、`getStaticProps`はすべてのリクエストで呼び出されます。オンデマンドISR設定が正しいことを確認するには、[本番ビルド](/docs/14/pages/api-reference/cli/next#next-build-options)を作成し、[本番サーバー](/docs/14/pages/api-reference/cli/next#next-start-options)を起動する必要があります:
...
インクリメンタル静的再生成 (ISR) は、`next start`を使用する場合、[セルフホスティングNext.jsサイト](/docs/14/pages/building-your-application/deploying#self-hosting)でそのまま動作します。

[Next.jsのセルフホスティング](/docs/14/pages/building-your-application/deploying#self-hosting)について詳しく学んでください。
