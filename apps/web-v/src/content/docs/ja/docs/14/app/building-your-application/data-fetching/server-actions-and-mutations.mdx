...
sidebar:
  order: 2010202
  label: "サーバーアクションとデータ変更 (Server Actions and Mutations)"
---
...
- サーバーアクションはNext.jsの[キャッシュと再検証](/docs/14/app/building-your-application/caching)アーキテクチャと統合されています。アクションが呼び出されると、Next.jsは更新されたUIと新しいデータを単一のサーバーラウンドトリップで返すことができます。
...
- サーバーアクションは、使用されるページまたはレイアウトから[ランタイム](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)を継承します。
- サーバーアクションは、使用されるページまたはレイアウトから[ルートセグメント設定](/docs/14/app/api-reference/file-conventions/route-segment-config)を継承し、`maxDuration`などのフィールドを含みます。
...
エラーがスローされると、クライアント側で最も近い[`error.js`](/docs/14/app/building-your-application/routing/error-handling)または`<Suspense>`境界でキャッチされます。UIで処理するために`try/catch`を使用してエラーを返すことを推奨します。
...
[`revalidatePath`](/docs/14/app/api-reference/functions/revalidatePath) APIを使用して、サーバーアクション内で[Next.jsキャッシュ](/docs/14/app/building-your-application/caching)を再検証できます:
...
または、キャッシュタグを使用して特定のデータフェッチを無効化するには[`revalidateTag`](/docs/14/app/api-reference/functions/revalidateTag)を使用します:
...
サーバーアクションの完了後にユーザーを別のルートにリダイレクトしたい場合、[`redirect`](/docs/14/app/api-reference/functions/redirect) APIを使用できます。`redirect`は`try/catch`ブロックの外で呼び出す必要があります:
...
[`cookies`](/docs/14/app/api-reference/functions/cookies) APIを使用して、サーバーアクション内でCookieを`取得`、`設定`、`削除`できます:
...
サーバーアクションからのCookie削除の[追加例](/docs/14/app/api-reference/functions/cookies#deleting-cookies)を参照してください。
...
> **知っておくと良いこと:** 機密値がクライアントに公開されるのを防ぐために、暗号化だけに依存することは推奨しません。代わりに、[Reactのtaint API](/docs/14/app/building-your-application/data-fetching/patterns#preventing-sensitive-data-from-being-exposed-to-the-client)を使用して、特定のデータがクライアントに送信されないように積極的に防止する必要があります。
...
リバースプロキシや多層バックエンドアーキテクチャ（サーバーAPIが本番ドメインと異なる）を使用する大規模なアプリケーションでは、[`serverActions.allowedOrigins`](/docs/14/app/api-reference/next-config-js/serverActions)オプションを使用して安全なオリジンのリストを指定することを推奨します。このオプションは文字列の配列を受け取ります。
