...
例えば、[functions API リファレンス](/docs/14/app/api-reference/functions) では、特定の関数を簡単に見つけられるようにページがアルファベット順にソートされています：
...
しかし、[ルーティングセクション](/docs/14/app/building-your-application/routing) では、開発者がこれらの概念を学ぶべき順序でソートするために、ファイルに2桁の数字が付けられています：
...
- 子ページを持つページには自動的にリンクが生成されます。例えば、[最適化](/docs/14/app/building-your-application/optimizing) セクションには、すべての子ページへのリンクがあります。
