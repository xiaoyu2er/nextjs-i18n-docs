...
`headers`関数を使用すると、[サーバーコンポーネント](/docs/14/app/building-your-application/rendering/server-components)からHTTP受信リクエストヘッダーを読み取ることができます。
...
> **知っておくと便利**:
>
> - `headers()`は**[ダイナミック関数](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)**であり、返される値を事前に知ることはできません。レイアウトやページで使用すると、ルートはリクエスト時に**[動的レンダリング](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)**が有効になります。
...
`headers()`は[サスペンスを使ったデータフェッチング](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)と組み合わせて使用できます。
