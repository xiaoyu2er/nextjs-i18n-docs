...
translation-updated-at: 2025-06-06T16:40:46.033Z
title: 環境変数
...
これにより、`process.env.DB_HOST`、`process.env.DB_USER`、`process.env.DB_PASS`がNode.js環境に自動的に読み込まれ、[Next.jsのデータフェッチングメソッド](/docs/14/pages/building-your-application/data-fetching)や[APIルート](/docs/14/pages/building-your-application/routing/api-routes)で使用できるようになります。

例えば、[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)を使用する場合:
...
> これにより、`process.env.DB_HOST`、`process.env.DB_USER`、`process.env.DB_PASS`がNode.js環境に自動的に読み込まれ、[Route Handlers](/docs/14/app/building-your-application/routing/route-handlers)で使用できるようになります。
...
ランタイム環境変数を読み取るには、`getServerSideProps`を使用するか、[App Routerへの段階的移行](/docs/14/app/building-your-application/upgrading/app-router-migration)をお勧めします。App Routerでは、動的レンダリング中にサーバー上で安全に環境変数を読み取ることができます。これにより、異なる値を持つ複数の環境を通じてプロモート可能な単一のDockerイメージを使用できます。
...
- スタンドアロン出力モードでは動作しないため、[runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration)オプションの使用は推奨しません。代わりに[App Routerへの段階的移行](/docs/14/app/building-your-application/upgrading/app-router-migration)をお勧めします。
...
- [`/src`ディレクトリ](/docs/14/app/building-your-application/configuring/src-directory)を使用している場合、`.env.*`ファイルはプロジェクトのルートに残す必要があります。
