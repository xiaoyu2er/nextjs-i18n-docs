...
React 17では新しい[JSX Transform](https://reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html)が導入され、JSX使用時に`import React from 'react'`が不要になりました。React 17ではNext.jsが自動的に新しい変換を使用します。[codemod](/docs/14/pages/building-your-application/upgrading/codemods#add-missing-react-import)を使用して、誤って`React`をインポートせずに使用している箇所を修正できます。
