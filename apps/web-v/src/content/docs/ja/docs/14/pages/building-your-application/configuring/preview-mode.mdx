...
> **注**: この機能は [Draft Mode](/docs/14/pages/building-your-application/configuring/draft-mode) に置き換えられました。
...
[Pages ドキュメント](/docs/14/pages/building-your-application/routing/pages-and-layouts) と [データ取得ドキュメント](/docs/14/pages/building-your-application/data-fetching) では、`getStaticProps` と `getStaticPaths` を使用してビルド時にページを事前レンダリングする方法（**静的生成**）について説明しました。
...
> Next.js のAPIルートに慣れていない場合は、まず [APIルートドキュメント](/docs/14/pages/building-your-application/routing/api-routes) をご覧ください。
...
> **豆知識**: レンダリング中に `next/router` は `isPreview` フラグを公開します。詳細は [routerオブジェクトのドキュメント](/docs/14/pages/api-reference/functions/use-router#router-object) を参照してください。
...
その後、`/api/clear-preview-mode-cookies` にリクエストを送信してAPIルートを呼び出します。[`next/link`](/docs/14/pages/api-reference/components/link) を使用してこのルートを呼び出す場合は、リンクのプリフェッチ中に `clearPreviewData` が呼び出されないように `prefetch={false}` を渡す必要があります。
...
> **豆知識**: プレビューモードを使用する場合は `Cache-Control` ヘッダーを設定しないでください。バイパスできないためです。代わりに [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) の使用を推奨します。
