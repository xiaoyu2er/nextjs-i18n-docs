...
データをリクエスト時に取得する必要がなかったり、データと事前レンダリングされた HTML をキャッシュしたい場合は、[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) の使用を推奨します。
...
- ユーザーが [`next/link`](/docs/14/pages/api-reference/components/link) または [`next/router`](/docs/14/pages/api-reference/functions/use-router) 経由でページにアクセスすると、Next.js はサーバーに API リクエストを送信し、`getServerSideProps` が実行されます
- `getServerSideProps` はサーバー上で実行されるため、データ取得に Next.js の [API Route](/docs/14/pages/building-your-application/routing/api-routes) を呼び出す必要はありません。代わりに、CMS、データベース、その他のサードパーティ API を直接 `getServerSideProps` 内から呼び出せます

> **補足:**
>
> - `getServerSideProps` で使用可能なパラメータと props については、[`getServerSideProps` API リファレンス](/docs/14/pages/api-reference/functions/get-server-side-props)を参照してください
> - クライアントサイドバンドルから Next.js が削除する内容を確認するには、[next-code-elimination ツール](https://next-code-elimination.vercel.app/)を使用できます

## エラー処理

`getServerSideProps` 内でエラーが発生すると、`pages/500.js` ファイルが表示されます。500 ページの作成方法については、[500 ページ](/docs/14/pages/building-your-application/routing/custom-error#500-page)のドキュメントを参照してください。開発中はこのファイルは使用されず、代わりに開発用エラーオーバーレイが表示されます。

## エッジケース

### Edge ランタイム

`getServerSideProps` は [サーバーレスと Edge ランタイム](/docs/14/pages/building-your-application/rendering/edge-and-nodejs-runtimes)の両方で使用でき、どちらでも props を設定できます。
...
ただし、`cache-control` を使用する前に、[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) と [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration) がユースケースに適しているかどうかを検討することを推奨します。