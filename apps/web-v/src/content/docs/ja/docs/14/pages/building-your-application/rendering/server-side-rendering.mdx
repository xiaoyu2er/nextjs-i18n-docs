...
translation-updated-at: 2025-06-06T16:42:32.612Z
...
ご覧の通り、`getServerSideProps` は `getStaticProps` と似ていますが、違いは `getServerSideProps` がビルド時ではなく各リクエストごとに実行される点です。

`getServerSideProps` の動作について詳しく知りたい場合は、[データ取得のドキュメント](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)をご覧ください。