...
> **知っておくと良いこと**:
>
> - カスタムサーバーの使用を決定する前に、Next.jsの統合ルーターがアプリの要件を満たせない場合にのみ使用するようにしてください。カスタムサーバーを使用すると、**サーバーレス関数**や**[自動静的最適化](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)**などの重要なパフォーマンス最適化が失われます。
...
| オプション      | タイプ               | 説明                                                                                                   |
| -------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| `conf`         | `Object`           | [next.config.js](/docs/14/pages/api-reference/next-config-js)で使用するのと同じオブジェクト。デフォルトは`{}` |
...
> また、クライアントサイドルーターを設定して、ファイル名ルートへのクライアントサイドリダイレクトを禁止することもできます。詳細は[`router.beforePopState`](/docs/14/pages/api-reference/functions/use-router#routerbeforepopstate)を参照してください。