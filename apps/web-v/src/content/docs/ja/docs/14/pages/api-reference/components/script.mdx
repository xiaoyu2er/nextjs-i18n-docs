...
このAPIリファレンスでは、Scriptコンポーネントで利用可能な[props](#props)の使用方法を理解するのに役立ちます。機能と使用方法については、[スクリプトの最適化](/docs/14/app/building-your-application/optimizing/scripts)ページをご覧ください。
...
> **警告:** `worker`戦略はまだ安定しておらず、[`app`](/docs/14/app/building-your-application/routing/defining-routes)ディレクトリではまだ動作しません。注意して使用してください。
