...
この機能は**セキュリティを最優先**に設計されています。まず、URLインポートを許可するドメインを明示的に指定する実験的フラグを導入しました。さらに[Edge Runtime](/docs/14/app/api-reference/edge)を使用してブラウザのサンドボックス内でのみ実行されるように制限する取り組みを進めています。
