...
インターセプティングルートは[並列ルート (Parallel Routes)](/docs/14/app/building-your-application/routing/parallel-routes) と組み合わせてモーダルを作成できます。これにより、モーダル実装時の一般的な課題を解決可能です:
...
詳細な手順については[並列ルートのドキュメント](/docs/14/app/building-your-application/routing/parallel-routes#modals)を、実装例については[画像ギャラリーのサンプル](https://github.com/vercel-labs/nextgram)を参照してください。
