...
> - プロジェクトがデフォルトの`loader`で[画像最適化](/docs/14/app/building-your-application/optimizing/images)を使用している場合、依存関係として`sharp`をインストールする必要があります：
...
> **知っておくと良いこと**:
>
> - `next.config.js`は`next build`中に読み込まれ、`server.js`出力ファイルにシリアライズされます。レガシーな[`serverRuntimeConfig`または`publicRuntimeConfig`オプション](/docs/14/pages/api-reference/next-config-js/runtime-configuration)が使用されている場合、値はビルド時の値に固定されます。
> - プロジェクトが特定のポートやホスト名をリッスンする必要がある場合、`server.js`を実行する前に`PORT`または`HOSTNAME`環境変数を定義できます。例えば、`PORT=8080 HOSTNAME=0.0.0.0 node server.js`を実行すると、`http://0.0.0.0:8080`でサーバーが起動します。
> - プロジェクトがデフォルトの`loader`で[画像最適化](/docs/14/pages/building-your-application/optimizing/images)を使用している場合、依存関係として`sharp`をインストールする必要があります：
