...
以前に動的ルートのために `vercel.json` ファイルで `routes` を設定していた場合、Next.js 9の新しい[動的ルーティング機能](/docs/14/pages/building-your-application/routing/dynamic-routes)を利用すればこれらのルールは削除できます。

Next.js 9の動的ルートは[Vercel](https://vercel.com/)上で**自動的に設定**され、`vercel.json` のカスタマイズは不要です。

[動的ルーティングについて詳しく読む](/docs/14/pages/building-your-application/routing/dynamic-routes)

## カスタムAppファイルの確認 (`pages/_app.js`)

以前に[カスタム `<App>`](/docs/14/pages/building-your-application/routing/custom-app)の例をコピーしていた場合、`getInitialProps` を削除できる可能性があります。
