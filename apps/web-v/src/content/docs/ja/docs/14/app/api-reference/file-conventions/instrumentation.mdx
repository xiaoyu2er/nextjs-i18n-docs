...
使用するには、このファイルをアプリケーションの**ルート**ディレクトリ、または [`src` フォルダ](/docs/14/app/building-your-application/configuring/src-directory) を使用している場合はその中に配置します。

## 設定オプション

インストルメンテーションは現在実験的な機能です。`instrumentation` ファイルを使用するには、`next.config.js` で [`experimental.instrumentationHook = true;`](/docs/14/app/api-reference/next-config-js/instrumentationHook) を明示的に設定する必要があります:
