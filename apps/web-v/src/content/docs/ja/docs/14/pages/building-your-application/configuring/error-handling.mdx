...
Next.jsはデフォルトで静的500ページを提供し、アプリケーションで発生するサーバーサイドエラーを処理します。また、`pages/500.js` ファイルを作成することで[このページをカスタマイズ](/docs/14/pages/building-your-application/routing/custom-error#customizing-the-500-page)できます。

500ページがある場合でも、アプリユーザーには具体的なエラー内容は表示されません。

`file not found` などの特定のランタイムエラーを処理するには、[404ページ](/docs/14/pages/building-your-application/routing/custom-error#404-page)も使用できます。
