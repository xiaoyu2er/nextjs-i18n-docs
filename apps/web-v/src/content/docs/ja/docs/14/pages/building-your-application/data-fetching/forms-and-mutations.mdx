...
> **知っておくと良いこと:**
>
> - 近く、App Routerへの[段階的な移行](/docs/14/app/building-your-application/upgrading/app-router-migration)と[Server Actions](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)の使用を推奨する予定です。Server Actionsを使用すると、API Routeを手動で作成せずに、コンポーネントから直接呼び出せる非同期サーバー関数を定義できます。
> - API Routesは[デフォルトでCORSヘッダーを指定しません](https://developer.mozilla.org/docs/Web/HTTP/CORS)。つまり、同一オリジンに限定されます。
> - API Routesはサーバー上で実行されるため、[環境変数](/docs/14/pages/building-your-application/configuring/environment-variables)を通じてAPIキーなどの機密情報をクライアントに公開せずに使用できます。これはアプリケーションのセキュリティにとって重要です。
...
ミューテーション後にユーザーを別のルートにリダイレクトしたい場合、[`redirect`](/docs/14/pages/building-your-application/routing/api-routes#response-helpers)を使用して絶対URLまたは相対URLにリダイレクトできます:
...
API Route内で[`cookies`](/docs/14/pages/building-your-application/routing/api-routes#request-helpers)リクエストヘルパーを使用してCookieを読み取れます:
