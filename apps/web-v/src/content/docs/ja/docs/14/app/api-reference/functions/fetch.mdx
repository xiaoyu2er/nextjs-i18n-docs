...
リクエストが Next.js の[データキャッシュ (Data Cache)](/docs/14/app/building-your-application/caching#data-cache) とどのように相互作用するかを設定します。
...
> **豆知識**:
>
> - `cache` オプションを指定しない場合、Next.js はデフォルトで `force-cache` になりますが、`cookies()` などの[動的関数 (dynamic function)](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) が使用されている場合は `no-store` がデフォルトになります。
...
> **豆知識**:
>
> - 個々の `fetch()` リクエストで設定された `revalidate` の数値がルートの[デフォルトの `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate) よりも小さい場合、ルート全体の再検証間隔が短縮されます。
