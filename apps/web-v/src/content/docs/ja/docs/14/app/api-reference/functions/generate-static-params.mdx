...
`generateStaticParams` 関数は、[動的ルートセグメント](/docs/14/app/building-your-application/routing/dynamic-routes) と組み合わせて使用することで、リクエスト時にオンデマンドで生成する代わりに、ビルド時にルートを[**静的生成**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)できます。
...
> **知っておくと便利**
>
> - [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) セグメント設定オプションを使用して、`generateStaticParams`で生成されなかった動的セグメントにアクセスした時の挙動を制御できます。
...
> - `generateStaticParams` は Pages Router の [`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) 関数を置き換えます。
...
> **知っておくと便利**: `fetch` リクエストは、`generate` プレフィックスが付いた関数、レイアウト、ページ、サーバーコンポーネント間で同じデータに対して自動的に[メモ化](/docs/14/app/building-your-application/caching#request-memoization)されます。`fetch` が利用できない場合、React の [`cache` を使用できます](/docs/14/app/building-your-application/caching#request-memoization)。

### paramsのサブセットのみを生成

生成したい動的セグメントのみを含むオブジェクトの配列を返すことで、ルートの params のサブセットのみを生成できます。その後、[`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) セグメント設定オプションを使用して、`generateStaticParams` で生成されなかった動的セグメントにアクセスした時の挙動を制御できます。
