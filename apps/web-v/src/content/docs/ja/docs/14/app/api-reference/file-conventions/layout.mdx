...
レイアウトコンポーネントは `children` prop を受け取り、使用する必要があります。レンダリング時、`children` にはレイアウトがラップしているルートセグメントが含まれます。これには主に子の [レイアウト](/docs/14/app/building-your-application/routing/pages-and-layouts#pages) コンポーネント（存在する場合）や [ページ](/docs/14/app/building-your-application/routing/pages-and-layouts#pages) コンポーネントが含まれますが、状況に応じて [ローディング](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) や [エラー](/docs/14/app/building-your-application/routing/error-handling) などの特別なファイルも含まれる可能性があります。

### `params` (オプション)

ルートセグメントからそのレイアウトまでの [動的ルートパラメータ](/docs/14/app/building-your-application/routing/dynamic-routes) オブジェクトです。
...
[ページ](/docs/14/app/api-reference/file-conventions/page) とは異なり、レイアウトコンポーネントは `searchParams` prop を受け取りません。これは、共有レイアウトが [ナビゲーション中に再レンダリングされない](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering) ためで、ナビゲーション間で `searchParams` が古くなる可能性があるからです。
...
- 代わりに、ページの [`searchParams`](/docs/14/app/api-reference/file-conventions/page#searchparams-optional) prop を使用するか、クライアントコンポーネント内で [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params) フックを使用してください。これらは最新の `searchParams` でクライアント側で再レンダリングされます。

### ルートレイアウト

- `app` ディレクトリには必ずルート `app/layout.js` を含める必要があります。
- ルートレイアウトには必ず `<html>` タグと `<body>` タグを定義する必要があります。
  - ルートレイアウトに `<title>` や `<meta>` などの `<head>` タグを手動で追加するべきではありません。代わりに、ストリーミングや `<head>` 要素の重複排除などの高度な要件を自動的に処理する [メタデータ API](/docs/14/app/api-reference/functions/generate-metadata) を使用してください。
- [ルートグループ](/docs/14/app/building-your-application/routing/route-groups) を使用して複数のルートレイアウトを作成できます。
