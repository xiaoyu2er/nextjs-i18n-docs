...
> **豆知識**: ビルド時にデータを取得する必要がある場合、このページ内で[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)を使用できます。
...
- `Error`コンポーネントは現在、[`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)や[`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)などのNext.js[データ取得方法](/docs/14/pages/building-your-application/data-fetching)をサポートしていません。
- `_error`は`_app`と同様に予約されたパス名です。`_error`はエラーページのカスタムレイアウトと動作を定義するために使用されます。[ルーティング](/docs/14/pages/building-your-application/routing)経由で直接アクセスしたり、[カスタムサーバー](/docs/14/pages/building-your-application/configuring/custom-server)でレンダリングすると、`/_error`は404を表示します。