...
デフォルトでは、サーバーコンポーネントは自動的に[コード分割](https://developer.mozilla.org/docs/Glossary/Code_splitting)され、[ストリーミング](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)を使用してサーバーからクライアントへUIを段階的に送信できます。遅延読み込みはクライアントコンポーネントに適用されます。
