...
このCodemodは、[動的OG画像生成](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation)で使用するインポートを `next/server` から `next/og` に移動します。
...
[Linkコンポーネント](/docs/14/app/api-reference/components/link)内の `<a>` タグを削除、または自動修正できないLinkに `legacyBehavior` プロパティを追加します。

</AppOnly>

<PagesOnly>

[Linkコンポーネント](/docs/14/pages/api-reference/components/link)内の `<a>` タグを削除、または自動修正できないLinkに `legacyBehavior` プロパティを追加します。
