...
translation-updated-at: 2025-06-06T16:40:54.079Z
title: ESLint
...
> `next build` 時に ESLint を実行したくない場合は、[ESLint の無視](/docs/14/app/api-reference/next-config-js/eslint) のドキュメントを参照してください。

</AppOnly>

<PagesOnly>

> `next build` 時に ESLint を実行したくない場合は、[ESLint の無視](/docs/14/pages/api-reference/next-config-js/eslint) のドキュメントを参照してください。
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | [Next.js のデータフェッチ関数](/docs/14/pages/building-your-application/data-fetching) での一般的なタイポを防止します |
...
パフォーマンス向上のため、ESLint によって処理されたファイルの情報はデフォルトでキャッシュされます。これは `.next/cache` または定義した [ビルドディレクトリ](/docs/14/app/api-reference/next-config-js/distDir) に保存されます。単一のソースファイルの内容以上のものに依存する ESLint ルールを含めていてキャッシュを無効にする必要がある場合は、`next lint` で `--no-cache` フラグを使用してください。

</AppOnly>

<PagesOnly>

パフォーマンス向上のため、ESLint によって処理されたファイルの情報はデフォルトでキャッシュされます。これは `.next/cache` または定義した [ビルドディレクトリ](/docs/14/pages/api-reference/next-config-js/distDir) に保存されます。単一のソースファイルの内容以上のものに依存する ESLint ルールを含めていてキャッシュを無効にする必要がある場合は、`next lint` で `--no-cache` フラグを使用してください。
...
> `next/core-web-vitals` エントリポイントは、[Create Next App](/docs/14/app/api-reference/cli/create-next-app) で構築された新しいアプリケーションに自動的に含まれます。
...
- 以下のプラグインのいずれかが既にインストールされている場合（個別または`airbnb`、`react-app`などの別の設定経由）:
  ...
  - `jsx-a11y`
  - `import`
- Next.js内でBabelが設定されている方法とは異なる特定の`parserOptions`を定義している場合（[Babel設定をカスタマイズ](/docs/14/pages/building-your-application/configuring/babel)していない限り推奨されません）
