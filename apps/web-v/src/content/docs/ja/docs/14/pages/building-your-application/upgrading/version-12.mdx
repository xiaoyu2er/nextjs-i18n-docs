...
Next.jsは[カスタムBabel設定](/docs/14/pages/building-your-application/configuring/babel)を持つアプリケーションとの完全な後方互換性を提供します。styled-jsxや`getStaticProps`/`getStaticPaths`/`getServerSideProps`のツリーシェイキングなど、Next.jsがデフォルトで処理するすべての変換がRustに移植されています。
...
詳細は[ドキュメント](/docs/14/pages/building-your-application/optimizing/images#styling)を参照してください。
...
現在`target`オプションを`serverless`に設定している場合は、[新しい出力の活用方法に関するドキュメント](/docs/14/pages/api-reference/next-config-js/output)を参照してください。