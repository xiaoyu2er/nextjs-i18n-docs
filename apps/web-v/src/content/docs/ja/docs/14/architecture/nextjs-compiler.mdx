...
このオプションはNext.js 13.5で[`optimizePackageImports`](/docs/14/app/api-reference/next-config-js/optimizePackageImports)に置き換えられました。手動でインポートパスを設定する必要がない新しいオプションを使用することをお勧めします。
