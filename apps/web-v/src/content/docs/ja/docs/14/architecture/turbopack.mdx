...
Next.js における Turbopack は、ほとんどのユーザーにとって設定不要で、より高度なユースケースにも拡張可能です。Turbopack で現在サポートされている機能の詳細については、[API リファレンス](/docs/14/app/api-reference/next-config-js/turbo)を参照してください。
...
- `next.config.js` における [`webpack()`](/docs/14/app/api-reference/next-config-js/webpack) 設定
  - Turbopack は Webpack を置き換えるため、webpack 設定はサポートされません。
  - Turbopack を設定するには、[ドキュメント](/docs/14/app/api-reference/next-config-js/turbo)を参照してください。
  - [Webpack ローダー](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders)の一部は Turbopack でサポートされています。
- Babel (`.babelrc`)
  ...
  - 特定のユースケースがカバーされていないことを確認した上で Babel を使用する必要がある場合は、Turbopack の [カスタム webpack ローダーサポート](/docs/14/app/api-reference/next-config-js/turbo#webpack-loaders)を利用して `babel-loader` を含めることができます。
- App Router でのルートレイアウトの自動生成
  - この動作は現在サポートされておらず、代わりに手動でルートレイアウトを追加するようエラーが表示されます。
- `@next/font` (レガシーフォントサポート)
  - `@next/font` は非推奨となり、`next/font` が推奨されます。[`next/font`](/docs/14/app/building-your-application/optimizing/fonts) は Turbopack で完全にサポートされています。
