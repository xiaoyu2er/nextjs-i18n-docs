...
`generateStaticParams` 函數可與[動態路由區段](/docs/14/app/building-your-application/routing/dynamic-routes)搭配使用，用於在建置時[**靜態生成**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)路由，而非在請求時按需生成。
...
> **須知事項**
>
> - 您可以使用 [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) 區段配置選項，來控制當訪問未由 `generateStaticParams` 生成的動態區段時會發生什麼。
...
> - `generateStaticParams` 取代了 Pages Router 中的 [`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) 函數。
...
> **須知事項**：在所有以 `generate` 為前綴的函數、Layouts、Pages 和 Server Components 中，`fetch` 請求會自動[記憶化](/docs/14/app/building-your-application/caching#request-memoization)。如果無法使用 `fetch`，可以使用 React 的 [`cache`](/docs/14/app/building-your-application/caching#request-memoization)。

### 僅生成部分 params

您可以透過回傳僅包含您想生成的動態區段的物件陣列，來為路由生成部分 params。然後，透過使用 [`dynamicParams`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamicparams) 區段配置選項，您可以控制當訪問未由 `generateStaticParams` 生成的動態區段時會發生什麼。
