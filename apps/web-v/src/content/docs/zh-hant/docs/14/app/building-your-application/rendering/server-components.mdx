...
Next.js 預設使用伺服器元件 (Server Components)。這讓您無需額外配置即可自動實現伺服器端渲染，並可在需要時選擇使用客戶端元件 (Client Components)，詳見[客戶端元件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components)。
...
使用靜態渲染時，路由會在**建置時**或在[資料重新驗證 (data revalidation)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) 後於背景渲染。結果會被快取並可推送至[內容傳遞網路 (CDN)](https://developer.mozilla.org/docs/Glossary/CDN)。這項優化讓您能在使用者和伺服器請求之間共享渲染工作的結果。
...
>
> 了解更多關於[全路由快取 (full-route cache)](/docs/14/app/building-your-application/caching#full-route-cache) 和[資料快取 (Data Cache)](/docs/14/app/building-your-application/caching#data-cache)。

#### 切換至動態渲染

在渲染過程中，如果發現[動態函式 (dynamic functions)](#dynamic-functions) 或[未快取的資料請求](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#opting-out-of-data-caching)，Next.js 會切換為動態渲染整個路由。下表總結了動態函式和資料快取如何影響路由是靜態還是動態渲染：
...
作為開發者，您無需在靜態和動態渲染之間選擇，因為 Next.js 會根據使用的功能和 API 自動為每個路由選擇最佳渲染策略。相反，您選擇何時[快取或重新驗證特定資料](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)，並可選擇[串流 (Streaming)](#streaming) 部分 UI。

#### 動態函式

動態函式依賴僅能在請求時確定的資訊，例如使用者的 cookies、當前請求標頭或 URL 的搜尋參數 (search params)。在 Next.js 中，這些動態函式包括：

- **[`cookies()`](/docs/14/app/api-reference/functions/cookies) 和 [`headers()`](/docs/14/app/api-reference/functions/headers)**：在伺服器元件 (Server Components) 中使用這些函式會讓整個路由在請求時切換為動態渲染。
- **[`searchParams`](/docs/14/app/api-reference/file-conventions/page#searchparams-optional)**：在[頁面 (Page)](/docs/14/app/api-reference/file-conventions/page) 上使用 `searchParams` prop 會讓該頁面在請求時切換為動態渲染。
...
您可以使用 `loading.js` 和 [React Suspense](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) 的 UI 元件來開始串流路由區段。詳見[載入 UI 與串流 (Loading UI and Streaming)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) 章節以獲取更多資訊。