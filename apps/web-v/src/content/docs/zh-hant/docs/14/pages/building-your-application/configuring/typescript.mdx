...
- 警告是否傳遞了 [區段配置選項](/docs/14/app/api-reference/file-conventions/route-segment-config) 的無效值。
- 顯示可用的選項和上下文文件。
...
2. **元件之間的簡化資料流**：由於移除了 `_app` 而改用根佈局，現在更容易可視化元件和頁面之間的資料流。以前，個別 `pages` 和 `_app` 之間流動的資料難以鍵入，並且可能引入令人困惑的錯誤。透過應用程式路由中的 [同位置資料獲取](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)，這不再是問題。

[Next.js 中的資料獲取](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) 現在提供了盡可能接近端到端的類型安全性，而無需對您的資料庫或內容提供者選擇進行規範。
...
對於 [`getStaticProps`](/docs/14/pages/api-reference/functions/get-static-props)、[`getStaticPaths`](/docs/14/pages/api-reference/functions/get-static-paths) 和 [`getServerSideProps`](/docs/14/pages/api-reference/functions/get-server-side-props)，您可以分別使用 `GetStaticProps`、`GetStaticPaths` 和 `GetServerSideProps` 類型：
...
如果您有 [自訂 `App`](/docs/14/pages/building-your-application/routing/custom-app)，您可以使用內建類型 `AppProps` 並將檔案名稱更改為 `./pages/_app.tsx`，如下所示：
...
您可以在 [模組路徑別名文件](/docs/14/app/building-your-application/configuring/absolute-imports-and-module-aliases) 上了解更多關於此功能的資訊。

</AppOnly>

<PagesOnly>

您可以在 [模組路徑別名文件](/docs/14/pages/building-your-application/configuring/absolute-imports-and-module-aliases) 上了解更多關於此功能的資訊。
