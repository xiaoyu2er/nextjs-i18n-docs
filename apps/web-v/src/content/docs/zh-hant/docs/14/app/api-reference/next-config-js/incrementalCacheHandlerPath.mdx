...
在 Next.js 中，[預設快取處理器](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) 用於 Pages 和 App Router 時使用檔案系統快取。這不需要任何設定，但你可以透過在 `next.config.js` 中使用 `cacheHandler` 欄位來自訂快取處理器。
...
查看 [自訂快取處理器](/docs/14/app/building-your-application/deploying#configuring-caching) 的範例並了解更多實作細節。
...
回傳 `Promise<void>`。了解更多關於 [重新驗證資料](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating) 或 [`revalidateTag()`](/docs/14/app/api-reference/functions/revalidateTag) 函數。
