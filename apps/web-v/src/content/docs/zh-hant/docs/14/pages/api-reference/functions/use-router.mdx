...
- `query`: `Object` - 解析為物件的查詢字串，包含[動態路由](/docs/14/pages/building-your-application/routing/dynamic-routes)參數。如果頁面未使用[伺服器端渲染 (SSR)](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)，預渲染期間將為空物件。預設為 `{}`
- `asPath`: `String` - 瀏覽器中顯示的路徑，包含搜尋參數並遵循 `trailingSlash` 配置。不包含 `basePath` 和 `locale`。
- `isFallback`: `boolean` - 當前頁面是否處於[後備模式 (fallback mode)](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)。
- `basePath`: `String` - 當前啟用的 [basePath](/docs/14/app/api-reference/next-config-js/basePath)（如果已啟用）。
...
- `isReady`: `boolean` - 路由器欄位是否已在客戶端更新並可供使用。應僅在 `useEffect` 方法中使用，不用於伺服器端的條件渲染。相關使用案例請參閱[自動靜態優化頁面](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)的文件。
- `isPreview`: `boolean` - 應用程式當前是否處於[預覽模式 (preview mode)](/docs/14/pages/building-your-application/configuring/preview-mode)。

> 如果頁面使用伺服器端渲染或[自動靜態優化](/docs/14/pages/building-your-application/rendering/automatic-static-optimization)，使用 `asPath` 欄位可能導致客戶端與伺服器不一致。在 `isReady` 欄位為 `true` 之前，避免使用 `asPath`。

`router` 中包含以下方法：

### router.push

處理客戶端轉換，此方法適用於 [`next/link`](/docs/14/pages/api-reference/components/link) 不足的情況。
...
- `options` - 可選物件，包含以下配置選項：
  ...
  - [`shallow`](/docs/14/pages/building-your-application/routing/linking-and-navigating#shallow-routing): 更新當前頁面的路徑而不重新執行 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)、[`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 或 [`getInitialProps`](/docs/14/pages/api-reference/functions/get-initial-props)。預設為 `false`
...
將用戶重定向到 `pages/login.js`，適用於[身份驗證](/docs/14/pages/building-your-application/authentication)後的頁面：
...
您可以像在 [`next/link`](/docs/14/pages/api-reference/components/link#with-url-object) 中一樣使用 URL 物件。適用於 `url` 和 `as` 參數：
...
類似於 [`next/link`](/docs/14/pages/api-reference/components/link) 中的 `replace` 屬性，`router.replace` 會阻止將新的 URL 條目添加到 `history` 堆疊中。
...
預取頁面以實現更快的客戶端轉換。此方法僅適用於沒有 [`next/link`](/docs/14/pages/api-reference/components/link) 的導航，因為 `next/link` 會自動處理頁面預取。
...
在某些情況下（例如使用[自訂伺服器 (Custom Server)](/docs/14/pages/building-your-application/configuring/custom-server)），您可能希望監聽 [popstate](https://developer.mozilla.org/docs/Web/Events/popstate) 並在路由器處理之前執行某些操作。
...
如果 `cb` 返回 `false`，Next.js 路由器將不會處理 `popstate`，您將負責在這種情況下處理它。請參閱[禁用文件系統路由](/docs/14/pages/building-your-application/configuring/custom-server#disabling-file-system-routing)。
...
> **重要提示**：這裡的 `url` 是瀏覽器中顯示的 URL，包含 [`basePath`](/docs/14/app/api-reference/next-config-js/basePath)。
...
> 我們在此範例中使用[自訂應用程式 (Custom App)](/docs/14/pages/building-your-application/routing/custom-app) (`pages/_app.js`) 來訂閱事件，因為它在頁面導航時不會卸載，但您可以在應用程式的任何元件中訂閱路由器事件。
