...
[中介軟體](/docs/14/app/building-your-application/routing/middleware) 讓您可以在頁面渲染前新增標頭並產生 nonce。
...
預設情況下，中介軟體會在所有請求上執行。您可以使用 [`matcher`](/docs/14/app/building-your-application/routing/middleware#matcher) 來篩選中介軟體，使其僅在特定路徑上執行。
...
現在您可以從 [伺服器元件 (Server Component)](/docs/14/app/building-your-application/rendering/server-components) 使用 [`headers`](/docs/14/app/api-reference/functions/headers) 讀取 nonce：
...
對於不需要 nonce 的應用程式，您可以直接在 [`next.config.js`](/docs/14/app/api-reference/next-config-js) 檔案中設定 CSP 標頭：
