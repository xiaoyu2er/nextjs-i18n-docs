...
- 自動模擬樣式表 (`.css`、`.module.css` 及其 scss 變體)、圖片匯入和 [`next/font`](/docs/14/pages/building-your-application/optimizing/fonts)
- 將 `.env` (及其所有變體) 載入 `process.env`
- 從測試解析和轉換中忽略 `node_modules`
- 從測試解析中忽略 `.next`
- 載入 `next.config.js` 以啟用 SWC 轉換的標誌

> **須知事項：** 若要直接測試環境變數，請在獨立的設定腳本或 `jest.config.ts` 檔案中手動載入它們。更多資訊請參閱 [測試環境變數](/docs/14/pages/building-your-application/configuring/environment-variables#test-environment-variables)。
...
如果你的專案使用 [模組路徑別名](/docs/14/pages/building-your-application/configuring/absolute-imports-and-module-aliases)，你需要配置 Jest 來解析這些匯入，方法是將 `jsconfig.json` 檔案中的 `paths` 選項與 `jest.config.js` 檔案中的 `moduleNameMapper` 選項匹配。例如：
