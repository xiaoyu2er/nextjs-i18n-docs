...
如果您先前在 `vercel.json` 檔案中為動態路由配置了 `routes`，當使用 Next.js 9 的新功能[動態路由 (Dynamic Routing)](/docs/14/pages/building-your-application/routing/dynamic-routes) 時，這些規則可以移除。

Next.js 9 的動態路由**在 [Vercel](https://vercel.com/) 上會自動配置**，無需任何 `vercel.json` 自訂設定。

您可以閱讀更多關於[動態路由的資訊](/docs/14/pages/building-your-application/routing/dynamic-routes)。

## 檢查您的自訂 App 檔案 (`pages/_app.js`)

如果您先前複製了[自訂 `<App>`](/docs/14/pages/building-your-application/routing/custom-app) 範例，您或許可以移除 `getInitialProps`。
