...
1. `rewrite` 到產生回應的路由 ([頁面](/docs/14/app/building-your-application/routing/pages-and-layouts) 或 [路由處理程式](/docs/14/app/building-your-application/routing/route-handlers))
...
1. `rewrite` 到產生回應的路由 ([頁面](/docs/14/pages/building-your-application/routing/pages-and-layouts) 或 [Edge API 路由](/docs/14/pages/building-your-application/routing/api-routes))
2. 直接回傳 `NextResponse`。請參閱 [產生回應](#producing-a-response)
...
> **小知識：** 您可以在[路由處理程式 (Route Handlers)](/docs/14/app/building-your-application/routing/route-handlers#cors) 中為個別路由設定 CORS 標頭。
...
中介軟體目前僅支援 [Edge 執行環境](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)，無法使用 Node.js 執行環境。
