...
- 在相同路由區段層級啟用 [巢狀佈局 (nested layouts)](/docs/14/app/building-your-application/routing/pages-and-layouts)：
...
若要建立多個 [根佈局 (root layouts)](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required)，請移除頂層的 `layout.js` 檔案，並在每個路由群組中新增 `layout.js` 檔案。這適用於將應用程式分割成具有完全不同 UI 或體驗的區塊。每個根佈局都需要加入 `<html>` 和 `<body>` 標籤。
