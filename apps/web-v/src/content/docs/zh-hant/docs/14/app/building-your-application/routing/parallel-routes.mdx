...
然而，插槽**不是** [路由區段 (route segments)](/docs/14/app/building-your-application/routing#route-segments)，也不會影響 URL 結構。例如，對於 `/@analytics/views`，URL 會是 `/views`，因為 `@analytics` 是一個插槽。
...
- [**軟導航 (Soft Navigation)**](/docs/14/app/building-your-application/routing/linking-and-navigating#5-soft-navigation)：在客戶端導航期間，Next.js 會執行 [部分渲染 (partial render)](/docs/14/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)，變更插槽內的子頁面，同時保持其他插槽的活動子頁面，即使它們與當前 URL 不匹配。
...
[`useSelectedLayoutSegment`](/docs/14/app/api-reference/functions/use-selected-layout-segment) 和 [`useSelectedLayoutSegments`](/docs/14/app/api-reference/functions/use-selected-layout-segments) 都接受一個 `parallelRoutesKey` 參數，允許您讀取插槽內的活動路由區段。
...
在 `@analytics` 內，建立一個 [`layout`](/docs/14/app/building-your-application/routing/pages-and-layouts) 檔案，以在兩個頁面之間共享標籤：
...
平行路由可以與 [攔截路由 (Intercepting Routes)](/docs/14/app/building-your-application/routing/intercepting-routes) 一起使用來建立模態框。這讓您可以解決建立模態框時的常見挑戰，例如：
...
然後，在 `@auth` 插槽內，新增一個 [`default.js`](/docs/14/app/api-reference/file-conventions/default) 檔案，返回 `null`。這確保了模態框在不活動時不會被渲染。
...
> **須知：**
>
> - 用於攔截路由的慣例 (例如 `(.)`) 取決於您的檔案系統結構。請參閱 [攔截路由慣例](/docs/14/app/building-your-application/routing/intercepting-routes#convention)。
> - 透過將 `<Modal>` 功能與模態框內容 (`<Login>`) 分開，您可以確保模態框內的任何內容 (例如 [表單](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations#forms)) 都是伺服器元件。請參閱 [交錯客戶端和伺服器元件](/docs/14/app/building-your-application/rendering/composition-patterns#supported-pattern-passing-server-components-to-client-components-as-props) 以獲取更多資訊。
...
請參閱 [載入 UI](/docs/14/app/building-your-application/routing/loading-ui-and-streaming) 和 [錯誤處理](/docs/14/app/building-your-application/routing/error-handling) 文件以獲取更多資訊。