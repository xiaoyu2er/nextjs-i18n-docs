...
您可以使用 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) 和 [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths) 為 `pages` 目錄中的每個頁面生成 HTML 檔案（或為[動態路由](/docs/14/app/building-your-application/routing/dynamic-routes)生成更多檔案）。
...
- [使用 `getStaticPaths` 的動態路由](/docs/14/app/building-your-application/routing/dynamic-routes)
- 使用 `next/link` 進行預取
- 預載入 JavaScript
- [動態匯入](/docs/14/pages/building-your-application/optimizing/lazy-loading)
- 任何樣式選項（例如 CSS 模組、styled-jsx）
- [客戶端資料獲取](/docs/14/pages/building-your-application/data-fetching/client-side)
...
透過在 `next.config.js` 中定義自訂圖片載入器，可以在靜態匯出中使用 `next/image` 的[圖片優化](/docs/14/app/building-your-application/optimizing/images)功能。例如，您可以使用 Cloudinary 等服務優化圖片：
...
- 使用 `dynamicParams: true` 的[動態路由](/docs/14/app/building-your-application/routing/dynamic-routes)
- 沒有 `generateStaticParams()` 的[動態路由](/docs/14/app/building-your-application/routing/dynamic-routes)
- 依賴 Request 的[路由處理器](/docs/14/app/building-your-application/routing/route-handlers)
...
- [中介軟體 (Middleware)](/docs/14/app/building-your-application/routing/middleware)
- [增量靜態再生 (ISR)](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating)
- 使用預設 `loader` 的[圖片優化](/docs/14/app/building-your-application/optimizing/images)
- [草稿模式 (Draft Mode)](/docs/14/app/building-your-application/configuring/draft-mode)

嘗試在 `next dev` 中使用這些功能會導致錯誤，類似於在根佈局中將 [`dynamic`](/docs/14/app/api-reference/file-conventions/route-segment-config#dynamic) 選項設為 `error`。
...
- [國際化路由](/docs/14/pages/building-your-application/routing/internationalization)
...
- [中介軟體 (Middleware)](/docs/14/pages/building-your-application/routing/middleware)
- [增量靜態再生 (ISR)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration)
- 使用預設 `loader` 的[圖片優化](/docs/14/pages/building-your-application/optimizing/images)
- [草稿模式 (Draft Mode)](/docs/14/pages/building-your-application/configuring/draft-mode)
