...
translation-updated-at: 2025-05-25T21:03:30.445Z
title: rewrites
...
Rewrites 作為 URL 代理並遮罩目標路徑，使使用者看起來像是沒有改變他們在網站上的位置。相比之下，[redirects](/docs/14/app/api-reference/next-config-js/redirects) 會重新路由到新頁面並顯示 URL 變更。

</AppOnly>

<PagesOnly>

Rewrites 作為 URL 代理並遮罩目標路徑，使使用者看起來像是沒有改變他們在網站上的位置。相比之下，[redirects](/docs/14/pages/api-reference/next-config-js/redirects) 會重新路由到新頁面並顯示 URL 變更。
...
1. 檢查/應用 [headers](/docs/14/app/api-reference/next-config-js/headers)
2. 檢查/應用 [redirects](/docs/14/app/api-reference/next-config-js/redirects)
3. 檢查/應用 `beforeFiles` rewrites
4. 檢查/提供來自 [public 目錄](/docs/14/app/building-your-application/optimizing/static-assets)、`_next/static` 檔案和非動態頁面的靜態檔案
5. 檢查/應用 `afterFiles` rewrites，如果其中一個 rewrites 匹配，我們會在每次匹配後檢查動態路由/靜態檔案
6. 檢查/應用 `fallback` rewrites，這些會在渲染 404 頁面之前和檢查所有動態路由/靜態資源之後應用。如果您在 `getStaticPaths` 中使用 [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)，則 `next.config.js` 中定義的 `fallback` rewrites _不會_ 執行。

</AppOnly>

<PagesOnly>

1. 檢查/應用 [headers](/docs/14/pages/api-reference/next-config-js/headers)
2. 檢查/應用 [redirects](/docs/14/pages/api-reference/next-config-js/redirects)
3. 檢查/應用 `beforeFiles` rewrites
4. 檢查/提供來自 [public 目錄](/docs/14/pages/building-your-application/optimizing/static-assets)、`_next/static` 檔案和非動態頁面的靜態檔案
5. 檢查/應用 `afterFiles` rewrites，如果其中一個 rewrites 匹配，我們會在每次匹配後檢查動態路由/靜態檔案
6. 檢查/應用 `fallback` rewrites，這些會在渲染 404 頁面之前和檢查所有動態路由/靜態資源之後應用。如果您在 `getStaticPaths` 中使用 [fallback: true/'blocking'](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)，則 `next.config.js` 中定義的 `fallback` rewrites _不會_ 執行。
...
> **須知**：來自 [Automatic Static Optimization](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) 或 [prerendering](/docs/14/pages/building-your-application/data-fetching/get-static-props) 的靜態頁面的 rewrite 參數將在 hydration 後在客戶端解析並提供在查詢中。
...
當與 rewrites 一起使用 [`basePath` 支援](/docs/14/app/api-reference/next-config-js/basePath) 時，每個 `source` 和 `destination` 會自動加上 `basePath` 前綴，除非您在 rewrite 中添加 `basePath: false`：
...
當與 rewrites 一起使用 [`i18n` 支援](/docs/14/app/building-your-application/routing/internationalization) 時，每個 `source` 和 `destination` 會自動加上前綴以處理配置的 `locales`，除非您在 rewrite 中添加 `locale: false`。如果使用 `locale: false`，您必須在 `source` 和 `destination` 前加上 locale 才能正確匹配。

</AppOnly>

<PagesOnly>

當與 rewrites 一起使用 [`i18n` 支援](/docs/14/pages/building-your-application/routing/internationalization) 時，每個 `source` 和 `destination` 會自動加上前綴以處理配置的 `locales`，除非您在 rewrite 中添加 `locale: false`。如果使用 `locale: false`，您必須在 `source` 和 `destination` 前加上 locale 才能正確匹配。
