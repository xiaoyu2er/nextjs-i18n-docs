...
- `props` - 包含當前路由參數的物件：
  ...
  - `params` - 包含從根區段到呼叫 `generateMetadata` 區段的 [動態路由參數](/docs/14/app/building-your-application/routing/dynamic-routes) 物件。範例：
...
> - `fetch` 請求會自動在 `generateMetadata`、`generateStaticParams`、Layouts、Pages 和 Server Components 之間 [記憶化 (memoized)](/docs/14/app/building-your-application/caching#request-memoization)。若無法使用 `fetch`，可使用 React [`cache`](/docs/14/app/building-your-application/caching#request-memoization)。
> - `searchParams` 僅在 `page.js` 區段中可用。
> - Next.js 方法 [`redirect()`](/docs/14/app/api-reference/functions/redirect) 和 [`notFound()`](/docs/14/app/api-reference/functions/not-found) 也可在 `generateMetadata` 中使用。
...
> **小知識**:
>
> - 對於 Open Graph 圖片，使用[基於檔案的 Metadata API](/docs/14/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif) 可能更方便。無需手動同步配置匯出與實際檔案，基於檔案的 API 會自動為您生成正確的中繼資料。
...
> **小知識**：我們建議盡可能使用[基於檔案的 Metadata API](/docs/14/app/api-reference/file-conventions/metadata/app-icons#image-files-ico-jpg-png) 來設定圖示。無需手動同步配置匯出與實際檔案，基於檔案的 API 會自動為您生成正確的中繼資料。
...
> **已棄用**：`metadata` 中的 `themeColor` 選項自 Next.js 14 起已棄用。請改用 [`viewport` 配置](/docs/14/app/api-reference/functions/generate-viewport)。
...
> **已棄用**：`metadata` 中的 `viewport` 選項自 Next.js 14 起已棄用。請改用 [`viewport` 配置](/docs/14/app/api-reference/functions/generate-viewport)。
...
| 中繼資料                      | 建議作法                                                                                                                                                                                                                                     |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | 透過 [`redirect()`](/docs/14/app/api-reference/functions/redirect)、[中介軟體 (Middleware)](/docs/14/app/building-your-application/routing/middleware#nextresponse) 或 [安全標頭 (Security Headers)](/docs/14/app/api-reference/next-config-js/headers) 設定適當的 HTTP 標頭 |
| `<base>`                      | 直接在版面配置或頁面中渲染此標籤。                                                                                                                                                                                                       |
| `<noscript>`                  | 直接在版面配置或頁面中渲染此標籤。                                                                                                                                                                                                       |
| `<style>`                     | 深入了解 [Next.js 中的樣式設定](/docs/14/app/building-your-application/styling/css-modules)。                                                                                                                                                    |
| `<script>`                    | 深入了解 [指令碼使用方式](/docs/14/app/building-your-application/optimizing/scripts)。                                                                                                                                                          |
...
您可以使用 `Metadata` 類型為中繼資料添加類型安全。如果您的 IDE 使用[內建 TypeScript 插件](/docs/14/app/building-your-application/configuring/typescript)，則無需手動添加類型，但仍可明確添加。
...
| 版本       | 變更內容                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.2.0` | `viewport`、`themeColor` 和 `colorScheme` 已被棄用，改為使用 [`viewport` 設定](/docs/14/app/api-reference/functions/generate-viewport)。 |
