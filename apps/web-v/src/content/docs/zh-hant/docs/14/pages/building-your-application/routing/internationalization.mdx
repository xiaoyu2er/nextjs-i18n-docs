...
在 Next.js 12 和 [中介軟體 (Middleware)](/docs/14/pages/building-your-application/routing/middleware) 中，我們可以使用 [解決方案](https://github.com/vercel/next.js/discussions/18419) 為預設語言環境添加前綴。
...
接下來，我們可以使用 [中介軟體 (Middleware)](/docs/14/pages/building-your-application/routing/middleware) 添加自訂路由規則：
...
此 [中介軟體 (Middleware)](/docs/14/pages/building-your-application/routing/middleware) 會跳過為 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 和 [靜態檔案](/docs/14/pages/building-your-application/optimizing/static-assets)（如字型或圖片）添加預設前綴。如果請求是針對預設語言環境，我們會重新導向至帶有前綴 `/en` 的路徑。
...
您可以透過 Next.js 路由器存取語言環境資訊。例如，使用 [`useRouter()`](/docs/14/pages/api-reference/functions/use-router) 鉤子時，以下屬性可用：

- `locale` 包含目前使用的語言環境。
- `locales` 包含所有設定的語言環境。
- `defaultLocale` 包含設定的預設語言環境。

當使用 `getStaticProps` 或 `getServerSideProps` [預渲染 (pre-rendering)](/docs/14/pages/building-your-application/rendering/static-site-generation) 頁面時，語言環境資訊會提供給函式的 [context](/docs/14/pages/building-your-application/data-fetching/get-static-props)。
...
請注意，若要僅切換 `locale` 同時保留所有路由資訊（如 [動態路由 (dynamic route)](/docs/14/pages/building-your-application/routing/dynamic-routes) 查詢值或隱藏的 href 查詢值），您可以將 `href` 參數作為物件提供：
...
有關 `router.push` 的物件結構更多資訊，請參閱 [此處](/docs/14/pages/api-reference/functions/use-router#with-url-object)。
...
Next.js 不知道頁面的變體，因此需要您使用 [`next/head`](/docs/14/pages/api-reference/components/head) 添加 `hreflang` 元標籤。您可以在 [Google Webmasters 文件](https://support.google.com/webmasters/answer/189077) 中了解更多關於 `hreflang` 的資訊。

## 這如何與靜態生成 (Static Generation) 配合使用？

> 請注意，國際化路由不與 [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) 整合，因為它不利用 Next.js 路由層。不使用 `output: 'export'` 的混合 Next.js 應用程式完全支援。

### 動態路由和 `getStaticProps` 頁面

對於使用 `getStaticProps` 與 [動態路由 (Dynamic Routes)](/docs/14/pages/building-your-application/routing/dynamic-routes) 的頁面，所有需要預渲染的語言環境變體都需要從 [`getStaticPaths`](/docs/14/pages/building-your-application/data-fetching/get-static-paths) 返回。除了為 `paths` 返回的 `params` 物件外，您還可以返回一個 `locale` 欄位，指定要渲染的語言環境。例如：
...
對於 [自動靜態優化 (Automatically Statically Optimized)](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) 和非動態 `getStaticProps` 頁面，**會為每個語言環境生成一個頁面版本**。這點很重要，因為根據 `getStaticProps` 中設定的語言環境數量，可能會增加建置時間。

例如，如果您設定了 50 個語言環境和 10 個使用 `getStaticProps` 的非動態頁面，這意味著 `getStaticProps` 將被呼叫 500 次。每次建置時會生成這 10 個頁面的 50 個版本。

要減少使用 `getStaticProps` 的動態頁面建置時間，請使用 [`fallback` 模式](/docs/14/pages/api-reference/functions/get-static-paths#fallback-true)。這允許您從 `getStaticPaths` 僅返回最受歡迎的路徑和語言環境進行建置時的預渲染。然後，Next.js 會在請求時於運行時建置其餘頁面。

### 自動靜態優化頁面

對於 [自動靜態優化 (automatically statically optimized)](/docs/14/pages/building-your-application/rendering/automatic-static-optimization) 的頁面，會為每個語言環境生成一個頁面版本。
...
> **須知**：這些限制最初是為了避免 [建置時的潛在效能問題](#dynamic-routes-and-getstaticprops-pages) 而添加的。您可以在 Next.js 12 中使用 [中介軟體 (Middleware)](/docs/14/pages/building-your-application/routing/middleware) 進行自訂路由來解決這些限制。