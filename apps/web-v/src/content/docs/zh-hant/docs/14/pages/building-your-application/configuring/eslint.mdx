...
> 如果您不希望 ESLint 在 `next build` 時執行，請參考 [忽略 ESLint](/docs/14/app/api-reference/next-config-js/eslint) 的文件。

</AppOnly>

<PagesOnly>

> 如果您不希望 ESLint 在 `next build` 時執行，請參考 [忽略 ESLint](/docs/14/pages/api-reference/next-config-js/eslint) 的文件。
...
| <Check size={18} /> | @next/next/no-typos                                                                                                      | 防止在 [Next.js 資料獲取函數](/docs/14/pages/building-your-application/data-fetching) 中出現常見拼寫錯誤 |
...
為了提高效能，ESLint 處理的檔案資訊預設會被快取。這些資訊儲存在 `.next/cache` 或您定義的 [建構目錄](/docs/14/app/api-reference/next-config-js/distDir) 中。如果您包含的任何 ESLint 規則依賴於多個原始檔案的內容並需要停用快取，請在 `next lint` 中使用 `--no-cache` 標誌。

</AppOnly>

<PagesOnly>

為了提高效能，ESLint 處理的檔案資訊預設會被快取。這些資訊儲存在 `.next/cache` 或您定義的 [建構目錄](/docs/14/pages/api-reference/next-config-js/distDir) 中。如果您包含的任何 ESLint 規則依賴於多個原始檔案的內容並需要停用快取，請在 `next lint` 中使用 `--no-cache` 標誌。
...
> 對於使用 [Create Next App](/docs/14/app/api-reference/cli/create-next-app) 建立的新應用程式，會自動包含 `next/core-web-vitals` 入口點。
...
- 您已安裝以下任一插件（單獨安裝或透過其他配置如 `airbnb` 或 `react-app` 安裝）：
  ...
  - `jsx-a11y`
  - `import`
- 您定義了特定的 `parserOptions` 且與 Next.js 中的 Babel 配置不同（除非您已[自訂 Babel 配置](/docs/14/pages/building-your-application/configuring/babel)，否則不建議這麼做）
