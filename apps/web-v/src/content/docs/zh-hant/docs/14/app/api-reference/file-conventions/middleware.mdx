...
`middleware.js|ts` 檔案用於撰寫 [中介軟體 (Middleware)](/docs/14/app/building-your-application/routing/middleware)，並在請求完成前於伺服器端執行程式碼。根據傳入的請求，您可以透過重寫、重新導向、修改請求或回應標頭，或直接回應來修改回應內容。
...
中介軟體可以使用 [`NextResponse`](/docs/14/app/building-your-application/routing/middleware#nextresponse) 物件，它擴展了 [Web Response API](https://developer.mozilla.org/en-US/docs/Web/API/Response)。透過回傳 `NextResponse` 物件，您可以直接操作 cookies、設定標頭、實作重新導向和重寫路徑。

> **須知**：對於重新導向，您也可以使用 `Response.redirect` 代替 `NextResponse.redirect`。

## 執行環境

中介軟體僅支援 [Edge 執行環境 (Edge Runtime)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)。無法使用 Node.js 執行環境。
