...
與其使用 [React Context](https://react.dev/learn/passing-data-deeply-with-context) (在伺服器上不可用) 或將資料作為 props 傳遞，您可以使用 [`fetch`](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#fetching-data-on-the-server-with-fetch) 或 React 的 `cache` 函數在需要資料的元件中獲取相同的資料，而無需擔心對相同資料發出重複請求。這是因為 React 擴展了 `fetch` 以自動記憶化資料請求，而 `cache` 函數可以在 `fetch` 不可用時使用。

了解更多關於 React 中的 [記憶化](/docs/14/app/building-your-application/caching#request-memoization)。
...
如果您的客戶端元件依賴於不可序列化的資料，您可以使用 [第三方套件在客戶端獲取資料](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#fetching-data-on-the-client-with-third-party-libraries) 或通過 [路由處理程式](/docs/14/app/building-your-application/routing/route-handlers) 在伺服器上獲取。

## 交錯使用伺服器與客戶端元件

當交錯使用客戶端元件 (Client Components) 和伺服器元件 (Server Components) 時，將您的 UI 視為元件樹會有所幫助。從[根佈局](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required)（這是一個伺服器元件）開始，您可以透過添加 `"use client"` 指令來在客戶端渲染某些元件子樹。
