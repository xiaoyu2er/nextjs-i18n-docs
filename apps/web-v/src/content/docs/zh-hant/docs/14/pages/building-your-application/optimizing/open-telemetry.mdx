...
接著，在專案的**根目錄**（或使用 `src` 資料夾時在其中）創建一個自訂的 [`instrumentation.ts`](/docs/14/app/building-your-application/optimizing/instrumentation)（或 `.js`）檔案：

</AppOnly>

<PagesOnly>

接著，在專案的**根目錄**（或使用 `src` 資料夾時在其中）創建一個自訂的 [`instrumentation.ts`](/docs/14/pages/building-your-application/optimizing/instrumentation)（或 `.js`）檔案：
...
> - 若使用 [`pageExtensions` 配置選項](/docs/14/app/api-reference/next-config-js/pageExtensions) 添加後綴，則需相應更新 `instrumentation` 的檔案名。
...
>
> - `instrumentation` 檔案應位於專案的根目錄，而非 `app` 或 `pages` 目錄內。若使用 `src` 資料夾，請將檔案放在 `src` 中與 `pages` 和 `app` 同級。
> - 若使用 [`pageExtensions` 配置選項](/docs/14/pages/api-reference/next-config-js/pageExtensions) 添加後綴，則需相應更新 `instrumentation` 的檔案名。
> - 我們已創建了一個基礎的 [with-opentelemetry](https://github.com/vercel/next.js/tree/canary/examples/with-opentelemetry) 範例供您參考。
...
OpenTelemetry 收集器並非必需。您可以使用自訂的 OpenTelemetry 導出器配合 [`@vercel/otel`](#使用-vercelotel) 或 [手動 OpenTelemetry 配置](/docs/14/pages/building-your-application/optimizing/open-telemetry#手動-opentelemetry-配置)。
