...
當使用頁面路由器時，除非存在 [中介軟體](/docs/14/app/building-your-application/routing/middleware) 並匹配路徑，否則重新導向不會應用於客戶端路由（`Link`、`router.push`）。
...
當使用 [`basePath` 支援](/docs/14/app/api-reference/next-config-js/basePath) 進行重新導向時，每個 `source` 和 `destination` 會自動加上 `basePath` 前綴，除非您在重新導向中新增 `basePath: false`：
...
當使用 [`i18n` 支援](/docs/14/app/building-your-application/routing/internationalization) 進行重新導向時，每個 `source` 和 `destination` 會自動加上前綴以處理配置的 `locales`，除非您在重新導向中新增 `locale: false`。如果使用 `locale: false`，您必須在 `source` 和 `destination` 前加上語言環境前綴才能正確匹配。

</AppOnly>

<PagesOnly>

當使用 [`i18n` 支援](/docs/14/pages/building-your-application/routing/internationalization) 進行重新導向時，每個 `source` 和 `destination` 會自動加上前綴以處理配置的 `locales`，除非您在重新導向中新增 `locale: false`。如果使用 `locale: false`，您必須在 `source` 和 `destination` 前加上語言環境前綴才能正確匹配。
...
- 在 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 和 [路由處理程式](/docs/14/app/building-your-application/routing/route-handlers) 中，您可以根據傳入的請求進行重新導向。
- 在 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) 和 [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 中，您可以在請求時重新導向特定頁面。
