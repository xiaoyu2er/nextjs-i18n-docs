...
配置請求如何與 Next.js [資料快取](/docs/14/app/building-your-application/caching#data-cache) 互動。
...
> **須知**：
>
> - 如果未提供 `cache` 選項，Next.js 預設使用 `force-cache`，除非使用了 [動態函式](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) 如 `cookies()`，此時會預設為 `no-store`。
...
> **須知**：
>
> - 如果單個 `fetch()` 請求設定的 `revalidate` 數值低於路由的 [預設 `revalidate`](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate)，則整個路由的重新驗證間隔會縮短。
