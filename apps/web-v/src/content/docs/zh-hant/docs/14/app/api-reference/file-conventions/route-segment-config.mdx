...
路由區段選項允許您透過直接導出以下變數來配置 [頁面](/docs/14/app/building-your-application/routing/pages-and-layouts)、[佈局](/docs/14/app/building-your-application/routing/pages-and-layouts) 或 [路由處理器](/docs/14/app/building-your-application/routing/route-handlers) 的行為：
...
- **`'force-dynamic'`**: 強制 [動態渲染](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)，這將導致每個使用者在請求時渲染路由。此選項等同於 `pages` 目錄中的 `getServerSideProps()`。

- **`'error'`**: 強制靜態渲染並快取佈局或頁面的數據，如果任何元件使用 [動態函數](/docs/14/app/building-your-application/rendering/server-components#dynamic-functions) 或未快取的數據，則會引發錯誤。此選項等同於：
...
- **`'force-static'`**: 強制靜態渲染並快取佈局或頁面的數據，透過強制 [`cookies()`](/docs/14/app/api-reference/functions/cookies)、[`headers()`](/docs/14/app/api-reference/functions/headers) 和 [`useSearchParams()`](/docs/14/app/api-reference/functions/use-search-params) 返回空值。

> **須知事項**:
>
> - 有關如何從 `getServerSideProps` 和 `getStaticProps` 遷移到 `dynamic: 'force-dynamic'` 和 `dynamic: 'error'` 的說明，請參閱 [升級指南](/docs/14/app/building-your-application/upgrading/app-router-migration#step-6-migrating-data-fetching-methods)。

### `dynamicParams`

控制當訪問未使用 [generateStaticParams](/docs/14/app/api-reference/functions/generate-static-params) 生成的動態區段時會發生什麼。
...
> - 當 `dynamicParams = true` 時，該區段使用 [串流伺服器渲染 (Streaming Server Rendering)](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense)。
...
- **`false`** (預設): 預設啟發式方法，快取任何將其 `cache` 選項設定為 `'force-cache'` 的 `fetch` 請求，或在使用 [動態函數](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) 之前發現的請求。語意上等同於 `revalidate: Infinity`，這實際上意味著資源應無限期快取。個別 `fetch` 請求仍可使用 `cache: 'no-store'` 或 `revalidate: 0` 來避免被快取並使路由動態渲染。或者將 `revalidate` 設定為低於路由預設值的正數，以增加路由的重新驗證頻率。
- **`0`**: 確保佈局或頁面始終 [動態渲染](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)，即使未發現動態函數或未快取的數據獲取。此選項會將未設定 `cache` 選項的 `fetch` 請求的預設值變更為 `'no-store'`，但保留選擇 `'force-cache'` 或使用正數 `revalidate` 的 `fetch` 請求不變。
- **`number`**: (以秒為單位) 將佈局或頁面的預設重新驗證頻率設定為 `n` 秒。

> **須知事項**: `revalidate` 選項僅在使用 [Node.js 運行時 (Node.js Runtime)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes#nodejs-runtime) 時可用。這意味著將 `revalidate` 選項與 `runtime = 'edge'` 一起使用將無效。
...
預設情況下，Next.js **會快取** 任何在使用 [動態函數](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) 之前可達的 `fetch()` 請求，並且 **不會快取** 在使用動態函數之後發現的 `fetch` 請求。
...
了解更多關於 [邊緣和 Node.js 運行時 (Edge and Node.js runtimes)](/docs/14/app/building-your-application/rendering/edge-and-nodejs-runtimes)。
...
> **須知事項**:
>
> - 如果使用 [伺服器操作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)，請在頁面層級設定 `maxDuration` 以變更頁面上使用的所有伺服器操作的預設逾時時間。

### `generateStaticParams`

`generateStaticParams` 函數可以與 [動態路由區段](/docs/14/app/building-your-application/routing/dynamic-routes) 結合使用，以定義在構建時靜態生成的路由區段參數列表，而不是在請求時按需生成。

詳情請參閱 [API 參考](/docs/14/app/api-reference/functions/generate-static-params)。