...
> **小知識：**
>
> - 我們即將推薦[漸進式採用](/docs/14/app/building-your-application/upgrading/app-router-migration) App 路由並使用[伺服器動作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)來處理表單提交與資料異動。伺服器動作讓您能直接從元件呼叫非同步伺服器函式，無需手動建立 API 路由。
> - API 路由[不指定 CORS 標頭](https://developer.mozilla.org/docs/Web/HTTP/CORS)，這表示預設僅允許同源請求。
> - 由於 API 路由在伺服器端執行，我們能透過[環境變數 (Environment Variables)](/docs/14/pages/building-your-application/configuring/environment-variables)使用敏感值（如 API 金鑰）而不暴露給客戶端。這對應用程式的安全性至關重要。
...
如果您想在異動後將使用者重新導向到不同路由，可以使用 [`redirect`](/docs/14/pages/building-your-application/routing/api-routes#response-helpers) 導向任何絕對或相對 URL：
...
您可以在 API 路由中使用 [`cookies`](/docs/14/pages/building-your-application/routing/api-routes#request-helpers) 請求輔助工具來讀取 Cookie：
