...
例如，在 [函數 API 參考](/docs/14/app/api-reference/functions) 中，頁面按字母順序排序，這讓開發者更容易找到特定函數：
...
但在 [路由章節](/docs/14/app/building-your-application/routing) 中，文件以兩位數字為前綴，按開發者學習這些概念的順序排序：
...
- 連結將以卡片形式顯示在頁面主要內容下方。
- 對於有子頁面的頁面，將自動生成連結。例如，[優化](/docs/14/app/building-your-application/optimizing) 章節包含指向其所有子頁面的連結。
