...
查看**新版** [`next/image` API 參考文件](/docs/14/pages/api-reference/components/image)
...
- [靜態匯入](/docs/14/pages/building-your-application/optimizing/images#local-images)的圖片檔案
...
`width` 屬性是必填的，除非是[靜態匯入的圖片](/docs/14/pages/building-your-application/optimizing/images#local-images)，或使用 `layout="fill"` 的情況。
...
`height` 屬性是必填的，除非是[靜態匯入的圖片](/docs/14/pages/building-your-application/optimizing/images#local-images)，或使用 `layout="fill"` 的情況。
...
當為 `blur` 時，將使用 [`blurDataURL`](#blurdataurl) 屬性作為佔位符。如果 `src` 是[靜態匯入](/docs/14/pages/building-your-application/optimizing/images#local-images)的物件，且匯入的圖片為 `.jpg`、`.png`、`.webp` 或 `.avif`，則會自動填充 `blurDataURL`。
...
> 使用 [`output: 'export'`](/docs/14/pages/building-your-application/deploying/static-exports) 時，圖片無法在構建時最佳化，僅能按需最佳化。要將 `next/legacy/image` 與 `output: 'export'` 一起使用，您需要使用非預設的載入器。[詳見討論。](https://github.com/vercel/next.js/discussions/19065)
...
您可以為快取的最佳化圖片配置存留時間（TTL，單位：秒）。在許多情況下，最好使用 [靜態圖片匯入](/docs/14/pages/building-your-application/optimizing/images#local-images)，它會自動對檔案內容進行雜湊處理，並以 `Cache-Control` 標頭 `immutable` 永久快取圖片。
...
如果需要按圖片變更快取行為，可以配置 [`headers`](/docs/14/pages/api-reference/next-config-js/headers) 在上游圖片（例如 `/some-asset.jpg`，而非 `/_next/image` 本身）上設定 `Cache-Control` 標頭。
...
預設 [載入器](#loader) 不會最佳化 SVG 圖片，原因如下：首先，SVG 是一種向量格式，意味著可以無損調整大小。其次，SVG 具有許多與 HTML/CSS 相同的功能，如果沒有適當的 [內容安全政策 (CSP) 標頭](/docs/14/app/api-reference/next-config-js/headers#content-security-policy)，可能導致安全漏洞。
