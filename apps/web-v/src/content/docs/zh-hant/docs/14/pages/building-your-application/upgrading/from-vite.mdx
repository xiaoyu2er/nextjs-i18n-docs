...
透過內建的 [透過 React Suspense 進行串流](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense) 支援，您可以更有意義地決定 UI 的哪些部分要優先載入以及載入順序，而不會引入網路瀑布效應。
...
[Next.js 中介軟體](/docs/14/app/building-your-application/routing/middleware) 允許您在請求完成前在伺服器上執行程式碼。這對於避免使用者在訪問需要驗證的頁面時看到未驗證內容的閃爍特別有用，可以將使用者重定向到登入頁面。中介軟體也適用於實驗和 [國際化](/docs/14/app/building-your-application/routing/internationalization)。

### 內建優化

[圖片](/docs/14/app/building-your-application/optimizing/images)、[字型](/docs/14/app/building-your-application/optimizing/fonts) 和 [第三方腳本](/docs/14/app/building-your-application/optimizing/scripts) 通常對應用程式的效能有重大影響。Next.js 提供了內建元件，可以自動為您優化這些內容。
...
在專案根目錄下建立 `next.config.mjs` 檔案。此檔案將包含您的 [Next.js 設定選項](/docs/14/app/api-reference/next-config-js)。
...
您可以在 [Next.js 文件](/docs/14/app/building-your-application/configuring/typescript#typescript-plugin) 中找到更多關於設定 TypeScript 的資訊。

### 步驟 4：建立根佈局

Next.js [App Router](/docs/14/app) 應用程式必須包含一個 [根佈局](/docs/14/app/building-your-application/routing/pages-and-layouts#root-layout-required) 檔案，這是一個 [React 伺服器元件](/docs/14/app/building-your-application/rendering/server-components)，它將包裹應用程式中的所有頁面。此檔案定義在 `app` 目錄的頂層。
...
5. 任何 [中繼資料檔案](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) 如 `favicon.ico`、`icon.png`、`robots.txt` 只要放置在 `app` 目錄的頂層，就會自動新增到應用程式的 `<head>` 標籤中。將 [所有支援的檔案](/docs/14/app/building-your-application/optimizing/metadata#file-based-metadata) 移至 `app` 目錄後，您可以安全地刪除它們的 `<link>` 標籤：
...
6. 最後，Next.js 可以透過 [中繼資料 API](/docs/14/app/building-your-application/optimizing/metadata) 管理您的最後幾個 `<head>` 標籤。將您的最終中繼資料資訊移至匯出的 [`metadata` 物件](/docs/14/app/api-reference/functions/generate-metadata#metadata-object)：
...
透過上述變更，您從在 `index.html` 中宣告所有內容轉變為使用 Next.js 內建的基於慣例的方法（[中繼資料 API](/docs/14/app/building-your-application/optimizing/metadata)）。這種方法使您能夠更輕鬆地提升頁面的 SEO 和網路分享性。
...
此目錄稱為
[可選的全面捕捉路由區段](/docs/14/app/building-your-application/routing/dynamic-routes#optional-catch-all-segments)。
Next.js 使用基於檔案系統的路由器，其中
[目錄用於定義路由](/docs/14/app/building-your-application/routing/defining-routes#creating-routes)。
...
此檔案是一個 [伺服器元件 (Server Component)](/docs/14/app/building-your-application/rendering/server-components)。當您執行 `next build` 時，此檔案會被預渲染為靜態資源。它_不需要_任何動態程式碼。

此檔案匯入了我們的全局 CSS，並告訴 [`generateStaticParams`](/docs/14/app/api-reference/functions/generate-static-params) 我們只會生成一個路由，即位於 `/` 的索引路由。
...
此檔案是一個 [客戶端元件 (Client Component)](/docs/14/app/building-your-application/rendering/client-components)，由 `'use client'` 指令定義。客戶端元件在發送到客戶端之前仍會在伺服器上 [預渲染為 HTML](/docs/14/app/building-your-application/rendering/client-components#how-are-client-components-rendered)。
...
在 Next.js 中，靜態圖片匯入會返回一個物件。該物件可以直接與 Next.js 的 [`<Image>` 元件](/docs/14/app/api-reference/components/image) 一起使用，或者您可以使用物件的 `src` 屬性與現有的 `<img>` 標籤。

`<Image>` 元件具有 [自動圖片優化](/docs/14/app/building-your-application/optimizing/images) 的額外優勢。`<Image>` 元件會根據圖片的尺寸自動設定結果 `<img>` 的 `width` 和 `height` 屬性。這可以防止圖片載入時的版面位移。然而，如果您的應用程式包含僅設定其中一個尺寸而另一個尺寸未設定為 `auto` 的圖片，這可能會導致問題。當未設定為 `auto` 時，尺寸將預設為 `<img>` 尺寸屬性的值，這可能導致圖片顯示失真。

保留 `<img>` 標籤將減少應用程式中的變更量並避免上述問題。您可以稍後選擇性地遷移到 `<Image>` 元件，以透過 [配置載入器 (loader)](/docs/14/app/building-your-application/optimizing/images#loaders) 來利用圖片優化，或者轉移到具有自動圖片優化的預設 Next.js 伺服器。
...
Next.js 支援類似於 Vite 的 `.env` [環境變數](/docs/14/app/building-your-application/configuring/environment-variables)。主要區別在於用於在客戶端公開環境變數的前綴。
...
2. **在您的 `next.config.mjs` 檔案中將 [`basePath`](/docs/14/app/api-reference/next-config-js/basePath) 設定為 `process.env.NEXT_PUBLIC_BASE_PATH`：**
...
- [使用 `next/font` 優化字體](/docs/14/app/building-your-application/optimizing/fonts)
- [使用 `<Script>` 元件優化第三方腳本](/docs/14/app/building-your-application/optimizing/scripts)
- [更新您的 ESLint 配置以支援 Next.js 規則](/docs/14/app/building-your-application/configuring/eslint)
