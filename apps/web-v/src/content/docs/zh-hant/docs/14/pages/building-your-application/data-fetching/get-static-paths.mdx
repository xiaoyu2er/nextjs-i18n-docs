...
如果一個頁面具有[動態路由 (Dynamic Routes)](/docs/14/pages/building-your-application/routing/dynamic-routes) 並使用 `getStaticProps`，則需要定義一個要靜態生成的路徑列表。
...
[`getStaticPaths` API 參考文件](/docs/14/pages/api-reference/functions/get-static-paths) 涵蓋了所有可與 `getStaticPaths` 一起使用的參數和屬性。
...
- **不能** 將 `getStaticPaths` 與 [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props) 一起使用
- 可以從同時使用 `getStaticProps` 的[動態路由 (Dynamic Route)](/docs/14/pages/building-your-application/routing/dynamic-routes) 導出 `getStaticPaths`
...
`getStaticPaths` 允許你控制在建置期間生成哪些頁面，而不是透過 [`fallback`](/docs/14/pages/api-reference/functions/get-static-paths#fallback-blocking) 按需生成。在建置期間生成更多頁面會導致建置速度變慢。
