...
在[頁面文件](/docs/14/pages/building-your-application/routing/pages-and-layouts)與[資料獲取文件](/docs/14/pages/building-your-application/data-fetching)中，我們討論了如何使用 `getStaticProps` 和 `getStaticPaths` 在建置時預渲染頁面（**靜態生成**）。
...
> 若不熟悉 Next.js API 路由，請先參閱 [API 路由文件](/docs/14/pages/building-your-application/routing/api-routes)。
...
然後發送請求至 `/api/disable-draft` 以呼叫 API 路由。若使用 [`next/link`](/docs/14/pages/api-reference/components/link) 呼叫此路由，必須傳遞 `prefetch={false}` 以防預取時意外刪除 cookie。

### 與 `getServerSideProps` 協同工作

草稿模式可與 `getServerSideProps` 協同工作，並作為 `draftMode` 鍵存在於 [`context`](/docs/14/pages/api-reference/functions/get-server-side-props#context-parameter) 物件中。

> **須知**：使用草稿模式時不應設定 `Cache-Control` 標頭，因其無法被繞過。建議改用 [ISR](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration)。
