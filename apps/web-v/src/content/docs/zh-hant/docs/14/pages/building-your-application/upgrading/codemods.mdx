...
此 codemod 將 [動態 OG 圖片生成](/docs/14/app/building-your-application/optimizing/metadata#dynamic-image-generation) 相關的導入從 `next/server` 轉移至 `next/og`。
...
移除 [Link 元件](/docs/14/app/api-reference/components/link) 中的 `<a>` 標籤，或為無法自動修復的 Links 添加 `legacyBehavior` 屬性。

</AppOnly>

<PagesOnly>

移除 [Link 元件](/docs/14/pages/api-reference/components/link) 中的 `<a>` 標籤，或為無法自動修復的 Links 添加 `legacyBehavior` 屬性。
