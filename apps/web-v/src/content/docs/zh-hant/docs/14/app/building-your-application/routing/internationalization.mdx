...
路由可透過子路徑 (`/fr/products`) 或網域 (`my-site.fr/products`) 實現國際化。有了這些資訊，您現在可以根據[中介層 (Middleware)](/docs/14/app/building-your-application/routing/middleware) 中的地區設定來重新導向使用者。
...
由於 `app/` 目錄中的所有佈局和頁面預設為[伺服器元件 (Server Components)](/docs/14/app/building-your-application/rendering/server-components)，我們無需擔心翻譯檔案的大小會影響客戶端 JavaScript 套件大小。此程式碼**僅會在伺服器端執行**，只有產生的 HTML 會被傳送到瀏覽器。
