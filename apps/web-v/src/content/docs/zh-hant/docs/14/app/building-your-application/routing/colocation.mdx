...
除了[路由資料夾與檔案慣例](/docs/14/getting-started/project-structure#app-routing-conventions)外，Next.js 對於專案檔案組織與共置方式採取**非強制性**立場。
...
在 `app` 目錄中，[巢狀資料夾層級結構](/docs/14/app/building-your-application/routing#route-segments)定義了路由結構。
...
> - 若不使用私有資料夾，了解 Next.js [特殊檔案慣例](/docs/14/getting-started/project-structure#routing-files)有助於避免意外命名衝突。
...
- [將路由分組組織](/docs/14/app/building-your-application/routing/route-groups#organize-routes-without-affecting-the-url-path)，例如按網站區段、用途或團隊。
- 在同一路由區段層級啟用巢狀佈局：
  - [在同一區段建立多個巢狀佈局，包括多個根佈局](/docs/14/app/building-your-application/routing/route-groups#creating-multiple-root-layouts)
  - [將佈局添加到共用區段中的路由子集](/docs/14/app/building-your-application/routing/route-groups#opting-specific-segments-into-a-layout)

### `src` 目錄

Next.js 支援將應用程式碼 (包括 `app`) 儲存在可選的 [`src` 目錄](/docs/14/app/building-your-application/configuring/src-directory)中。這可將應用程式碼與主要位於專案根目錄的專案設定檔分離。
...
Next.js 支援[模組路徑別名](/docs/14/app/building-your-application/configuring/absolute-imports-and-module-aliases)，讓深度巢狀的專案檔案中的導入更易讀與維護。
