...
當使用 `next/link` 和 `next/router` 連結到其他頁面時，`basePath` 會自動套用。
...
當使用 [`next/image`](/docs/14/app/api-reference/components/image) 元件時，您需要在 `src` 前面加上 `basePath`。

</AppOnly>

<PagesOnly>

當使用 [`next/image`](/docs/14/pages/api-reference/components/image) 元件時，您需要在 `src` 前面加上 `basePath`。
