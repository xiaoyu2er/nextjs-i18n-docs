...
> **注意**：此功能已被 [草稿模式 (Draft Mode)](/docs/14/pages/building-your-application/configuring/draft-mode) 取代。
...
在 [頁面文件](/docs/14/pages/building-your-application/routing/pages-and-layouts) 和 [資料獲取文件](/docs/14/pages/building-your-application/data-fetching) 中，我們討論了如何使用 `getStaticProps` 和 `getStaticPaths` 在建置時預渲染頁面（**靜態生成 (Static Generation)**）。
...
> 如果您不熟悉 Next.js API 路由，請先查看 [API 路由文件](/docs/14/pages/building-your-application/routing/api-routes)。
...
> **須知**：在渲染期間，`next/router` 會公開一個 `isPreview` 標誌，請參閱 [router 物件文件](/docs/14/pages/api-reference/functions/use-router#router-object) 以獲取更多資訊。
...
然後，發送請求到 `/api/clear-preview-mode-cookies` 以呼叫 API 路由。如果使用 [`next/link`](/docs/14/pages/api-reference/components/link) 呼叫此路由，則必須傳遞 `prefetch={false}` 以防止在連結預取期間呼叫 `clearPreviewData`。
...
> **須知**：使用預覽模式時不應設定 `Cache-Control` 標頭，因為它無法被繞過。相反，我們建議使用 [ISR (增量靜態再生)](/docs/14/pages/building-your-application/data-fetching/incremental-static-regeneration)。
