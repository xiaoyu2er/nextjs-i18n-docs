...
本 API 參考文件將幫助您理解如何使用 [`next/font/google`](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) 和 [`next/font/local`](/docs/14/app/building-your-application/optimizing/fonts#local-fonts)。如需功能和使用說明，請參閱 [優化字型](/docs/14/app/building-your-application/optimizing/fonts) 頁面。

### 字型函式參數

關於使用方式，請參閱 [Google 字型](/docs/14/app/building-your-application/optimizing/fonts#google-fonts) 和 [本地字型](/docs/14/app/building-your-application/optimizing/fonts#local-fonts)。
...
字型的 [`subsets`](https://fonts.google.com/knowledge/glossary/subsetting)，定義為字串陣列，包含您想要 [預載入](/docs/14/app/building-your-application/optimizing/fonts#specifying-a-subset) 的子集名稱。當 [`preload`](#preload) 選項為 `true` (預設值) 時，透過 `subsets` 指定的字型會在 head 中注入 link preload 標籤。
...
布林值，指定是否應 [預載入](/docs/14/app/building-your-application/optimizing/fonts#preloading) 字型。預設為 `true`。
