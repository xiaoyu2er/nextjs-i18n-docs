...
> **須知事項**: 目前 [`edge` 運行時](/docs/14/pages/api-reference/edge) 與 ISR 不相容，但您可以透過手動設定 `cache-control` 標頭來利用 `stale-while-revalidate` 機制。
...
> **須知事項**: [中介軟體](/docs/14/pages/building-your-application/routing/middleware) 不會為按需 ISR 請求執行。相反，您需要在想要重新驗證的 _確切_ 路徑上呼叫 `revalidate()`。例如，如果您有 `pages/blog/[slug].js` 和從 `/post-1` 重寫到 `/blog/post-1` 的路由，您需要呼叫 `res.revalidate('/blog/post-1')`。
...
接下來，將該秘密添加為應用程式的[環境變數](/docs/14/pages/building-your-application/configuring/environment-variables)。最後，建立重新驗證 API 路由：
...
當使用 `next dev` 本地運行時，`getStaticProps` 會在每個請求上被呼叫。要驗證您的按需 ISR 配置是否正確，您需要建立一個[生產版本](/docs/14/pages/api-reference/cli/next#next-build-options)並啟動[生產伺服器](/docs/14/pages/api-reference/cli/next#next-start-options)：
...
增量靜態再生 (ISR) 在[自託管的 Next.js 網站](/docs/14/pages/building-your-application/deploying#self-hosting)上開箱即用，當您使用 `next start` 時。

了解更多關於[自託管 Next.js](/docs/14/pages/building-your-application/deploying#self-hosting) 的資訊。
