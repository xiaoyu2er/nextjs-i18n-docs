...
| 名稱            | 說明                                                                                                                                                                                                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`        | 如果此頁面使用[動態路由](/docs/14/pages/building-your-application/routing/dynamic-routes)，`params` 包含路由參數。如果頁面名稱是 `[id].js`，則 `params` 會像 `{ id: ... }`。 |
...
| `preview`       | (已棄用，改用 `draftMode`) 如果頁面處於[預覽模式](/docs/14/pages/building-your-application/configuring/preview-mode)，則 `preview` 為 `true`，否則為 `false`。                                          |
| `previewData`   | (已棄用，改用 `draftMode`) 由 `setPreviewData` 設定的[預覽](/docs/14/pages/building-your-application/configuring/preview-mode) 資料。                                                                              |
| `draftMode`     | 如果頁面處於[草稿模式](/docs/14/pages/building-your-application/configuring/draft-mode)，則 `draftMode` 為 `true`，否則為 `false`。                                                                         |
...
`notFound` 布林值允許頁面返回 `404` 狀態和 [404 頁面](/docs/14/pages/building-your-application/routing/custom-error#404-page)。當 `notFound: true` 時，即使之前成功生成了頁面，頁面也會返回 `404`。這旨在支援諸如用戶生成內容被作者刪除等用例。
...
| 版本   | 變更                                                                                                     |
| --------- | ----------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/14/app/building-your-application/data-fetching) 現已穩定，並簡化了資料獲取 |
