...
> **須知**：如果您使用的是 Next.js 13 之前的版本，請參考 [next/legacy/image](/docs/14/pages/api-reference/components/image-legacy) 文件，因為元件名稱已變更。

</PagesOnly>

此 API 參考將幫助您了解如何使用圖片元件提供的 [屬性 (props)](#props) 和 [配置選項](#configuration-options)。關於功能和用法，請參閱 [圖片元件](/docs/14/app/building-your-application/optimizing/images) 頁面。
...
- [靜態匯入](/docs/14/app/building-your-application/optimizing/images#local-images) 的圖片檔案
...
必填，除非是 [靜態匯入的圖片](/docs/14/app/building-your-application/optimizing/images#local-images) 或使用 [`fill` 屬性](#fill) 的圖片。
...
> **須知**：使用像 `loader` 這樣接受函式的屬性，需要使用 [客戶端元件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components) 來序列化提供的函式。
...
當為 `blur` 時，將使用 [`blurDataURL`](#blurdataurl) 屬性作為佔位符。如果 `src` 是 [靜態匯入](/docs/14/app/building-your-application/optimizing/images#local-images) 的物件，且匯入的圖片為 `.jpg`、`.png`、`.webp` 或 `.avif`，則 `blurDataURL` 將自動填充，除非檢測到圖片是動畫。
...
> **須知**：使用像 `onLoadingComplete` 這樣接受函式的屬性，需要使用 [客戶端元件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components) 來序列化提供的函式。
...
> **須知**：使用像 `onLoad` 這樣接受函式的屬性，需要使用 [客戶端元件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components) 來序列化提供的函式。
...
> **須知**：使用像 `onError` 這樣接受函式的屬性，需要使用 [客戶端元件 (Client Components)](/docs/14/app/building-your-application/rendering/client-components) 來序列化提供的函式。
...
- [自訂圖片載入器配置](/docs/14/app/api-reference/next-config-js/images#example-loader-configuration)

<AppOnly>

> **須知**：自訂圖片載入器文件（接受一個函數）需要使用 [客戶端組件](/docs/14/app/building-your-application/rendering/client-components) 來序列化提供的函數。
...
你可以配置緩存優化圖片的生存時間（TTL，以秒為單位）。在許多情況下，最好使用 [靜態圖片導入](/docs/14/app/building-your-application/optimizing/images#local-images)，它會自動哈希文件內容並使用 `Cache-Control` 標頭 `immutable` 永久緩存圖片。
...
如果需要按圖片更改緩存行為，可以配置 [`headers`](/docs/14/app/api-reference/next-config-js/headers) 在上游圖片（例如 `/some-asset.jpg`，而不是 `/_next/image` 本身）上設置 `Cache-Control` 標頭。
...
預設的 [loader](#loader) 不會對 SVG 圖片進行最佳化，原因如下：首先，SVG 是一種向量格式，意味著它可以無損縮放。其次，SVG 具有許多與 HTML/CSS 相同的功能，若沒有適當的 [內容安全政策 (CSP) 標頭](/docs/14/app/api-reference/next-config-js/headers#content-security-policy)，可能導致安全漏洞。
...
| `v13.0.6`  | 新增 `ref` 屬性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `v13.0.0`  | `next/image` 導入名稱變更為 `next/legacy/image`。`next/future/image` 導入名稱變更為 `next/image`。提供 [自動重構工具](/docs/14/app/building-your-application/upgrading/codemods#next-image-to-legacy-image) 可安全自動重命名導入。移除 `<span>` 包裹層。移除 `layout`、`objectFit`、`objectPosition`、`lazyBoundary`、`lazyRoot` 屬性。`alt` 成為必填屬性。`onLoadingComplete` 接收 `img` 元素參照。移除內建載入器配置。                                                                                        |
