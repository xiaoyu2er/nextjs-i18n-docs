...
這會自動將 `process.env.DB_HOST`、`process.env.DB_USER` 和 `process.env.DB_PASS` 載入 Node.js 環境，讓您可以在 [Next.js 資料獲取方法](/docs/14/pages/building-your-application/data-fetching) 和 [API 路由](/docs/14/pages/building-your-application/routing/api-routes) 中使用。

例如，使用 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)：
...
> 這會自動將 `process.env.DB_HOST`、`process.env.DB_USER` 和 `process.env.DB_PASS` 載入 Node.js 環境，讓您可以在 [路由處理器](/docs/14/app/building-your-application/routing/route-handlers) 中使用。
...
要讀取執行時環境變數，我們建議使用 `getServerSideProps` 或 [逐步採用 App 路由器](/docs/14/app/building-your-application/upgrading/app-router-migration)。使用 App 路由器時，我們可以在動態渲染期間安全地在伺服器上讀取環境變數。這讓您可以使用一個單一的 Docker 映像，在多個具有不同值的環境中推廣。
...
- 您可以使用 [`register` 函式](/docs/14/app/building-your-application/optimizing/instrumentation) 在伺服器啟動時執行程式碼。
- 我們不建議使用 [runtimeConfig](/docs/14/pages/api-reference/next-config-js/runtime-configuration) 選項，因為這不適用於獨立輸出模式。相反，我們建議 [逐步採用](/docs/14/app/building-your-application/upgrading/app-router-migration) App 路由器。
...
- 如果您使用 [`/src` 目錄](/docs/14/app/building-your-application/configuring/src-directory)，`.env.*` 檔案應該保留在專案的根目錄中。
