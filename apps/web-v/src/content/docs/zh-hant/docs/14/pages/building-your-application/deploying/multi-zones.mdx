...
定義新區域不需要特殊 API。一個區域就是一個普通的 Next.js 應用程式，您只需配置 [basePath](/docs/14/app/api-reference/next-config-js/basePath) 以避免與其他區域的頁面和靜態檔案發生衝突。
...
如果區域提供的頁面沒有共同的路徑前綴（例如 `/home` 和 `/blog`），您也可以設定 [`assetPrefix`](/docs/14/app/api-reference/next-config-js/assetPrefix)，確保該區域的所有 Next.js 資源都在唯一的路徑前綴下提供，而不需要為應用程式中的其他路由新增路徑前綴。

## 如何將請求路由到正確的區域

設定多區域後，您需要將路徑路由到正確的區域，因為它們由不同的應用程式提供服務。您可以使用任何 HTTP 代理來實現這一點，但也可以使用其中一個 Next.js 應用程式來路由整個網域的請求。

要使用 Next.js 應用程式路由到正確的區域，您可以使用 [`rewrites`](/docs/14/app/api-reference/next-config-js/rewrites)。對於由不同區域提供的每個路徑，您可以新增重寫規則，將該路徑發送到其他區域的網域。例如：
...
連結到不同區域的路徑時，應使用 `a` 標籤而非 Next.js 的 [`<Link>`](/docs/14/pages/api-reference/components/link) 元件。這是因為 Next.js 會嘗試預取並對 `<Link>` 元件中的任何相對路徑執行軟導航，這在跨區域時將無法運作。
