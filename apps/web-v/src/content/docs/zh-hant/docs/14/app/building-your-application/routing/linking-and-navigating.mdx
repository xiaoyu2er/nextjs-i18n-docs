...
- 使用 [`useRouter` 鉤子](#userouter-hook) ([客戶端元件](/docs/14/app/building-your-application/rendering/client-components))
- 使用 [`redirect` 函式](#redirect-function) ([伺服器元件](/docs/14/app/building-your-application/rendering/server-components))
...
你還可以傳遞其他可選屬性給 `<Link>`。詳見 [API 參考文件](/docs/14/app/api-reference/components/link)。

### 範例

#### 連結到動態區段

當連結到 [動態區段](/docs/14/app/building-your-application/routing/dynamic-routes) 時，你可以使用 [模板字面量和插值](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals) 來生成連結列表。例如，生成部落格文章列表：
...
你可以使用 [`usePathname()`](/docs/14/app/api-reference/functions/use-pathname) 來判斷連結是否處於活動狀態。例如，為活動連結添加類別時，可以檢查當前的 `pathname` 是否與連結的 `href` 相符：
...
> **須知**：
>
> - 如果頁面在導航時不在視窗的可見範圍內，Next.js 會滾動到 [頁面](/docs/14/app/building-your-application/routing/pages-and-layouts#pages)。
...
`useRouter` 鉤子允許你從 [客戶端元件](/docs/14/app/building-your-application/rendering/client-components) 中以程式方式變更路由。
...
完整的 `useRouter` 方法列表，請參閱 [API 參考文件](/docs/14/app/api-reference/functions/use-router)。

> **建議**：除非有特定需求，否則請使用 `<Link>` 元件在路由之間導航。

## `redirect` 函式

對於 [伺服器元件](/docs/14/app/building-your-application/rendering/server-components)，請改用 `redirect` 函式。
...
> - 如果你想在渲染過程之前進行重定向，請使用 [`next.config.js`](/docs/14/app/building-your-application/routing/redirecting#redirects-in-nextconfigjs) 或 [中介軟體](/docs/14/app/building-your-application/routing/redirecting#nextresponseredirect-in-middleware)。

更多資訊請參閱 [`redirect` API 參考文件](/docs/14/app/api-reference/functions/redirect)。
...
`pushState` 和 `replaceState` 呼叫會與 Next.js 路由器整合，讓你與 [`usePathname`](/docs/14/app/api-reference/functions/use-pathname) 和 [`useSearchParams`](/docs/14/app/api-reference/functions/use-search-params) 同步。
...
[伺服器元件](/docs/14/app/building-your-application/rendering/server-components) 讓你的應用程式程式碼能根據路由區段自動進行程式碼分割。這意味著導航時只會載入當前路由所需的程式碼。
...
`<Link>` 的預設預取行為（即未指定 `prefetch` 屬性或設為 `null`）會根據 [`loading.js`](/docs/14/app/api-reference/file-conventions/loading) 的使用情況而有所不同。只有共享的佈局，直到第一個 `loading.js` 文件的渲染「樹」中的元件會被預取並快取 `30s`。這減少了獲取整個動態路由的成本，意味著你可以顯示 [即時載入狀態](/docs/14/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states)，為使用者提供更好的視覺反饋。

你可以通過將 `prefetch` 屬性設為 `false` 來禁用預取。或者，你也可以通過將 `prefetch` 屬性設為 `true` 來預取超出載入邊界的完整頁面資料。

更多資訊請參閱 [`<Link>` API 參考文件](/docs/14/app/api-reference/components/link)。
...
Next.js 有一個稱為 [路由器快取 (Router Cache)](/docs/14/app/building-your-application/caching#router-cache) 的 **記憶體客戶端快取**。當使用者在應用程式中導航時，[預取](#2-prefetching) 的路由區段和已訪問路由的 React 伺服器元件負載會被儲存在快取中。

這意味著在導航時，會盡可能重用快取，而不是向伺服器發送新請求 — 通過減少請求和傳輸的資料量來提升效能。

了解更多關於 [路由器快取](/docs/14/app/building-your-application/caching#router-cache) 的運作方式和如何配置它。
...
預設情況下，Next.js 會在向後和向前導航時保持滾動位置，並重用 [路由器快取](/docs/14/app/building-your-application/caching#router-cache) 中的路由區段。
