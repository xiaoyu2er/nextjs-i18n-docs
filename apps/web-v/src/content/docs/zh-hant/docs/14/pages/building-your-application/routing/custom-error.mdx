...
> **須知**：如果你需要在建置時獲取資料，可以在這個頁面中使用 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props)。
...
- `Error` 目前不支援 Next.js 的 [資料獲取方法](/docs/14/pages/building-your-application/data-fetching) 如 [`getStaticProps`](/docs/14/pages/building-your-application/data-fetching/get-static-props) 或 [`getServerSideProps`](/docs/14/pages/building-your-application/data-fetching/get-server-side-props)。
- `_error` 和 `_app` 一樣是保留的路徑名稱。`_error` 用於定義錯誤頁面的自訂佈局和行為。直接透過 [路由](/docs/14/pages/building-your-application/routing) 存取或透過 [自訂伺服器](/docs/14/pages/building-your-application/configuring/custom-server) 渲染時，`/_error` 會顯示 404 頁面。