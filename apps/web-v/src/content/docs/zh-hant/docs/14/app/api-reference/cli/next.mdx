...
translation-updated-at: 2025-05-25T21:12:38.715Z
title: next CLI 指令
description: 了解如何使用 Next.js CLI 來執行和建構您的應用程式。
sidebar:
  order: 20206
---
...
| [`build`](#next-build-options)        | 為您的應用程式建立優化的生產環境建構版本，並顯示每個路由的相關資訊。                                                                                                                                                        |
| [`start`](#next-start-options)        | 以生產模式啟動 Next.js。應用程式應先使用 `next build` 進行編譯。                                                                                                                                                             |
| [`info`](next-info-options)           | 列印當前系統的相關詳細資訊，可用於回報 Next.js 錯誤。                                                                                                                                                                        |
| [`lint`](next-lint-options)           | 為 `/src`、`/app`、`/pages`、`/components` 和 `/lib` 目錄中的所有檔案執行 ESLint。如果您的應用程式中尚未配置 ESLint，它還會提供引導式設定來安裝所需的依賴項。                                                                |
| [`telemetry`](next-telemetry-options) | 允許您啟用或禁用 Next.js 完全匿名的遙測資料收集功能。                                                                                                                                                                        |
...
| `--turbo`                                | 使用 [Turbopack](https://nextjs.org/docs/architecture/turbopack) 啟動開發模式。                                                           |
| `-p` 或 `--port <port>`                  | 指定啟動應用程式的埠號。預設值：3000，環境變數：PORT                                                                    |
| `-H`或 `--hostname <hostname>`           | 指定啟動應用程式的主機名稱。適用於讓網路上的其他裝置存取應用程式。預設值：0.0.0.0 |
| `--experimental-https`                   | 使用 HTTPS 啟動伺服器並生成自簽署憑證。                                                                                |
...
這些數值皆為 [**使用 gzip 壓縮後**](/docs/14/app/api-reference/next-config-js/compress) 的大小。首次載入效能會以綠色、黃色或紅色標示。建議保持綠色以獲得最佳效能。
...
| `-h, --help`                       | 顯示所有可用選項。                                                                                                                   |
| `[directory]`                      | 指定建構應用程式的目錄。若未提供，則使用當前目錄。                                           |
| `-d` 或 `--debug`                  | 啟用更詳細的建構輸出。啟用此標記後，會顯示額外的建構資訊，如重寫、重新導向和標頭。      |
|                                    |
| `--profile`                        | 啟用 React 的生產環境 [效能分析功能](https://react.dev/reference/react/Profiler)。                                                         |
| `--no-lint`                        | 停用程式碼檢查。                                                                                                                             |
| `--no-mangling`                    | 停用 [名稱改編 (Name mangling)](https://en.wikipedia.org/wiki/Name_mangling)。這可能會影響效能，僅建議用於除錯目的。 |
| `--experimental-app-only`          | 僅建構應用程式路由器 (App Router) 的路由。                                                                                                                |
...
`next info` 列印當前系統的相關詳細資訊，可在提交 [GitHub 問題](https://github.com/vercel/next.js/issues) 時用於回報 Next.js 錯誤。這些資訊包含作業系統平台/架構/版本、二進位檔 (Node.js、npm、Yarn、pnpm)、套件版本 (`next`、`react`、`react-dom`) 等。
...
`next lint` 為 `pages/`、`app/`、`components/`、`lib/` 和 `src/` 目錄中的所有檔案執行 ESLint。如果您的應用程式中尚未配置 ESLint，它還會提供引導式設定來安裝所需的依賴項。
...
| `--strict`                                            | 使用 Next.js 嚴格設定建立 `.eslintrc.json` 檔案。                                       |
...
| `--error-on-unmatched-pattern`                        | 當任何檔案模式未匹配時報告錯誤。                                                          |
| `-h, --help`                                          | 顯示此訊息。                                                                                        |

### `next telemetry` 選項

Next.js 會收集**完全匿名**的遙測資料以了解一般使用情況。參與此匿名計畫是自願的，您可以選擇退出。
...
| ------------ | --------------------------------------- |
| `-h, --help` | 顯示所有可用選項。             |
| `--enable`   | 啟用 Next.js 遙測資料收集功能。  |
| `--disable`  | 停用 Next.js 遙測資料收集功能。 |

了解更多關於 [遙測功能](/telemetry)。
...
您也可以透過 `--experimental-https-key` 和 `--experimental-https-cert` 提供自訂的金鑰與憑證。此外，還能選擇性地透過 `--experimental-https-ca` 提供自訂的 CA 憑證。

```bash filename="Terminal"
next dev --experimental-https --experimental-https-key ./certificates/localhost-key.pem --experimental-https-cert ./certificates/localhost.pem
```

`next dev --experimental-https` 僅適用於開發階段，並會透過 [`mkcert`](https://github.com/FiloSottile/mkcert) 建立本地信任的憑證。在正式環境中，請使用由受信任機構核發的正式憑證。

> **須知**：當您部署至 Vercel 時，系統會為您的 Next.js 應用程式[自動設定 HTTPS](https://vercel.com/docs/security/encryption)。

### 設定下游代理伺服器的逾時時間

當您將 Next.js 部署在下游代理伺服器（例如 AWS ELB/ALB 這類負載平衡器）後方時，務必為 Next.js 底層的 HTTP 伺服器設定[保持連線逾時時間](https://nodejs.org/api/http.html#http_server_keepalivetimeout)，且該時間應_大於_下游代理伺服器的逾時設定。否則，一旦某個 TCP 連線達到保持連線的逾時時間，Node.js 便會立即終止該連線，而不會通知下游代理伺服器。這會導致代理伺服器嘗試重複使用已被 Node.js 終止的連線時，發生代理錯誤。
...
您可以將任何 [Node.js 參數](https://nodejs.org/api/cli.html#cli_node_options_options)傳遞給 `next` 指令。例如：
