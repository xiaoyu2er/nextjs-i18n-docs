...
> **須知**
>
> - 預設情況下，生成的圖片會進行[**靜態優化**](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)（在構建時生成並緩存），除非它們使用了[動態函式](/docs/14/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)或未緩存的資料。
> - 您可以在同一檔案中使用 [`generateImageMetadata`](/docs/14/app/api-reference/functions/generate-image-metadata) 生成多個圖片。

生成圖片最簡單的方法是使用 `next/og` 中的 [ImageResponse](/docs/14/app/api-reference/functions/image-response) API。
...
一個物件，包含從根區段到 `opengraph-image` 或 `twitter-image` 所在區段的[動態路由參數](/docs/14/app/building-your-application/routing/dynamic-routes)物件。
...
`opengraph-image` 和 `twitter-image` 是特殊的[路由處理器](/docs/14/app/building-your-application/routing/route-handlers)，可以使用與頁面和佈局相同的[路由區段配置](/docs/14/app/api-reference/file-conventions/route-segment-config)選項。
...
> **須知**：
> 預設情況下，此生成的圖片會進行[靜態優化](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)。您可以配置個別的 `fetch` [`選項`](/docs/14/app/api-reference/functions/fetch) 或路由區段的[選項](/docs/14/app/api-reference/file-conventions/route-segment-config#revalidate)來改變此行為。
