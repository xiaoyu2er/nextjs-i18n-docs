...
- **Edge 運行環境** 基於 [Web API](/docs/14/app/api-reference/edge)。
...
| ------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------- | ---------------- |
| 冷啟動                                                                                                                             | /      | 一般     | 低              |
| [HTTP 串流](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)                                                | 支援    | 支援        | 支援              |
| I/O                                                                                                                                    | 全部    | 全部        | 僅 `fetch`          |
| 擴展性                                                                                                                           | /      | 高       | 最高          |
| 安全性                                                                                                                              | 一般 | 高       | 高             |
| 延遲                                                                                                                               | 一般 | 低        | 最低           |
| npm 套件                                                                                                                          | 全部    | 全部        | 較小子集 |
| [靜態渲染](/docs/14/app/building-your-application/rendering/server-components#static-rendering-default)                          | 支援    | 支援        | 不支援               |
| [動態渲染](/docs/14/app/building-your-application/rendering/server-components#dynamic-rendering)                                | 支援    | 支援        | 支援              |
| [使用 `fetch` 重新驗證資料](/docs/14/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) | 支援    | 支援        | 支援              |
...
您可以為 Next.js 應用程式中的個別路由區段指定運行環境。為此，[宣告並匯出名為 `runtime` 的變數](/docs/14/app/api-reference/file-conventions/route-segment-config)。該變數必須是字串，且值必須為 `'nodejs'` 或 `'edge'` 運行環境。
...
> 請參考 [Node.js 文件](https://nodejs.org/docs/latest/api/) 和 [Edge 文件](/docs/14/app/api-reference/edge) 以獲取完整的可用 API 列表。根據您的部署基礎設施，兩種運行環境也都支援[串流](/docs/14/app/building-your-application/routing/loading-ui-and-streaming)。