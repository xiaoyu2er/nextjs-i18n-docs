...
> - 如果您是從遷移 `metadata` 匯出而來，可以使用 [metadata-to-viewport-export codemod](/docs/14/app/building-your-application/upgrading/codemods#metadata-to-viewport-export) 來更新您的變更。
...
您可以使用 `Viewport` 型別為您的視窗設定物件增加型別安全性。如果您在 IDE 中使用 [內建的 TypeScript 插件](/docs/14/app/building-your-application/configuring/typescript)，則不需要手動添加型別，但您仍然可以明確添加。
