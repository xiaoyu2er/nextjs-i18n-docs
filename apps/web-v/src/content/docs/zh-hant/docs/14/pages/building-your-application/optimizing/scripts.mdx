...
請參閱 [`next/script`](/docs/14/app/api-reference/components/script#strategy) API 參考文檔以深入了解每種策略及其使用案例。

### 將腳本卸載至 Web Worker（實驗性）

> **警告**：`worker` 策略尚未穩定，且尚不適用於 [`app`](/docs/14/app/building-your-application/routing/defining-routes) 目錄。請謹慎使用。
...
這些處理程序僅在 `next/script` 被導入並在 [客戶端元件](/docs/14/app/building-your-application/rendering/client-components) 中使用時有效，其中 `"use client"` 被定義為代碼的第一行：
...
請參閱 [`next/script`](/docs/14/app/api-reference/components/script#onload) API 參考以深入了解每個事件處理程序並查看範例。
...
請參閱 [`next/script`](/docs/14/pages/api-reference/components/script#onload) API 參考以深入了解每個事件處理程序並查看範例。
