...
translation-updated-at: 2025-05-25T21:09:52.336Z
title: 重新導向 (Redirecting)
description: 了解在 Next.js 中處理重新導向 (redirects) 的不同方式。
...
`redirect` 函式允許您將使用者重新導向至另一個 URL。您可以在[伺服器元件](/docs/14/app/building-your-application/rendering/server-components)、[路由處理器](/docs/14/app/building-your-application/routing/route-handlers)和[伺服器動作](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)中呼叫 `redirect`。
...
詳見 [`redirect` API 參考文件](/docs/14/app/api-reference/functions/redirect)。

## `permanentRedirect` 函式

`permanentRedirect` 函式允許您**永久**重新導向使用者至另一個 URL。您可以在[伺服器元件](/docs/14/app/building-your-application/rendering/server-components)、[路由處理器](/docs/14/app/building-your-application/routing/route-handlers)和[伺服器動作](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)中呼叫 `permanentRedirect`。
...
詳見 [`permanentRedirect` API 參考文件](/docs/14/app/api-reference/functions/permanentRedirect)。
...
> **須知事項**:
>
> - 若無需以程式方式導向使用者，應使用 [`<Link>`](/docs/14/app/api-reference/components/link) 元件。

<AppOnly>

詳見 [`useRouter` API 參考文件](/docs/14/app/api-reference/functions/use-router)。

</AppOnly>

<PagesOnly>

詳見 [`useRouter` API 參考文件](/docs/14/pages/api-reference/functions/use-router)。
...
`redirects` 支援[路徑](/docs/14/app/api-reference/next-config-js/redirects#path-matching)、[標頭、Cookie 和查詢匹配](/docs/14/app/api-reference/next-config-js/redirects#header-cookie-and-query-matching)，讓您能根據傳入請求靈活重新導向使用者。
...
詳見 [`redirects` API 參考文件](/docs/14/app/api-reference/next-config-js/redirects)。

> **須知事項**:
>
> - `redirects` 可透過 `permanent` 選項返回 307 (暫時重新導向) 或 308 (永久重新導向) 狀態碼。
> - 在某些平台上，`redirects` 可能有數量限制。例如在 Vercel 上最多 1,024 個重新導向。若要管理大量重新導向 (1000+)，可考慮使用[中介軟體](/docs/14/app/building-your-application/routing/middleware)建立自訂解決方案。詳見[大規模管理重新導向](#managing-redirects-at-scale-advanced)。
...
詳見[中介軟體](/docs/14/app/building-your-application/routing/middleware)文件。
...
如果存在，則將請求轉發給 <AppOnly>[路由處理器 (Route Handler)](/docs/14/app/building-your-application/routing/route-handlers)</AppOnly> <PagesOnly>[API 路由 (API Routes)](/docs/14/pages/building-your-application/routing/api-routes)</PagesOnly>，該處理器會檢查實際檔案並將使用者重新導向至適當的 URL。這樣可以避免將大型重新導向檔案匯入中介軟體，從而減慢每個傳入請求的速度。
