...
`"use client"` 用於宣告伺服器元件 (Server Components) 和客戶端元件 (Client Components) 模組之間的[邊界 (boundary)](/docs/14/app/building-your-application/rendering#network-boundary)。這表示在檔案中定義 `"use client"` 後，所有導入該檔案的模組（包括子元件）都會被視為客戶端套件的一部分。
...
您可以透過交錯使用客戶端元件 (Client Components) 和伺服器元件 (Server Components) 以及[伺服器操作 (Server Actions)](/docs/14/app/building-your-application/data-fetching/server-actions-and-mutations)，將程式碼保留在伺服器上，即使理論上它嵌套在客戶端元件 (Client Components) 中。詳情請參閱[組合模式 (Composition Patterns)](/docs/14/app/building-your-application/rendering/composition-patterns) 頁面。