...
//#region node_modules/.nitro/vite/services/ssr/assets/parallel-routes-Bpb-VMEC.js
var import_jsx_runtime = /* @__PURE__ */ __toESM(require_jsx_runtime());
var parallel_routes_exports = /* @__PURE__ */ __exportAll({
	default: () => MDXContent,
...
var frontmatter = {
	...
	"related": { "links": ["app/api-reference/file-conventions/default"] }
};
var structuredData = {
	"contents": [
		{
		...
		{
			"heading": void 0,
			"content": "For example, considering a dashboard, you can use parallel routes to simultaneously render the `team` and `analytics` pages:"
		},
		{
			"heading": "slots",
			"content": "Parallel routes are created using named **slots**. Slots are defined with the `@folder` convention. For example, the following file structure defines two slots: `@analytics` and `@team`:"
		},
		{
			"heading": "slots",
			"content": "Slots are passed as props to the shared parent layout. For the example above, the component in `app/layout.js` now accepts the `@analytics` and `@team` slots props, and can render them in parallel alongside the `children` prop:"
		},
		{
			"heading": "slots",
			"content": "However, slots are **not** route segments and do not affect the URL structure. For example, for `/@analytics/views`, the URL will be `/views` since `@analytics` is a slot. Slots are combined with the regular Page component to form the final page associated with the route segment. Because of this, you cannot have separate static and dynamic slots at the same route segment level. If one slot is dynamic, all slots at that level must be dynamic."
		},
		{
			"heading": "slots",
		...
		{
			"heading": "defaultjs",
			"content": "You can define a `default.js` file to render as a fallback for unmatched slots during the initial load or full-page reload."
		},
		{
			"heading": "defaultjs",
		...
		{
			"heading": "behavior",
			"content": "By default, Next.js keeps track of the active *state* (or subpage) for each slot. However, the content rendered within a slot will depend on the type of navigation:"
		},
		{
			"heading": "behavior",
			"content": "**Soft Navigation**: During client-side navigation, Next.js will perform a partial render, changing the subpage within the slot, while maintaining the other slot's active subpages, even if they don't match the current URL."
		},
		{
			"heading": "behavior",
			"content": "**Hard Navigation**: After a full-page load (browser refresh), Next.js cannot determine the active state for the slots that don't match the current URL. Instead, it will render a `default.js` file for the unmatched slots, or `404` if `default.js` doesn't exist."
		},
		{
			"heading": "behavior",
		...
		{
			"heading": "with-useselectedlayoutsegments",
			"content": "Both `useSelectedLayoutSegment` and `useSelectedLayoutSegments` accept a `parallelRoutesKey` parameter, which allows you to read the active route segment within a slot."
		},
		{
			"heading": "with-useselectedlayoutsegments",
			"content": "When a user navigates to `app/@auth/login` (or `/login` in the URL bar), `loginSegment` will be equal to the string `\"login\"`."
		},
		{
			"heading": "conditional-routes",
			"content": "You can use Parallel Routes to conditionally render routes based on certain conditions, such as user role. For example, to render a different dashboard page for the `/admin` or `/user` roles:"
		},
		{
			"heading": "tab-groups",
			"content": "You can add a `layout` inside a slot to allow users to navigate the slot independently. This is useful for creating tabs."
		},
		{
			"heading": "tab-groups",
			"content": "For example, the `@analytics` slot has two subpages: `/page-views` and `/visitors`."
		},
		{
			"heading": "tab-groups",
			"content": "Within `@analytics`, create a `layout` file to share the tabs between the two pages:"
		},
		{
			"heading": "modals",
			"content": "Parallel Routes can be used together with Intercepting Routes to create modals that support deep linking. This allows you to solve common challenges when building modals, such as:"
		},
		{
			"heading": "modals",
		...
		{
			"heading": "modals",
			"content": "Consider the following UI pattern, where a user can open a login modal from a layout using client-side navigation, or access a separate `/login` page:"
		},
		{
			"heading": "modals",
			"content": "To implement this pattern, start by creating a `/login` route that renders your **main** login page."
		},
		{
			"heading": "modals",
			"content": "Then, inside the `@auth` slot, add `default.js` file that returns `null`. This ensures that the modal is not rendered when it's not active."
		},
		{
			"heading": "modals",
			"content": "Inside your `@auth` slot, intercept the `/login` route by updating the `/(.)login` folder. Import the `<Modal>` component and its children into the `/(.)login/page.tsx` file:"
		},
		{
			"heading": "modals",
		...
		},
		{
			...
			"content": "Now, you can leverage the Next.js router to open and close the modal. This ensures the URL is correctly updated when the modal is open, and when navigating backwards and forwards."
		},
		{
			"heading": "opening-the-modal",
			"content": "To open the modal, pass the `@auth` slot as a prop to the parent layout and render it alongside the `children` prop."
		},
		{
			"heading": "opening-the-modal",
			"content": "When the user clicks the `<Link>`, the modal will open instead of navigating to the `/login` page. However, on refresh or initial load, navigating to `/login` will take the user to the main login page."
		},
		{
			"heading": "closing-the-modal",
			"content": "You can close the modal by calling `router.back()` or by using the `Link` component."
		},
		{
			"heading": "closing-the-modal",
			"content": "When using the `Link` component to navigate away from a page that shouldn't render the `@auth` slot anymore, we need to make sure the parallel route matches to a component that returns `null`. For example, when navigating back to the root page, we create a `@auth/page.tsx` component:"
		},
		{
			"heading": "closing-the-modal",
			"content": "Or if navigating to any other page (such as `/foo`, `/foo/bar`, etc), you can use a catch-all slot:"
		},
		{
			"heading": "closing-the-modal",
		...
		},
		{
			...
			"content": "Parallel Routes can be streamed independently, allowing you to define independent error and loading states for each route:"
		},
		{
			"heading": "loading-and-error-ui",
			"content": "See the Loading UI and Error Handling documentation for more information."
		}
	],
	"headings": [
		{
		...
		},
		{
			...
			"content": "With `useSelectedLayoutSegment(s)`"
		},
		{
...
var toc = [
	...
	},
	{
		depth: 3,
		url: "#slots",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Slots" })
	},
	{
		depth: 3,
		url: "#defaultjs",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)("code", { children: "default.js" }) })
	},
	{
		depth: 2,
		url: "#behavior",
	...
	{
		depth: 3,
		url: "#with-useselectedlayoutsegments",
		title: (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: ["With ", (0, import_jsx_runtime.jsx)("code", { children: "useSelectedLayoutSegment(s)" })] })
	},
	{
		depth: 3,
		url: "#conditional-routes",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Conditional Routes" })
	},
	{
		depth: 3,
		url: "#tab-groups",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Tab Groups" })
	},
	{
		depth: 3,
		url: "#modals",
	...
	{
		depth: 4,
		url: "#opening-the-modal",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Opening the modal" })
	},
	{
		depth: 4,
		url: "#closing-the-modal",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Closing the modal" })
	},
	{
		depth: 3,
		url: "#loading-and-error-ui",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Loading and Error UI" })
	}
];
function _createMdxContent(props) {
	...
	return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		(0, import_jsx_runtime.jsx)(_components.p, { children: "Parallel Routes allows you to simultaneously or conditionally render one or more pages within the same layout. They are useful for highly dynamic sections of an app, such as dashboards and feeds on social sites." }),
		"\n",
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/@analytics/views" }),
			", the URL will be ",
		...
		(0, import_jsx_runtime.jsxs)(_components.blockquote, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
				...
				(0, import_jsx_runtime.jsxs)(_components.li, { children: [
					...
					(0, import_jsx_runtime.jsx)(_components.code, { children: "app/page.js" }),
					" is equivalent to ",
					(0, import_jsx_runtime.jsx)(_components.code, { children: "app/@children/page.js" }),
					"."
				] }),
				"\n"
			] }),
		...
		(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
				(0, import_jsx_runtime.jsx)(_components.a, {
					...
					children: (0, import_jsx_runtime.jsx)(_components.strong, { children: "Soft Navigation" })
				}),
				": During client-side navigation, Next.js will perform a ",
				(0, import_jsx_runtime.jsx)(_components.a, {
					href: "/docs/app/getting-started/linking-and-navigating#client-side-transitions",
					children: "partial render"
				}),
			...
			"\n",
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
				(0, import_jsx_runtime.jsx)(_components.strong, { children: "Hard Navigation" }),
		...
		(0, import_jsx_runtime.jsxs)(_components.h3, {
			id: "with-useselectedlayoutsegments",
			children: ["With ", (0, import_jsx_runtime.jsx)(_components.code, { children: "useSelectedLayoutSegment(s)" })]
		}),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			"Both ",
			...
			" accept a ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "parallelRoutesKey" }),
							...
							(0, import_jsx_runtime.jsxs)(_components.span, {
								...
								children: [
									...
									(0, import_jsx_runtime.jsx)(_components.span, {
										...
										children: " { useSelectedLayoutSegment } "
									}),
									(0, import_jsx_runtime.jsx)(_components.span, {
										style: {
											"--shiki-light": "#D73A49",
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "app/@auth/login" }),
			" (or ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/login" }),
			" in the URL bar), ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "loginSegment" }),
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/admin" }),
			" or ",
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/page-views" }),
			" and ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/visitors" }),
			"."
		] }),
		"\n",
		(0, import_jsx_runtime.jsx)(Image, {
		...
		"\n",
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			defaultValue: "app/@analytics/layout.tsx",
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@analytics/layout.tsx",
					children: "app/@analytics/layout.tsx"
				}), (0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@analytics/layout.js",
					children: "app/@analytics/layout.js"
				})] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/@analytics/layout.tsx",
					children: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(_components.pre, {
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.a, {
				href: "/docs/app/api-reference/file-conventions/intercepting-routes",
		...
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			...
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					...
					value: "app/login/page.js",
					children: "app/login/page.js"
				})] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/login/page.tsx",
					children: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(_components.pre, {
		...
		"\n",
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			defaultValue: "app/@auth/default.tsx",
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/default.tsx",
					children: "app/@auth/default.tsx"
				}), (0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/default.js",
					children: "app/@auth/default.js"
				})] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/@auth/default.tsx",
					children: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(_components.pre, {
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			" slot, intercept the ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/login" }),
			" route by updating the ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/(.)login" }),
			" folder. Import the ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "<Modal>" }),
			" component and its children into the ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/(.)login/page.tsx" }),
			" file:"
		] }),
		"\n",
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			defaultValue: "app/@auth/(.)login/page.tsx",
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/(.)login/page.tsx",
					children: "app/@auth/(.)login/page.tsx"
				}), (0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/(.)login/page.js",
					children: "app/@auth/(.)login/page.js"
				})] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/@auth/(.)login/page.tsx",
					children: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(_components.pre, {
		...
		(0, import_jsx_runtime.jsxs)(_components.blockquote, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
				...
				(0, import_jsx_runtime.jsxs)(_components.li, { children: [
					...
					", depends on your file-system structure. See ",
					(0, import_jsx_runtime.jsx)(_components.a, {
						href: "/docs/app/api-reference/file-conventions/intercepting-routes#convention",
						children: "Intercepting Routes convention"
					}),
					"."
				] }),
				"\n",
				(0, import_jsx_runtime.jsxs)(_components.li, { children: [
					...
					(0, import_jsx_runtime.jsx)(_components.a, {
						href: "/docs/app/getting-started/server-and-client-components#examples#supported-pattern-passing-server-components-to-client-components-as-props",
						children: "Interleaving Client and Server Components"
					}),
		...
		(0, import_jsx_runtime.jsx)(_components.h4, {
			id: "opening-the-modal",
			children: "Opening the modal"
		}),
		"\n",
		(0, import_jsx_runtime.jsx)(_components.p, { children: "Now, you can leverage the Next.js router to open and close the modal. This ensures the URL is correctly updated when the modal is open, and when navigating backwards and forwards." }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
		...
		(0, import_jsx_runtime.jsx)(_components.h4, {
			id: "closing-the-modal",
			children: "Closing the modal"
		}),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			"You can close the modal by calling ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "router.back()" }),
			" or by using the ",
			(0, import_jsx_runtime.jsx)(_components.code, { children: "Link" }),
			" component."
		] }),
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "@auth/page.tsx" }),
			" component:"
		] }),
		"\n",
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			...
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [
					...
					(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
						value: "app/@auth/page.tsx",
						children: "app/@auth/page.tsx"
					}),
					(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
						value: "app/@auth/page.js",
						children: "app/@auth/page.js"
					})
				] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/ui/modal.tsx",
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "/foo" }),
			", ",
		...
		"\n",
		(0, import_jsx_runtime.jsxs)(CodeBlockTabs, {
			defaultValue: "app/@auth/[...catchAll]/page.tsx",
			children: [
				(0, import_jsx_runtime.jsxs)(CodeBlockTabsList, { children: [(0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/[...catchAll]/page.tsx",
					children: "app/@auth/[...catchAll]/page.tsx"
				}), (0, import_jsx_runtime.jsx)(CodeBlockTabsTrigger, {
					value: "app/@auth/[...catchAll]/page.js",
					children: "app/@auth/[...catchAll]/page.js"
				})] }),
				(0, import_jsx_runtime.jsx)(CodeBlockTab, {
					value: "app/@auth/[...catchAll]/page.tsx",
					children: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(_components.pre, {
		...
		(0, import_jsx_runtime.jsxs)(_components.blockquote, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
				...
				(0, import_jsx_runtime.jsxs)(_components.li, { children: [
					...
					(0, import_jsx_runtime.jsx)(_components.code, { children: "@auth" }),
					" slot to close the modal because of how parallel routes behave(#behavior). Since client-side navigations to a route that no longer match the slot will remain visible, we need to match the slot to a route that returns ",
					(0, import_jsx_runtime.jsx)(_components.code, { children: "null" }),
					" to close the modal."
				] }),
				"\n",
		...
		(0, import_jsx_runtime.jsx)(_components.h3, {
			id: "loading-and-error-ui",
			children: "Loading and Error UI"
		}),
		"\n",
		(0, import_jsx_runtime.jsx)(_components.p, { children: "Parallel Routes can be streamed independently, allowing you to define independent error and loading states for each route:" }),
		"\n",
		(0, import_jsx_runtime.jsx)(Image, {
...
//#endregion
export { toc as a, structuredData as i, frontmatter as n, parallel_routes_exports as r, MDXContent as t };
