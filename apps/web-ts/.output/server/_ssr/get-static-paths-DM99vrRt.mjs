...
//#region node_modules/.nitro/vite/services/ssr/assets/get-static-paths-DM99vrRt.js
var import_jsx_runtime = /* @__PURE__ */ __toESM(require_jsx_runtime());
var get_static_paths_exports = /* @__PURE__ */ __exportAll({
	default: () => MDXContent,
...
var structuredData = {
	"contents": [
		...
		{
			"heading": "getstaticpaths-return-values",
			"content": "The `getStaticPaths` function should return an object with the following **required** properties:"
		},
		{
			"heading": "paths",
		...
		{
			"heading": "paths",
			"content": "Then, Next.js will statically generate `/posts/1` and `/posts/2` during `next build` using the page component in `pages/posts/[id].js`."
		},
		{
			"heading": "paths",
			"content": "The value for each `params` object must match the parameters used in the page name:"
		},
		{
			"heading": "paths",
		...
		{
			"heading": "paths",
			"content": "If the page name uses catch-all routes like `pages/[...slug]`, then `params` should contain `slug` (which is an array). If this array is `['hello', 'world']`, then Next.js will statically generate the page at `/hello/world`."
		},
		{
			"heading": "paths",
			"content": "If the page uses an optional catch-all route, use `null`, `[]`, `undefined` or `false` to render the root-most route. For example, if you supply `slug: false` for `pages/[[...slug]]`, Next.js will statically generate the page `/`."
		},
		{
			"heading": "paths",
		...
		{
			"heading": "paths",
			"content": "Separate of the `params` object a `locale` field can be returned when i18n is configured, which configures the locale for the path being generated."
		},
		{
		...
		{
			"heading": "fallback-false",
			"content": "When `next build` is run, Next.js will check if `getStaticPaths` returned `fallback: false`, it will then build **only** the paths returned by `getStaticPaths`. This option is useful if you have a small number of paths to create, or new page data is not added often. If you find that you need to add more paths, and you have `fallback: false`, you will need to run `next build` again so that the new paths can be generated."
		},
		{
			"heading": "fallback-false",
			"content": "The following example pre-renders one blog post per page called `pages/posts/[id].js`. The list of blog posts will be fetched from a CMS and returned by `getStaticPaths`. Then, for each page, it fetches the post data from a CMS using `getStaticProps`."
		},
		{
			"heading": "fallback-true",
			"content": "Static generation of a large number of pages"
		},
		{
			"heading": "fallback-true",
			"content": "If `fallback` is `true`, then the behavior of `getStaticProps` changes in the following ways:"
		},
		{
			"heading": "fallback-true",
		...
		{
			"heading": "fallback-true",
			"content": "The paths that have not been generated at build time will **not** result in a 404 page. Instead, Next.js will serve a “fallback” version of the page on the first request to such a path. Web crawlers, such as Google, won't be served a fallback and instead the path will behave as in `fallback: 'blocking'`."
		},
		{
			"heading": "fallback-true",
			"content": "When a page with `fallback: true` is navigated to through `next/link` or `next/router` (client-side) Next.js will *not* serve a fallback and instead the page will behave as `fallback: 'blocking'`."
		},
		{
			"heading": "fallback-true",
			"content": "In the background, Next.js will statically generate the requested path `HTML` and `JSON`. This includes running `getStaticProps`."
		},
		{
			"heading": "fallback-true",
			"content": "When complete, the browser receives the `JSON` for the generated path. This will be used to automatically render the page with the required props. From the user’s perspective, the page will be swapped from the fallback page to the full page."
		},
		{
			"heading": "fallback-true",
		...
		{
			"heading": "fallback-true",
			"content": "> **Good to know**: `fallback: true` is not supported when using `output: 'export'`."
		},
		{
			"heading": "when-is-fallback-true-useful",
			"content": "`fallback: true` is useful if your app has a very large number of static pages that depend on data (such as a very large e-commerce site). If you want to pre-render all product pages, the builds would take a very long time."
		},
		{
			"heading": "when-is-fallback-true-useful",
			"content": "Instead, you may statically generate a small subset of pages and use `fallback: true` for the rest. When someone requests a page that is not generated yet, the user will see the page with a loading indicator or skeleton component."
		},
		{
			"heading": "when-is-fallback-true-useful",
			"content": "Shortly after, `getStaticProps` finishes and the page will be rendered with the requested data. From now on, everyone who requests the same page will get the statically pre-rendered page."
		},
		{
			"heading": "when-is-fallback-true-useful",
			"content": "This ensures that users always have a fast experience while preserving fast builds and the benefits of Static Generation."
		},
		{
			"heading": "when-is-fallback-true-useful",
			"content": "`fallback: true` will not *update* generated pages, for that take a look at Incremental Static Regeneration."
		},
		{
			"heading": "fallback-blocking",
			"content": "If `fallback` is `'blocking'`, new paths not returned by `getStaticPaths` will wait for the `HTML` to be generated, identical to SSR (hence why *blocking*), and then be cached for future requests so it only happens once per path."
		},
		{
			"heading": "fallback-blocking",
			"content": "`getStaticProps` will behave as follows:"
		},
		{
			"heading": "fallback-blocking",
			"content": "The paths returned from `getStaticPaths` will be rendered to `HTML` at build time by `getStaticProps`."
		...
		{
			"heading": "fallback-blocking",
			"content": "The paths that have not been generated at build time will **not** result in a 404 page. Instead, Next.js will SSR on the first request and return the generated `HTML`."
		},
		{
			"heading": "fallback-blocking",
			"content": "When complete, the browser receives the `HTML` for the generated path. From the user’s perspective, it will transition from \"the browser is requesting the page\" to \"the full page is loaded\". There is no flash of loading/fallback state."
		},
		{
			"heading": "fallback-blocking",
			"content": "At the same time, Next.js adds this path to the list of pre-rendered pages. Subsequent requests to the same path will serve the generated page, like other pages pre-rendered at build time."
		...
		{
			"heading": "fallback-blocking",
			"content": "`fallback: 'blocking'` will not *update* generated pages by default. To update generated pages, use Incremental Static Regeneration in conjunction with `fallback: 'blocking'`."
		},
		{
			"heading": "fallback-blocking",
			"content": "> **Good to know**: `fallback: 'blocking'` is not supported when using `output: 'export'`."
		},
		{
			"heading": "fallback-pages",
			"content": "In the “fallback” version of a page:"
		},
		{
			"heading": "fallback-pages",
			"content": "The page’s props will be empty."
		},
		{
			"heading": "fallback-pages",
			"content": "Using the router, you can detect if the fallback is being rendered, `router.isFallback` will be `true`."
		},
		{
			"heading": "fallback-pages",
			"content": "The following example showcases using `isFallback`:"
		},
		{
		...
		{
			"heading": "version-history",
			"content": "App Router is now stable with simplified data fetching, including `generateStaticParams()`"
		},
		{
			"heading": "version-history",
			"content": "`v12.2.0`"
		...
		{
			"heading": "version-history",
			"content": "`getStaticPaths` introduced."
		}
	],
	"headings": [
		{
...
var toc = [
	{
		depth: 2,
		url: "#getstaticpaths-return-values",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "getStaticPaths return values" })
	},
	{
		depth: 3,
		url: "#paths",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)("code", { children: "paths" }) })
	},
	{
		depth: 3,
		url: "#fallback-false",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)("code", { children: "fallback: false" }) })
	},
	{
		depth: 3,
		url: "#fallback-true",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)("code", { children: "fallback: true" }) })
	},
	{
		depth: 4,
		url: "#when-is-fallback-true-useful",
		title: (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
			"When is ",
			(0, import_jsx_runtime.jsx)("code", { children: "fallback: true" }),
			" useful?"
		] })
	},
	{
		depth: 3,
		url: "#fallback-blocking",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)("code", { children: "fallback: 'blocking'" }) })
	},
	{
		depth: 3,
		url: "#fallback-pages",
		title: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: "Fallback pages" })
	},
	{
		depth: 2,
		url: "#version-history",
...
function _createMdxContent(props) {
	...
	return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		...
		(0, import_jsx_runtime.jsx)(_components.h2, {
			id: "getstaticpaths-return-values",
			children: "getStaticPaths return values"
		}),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			"The ",
		...
		(0, import_jsx_runtime.jsx)(_components.h3, {
			id: "paths",
			children: (0, import_jsx_runtime.jsx)(_components.code, { children: "paths" })
		}),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			"The ",
			...
			children: (0, import_jsx_runtime.jsxs)(_components.code, { children: [
				...
				(0, import_jsx_runtime.jsx)(_components.span, {
					...
					children: (0, import_jsx_runtime.jsx)(_components.span, {
						...
						children: "      // with i18n configured the locale for the path can be returned as well"
					})
				}),
				"\n",
				(0, import_jsx_runtime.jsxs)(_components.span, {
		...
		(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
				...
				(0, import_jsx_runtime.jsx)(_components.code, { children: "slug: false" }),
				" for ",
				(0, import_jsx_runtime.jsx)(_components.code, { children: "pages/[[...slug]]" }),
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.code, { children: "params" }),
			" strings are ",
			(0, import_jsx_runtime.jsx)(_components.strong, { children: "case-sensitive" }),
		...
		(0, import_jsx_runtime.jsx)(_components.h3, {
			id: "fallback-true",
			children: (0, import_jsx_runtime.jsx)(_components.code, { children: "fallback: true" })
		}),
		"\n",
		(0, import_jsx_runtime.jsxs)("details", { children: [(0, import_jsx_runtime.jsx)("summary", { children: "Examples" }), (0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.li, { children: (0, import_jsx_runtime.jsx)(_components.a, {
				...
				children: "Static generation of a large number of pages"
			}) }),
			"\n"
		] })] }),
		"\n",
		...
		(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
				...
				(0, import_jsx_runtime.jsx)(_components.a, {
					href: "#fallback-blocking",
					children: (0, import_jsx_runtime.jsx)(_components.code, { children: "fallback: 'blocking'" })
				}),
				"."
			] }),
			"\n",
			...
			"\n",
			(0, import_jsx_runtime.jsx)(_components.li, { children: "At the same time, Next.js adds this path to the list of pre-rendered pages. Subsequent requests to the same path will serve the generated page, like other pages pre-rendered at build time." }),
			"\n"
		] }),
		"\n",
		...
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [(0, import_jsx_runtime.jsx)(_components.code, { children: "fallback: true" }), " is useful if your app has a very large number of static pages that depend on data (such as a very large e-commerce site). If you want to pre-render all product pages, the builds would take a very long time."] }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
		...
		"\n",
		(0, import_jsx_runtime.jsx)(_components.p, { children: "This ensures that users always have a fast experience while preserving fast builds and the benefits of Static Generation." }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			(0, import_jsx_runtime.jsx)(_components.code, { children: "fallback: true" }),
			" will not ",
			(0, import_jsx_runtime.jsx)(_components.em, { children: "update" }),
		...
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [
			...
			(0, import_jsx_runtime.jsx)(_components.em, { children: "blocking" }),
			"), and then be cached for future requests so it only happens once per path."
		] }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.p, { children: [(0, import_jsx_runtime.jsx)(_components.code, { children: "getStaticProps" }), " will behave as follows:"] }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			"\n",
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
		...
		(0, import_jsx_runtime.jsx)(_components.h3, {
			id: "fallback-pages",
			children: "Fallback pages"
		}),
		"\n",
		(0, import_jsx_runtime.jsx)(_components.p, { children: "In the “fallback” version of a page:" }),
		"\n",
		(0, import_jsx_runtime.jsxs)(_components.ul, { children: [
			"\n",
			(0, import_jsx_runtime.jsx)(_components.li, { children: "The page’s props will be empty." }),
			"\n",
			(0, import_jsx_runtime.jsxs)(_components.li, { children: [
				"Using the ",
				(0, import_jsx_runtime.jsx)(_components.a, {
				...
				(0, import_jsx_runtime.jsx)(_components.code, { children: "router.isFallback" }),
				" will be ",
				(0, import_jsx_runtime.jsx)(_components.code, { children: "true" }),
				"."
			] }),
			...
			children: (0, import_jsx_runtime.jsxs)(_components.code, { children: [
				...
				(0, import_jsx_runtime.jsx)(_components.span, {
					...
					children: (0, import_jsx_runtime.jsx)(_components.span, {
						...
						children: "    // Re-generate the post at most once per second"
					})
				}),
				"\n",
				(0, import_jsx_runtime.jsx)(_components.span, {
					...
					children: (0, import_jsx_runtime.jsx)(_components.span, {
						...
						children: "    // if a request comes in"
					})
				}),
				"\n",
				(0, import_jsx_runtime.jsxs)(_components.span, {
		...
		(0, import_jsx_runtime.jsxs)(_components.table, { children: [(0, import_jsx_runtime.jsx)(_components.thead, { children: (0, import_jsx_runtime.jsxs)(_components.tr, { children: [(0, import_jsx_runtime.jsx)(_components.th, { children: "Version" }), (0, import_jsx_runtime.jsx)(_components.th, { children: "Changes" })] }) }), (0, import_jsx_runtime.jsxs)(_components.tbody, { children: [
			...
			(0, import_jsx_runtime.jsxs)(_components.tr, { children: [(0, import_jsx_runtime.jsx)(_components.td, { children: (0, import_jsx_runtime.jsx)(_components.code, { children: "v9.3.0" }) }), (0, import_jsx_runtime.jsxs)(_components.td, { children: [(0, import_jsx_runtime.jsx)(_components.code, { children: "getStaticPaths" }), " introduced."] })] })
		] })] })
	] });
}
...
//#endregion
export { toc as a, structuredData as i, frontmatter as n, get_static_paths_exports as r, MDXContent as t };
