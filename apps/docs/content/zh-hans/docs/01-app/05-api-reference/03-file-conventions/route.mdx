...
translation-updated-at: 2025-05-20T22:26:27.197Z
title: route.js
...
**route** 文件允许您为指定路由创建自定义请求处理器。支持以下 [HTTP 方法](https://developer.mozilla.org/docs/Web/HTTP/Methods)：`GET`、`POST`、`PUT`、`PATCH`、`DELETE`、`HEAD` 和 `OPTIONS`。
...
- **`params`**: 一个解析为包含当前路由 [动态路由参数](/docs/app/building-your-application/routing/dynamic-routes) 对象的 Promise。
...
| `v13.2.0`    | 路由处理器 (Route Handlers) 功能引入。                                                             |