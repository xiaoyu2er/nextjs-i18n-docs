...
translation-updated-at: 2025-05-20T23:01:26.471Z
title: opengraph-image 与 twitter-image
...
当用户在社交网络或消息应用中分享您网站的链接时，这些图片会显示出来。
...
Next.js 会评估该文件，并自动将适当的标签添加到应用的 `<head>` 元素中。
...
除了使用[图片文件](#image-files-jpg-png-gif)，您还可以通过代码**生成**图片。
...
> - 您可以使用 [`generateImageMetadata`](/docs/app/api-reference/functions/generate-image-metadata) 在同一个文件中生成多个图片。
> - `opengraph-image.js` 和 `twitter-image.js` 是特殊的路由处理器，默认会缓存，除非使用了[动态 API](/docs/app/deep-dive/caching#dynamic-apis) 或[动态配置](/docs/app/deep-dive/caching#segment-config-options)选项。
