...
translation-updated-at: 2025-05-20T22:26:38.795Z
title: cookies
description: 关于 cookies 函数的 API 参考文档。
---

`cookies` 是一个**异步**函数，允许您在[服务端组件 (Server Components)](/docs/app/getting-started/server-and-client-components) 中读取 HTTP 传入请求的 cookies，并在[服务端操作 (Server Actions)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) 或[路由处理器 (Route Handlers)](/docs/app/building-your-application/routing/route-handlers) 中读取/写入传出请求的 cookies。
...
| `partitioned`   | 布尔值                                 | 指示 cookie 是否[分区 (partitioned)](https://github.com/privacycg/CHIPS) |
...
- `cookies` 是一个**异步**函数，返回一个 Promise。您必须使用 `async/await` 或 React 的 [`use`](https://react.dev/reference/react/use) 函数来访问 cookies。
  - 在版本 14 及更早版本中，`cookies` 是同步函数。为了向后兼容，您仍然可以在 Next.js 15 中同步访问它，但此行为将在未来弃用。
...
- `.delete` 方法只能在以下情况下调用：
  - 在[服务端操作 (Server Action)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) 或[路由处理器 (Route Handler)](/docs/app/building-your-application/routing/route-handlers) 中。
  - 如果它属于与调用 `.set` 相同的域名。对于通配符域名，特定子域名必须完全匹配。此外，代码必须在与要删除的 cookie 相同的协议（HTTP 或 HTTPS）上执行。
- HTTP 不允许在流式传输开始后设置 cookies，因此您必须在[服务端操作 (Server Action)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) 或[路由处理器 (Route Handler)](/docs/app/building-your-application/routing/route-handlers) 中使用 `.set`。
...
您可以在[服务端操作 (Server Action)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) 或[路由处理器 (Route Handler)](/docs/app/building-your-application/routing/route-handlers) 中使用 `(await cookies()).set(name, value, options)` 方法设置 cookie。[`options` 对象](#options) 是可选的。
