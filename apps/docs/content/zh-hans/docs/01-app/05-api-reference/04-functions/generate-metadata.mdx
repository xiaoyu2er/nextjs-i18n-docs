...
translation-updated-at: 2025-05-21T19:25:22.716Z
title: generateMetadata
description: 了解如何为 Next.js 应用添加元数据 (Metadata) 以提升搜索引擎优化 (SEO) 和网页分享效果。
related:
  title: 后续步骤
...
> 查看 [元数据字段](#metadata-fields) 获取完整支持的选项列表。

## `generateMetadata` 函数

动态元数据依赖于**动态信息**，例如当前路由参数、外部数据或父级段中的 `metadata`，可以通过导出一个返回 [`Metadata` 对象](#metadata-fields) 的 `generateMetadata` 函数来设置。
...
> - 不能从同一路由段同时导出 `metadata` 对象和 `generateMetadata` 函数。
> - `generateMetadata` 中的 `fetch` 请求会自动[记忆化](/docs/app/deep-dive/caching#request-memoization)，以便在 `generateMetadata`、`generateStaticParams`、布局、页面和服务端组件之间共享相同的数据。
> - 如果无法使用 `fetch`，可以使用 React 的 [`cache` 函数](/docs/app/deep-dive/caching#react-cache-function)。
> - [基于文件的元数据](/docs/app/api-reference/file-conventions/metadata) 优先级更高，会覆盖 `metadata` 对象和 `generateMetadata` 函数。
...
`generateMetadata` 应返回一个包含一个或多个元数字段的 [`Metadata` 对象](#metadata-fields)。

> **须知**:
>
> - 如果元数据不依赖于运行时信息，应使用静态的 [`metadata` 对象](#the-metadata-object) 而非 `generateMetadata` 来定义。
> - `fetch` 请求会自动[记忆化](/docs/app/deep-dive/caching#request-memoization)，以便在 `generateMetadata`、`generateStaticParams`、布局、页面和服务端组件之间共享相同的数据。如果无法使用 `fetch`，可以使用 React 的 [`cache` 函数](/docs/app/deep-dive/caching#react-cache-function)。
...
`title` 属性用于设置文档的标题。可以定义为简单的[字符串](#string) 或可选的[模板对象](#template)。
...
> - `layout.js`
>
>   - `title` (字符串) 和 `title.default` 为未定义自身 `title` 的子段定义默认标题。如果存在，它将扩展最近父级段的 `title.template`。
>   - `title.absolute` 为子段定义默认标题。它忽略父级段的 `title.template`。
>   - `title.template` 为子段定义新的标题模板。
>
> - `page.js`
>   - 如果页面未定义自己的标题，将使用最近父级解析后的标题。
>   - `title` (字符串) 定义路由的标题。如果存在，它将扩展最近父级段的 `title.template`。
>   - `title.absolute` 定义路由的标题。它忽略父级段的 `title.template`。
...
`metadataBase` 是一个便捷选项，用于为需要完整 URL 的元数字段设置基础 URL 前缀。

- `metadataBase` 允许**当前路由段及以下**的基于 URL 的元数字段使用**相对路径**，而非必须的绝对 URL。
- 字段的相对路径将与 `metadataBase` 组合形成完整的 URL。
...
> **须知**:
>
> - `metadataBase` 通常设置在根 `app/layout.js` 中，以应用于所有路由的基于 URL 的元数字段。
> - 所有需要绝对 URL 的基于 URL 的元数字段都可以通过 `metadataBase` 选项配置。
> - `metadataBase` 可以包含子域名（例如 `https://app.acme.com`）或基础路径（例如 `https://acme.com/start/from/here`）。
> - 如果元数字段提供了绝对 URL，`metadataBase` 将被忽略。
> - 在未配置 `metadataBase` 的情况下在基于 URL 的元数字段中使用相对路径会导致构建错误。
...
- `metadataBase` 和元数字段之间的尾部斜杠会被规范化。
- 元数字段中的 "绝对" 路径（通常替换整个 URL 路径）被视为 "相对" 路径（从 `metadataBase` 的末尾开始）。
...
继承上述 `metadataBase` 并设置自身值的任何元数字段将按以下方式解析：
...
> **须知**:
>
> - 对于 Open Graph 图片，使用 [基于文件的元数据 API](/docs/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif) 可能更为便捷。相比手动同步配置导出与实际文件，基于文件的 API 会自动为您生成正确的元数据。
...
> **须知**：我们建议尽可能使用 [基于文件的元数据 API](/docs/app/api-reference/file-conventions/metadata/app-icons#image-files-ico-jpg-png) 来处理图标。相比手动同步配置导出与实际文件，基于文件的 API 会自动为您生成正确的元数据。
...
> **须知**：`msapplication-*` 元标签在 Chromium 版本的 Microsoft Edge 中不再支持，因此不再需要。
...
所有元数据选项都应通过内置支持覆盖。然而，可能存在特定于您网站的自定义元数据标签，或刚刚发布的全新元数据标签。您可以使用 `other` 选项来渲染任何自定义元数据标签。
...
您可以通过使用 `Metadata` 类型为元数据添加类型安全。如果您的 IDE 中使用了[内置的 TypeScript 插件](/docs/app/api-reference/config/typescript)，则无需手动添加类型，但您仍然可以显式添加它。
...
| 元数据                      | 建议                                                                                                                                                                                                                                            |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | 通过 [`redirect()`](/docs/app/api-reference/functions/redirect)、[中间件](/docs/app/building-your-application/routing/middleware#nextresponse) 或 [安全头](/docs/app/api-reference/config/next-config-js/headers) 使用适当的 HTTP 头 |
| `<base>`                      | 在布局或页面本身渲染该标签。                                                                                                                                                                                                              |
...
`<link>` 元素有许多 `rel` 关键字，可用于向浏览器提示可能需要外部资源。浏览器根据这些关键字应用预加载优化。
...
> **须知**:
>
> - 这些方法目前仅在客户端组件中受支持，这些组件在初始页面加载时仍由服务端渲染。
...
`generateMetadata` 返回的元数据会流式传输到客户端。这使得 Next.js 可以在元数据解析后立即将其注入 HTML。

由于页面元数据主要针对机器人和爬虫，Next.js 会为可以执行 JavaScript 并检查完整页面 DOM 的机器人（如 `Googlebot`）流式传输元数据。但对于 **HTML 受限** 的机器人（如 `Twitterbot`），元数据会继续阻塞页面渲染，因为这些机器人在爬取时无法执行 JavaScript。

Next.js 会自动检测传入请求的用户代理，以决定是提供流式元数据还是回退到阻塞式元数据。

如果需要自定义此列表，您可以在 `next.config.js` 中使用 `htmlLimitedBots` 选项手动定义它们。Next.js 会确保匹配此正则表达式的用户代理在请求您的网页时接收阻塞式元数据。
...
指定 `htmlLimitedBots` 配置将覆盖 Next.js 的默认列表，允许您完全控制哪些用户代理应选择此行为。这是高级行为，默认设置应能满足大多数情况。

### 顺序

元数据按顺序评估，从根分段开始，一直到最接近最终 `page.js` 分段的分段。例如：
...
按照[评估顺序](#ordering)，从同一路由的多个分段导出的元数据对象会**浅合并**在一起，形成路由的最终元数据输出。重复的键会根据它们的顺序**替换**。

这意味着在较早分段中定义的嵌套字段（如 [`openGraph`](/docs/app/api-reference/functions/generate-metadata#opengraph) 和 [`robots`](/docs/app/api-reference/functions/generate-metadata#robots)）会被最后一个定义它们的分段**覆盖**。
...
如果您希望在分段之间共享某些嵌套字段同时覆盖其他字段，可以将它们提取到单独的变量中：
...
| `v13.2.0` | `viewport`、`themeColor` 和 `colorScheme` 被弃用，转而支持 [`viewport` 配置](/docs/app/api-reference/functions/generate-viewport)。 |
