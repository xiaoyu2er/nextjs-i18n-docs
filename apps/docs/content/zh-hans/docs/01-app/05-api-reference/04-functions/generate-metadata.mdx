...
translation-updated-at: 2025-05-20T22:31:07.995Z
...
> - `metadata` 对象和 `generateMetadata` 函数导出**仅支持在服务器组件中使用**。
> - 不能从同一路由段同时导出 `metadata` 对象和 `generateMetadata` 函数。
> - `generateMetadata` 中的 `fetch` 请求会自动[记忆化](/docs/app/deep-dive/caching#request-memoization)，以便在 `generateMetadata`、`generateStaticParams`、布局、页面和服务器组件之间共享相同的数据。
...
> - `fetch` 请求会自动[记忆化](/docs/app/deep-dive/caching#request-memoization)，以便在 `generateMetadata`、`generateStaticParams`、布局、页面和服务器组件之间共享相同的数据。如果 `fetch` 不可用，可以使用 React 的 [`cache` 函数](/docs/app/deep-dive/caching#react-cache-function)。
...
>   - `page.js` 中定义的 `title.template` 无效，因为页面始终是终止段（没有子路由段）。
>
...
>   - `title` (字符串) 定义路由的标题。如果存在，它将增强最近父段的 `title.template`。
...
> - 所有需要绝对 URL 的基于 URL 的 `metadata` 字段都可以通过 `metadataBase` 选项配置。
...
任何继承上述 `metadataBase` 并设置自身值的 `metadata` 字段将按以下方式解析：
...
> **须知**:
>
> - 对于 Open Graph 图片，使用 [基于文件的元数据 API](/docs/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif) 可能更方便。相比手动同步配置文件与实际文件，基于文件的 API 会自动为你生成正确的元数据。
...
> **须知**：我们建议尽可能使用 [基于文件的元数据 API](/docs/app/api-reference/file-conventions/metadata/app-icons#image-files-ico-jpg-png) 来管理图标。相比手动同步配置文件与实际文件，基于文件的 API 会自动为你生成正确的元数据。
...
你可以将 Facebook 应用或 Facebook 账号与你的网页连接，用于某些 Facebook 社交插件 [Facebook 文档](https://developers.facebook.com/docs/plugins/comments/#moderation-setup-instructions)
...
如果你想生成多个 fb:admins 元标签，可以使用数组值。
...
你可以在网页上启用或禁用 [Pinterest Rich Pins](https://developers.pinterest.com/docs/web-features/rich-pins-overview/)。
...
所有元数据选项都应通过内置支持来覆盖。然而，可能存在特定于你的站点的自定义元数据标签，或刚刚发布的全新元数据标签。你可以使用 `other` 选项来渲染任何自定义元数据标签。
...
以下元数据类型目前没有内置支持。不过，它们仍然可以在布局或页面本身中渲染。

### 类型

您可以通过使用 `Metadata` 类型为元数据添加类型安全。如果您的 IDE 中使用了 [内置 TypeScript 插件](/docs/app/api-reference/config/typescript)，则无需手动添加类型，但您仍可以显式添加。
...
| 元数据                      | 推荐方式                                                                                                                                                                                                                                            |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | 通过 [`redirect()`](/docs/app/api-reference/functions/redirect)、[中间件](/docs/app/building-your-application/routing/middleware#nextresponse) 或 [安全头](/docs/app/api-reference/config/next-config-js/headers) 设置适当的 HTTP 头 |
| `<base>`                      | 在布局或页面中直接渲染该标签。                                                                                                                                                                                                              |
...
`<link>` 元素有一系列 `rel` 关键字，可用于向浏览器提示可能需要的外部资源。浏览器根据这些关键字应用预加载优化。
...
由于页面元数据主要针对机器人和爬虫，Next.js 会为能执行 JavaScript 并检查完整页面 DOM 的机器人（如 `Googlebot`）流式传输元数据。但对于 **HTML 受限** 的机器人（如 `Twitterbot`），由于它们无法在执行 JavaScript 时爬取页面，元数据会继续阻塞页面渲染。

Next.js 会自动检测传入请求的用户代理，以决定是提供流式元数据还是回退到阻塞式元数据。

如果需要自定义此列表，可以在 `next.config.js` 中使用 `htmlLimitedBots` 选项手动定义。Next.js 将确保匹配此正则表达式的用户代理在请求您的网页时收到阻塞式元数据。
...
指定 `htmlLimitedBots` 配置将覆盖 Next.js 的默认列表，允许您完全控制哪些用户代理应选择此行为。这是高级行为，默认设置已能满足大多数情况。
...
按照 [评估顺序](#ordering)，同一路由中多个段导出的元数据对象会 **浅层合并** 形成路由的最终元数据输出。重复键会根据顺序 **替换**。

这意味着嵌套字段如 [`openGraph`](/docs/app/api-reference/functions/generate-metadata#opengraph) 和 [`robots`](/docs/app/api-reference/functions/generate-metadata#robots) 在较早段中定义的元数据会被最后定义它们的段 **覆盖**。
...
如果希望在段之间共享某些嵌套字段同时覆盖其他字段，可以将它们提取到单独的变量中：
...
| `v13.2.0` | `viewport`、`themeColor` 和 `colorScheme` 弃用，推荐使用 [`viewport` 配置](/docs/app/api-reference/functions/generate-viewport)。 |
