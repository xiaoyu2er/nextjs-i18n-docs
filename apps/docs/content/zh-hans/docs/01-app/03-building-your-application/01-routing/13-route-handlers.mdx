...
translation-updated-at: 2025-05-20T22:52:48.045Z
...
> **须知**：路由处理器仅在 `app` 目录内可用。它们等同于 `pages` 目录中的 [API 路由](/docs/pages/building-your-application/routing/api-routes)，这意味着您**无需**同时使用 API 路由和路由处理器。
...
路由处理器可以嵌套在 `app` 目录的任何位置，类似于 `page.js` 和 `layout.js`。但在与 `page.js` 相同的路由段级别**不能**存在 `route.js` 文件。
...
除了支持原生 [Request](https://developer.mozilla.org/docs/Web/API/Request) 和 [Response](https://developer.mozilla.org/docs/Web/API/Response) API 外，Next.js 还通过 [`NextRequest`](/docs/app/api-reference/functions/next-request) 和 [`NextResponse`](/docs/app/api-reference/functions/next-response) 对其进行了扩展，为高级用例提供了便捷的辅助方法。

## 行为

### 缓存

路由处理器默认不缓存。但您可以选择为 `GET` 方法启用缓存。其他支持的 HTTP 方法**不会**被缓存。要缓存 `GET` 方法，请在路由处理器文件中使用 [路由配置选项](/docs/app/api-reference/file-conventions/route-segment-config#dynamic)，例如 `export const dynamic = 'force-static'`。
...
您可以使用增量静态再生 (ISR) [重新验证缓存数据](/docs/app/building-your-application/data-fetching/incremental-static-regeneration)：
...
您可以使用 `next/headers` 中的 [`cookies`](/docs/app/api-reference/functions/cookies) 读取或设置 cookies。此服务器函数可以直接在路由处理器中调用，或嵌套在另一个函数内。
...
您可以使用 `next/headers` 中的 [`headers`](/docs/app/api-reference/functions/headers) 读取标头。此服务器函数可以直接在路由处理器中调用，或嵌套在另一个函数内。
...
传递给路由处理器的请求对象是一个 `NextRequest` 实例，它包含 [一些额外的便捷方法](/docs/app/api-reference/functions/next-request#nexturl)，例如用于更轻松处理查询参数的方法。
...
您可以使用路由处理器 (Route Handler) 来接收来自第三方服务的 webhook 请求：
...
值得注意的是，与使用页面路由 (Pages Router) 的 API 路由不同，您不需要使用 `bodyParser` 或进行任何额外配置。
