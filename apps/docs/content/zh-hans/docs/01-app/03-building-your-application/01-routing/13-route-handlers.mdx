...
translation-updated-at: 2025-05-21T19:22:10.044Z
...
路由处理器在 `app` 目录下的 [`route.js|ts` 文件](/docs/app/api-reference/file-conventions/route)中定义：
...
路由处理器可以嵌套在 `app` 目录的任何位置，类似于 `page.js` 和 `layout.js`。但在与 `page.js` 相同的路由层级**不能**存在 `route.js` 文件。
...
除了支持原生 [Request](https://developer.mozilla.org/docs/Web/API/Request) 和 [Response](https://developer.mozilla.org/docs/Web/API/Response) API 外，Next.js 还通过 [`NextRequest`](/docs/app/api-reference/functions/next-request) 和 [`NextResponse`](/docs/app/api-reference/functions/next-response) 对其进行了扩展，为高级用例提供了便捷的辅助功能。

## 行为

### 缓存

路由处理器默认不缓存。但您可以为 `GET` 方法启用缓存。其他支持的 HTTP 方法**不会**被缓存。要缓存 `GET` 方法，请在路由处理器文件中使用 [路由配置选项](/docs/app/api-reference/file-conventions/route-segment-config#dynamic)，例如 `export const dynamic = 'force-static'`。
...
每个 `route.js` 或 `page.js` 文件接管该路由的所有 HTTP 方法。
...
您可以使用 `next/headers` 中的 [`cookies`](/docs/app/api-reference/functions/cookies) 读取或设置 cookies。此服务器函数可以直接在路由处理器中调用，或嵌套在另一个函数中。
...
您还可以使用底层 Web API 从请求 ([`NextRequest`](/docs/app/api-reference/functions/next-request)) 中读取 cookies：
...
您还可以使用底层 Web API 从请求 ([`NextRequest`](/docs/app/api-reference/functions/next-request)) 中读取标头：
...
传递给路由处理器的请求对象是一个 `NextRequest` 实例，它包含 [一些额外的便捷方法](/docs/app/api-reference/functions/next-request#nexturl)，例如更轻松地处理查询参数的方法。
...
流式传输通常与大型语言模型 (LLMs)（如 OpenAI）结合使用，用于生成 AI 内容。了解更多关于 [AI SDK](https://sdk.vercel.ai/docs/introduction) 的信息。
...
由于 `formData` 数据都是字符串，您可能希望使用 [`zod-form-data`](https://www.npmjs.com/zod-form-data) 来验证请求并以您喜欢的格式（例如 `number`）检索数据。

### CORS

您可以使用标准 Web API 方法为特定路由处理器设置 CORS 标头：
...
> **须知**：
>
> - 要为多个路由处理器添加 CORS 标头，您可以使用 [中间件 (Middleware)](/docs/app/building-your-application/routing/middleware#cors) 或 [`next.config.js` 文件](/docs/app/api-reference/config/next-config-js/headers#cors)。
> - 或者，查看我们的 [CORS 示例](https://github.com/vercel/examples/blob/main/edge-functions/cors/lib/cors.ts) 包。

### Webhooks 网络钩子

您可以使用路由处理器 (Route Handler) 来接收来自第三方服务的网络钩子 (webhooks)：
...
路由处理器 (Route Handlers) 使用与页面和布局相同的 [路由段配置](/docs/app/api-reference/file-conventions/route-segment-config)。
