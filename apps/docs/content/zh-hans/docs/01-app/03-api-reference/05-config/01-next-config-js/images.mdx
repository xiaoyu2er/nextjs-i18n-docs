...
该 `loaderFile` 必须指向 Next.js 应用根目录下的文件。该文件需导出一个返回字符串的默认函数，例如：
...
> **须知**：自定义图片加载器文件（接受函数参数）需要使用 [客户端组件 (Client Components)](/docs/app/getting-started/server-and-client-components) 来序列化提供的函数。

要了解更多关于配置内置 [图片优化 API](/docs/app/api-reference/components/image) 和 [图片组件](/docs/app/api-reference/components/image) 的行为，请参阅 [图片配置选项](/docs/app/api-reference/components/image#configuration-options) 了解可用选项。
...
要了解更多关于配置内置 [图片优化 API](/docs/pages/api-reference/components/image) 和 [图片组件](/docs/pages/api-reference/components/image) 的行为，请参阅 [图片配置选项](/docs/pages/api-reference/components/image#configuration-options) 了解可用选项。
...
```js
// 文档：https://imagekit.io/docs/image-transformation
...
```js
// 文档：https://docs.n7.io/aio/intergrations/
