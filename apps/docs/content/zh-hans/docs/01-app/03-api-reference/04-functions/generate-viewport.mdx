...
> **须知**:
>
> - `viewport` 对象和 `generateViewport` 函数导出 **仅支持服务端组件 (Server Components)**。
> - 不能从同一个路由段同时导出 `viewport` 对象和 `generateViewport` 函数。
> - 如果您正在迁移 `metadata` 导出，可以使用 [metadata-to-viewport-export 代码修改工具](/docs/app/guides/upgrading/codemods#metadata-to-viewport-export) 来更新变更。
...
> **须知**:
>
> - 如果视口不依赖于运行时信息，应使用静态的 [`viewport` 对象](#the-viewport-object) 而非 `generateViewport` 来定义。
...
> **须知**: `viewport` meta 标签会自动设置，通常无需手动配置，因为默认值已足够。但为了完整性，仍提供相关信息。
...
您可以通过使用 `Viewport` 类型为视口对象添加类型安全。如果您的 IDE 中使用了 [内置的 TypeScript 插件](/docs/app/api-reference/config/typescript)，则无需手动添加类型，但仍可以显式添加。
