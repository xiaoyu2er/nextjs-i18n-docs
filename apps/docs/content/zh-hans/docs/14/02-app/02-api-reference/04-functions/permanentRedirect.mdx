...
默认情况下，在[服务端操作 (Server Actions)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) 中会使用 `push`（在浏览器历史记录栈中添加新条目），其他场景则使用 `replace`（替换浏览器历史记录栈中的当前 URL）。可通过指定 `type` 参数覆盖此行为。
