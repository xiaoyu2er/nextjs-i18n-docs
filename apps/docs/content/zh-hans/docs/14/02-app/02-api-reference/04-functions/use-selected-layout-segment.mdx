...
> **须知**:
>
> - 由于 `useSelectedLayoutSegment` 是一个[客户端组件 (Client Component)](/docs/app/building-your-application/rendering/client-components) 钩子，而布局默认是[服务端组件 (Server Component)](/docs/app/building-your-application/rendering/server-components)，因此 `useSelectedLayoutSegment` 通常通过导入到布局中的客户端组件调用。
...
`useSelectedLayoutSegment` _可选_ 接受一个 [`parallelRoutesKey`](/docs/app/building-your-application/routing/parallel-routes#useselectedlayoutsegments)，用于读取该插槽内的活动路由片段。
