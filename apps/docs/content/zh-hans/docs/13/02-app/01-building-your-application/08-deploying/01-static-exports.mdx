...
translation-updated-at: 2025-05-21T21:01:37.798Z
title: 静态导出
description: Next.js 允许您从静态站点或单页应用 (SPA) 开始，之后可选择升级使用需要服务器的功能。
---

{/* 本文档内容在应用路由器和页面路由器之间共享。您可以使用 `<PagesOnly>内容</PagesOnly>` 组件添加特定于页面路由器的内容。任何共享内容不应包裹在组件中。 */}

Next.js 允许您从静态站点或单页应用 (SPA) 开始，之后可选择升级使用需要服务器的功能。

当运行 `next build` 时，Next.js 会为每个路由生成一个 HTML 文件。通过将严格的 SPA 拆分为单独的 HTML 文件，Next.js 可以避免在客户端加载不必要的 JavaScript 代码，从而减少包大小并实现更快的页面加载。
...
生成的组件将被渲染为初始页面加载的静态 HTML 和客户端路由导航的静态负载。使用静态导出时，除非服务端组件使用了[动态服务端功能](#不支持的功能)，否则无需更改它们。

```tsx filename="app/page.tsx" switcher
export default async function Page() {
  // 这个 fetch 会在 `next build` 期间在服务端运行
...
如果您想在客户端获取数据，可以使用客户端组件配合 [SWR](https://github.com/vercel/swr) 来缓存请求。
...
通过 `next/image` 实现的[图片优化](/docs/app/building-your-application/optimizing/images)可以与静态导出一起使用，只需在 `next.config.js` 中定义自定义图片加载器。例如，您可以使用 Cloudinary 等服务优化图片：
...
然后您可以在应用程序中使用 `next/image`，定义 Cloudinary 中图片的相对路径：
...
客户端组件在 `next build` 期间会被预渲染为 HTML。由于 `window`、`localStorage` 和 `navigator` 等 [Web API](https://developer.mozilla.org/docs/Web/API) 在服务器上不可用，您需要确保仅在浏览器中运行时才安全访问这些 API。例如：
...
运行 `next build` 时，Next.js 会将静态导出生成到 `out` 文件夹中。不再需要使用 `next export`。例如，假设您有以下路由：
...
如果您使用的是 Nginx 等静态主机，可以配置从传入请求到正确文件的重写：
...
| 版本      | 变更                                                                                                               |
| --------- | ------------------------------------------------------------------------------------------------------------------ |
| `v13.4.0` | 应用路由器 (稳定版) 添加了增强的静态导出支持，包括使用 React 服务端组件和路由处理器。                              |
