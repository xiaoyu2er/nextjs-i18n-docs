...
translation-updated-at: 2025-05-29T19:42:50.039Z
title: 静态资源（`public` 目录）
description: 了解如何在 Next.js 应用中通过 `public` 目录提供静态文件。
---

Next.js 是一个用于构建全栈 Web 应用的 React 框架。您可以使用 React 组件构建用户界面，并通过 Next.js 获得额外的功能与优化。

它还会自动配置底层工具（如打包器和编译器），让您能专注于产品开发并快速交付。

无论您是独立开发者还是团队一员，Next.js 都能帮助您构建交互式、动态且高效的 React 应用。

## 静态文件服务

Next.js 支持通过 `/public` 目录提供静态资源（如图片、字体等）。放置在其中的文件会自动映射到应用根路径下。

例如，添加 `public/me.png` 后，可通过以下方式访问：
...
1. **组织文件结构**  
   推荐按类型创建子目录（如 `public/images`、`public/fonts`）

2. **缓存策略**  
   静态资源默认会被浏览器缓存。可通过在文件名中添加哈希实现长期缓存：

   ```bash
   public/images/logo-abc123.png
   ```

3. **性能优化**  
   对于关键资源，考虑使用 `next/image` 组件进行自动优化。

<AppOnly>
## 仅限 App Router 的功能

当使用 App Router 时，静态资源仍通过 `public` 目录提供，但推荐将非公共资源移至 `app` 目录下的专用文件夹。
</AppOnly>

<PagesOnly>
## 仅限 Pages Router 的说明

在 Pages Router 中，`public` 目录是唯一官方支持的静态资源存放位置。请勿在其他位置存放需要直接引用的文件。
</PagesOnly>
```---
title: Next.js 12.1 版本发布
description: >-
  Next.js 12.1 带来了按需增量静态再生 (ISR)、styled-components 和 Relay 支持、零配置 Jest 集成等重磅功能！
author:
  - name: Balázs Orbán
...
我们非常兴奋地在 Next.js 12.1 中发布了用户最期待的功能：

*   [**按需增量静态再生 (Beta)**](#on-demand-incremental-static-regeneration-beta)：立即重新验证使用 `getStaticProps` 的页面
...
*   [**自托管优化**](#self-hosted-nextjs-improvements)：Docker 镜像体积减少约 80%
...
Next.js 现在提供了 `unstable_revalidate()` 函数，允许您重新验证使用 `getStaticProps` 的单个页面。这是自 Next.js 9.5 引入[增量静态再生](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration) (ISR) 功能以来，用户呼声最高的特性之一。

ISR 发布后，我们看到 [Tripadvisor](https://vercel.com/customers/cruise-critic)、[Parachute](https://vercel.com/customers/parachute)、HashiCorp 等公司显著缩短了构建时间，同时保持了卓越的性能。现在，我们响应开发者对基于时间间隔重新验证的反馈，提供了更灵活的解决方案。

当前若设置 `revalidate` 时间为 `60`，所有访问者在一分钟内看到的都是相同的静态页面版本。原先只有在一分钟后有人访问该页面时，缓存才会失效。现在您可以手动按需清除 Next.js 中特定页面的缓存。
...
在 `getStaticProps` 中，使用按需重新验证时无需指定 `revalidate`。若省略 `revalidate`，Next.js 将默认使用 `false`（不重新验证），仅在调用 `unstable_revalidate()` 时按需重新验证页面。

增量静态再生功能支持所有兼容 [Next.js 构建 API](/docs/pages/building-your-application/deploying#nextjs-build-api) (`next build`) 的部署平台。在 Vercel 上部署时，按需重新验证可在约 300ms 内完成全球边缘节点更新。
...
我们致力于让所有 Next.js 应用获得更快的生产构建速度和即时本地开发反馈。[Next.js 12](https://nextjs.org/blog/next-12) 引入了基于 Rust 的[新一代编译器](/docs/architecture/nextjs-compiler)。

12.1 版本中，我们为 Next.js 编译器新增了以下支持：
...
新增这六项转换能力后，我们已将最常见的 Babel 插件迁移至 Rust 编译器。如果您希望支持其他插件，请[参与讨论](https://github.com/vercel/next.js/discussions/30174)。此外，我们正在开发基于 SWC 的[高性能 WebAssembly 插件系统](https://twitter.com/swc_rs/status/1492454606118752257)。
...
*   从测试解析中排除 `.next` 目录
*   读取 `next.config.js` 中的 Next.js 编译器转换标志
...
[Next.js 12](https://nextjs.org/blog/next-12) 引入了基于 SWC 的代码压缩功能，早期测试显示其速度比 Terser 快 7 倍。该功能在 12.1 版本进入候选发布 (RC) 阶段，将在 12.2 成为默认选项。
...
内置图片优化 API 现已支持与 [ISR 页面](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration)相同的模式，图片会先返回缓存版本并在后台重新验证（即 `stale-while-revalidate` 策略）。
...
Next.js 现在能自动创建 `standalone` 目录，仅复制生产部署所需的文件。这使得自托管应用的 **Docker 镜像体积减小约 80%**。

在 `next.config.js` 中启用此功能：
...
这将在 `.next/standalone` 生成独立部署目录，无需安装 `node_modules`。

[查阅文档](/docs/pages/api-reference/next-config-js/output#automatically-copying-traced-files-experimental)或从 [Docker 示例](https://github.com/vercel/next.js/tree/canary/examples/with-docker)开始。我们还新增了支持按环境加载不同 `.env` 文件的[多环境 Docker 示例](https://github.com/vercel/next.js/tree/canary/examples/with-docker-multi-env)。

[React 18、服务端组件与 SSR 流式渲染 (Alpha)](#react-18-server-components-and-ssr-streaming-alpha)
-------------------------------------------------------------------------------------------------------------

如 [Next.js 大会](https://www.youtube.com/watch?v=WlP2TB2ORL4)所示，我们正将 React 18、服务端 Suspense、流式 SSR 和服务端组件引入 Next.js。

服务端组件是 React 的新特性，通过分离服务端和客户端代码减少 JavaScript 包体积。开发者可以构建横跨服务端与客户端的应用，兼具客户端应用的丰富交互性和传统服务端渲染的卓越性能。

对于[关注](https://github.com/vercel/next.js/discussions/31263)服务端组件从 Alpha 到 Beta 进展的开发者，近期更新包括：
...
我们正在实现让整个 Next.js 应用在边缘网络进行服务端渲染。您可以选择哪些页面启用 Edge 运行时，逐步从 Node.js 迁移。

我们很快会发布关于 Edge 运行时的深度解析。两个演示项目 ([next-server-components](https://github.com/vercel/next-server-components) 和 [next-rsc-demo](https://github.com/vercel/next-rsc-demo)) 已更新至最新版本。

[其他改进与错误修复](#other-bug-fixes-and-improvements)
---------------------------------------------------------------------

*   使用 `<a>` 的 `target="blank"` 时，ESLint 不再警告应使用 `next/link`
*   `.d.ts` 文件不再被视为页面文件
*   屏幕阅读器播报页面变更时，`document.title` 现在优先于 `h1`
*   现在支持创建 `pages/index/[...dynamic].js`，此前因 `index` 是保留字而无法实现
*   使用 `dynamic(() => import('./some-component'), { ssr: false })` 时，导入内容会自动从服务端代码中被摇树优化
*   为减小安装体积并提升安全性，我们预编译了更多依赖项，最新加入的是 `node-fetch`
*   改进使用[中间件](/docs/pages/building-your-application/routing/middleware)时的快速刷新体验
*   支持 ESLint 8 与[内置 ESLint 集成](/docs/pages/building-your-application/configuring/eslint)
*   `styled-jsx` 升级至 5.0，为所有编译错误提供新的帮助链接
*   Edge 运行时：支持 `AbortController` 和 `AbortSignal`
*   Edge 运行时：新增 `CryptoKey` 和 `globalThis.CryptoKey`
*   大型 Next.js 应用的快速刷新时间提升约 60%（涉及超过 1,000 个模块的重新加载）
*   快速刷新失败导致页面完全重载时，现在会显示通知
*   使用 React 18 严格模式时，初始页面加载的播报现在会被正确跳过
*   自 2021 年 12 月以来，我们已[解决近 300 个问题](https://nextjs-issue-tracker.vercel.app/)（共处理近 1000 个 issue）
...
调研包含两部分：8个问题的快速调研和针对具体功能的深入调研。如果您有时间，我们希望能完成两部分内容，但也可以只提交第一部分的结果。

所有回答将完全匿名，您也可以选择分享应用链接。
