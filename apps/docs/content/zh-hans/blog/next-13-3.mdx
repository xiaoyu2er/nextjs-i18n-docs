...
translation-updated-at: 2025-05-29T19:40:43.853Z
title: 关于 Next.js
description: Next.js 是一个用于构建全栈 Web 应用的 React 框架。通过 React 组件构建用户界面，并利用 Next.js 提供额外功能和优化。
---

Next.js 是一个基于 React 的全栈 Web 应用开发框架。您可以使用 React 组件构建用户界面，并通过 Next.js 获得更多功能和优化。

该框架还会自动配置底层工具（如打包器和编译器），让开发者能够专注于产品构建与快速交付。

无论您是独立开发者还是团队一员，Next.js 都能帮助您构建交互式、动态且高效的 React 应用程序。---
title: Next.js 13.3
description: >-
  Next.js 13.3 引入了动态 Open Graph 图片生成、高级静态导出、并行路由与拦截路由、OTEL 支持等新特性。
author:
...
我们即将在下个次要版本中将应用路由器标记为稳定版，目前正集中精力优化性能、改进行为并修复错误。

虽然仍在开发部分功能（如数据变更），但这些不会影响应用路由器其他功能的 API 接口。期待看到您使用应用路由器构建的作品，并[欢迎提供反馈](https://github.com/vercel/next.js/discussions/41745)。

[基于文件的元数据 API](#file-based-metadata-api)
---------------------------------------------------

在 [Next.js 13.2](https://nextjs.org/blog/next-13-2#built-in-seo-support-with-new-metadata-api) 中，我们推出了新的元数据 API，允许您通过从布局或页面导出元数据对象来定义 HTML `head` 元素中的元数据（如 `title`、`meta` 和 `link` 标签）。
...
除了基于配置的元数据外，元数据 API 现在支持新的文件约定，方便您为提升 SEO 和网页分享体验定制页面：
...
Next.js 将在生产环境中自动为这些文件添加哈希值（用于文件名）以支持缓存，并使用正确的元数据信息（如资源 URL、文件类型和图片尺寸）更新相关 `head` 元素。
...
向应用添加静态文件通常是最简单的方法，但有时您可能需要动态创建文件。对于每个静态文件约定，都有对应的动态 `(.js|.jsx|.ts|.tsx)` 变体，允许您编写代码生成文件。

例如，虽然您可以添加静态 `sitemap.xml` 文件，但大多数网站的某些页面是使用外部数据源动态生成的。要生成动态站点地图，您可以添加返回动态路由数组的 `sitemap.js` 文件。
...
通过[基于配置](/docs/app/api-reference/file-conventions/metadata#metadata-object)和新的基于文件的选项，您现在拥有涵盖静态和动态元数据的全面元数据 API。
...
`@vercel/og` 在 [Next.js 大会](https://nextjs.org/conf) 中经受了考验，为每位参会者生成了超过 10 万张动态门票图片。自发布以来，Vercel 客户广泛采用该库，下载量超过 90 万次。我们很高兴能将动态图片生成功能带给所有 Next.js 应用，而无需额外安装包。
...
`ImageResponse` 天然与 Next.js 其他 API 良好集成，包括路由处理器和基于文件的元数据。例如，您可以在 `opengraph-image.tsx` 文件中使用 `ImageResponse`，在构建时或请求时动态生成 Open Graph 和 Twitter 图片。
...
您可以先构建静态网站或单页应用 (SPA)，之后根据需要升级使用需要服务器的 Next.js 功能。

运行 `next build` 时，Next.js 会为每个路由生成 HTML 文件。通过将严格的 SPA 拆分为单独的 HTML 文件，Next.js 可以避免在客户端加载不必要的 JavaScript 代码，减少包大小并实现更快的页面加载。
...
静态导出与应用路由器的新功能兼容，包括静态路由处理器、Open Graph 图片和 React 服务端组件。

例如，服务端组件将在构建期间运行，类似于传统的静态站点生成，将组件渲染为初始页面加载的静态 HTML 和客户端路由导航的静态有效负载。

以前，要在 `pages` 目录中使用静态导出，您需要运行 `next export`。然而，通过 `next.config.js` 选项，当设置 `output: 'export'` 时，`next build` 将输出 `out` 目录。您可以为应用路由器和 `pages` 目录使用相同的配置。这意味着不再需要 `next export`。

通过高级静态导出支持，您将在开发过程 (`next dev`) 中更早地收到错误，例如尝试使用需要服务器的动态函数（如 `cookies()` 或 `headers()`）。
...
Next.js 13.3 引入了新的动态约定，允许您实现高级路由场景：**并行路由和拦截路由**。这些功能使您可以在同一视图中显示多个页面，如复杂的仪表板或模态框。

通过并行路由，您可以同时在同一视图中渲染一个或多个可以独立导航的页面。它还可以用于有条件地渲染页面。
...
拦截路由允许您在当前布局中加载新路由，同时"屏蔽"浏览器 URL。这在保持当前页面上下文很重要时非常有用，例如通过模态框展开信息流中的照片，而信息流保持在模态框的背景中。
...
*   **Turbopack：** 添加了对中间件、所有 `next/font` 选项以及服务端组件流式传输的支持，接近 Beta 版（[查看演示](https://twitter.com/padmaia/status/1641130761775464448)）。我们还在成熟的 Next.js 应用（如 [vercel.com](https://vercel.com) 和 [nextjs.org](https://nextjs.org/)）上修补了更多发现的错误。[了解更多](/docs/architecture/turbopack)。
*   **`next.config.js` 的快速刷新：** 更改 `next.config.js` 现在会自动重启本地开发服务器。这扩展了 `.env`、`.env.*`、`jsconfig.json`、`tsconfig.json` 配置文件的自动重新加载。
...
*   **`create-next-app` 的 Tailwind CSS：** 使用 `npx create-next-app@latest` 启动新项目时，您现在可以选择 Tailwind CSS 或使用 `--tailwind` 标志，以预配置此样式解决方案的应用。
*   **路由处理器：** 使用 `export default` 而不是 [支持的 HTTP 动词](/docs/app/api-reference/file-conventions/route) 现在会在 `route.ts` 中抛出有用的错误。[了解更多关于路由处理器](/docs/app/building-your-application/routing/route-handlers)。
...
*   **元数据：** 之前用于元数据的 API (`head.js`) 已在 13.2 中弃用，现已被移除。请改用通过 [元数据](/docs/app/api-reference/file-conventions/metadata) API 内置的 SEO 支持。
*   **选择退出路由的文件夹：** 在文件夹前添加 \_ 以使其及其子段退出路由。例如，`app/_dashboard/page.tsx` 将不可路由。
*   **应用路由器：** 我们添加了新的 `useParams` 客户端组件钩子，用于读取给定路由段的动态参数。[了解更多](/docs/app/api-reference/functions/use-params)。
...
*   **改进的客户端路由缓存：** `router.refresh()` 现在将使整个缓存无效，并且搜索参数现在是缓存键的一部分，允许在两个搜索参数之间导航（例如 `/?search=leerob` 和 `/?search=tim`）以正确恢复依赖参数的内容。

[社区](#community)
-----------------------

Next.js 是超过 2,600 名独立开发者、Google 和 Meta 等行业合作伙伴以及 Vercel 核心团队共同努力的成果。每周超过 420 万 npm 下载量和 104,000+ GitHub 星标，Next.js 是构建 Web 的最流行方式之一。
