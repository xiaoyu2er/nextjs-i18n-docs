...
translation-updated-at: 2025-06-06T17:31:40.613Z
title: Ленивая загрузка (Lazy Loading)
description: Ленивая загрузка импортируемых библиотек и React-компонентов для улучшения производительности загрузки приложения.
---
...
При динамическом импорте Серверного компонента только дочерние Клиентские компоненты будут загружаться лениво — сам Серверный компонент нет.
...
Внешние библиотеки можно загружать по требованию с помощью функции [`import()`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/import). В этом примере используется библиотека `fuse.js` для нечёткого поиска. Модуль загружается на клиенте только после ввода пользователем поискового запроса.
...
Для динамического импорта именованного экспорта можно вернуть его из Promise, возвращаемого функцией [`import()`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/import):
...
Для динамического импорта именованного экспорта можно вернуть его из [Promise](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise), возвращаемого [`import()`](https://github.com/tc39/proposal-dynamic-import#example):
