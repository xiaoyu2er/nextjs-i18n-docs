...
translation-updated-at: 2025-06-06T17:40:09.127Z
...
Они полезны для добавления значков приложений, которые отображаются в таких местах, как вкладки браузера, домашние экраны телефонов и результаты поисковых систем.
...
| Конвенция файла           | Поддерживаемые форматы файлов           | Допустимые расположения |
| ------------------------- | --------------------------------------- | ----------------------- |
...
> - Favicon можно задать только в корневом сегменте `/app`. Для более детальной настройки используйте [`icon`](#icon).
...
> - Атрибут `sizes="any"` добавляется к выводу `favicon.ico`, чтобы [избежать бага в браузере](https://evilmartians.com/chronicles/how-to-favicon-in-2021-six-files-that-fit-most-needs), когда иконка `.ico` предпочитается `.svg`.
...
| Конвенция файла | Поддерживаемые форматы файлов |
| --------------- | ----------------------------- |
| `icon`          | `.js`, `.ts`, `.tsx`          |
| `apple-icon`    | `.js`, `.ts`, `.tsx`          |

Самый простой способ сгенерировать иконку — использовать API [`ImageResponse`](/docs/app/api-reference/functions/image-response) из `next/server`.
...
> - Вы можете генерировать несколько иконок в одном файле с помощью [`generateImageMetadata`](/docs/app/api-reference/functions/generate-image-metadata).
...
Объект, содержащий [параметры динамического маршрута](/docs/app/building-your-application/routing/dynamic-routes) от корневого сегмента до сегмента, в котором находится `icon` или `apple-icon`.
...
`icon` и `apple-icon` являются специализированными [Обработчиками маршрутов (Route Handlers)](/docs/app/building-your-application/routing/route-handlers), которые могут использовать те же [параметры конфигурации сегмента маршрута](/docs/app/api-reference/file-conventions/route-segment-config), что и Страницы (Pages) и Макеты (Layouts).
