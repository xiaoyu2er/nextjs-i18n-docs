...
translation-updated-at: 2025-06-06T17:24:12.135Z
...
Хук `useReportWebVitals` позволяет отчитываться о [Core Web Vitals](https://web.dev/vitals/) и может использоваться в сочетании с вашей аналитической службой.
...
Объект `metric`, передаваемый в качестве аргумента хука, содержит несколько свойств:
...
- `rating`: Качественная оценка значения метрики, отражающая уровень производительности. Возможные значения: `"good"`, `"needs-improvement"` и `"poor"`. Оценка обычно определяется путём сравнения значения метрики с предопределёнными порогами, указывающими на приемлемую или неоптимальную производительность.
...
[Web Vitals](https://web.dev/vitals/) — это набор полезных метрик, которые отражают пользовательский опыт взаимодействия с веб-страницей. Включают следующие метрики:
...
В дополнение к основным метрикам, перечисленным выше, существуют дополнительные пользовательские метрики, измеряющие время гидратации и отрисовки страницы:
...
[Vercel Speed Insights](https://vercel.com/docs/concepts/speed-insights) автоматически настраиваются на развёртываниях Vercel и не требуют использования `useReportWebVitals`. Этот хук полезен при локальной разработке или при использовании другой аналитической службы.
