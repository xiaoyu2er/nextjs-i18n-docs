...
translation-updated-at: 2025-06-06T17:00:44.826Z
...
Playwright — это фреймворк для тестирования, который позволяет автоматизировать работу с Chromium, Firefox и WebKit через единый API. С его помощью можно писать **сквозные тесты (E2E)**. В этом руководстве показано, как настроить Playwright с Next.js и написать первые тесты.
...
Это запустит серию вопросов для настройки и конфигурации Playwright в вашем проекте, включая добавление файла `playwright.config.ts`. Подробное руководство см. в [документации Playwright по установке](https://playwright.dev/docs/intro#installation).
...
> **Полезно знать**:
>
> Вместо `page.goto("http://localhost:3000/")` можно использовать `page.goto("/")`, если добавить [`"baseURL": "http://localhost:3000"`](https://playwright.dev/docs/api/class-testoptions#test-options-base-url) в [файл конфигурации](https://playwright.dev/docs/test-configuration) `playwright.config.ts`.

### Запуск тестов Playwright

Playwright будет имитировать действия пользователя в трёх браузерах: Chromium, Firefox и Webkit. Для этого требуется, чтобы сервер Next.js был запущен. Рекомендуется запускать тесты против production-сборки, чтобы поведение приложения было максимально приближено к реальному.
