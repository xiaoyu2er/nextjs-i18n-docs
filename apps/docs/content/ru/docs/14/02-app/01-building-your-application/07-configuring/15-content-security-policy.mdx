...
translation-updated-at: 2025-06-06T17:02:12.947Z
...
{/* Содержание этого документа является общим для маршрутизаторов app и pages. Вы можете использовать компонент `<PagesOnly>Содержание</PagesOnly>` для добавления контента, специфичного для маршрутизатора Pages. Общий контент не должен быть обёрнут в компонент. */}
...
[Нонс](https://developer.mozilla.org/docs/Web/HTML/Global_attributes/nonce) — это уникальная случайная строка символов, созданная для однократного использования. Он используется вместе с CSP для выборочного разрешения выполнения определённых встроенных скриптов или стилей, обходя строгие директивы CSP.
...
[Middleware](/docs/app/building-your-application/routing/middleware) позволяет добавлять заголовки и генерировать нонсы перед отрисовкой страницы.
...
Рекомендуем исключать из обработки префетчи (из `next/link`) и статические ресурсы, которые не требуют заголовка CSP.
...
Теперь вы можете прочитать нонс из [Серверного Компонента](/docs/app/building-your-application/rendering/server-components), используя [`headers`](/docs/app/api-reference/functions/headers):
...
Для приложений, которые не требуют нонсов, вы можете установить заголовок CSP непосредственно в файле [`next.config.js`](/docs/app/api-reference/next-config-js):
...
Рекомендуем использовать Next.js версии `v13.4.20+` для корректной обработки и применения нонсов.