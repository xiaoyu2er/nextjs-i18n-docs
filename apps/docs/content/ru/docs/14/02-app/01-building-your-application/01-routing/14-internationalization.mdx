...
translation-updated-at: 2025-06-06T17:08:49.798Z
title: Интернационализация
description: Добавьте поддержку нескольких языков с помощью интернационализированной маршрутизации и локализованного контента.
---
...
- **Локаль (Locale):** Идентификатор для набора предпочтений языка и форматирования. Обычно включает предпочитаемый язык пользователя и, возможно, его географический регион.
...
Рекомендуется использовать языковые предпочтения пользователя в браузере для выбора локали. Изменение предпочитаемого языка изменит заголовок `Accept-Language` в запросе к вашему приложению.
...
Изменение отображаемого контента на основе предпочитаемой локали пользователя (локализация) не является специфичной для Next.js функцией. Описанные ниже паттерны будут работать одинаково в любом веб-приложении.

Предположим, мы хотим поддерживать контент на английском и нидерландском языках в нашем приложении. Мы можем создать два разных "словаря" — объекты, которые сопоставляют ключи с локализованными строками. Например:
...
Поскольку все макеты и страницы в директории `app/` по умолчанию являются [Серверными компонентами](/docs/app/building-your-application/rendering/server-components), нам не нужно беспокоиться о размере файлов переводов, влияющем на размер клиентского JavaScript-бандла. Этот код будет выполняться **только на сервере**, и только результирующий HTML будет отправлен в браузер.
