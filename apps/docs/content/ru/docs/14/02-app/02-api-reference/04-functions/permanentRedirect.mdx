...
translation-updated-at: 2025-06-06T16:55:47.282Z
...
Функция `permanentRedirect` позволяет перенаправлять пользователя на другой URL. `permanentRedirect` может использоваться в серверных компонентах (Server Components), клиентских компонентах (Client Components), [обработчиках маршрутов (Route Handlers)](/docs/app/building-your-application/routing/route-handlers) и [серверных действиях (Server Actions)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations).
...
> **Полезно знать**: Если вам нужно вернуть временное перенаправление с кодом 307 (Temporary Redirect) вместо постоянного (308), используйте функцию [`redirect`](/docs/app/api-reference/functions/redirect).
