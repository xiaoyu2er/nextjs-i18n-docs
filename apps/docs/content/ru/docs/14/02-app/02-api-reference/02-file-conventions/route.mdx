...
translation-updated-at: 2025-06-06T16:54:11.928Z
title: route.js
...
В настоящее время единственное значение `context` — это `params`, который представляет собой объект, содержащий [динамические параметры маршрута](/docs/app/building-your-application/routing/dynamic-routes) для текущего маршрута.
...
Обработчики маршрутов могут расширять Web API Response, возвращая объект `NextResponse`. Это позволяет легко устанавливать cookies, заголовки, выполнять редиректы и перезаписи. [См. справочник API](/docs/app/api-reference/functions/next-response).
