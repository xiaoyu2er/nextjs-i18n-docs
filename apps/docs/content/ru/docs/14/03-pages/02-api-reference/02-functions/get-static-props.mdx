...
translation-updated-at: 2025-06-06T16:46:43.004Z
...
| Название           | Описание                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`           | Содержит параметры маршрута для страниц, использующих [динамические маршруты](/docs/pages/building-your-application/routing/dynamic-routes). Например, если страница называется `[id].js`, то `params` будет выглядеть как `{ id: ... }`. Следует использовать вместе с `getStaticPaths`, о чём будет рассказано позже.                                                                                                                          |
...
| `revalidateReason` | Указывает причину вызова функции. Может принимать значения: "build" (выполняется во время сборки), "stale" (срок повторной валидации истёк или выполняется в [режиме разработки](/docs/pages/building-your-application/data-fetching/get-static-props#runs-on-every-request-in-development)), "on-demand" (запущено через [по требованию ревалидацию (on-demand revalidation)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation)) |
...
Объект `props` представляет собой пару ключ-значение, где каждое значение передаётся в компонент страницы. Это должен быть [сериализуемый объект](https://developer.mozilla.org/docs/Glossary/Serialization), чтобы любые пропсы могли быть сериализованы с помощью [`JSON.stringify`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify).
...
Подробнее о [инкрементальной статической регенерации (Incremental Static Regeneration)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration).
...
Булево значение `notFound` позволяет странице возвращать статус `404` и [страницу 404](/docs/pages/building-your-application/routing/custom-error#404-page). При `notFound: true` страница вернёт `404`, даже если ранее была успешно сгенерирована. Это полезно для случаев, когда пользовательский контент удаляется автором. Обратите внимание, что `notFound` следует тем же правилам `revalidate`, [описанным здесь](#revalidate).
...
> **Важно**: `notFound` не требуется для режима [`fallback: false`](/docs/pages/api-reference/functions/get-static-paths#fallback-false), так как предварительно рендерятся только пути, возвращённые из `getStaticPaths`.
...
| Версия    | Изменения                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/app/building-your-application/data-fetching) теперь стабилен с упрощённым получением данных                                                           |
| `v12.2.0` | [Инкрементальная статическая регенерация по требованию (On-Demand ISR)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) стала стабильной.    |
| `v12.1.0` | Добавлена [инкрементальная статическая регенерация по требованию (On-Demand ISR)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) (бета). |
| `v10.0.0` | Добавлены параметры `locale`, `locales`, `defaultLocale` и `notFound`.                                                                                                   |
| `v10.0.0` | Добавлен вариант возврата `fallback: 'blocking'`.                                                                                                                           |
| `v9.5.0`  | Стабильная [инкрементальная статическая регенерация (ISR)](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration)                                         |
