...
translation-updated-at: 2025-06-06T16:47:25.311Z
title: Версия 12
...
[Middleware](/docs/messages/middleware-upgrade-guide) - Если вы использовали Middleware до версии `12.2`, ознакомьтесь с [руководством по обновлению](/docs/messages/middleware-upgrade-guide) для получения дополнительной информации.
...
[Минимальная версия React](https://react.dev/learn/add-react-to-an-existing-project) - Минимальная требуемая версия React - `17.0.2`. Для обновления выполните следующую команду в терминале:
...
Чтобы расставить приоритеты для преобразований, которые помогут вам перейти на SWC, предоставьте ваш `.babelrc` в [этой ветке обсуждений](https://github.com/vercel/next.js/discussions/30174).
...
Ранее Next.js использовал подключение через [server-sent events](https://developer.mozilla.org/docs/Web/API/Server-sent_events) для получения событий HMR. В Next.js 12 теперь используется подключение через WebSocket.

В некоторых случаях при проксировании запросов к серверу разработки Next.js вам может потребоваться убедиться, что запрос на обновление обрабатывается корректно. Например, в `nginx` вам нужно добавить следующую конфигурацию:
...
<VirtualHost *:443>
 ...
 # Next.js 12 использует веб-сокеты
 <Location /_next/webpack-hmr>
    RewriteEngine On
...
Если вы уже используете webpack 5, можете пропустить этот раздел.
...
Если ваше приложение всё ещё использует webpack 4 с помощью флага отключения, вы теперь увидите ошибку со ссылкой на [документацию по обновлению до webpack 5](/docs/messages/webpack5).

#### Опция `target` устарела

Если у вас нет `target` в `next.config.js`, можете пропустить этот раздел.

Опция target устарела в пользу встроенной поддержки трассировки зависимостей, необходимых для запуска страницы.

Во время выполнения `next build` Next.js автоматически трассирует каждую страницу и её зависимости, чтобы определить все файлы, необходимые для развёртывания производственной версии вашего приложения.

Если вы в настоящее время используете опцию `target` со значением `serverless`, ознакомьтесь с [документацией о том, как использовать новый output](/docs/pages/api-reference/next-config-js/output).