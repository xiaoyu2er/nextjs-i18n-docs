...
translation-updated-at: 2025-06-06T16:47:15.329Z
title: Версия 11
...
Webpack 5 теперь используется по умолчанию для всех приложений Next.js. Если у вас не было пользовательской конфигурации webpack, ваше приложение уже использует webpack 5. Если у вас есть пользовательская конфигурация webpack, вы можете обратиться к [документации Next.js по webpack 5](/docs/messages/webpack5) для получения инструкций по обновлению.

### Очистка `distDir` теперь включена по умолчанию

Директория с результатами сборки (по умолчанию `.next`) теперь очищается автоматически, за исключением кешей Next.js. Подробнее можно узнать в [RFC по очистке `distDir`](https://github.com/vercel/next.js/discussions/6009).
...
Если ваш `pages/_app.js` импортирует `Container` из `next/app`, вы можете удалить `Container`, так как он был удалён. Подробнее в [документации](/docs/messages/app-container-deprecated).
