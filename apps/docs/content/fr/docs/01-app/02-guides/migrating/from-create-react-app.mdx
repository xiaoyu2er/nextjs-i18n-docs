...
translation-updated-at: 2025-06-02T20:14:22.369Z
...
Create React App utilise React exclusivement côté client. Les applications uniquement côté client, aussi appelées [applications monopages (SPA)](/docs/app/guides/single-page-applications), subissent souvent des temps de chargement initiaux lents. Cela est dû à plusieurs facteurs :
...
Avec le support natif du [streaming via React Suspense](/docs/app/building-your-application/routing/loading-ui-and-streaming#streaming-with-suspense), vous pouvez définir quelles parties de votre UI se chargent en premier et dans quel ordre, sans créer de cascades réseau.

Cela vous permet de construire des pages qui se chargent plus rapidement et éliminent les [décalages de mise en page](https://vercel.com/blog/how-core-web-vitals-affect-seo).
...
Les [images](/docs/app/api-reference/components/image), [polices](/docs/app/api-reference/components/font) et [scripts tiers](/docs/app/guides/scripts) ont souvent un impact important sur les performances d'une application. Next.js inclut des composants spécialisés et des APIs qui les optimisent automatiquement pour vous.

## Étapes de migration

Notre objectif est d'obtenir une application Next.js fonctionnelle aussi rapidement que possible pour que vous puissiez ensuite adopter les fonctionnalités de Next.js progressivement. Pour commencer, nous traiterons votre application comme une application purement côté client ([SPA](/docs/app/guides/single-page-applications)) sans remplacer immédiatement votre routeur existant. Cela réduit la complexité et les conflits de fusion.
...
Une application Next.js avec [App Router](/docs/app) doit inclure un fichier de [mise en page racine](/docs/app/api-reference/file-conventions/layout#root-layout), qui est un [composant serveur React](/docs/app/getting-started/server-and-client-components) qui englobera toutes vos pages.
...
> **Bon à savoir** : Next.js ignore par défaut le `public/manifest.json` de CRA, les icônes supplémentaires et la [configuration de test](/docs/app/guides/testing). Si vous en avez besoin, Next.js les supporte avec son [API Metadata](/docs/app/getting-started/metadata-and-og-images) et sa configuration de [test](/docs/app/guides/testing).
...
Avec les changements ci-dessus, vous êtes passé de la déclaration de tout dans votre `index.html` à l'utilisation de l'approche conventionnelle de Next.js intégrée au framework ([API Metadata](/docs/app/getting-started/metadata-and-og-images)). Cette approche vous permet d'améliorer plus facilement votre SEO et la partageabilité web de vos pages.

### Étape 5 : Styles

Comme CRA, Next.js supporte les [CSS Modules](/docs/app/getting-started/css#css-modules) nativement. Il supporte aussi les [imports CSS globaux](/docs/app/getting-started/css#global-css).
...
Si vous avez utilisé le champ `homepage` dans votre `package.json` de CRA pour servir l'application sous un sous-chemin spécifique, vous pouvez le reproduire dans Next.js en utilisant la configuration [`basePath`](/docs/app/api-reference/config/next-config-js/basePath) dans `next.config.ts` :
...
Vous pouvez toujours fournir une [configuration webpack personnalisée](/docs/app/api-reference/config/next-config-js/webpack) si vous avez besoin de migrer des paramètres webpack avancés depuis CRA.
...
> **Remarque :** L'utilisation d'une exportation statique (`output: 'export'`) [ne prend pas en charge](https://github.com/vercel/next.js/issues/54393) actuellement le hook `useParams` ou d'autres fonctionnalités serveur. Pour utiliser toutes les fonctionnalités de Next.js, supprimez `output: 'export'` de votre `next.config.ts`.
