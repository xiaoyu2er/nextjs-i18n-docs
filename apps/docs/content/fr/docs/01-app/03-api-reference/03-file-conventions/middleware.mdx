...
translation-updated-at: 2025-06-02T20:12:54.425Z
title: Middleware
...
Le Middleware s'exécute avant que le contenu mis en cache et les routes ne soient appariés. Consultez [Appariement des chemins](#matching-paths) pour plus de détails.
...
Le Middleware n'est _pas_ adapté pour :
...
3. Peuvent avoir des modificateurs sur les paramètres nommés (commençant par `:`) : `/about/:path*` correspond à `/about/a/b/c` car `*` signifie _zéro ou plus_. `?` signifie _zéro ou un_ et `+` _un ou plus_
...
1. `rewrite` vers une route ([Page](/docs/app/api-reference/file-conventions/page) ou [Gestionnaire de route](/docs/app/building-your-application/routing/route-handlers)) qui produit une réponse
2. retourner une `NextResponse` directement. Voir [Production d'une réponse](#producing-a-response)
...
1. `rewrite` vers une route ([Page](/docs/pages/building-your-application/routing/pages-and-layouts) ou [Route API Edge](/docs/pages/building-your-application/routing/api-routes)) qui produit une réponse
2. retourner une `NextResponse` directement. Voir [Production d'une réponse](#producing-a-response)
...
> **Bon à savoir** : Évitez de définir de gros en-têtes car cela peut causer une erreur [431 Request Header Fields Too Large](https://developer.mozilla.org/docs/Web/HTTP/Status/431) en fonction de la configuration de votre serveur web backend.

### CORS

Vous pouvez définir des en-têtes CORS dans le Middleware pour autoriser les requêtes cross-origin, y compris les requêtes [simples](https://developer.mozilla.org/fr/docs/Web/HTTP/CORS#requêtes_simples) et [pré-vérifiées](https://developer.mozilla.org/fr/docs/Web/HTTP/CORS#requêtes_pré-vérifiées).
...
> **Bon à savoir** : Vous pouvez configurer les en-têtes CORS pour des routes individuelles dans les [Gestionnaires de route](/docs/app/building-your-application/routing/route-handlers#cors).

</AppOnly>

## Production d'une réponse

Vous pouvez répondre directement depuis un Middleware en retournant une instance de `Response` ou `NextResponse`. (Disponible depuis [Next.js v13.1.0](https://nextjs.org/blog/next-13-1#nextjs-advanced-middleware))
...
Apprenez comment [configurer le Middleware](/docs/app/guides/self-hosting#middleware) lors de l'hébergement autonome de Next.js.
...
| `v12.0.9` | Application des URL absolues dans le Edge Runtime ([PR](https://github.com/vercel/next.js/pull/33410))    |
