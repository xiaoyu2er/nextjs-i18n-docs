...
translation-updated-at: 2025-06-02T19:58:25.446Z
title: Turbopack
...
- **AMP**
  Non prévu pour Turbopack dans Next.js.
- **Yarn PnP**
  Non prévu pour Turbopack dans Next.js.
...
Pour une analyse détaillée de chaque drapeau de fonctionnalité et de son statut, consultez la [Référence API de Turbopack](/docs/app/api-reference/config/next-config-js/turbopack).
...
- **`moduleIds`**
  Définissez comment les ID de modules sont générés (`'named'` vs `'deterministic'`).
- **`treeShaking`**
