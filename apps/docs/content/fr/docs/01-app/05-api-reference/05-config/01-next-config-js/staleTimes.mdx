...
translation-updated-at: 2025-06-02T20:13:09.740Z
title: staleTimes
description: Apprenez comment remplacer le temps d'invalidation du cache du routeur client.
...
Les propriétés `static` et `dynamic` correspondent à la période de temps (en secondes) basée sur différents types de [préchargement des liens](/docs/app/api-reference/components/link#prefetch).
...
- La propriété `static` est utilisée pour les pages générées statiquement, ou lorsque la prop `prefetch` sur `Link` est définie à `true`, ou lors de l'appel à [`router.prefetch`](/docs/app/deep-dive/caching#routerprefetch).
...
> - Cela n'affecte pas le [rendu partiel](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering), **ce qui signifie que les mises en page partagées ne seront pas automatiquement rechargées à chaque navigation, seul le segment de page qui change le sera.**
...
Vous pouvez en apprendre davantage sur le cache du routeur client [ici](/docs/app/deep-dive/caching#client-side-router-cache).
