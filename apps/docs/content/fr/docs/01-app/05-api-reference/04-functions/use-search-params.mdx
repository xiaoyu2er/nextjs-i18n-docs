...
translation-updated-at: 2025-06-02T20:07:54.101Z
...
> **Bon à savoir** :
>
> - `useSearchParams` est un hook de [Composant Client](/docs/app/getting-started/server-and-client-components) et **n'est pas supporté** dans les [Composants Serveur](/docs/app/getting-started/server-and-client-components) pour éviter des valeurs obsolètes pendant le [rendu partiel](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering).
...
Si une route est [rendue statiquement](/docs/app/getting-started/partial-prerendering#static-rendering), l'appel à `useSearchParams` entraînera le rendu côté client de l'arborescence des composants jusqu'à la limite [`Suspense`](/docs/app/building-your-application/routing/loading-ui-and-streaming#example) la plus proche.
...
Nous recommandons d'encadrer le Composant Client qui utilise `useSearchParams` dans une limite `<Suspense/>`. Cela permettra aux Composants Clients situés au-dessus d'être rendus statiquement et envoyés comme partie du HTML initial. [Exemple](/docs/app/api-reference/functions/use-search-params#static-rendering).
...
// Lorsque la valeur sera disponible pendant l'hydratation de React, le fallback
...
> **Bon à savoir** : Définir l'option [`dynamic` de configuration de segment de route](/docs/app/api-reference/file-conventions/route-segment-config#dynamic) à `force-dynamic` peut être utilisé pour forcer un rendu dynamique.
...
Contrairement aux Pages, les [Layouts](/docs/app/api-reference/file-conventions/layout) (Composants Serveur) **ne reçoivent pas** la prop `searchParams`. C'est parce qu'un layout partagé [n'est pas re-rendu pendant la navigation](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering), ce qui pourrait entraîner des `searchParams` obsolètes entre les navigations. Voir [explication détaillée](/docs/app/api-reference/file-conventions/layout#query-params).

À la place, utilisez la prop [`searchParams`](/docs/app/api-reference/file-conventions/page) de la Page ou le hook [`useSearchParams`](/docs/app/api-reference/functions/use-search-params) dans un Composant Client, qui est re-rendu côté client avec les derniers `searchParams`.
