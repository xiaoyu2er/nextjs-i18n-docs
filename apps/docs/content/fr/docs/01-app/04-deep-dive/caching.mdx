...
translation-updated-at: 2025-06-02T20:03:05.162Z
title: Mécanismes de cache dans Next.js
nav_title: Cache
...
<Image
  alt="Diagramme montrant le comportement de cache par défaut dans Next.js pour les quatre mécanismes, avec HIT, MISS et SET au moment de la construction et lors de la première visite d'une route."
...
Next.js étend l'API [`fetch`](#fetch) pour **mémoriser** automatiquement les requêtes ayant la même URL et les mêmes options. Cela signifie que vous pouvez appeler une fonction fetch pour les mêmes données à plusieurs endroits dans un arbre de composants React tout en ne l'exécutant qu'une seule fois.
...
<Image
  alt="Diagramme montrant comment fonctionne la mémorisation fetch pendant le rendu React."
...
> - Pour les cas où `fetch` n'est pas adapté (par exemple certains clients de base de données, CMS ou GraphQL), vous pouvez utiliser la [fonction React `cache`](#react-cache-function) pour mémoriser les fonctions.
...
Ce comportement est similaire à [**stale-while-revalidate**](https://web.dev/articles/stale-while-revalidate).

#### Revalidation à la demande

Les données peuvent être revalidées à la demande par chemin ([`revalidatePath`](#revalidatepath)) ou par tag de cache ([`revalidateTag`](#fetch-optionsnexttags-and-revalidatetag)).
...
Si vous ne _voulez pas_ mettre en cache la réponse de `fetch`, vous pouvez faire ce qui suit :
...
>
> Pour en savoir plus, consultez la documentation des [Server Components](/docs/app/getting-started/server-and-client-components).
...
3. Les instructions JavaScript sont utilisées pour [hydrater](https://react.dev/reference/react-dom/client/hydrateRoot) les Client Components et rendre l'application interactive.
...
<Image
  alt="Comment le rendu statique et dynamique affecte le cache complet de la route. Les routes statiques sont mises en cache au moment de la construction ou après une revalidation des données, tandis que les routes dynamiques ne sont jamais mises en cache"
...
- **[Revalidation des données](/docs/app/deep-dive/caching#revalidating)** : Revalider le [cache de données](#data-cache) invalidera également le cache du routeur en re-rendant les composants sur le serveur et en mettant en cache le nouveau résultat de rendu.
...
- **Utilisant une [API dynamique](#dynamic-apis)** : Cela exclura la route du cache complet de la route et la rendra dynamiquement au moment de la requête. Le cache de données peut toujours être utilisé.
...
Lorsqu'un utilisateur navigue entre les routes, Next.js met en cache les segments de route visités et [précharge](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) les routes vers lesquelles l'utilisateur est susceptible de naviguer. Cela permet une navigation instantanée en arrière/avant, aucun rechargement complet de page entre les navigations, et la préservation de l'état React et de l'état du navigateur.

Avec le cache du routeur :

- **Les mises en page** sont mises en cache et réutilisées lors de la navigation ([rendu partiel](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)).
- **Les états de chargement** sont mis en cache et réutilisés lors de la navigation pour une [navigation instantanée](/docs/app/building-your-application/routing/loading-ui-and-streaming#instant-loading-states).
- **Les pages** ne sont pas mises en cache par défaut, mais sont réutilisées lors de la navigation en arrière et en avant dans le navigateur. Vous pouvez activer la mise en cache pour les segments de page en utilisant l'option de configuration expérimentale [`staleTimes`](/docs/app/api-reference/config/next-config-js/staleTimes).
...
> **Bon à savoir** : L'option de configuration expérimentale [`staleTimes`](/docs/app/api-reference/config/next-config-js/staleTimes) peut être utilisée pour ajuster les temps d'invalidation automatique mentionnés ci-dessus.
...
- Dans une **action serveur** :
  ...
  - Utiliser [`cookies.set`](/docs/app/api-reference/functions/cookies#setting-a-cookie) ou [`cookies.delete`](/docs/app/api-reference/functions/cookies#deleting-cookies) invalide le cache du routeur pour éviter que les routes utilisant des cookies ne deviennent obsolètes (par exemple, l'authentification).
...
> **Bon à savoir** : Vous pouvez également désactiver le [préchargement](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) en définissant la prop `prefetch` du composant `<Link>` sur `false`.
...
| API                                                                     | Cache du routeur            | Cache complet de la route | Cache de données      | Cache React |
| ----------------------------------------------------------------------- | -------------------------- | --------------------- | --------------------- | ----------- |
...
| [`headers`, `searchParams`](#dynamic-apis)                              |                            | Désactiver           |                       |             |
...
1. Lors de l'utilisation de `fetch` ou de [`unstable_cache`](/docs/app/api-reference/functions/unstable_cache), vous avez la possibilité d'étiqueter les entrées de cache avec une ou plusieurs étiquettes.
...
2. [Actions serveur](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) - pour revalider les données après une action utilisateur (par exemple, la soumission d'un formulaire). Cela invalidera le cache du routeur pour la route associée.
...
2. [Actions serveur](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) - pour revalider les données après une interaction utilisateur (par exemple, la soumission d'un formulaire, un clic sur un bouton).
...
Pour les [segments dynamiques](/docs/app/api-reference/file-conventions/dynamic-routes) (par exemple `app/blog/[slug]/page.js`), les chemins fournis par `generateStaticParams` sont mis en cache dans le Full Route Cache lors de la construction. Au moment de la requête, Next.js mettra également en cache les chemins qui n'étaient pas connus lors de la construction lors de leur première visite.
...
Pour rendre statiquement tous les chemins lors de leur première visite, retournez un tableau vide (aucun chemin ne sera rendu lors de la construction) ou utilisez [`export const dynamic = 'force-static'`](/docs/app/api-reference/file-conventions/route-segment-config#dynamic) :
