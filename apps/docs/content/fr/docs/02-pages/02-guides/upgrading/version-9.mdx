...
translation-updated-at: 2025-06-02T20:04:58.465Z
title: Comment effectuer la mise à jour vers la version 9
nav_title: Version 9
description: Guide pour mettre à jour votre application Next.js de la version 8 à la version 9.
---
...
## Vérifiez votre fichier App personnalisé (`pages/_app.js`)

Si vous avez précédemment copié l'exemple de [l'`<App>` personnalisé](/docs/pages/building-your-application/routing/custom-app), vous pourrez peut-être supprimer votre `getInitialProps`.

Supprimer `getInitialProps` de `pages/_app.js` (quand c'est possible) est important pour bénéficier des nouvelles fonctionnalités de Next.js !

Le `getInitialProps` suivant ne fait rien et peut être supprimé :

```js
class MyApp extends App {
  // Supprimez-moi, je ne fais rien !
...
> Cette liste a été créée par la communauté pour vous aider à effectuer la mise à jour. Si vous trouvez d'autres différences, veuillez envoyer une pull-request pour aider les autres utilisateurs.
...
### La clé `config` est maintenant une exportation dans une page
...
Vous devez renommer une exportation `config` qui n'est pas destinée à Next.js en quelque chose de différent.

### `next/dynamic` ne rend plus "loading..." par défaut pendant le chargement

Les composants dynamiques ne rendront plus rien par défaut pendant le chargement. Vous pouvez toujours personnaliser ce comportement en définissant la propriété `loading` :
...
Les pages de ce répertoire ne contiendront plus de bundle côté client.

## Fonctionnalités dépréciées

### `next/dynamic` a déprécié le chargement de plusieurs modules à la fois

La capacité à charger plusieurs modules à la fois a été dépréciée dans `next/dynamic` pour se rapprocher de l'implémentation de React (`React.lazy` et `Suspense`).
