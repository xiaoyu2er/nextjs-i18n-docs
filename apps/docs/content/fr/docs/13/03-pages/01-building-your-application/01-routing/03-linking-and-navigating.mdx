...
translation-updated-at: 2025-06-06T17:31:10.919Z
...
L'exemple ci-dessus utilise plusieurs liens. Chacun correspond à un chemin (`href`) vers une page connue :
...
Vous recevrez le `pathname` et le `query` mis à jour via l'[objet `router`](/docs/pages/api-reference/functions/use-router#router-object) (ajouté par [`useRouter`](/docs/pages/api-reference/functions/use-router) ou [`withRouter`](/docs/pages/api-reference/functions/use-router#withrouter)), sans perdre l'état.
...
Vous pouvez également surveiller les changements d'URL via [`componentDidUpdate`](https://react.dev/reference/react/Component#componentdidupdate) comme indiqué ci-dessous :
...
Le routage superficiel fonctionne **uniquement** pour les changements d'URL dans la page actuelle. Par exemple, supposons que nous ayons une autre page appelée `pages/about.js`, et que vous exécutiez ceci :
