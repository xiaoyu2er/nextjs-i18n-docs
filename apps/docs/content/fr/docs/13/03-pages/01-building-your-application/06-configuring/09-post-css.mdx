...
translation-updated-at: 2025-06-06T17:27:13.626Z
title: PostCSS
...
- [Exemple avec Tailwind CSS](https://github.com/vercel/next.js/tree/canary/examples/with-tailwindcss)

</details>

## Comportement par défaut

Next.js compile le CSS pour sa [prise en charge intégrée du CSS](/docs/pages/building-your-application/styling) en utilisant PostCSS.
...
- Les nouvelles fonctionnalités CSS sont automatiquement compilées pour la compatibilité avec Internet Explorer 11 :
  ...
  - [Plages de Media Queries](https://developer.mozilla.org/docs/Web/CSS/Media_Queries/Using_media_queries#Syntax_improvements_in_Level_4)
...
dans tout votre projet en configurant autoprefixer avec la configuration ci-dessous (repliée).
...
Pour personnaliser browserslist, créez une clé `browserslist` dans votre `package.json` comme ceci :
...
Aucune configuration n'est nécessaire pour prendre en charge les Modules CSS. Pour activer les Modules CSS pour un fichier, renommez le fichier avec l'extension `.module.css`.

Vous pouvez en apprendre plus sur [la prise en charge des Modules CSS par Next.js ici](/docs/pages/building-your-application/styling).
...
> **Bon à savoir** : Next.js permet également de nommer le fichier `.postcssrc.json`, ou de lire la configuration depuis la clé `postcss` dans `package.json`.
...
Ne **pas utiliser `require()`** pour importer les plugins PostCSS. Les plugins doivent être fournis sous forme de chaînes de caractères.
