...
translation-updated-at: 2025-06-06T17:27:05.901Z
...
- [Mise en cache SSR](https://github.com/vercel/next.js/tree/canary/examples/ssr-caching)

</details>

Par défaut, Next.js inclut son propre serveur avec `next start`. Si vous avez un backend existant, vous pouvez toujours l'utiliser avec Next.js (ce n'est pas un serveur personnalisé). Un serveur Next.js personnalisé vous permet de démarrer un serveur 100% programmatiquement afin d'utiliser des modèles de serveur personnalisés. La plupart du temps, vous n'en aurez pas besoin - mais cette option est disponible pour une personnalisation complète.

> **Bon à savoir** :
>
> - Avant de décider d'utiliser un serveur personnalisé, gardez à l'esprit qu'il ne devrait être utilisé que lorsque le routeur intégré de Next.js ne peut pas répondre aux exigences de votre application. Un serveur personnalisé supprimera des optimisations de performance importantes, comme les **fonctions serverless** et l'**[Optimisation statique automatique](/docs/pages/building-your-application/rendering/automatic-static-optimization)**.
...
app.prepare().then(() => {
  createServer(async (req, res) => {
    try {
      ...
      console.error('Erreur lors du traitement de', req.url, err)
      res.statusCode = 500
...
> `server.js` ne passe pas par babel ou webpack. Assurez-vous que la syntaxe et les sources requises par ce fichier sont compatibles avec la version actuelle de node que vous utilisez.

Pour exécuter le serveur personnalisé, vous devez mettre à jour les `scripts` dans `package.json` comme suit :
...
Le serveur personnalisé utilise l'import suivant pour connecter le serveur avec l'application Next.js :
...
| `quiet`        | `Boolean`          | (_Optionnel_) Masque les messages d'erreur contenant des informations sur le serveur. Par défaut `false`       |
...
Par défaut, `Next` sert chaque fichier du dossier `pages` sous un chemin correspondant au nom du fichier. Si votre projet utilise un serveur personnalisé, ce comportement peut entraîner le même contenu étant servi depuis plusieurs chemins, ce qui peut poser des problèmes pour le SEO et l'expérience utilisateur.
...
> Notez que `useFileSystemPublicRoutes` désactive les routes basées sur les noms de fichiers pour le SSR ; le routage côté client peut toujours accéder à ces chemins. Lorsque vous utilisez cette option, vous devez empêcher la navigation vers les routes que vous ne voulez pas de manière programmatique.

> Vous pouvez également configurer le routeur côté client pour interdire les redirections vers les routes basées sur les noms de fichiers ; pour cela, référez-vous à [`router.beforePopState`](/docs/pages/api-reference/functions/use-router#routerbeforepopstate).