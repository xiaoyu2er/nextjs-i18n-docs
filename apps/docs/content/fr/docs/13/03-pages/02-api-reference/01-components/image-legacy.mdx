...
translation-updated-at: 2025-06-06T17:29:54.929Z
title: Composant <Image> (Legacy)
description: Optimisation d'image rétrocompatible avec le composant Image Legacy.
---
...
- Supprime l'implémentation `IntersectionObserver` en faveur du [chargement paresseux natif](https://caniuse.com/loading-lazy-attr)
  - Supprime la prop `lazyBoundary` car il n'y a pas d'équivalent natif
  - Supprime la prop `lazyRoot` car il n'y a pas d'équivalent natif
- Supprime la configuration `loader` en faveur de la prop [`loader`](#loader)
- Change la prop `alt` de facultative à obligatoire
...
Lorsque vous utilisez une URL externe, vous devez l'ajouter à [remotePatterns](#remote-patterns) dans `next.config.js`.
...
- [Démo du layout `fill`](https://image-legacy-component.nextjs.gallery/layout-fill)
  - Avec `fill`, l'image s'étirera en largeur et en hauteur pour remplir l'élément parent, à condition que celui-ci soit relatif.
  - Cela est généralement associé à la propriété [`objectFit`](#objectfit).
...
Une chaîne qui fournit des informations sur la largeur de l'image à différents points de rupture. La valeur de `sizes` affecte grandement les performances pour les images utilisant `layout="responsive"` ou `layout="fill"`. Elle est ignorée pour les images utilisant `layout="intrinsic"` ou `layout="fixed"`.
...
Deuxièmement, la valeur `sizes` est analysée et utilisée pour réduire les valeurs dans l'ensemble de sources créé automatiquement. Si la propriété `sizes` inclut des valeurs comme `50vw`, qui représentent un pourcentage de la largeur de la fenêtre, alors l'ensemble de sources est réduit pour exclure les valeurs trop petites pour être jamais nécessaires.

Par exemple, si vous savez que votre style fera qu'une image occupera toute la largeur sur les appareils mobiles, une disposition à 2 colonnes sur les tablettes et une disposition à 3 colonnes sur les ordinateurs de bureau, vous devriez inclure une propriété `sizes` comme celle-ci :
...
Lorsque vrai, l'image sera considérée comme haute priorité et [préchargée](https://web.dev/preload-responsive-images/). Le chargement paresseux est automatiquement désactivé pour les images utilisant `priority`.
...
Ne doit être utilisé que lorsque l'image est visible au-dessus de la ligne de pli. Par défaut à `false`.

### placeholder

Un placeholder à utiliser pendant le chargement de l'image. Les valeurs possibles sont `blur` ou `empty`. Par défaut à `empty`.

Lorsque `blur`, la propriété [`blurDataURL`](#blurdataurl) sera utilisée comme placeholder. Si `src` est un objet provenant d'une [importation statique](/docs/pages/building-your-application/optimizing/images#local-images) et que l'image importée est `.jpg`, `.png`, `.webp` ou `.avif`, alors `blurDataURL` sera automatiquement remplie.
...
- [Démo de l'effet scintillant avec la prop `blurDataURL`](https://image-legacy-component.nextjs.gallery/shimmer)
...
> **Attention** : Cette propriété est uniquement destinée à une utilisation avancée. Passer une image en chargement `eager` nuira généralement **aux performances**.
>
> Nous recommandons plutôt d'utiliser la propriété [`priority`](#priority), qui charge correctement l'image de manière anticipée pour presque tous les cas d'utilisation.

Le comportement de chargement de l'image. Par défaut à `lazy`.

Lorsque `lazy`, diffère le chargement de l'image jusqu'à ce qu'elle atteigne une distance calculée depuis la fenêtre.
...
Une [URL Data](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs) à utiliser comme image placeholder avant que l'image `src` ne soit chargée avec succès. Ne prend effet que combinée avec [`placeholder="blur"`](#placeholder).
...
Lorsque défini sur `true`, l'image source sera servie telle quelle sans modification de qualité, taille ou format. Par défaut, la valeur est `false`.
...
- `srcSet`. Utilisez plutôt les [Tailles d'appareil (Device Sizes)](#device-sizes).
...
> **Bon à savoir** : L'exemple ci-dessus garantira que la propriété `src` de `next/legacy/image` doit commencer par `https://example.com/account123/`. Tout autre protocole, nom d'hôte, port ou chemin non correspondant renverra une erreur 400 Bad Request.
...
> **Bon à savoir** : L'exemple ci-dessus garantira que la propriété `src` de `next/legacy/image` doit commencer par `https://img1.example.com` ou `https://me.avatar.example.com` ou tout nombre de sous-domaines. Tout autre protocole ou nom d'hôte non correspondant renverra une erreur 400 Bad Request.

Les modèles génériques (wildcard) peuvent être utilisés pour `pathname` et `hostname` et ont la syntaxe suivante :
...
> **Avertissement** : Nous recommandons de configurer des [`remotePatterns`](#remote-patterns) stricts au lieu de `domains` pour protéger votre application contre les utilisateurs malveillants. Utilisez `domains` uniquement si vous possédez tout le contenu servi depuis le domaine.
...
### Configuration du chargeur (Loader Configuration)

Si vous souhaitez utiliser un fournisseur cloud pour optimiser les images au lieu de l'API d'optimisation d'images intégrée de Next.js, vous pouvez configurer le `loader` et le préfixe `path` dans votre fichier `next.config.js`. Cela vous permet d'utiliser des URLs relatives pour la propriété [`src`](#src) de l'Image et de générer automatiquement l'URL absolue correcte pour votre fournisseur.
...
Les fournisseurs cloud d'optimisation d'images suivants sont inclus :
...
> **Bon à savoir** : AVIF prend généralement 20 % plus de temps à encoder mais se compresse 20 % plus petit que WebP. Cela signifie que la première fois qu'une image est demandée, elle sera généralement plus lente, puis les requêtes suivantes qui sont mises en cache seront plus rapides.
...
L'expiration (ou plutôt Max Age) est définie par la configuration [`minimumCacheTTL`](#minimum-cache-ttl) ou l'en-tête `Cache-Control` de l'image source, selon la valeur la plus grande. Plus précisément, la valeur `max-age` de l'en-tête `Cache-Control` est utilisée. Si `s-maxage` et `max-age` sont tous deux trouvés, `s-maxage` est préféré. Le `max-age` est également transmis à tous les clients en aval, y compris les CDN et les navigateurs.

- Vous pouvez configurer [`minimumCacheTTL`](#minimum-cache-ttl) pour augmenter la durée de mise en cache lorsque l'image source ne comprend pas d'en-tête `Cache-Control` ou que la valeur est très faible.
...
### Durée minimale de mise en cache (Minimum Cache TTL)

Vous pouvez configurer la durée de vie (TTL) en secondes pour les images optimisées mises en cache. Dans de nombreux cas, il est préférable d'utiliser une [importation d'image statique (Static Image Import)](/docs/pages/building-your-application/optimizing/images#local-images) qui hachera automatiquement le contenu du fichier et mettra l'image en cache indéfiniment avec un en-tête `Cache-Control` de `immutable`.
...
L'expiration (ou plutôt Max Age) de l'image optimisée est définie par `minimumCacheTTL` ou l'en-tête `Cache-Control` de l'image source, selon la valeur la plus grande.

Si vous devez modifier le comportement de mise en cache par image, vous pouvez configurer [`headers`](/docs/pages/api-reference/next-config-js/headers) pour définir l'en-tête `Cache-Control` sur l'image source (par exemple `/some-asset.jpg`, et non `/_next/image` elle-même).
...
### Désactiver les imports statiques (Disable Static Imports)
...
Le [chargeur](#loader) par défaut n'optimise pas les images SVG pour plusieurs raisons. Premièrement, SVG est un format vectoriel, ce qui signifie qu'il peut être redimensionné sans perte. Deuxièmement, SVG a de nombreuses fonctionnalités similaires à HTML/CSS, ce qui peut entraîner des vulnérabilités sans les bons [en-têtes de politique de sécurité du contenu (CSP)](/docs/app/api-reference/next-config-js/headers).
