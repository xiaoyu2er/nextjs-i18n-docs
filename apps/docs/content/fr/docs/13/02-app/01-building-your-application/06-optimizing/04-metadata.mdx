...
translation-updated-at: 2025-06-06T17:42:40.509Z
title: Métadonnées
...
Pour définir des métadonnées statiques, exportez un [objet `Metadata`](/docs/app/api-reference/functions/generate-metadata#metadata-object) depuis un fichier `layout.js` ou `page.js` statique.
...
> - Les requêtes `fetch` sont automatiquement [mémoïsées](/docs/app/building-your-application/caching#request-memoization) pour les mêmes données dans `generateMetadata`, `generateStaticParams`, les mises en page, les pages et les composants serveur. Le [`cache` de React peut être utilisé](/docs/app/building-your-application/caching#request-memoization) si `fetch` n'est pas disponible.
> - Next.js attendra que la récupération des données dans `generateMetadata` soit terminée avant de diffuser l'interface utilisateur au client. Cela garantit que la première partie d'une [réponse en streaming](/docs/app/building-your-application/routing/loading-ui-and-streaming) inclut les balises `<head>`.
...
Pour des implémentations et exemples, consultez la [Référence API des fichiers de métadonnées](/docs/app/api-reference/functions/generate-metadata) et la [Génération d'images dynamiques](#dynamic-image-generation).
...
En suivant [l'ordre d'évaluation](#ordering), les objets Metadata exportés depuis plusieurs segments d'une même route sont fusionnés de manière superficielle pour former les métadonnées finales de la route. Les clés en double sont **remplacées** en fonction de leur ordre.
...
`ImageResponse` utilise le [Runtime Edge](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime), et Next.js ajoute automatiquement les en-têtes corrects pour les images mises en cache au niveau du edge, améliorant ainsi les performances et réduisant les recalculs.
