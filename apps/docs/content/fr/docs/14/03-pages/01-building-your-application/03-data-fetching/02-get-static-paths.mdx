...
La [référence API de `getStaticPaths`](/docs/pages/api-reference/functions/get-static-paths) couvre tous les paramètres et props utilisables avec `getStaticPaths`.
...
`getStaticPaths` vous permet de contrôler quelles pages sont générées pendant le build plutôt qu'à la demande avec [`fallback`](/docs/pages/api-reference/functions/get-static-paths#fallback-blocking). Générer plus de pages pendant un build ralentira les builds.
