...
Nous recommandons de démarrer une nouvelle application Next.js en utilisant [`create-next-app`](/docs/app/api-reference/cli/create-next-app), qui configure automatiquement tout pour vous. Pour créer un projet, exécutez :
...
Après les invites, [`create-next-app`](/docs/app/api-reference/cli/create-next-app) créera un dossier avec le nom de votre projet et installera les dépendances requises.

Si vous débutez avec Next.js, consultez la documentation sur la [structure de projet](/docs/getting-started/project-structure) pour un aperçu de tous les fichiers et dossiers possibles dans votre application.
...
- `dev` : exécute [`next dev`](/docs/app/api-reference/cli/next#next-dev-options) pour démarrer Next.js en mode développement.
- `build` : exécute [`next build`](/docs/app/api-reference/cli/next#next-build-options) pour construire l'application pour une utilisation en production.
- `start` : exécute [`next start`](/docs/app/api-reference/cli/next#next-start-options) pour démarrer un serveur de production Next.js.
- `lint` : exécute [`next lint`](/docs/app/api-reference/cli/next#next-lint-options) pour configurer ESLint intégré à Next.js.
