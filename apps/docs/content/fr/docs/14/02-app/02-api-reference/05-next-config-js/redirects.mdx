...
Lorsque vous utilisez le support [`basePath`](/docs/app/api-reference/next-config-js/basePath) avec des redirections, chaque `source` et `destination` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à la redirection :
