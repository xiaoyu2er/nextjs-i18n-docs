...
La fonction `generateStaticParams` peut être utilisée en combinaison avec les [segments de route dynamiques](/docs/app/building-your-application/routing/dynamic-routes) pour [**générer statiquement**](/docs/app/building-your-application/rendering/server-components#static-rendering-default) des routes au moment de la construction plutôt qu'à la demande lors d'une requête.
...
> **Bon à savoir** : Les requêtes `fetch` sont automatiquement [mémoïsées](/docs/app/building-your-application/caching#request-memoization) pour les mêmes données à travers toutes les fonctions préfixées par `generate`, les Layouts, Pages et Composants Serveur. React [`cache` peut être utilisé](/docs/app/building-your-application/caching#request-memoization) si `fetch` n'est pas disponible.

### Générer seulement un sous-ensemble de params

Vous pouvez générer un sous-ensemble de params pour une route en retournant un tableau d'objets avec uniquement les segments dynamiques que vous souhaitez générer. Ensuite, en utilisant l'option de configuration de segment [`dynamicParams`](/docs/app/api-reference/file-conventions/route-segment-config#dynamicparams), vous pouvez contrôler ce qui se passe lorsqu'un segment dynamique non généré par `generateStaticParams` est visité.
