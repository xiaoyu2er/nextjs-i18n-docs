...
> **Bon à savoir** : Les pages statiques de [l'optimisation statique automatique](/docs/pages/building-your-application/rendering/automatic-static-optimization) ou les paramètres de prérendu des réécritures seront analysés côté client après l'hydratation et fournis dans la requête.
...
Lorsque vous utilisez le support de [`basePath`](/docs/app/api-reference/next-config-js/basePath) avec les réécritures, chaque `source` et `destination` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à la réécriture :
