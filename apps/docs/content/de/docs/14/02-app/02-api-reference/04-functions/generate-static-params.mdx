...
> **Wissenswert**
>
> - Sie können die [`dynamicParams`](/docs/app/api-reference/file-conventions/route-segment-config#dynamicparams)-Segmentkonfiguration verwenden, um zu steuern, was passiert, wenn ein dynamisches Segment aufgerufen wird, das nicht mit `generateStaticParams` generiert wurde.
...
> **Wissenswert**: `fetch`-Anfragen werden automatisch [zwischengespeichert](/docs/app/building-your-application/caching#request-memoization) für dieselben Daten über alle `generate`-präfixierten Funktionen, Layouts, Seiten und Serverkomponenten hinweg. React [`cache` kann verwendet werden](/docs/app/building-your-application/caching#request-memoization), falls `fetch` nicht verfügbar ist.

### Nur eine Teilmenge von Parametern generieren

Sie können eine Teilmenge von Parametern für eine Route generieren, indem Sie ein Array von Objekten zurückgeben, die nur die dynamischen Segmente enthalten, die Sie generieren möchten. Mit der [`dynamicParams`](/docs/app/api-reference/file-conventions/route-segment-config#dynamicparams)-Segmentkonfiguration können Sie steuern, was passiert, wenn ein dynamisches Segment aufgerufen wird, das nicht mit `generateStaticParams` generiert wurde.
