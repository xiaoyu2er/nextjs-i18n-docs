...
Ein Objekt, das das [dynamische Routenparameter](/docs/app/building-your-application/routing/dynamic-routes)-Objekt vom Root-Segment bis zu dem Segment enthält, in dem sich `opengraph-image` oder `twitter-image` befindet.
...
`opengraph-image` und `twitter-image` sind spezialisierte [Route Handlers](/docs/app/building-your-application/routing/route-handlers), die dieselben [Routensegment-Konfigurationsoptionen](/docs/app/api-reference/file-conventions/route-segment-config) wie Pages und Layouts verwenden können.
