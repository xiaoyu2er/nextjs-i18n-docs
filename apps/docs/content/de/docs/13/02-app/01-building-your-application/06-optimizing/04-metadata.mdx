...
translation-updated-at: 2025-06-06T18:07:08.558Z
title: Metadaten
description: Verwenden der Metadata API zur Definition von Metadaten in Layouts oder Seiten.
related:
...
- **Konfigurationsbasierte Metadaten**: Exportieren eines [statischen `metadata`-Objekts](/docs/app/api-reference/functions/generate-metadata#metadata-object) oder einer dynamischen [`generateMetadata`-Funktion](/docs/app/api-reference/functions/generate-metadata#generatemetadata-function) in einer `layout.js`- oder `page.js`-Datei.
- **Dateibasierte Metadaten**: Hinzufügen statischer oder dynamisch generierter spezieller Dateien zu Routensegmenten.
...
> - `fetch`-Anfragen werden automatisch [memoized](/docs/app/building-your-application/caching#request-memoization) für dieselben Daten über `generateMetadata`, `generateStaticParams`, Layouts, Seiten und Server Components hinweg. React [`cache` kann verwendet werden](/docs/app/building-your-application/caching#request-memoization), falls `fetch` nicht verfügbar ist.
> - Next.js wartet, bis das Datenabrufen innerhalb von `generateMetadata` abgeschlossen ist, bevor die UI an den Client gestreamt wird. Dies stellt sicher, dass der erste Teil einer [gestreamten Antwort](/docs/app/building-your-application/routing/loading-ui-and-streaming) die `<head>`-Tags enthält.
...
Für Implementierung und Beispiele siehe die [Metadata Files](/docs/app/api-reference/functions/generate-metadata) API-Referenz und [Dynamic Image Generation](#dynamic-image-generation).
...
Es gibt zwei standardmäßige `meta`-Tags, die immer hinzugefügt werden, auch wenn eine Route keine Metadaten definiert:
...
Metadaten werden in Reihenfolge ausgewertet, beginnend beim Root-Segment bis zum Segment, das am nächsten zum finalen `page.js`-Segment liegt. Beispiel:
...
Gemäß der [Auswertungsreihenfolge](#ordering) werden Metadata-Objekte, die aus mehreren Segmenten derselben Route exportiert werden, **oberflächlich** zusammengeführt, um die finalen Metadaten der Route zu bilden. Doppelte Schlüssel werden basierend auf ihrer Reihenfolge **ersetzt**.
...
`ImageResponse` verwendet die [Edge Runtime](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes#edge-runtime), und Next.js fügt automatisch die korrekten Header für zwischengespeicherte Bilder am Edge hinzu, was die Leistung verbessert und Neuberechnungen reduziert.

Zur Verwendung können Sie `ImageResponse` aus `next/server` importieren:
...
`ImageResponse` unterstützt gängige CSS-Eigenschaften, einschließlich Flexbox und absolute Positionierung, benutzerdefinierte Schriftarten, Textumbruch, Zentrierung und verschachtelte Bilder. [Vollständige Liste der unterstützten CSS-Eigenschaften](/docs/app/api-reference/functions/image-response).
...
> - Maximale Bundle-Größe von `500KB`. Die Bundle-Größe umfasst JSX, CSS, Schriftarten, Bilder und andere Assets. Bei Überschreitung sollten Sie die Größe der Assets reduzieren oder zur Laufzeit abrufen.
> - Nur `ttf`, `otf` und `woff` Schriftformate werden unterstützt. Für maximale Geschwindigkeit bei der Schriftanalyse sind `ttf` oder `otf` gegenüber `woff` bevorzugt.
