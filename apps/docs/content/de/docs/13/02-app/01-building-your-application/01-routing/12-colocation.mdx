...
translation-updated-at: 2025-06-06T18:09:20.271Z
...
Abgesehen von den [Routing-Ordner- und Dateikonventionen](/docs/getting-started/project-structure#app-routing-conventions) ist Next.js **nicht vorgegeben**, wie Sie Ihre Projektdateien organisieren und kolokalisieren.

Diese Seite zeigt Standardverhalten und Funktionen, die Sie zur Organisation Ihres Projekts nutzen können.
...
> - Während Sie Ihre Projektdateien in `app` kolokalisieren **können**, müssen Sie es **nicht**. Wenn Sie bevorzugen, können Sie sie [außerhalb des `app`-Verzeichnisses speichern](#store-project-files-outside-of-app).
...
Dies zeigt an, dass der Ordner ein privates Implementierungsdetail ist und nicht vom Routing-System berücksichtigt werden sollte, wodurch der Ordner und alle seine Unterordner **vom Routing ausgeschlossen** werden.
...
> - Wenn Sie keine privaten Ordner verwenden, wäre es hilfreich, die [speziellen Dateikonventionen](/docs/getting-started/project-structure#routing-files) von Next.js zu kennen, um unerwartete Namenskonflikte zu vermeiden.
...
Der folgende Abschnitt listet eine sehr allgemeine Übersicht über gängige Strategien. Die einfachste Erkenntnis ist, eine Strategie zu wählen, die für Sie und Ihr Team funktioniert, und konsistent im gesamten Projekt zu bleiben.
...
<Image
  alt="Ein Beispiel für eine Ordnerstruktur mit Projektdateien außerhalb von app"
...
<Image
  alt="Ein Beispiel für eine Ordnerstruktur mit Projektdateien innerhalb von app"
...
Diese Strategie speichert global geteilten Anwendungscode im Stammverzeichnis von `app` und **teilt** spezifischeren Anwendungscode in die Routensegmente auf, die sie verwenden.

<Image
  alt="Ein Beispiel für eine Ordnerstruktur mit nach Funktion oder Route aufgeteilten Projektdateien"
