...
translation-updated-at: 2025-06-06T17:58:17.761Z
title: route.js
description: API-Referenz für die spezielle Datei route.js.
---

Route Handler (Route-Handler) ermöglichen es Ihnen, benutzerdefinierte Anfragehandler für eine bestimmte Route unter Verwendung der Web-[Request](https://developer.mozilla.org/docs/Web/API/Request)- und [Response](https://developer.mozilla.org/docs/Web/API/Response)-APIs zu erstellen.
...
> **Gut zu wissen**: Route Handler sind nur innerhalb des `app`-Verzeichnisses verfügbar. Sie müssen **nicht** API-Routen (`pages`) und Route Handler (`app`) gemeinsam verwenden, da Route Handler alle Anwendungsfälle abdecken sollten.

## Parameter

### `request` (optional)

Das `request`-Objekt ist ein [NextRequest](/docs/app/api-reference/functions/next-request)-Objekt, das eine Erweiterung der Web-[Request](https://developer.mozilla.org/docs/Web/API/Request)-API darstellt. `NextRequest` gibt Ihnen zusätzliche Kontrolle über die eingehende Anfrage, einschließlich einfachem Zugriff auf `cookies` und einem erweiterten, geparsten URL-Objekt `nextUrl`.
...
Aktuell ist der einzige Wert von `context` `params`, ein Objekt, das die [dynamischen Routenparameter](/docs/app/building-your-application/routing/dynamic-routes) für die aktuelle Route enthält.
