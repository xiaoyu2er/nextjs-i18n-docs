...
translation-updated-at: 2025-06-06T17:17:22.555Z
title: Next.js Compiler
description: Der in Rust geschriebene Next.js Compiler, der Ihre Next.js-Anwendung transformiert und minimiert.
---

Der Next.js Compiler, geschrieben in Rust mit [SWC](http://swc.rs/), ermöglicht es Next.js, Ihren JavaScript-Code für die Produktion zu transformieren und zu minimieren. Dies ersetzt Babel für einzelne Dateien und Terser für die Minimierung von Ausgabebündeln.
...
[SWC](http://swc.rs/) ist eine erweiterbare, auf Rust basierende Plattform für die nächste Generation schneller Entwicklerwerkzeuge.
...
`minify`, `transpileTemplateLiterals` und `pure` sind noch nicht implementiert. Sie können den Fortschritt [hier](https://github.com/vercel/next.js/issues/30802) verfolgen. `ssr` und `displayName`-Transformationen sind die Hauptvoraussetzung für die Verwendung von `styled-components` in Next.js.
...
Next.js erkennt automatisch `jsxImportSource` in `jsconfig.json` oder `tsconfig.json` und wendet dies an. Dies wird häufig mit Bibliotheken wie [Theme UI](https://theme-ui.com) verwendet.
...
Wenn Sie ein benutzerdefiniertes Babel-Setup verwenden, [teilen Sie bitte Ihre Konfiguration mit](https://github.com/vercel/next.js/discussions/30174). Wir arbeiten daran, so viele häufig verwendete Babel-Transformationen wie möglich zu portieren sowie Plugins in Zukunft zu unterstützen.

## Versionsverlauf

| Version   | Änderungen                                                                                                                                                                                                  |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.1.0` | [Modul-Transpilierung](https://nextjs.org/blog/next-13-1#built-in-module-transpilation-stable) und [Modularisierte Imports](https://nextjs.org/blog/next-13-1#import-resolution-for-smaller-bundles) stabil. |
