...
translation-updated-at: 2025-06-06T17:52:33.403Z
title: Benutzerdefinierter Server
description: Starten einer Next.js-Anwendung programmatisch mit einem benutzerdefinierten Server.
---
...
app.prepare().then(() => {
  createServer(async (req, res) => {
    try {
      ...
      // Dies weist an, den Query-Teil der URL zu parsen.
    ...
    .listen(port, () => {
      console.log(`> Bereit unter http://${hostname}:${port}`)
    })
})
```
...
| -------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| `conf`         | `Object`           | Dasselbe Objekt, das Sie in [next.config.js](/docs/pages/api-reference/next-config-js) verwenden würden. Standardwert: `{}` |
| `customServer` | `Boolean`          | (_Optional_) Auf false setzen, wenn der Server von Next.js erstellt wurde                                     |
| `dev`          | `Boolean`          | (_Optional_) Ob Next.js im Entwicklungsmodus gestartet werden soll. Standardwert: `false`                     |
| `dir`          | `String`           | (_Optional_) Ort des Next.js-Projekts. Standardwert: `'.'`                                                    |
| `quiet`        | `Boolean`          | (_Optional_) Fehlermeldungen mit Serverinformationen ausblenden. Standardwert: `false`                        |
...
Standardmäßig serviert `Next` jede Datei im `pages`-Ordner unter einem Pfadnamen, der dem Dateinamen entspricht. Wenn Ihr Projekt einen benutzerdefinierten Server verwendet, kann dieses Verhalten dazu führen, dass derselbe Inhalt über mehrere Pfade serviert wird, was Probleme mit SEO und UX verursachen kann.
...
> Möglicherweise möchten Sie auch den clientseitigen Router so konfigurieren, dass clientseitige Weiterleitungen zu Dateinamen-Routen nicht erlaubt sind; dafür siehe [`router.beforePopState`](/docs/pages/api-reference/functions/use-router#routerbeforepopstate).