...
translation-updated-at: 2025-06-06T17:26:04.365Z
...
Dynamische Metadaten, die von **dynamischen Informationen** abhängen, wie z.B. den aktuellen Routenparametern, externen Daten oder `metadata` in übergeordneten Segmenten, können durch Exportieren einer `generateMetadata`-Funktion definiert werden, die ein [`Metadata`-Objekt](#metadata-felder) zurückgibt.
...
> - React [`cache` kann verwendet werden](/docs/app/deep-dive/caching#react-cache-function), falls `fetch` nicht verfügbar ist.
...
- `props` - Ein Objekt mit den Parametern der aktuellen Route:
  ...
  - `params` - Ein Objekt mit den [dynamischen Routenparametern](/docs/app/api-reference/file-conventions/dynamic-routes) vom Wurzelsegment bis zu dem Segment, aus dem `generateMetadata` aufgerufen wird. Beispiele:
...
> **Gut zu wissen**:
>
> - Wenn Metadaten nicht von Laufzeitinformationen abhängen, sollten sie mit dem statischen [`metadata`-Objekt](#das-metadata-objekt) anstelle von `generateMetadata` definiert werden.
...
Das `title`-Attribut wird verwendet, um den Titel des Dokuments festzulegen. Es kann als einfache [Zeichenkette](#zeichenkette) oder als optionales [Template-Objekt](#template) definiert werden.
...
Sie können Typsicherheit zu Ihren Metadaten hinzufügen, indem Sie den Typ `Metadata` verwenden. Wenn Sie das [integrierte TypeScript-Plugin](/docs/app/api-reference/config/typescript) in Ihrer IDE verwenden, müssen Sie den Typ nicht manuell hinzufügen, können ihn aber dennoch explizit angeben, wenn Sie möchten.
...
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | Verwenden Sie geeignete HTTP-Header über [`redirect()`](/docs/app/api-reference/functions/redirect), [Middleware](/docs/app/building-your-application/routing/middleware#nextresponse), [Security Headers](/docs/app/api-reference/config/next-config-js/headers) |
...
Gemäß der [Auswertungsreihenfolge](#reihenfolge) werden Metadata-Objekte, die aus mehreren Segmenten derselben Route exportiert werden, **flach** zusammengeführt, um die endgültige Metadatenausgabe einer Route zu bilden. Doppelte Schlüssel werden basierend auf ihrer Reihenfolge **ersetzt**.

Das bedeutet, dass Metadaten mit verschachtelten Feldern wie [`openGraph`](/docs/app/api-reference/functions/generate-metadata#opengraph) und [`robots`](/docs/app/api-reference/functions/generate-metadata#robots), die in einem früheren Segment definiert sind, durch das letzte Segment, das sie definiert, **überschrieben** werden.
...
| `v13.2.0` | `viewport`, `themeColor` und `colorScheme` zugunsten der [`viewport`-Konfiguration](/docs/app/api-reference/functions/generate-viewport) als veraltet markiert. |
