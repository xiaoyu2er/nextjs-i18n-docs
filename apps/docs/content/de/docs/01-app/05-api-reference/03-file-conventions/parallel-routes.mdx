...
translation-updated-at: 2025-06-02T20:11:54.528Z
...
<Image
  alt="Diagramm zu parallelen Routen"
...
<Image
  alt="Nicht übereinstimmende Routen in parallelen Routen"
...
Standardmäßig behält Next.js den aktiven _Zustand_ (oder die Unterseite) für jeden Slot im Auge. Der in einem Slot gerenderte Inhalt hängt jedoch von der Art der Navigation ab:

- [**Soft Navigation**](/docs/app/building-your-application/routing/linking-and-navigating#5-soft-navigation): Bei der clientseitigen Navigation führt Next.js ein [partielles Rendering](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering) durch, ändert die Unterseite innerhalb des Slots und behält die aktiven Unterseiten der anderen Slots bei, auch wenn sie nicht mit der aktuellen URL übereinstimmen.
- **Hard Navigation**: Nach einem vollständigen Seitenladen (Browser-Neuladen) kann Next.js den aktiven Zustand für die Slots, die nicht mit der aktuellen URL übereinstimmen, nicht bestimmen. Stattdessen wird eine [`default.js`](#defaultjs)-Datei für die nicht übereinstimmenden Slots gerendert oder `404`, wenn `default.js` nicht existiert.
...
<Image
  alt="Diagramm zu bedingten Routen"
...
Parallele Routen können zusammen mit [Intercepting Routes](/docs/app/api-reference/file-conventions/intercepting-routes) verwendet werden, um Modale mit Deep Linking zu erstellen. Dies ermöglicht es Ihnen, häufige Herausforderungen beim Erstellen von Modalen zu lösen, wie z.B.:
...
<Image
  alt="Diagramm zu parallelen Routen mit Auth-Modal"
...
<Image
  alt="Diagramm zu parallelen Routen mit Login-Seite"
...
Fügen Sie dann im `@auth`-Slot eine [`default.js`](/docs/app/api-reference/file-conventions/default)-Datei hinzu, die `null` zurückgibt. Dadurch wird sichergestellt, dass das Modal nicht gerendert wird, wenn es nicht aktiv ist.
...
> **Gut zu wissen:**
>
> - Die zur Interception der Route verwendete Konvention, z.B. `(.)`, hängt von Ihrer Dateisystemstruktur ab. Siehe [Intercepting Routes-Konvention](/docs/app/api-reference/file-conventions/intercepting-routes#convention).
> - Durch die Trennung der `<Modal>`-Funktionalität vom Modal-Inhalt (`<Login>`) können Sie sicherstellen, dass jeglicher Inhalt innerhalb des Modals, z.B. [Formulare](/docs/app/guides/forms), Server-Komponenten sind. Siehe [Interleaving Client and Server Components](/docs/app/getting-started/server-and-client-components#examples#supported-pattern-passing-server-components-to-client-components-as-props) für weitere Informationen.
...
> - [Sehen Sie sich ein Beispiel](https://github.com/vercel-labs/nextgram) für Modale mit Intercepted und Parallel Routes an.
...
Weitere Informationen finden Sie in der Dokumentation zu [Loading UI](/docs/app/building-your-application/routing/loading-ui-and-streaming) und [Error Handling](/docs/app/getting-started/error-handling).