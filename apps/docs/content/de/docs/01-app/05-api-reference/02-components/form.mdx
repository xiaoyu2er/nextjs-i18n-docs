...
translation-updated-at: 2025-06-02T20:11:07.573Z
title: Formular
description: Erfahren Sie, wie Sie die `<Form>`-Komponente verwenden können, um Formularübermittlungen und Suchparameter-Updates mit clientseitiger Navigation zu handhaben.
---

Die `<Form>`-Komponente erweitert das HTML-`<form>`-Element, um <AppOnly>[**Prefetching**](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) von [Lade-UI](/docs/app/building-your-application/routing/loading-ui-and-streaming),</AppOnly> **clientseitige Navigation** bei Übermittlung und **progressive Verbesserung** bereitzustellen.
...
- Wenn `action` ein **String** ist, verhält sich `<Form>` wie ein natives HTML-Formular, das eine **`GET`**-Methode verwendet. Die Formulardaten werden als Suchparameter in die URL kodiert, und bei Übermittlung des Formulars wird zur angegebenen URL navigiert. Zusätzlich führt Next.js folgendes aus:
  - [Prefetching](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) des Pfads, wenn das Formular sichtbar wird. Dies lädt die gemeinsame UI (z.B. `layout.js` und `loading.js`) vor, was zu schnellerer Navigation führt.
  - Führt eine [clientseitige Navigation](/docs/app/building-your-application/routing/linking-and-navigating#5-soft-navigation) anstelle eines vollständigen Seitenneuladens durch, wenn das Formular übermittelt wird. Dies erhält die gemeinsame UI und den clientseitigen Zustand.
- Wenn `action` eine **Funktion** (Server Action) ist, verhält sich `<Form>` wie ein [React-Formular](https://react.dev/reference/react-dom/components/form) und führt die Aktion bei Übermittlung des Formulars aus.
...
- **`replace`**: Ersetzt den aktuellen Verlaufseintrag anstatt einen neuen zum [Browserverlauf](https://developer.mozilla.org/en-US/docs/Web/API/History_API) hinzuzufügen. Standard ist `false`.
...
- **`action`**: Die Server Action, die bei Übermittlung des Formulars aufgerufen wird. Weitere Informationen finden Sie in den [React-Dokumenten](https://react.dev/reference/react-dom/components/form#props).
...
- **[`method`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#method), [`encType`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#enctype), [`target`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#target)**: Werden nicht unterstützt, da sie das `<Form>`-Verhalten überschreiben.
...
Auf der Ergebnisseite können Sie die Abfrage mit der [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional)-Prop von `page.js` abrufen und verwenden, um Daten aus einer externen Quelle abzurufen.
...
Wenn die `<Form>` im sichtbaren Bereich des Benutzers erscheint, wird die gemeinsame UI (wie `layout.js` und `loading.js`) auf der `/search`-Seite vorab geladen. Bei Übermittlung navigiert das Formular sofort zur neuen Route und zeigt die Lade-UI an, während die Ergebnisse abgerufen werden. Sie können die Fallback-UI mit [`loading.js`](/docs/app/api-reference/file-conventions/loading) gestalten:
...
Um Fälle abzudecken, in denen die gemeinsame UI noch nicht geladen ist, können Sie dem Benutzer mit [`useFormStatus`](https://react.dev/reference/react-dom/hooks/useFormStatus) sofortiges Feedback geben.
...
Weitere Beispiele finden Sie in den [Server Actions](/docs/app/building-your-application/data-fetching/server-actions-and-mutations)-Dokumenten.
