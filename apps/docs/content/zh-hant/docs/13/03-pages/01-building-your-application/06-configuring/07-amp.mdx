...
translation-updated-at: 2025-05-25T21:35:16.848Z
title: AMP
description: 只需最少配置，無需脫離 React 即可開始添加 AMP，提升頁面效能與載入速度。
---
...
要為頁面啟用 AMP 支援並了解不同的 AMP 配置，請閱讀 [`next/amp` API 文件](/docs/pages/building-your-application/configuring/amp)。

## 注意事項

- 目前僅支援 CSS-in-JS。AMP 頁面暫時不支援 [CSS 模組 (CSS Modules)](/docs/pages/building-your-application/styling)。您可以[為 Next.js 貢獻 CSS 模組支援](https://github.com/vercel/next.js/issues/10549)。

## 添加 AMP 元件

AMP 社群提供[眾多元件](https://amp.dev/documentation/components/)來增強 AMP 頁面的互動性。Next.js 會自動導入頁面中使用的所有元件，無需手動導入 AMP 元件腳本：
...
開發期間，AMP 頁面會自動透過 [amphtml-validator](https://www.npmjs.com/package/amphtml-validator) 進行驗證。錯誤與警告會顯示在啟動 Next.js 的終端機中。

在[靜態 HTML 導出 (Static HTML export)](/docs/pages/building-your-application/deploying/static-exports) 時也會驗證頁面，任何警告/錯誤將輸出至終端機。若有 AMP 錯誤會導致導出程序以狀態碼 `1` 結束，因為導出的內容不符合 AMP 規範。
...
作為解決方案，您可以在專案中手動建立 `amp.d.ts` 檔案並加入這些[自訂類型](https://stackoverflow.com/a/50601125)。