...
translation-updated-at: 2025-05-25T21:36:00.423Z
...
當您的頁面從無頭 CMS (headless CMS) 獲取資料時，靜態生成非常有用。但是，當您在無頭 CMS 上撰寫草稿並希望立即在頁面上**預覽**草稿時，這並不理想。您會希望 Next.js 在**請求時**而非建置時渲染這些頁面，並獲取草稿內容而非已發布的內容。您會希望 Next.js 僅在這種特定情況下繞過靜態生成。
...
實際上，您會希望從無頭 CMS **安全地** 呼叫此 API 路由。具體步驟會根據您使用的無頭 CMS 而有所不同，但以下是一些常見的步驟。

這些步驟假設您使用的無頭 CMS 支援設定 **自訂預覽 URL**。如果不支援，您仍然可以使用此方法來保護預覽 URL，但需要手動建構和存取預覽 URL。

**首先**，您應該使用您選擇的令牌生成器建立一個 **秘密令牌字串**。此秘密僅由您的 Next.js 應用和無頭 CMS 知道。這可以防止沒有存取 CMS 權限的人存取預覽 URL。
...
export default async (req, res) => {
  ...
  // getPostBySlug 會實作所需的獲取邏輯到無頭 CMS
...
如果您請求一個具有 `getStaticProps` 的頁面，並且設定了預覽模式 cookies（通過 `res.setPreviewData`），則 `getStaticProps` 將在**請求時**（而非建置時）被呼叫。
...
我們在預覽 API 路由中使用了 `res.setPreviewData({})`，因此 `context.previewData` 將為 `{}`。如果需要，您可以使用它來傳遞會話資訊從預覽 API 路由到 `getStaticProps`。
...
> **須知**：在渲染期間，`next/router` 會公開一個 `isPreview` 標誌，詳見 [router 物件文件](/docs/pages/api-reference/functions/use-router#router-object)。
...
然後，發送請求到 `/api/clear-preview-mode-cookies` 以呼叫 API 路由。如果使用 [`next/link`](/docs/pages/api-reference/components/link) 呼叫此路由，您必須傳遞 `prefetch={false}` 以防止在連結預取時呼叫 `clearPreviewData`。
...
預覽模式也可以在 `getServerSideProps` 上使用。它也會在包含 `preview` 和 `previewData` 的 `context` 物件上可用。
