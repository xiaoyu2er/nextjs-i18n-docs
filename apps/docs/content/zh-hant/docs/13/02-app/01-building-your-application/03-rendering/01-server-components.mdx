...
translation-updated-at: 2025-05-25T21:51:22.493Z
title: 伺服器元件 (Server Components)
description: 了解如何使用 React 伺服器元件 (Server Components) 在伺服器端渲染部分應用程式。
related:
...
Next.js 預設使用伺服器元件 (Server Components)。這讓你能自動實現伺服器端渲染，無需額外配置。當需要時，你也可以選擇使用客戶端元件 (Client Components)，詳見[客戶端元件 (Client Components)](/docs/app/building-your-application/rendering/client-components)。

## 伺服器元件 (Server Components) 如何渲染？

在伺服器端，Next.js 使用 React 的 API 來協調渲染工作。渲染工作會被拆分為多個區塊：按個別路由區段和 [Suspense 邊界 (Suspense Boundaries)](https://react.dev/reference/react/Suspense) 劃分。
...
3. JavaScript 指令用於[水合 (hydrate)](https://react.dev/reference/react-dom/client/hydrateRoot) 客戶端元件 (Client Components)，使應用程式具備互動性。
...
使用靜態渲染時，路由會在**建置時**或在[資料重新驗證 (data revalidation)](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data) 後於背景渲染。結果會被快取並推送至[內容傳遞網路 (CDN)](https://developer.mozilla.org/docs/Glossary/CDN)。這項優化讓你能在使用者和伺服器請求之間共享渲染工作的結果。
...
>
> 了解更多關於[全路由快取 (full-route cache)](/docs/app/building-your-application/caching#full-route-cache) 和[資料快取 (Data Cache)](/docs/app/building-your-application/caching#data-cache) 的資訊。

#### 切換至動態渲染

在渲染過程中，如果發現[動態函數 (dynamic functions)](#dynamic-functions) 或[未快取的資料請求](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#opting-out-of-data-caching)，Next.js 會切換為動態渲染整個路由。下表總結了動態函數和資料快取如何影響路由是靜態還是動態渲染：
...
作為開發者，你無需在靜態和動態渲染之間選擇，因為 Next.js 會根據使用的功能和 API 自動為每個路由選擇最佳渲染策略。相反地，你可以選擇何時[快取或重新驗證特定資料](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating)，並可以選擇[串流 (Streaming)](#streaming) 部分 UI。

#### 動態函數

動態函數依賴於僅能在請求時確定的資訊，例如使用者的 cookies、當前請求標頭或 URL 的搜尋參數 (search params)。在 Next.js 中，這些動態函數包括：

- **[`cookies()`](/docs/app/api-reference/functions/cookies) 和 [`headers()`](/docs/app/api-reference/functions/headers)**：在伺服器元件 (Server Components) 中使用這些函數會讓整個路由在請求時切換為動態渲染。
- **[`useSearchParams()`](/docs/app/api-reference/functions/use-search-params)**：
  ...
  - 建議將使用 `useSearchParams()` 的客戶端元件 (Client Components) 包裹在 `<Suspense/>` 邊界中。這將允許其上方的任何客戶端元件 (Client Components) 進行靜態渲染。[範例](/docs/app/api-reference/functions/use-search-params#static-rendering)。
- **[`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional)**：使用[頁面 (Pages)](/docs/app/api-reference/file-conventions/page) 屬性會讓頁面在請求時切換為動態渲染。
...
<Image
  alt="圖表展示串流期間路由區段的並行化，顯示資料獲取、渲染和個別區塊的水合過程。"
...
<Image
  alt="圖表展示客戶端部分渲染的頁面，以及正在串流區塊的載入 UI。"
...
你可以使用 `loading.js` 和 [React Suspense](/docs/app/building-your-application/routing/loading-ui-and-streaming) 的 UI 元件來開始串流路由區段。詳見[載入 UI 與串流 (Loading UI and Streaming)](/docs/app/building-your-application/routing/loading-ui-and-streaming) 章節以獲取更多資訊。