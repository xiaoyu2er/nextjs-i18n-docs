...
translation-updated-at: 2025-05-25T21:49:55.742Z
title: CSS-in-JS
...
> **警告**：目前伺服器元件 (Server Components) 不支援需要執行時 JavaScript 的 CSS-in-JS 函式庫。將 CSS-in-JS 與 React 新功能 (如伺服器元件和串流) 一起使用，需要函式庫作者支援最新版本的 React，包括 [並行渲染 (concurrent rendering)](https://react.dev/blog/2022/03/29/react-v18#what-is-concurrent-react)。
>
...
可以使用任何現有的 CSS-in-JS 解決方案。最簡單的是行內樣式：
...
我們捆綁了 [styled-jsx](https://github.com/vercel/styled-jsx) 以提供對隔離作用域 CSS 的支援。目標是支援類似於 Web Components 的「影子 CSS」，但遺憾的是 [不支援伺服器渲染且僅限 JS](https://github.com/w3c/webcomponents/issues/71)。
...
是的，如果您停用 JavaScript，CSS 仍會在生產版本 (`next start`) 中載入。在開發期間，我們需要啟用 JavaScript 以提供最佳開發者體驗，包括 [快速刷新 (Fast Refresh)](https://nextjs.org/blog/next-9-4#fast-refresh)。
