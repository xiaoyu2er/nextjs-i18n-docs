...
`useRouter` 鉤子允許您在[客戶端元件 (Client Components)](/docs/app/building-your-application/rendering/client-components) 中以程式化的方式變更路由。
...
> **須知事項**：`<NavigationEvents>` 被包裹在 [`Suspense` 邊界](/docs/app/building-your-application/routing/loading-ui-and-streaming#example) 中，因為 [`useSearchParams()`](/docs/app/api-reference/functions/use-search-params) 在[靜態渲染 (static rendering)](/docs/app/building-your-application/rendering/server-components#static-rendering-default) 期間會導致客戶端渲染至最接近的 `Suspense` 邊界。[了解更多](/docs/app/api-reference/functions/use-search-params#behavior)。
