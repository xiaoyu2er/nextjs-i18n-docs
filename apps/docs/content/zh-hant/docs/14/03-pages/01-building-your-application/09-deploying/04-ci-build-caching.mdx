...
translation-updated-at: 2025-05-25T21:07:17.685Z
title: 持續整合 (CI) 建置快取
description: 了解如何設定 CI 來快取 Next.js 建置
---

為了提升建置效能，Next.js 會將快取儲存在 `.next/cache` 中，這些快取在建置之間是共用的。
...
with:
  # 關於使用 `yarn` 快取的說明請見此處 https://github.com/actions/cache/blob/main/examples.md#node---yarn 或者您可以使用 actions/setup-node 的快取功能 https://github.com/actions/setup-node
  path: |
    ~/.npm
    ${{ github.workspace }}/.next/cache
  # 當套件或原始檔變更時產生新的快取。
  key: ${{ runner.os }}-nextjs-${{ hashFiles('**/package-lock.json') }}-${{ hashFiles('**/*.js', '**/*.jsx', '**/*.ts', '**/*.tsx') }}
  # 如果原始檔變更但套件未變更，則從先前的快取重建。
  restore-keys: |
...
在您的 `bitbucket-pipelines.yml` 頂層（與 `pipelines` 同層級）加入或合併以下內容：
...
使用 Azure Pipelines 的 [快取任務](https://docs.microsoft.com/en-us/azure/devops/pipelines/tasks/utility/cache)，在您的管線 yaml 檔案中執行 `next build` 的任務之前加入以下任務：
