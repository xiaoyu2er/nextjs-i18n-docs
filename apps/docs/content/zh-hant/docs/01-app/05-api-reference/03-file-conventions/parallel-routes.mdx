...
translation-updated-at: 2025-06-01T22:31:45.593Z
...
平行路由是使用命名**插槽**建立的。插槽透過 `@folder` 慣例定義。例如，以下檔案結構定義了兩個插槽：`@analytics` 和 `@team`：
...
然而，插槽**不是**路由區段，不會影響 URL 結構。例如，對於 `/@analytics/views`，URL 會是 `/views`，因為 `@analytics` 是一個插槽。插槽與常規的[頁面](/docs/app/api-reference/file-conventions/page)元件結合，形成與路由區段關聯的最終頁面。因此，您無法在同一路由區段級別同時擁有獨立的[靜態](/docs/app/getting-started/partial-prerendering#static-rendering)和[動態](/docs/app/getting-started/partial-prerendering#dynamic-rendering)插槽。如果一個插槽是動態的，該級別的所有插槽都必須是動態的。

> **須知**：
>
> - `children` prop 是一個隱式插槽，不需要映射到資料夾。這意味著 `app/page.js` 等同於 `app/@children/page.js`。
...
預設情況下，Next.js 會追蹤每個插槽的活動 _狀態_（或子頁面）。然而，插槽內渲染的內容將取決於導航類型：

- [**軟導航**](/docs/app/building-your-application/routing/linking-and-navigating#5-soft-navigation)：在客戶端導航期間，Next.js 會執行[部分渲染](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering)，改變插槽內的子頁面，同時保持其他插槽的活動子頁面，即使它們與當前 URL 不匹配。
- **硬導航**：在完整頁面載入（瀏覽器重新整理）後，Next.js 無法確定不匹配當前 URL 的插槽的活動狀態。相反，它會為不匹配的插槽渲染 [`default.js`](#defaultjs) 檔案，如果 `default.js` 不存在，則渲染 `404`。

> **須知**：
>
> - 對於不匹配路由的 `404` 有助於確保您不會意外地在不應渲染平行路由的頁面上渲染它。
...
平行路由可以與[攔截路由 (Intercepting Routes)](/docs/app/api-reference/file-conventions/intercepting-routes) 一起使用，建立支援深層連結的模態框。這讓您可以解決建立模態框時的常見挑戰，例如：
...
然後，在 `@auth` 插槽內，添加 [`default.js`](/docs/app/api-reference/file-conventions/default) 檔案，返回 `null`。這確保模態框在不活動時不會渲染。
...
> **須知：**
>
> - 用於攔截路由的慣例（例如 `(.)`）取決於您的檔案系統結構。請參閱[攔截路由慣例](/docs/app/api-reference/file-conventions/intercepting-routes#convention)。
> - 透過將 `<Modal>` 功能與模態框內容（`<Login>`）分開，您可以確保模態框內的任何內容（例如[表單](/docs/app/guides/forms)）都是伺服器元件。請參閱[交錯客戶端和伺服器元件](/docs/app/getting-started/server-and-client-components#examples#supported-pattern-passing-server-components-to-client-components-as-props)以獲取更多資訊。
...
> **須知：**
>
> - 我們在 `@auth` 插槽中使用萬用路由來關閉模態框，因為平行路由的行為(#behavior)。由於客戶端導航到不再匹配插槽的路由時，插槽仍會保持可見，我們需要將插槽匹配到一個返回 `null` 的路由以關閉模態框。
> - 其他範例可能包括在相簿中開啟照片模態框，同時擁有專用的 `/photo/[id]` 頁面，或在側邊模態框中開啟購物車。
> - [查看範例](https://github.com/vercel-labs/nextgram)了解使用攔截和平行路由的模態框。
...
請參閱[載入 UI](/docs/app/building-your-application/routing/loading-ui-and-streaming) 和[錯誤處理](/docs/app/getting-started/error-handling)文件以獲取更多資訊。