...
translation-updated-at: 2025-05-25T20:50:53.831Z
title: route.js
...
`request` 物件是一個 [NextRequest](/docs/app/api-reference/functions/next-request) 物件，它擴充了 Web [Request](https://developer.mozilla.org/docs/Web/API/Request) API。`NextRequest` 讓您能進一步控制傳入的請求，包括輕鬆存取 `cookies` 和一個擴充的、解析過的 URL 物件 `nextUrl`。
...
- **`params`**: 一個解析為包含當前路由 [動態路由參數](/docs/app/building-your-application/routing/dynamic-routes) 物件的 Promise。
