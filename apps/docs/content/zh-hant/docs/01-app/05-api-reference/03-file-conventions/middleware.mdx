...
translation-updated-at: 2025-05-25T20:51:45.417Z
title: middleware.js
...
`middleware.js|ts` 檔案用於編寫 [中介軟體 (Middleware)](/docs/app/building-your-application/routing/middleware)，並在請求完成前於伺服器端執行程式碼。根據傳入的請求，您可以透過重寫、重新導向、修改請求或回應標頭，或直接回應來修改回應內容。
...
可選地，可以與中介軟體函式一起匯出一個設定物件。此物件包含 [matcher](#matcher) 來指定中介軟體適用的路徑。
...
- 多個路徑：使用陣列列出多個路徑，例如 `matcher: ['/about', '/contact']`，這會讓中介軟體同時套用於 `/about` 和 `/contact`。
...
中介軟體預設使用 [Edge 執行環境 (Edge runtime)](/docs/app/api-reference/edge)。若不需要此設定，您可以使用 [完整的 Node.js 執行環境](/blog/next-15-2#nodejs-middleware-experimental) 來執行中介軟體。
