...
translation-updated-at: 2025-06-01T22:29:37.451Z
title: 專案結構與組織
nav_title: 專案結構
description: 關於 Next.js 中的資料夾與檔案慣例概述，以及如何組織您的專案。
---
...
| [`public`](/docs/app/api-reference/file-conventions/public-folder) | 靜態資源檔案                       |
...
| [`not-found`](/docs/app/api-reference/file-conventions/not-found)             | `.js` `.jsx` `.tsx` | 404 頁面                     |
...
| [`default`](/docs/app/api-reference/file-conventions/default)                 | `.js` `.jsx` `.tsx` | 平行路由的後備頁面           |
...
| [`[...folder]`](/docs/app/api-reference/file-conventions/dynamic-routes#catch-all-segments)            | 全捕捉路由區段                   |
| [`[[...folder]]`](/docs/app/api-reference/file-conventions/dynamic-routes#optional-catch-all-segments) | 可選全捕捉路由區段               |

### 路由群組與私有資料夾

|                                                                                |                                                  |
| ------------------------------------------------------------------------------ | ------------------------------------------------ |
| [`(folder)`](/docs/app/api-reference/file-conventions/route-groups#convention) | 不影響路由的分組                                 |
...
| [`(.)folder`](/docs/app/api-reference/file-conventions/intercepting-routes#convention)      | 攔截同層級路由             |
| [`(..)folder`](/docs/app/api-reference/file-conventions/intercepting-routes#convention)     | 攔截上一層級路由           |
| [`(..)(..)folder`](/docs/app/api-reference/file-conventions/intercepting-routes#convention) | 攔截上兩層級路由           |
| [`(...)folder`](/docs/app/api-reference/file-conventions/intercepting-routes#convention)    | 從根目錄攔截路由           |
...
Next.js 對於如何組織和共置專案檔案**沒有強制規範**。但它確實提供了幾項功能來幫助您組織專案。
...
然而，即使路由結構是透過資料夾定義的，**除非**在路由區段中添加 `page.js` 或 `route.js` 檔案，否則該路由**不會公開可存取**。

<Image
  alt="圖表顯示在路由區段添加 page.js 或 route.js 檔案前，路由不會公開可存取"
...
而且，即使路由已公開可存取，只有 `page.js` 或 `route.js` **回傳的內容**會傳送到客戶端。

<Image
  alt="圖表顯示 page.js 和 route.js 檔案如何讓路由公開可存取"
...
這意味著**專案檔案**可以**安全地共置**於 `app` 目錄的路由區段中，而不會意外變成可路由的。

<Image
  alt="圖表顯示共置的專案檔案即使區段包含 page.js 或 route.js 也不會變成可路由"
...
> **須知事項**：雖然你可以將專案檔案共置於 `app` 目錄中，但這並非必要。如果你偏好，也可以[將它們保留在 `app` 目錄外](#store-project-files-outside-of-app)。

### 私有資料夾 (Private folders)

可以透過在資料夾名稱前加上底線來建立私有資料夾：`_folderName`

這表示該資料夾是私有實作細節，不應被路由系統考慮，從而**將該資料夾及其所有子資料夾排除在路由之外**。
...
由於 `app` 目錄中的檔案預設可以[安全共置](#colocation)，私有資料夾並非共置的必要條件。但它們在以下情況很有用：
...
> - 如果不使用私有資料夾，了解 Next.js 的[特殊檔案慣例](/docs/app/getting-started/project-structure#routing-files)有助於避免意外的命名衝突。
...
Next.js 支援將應用程式碼（包括 `app`）儲存在可選的 [`src` 資料夾](/docs/app/api-reference/file-conventions/src-folder)中。這將應用程式碼與主要位於專案根目錄的專案設定檔分開。
...
> **須知事項**：在下面的範例中，我們使用 `components` 和 `lib` 資料夾作為通用佔位符，它們的命名沒有特殊的框架意義，你的專案可能會使用其他資料夾，如 `ui`、`utils`、`hooks`、`styles` 等。
...
此策略將全域共用的應用程式碼儲存在 `app` 根目錄中，並將更具體的應用程式碼**拆分**到使用它們的路由區段中。
...
要透過 `loading.js` 檔案將[載入骨架](/docs/app/building-your-application/routing/loading-ui-and-streaming)應用到特定路由，可以建立一個新的路由群組（例如 `/(overview)`），然後將你的 `loading.tsx` 移動到該路由群組中。
...
要建立多個[根佈局](/docs/app/api-reference/file-conventions/layout#root-layout)，請移除頂層的 `layout.js` 檔案，並在每個路由群組中添加一個 `layout.js` 檔案。這適用於將應用程式分割為具有完全不同 UI 或體驗的區塊。每個根佈局都需要添加 `<html>` 和 `<body>` 標籤。
