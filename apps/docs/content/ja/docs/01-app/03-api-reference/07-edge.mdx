...
- **Edge ランタイム**: より限定された [API セット](#reference) を含み、[Middleware](/docs/app/api-reference/file-conventions/middleware) で使用されます

## 注意事項

- Edge ランタイムはすべての Node.js API をサポートしていません。一部のパッケージは期待通りに動作しない可能性があります
- Edge ランタイムは 静的生成の増分更新 (ISR) をサポートしていません
- 両方のランタイムはデプロイアダプターに応じて [ストリーミング](/docs/app/api-reference/file-conventions/loading) をサポートできます
...
| API                                                                                 | 説明                                |
| ----------------------------------------------------------------------------------- | ----------------------------------- |
| [`atob`](https://developer.mozilla.org/en-US/docs/Web/API/atob)                     | base-64 エンコード文字列をデコード  |
...
| [`TextDecoderStream`](https://developer.mozilla.org/docs/Web/API/TextDecoderStream) | ストリーム用チェーン可能デコーダー   |
| [`TextEncoder`](https://developer.mozilla.org/docs/Web/API/TextEncoder)             | 文字列を Uint8Array にエンコード    |
| [`TextEncoderStream`](https://developer.mozilla.org/docs/Web/API/TextEncoderStream) | ストリーム用チェーン可能エンコーダー |
...
| API                                                                       | 説明                                                                                         |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| [`crypto`](https://developer.mozilla.org/docs/Web/API/Window/crypto)      | プラットフォームの暗号機能にアクセス                                                         |
...
| [`ArrayBuffer`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer)               | 汎用的な固定長の生バイナリデータバッファを表す                                                                                                                                                      |
...
| [`queueMicrotask`](https://developer.mozilla.org/docs/Web/API/queueMicrotask)                                         | 実行するマイクロタスクをキューに入れる                                                                                                                                                              |
...
| [`SharedArrayBuffer`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer)   | 汎用的な固定長の生バイナリデータバッファを表す                                                                                                                                                      |
...
| [`URIError`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/URIError)                     | グローバルURI処理関数が誤った方法で使用された場合のエラーを表す                                                                                                                                     |
...
| [`WeakMap`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/WeakMap)                       | キーが弱参照されるキー/値ペアのコレクションを表す                                                                                                                                                   |
...
まれなケースとして、コードに（またはインポートしたモジュールに）ランタイム時に到達できない動的コード評価文が含まれており、ツリーシェイキングで除去できない場合があります。
...
`unstable_allowDynamic` は [グロブ](https://github.com/micromatch/micromatch#matching-features)、またはグロブの配列で、特定のファイルに対する動的コード評価を無視します。グロブはアプリケーションのルートフォルダからの相対パスです。

これらの文が Edge で実行された場合、**エラーがスローされランタイムエラーを引き起こす**ことに注意してください。
