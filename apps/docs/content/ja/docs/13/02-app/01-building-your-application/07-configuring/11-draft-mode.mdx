...
translation-updated-at: 2025-06-06T17:21:46.610Z
title: ドラフトモード
description: Next.jsには静的ページと動的ページを切り替えるドラフトモードがあります。App Routerでの動作について学びましょう。
---

静的レンダリングはヘッドレスCMSからデータを取得するページに便利です。しかし、ヘッドレスCMSでドラフトを執筆中に、すぐにページ上でそのドラフトを確認したい場合には理想的ではありません。このような場合、Next.jsにはビルド時ではなく**リクエスト時**にページをレンダリングし、公開済みコンテンツではなくドラフトコンテンツを取得してほしいでしょう。この特定のケースでのみ、Next.jsが[動的レンダリング](/docs/app/building-your-application/rendering/server-components#dynamic-rendering)に切り替わるようにしたいはずです。
...
`/api/draft`にアクセスし、ブラウザの開発者ツールで確認することで手動でテストできます。`__prerender_bypass`という名前のCookieを持つ`Set-Cookie`レスポンスヘッダーに注目してください。
...
**まず**、任意のトークンジェネレーターを使用して**秘密トークン文字列**を作成します。この秘密はNext.jsアプリとヘッドレスCMSのみが知っている状態にします。これにより、CMSにアクセスできない人がドラフトURLにアクセスするのを防ぎます。
...
Cookieが設定されたページをリクエストすると、データはビルド時ではなく**リクエスト時**に取得されます。
...
これで完了です！ヘッドレスCMSから（`secret`と`slug`を使用して）ドラフトルートハンドラにアクセスするか、手動でアクセスすると、ドラフトコンテンツを確認できるようになります。また、ドラフトを公開せずに更新した場合も、ドラフトを確認できます。
...
ドラフトモードCookieを手動でクリアするには、`draftMode().disable()`を呼び出すルートハンドラを作成します:
...
その後、`/api/disable-draft`にリクエストを送信してルートハンドラを呼び出します。[`next/link`](/docs/app/api-reference/components/link)を使用してこのルートを呼び出す場合、プリフェッチ時に誤ってCookieが削除されないように`prefetch={false}`を渡す必要があります。
