...
translation-updated-at: 2025-06-06T17:18:22.456Z
title: fetch
...
Next.js は [Web `fetch()` API](https://developer.mozilla.org/docs/Web/API/Fetch_API) を拡張しているため、[利用可能なネイティブオプション](https://developer.mozilla.org/docs/Web/API/fetch#parameters) をすべて使用できます。
...
> **知っておくと便利**:
>
> - `cache` オプションを指定しない場合、Next.js はデフォルトで `force-cache` を使用しますが、`cookies()` などの [動的関数 (dynamic function)](/docs/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions) が使用されている場合は `no-store` がデフォルトになります。
...
> **知っておくと便利**:
>
> - 個々の `fetch()` リクエストで設定された `revalidate` の値がルートの [デフォルトの `revalidate`](/docs/app/api-reference/file-conventions/route-segment-config#revalidate) よりも小さい場合、ルート全体の再検証間隔が短縮されます。
> - 同じルート内で同じ URL に対する2つの fetch リクエストに異なる `revalidate` 値がある場合、小さい方が使用されます。
> - 便利なことに、`revalidate` が数値に設定されている場合、`cache` オプションを設定する必要はありません。`0` は `cache: 'no-store'` を意味し、正の値は `cache: 'force-cache'` を意味するためです。
...
リソースのキャッシュタグを設定します。その後、[`revalidateTag`](https://nextjs.org/docs/app/api-reference/functions/revalidateTag) を使用してオンデマンドでデータを再検証できます。カスタムタグの最大長は256文字です。
