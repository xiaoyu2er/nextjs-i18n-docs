...
translation-updated-at: 2025-06-06T17:33:30.176Z
...
- [画像ファイル (.jpg, .png, .gif) を使用](#画像ファイル-jpg-png-gif)
...
`opengraph-image.(jpg|jpeg|png|gif)` 画像と同じルートセグメントに、代替テキスト用の `opengraph-image.alt.txt` ファイルを追加します。
...
`twitter-image.(jpg|jpeg|png|gif)` 画像と同じルートセグメントに、代替テキスト用の `twitter-image.alt.txt` ファイルを追加します。
...
[画像ファイル](#画像ファイル-jpg-png-gif)を使用するだけでなく、コードを使用して画像をプログラムで**生成**できます。
...
> **知っておくと良いこと**
>
> - デフォルトでは、生成された画像は[**静的最適化**](/docs/app/building-your-application/rendering/server-components#static-rendering-default)されます（ビルド時に生成されキャッシュされます）。ただし、[動的関数](/docs/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-functions)またはキャッシュされていないデータを使用する場合は除きます。
> - [`generateImageMetadata`](/docs/app/api-reference/functions/generate-image-metadata) を使用して、同じファイル内で複数の画像を生成できます。

画像を生成する最も簡単な方法は、`next/server` の [ImageResponse](/docs/app/api-reference/functions/image-response) API を使用することです。
...
`opengraph-image` と `twitter-image` は特殊な[ルートハンドラ](/docs/app/building-your-application/routing/route-handlers)であり、ページやレイアウトと同じ[ルートセグメント設定](/docs/app/api-reference/file-conventions/route-segment-config)オプションを使用できます。
