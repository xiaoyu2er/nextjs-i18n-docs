...
translation-updated-at: 2025-06-06T17:11:22.290Z
...
Si `getServerSideProps` o `getInitialProps` está presente en una página, Next.js cambiará a renderizar la página bajo demanda, por solicitud (lo que significa [Renderizado del lado del servidor (SSR)](/docs/pages/building-your-application/rendering/server-side-rendering)).
...
Los casos en los que la consulta se actualizará después de la hidratación, activando otro renderizado, son:
...
- Hay [reescrituras (rewrites)](/docs/pages/api-reference/next-config-js/rewrites) configuradas en tu `next.config.js`, ya que estas pueden tener parámetros que pueden necesitar ser analizados y proporcionados en la `query`.
...
- Si tienes una [`App` personalizada](/docs/pages/building-your-application/routing/custom-app) con `getInitialProps`, entonces esta optimización se desactivará en páginas sin [Generación Estática](/docs/pages/building-your-application/data-fetching/get-static-props).
