...
translation-updated-at: 2025-06-06T17:14:32.059Z
title: Rutas API
description: Next.js soporta Rutas API, que le permiten construir su API sin salir de su aplicación Next.js. Aprenda cómo funciona aquí.
---

<details>
  <summary>Ejemplos</summary>

- [Rutas API básicas](https://github.com/vercel/next.js/tree/canary/examples/api-routes)
...
> **Bueno saber**: Si está usando el App Router, puede usar [Componentes del Servidor](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating) o [Manejadores de Ruta](/docs/app/building-your-application/routing/route-handlers) en lugar de Rutas API.

Las rutas API proporcionan una solución para construir una **API pública** con Next.js.

Cualquier archivo dentro de la carpeta `pages/api` se mapea a `/api/*` y será tratado como un endpoint API en lugar de una `página`. Son paquetes exclusivos del lado del servidor y no aumentarán el tamaño de su paquete del lado del cliente.
...
> **Bueno saber**:
>
> - Las Rutas API [no especifican cabeceras CORS](https://developer.mozilla.org/docs/Web/HTTP/CORS), lo que significa que son **solo del mismo origen** por defecto. Puede personalizar este comportamiento envolviendo el manejador de solicitudes con los [helpers de solicitud CORS](https://github.com/vercel/next.js/tree/canary/examples/api-routes-cors).
> - Las Rutas API no pueden usarse con [exportaciones estáticas](/docs/pages/building-your-application/deploying/static-exports). Sin embargo, los [Manejadores de Ruta](/docs/app/building-your-application/routing/route-handlers) en el App Router sí pueden.
> - Las Rutas API se verán afectadas por la configuración [`pageExtensions`](/docs/pages/api-reference/next-config-js/pageExtensions) en `next.config.js`.
...
- `req`: Una instancia de [http.IncomingMessage](https://nodejs.org/api/http.html#class-httpincomingmessage), más algunos [middlewares preconstruidos](/docs/pages/api-reference/functions/next-server)
- `res`: Una instancia de [http.ServerResponse](https://nodejs.org/api/http.html#class-httpserverresponse), más algunas [funciones helper](/docs/pages/api-reference/functions/next-server)
...
- `req.query` - Un objeto que contiene la [cadena de consulta](https://es.wikipedia.org/wiki/Query_string). Por defecto `{}`
...
`bodyParser` está habilitado automáticamente. Si desea consumir el cuerpo como un `Stream` o con [`raw-body`](https://www.npmjs.com/package/raw-body), puede establecer esto en `false`.

Un caso de uso para deshabilitar el `bodyParsing` automático es permitirle verificar el cuerpo crudo de una solicitud de **webhook**, por ejemplo [desde GitHub](https://docs.github.com/es/developers/webhooks-and-events/webhooks/securing-your-webhooks#validating-payloads-from-github).
...
`externalResolver` es una bandera explícita que le indica al servidor que esta ruta está siendo manejada por un resolver externo como _express_ o _connect_. Habilitar esta opción desactiva las advertencias para solicitudes no resueltas.
...
El [objeto Server Response](https://nodejs.org/api/http.html#http_class_http_serverresponse), (a menudo abreviado como `res`) incluye un conjunto de métodos helper similares a Express.js para mejorar la experiencia del desarrollador y aumentar la velocidad de creación de nuevos endpoints API.

Los helpers incluidos son:

- `res.status(code)` - Una función para establecer el código de estado. `code` debe ser un [código de estado HTTP](https://es.wikipedia.org/wiki/Anexo:C%C3%B3digos_de_estado_HTTP) válido
- `res.json(body)` - Envía una respuesta JSON. `body` debe ser un [objeto serializable](https://developer.mozilla.org/es/docs/Glossary/Serialization)
- `res.send(body)` - Envía la respuesta HTTP. `body` puede ser un `string`, un `object` o un `Buffer`
- `res.redirect([status,] path)` - Redirige a una ruta o URL especificada. `status` debe ser un [código de estado HTTP](https://es.wikipedia.org/wiki/Anexo:C%C3%B3digos_de_estado_HTTP) válido. Si no se especifica, `status` por defecto es "307" "Redirección temporal".
- `res.revalidate(urlPath)` - [Revalida una página bajo demanda](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) usando `getStaticProps`. `urlPath` debe ser un `string`.
...
El siguiente ejemplo establece el código de estado de la respuesta a `200` (`OK`) y devuelve una propiedad `message` con el valor de `Hello from Next.js!` como una respuesta JSON:
...
Al enviar una respuesta al cliente puede enviar una respuesta JSON, esta debe ser un [objeto serializable](https://developer.mozilla.org/es/docs/Glossary/Serialization).
En una aplicación del mundo real, es posible que desee informar al cliente sobre el estado de la solicitud dependiendo del resultado del endpoint solicitado.
...
Puede hacer que sus Rutas API sean más seguras en cuanto a tipos importando los tipos `NextApiRequest` y `NextApiResponse` de `next`, además de estos, también puede tipar sus datos de respuesta:
...
> **Bueno saber**: El cuerpo de `NextApiRequest` es `any` porque el cliente puede incluir cualquier carga útil. Debe validar el tipo/forma del cuerpo en tiempo de ejecución antes de usarlo.
...
> **Bueno saber**: Puede usar nombres distintos a `slug`, como: `[...param]`
...
Si desea usar Rutas API con el Edge Runtime, recomendamos adoptar incrementalmente el App Router y usar [Manejadores de Ruta](/docs/app/building-your-application/routing/route-handlers) en su lugar.

La firma de función de los Manejadores de Ruta es isomórfica, lo que significa que puede usar la misma función para ambos entornos de ejecución Edge y Node.js.