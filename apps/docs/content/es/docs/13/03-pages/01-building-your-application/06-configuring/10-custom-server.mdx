...
translation-updated-at: 2025-06-06T17:09:22.195Z
...
Por defecto, Next.js incluye su propio servidor con `next start`. Si ya tiene un backend existente, aún puede usarlo con Next.js (esto no es un servidor personalizado). Un servidor personalizado de Next.js le permite iniciar un servidor 100% programáticamente para utilizar patrones de servidor personalizados. La mayoría de las veces no necesitará esto, pero está disponible para una personalización completa.
...
| `customServer` | `Boolean`          | (_Opcional_) Establézcalo en false cuando el servidor fue creado por Next.js                                  |
| `dev`          | `Boolean`          | (_Opcional_) Si Next.js debe iniciarse en modo desarrollo. Por defecto `false`                                |
...
La `app` devuelta puede usarse luego para que Next.js maneje las solicitudes según sea necesario.

## Deshabilitar el enrutamiento por sistema de archivos

Por defecto, `Next` servirá cada archivo en la carpeta `pages` bajo un pathname que coincida con el nombre del archivo. Si su proyecto usa un servidor personalizado, este comportamiento puede resultar en que el mismo contenido se sirva desde múltiples rutas, lo que puede causar problemas con SEO y UX.
