...
translation-updated-at: 2025-06-06T17:10:34.983Z
...
En la [documentación de Páginas](/docs/pages/building-your-application/routing/pages-and-layouts) y la [documentación de Obtención de Datos](/docs/pages/building-your-application/data-fetching), explicamos cómo pre-renderizar una página en tiempo de construcción (**Generación Estática**) usando `getStaticProps` y `getStaticPaths`.

La Generación Estática es útil cuando tus páginas obtienen datos de un CMS headless. Sin embargo, no es ideal cuando estás escribiendo un borrador en tu CMS headless y quieres **previsualizarlo** inmediatamente en tu página. En este caso, querrías que Next.js renderice estas páginas en **tiempo de solicitud** en lugar de en tiempo de construcción, y que obtenga el contenido del borrador en lugar del contenido publicado. Querrías que Next.js evite la Generación Estática solo para este caso específico.
...
`res.setPreviewData` establece algunas **cookies** en el navegador que activan el modo de vista previa. Cualquier solicitud a Next.js que contenga estas cookies se considerará en **modo de vista previa**, y el comportamiento para las páginas generadas estáticamente cambiará (más sobre esto más adelante).
...
En la práctica, querrás llamar a esta ruta API de forma _segura_ desde tu CMS headless. Los pasos específicos variarán según el CMS headless que estés usando, pero aquí hay algunos pasos comunes que podrías seguir.

Estos pasos asumen que el CMS headless que estás usando admite la configuración de **URLs de vista previa personalizadas**. Si no es así, aún puedes usar este método para proteger tus URLs de vista previa, pero necesitarás construir y acceder a la URL de vista previa manualmente.
...
**Segundo**, si tu CMS headless admite la configuración de URLs de vista previa personalizadas, especifica lo siguiente como la URL de vista previa. Esto asume que tu ruta API de vista previa está ubicada en `pages/api/preview.js`.
...
- `<path>` debe ser la ruta de la página que deseas previsualizar. Si deseas previsualizar `/posts/foo`, entonces debes usar `&slug=/posts/foo`.

Tu CMS headless podría permitirte incluir una variable en la URL de vista previa para que `<path>` pueda establecerse dinámicamente según los datos del CMS, como: `&slug=/posts/{entry.fields.slug}`
...
export default async (req, res) => {
  ...
  // getPostBySlug implementaría la lógica de consulta necesaria al CMS headless
...
Si solicitas una página que tiene `getStaticProps` con las cookies del modo de vista previa establecidas (a través de `res.setPreviewData`), entonces `getStaticProps` se llamará en **tiempo de solicitud** (en lugar de en tiempo de construcción).
...
Por ejemplo, tu CMS headless podría tener un punto final API diferente para los posts en borrador. Si es así, puedes usar `context.preview` para modificar la URL del punto final API como se muestra a continuación:

```js
export async function getStaticProps(context) {
  // Si context.preview es true, añade "/preview" al punto final API
  // para solicitar datos de borrador en lugar de datos publicados. Esto variará
  // según el CMS headless que estés usando.
...
Establece esto como la URL de vista previa en tu CMS headless o accede manualmente, y deberías poder ver la vista previa.
...
- `path`: Especifica la ruta bajo la cual se debe aplicar la cookie. Por defecto es `/`, lo que activa el modo de vista previa para todas las rutas.
...
Para limpiar manualmente las cookies del Modo de Vista Previa, crea una ruta API que llame a `clearPreviewData()`:
...
Tanto el valor de la cookie de bypass como la clave privada para cifrar los `previewData` cambian cuando se completa `next build`. Esto asegura que la cookie de bypass no pueda ser adivinada.

> **Bueno saber**: Para probar el Modo de Vista Previa localmente sobre HTTP, tu navegador deberá permitir cookies de terceros y acceso al almacenamiento local.