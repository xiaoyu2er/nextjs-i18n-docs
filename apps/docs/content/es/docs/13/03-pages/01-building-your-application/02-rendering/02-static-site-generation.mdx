...
translation-updated-at: 2025-06-06T17:12:30.504Z
title: Generación de sitios estáticos (SSG)
description: Uso de Generación de sitios estáticos (SSG) para prerenderizar páginas en tiempo de compilación.
---
...
Si una página utiliza **Generación Estática**, el HTML de la página se genera en **tiempo de compilación**. Esto significa que en producción, el HTML de la página se genera cuando ejecutas `next build`. Este HTML se reutilizará en cada solicitud y puede ser almacenado en caché por una CDN.

En Next.js, puedes generar páginas estáticamente **con o sin datos**. Veamos cada caso.

### Generación Estática sin datos

Por defecto, Next.js prerrenderiza páginas usando Generación Estática sin obtener datos. Aquí tienes un ejemplo:
...
2. Las **rutas** de tu página dependen de datos externos: Usa `getStaticPaths` (normalmente junto con `getStaticProps`).
...
export async function getStaticPaths() {
  ...
  // Solo prerrenderizaremos estas rutas en tiempo de compilación.
...
- Usar Generación Estática con **obtención de datos del lado del cliente:** Puedes omitir el prerrenderizado de algunas partes de una página y luego usar JavaScript del lado del cliente para poblarlas. Para aprender más sobre este enfoque, consulta la [documentación de Obtención de Datos](/docs/pages/building-your-application/data-fetching/client-side).
- Usar **Renderizado del lado del servidor (SSR):** Next.js prerrenderiza una página en cada solicitud. Será más lento porque la página no puede ser almacenada en caché por una CDN, pero la página prerrenderizada siempre estará actualizada. Hablaremos de este enfoque más adelante.