...
translation-updated-at: 2025-06-06T17:07:38.970Z
...
Exportar una función llamada `getStaticProps` generará previamente una página en el momento de compilación usando las propiedades devueltas por la función:
...
| Nombre           | Descripción                                                                                                                                                                                                                                                                                          |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `params`        | Contiene los parámetros de ruta para páginas que usan [rutas dinámicas](/docs/pages/building-your-application/routing/dynamic-routes). Por ejemplo, si el nombre de la página es `[id].js`, entonces `params` tendrá la forma `{ id: ... }`. Debes usar esto junto con `getStaticPaths`, que explicaremos más adelante. |
| `preview`       | (Obsoleto en favor de `draftMode`) `preview` es `true` si la página está en [Modo de Vista Previa](/docs/pages/building-your-application/configuring/preview-mode) y `false` en caso contrario.                                                                                                                         |
| `previewData`   | (Obsoleto en favor de `draftMode`) Los datos de [vista previa](/docs/pages/building-your-application/configuring/preview-mode) configurados por `setPreviewData`.                                                                                                                                                             |
...
```js
// Esta función se llama en el momento de compilación en el servidor.
...
Si las redirecciones son conocidas en el momento de compilación, deben agregarse en [`next.config.js`](/docs/pages/api-reference/next-config-js/redirects) en su lugar.
...
Dado que Next.js compila tu código en un directorio separado, no puedes usar `__dirname` ya que la ruta que devuelve será diferente a la del Enrutador de Páginas.
...
// posts se llenará en el momento de compilación por getStaticProps()
function Blog({ posts }) {
  return (
    <ul>
      {posts.map((post) => (
...
export async function getStaticProps() {
  ...
  // recibirá `posts` como prop en el momento de compilación
  return {
    props: {
...
| `v12.1.0` | Se agregó [Regeneración Estática Incremental Bajo Demanda](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) (beta). |
