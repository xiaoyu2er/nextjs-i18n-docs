...
translation-updated-at: 2025-06-06T17:21:31.492Z
...
Cuando ejecutas `next build` para generar una exportación estática, los Componentes del Servidor consumidos dentro del directorio `app` se ejecutarán durante la compilación, similar a la generación tradicional de sitios estáticos.

El componente resultante se renderizará en HTML estático para la carga inicial de la página y en una carga útil estática para la navegación entre rutas en el cliente. No se requieren cambios en tus Componentes del Servidor al usar la exportación estática, a menos que consuman [funciones dinámicas del servidor](#funciones-no-admitidas).

```tsx filename="app/page.tsx" switcher
export default async function Page() {
  // Esta petición fetch se ejecutará en el servidor durante `next build`
...
Si deseas realizar la obtención de datos en el cliente, puedes usar un Componente del Cliente con [SWR](https://github.com/vercel/swr) para memorizar las peticiones.
...
Dado que las transiciones entre rutas ocurren en el cliente, esto se comporta como una SPA tradicional. Por ejemplo, la siguiente ruta de índice te permite navegar a diferentes publicaciones en el cliente:
...
Los Manejadores de Ruta renderizarán una respuesta estática al ejecutar `next build`. Solo se admite el verbo HTTP `GET`. Esto puede usarse para generar archivos estáticos HTML, JSON, TXT u otros a partir de datos cacheados o no cacheados. Por ejemplo:
...
Los Componentes del Cliente se prerrenderizan a HTML durante `next build`. Dado que [APIs web](https://developer.mozilla.org/docs/Web/API) como `window`, `localStorage` y `navigator` no están disponibles en el servidor, debes acceder a estas APIs de forma segura solo cuando se ejecuta en el navegador. Por ejemplo:
...
Al ejecutar `next build`, Next.js genera la exportación estática en la carpeta `out`. Ya no es necesario usar `next export`. Por ejemplo, supongamos que tienes las siguientes rutas:
