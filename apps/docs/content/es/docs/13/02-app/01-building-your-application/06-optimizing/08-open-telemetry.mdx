...
translation-updated-at: 2025-06-06T17:24:28.508Z
title: OpenTelemetry
...
> **Es bueno saberlo**: Esta característica es **experimental**, debes activarla explícitamente configurando `experimental.instrumentationHook = true;` en tu `next.config.js`.
...
Esta documentación utiliza términos como _Span_, _Trace_ o _Exporter_ a lo largo del documento, todos los cuales puedes encontrar en [el manual de observabilidad de OpenTelemetry](https://opentelemetry.io/docs/concepts/observability-primer/).
...
> **Es bueno saberlo**: Actualmente solo soportamos enlaces de OpenTelemetry en funciones serverless.
> No proporcionamos ninguno para código en el `edge` o del lado del cliente.
...
Luego, crea un archivo personalizado [`instrumentation.ts`](/docs/app/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):

</AppOnly>

<PagesOnly>

Luego, crea un archivo personalizado [`instrumentation.ts`](/docs/pages/building-your-application/optimizing/instrumentation) (o `.js`) en el **directorio raíz** del proyecto (o dentro de la carpeta `src` si usas una):
...
Si nuestro envoltorio `@vercel/otel` no se ajusta a tus necesidades, puedes configurar OpenTelemetry manualmente.
...
Las APIs de OpenTelemetry no son compatibles con el runtime edge, así que debes asegurarte de importarlas solo cuando `process.env.NEXT_RUNTIME === 'nodejs'`. Recomendamos crear un nuevo archivo `instrumentation.node.ts` que importes condicionalmente solo cuando uses node:
...
Hacer esto es equivalente a usar `@vercel/otel`, pero es posible modificarlo y extenderlo.
Por ejemplo, podrías usar `@opentelemetry/exporter-trace-otlp-grpc` en lugar de `@opentelemetry/exporter-trace-otlp-http` o puedes especificar más atributos de recurso.

## Probando tu instrumentación

Necesitas un colector OpenTelemetry con un backend compatible para probar los traces de OpenTelemetry localmente.
...
Para hacer esto, sigue la [guía de inicio de OpenTelemetry Collector](https://opentelemetry.io/docs/collector/getting-started/), que te guiará a través de la configuración del colector y cómo configurarlo para recibir datos de tu aplicación Next.js.
...
Recomendamos usar OpenTelemetry Collector.
Si eso no es posible en tu plataforma, puedes usar un exportador personalizado de OpenTelemetry con [configuración manual de OpenTelemetry](/docs/pages/building-your-application/optimizing/open-telemetry#manual-opentelemetry-configuration)
...
- `next.page`
  ...
  - Puede usarse como un identificador único solo cuando se combina con `next.route` porque `/layout` puede usarse para identificar tanto `/(groupA)/layout.ts` como `/(groupB)/layout.ts`
