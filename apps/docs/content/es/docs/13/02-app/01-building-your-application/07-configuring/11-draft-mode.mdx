...
translation-updated-at: 2025-06-06T17:21:52.995Z
title: Modo borrador (Draft Mode)
description: Next.js incluye un modo borrador para alternar entre páginas estáticas y dinámicas. Aprende cómo funciona con el App Router aquí.
---

El renderizado estático es útil cuando tus páginas obtienen datos de un CMS headless. Sin embargo, no es ideal cuando estás escribiendo un borrador en tu CMS headless y quieres verlo inmediatamente en tu página. En este caso, preferirías que Next.js renderice estas páginas en **tiempo de solicitud** en lugar de en tiempo de construcción, y que obtenga el contenido del borrador en vez del contenido publicado. Querrías que Next.js cambie a [renderizado dinámico](/docs/app/building-your-application/rendering/server-components#dynamic-rendering) solo para este caso específico.

Next.js tiene una función llamada **Modo Borrador (Draft Mode)** que resuelve este problema. A continuación, las instrucciones para usarlo.
...
Esto establecerá una **cookie** para habilitar el modo borrador. Las solicitudes posteriores que incluyan esta cookie activarán el **Modo Borrador**, cambiando el comportamiento para páginas generadas estáticamente (más sobre esto más adelante).

Puedes probar esto manualmente visitando `/api/draft` y revisando las herramientas de desarrollo de tu navegador. Nota el encabezado de respuesta `Set-Cookie` con una cookie llamada `__prerender_bypass`.
...
**Primero**, debes crear un **token secreto** usando un generador de tokens de tu preferencia. Este secreto solo lo conocerán tu aplicación Next.js y tu CMS headless. Esto evita que personas sin acceso al CMS puedan acceder a las URLs de borrador.
...
export async function GET(request: Request) {
  // Parsea los parámetros de la cadena de consulta
...
Si solicitas una página que tiene la cookie establecida, los datos se obtendrán en **tiempo de solicitud** (en lugar de en tiempo de construcción).
