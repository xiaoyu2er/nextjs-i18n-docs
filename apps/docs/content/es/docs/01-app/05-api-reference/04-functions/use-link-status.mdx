...
translation-updated-at: 2025-06-02T20:06:48.300Z
title: useLinkStatus
...
El hook `useLinkStatus` te permite rastrear el estado **pendiente** de un `<Link>`. Puedes usarlo para mostrar retroalimentación visual en línea al usuario (como spinners o efectos de texto) mientras se completa la navegación a una nueva ruta.

`useLinkStatus` es útil cuando:

- La [precarga (prefetching)](/docs/app/building-your-application/routing/linking-and-navigating#2-prefetching) está deshabilitada o en progreso, lo que significa que la navegación está bloqueada.
...
`useLinkStatus` no recibe ningún parámetro.
