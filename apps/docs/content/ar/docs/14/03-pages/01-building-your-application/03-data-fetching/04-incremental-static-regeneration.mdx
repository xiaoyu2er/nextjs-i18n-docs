...
بعد ذلك، أضف الرمز السري كـ [متغير بيئة](/docs/pages/building-your-application/configuring/environment-variables) إلى تطبيقك. أخيرًا، أنشئ مسار API إعادة التحقق:
...
عند التشغيل محليًا باستخدام `next dev`، يتم استدعاء `getStaticProps` عند كل طلب. للتحقق من صحة تكوين ISR عند الطلب، ستحتاج إلى إنشاء [بناء إنتاج](/docs/pages/api-reference/cli/next#next-build-options) وبدء [خادم الإنتاج](/docs/pages/api-reference/cli/next#next-start-options):
...
يعمل التحديث التدريجي للصفحات الثابتة (ISR) على [مواقع Next.js المخصصة للاستضافة الذاتية](/docs/pages/building-your-application/deploying#self-hosting) مباشرة عند استخدام `next start`.

تعلم المزيد عن [استضافة Next.js ذاتيًا](/docs/pages/building-your-application/deploying#self-hosting).
