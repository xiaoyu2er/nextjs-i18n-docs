...
يسمح `getStaticPaths` لك بالتحكم في الصفحات التي يتم إنشاؤها أثناء البناء بدلاً من إنشائها عند الطلب باستخدام [`fallback`](/docs/pages/api-reference/functions/get-static-paths#fallback-blocking). إنشاء المزيد من الصفحات أثناء البناء سيؤدي إلى عمليات بناء أبطأ.
