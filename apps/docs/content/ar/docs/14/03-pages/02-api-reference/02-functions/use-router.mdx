...
- `isReady`: `boolean` - ما إذا كانت حقول الموجه محدثة من جانب العميل وجاهزة للاستخدام. يجب استخدامها فقط داخل طرق `useEffect` وليس للتصيير الشرطي على الخادم. راجع الوثائق ذات الصلة لحالة الاستخدام مع [الصفحات المحسنة تلقائيًا بشكل ثابت](/docs/pages/building-your-application/rendering/automatic-static-optimization).
...
تحويل المستخدم إلى `pages/login.js`، مفيد للصفحات خلف [المصادقة](/docs/pages/building-your-application/authentication):
...
يمكنك استخدام كائن URL بنفس الطريقة التي يمكنك استخدامها مع [`next/link`](/docs/pages/api-reference/components/link#with-url-object). يعمل لكل من معلمتي `url` و `as`:
...
جلب الصفحات مسبقًا لتحقيق انتقالات أسرع من جانب العميل. هذه الطريقة مفيدة فقط للتنقلات بدون [`next/link`](/docs/pages/api-reference/components/link)، حيث يتولى `next/link` جلب الصفحات مسبقًا تلقائيًا.
...
إذا أعادت `cb` `false`، فلن يتعامل موجه Next.js مع `popstate`، وسيكون مسؤوليتك التعامل معه في تلك الحالة. راجع [تعطيل توجيه نظام الملفات](/docs/pages/building-your-application/configuring/custom-server#disabling-file-system-routing).
...
> نستخدم [تطبيقًا مخصصًا](/docs/pages/building-your-application/routing/custom-app) (`pages/_app.js`) لهذا المثال للاشتراك في الحدث لأنه لا يتم فك تحميله عند تنقلات الصفحة، ولكن يمكنك الاشتراك في أحداث الموجه في أي مكون في تطبيقك.
