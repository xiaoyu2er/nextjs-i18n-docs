...
لتعريف البيانات الوصفية الثابتة، قم بتصدير [كائن `Metadata`](/docs/app/api-reference/functions/generate-metadata#metadata-object) من ملف `layout.js` أو ملف `page.js` ثابت.
...
> - طلبات `fetch` يتم [تخزينها مؤقتًا تلقائيًا](/docs/app/building-your-application/caching#request-memoization) لنفس البيانات عبر `generateMetadata`، `generateStaticParams`، التخطيطات، الصفحات، ومكونات الخادم. يمكن استخدام [`cache` من React](/docs/app/building-your-application/caching#request-memoization) إذا كان `fetch` غير متاح.
> - سينتظر Next.js اكتمال جلب البيانات داخل `generateMetadata` قبل بث واجهة المستخدم إلى العميل. هذا يضمن أن الجزء الأول من [الاستجابة المبثوثة](/docs/app/building-your-application/routing/loading-ui-and-streaming) يتضمن علامات `<head>`.
...
للتنفيذ والأمثلة، راجع [مرجع Metadata Files API](/docs/app/api-reference/file-conventions/metadata) و [إنشاء الصور الديناميكية](#dynamic-image-generation).
...
> **معلومة مفيدة**: يمكنك تجاوز علامة [`viewport`](/docs/app/api-reference/functions/generate-metadata#viewport) الافتراضية.
...
هذا يعني أن البيانات الوصفية ذات الحقول المتداخلة مثل [`openGraph`](/docs/app/api-reference/functions/generate-metadata#opengraph) و [`robots`](/docs/app/api-reference/functions/generate-metadata#robots) التي تم تعريفها في مقطع سابق يتم **تجاوزها** بواسطة آخر مقطع يقوم بتعريفها.
...
يتكامل `ImageResponse` جيدًا مع واجهات Next.js الأخرى، بما في ذلك [Route Handlers](/docs/app/building-your-application/routing/route-handlers) والبيانات الوصفية المعتمدة على الملفات. على سبيل المثال، يمكنك استخدام `ImageResponse` في ملف `opengraph-image.tsx` لإنشاء صور Open Graph أثناء وقت البناء أو ديناميكيًا عند وقت الطلب.

يدعم `ImageResponse` خصائص CSS الشائعة بما في ذلك flexbox والتحديد المطلق، الخطوط المخصصة، التفاف النص، المركزة، والصور المتداخلة. [راجع القائمة الكاملة لخصائص CSS المدعومة](/docs/app/api-reference/functions/image-response).
