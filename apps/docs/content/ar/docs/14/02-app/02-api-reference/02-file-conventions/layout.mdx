...
يجب أن تقبل مكونات التنسيق وتستخدم خاصية `children`. أثناء التصيير، سيتم تعبئة `children` بشرائح المسار التي يحيط بها التنسيق. ستكون هذه بشكل أساسي مكون [التنسيق](/docs/app/building-your-application/routing/pages-and-layouts#pages) الفرعي (إذا وجد) أو [الصفحة](/docs/app/building-your-application/routing/pages-and-layouts#pages)، ولكن قد تكون أيضًا ملفات خاصة أخرى مثل [التحميل](/docs/app/building-your-application/routing/loading-ui-and-streaming) أو [الخطأ](/docs/app/building-your-application/routing/error-handling) عند الاقتضاء.

### `params` (اختياري)

كائن [معلمات المسار الديناميكي](/docs/app/building-your-application/routing/dynamic-routes) من شريحة الجذر وصولًا إلى ذلك التنسيق.
...
على عكس [الصفحات](/docs/app/api-reference/file-conventions/page)، مكونات التنسيق **لا** تتلقى خاصية `searchParams`. وذلك لأن التنسيق المشترك [لا يتم إعادة تصييره أثناء التنقل](/docs/app/building-your-application/routing/linking-and-navigating#4-partial-rendering) مما قد يؤدي إلى `searchParams` قديمة بين عمليات التنقل.
...
- بدلاً من ذلك، استخدم خاصية [`searchParams`](/docs/app/api-reference/file-conventions/page#searchparams-optional) للصفحة أو خطاف [`useSearchParams`](/docs/app/api-reference/functions/use-search-params) في مكون العميل، والذي يعاد تصييره على العميل بأحدث `searchParams`.

### تنسيقات الجذر

- **يجب** أن يتضمن دليل `app` ملف `app/layout.js` للجذر.
- **يجب** أن يحدد تنسيق الجذر علامات `<html>` و `<body>`.
  - **لا يجب** إضافة علامات `<head>` يدويًا مثل `<title>` و `<meta>` إلى تنسيقات الجذر. بدلاً من ذلك، استخدم [واجهة برمجة تطبيقات البيانات الوصفية (Metadata API)](/docs/app/api-reference/functions/generate-metadata) التي تتعامل تلقائيًا مع المتطلبات المتقدمة مثل البث (streaming) وإزالة تكرار عناصر `<head>`.
