...
translation-updated-at: 2025-06-06T17:55:25.907Z
...
| `fixed`               | مقاس محدد بـ `width` و `height` بالضبط                    | `1x`, `2x` (بناءً على [imageSizes](#image-sizes))                                                            | N/A     | نعم                   |
| `responsive`          | تغيير الحجم ليتناسب مع عرض الحاوية                          | `640w`, `750w`, ... `2048w`, `3840w` (بناءً على [imageSizes](#image-sizes) و [deviceSizes](#device-sizes)) | `100vw` | نعم                   |
...
عندما يكون `true`، ستعتبر الصورة ذات أولوية عالية وسيتم [تحميلها مسبقًا](https://web.dev/preload-responsive-images/). يتم تعطيل التحميل الكسول تلقائيًا للصور التي تستخدم `priority`.

يجب استخدام خاصية `priority` على أي صورة يتم اكتشافها كعنصر [أكبر محتوى مرئي (LCP)](https://nextjs.org/learn/seo/web-performance/lcp). قد يكون من المناسب وجود عدة صور ذات أولوية، حيث قد تكون صور مختلفة هي عنصر LCP لأحجام نافذة عرض مختلفة.
...
عند `blur`، سيتم استخدام خاصية [`blurDataURL`](#blurdataurl) كعنصر نائب. إذا كان `src` كائنًا من [استيراد ثابت](/docs/pages/building-your-application/optimizing/images#local-images) وكانت الصورة المستوردة من نوع `.jpg` أو `.png` أو `.webp` أو `.avif`، فسيتم ملء `blurDataURL` تلقائيًا.
...
يمكنك أيضًا [إنشاء عنوان URL لبيانات لون ثابت](https://png-pixel.com) لمطابقة الصورة.
...
إذا كانت الصورة متداخلة داخل عنصر أب قابل للتمرير بخلاف مستند الجذر، فستحتاج أيضًا إلى تعيين خاصية [lazyRoot](#lazyroot).
...
> **تحذير**: نوصي بتكوين [`remotePatterns`](#remote-patterns) بدقة بدلاً من `domains` لحماية تطبيقك من المستخدمين الضارين. استخدم `domains` فقط إذا كنت تملك كل المحتوى المقدم من النطاق.
...
إذا كنت تريد استخدام موفر سحابي لتحسين الصور بدلاً من استخدام واجهة برمجة تطبيقات تحسين الصور المدمجة في Next.js، يمكنك تكوين `loader` و `path` في ملف `next.config.js`. هذا يسمح لك باستخدام عناوين URL نسبية لخاصية [`src`](#src) للصورة وإنشاء عنوان URL المطلق الصحيح لموفرك تلقائيًا.
...
> يستخدم المحمل الافتراضي لمكون `next/legacy/image` [`squoosh`](https://www.npmjs.com/package/@squoosh/lib) لأنه سريع التثبيت ومناسب لبيئة التطوير. عند استخدام `next start` في بيئة الإنتاج، يُوصى بشدة بتثبيت [`sharp`](https://www.npmjs.com/package/sharp) عن طريق تشغيل `npm i sharp` في دليل مشروعك. هذا ليس ضروريًا لنشرات Vercel، حيث يتم تثبيت `sharp` تلقائيًا.
...
يمكنك تحديد قائمة بعروض الصور باستخدام خاصية `images.imageSizes` في ملف `next.config.js`. يتم دمج هذه الأبعاد مع مصفوفة [أحجام الأجهزة](#device-sizes) لتشكيل المصفوفة الكاملة للأحجام المستخدمة لإنشاء [srcset](https://developer.mozilla.org/docs/Web/API/HTMLImageElement/srcset) للصور.
...
إذا تطابق رأس `Accept` مع أكثر من تنسيق واحد من التنسيقات المكونة، يتم استخدام أول تطابق في المصفوفة. لذلك، ترتيب المصفوفة مهم. إذا لم يكن هناك تطابق (أو إذا كانت الصورة الأصلية [متحركة](#animated-images))، فستعود واجهة برمجة تطبيقات تحسين الصور إلى تنسيق الصورة الأصلية.
...
يتم تعريف تاريخ الانتهاء (أو بالأحرى الحد الأقصى للعمر) إما عن طريق تكوين [`minimumCacheTTL`](#minimum-cache-ttl) أو رأس `Cache-Control` للصورة الأصلية، أيهما أكبر. على وجه التحديد، يتم استخدام قيمة `max-age` لرأس `Cache-Control`. إذا تم العثور على كل من `s-maxage` و `max-age`، فسيتم تفضيل `s-maxage`. يتم أيضًا تمرير `max-age` إلى أي عملاء لاحقين بما في ذلك شبكات CDN والمتصفحات.
...
يمكنك تكوين وقت البقاء (TTL) بالثواني للصور المحسنة المخزنة مؤقتًا. في كثير من الحالات، من الأفضل استخدام [استيراد صورة ثابتة (Static Image Import)](/docs/pages/building-your-application/optimizing/images#local-images) والذي سيقوم تلقائيًا بتجزئة محتويات الملف وتخزين الصورة مؤقتًا إلى الأبد مع رأس `Cache-Control` بقيمة `immutable`.
...
إذا كنت بحاجة إلى تغيير سلوك التخزين المؤقت لكل صورة، يمكنك تكوين [`headers`](/docs/pages/api-reference/next-config-js/headers) لتعيين رأس `Cache-Control` على الصورة الأصلية (مثل `/some-asset.jpg`، وليس `/_next/image` نفسها).

لا توجد آلية لإبطال التخزين المؤقت في هذا الوقت، لذا من الأفضل الحفاظ على `minimumCacheTTL` منخفضًا. وإلا فقد تحتاج إلى تغيير خاصية [`src`](#src) يدويًا أو حذف `<distDir>/cache/images`.
...
لا يقوم المحمل [الافتراضي](#loader) بتحسين صور SVG لعدة أسباب. أولاً، SVG هو تنسيق متجه مما يعني أنه يمكن تغيير حجمه دون فقدان الجودة. ثانيًا، يحتوي SVG على العديد من الميزات المشابهة لـ HTML/CSS، مما قد يؤدي إلى ثغرات أمنية بدون رؤوس [سياسة أمان المحتوى (Content Security Policy (CSP))](/docs/app/api-reference/next-config-js/headers) المناسبة.
...
الكشف التلقائي عن الملفات المتحركة هو أفضل جهد ويدعم GIF وAPNG وWebP. إذا كنت ترغب في تجاوز تحسين الصور صراحةً لصورة متحركة معينة، استخدم خاصية [unoptimized](#unoptimized).
