...
translation-updated-at: 2025-06-06T17:57:13.075Z
title: الصفحات والتخطيطات
description: إنشاء أول صفحة وتخطيط مشترك باستخدام موجه الصفحات (Pages Router).
---

يحتوي موجه الصفحات (Pages Router) على نظام توجيه يعتمد على نظام الملفات مبني على مفهوم الصفحات.
...
إذا كان لديك تخطيط واحد لتطبيقك بالكامل، يمكنك إنشاء [تطبيق مخصص](/docs/pages/building-your-application/routing/custom-app) ولف تطبيقك بالتخطيط. نظرًا لأن مكون `<Layout />` يتم إعادة استخدامه عند تغيير الصفحات، سيتم الحفاظ على حالة المكون (مثل قيم الإدخال).
...
إذا كنت بحاجة إلى تخطيطات متعددة، يمكنك إضافة خاصية `getLayout` إلى صفحتك، مما يسمح لك بإرجاع مكون React للتخطيط. هذا يسمح لك بتعريف التخطيط على أساس كل صفحة. نظرًا لأننا نرجع دالة، يمكننا الحصول على تخطيطات متداخلة معقدة إذا رغبت في ذلك.
...
نمط التخطيط هذا يمكّن من استمرارية الحالة لأن شجرة مكونات React تظل محفوظة بين انتقالات الصفحات. مع شجرة المكونات، يمكن لـ React فهم العناصر التي تغيرت للحفاظ على الحالة.

> **جيد أن تعرف**: هذه العملية تسمى [التوفيق (reconciliation)](https://react.dev/learn/preserving-and-resetting-state)، وهي كيفية فهم React للعناصر التي تغيرت.
...
داخل تخطيطك، يمكنك جلب البيانات على جانب العميل باستخدام `useEffect` أو مكتبة مثل [SWR](https://swr.vercel.app/). لأن هذا الملف ليس [صفحة](/docs/pages/building-your-application/routing/pages-and-layouts)، لا يمكنك استخدام `getStaticProps` أو `getServerSideProps` حاليًا.
