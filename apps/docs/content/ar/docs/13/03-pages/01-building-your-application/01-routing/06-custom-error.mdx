...
translation-updated-at: 2025-06-06T17:55:52.846Z
title: أخطاء مخصصة
description: تجاوز وتوسيع صفحة الخطأ المضمنة للتعامل مع الأخطاء المخصصة.
---
...
export default function Custom500() {
  return <h1>500 - حدث خطأ من جانب الخادم</h1>
}
```

> **معلومة مفيدة**: يمكنك استخدام [`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props) داخل هذه الصفحة إذا كنت بحاجة إلى جلب البيانات أثناء عملية البناء.

### تخصيص متقدم لصفحات الأخطاء
...
function Error({ statusCode }) {
  return (
    <p>
      {statusCode
        ? `حدث خطأ ${statusCode} من جانب الخادم`
        : 'حدث خطأ من جانب العميل'}
    </p>
  )
}
...
> `pages/_error.js` يستخدم فقط في بيئة الإنتاج. في بيئة التطوير ستحصل على خطأ مع سلسلة الاستدعاءات (call stack) لمعرفة مصدر الخطأ.

### إعادة استخدام صفحة الخطأ المضمنة

إذا كنت ترغب في عرض صفحة الخطأ المضمنة، يمكنك ذلك عن طريق استيراد مكون `Error`:
...
يقبل مكون `Error` أيضًا خاصية `title` إذا كنت ترغب في تمرير رسالة نصية مع `statusCode`.
...
- `_error`، مثل `_app`، هو مسار محجوز. يستخدم `_error` لتحديد التخطيطات والسلوكيات المخصصة لصفحات الأخطاء. `/_error` سيعرض 404 عند الوصول إليه مباشرة عبر [التوجيه](/docs/pages/building-your-application/routing) أو العرض في [خادم مخصص](/docs/pages/building-your-application/configuring/custom-server).