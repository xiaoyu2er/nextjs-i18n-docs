...
translation-updated-at: 2025-06-06T17:26:21.683Z
title: generateMetadata
description: تعلم كيفية إضافة بيانات وصفية (Metadata) إلى تطبيق Next.js لتحسين تحسين محركات البحث (SEO) وإمكانية مشاركة الويب.
related:
  title: الخطوات التالية
  description: عرض جميع خيارات واجهة برمجة التطبيقات (API) لبيانات الوصف.
  links:
...
يمكن تعيين البيانات الوصفية الديناميكية التي تعتمد على **معلومات ديناميكية**، مثل معلمات المسار الحالية أو البيانات الخارجية أو `metadata` في الأجزاء الأصلية، عن طريق تصدير دالة `generateMetadata` تُرجع كائن [`Metadata`](#metadata-fields).
...
> - طلبات `fetch` داخل `generateMetadata` يتم [تخزينها مؤقتًا تلقائيًا](/docs/app/deep-dive/caching#request-memoization) لنفس البيانات عبر `generateMetadata` و `generateStaticParams` والتخطيطات والصفحات ومكونات الخادم.
> - يمكن استخدام [`cache` في React](/docs/app/deep-dive/caching#react-cache-function) إذا كان `fetch` غير متاح.
> - [البيانات الوصفية المستندة إلى الملفات](/docs/app/api-reference/file-conventions/metadata) لها الأولوية الأعلى وستتجاوز كائن `metadata` ودالة `generateMetadata`.
...
- `props` - كائن يحتوي على معلمات المسار الحالي:
  ...
  - `params` - كائن يحتوي على [معلمات المسار الديناميكي](/docs/app/api-reference/file-conventions/dynamic-routes) من المقطع الجذري وصولاً إلى المقطع الذي يتم استدعاء `generateMetadata` منه. أمثلة:
  ...
  - `searchParams` - كائن يحتوي على [معلمات البحث (search params)](https://developer.mozilla.org/docs/Learn/Common_questions/What_is_a_URL#parameters) للرابط الحالي. أمثلة:
...
> - يتم [تخزين](/docs/app/deep-dive/caching#request-memoization) طلبات `fetch` مؤقتًا تلقائيًا لنفس البيانات عبر `generateMetadata` و `generateStaticParams` والتخطيطات والصفحات ومكونات الخادم. يمكن استخدام [`cache` في React](/docs/app/deep-dive/caching#react-cache-function) إذا كان `fetch` غير متاح.
...
> **مهمل**: خيار `colorScheme` في `metadata` مهمل اعتبارًا من Next.js 14. يرجى استخدام [تكوين viewport](/docs/app/api-reference/functions/generate-viewport) بدلاً من ذلك.
...
يمكنك إضافة أمان النوع إلى البيانات الوصفية (metadata) باستخدام نوع `Metadata`. إذا كنت تستخدم [ملحق TypeScript المدمج](/docs/app/api-reference/config/typescript) في بيئة التطوير الخاصة بك، فلن تحتاج إلى إضافة النوع يدويًا، ولكن يمكنك إضافة ذلك بشكل صريح إذا أردت.
...
| `<style>`                     | تعلم المزيد حول [التنسيق في Next.js](/docs/app/getting-started/css).                                                                                                                                                                                     |
| `<script>`                    | تعلم المزيد حول [استخدام النصوص البرمجية (scripts)](/docs/app/guides/scripts).                                                                                                                                                                                               |
| `<link rel="stylesheet" />`   | `import` أوراق الأنماط مباشرة في التنسيق أو الصفحة نفسها.                                                                                                                                                                                               |
| `<link rel="preload />`       | استخدم [طريقة ReactDOM المسبقة (preload method)](#link-relpreload)                                                                                                                                                                                                           |
| `<link rel="preconnect" />`   | استخدم [طريقة ReactDOM للاتصال المسبق (preconnect method)](#link-relpreconnect)                                                                                                                                                                                                     |
| `<link rel="dns-prefetch" />` | استخدم [طريقة ReactDOM للبحث المسبق عن DNS (prefetchDNS method)](#link-reldns-prefetch)                                                                                                                                                                                                  |
...
بينما لا تدعم واجهة برمجة تطبيق البيانات الوصفية هذه التلميحات مباشرة، يمكنك استخدام [طرق `ReactDOM` الجديدة](https://github.com/facebook/react/pull/26237) لإدراجها بأمان في `<head>` المستند.
...
ابدأ تحميل مورد في وقت مبكر من دورة حياة عرض الصفحة (في المتصفح). [وثائق MDN](https://developer.mozilla.org/docs/Web/HTML/Attributes/rel/preload).
...
ابدأ اتصالًا مسبقًا بأصل (origin). [وثائق MDN](https://developer.mozilla.org/docs/Web/HTML/Attributes/rel/preconnect).
...
> **معلومة جيدة**: يمكنك تجاوز علامة [`viewport`](/docs/app/api-reference/functions/generate-metadata#viewport) الافتراضية.
...
هذا يعني أن البيانات الوصفية ذات الحقول المتداخلة مثل [`openGraph`](/docs/app/api-reference/functions/generate-metadata#opengraph) و [`robots`](/docs/app/api-reference/functions/generate-metadata#robots) التي تم تعريفها في جزء سابق يتم **تجاوزها** بواسطة الجزء الأخير الذي يحددها.
