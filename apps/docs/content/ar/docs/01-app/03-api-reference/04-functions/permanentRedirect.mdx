...
translation-updated-at: 2025-06-02T20:07:04.866Z
...
تتيح لك وظيفة `permanentRedirect` إعادة توجيه المستخدم إلى عنوان URL آخر. يمكن استخدام `permanentRedirect` في مكونات الخادم (Server Components)، ومكونات العميل (Client Components)، [وحدات معالجة المسارات (Route Handlers)](/docs/app/building-your-application/routing/route-handlers)، و [أفعال الخادم (Server Actions)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations).
...
> **معلومة مفيدة**: إذا كنت تفضل إرجاع إعادة توجيه HTTP 307 (مؤقتة) بدلاً من 308 (دائمة)، يمكنك استخدام [وظيفة `redirect`](/docs/app/api-reference/functions/redirect) بدلاً من ذلك.
...
بشكل افتراضي، ستستخدم `permanentRedirect` `push` (إضافة إدخال جديد إلى سجل المتصفح) في [أفعال الخادم (Server Actions)](/docs/app/building-your-application/data-fetching/server-actions-and-mutations) و `replace` (استبدال عنوان URL الحالي في سجل المتصفح) في كل مكان آخر. يمكنك تجاوز هذا السلوك عن طريق تحديد معامل `type`.
