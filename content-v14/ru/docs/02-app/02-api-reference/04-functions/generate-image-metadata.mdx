...
translation-updated-at: 2025-06-06T16:56:48.666Z
...
Функция `generateImageMetadata` должна возвращать `массив` объектов, содержащих метаданные изображения, такие как `alt` и `size`. Кроме того, каждый объект **обязательно** должен включать значение `id`, которое будет передано в пропсы функции генерации изображения.
