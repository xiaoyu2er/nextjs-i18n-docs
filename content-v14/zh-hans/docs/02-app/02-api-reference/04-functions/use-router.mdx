...
> **推荐：** 除非有特殊需求需要使用 `useRouter`，否则请使用 [`<Link>` 组件](/docs/app/building-your-application/routing/linking-and-navigating#link-component) 进行导航。
...
> **须知：** `<NavigationEvents>` 被包裹在 [`Suspense` 边界](/docs/app/building-your-application/routing/loading-ui-and-streaming#example) 中，因为在 [静态渲染 (static rendering)](/docs/app/building-your-application/rendering/server-components#static-rendering-default) 期间，[`useSearchParams()`](/docs/app/api-reference/functions/use-search-params) 会导致客户端渲染直至最近的 `Suspense` 边界。[了解更多](/docs/app/api-reference/functions/use-search-params#behavior)。
