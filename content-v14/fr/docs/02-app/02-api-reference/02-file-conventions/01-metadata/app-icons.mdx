...
La façon la plus simple de générer une icône est d'utiliser l'API [`ImageResponse`](/docs/app/api-reference/functions/image-response) de `next/og`.
...
Un objet contenant les [paramètres de route dynamique](/docs/app/building-your-application/routing/dynamic-routes) depuis le segment racine jusqu'au segment où `icon` ou `apple-icon` est colocalisé.
