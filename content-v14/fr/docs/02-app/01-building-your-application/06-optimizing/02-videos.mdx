...
translation-updated-at: 2025-06-06T17:18:17.781Z
...
| `autoPlay`    | Lance automatiquement la lecture de la vidéo au chargement de la page. Note : Les politiques de lecture automatique varient selon les navigateurs. | `<video autoPlay />`                 |
...
| `frameborder`     | Spécifie s'il faut afficher une bordure autour de l'iframe.            | `<iframe frameborder="0" />`           |
...
### Choix de la méthode d'intégration vidéo
...
La première étape consiste à créer un [composant serveur](https://nextjs.org/docs/app/building-your-application/rendering/server-components) qui génère l'iframe approprié pour intégrer la vidéo. Ce composant récupérera l'URL source de la vidéo et affichera l'iframe.
...
Après avoir créé le composant serveur pour intégrer la vidéo, l'étape suivante consiste à [diffuser](https://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming) le composant en utilisant [React Suspense](https://react.dev/reference/react/Suspense).
...
> - Mettez en œuvre des [stratégies de chargement des vidéos](https://yoast.com/site-speed-tips-for-faster-video/) en fonction des conditions du réseau, en particulier pour les utilisateurs avec des forfaits de données limités.

Cette approche améliore l'expérience utilisateur en évitant que la page ne se bloque, ce qui signifie que l'utilisateur peut interagir avec la page pendant que le composant vidéo se charge.
...
- **Contrôle et indépendance complets** : L'auto-hébergement vous donne une gestion directe de votre contenu vidéo, de la lecture à l'apparence, garantissant une pleine propriété et un contrôle, sans contraintes des plateformes externes.
- **Personnalisation pour des besoins spécifiques** : Idéal pour des exigences uniques, comme des vidéos d'arrière-plan dynamiques, il permet une personnalisation sur mesure pour s'aligner sur les besoins de conception et fonctionnels.
...
export default function Page() {
  return (
    <Suspense fallback={<p>Chargement de la vidéo...</p>}>
      <VideoComponent fileName="ma-video.mp4" />
    </Suspense>
  )
}
...
- **Comprendre les formats et codecs vidéo** : Choisissez le bon format et codec, comme MP4 pour la compatibilité ou WebM pour l'optimisation web, pour vos besoins vidéo. Pour plus de détails, consultez le [guide de Mozilla sur les codecs vidéo](https://developer.mozilla.org/fr/docs/Web/Media/Formats/Video_codecs).
...
- **Ajustement de la résolution et du débit binaire** : Ajustez la [résolution et le débit binaire](https://www.dacast.com/fr/blog/bitrate-vs-resolution/#:~:text=Les%20deux%20mesurent%20diff%C3%A9rents%20aspects,donnent%20des%20qualit%C3%A9s%20vid%C3%A9o%20diff%C3%A9rentes) en fonction de la plateforme de visualisation, avec des paramètres plus bas pour les appareils mobiles.
- **Réseaux de diffusion de contenu (CDN)** : Utilisez un CDN pour améliorer la vitesse de diffusion vidéo et gérer un trafic élevé. Lorsque vous utilisez certaines solutions de stockage, comme Vercel Blob, la fonctionnalité CDN est automatiquement gérée pour vous. [En savoir plus](https://vercel.com/docs/edge-network/overview?utm_source=next-site&utm_medium=docs&utm_campaign=next-website) sur les CDN et leurs avantages.

Explorez ces plateformes de diffusion vidéo pour intégrer des vidéos dans vos projets Next.js :

### Composant open source `next-video`

- Fournit un composant `<Video>` pour Next.js, compatible avec divers services d'hébergement, y compris [Vercel Blob](https://vercel.com/docs/storage/vercel-blob?utm_source=next-site&utm_medium=docs&utm_campaign=next-website), S3, Backblaze et Mux.
...
- Inclut un composant `<CldVideoPlayer>` pour une [prise en charge vidéo facile](https://next.cloudinary.dev/cldvideoplayer/basic-usage).
- Trouvez des [exemples](https://github.com/cloudinary-community/cloudinary-examples/?tab=readme-ov-file#nextjs) d'intégration de Cloudinary avec Next.js, y compris la [Diffusion adaptative en débit binaire](https://github.com/cloudinary-community/cloudinary-examples/tree/main/examples/nextjs-cldvideoplayer-abr).
...
- Découvrez les recommandations de Mux pour intégrer des [vidéos haute performance pour votre application Next.js](https://www.mux.com/for/nextjs).
- Explorez un [projet exemple](https://with-mux-video.vercel.app/) démontrant Mux avec Next.js.

### Fastly

- Découvrez comment intégrer les solutions de Fastly pour [la vidéo à la demande (video on demand)](https://www.fastly.com/products/streaming-media/video-on-demand) et le streaming multimédia dans Next.js.
