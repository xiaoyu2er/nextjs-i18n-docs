...
Lorsqu'il est combiné avec la [Régénération Statique Incrémentielle](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration), `getStaticProps` s'exécutera en arrière-plan pendant que la page obsolète est revalidée, et la page fraîche servie au navigateur.
...
Vous pouvez temporairement contourner la génération statique et rendre la page au **moment de la requête** au lieu du moment du build en utilisant le [**Mode Prévisualisation**](/docs/pages/building-your-application/configuring/preview-mode). Par exemple, vous pourriez utiliser un CMS headless et vouloir prévisualiser des brouillons avant leur publication.