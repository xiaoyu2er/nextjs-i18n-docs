...
Ensuite, envoyez une requête à `/api/disable-draft` pour invoquer la Route API. Si vous appelez cette route en utilisant [`next/link`](/docs/pages/api-reference/components/link), vous devez passer `prefetch={false}` pour éviter de supprimer accidentellement le cookie lors du préchargement.
...
> **Bon à savoir** : Vous ne devriez pas définir l'en-tête `Cache-Control` lors de l'utilisation du Mode Brouillon car il ne peut pas être contourné. Nous recommandons plutôt d'utiliser [ISR](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration).
