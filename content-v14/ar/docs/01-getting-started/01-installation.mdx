...
نوصي ببدء تطبيق Next.js جديد باستخدام [`create-next-app`](/docs/app/api-reference/cli/create-next-app)، والذي يقوم بإعداد كل شيء تلقائيًا لك. لإنشاء مشروع، قم بتنفيذ:
...
إذا كنت جديدًا في Next.js، راجع وثائق [هيكل المشروع](/docs/getting-started/project-structure) للحصول على نظرة عامة على جميع الملفات والمجلدات المحتملة في تطبيقك.

> **معلومة مفيدة**:
>
> - يأتي Next.js الآن مع تهيئة افتراضية لـ [TypeScript](/docs/app/building-your-application/configuring/typescript) و[ESLint](/docs/app/building-your-application/configuring/eslint) و[Tailwind CSS](/docs/app/building-your-application/styling/tailwind-css).
> - يمكنك اختياريًا استخدام [مجلد `src`](/docs/app/building-your-application/configuring/src-directory) في جذر مشروعك لفصل كود التطبيق عن ملفات التهيئة.
...
- `dev`: يشغل [`next dev`](/docs/app/api-reference/cli/next#next-dev-options) لبدء Next.js في وضع التطوير.
- `build`: يشغل [`next build`](/docs/app/api-reference/cli/next#next-build-options) لبناء التطبيق للاستخدام في الإنتاج.
- `start`: يشغل [`next start`](/docs/app/api-reference/cli/next#next-start-options) لبدء خادم Next.js للإنتاج.
- `lint`: يشغل [`next lint`](/docs/app/api-reference/cli/next#next-lint-options) لإعداد تهيئة ESLint المدمجة في Next.js.
...
لتطبيقات جديدة، نوصي باستخدام [موجه التطبيق](/docs/app). يسمح لك هذا الموجه باستخدام أحدث ميزات React وهو تطور لـ [موجه الصفحات](/docs/pages) بناءً على ملاحظات المجتمع.
...
قم بإنشاء [تخطيط جذر](/docs/app/building-your-application/routing/pages-and-layouts#root-layout-required) داخل `app/layout.tsx` مع العلامات المطلوبة `<html>` و`<body>`:
