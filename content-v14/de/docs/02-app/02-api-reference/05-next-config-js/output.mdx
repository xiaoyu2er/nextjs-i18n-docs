...
> - Wenn Ihr Projekt [Image Optimization](/docs/app/building-your-application/optimizing/images) mit dem Standard-`loader` verwendet, müssen Sie `sharp` als Abhängigkeit installieren:
...
> - Wenn Ihr Projekt auf einen bestimmten Port oder Hostnamen hören muss, können Sie die Umgebungsvariablen `PORT` oder `HOSTNAME` definieren, bevor Sie `server.js` ausführen. Zum Beispiel: `PORT=8080 HOSTNAME=0.0.0.0 node server.js`, um den Server unter `http://0.0.0.0:8080` zu starten.
> - Wenn Ihr Projekt [Image Optimization](/docs/pages/building-your-application/optimizing/images) mit dem Standard-`loader` verwendet, müssen Sie `sharp` als Abhängigkeit installieren:
