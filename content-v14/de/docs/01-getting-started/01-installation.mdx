...
Wir empfehlen, eine neue Next.js-App mit [`create-next-app`](/docs/app/api-reference/cli/create-next-app) zu starten, das alles automatisch für Sie einrichtet. Führen Sie folgenden Befehl aus, um ein Projekt zu erstellen:
...
Wenn Sie neu bei Next.js sind, lesen Sie die Dokumentation zur [Projektstruktur](/docs/getting-started/project-structure) für einen Überblick über alle möglichen Dateien und Ordner in Ihrer Anwendung.

> **Gut zu wissen**:
>
> - Next.js wird jetzt standardmäßig mit [TypeScript](/docs/app/building-your-application/configuring/typescript), [ESLint](/docs/app/building-your-application/configuring/eslint) und [Tailwind CSS](/docs/app/building-your-application/styling/tailwind-css) Konfiguration ausgeliefert.
...
- `dev`: führt [`next dev`](/docs/app/api-reference/cli/next#next-dev-options) aus, um Next.js im Entwicklungsmodus zu starten.
- `build`: führt [`next build`](/docs/app/api-reference/cli/next#next-build-options) aus, um die Anwendung für den Produktionseinsatz zu erstellen.
- `start`: führt [`next start`](/docs/app/api-reference/cli/next#next-start-options) aus, um einen Next.js-Produktionsserver zu starten.
- `lint`: führt [`next lint`](/docs/app/api-reference/cli/next#next-lint-options) aus, um die integrierte ESLint-Konfiguration von Next.js einzurichten.
...
Fügen Sie als Nächstes eine `_app.tsx`-Datei in `pages/` hinzu, um das globale Layout zu definieren. Erfahren Sie mehr über die [Custom App-Datei](/docs/pages/building-your-application/routing/custom-app).
...
Fügen Sie abschließend eine `_document.tsx`-Datei in `pages/` hinzu, um die initiale Serverantwort zu steuern. Erfahren Sie mehr über die [Custom Document-Datei](/docs/pages/building-your-application/routing/custom-document).
