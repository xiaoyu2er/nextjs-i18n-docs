...
當與標頭一起使用 [`basePath` 支援](/docs/app/api-reference/next-config-js/basePath) 時，每個 `source` 會自動前綴 `basePath`，除非您在標頭中添加 `basePath: false`：
...
當與標頭一起使用 [`i18n` 支援](/docs/app/building-your-application/routing/internationalization) 時，每個 `source` 會自動前綴以處理配置的 `locales`，除非您在標頭中添加 `locale: false`。如果使用 `locale: false`，您必須在 `source` 前添加語言環境才能正確匹配。

</AppOnly>

<PagesOnly>

當與標頭一起使用 [`i18n` 支援](/docs/pages/building-your-application/routing/internationalization) 時，每個 `source` 會自動前綴以處理配置的 `locales`，除非您在標頭中添加 `locale: false`。如果使用 `locale: false`，您必須在 `source` 前添加語言環境才能正確匹配。
...
了解更多關於使用應用程式路由器的 [快取](/docs/app/building-your-application/caching)。

</AppOnly>

<PagesOnly>

如果您需要重新驗證已 [靜態生成](/docs/pages/building-your-application/rendering/static-site-generation) 的頁面的快取，可以通過在頁面的 [`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props) 函數中設置 `revalidate` 屬性來實現。
...
**此標頭已被 CSP 的 `frame-ancestors` 選項取代**，現代瀏覽器對此有更好的支援（詳見 [內容安全政策](/docs/app/building-your-application/configuring/content-security-policy) 的配置細節）。
...
了解更多關於為您的應用程式添加 [內容安全政策](/docs/app/building-your-application/configuring/content-security-policy) 的信息。
