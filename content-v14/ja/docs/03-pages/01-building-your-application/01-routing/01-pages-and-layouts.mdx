...
> 動的ルーティングについて詳しくは、[動的ルーティングのドキュメント](/docs/pages/building-your-application/routing/dynamic-routes)をご覧ください。
...
アプリケーション全体で1つのレイアウトしかない場合、[カスタムアプリ](/docs/pages/building-your-application/routing/custom-app)を作成し、アプリケーションをレイアウトでラップできます。`<Layout />` コンポーネントはページ変更時に再利用されるため、コンポーネントの状態（入力値など）が保持されます。
