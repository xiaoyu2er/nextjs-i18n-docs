...
{/* El contenido de este documento se comparte entre el enrutador de app y pages. Puedes usar el componente `<PagesOnly>Contenido</PagesOnly>` para agregar contenido específico del Enrutador Pages. Cualquier contenido compartido no debe envolverse en un componente. */}
...
> **Bueno saber**:
>
> - Next.js se desplazará a la [Página](/docs/app/building-your-application/routing/pages-and-layouts#pages) si no está visible en el viewport al navegar.
...
- `false`: El prefetching no ocurrirá al entrar en el viewport, pero sí al pasar el cursor. Si deseas eliminar completamente el prefetching al pasar el cursor, considera usar una etiqueta `<a>` o [adoptar incrementalmente](/docs/app/building-your-application/upgrading/app-router-migration) el App Router, que permite deshabilitar el prefetching al pasar el cursor.
...
Un elemento `<a>` ya no es necesario como hijo de `<Link>`. Agrega la prop `legacyBehavior` para usar el comportamiento heredado o elimina el `<a>` para actualizar. Hay un [codemod disponible](/docs/app/building-your-application/upgrading/codemods#new-link) para actualizar automáticamente tu código.
...
| Versión   | Cambios                                                                                                                                                                                         |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.0.0` | Ya no requiere una etiqueta `<a>` como elemento hijo. Se proporciona un [codemod](/docs/app/building-your-application/upgrading/codemods#remove-a-tags-from-link-components) para actualizar automáticamente su base de código. |
