...
Nota que para manejar el cambio solo del `locale` manteniendo toda la información de enrutamiento como valores de consulta de [rutas dinámicas](/docs/pages/building-your-application/routing/dynamic-routes) o valores de consulta ocultos de href, puedes proporcionar el parámetro `href` como un objeto:
...
Next.js no conoce las variantes de una página, por lo que depende de ti agregar las etiquetas meta `hreflang` usando [`next/head`](/docs/pages/api-reference/components/head). Puedes aprender más sobre `hreflang` en la [documentación de Google Webmasters](https://support.google.com/webmasters/answer/189077).
...
> **Nota importante**: Estos límites se agregaron inicialmente para evitar posibles [problemas de rendimiento en tiempo de construcción](#rutas-dinámicas-y-páginas-con-getstaticprops). Puedes solucionar estos límites con enrutamiento personalizado usando [Middleware](/docs/pages/building-your-application/routing/middleware) en Next.js 12.