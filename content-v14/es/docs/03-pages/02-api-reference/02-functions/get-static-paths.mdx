...
- Si la página usa una [ruta opcional catch-all](/docs/pages/building-your-application/routing/dynamic-routes#optional-catch-all-segments), usa `null`, `[]`, `undefined` o `false` para renderizar la ruta raíz. Por ejemplo, si proporcionas `slug: false` para `pages/[[...slug]]`, Next.js generará estáticamente la página `/`.
...
Además del objeto `params`, se puede retornar un campo `locale` cuando [i18n está configurado](/docs/pages/building-your-application/routing/internationalization), que configura el locale para la ruta que se está generando.
...
El siguiente ejemplo pre-renderiza un post de blog por página llamado `pages/posts/[id].js`. La lista de posts se obtendrá de un CMS y será retornada por `getStaticPaths`. Luego, para cada página, obtiene los datos del post desde un CMS usando [`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props).
