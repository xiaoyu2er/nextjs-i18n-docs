...
translation-updated-at: 2025-06-06T17:32:05.711Z
title: Routage d'internationalisation (i18n)
nav_title: Internationalisation
description: Next.js prend en charge nativement le routage internationalisé et la détection de langue. En savoir plus ici.
---
...
La prise en charge du routage i18n est actuellement conçue pour compléter les solutions existantes de bibliothèques i18n comme [`react-intl`](https://formatjs.io/docs/getting-started/installation), [`react-i18next`](https://react.i18next.com/), [`lingui`](https://lingui.dev/), [`rosetta`](https://github.com/lukeed/rosetta), [`next-intl`](https://github.com/amannn/next-intl), [`next-translate`](https://github.com/aralroca/next-translate), [`next-multilingual`](https://github.com/Avansai/next-multilingual), [`typesafe-i18n`](https://github.com/ivanhofer/typesafe-i18n), [`tolgee`](https://tolgee.io/integrations/next), et d'autres en simplifiant les routes et l'analyse des locales.
...
Si vous ne prévoyez pas de prendre en charge toutes les régions d'un pays, il est donc recommandé d'inclure les locales de pays qui serviront de solutions de repli.
...
module.exports = {
  i18n: {
    ...
    // Il s'agit de la locale par défaut qui sera utilisée lors de la visite
    // d'un chemin sans préfixe de locale, par exemple `/hello`
    defaultLocale: 'en-US',
    // Voici une liste de domaines de locales et de la locale par défaut qu'ils
    ...
    domains: [
      ...
      },
      {
        domain: 'example.fr',
        defaultLocale: 'fr',
        // un champ http optionnel peut également être utilisé pour tester
...
Avec la configuration ci-dessus, `en-US`, `fr` et `nl-NL` seront disponibles pour le routage, et `en-US` est la locale par défaut. Si vous avez une page `pages/blog.js`, les URL suivantes seront disponibles :
...
Par exemple, si vous avez `pages/blog.js`, les URL suivantes seront disponibles :
...
### Préfixage de la locale par défaut
...
Ce [Middleware](/docs/pages/building-your-application/routing/middleware) ignore l'ajout du préfixe par défaut aux [routes API](/docs/pages/building-your-application/routing/api-routes) et aux fichiers [publics](/docs/pages/building-your-application/optimizing/static-assets) comme les polices ou les images. Si une requête est faite vers la locale par défaut, nous redirigeons vers notre préfixe `/en`.

### Désactivation de la détection automatique de la locale
...
## Accès aux informations de locale
...
Pour `next/link`, une prop `locale` peut être fournie pour passer à une locale différente de celle actuellement active. Si aucune prop `locale` n'est fournie, la locale active actuelle est utilisée lors des transitions côté client. Par exemple :
...
Notez que pour gérer le changement uniquement de la `locale` tout en conservant toutes les informations de routage comme les valeurs de requête des [routes dynamiques](/docs/pages/building-your-application/routing/dynamic-routes) ou les valeurs de requête cachées de l'href, vous pouvez fournir le paramètre `href` sous forme d'objet :

```jsx
import { useRouter } from 'next/router'
const router = useRouter()
const { pathname, asPath, query } = router
// change uniquement la locale et conserve toutes les autres informations de route, y compris la requête de l'href
...
Next.js prend en charge le remplacement de l'en-tête accept-language avec un cookie `NEXT_LOCALE=la-locale`. Ce cookie peut être défini à l'aide d'un sélecteur de langue, puis lorsqu'un utilisateur revient sur le site, il utilisera la locale spécifiée dans le cookie lors de la redirection de `/` vers l'emplacement de locale correct.
...
Pour les pages [Optimisées Statiquement Automatiquement](/docs/pages/building-your-application/rendering/automatic-static-optimization) et les pages non dynamiques avec `getStaticProps`, **une version de la page sera générée pour chaque locale**. Ceci est important à considérer car cela peut augmenter les temps de build en fonction du nombre de locales configurées dans `getStaticProps`.
...
Pour réduire le temps de build des pages dynamiques avec `getStaticProps`, utilisez un mode [`fallback`](/docs/pages/api-reference/functions/get-static-paths#fallback-true). Cela vous permet de retourner uniquement les chemins et locales les plus populaires depuis `getStaticPaths` pour le pré-rendu pendant le build. Ensuite, Next.js construira les pages restantes à l'exécution lorsqu'elles seront demandées.

### Pages optimisées statiquement automatiquement

Pour les pages qui sont [optimisées statiquement automatiquement](/docs/pages/building-your-application/rendering/automatic-static-optimization), une version de la page sera générée pour chaque locale.
...
export async function getStaticProps({ locale }) {
  // Appelez un point d'API externe pour obtenir des articles.
