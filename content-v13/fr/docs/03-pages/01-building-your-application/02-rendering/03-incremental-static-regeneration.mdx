...
translation-updated-at: 2025-06-06T17:30:18.261Z
...
> **Bon à savoir** : Le runtime [`edge`](/docs/app/api-reference/edge) n'est actuellement pas compatible avec ISR, bien que vous puissiez exploiter `stale-while-revalidate` en définissant manuellement l'en-tête `cache-control`.
...
Lors de l'exécution locale avec `next dev`, `getStaticProps` est invoqué à chaque requête. Pour vérifier que votre configuration ISR à la demande est correcte, vous devrez créer une [version de production](/docs/pages/api-reference/next-cli#build) et démarrer le [serveur de production](/docs/pages/api-reference/next-cli#production) :
...
La régénération statique incrémentielle (ISR) fonctionne sur [les sites Next.js auto-hébergés](/docs/pages/building-your-application/deploying#self-hosting) dès le départ lorsque vous utilisez `next start`.
...
| Version   | Changements                                                                                 |
| --------- | ------------------------------------------------------------------------------------------- |
| `v12.2.0` | ISR à la demande est stable                                                                 |
| `v12.1.0` | ISR à la demande ajouté (bêta).                                                             |
