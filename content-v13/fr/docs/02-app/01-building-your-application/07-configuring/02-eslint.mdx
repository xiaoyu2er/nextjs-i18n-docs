...
translation-updated-at: 2025-06-06T17:41:14.019Z
title: ESLint
...
Next.js offre une expérience intégrée [ESLint](https://eslint.org/) prête à l'emploi. Ajoutez `next lint` comme script dans `package.json` :
...
Si l'une des deux options de configuration est sélectionnée, Next.js installera automatiquement `eslint` et `eslint-config-next` comme dépendances dans votre application et créera un fichier `.eslintrc.json` à la racine de votre projet avec la configuration sélectionnée.
...
La configuration par défaut (`eslint-config-next`) inclut tout ce dont vous avez besoin pour une expérience de linting optimale prête à l'emploi avec Next.js. Si ESLint n'est pas déjà configuré dans votre application, nous recommandons d'utiliser `next lint` pour configurer ESLint avec cette configuration.
...
Cela prendra la priorité sur la configuration de `next.config.js`.
...
|                     | Règle                                                                                                                     | Description                                                                                                      |
| :-----------------: | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| <Check size={18} /> | [@next/next/google-font-display](/docs/messages/google-font-display)                                                     | Applique le comportement font-display avec les polices Google.                                                                 |
...
| <Check size={18} /> | [@next/next/no-async-client-component](/docs/messages/no-async-client-component)                                         | Empêche que les composants clients soient des fonctions asynchrones.                                                            |
...
Si ESLint est déjà configuré dans votre application, nous recommandons d'étendre directement ce plugin plutôt que d'inclure `eslint-config-next`, sauf si certaines conditions sont remplies. Consultez la section [Ensemble de règles recommandé du plugin](#recommended-plugin-ruleset) pour en savoir plus.
...
Si vous souhaitez modifier ou désactiver certaines règles fournies par les plugins pris en charge (`react`, `react-hooks`, `next`), vous pouvez les changer directement en utilisant la propriété `rules` dans votre `.eslintrc` :
...
`next/core-web-vitals` met à jour `eslint-plugin-next` pour signaler comme erreurs plusieurs règles qui sont des avertissements par défaut si elles affectent les [Core Web Vitals](https://web.dev/vitals/).

> Le point d'entrée `next/core-web-vitals` est automatiquement inclus pour les nouvelles applications créées avec [Create Next App](/docs/app/api-reference/create-next-app).
...
Si vous souhaitez utiliser `next lint` avec [lint-staged](https://github.com/okonet/lint-staged) pour exécuter le linter sur les fichiers git stagés, vous devrez ajouter ce qui suit au fichier `.lintstagedrc.js` à la racine de votre projet afin de spécifier l'utilisation du flag `--file`.
...
Si ESLint est déjà configuré dans votre application et qu'une des conditions suivantes est vraie :

- Vous avez un ou plusieurs des plugins suivants déjà installés (soit séparément, soit via une autre configuration comme `airbnb` ou `react-app`) :
  - `react`
  - `react-hooks`
  - `jsx-a11y`
  - `import`
- Vous avez défini des `parserOptions` spécifiques différents de la configuration Babel dans Next.js (ce n'est pas recommandé sauf si vous avez [personnalisé votre configuration Babel](/docs/pages/building-your-application/configuring/babel))
- Vous avez `eslint-plugin-import` installé avec des [résolveurs](https://github.com/benmosher/eslint-plugin-import#resolvers) Node.js et/ou TypeScript définis pour gérer les imports

Alors nous recommandons soit de supprimer ces paramètres si vous préférez la façon dont ces propriétés sont configurées dans [`eslint-config-next`](https://github.com/vercel/next.js/blob/canary/packages/eslint-config-next/index.js), soit d'étendre directement depuis le plugin ESLint de Next.js :
...
Cela élimine le risque de collisions ou d'erreurs pouvant survenir en raison de l'importation du même plugin ou parseur à travers plusieurs configurations.
