...
translation-updated-at: 2025-06-06T17:42:03.468Z
title: OpenTelemetry
description: Apprenez comment instrumenter votre application Next.js avec OpenTelemetry.
---
...
> **Bon à savoir** : Nous ne prenons actuellement en charge les bindings OpenTelemetry que dans les fonctions serverless.
> Nous n'en fournissons aucun pour le code `edge` ou côté client.
...
Faire cela est équivalent à utiliser `@vercel/otel`, mais il est possible de le modifier et de l'étendre.
Par exemple, vous pourriez utiliser `@opentelemetry/exporter-trace-otlp-grpc` au lieu de `@opentelemetry/exporter-trace-otlp-http` ou vous pouvez spécifier plus d'attributs de ressource.
...
Nous recommandons d'utiliser OpenTelemetry Collector.
Si ce n'est pas possible sur votre plateforme, vous pouvez utiliser un exporteur OpenTelemetry personnalisé avec une [configuration manuelle d'OpenTelemetry](/docs/pages/building-your-application/optimizing/open-telemetry#manual-opentelemetry-configuration)
...
Les attributs des spans suivent les [conventions sémantiques OpenTelemetry](https://opentelemetry.io/docs/reference/specification/trace/semantic_conventions/). Nous ajoutons également certains attributs personnalisés sous l'espace de noms `next` :
