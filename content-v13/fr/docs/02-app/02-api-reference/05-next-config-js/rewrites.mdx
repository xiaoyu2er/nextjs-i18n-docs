...
translation-updated-at: 2025-06-06T17:34:12.579Z
title: rewrites
...
Les réécritures (rewrites) vous permettent de mapper un chemin de requête entrant vers un chemin de destination différent.
...
Les réécritures agissent comme un proxy d'URL et masquent le chemin de destination, donnant l'impression que l'utilisateur n'a pas changé d'emplacement sur le site. En revanche, les [redirections](/docs/pages/api-reference/next-config-js/redirects) redirigeront vers une nouvelle page et montreront les changements d'URL.
...
- `has` est un tableau d'[objets has](#correspondance-dentete-de-cookie-et-de-requête) avec les propriétés `type`, `key` et `value`.
- `missing` est un tableau d'[objets missing](#correspondance-dentete-de-cookie-et-de-requête) avec les propriétés `type`, `key` et `value`.
...
> **Bon à savoir** : les réécritures dans `beforeFiles` ne vérifient pas immédiatement le système de fichiers/les routes dynamiques après avoir trouvé une correspondance, elles continuent jusqu'à ce que toutes les `beforeFiles` aient été vérifiées.

L'ordre dans lequel Next.js vérifie les routes est :

<AppOnly>

1. Les [en-têtes](/docs/app/api-reference/next-config-js/headers) sont vérifiés/appliqués
2. Les [redirections](/docs/app/api-reference/next-config-js/redirects) sont vérifiées/appliquées
3. Les réécritures `beforeFiles` sont vérifiées/appliquées
4. Les fichiers statiques du [répertoire public](/docs/app/building-your-application/optimizing/static-assets), les fichiers `_next/static` et les pages non dynamiques sont vérifiés/servis
...
6. Les réécritures `fallback` sont vérifiées/appliquées, elles sont appliquées avant le rendu de la page 404 et après la vérification des routes dynamiques/tous les fichiers statiques. Si vous utilisez [fallback: true/'blocking'](/docs/pages/api-reference/functions/get-static-paths#fallback-true) dans `getStaticPaths`, les réécritures `fallback` définies dans votre `next.config.js` ne seront _pas_ exécutées.

</AppOnly>

<PagesOnly>

1. Les [en-têtes](/docs/pages/api-reference/next-config-js/headers) sont vérifiés/appliqués
2. Les [redirections](/docs/pages/api-reference/next-config-js/redirects) sont vérifiées/appliquées
3. Les réécritures `beforeFiles` sont vérifiées/appliquées
4. Les fichiers statiques du [répertoire public](/docs/pages/building-your-application/optimizing/static-assets), les fichiers `_next/static` et les pages non dynamiques sont vérifiés/servis
5. Les réécritures `afterFiles` sont vérifiées/appliquées, si l'une de ces réécritures correspond, nous vérifions les routes dynamiques/fichiers statiques après chaque correspondance
6. Les réécritures `fallback` sont vérifiées/appliquées, elles sont appliquées avant le rendu de la page 404 et après la vérification des routes dynamiques/tous les fichiers statiques. Si vous utilisez [fallback: true/'blocking'](/docs/pages/api-reference/functions/get-static-paths#fallback-true) dans `getStaticPaths`, les réécritures `fallback` définies dans votre `next.config.js` ne seront _pas_ exécutées.
...
Vous pouvez toujours passer manuellement les paramètres dans la requête si l'un est déjà utilisé dans la destination en spécifiant la requête dans la `destination`.
...
Les caractères suivants `(`, `)`, `{`, `}`, `:`, `*`, `+`, `?` sont utilisés pour la correspondance de chemin regex, donc lorsqu'ils sont utilisés dans le `source` comme valeurs non spéciales, ils doivent être échappés en ajoutant `\\` avant eux :
...
Pour ne correspondre à une réécriture que lorsque les valeurs d'en-tête, de cookie ou de requête correspondent également, le champ `has` peut être utilisé, ou le champ `missing` pour ne pas correspondre. À la fois le `source` et tous les éléments `has` doivent correspondre, et tous les éléments `missing` ne doivent pas correspondre pour que la réécriture soit appliquée.
        ...
        has: [
          {
            ...
            // destination puisque value est fourni et n'utilise pas
...
Les réécritures vous permettent de réécrire vers une URL externe. Ceci est particulièrement utile pour adopter Next.js progressivement. Voici un exemple de réécriture pour rediriger la route `/blog` de votre application principale vers un site externe.
...
Vous pouvez également configurer Next.js pour qu'il bascule vers le proxy d'un site web existant après avoir vérifié toutes les routes Next.js.
...
Lorsque vous utilisez le support de [`basePath`](/docs/app/api-reference/next-config-js/basePath) avec des réécritures, chaque `source` et `destination` est automatiquement préfixé avec le `basePath` sauf si vous ajoutez `basePath: false` à la réécriture :
...
module.exports = {
  ...
  async rewrites() {
    return [
      ...
      {
        // n'ajoute pas /docs à /without-basePath puisque basePath: false est défini
...
Lorsque vous utilisez le support [`i18n`](/docs/app/building-your-application/routing/internationalization) avec des réécritures, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à la réécriture. Si `locale: false` est utilisé, vous devez préfixer la `source` et la `destination` avec une locale pour qu'elle corresponde correctement.

</AppOnly>

<PagesOnly>

Lorsque vous utilisez le support [`i18n`](/docs/pages/building-your-application/routing/internationalization) avec des réécritures, chaque `source` et `destination` est automatiquement préfixé pour gérer les `locales` configurées, sauf si vous ajoutez `locale: false` à la réécriture. Si `locale: false` est utilisé, vous devez préfixer la `source` et la `destination` avec une locale pour qu'elle corresponde correctement.
