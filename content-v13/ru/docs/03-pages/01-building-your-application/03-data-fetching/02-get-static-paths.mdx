...
translation-updated-at: 2025-06-06T17:18:28.209Z
...
Если страница использует [Динамические маршруты (Dynamic Routes)](/docs/pages/building-your-application/routing/dynamic-routes) и `getStaticProps`, ей необходимо определить список путей для статической генерации.
...
`getStaticPaths` выполняется только во время сборки в production, он не вызывается во время выполнения. Вы можете проверить, что код внутри `getStaticPaths` удален из клиентского бандла, [с помощью этого инструмента](https://next-code-elimination.vercel.app/).
...
- Можно экспортировать `getStaticPaths` из [Динамического маршрута (Dynamic Route)](/docs/pages/building-your-application/routing/dynamic-routes), который также использует `getStaticProps`
...
`getStaticPaths` позволяет контролировать, какие страницы генерируются во время сборки, а не по требованию с помощью [`fallback`](/docs/pages/api-reference/functions/get-static-paths#fallback-blocking). Генерация большего количества страниц во время сборки замедляет процесс.
