...
translation-updated-at: 2025-06-06T17:22:41.880Z
...
{/* Содержание этого документа общее для маршрутизаторов app и pages. Компонент `<PagesOnly>Content</PagesOnly>` можно использовать для добавления контента, специфичного для маршрутизатора Pages. Общий контент не должен быть обёрнут в компонент. */}
...
`exportPathMap` позволяет задать соответствие путей запросов целевым страницам для использования при экспорте. Пути, определённые в `exportPathMap`, также будут доступны при использовании [`next dev`](/docs/app/api-reference/next-cli#development).
...
- `distDir` — Абсолютный путь к директории `.next/` (настраивается с помощью конфига [`distDir`](/docs/pages/api-reference/next-config-js/distDir))
...
[`next export`](/docs/app/building-your-application/deploying/static-exports) использует `out` как стандартную выходную директорию. Её можно изменить с помощью аргумента `-o`:

</AppOnly>

<PagesOnly>

[`next export`](/docs/pages/building-your-application/deploying/static-exports) использует `out` как стандартную выходную директорию. Её можно изменить с помощью аргумента `-o`:
