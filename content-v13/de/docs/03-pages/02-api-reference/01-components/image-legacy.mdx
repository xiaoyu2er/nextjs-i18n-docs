...
translation-updated-at: 2025-06-06T17:55:03.722Z
title: <Image> (Legacy)
...
- [Legacy Image-Komponente](https://github.com/vercel/next.js/tree/canary/examples/image-legacy-component)
...
- Entfernt `<span>`-Wrapper um `<img>` zugunsten von [nativer berechneter Seitenverhältnis](https://caniuse.com/mdn-html_elements_img_aspect_ratio_computed_from_attributes)
...
- Entfernt `IntersectionObserver`-Implementierung zugunsten von [nativem Lazy Loading](https://caniuse.com/loading-lazy-attr)
  - Entfernt `lazyBoundary`-Prop, da es kein natives Äquivalent gibt
  - Entfernt `lazyRoot`-Prop, da es kein natives Äquivalent gibt
- Entfernt `loader`-Konfiguration zugunsten des [`loader`](#loader)-Props
...
- Eine [statisch importierte](/docs/pages/building-your-application/optimizing/images#local-images) Bilddatei
...
Bei Verwendung einer externen URL muss diese zu [remotePatterns](#remote-patterns) in `next.config.js` hinzugefügt werden.
...
Bei Verwendung von `layout="intrinsic"` oder `layout="fixed"` stellt die `width`-Eigenschaft die _gerenderte_ Breite in Pixeln dar und beeinflusst somit die Anzeigegröße des Bildes.
...
Die `width`-Eigenschaft ist erforderlich, außer für [statisch importierte Bilder](/docs/pages/building-your-application/optimizing/images#local-images) oder solche mit `layout="fill"`.
...
Bei Verwendung von `layout="intrinsic"` oder `layout="fixed"` stellt die `height`-Eigenschaft die _gerenderte_ Höhe in Pixeln dar und beeinflusst somit die Anzeigegröße des Bildes.
...
Die `height`-Eigenschaft ist erforderlich, außer für [statisch importierte Bilder](/docs/pages/building-your-application/optimizing/images#local-images) oder solche mit `layout="fill"`.

## Optionale Props

Die `<Image />`-Komponente akzeptiert eine Reihe zusätzlicher Eigenschaften neben den erforderlichen. Dieser Abschnitt beschreibt die am häufigsten verwendeten Eigenschaften der Image-Komponente. Details zu seltener verwendeten Eigenschaften finden Sie im Abschnitt [Erweiterte Props](#advanced-props).
...
| --------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------- | --------------------- |
| `intrinsic` (Standard) | Skaliert _herunter_, um in die Containerbreite zu passen, bis zur Bildgröße | `1x`, `2x` (basierend auf [imageSizes](#image-sizes))                                                            | N/A     | ja                   |
| `fixed`               | Exakt auf `width` und `height` skaliert                    | `1x`, `2x` (basierend auf [imageSizes](#image-sizes))                                                            | N/A     | ja                   |
| `responsive`          | Skaliert, um in die Containerbreite zu passen                          | `640w`, `750w`, ... `2048w`, `3840w` (basierend auf [imageSizes](#image-sizes) und [deviceSizes](#device-sizes)) | `100vw` | ja                   |
...
- [Demo Hintergrundbild](https://image-legacy-component.nextjs.gallery/background)

### loader

Eine benutzerdefinierte Funktion zur Auflösung von URLs. Das Setzen des Loaders als Prop der Image-Komponente überschreibt den Standard-Loader, der im [`images`-Abschnitt von `next.config.js`](#loader-configuration) definiert ist.
...
Sie sollten die `priority`-Eigenschaft für jedes Bild verwenden, das als [Largest Contentful Paint (LCP)](https://nextjs.org/learn/seo/web-performance/lcp)-Element erkannt wird. Es kann angemessen sein, mehrere Priority-Bilder zu haben, da verschiedene Bilder für verschiedene Viewport-Größen das LCP-Element sein können.
...
Bei `blur` wird die [`blurDataURL`](#blurdataurl)-Eigenschaft als Platzhalter verwendet. Wenn `src` ein Objekt aus einem [statischen Import](/docs/pages/building-your-application/optimizing/images#local-images) ist und das importierte Bild `.jpg`, `.png`, `.webp` oder `.avif` ist, wird `blurDataURL` automatisch ausgefüllt.
...
Dieser Wert wird an die [object-position CSS-Eigenschaft](https://developer.mozilla.org/docs/Web/CSS/object-position) übergeben, die auf das Bild angewendet wird.
...
> **Hinweis**: Diese Eigenschaft ist nur für fortgeschrittene Nutzung gedacht. Das Umschalten eines
> Bildes auf `eager`-Laden wird normalerweise die **Leistung beeinträchtigen**.
>
> Wir empfehlen stattdessen die Verwendung der [`priority`](#priority)-Eigenschaft, die
> das Bild für fast alle Anwendungsfälle korrekt eager lädt.
...
[Mehr erfahren](https://developer.mozilla.org/docs/Web/HTML/Element/img#attr-loading)

### blurDataURL

Eine [Data URL](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs), die
als Platzhalterbild verwendet wird, bevor das `src`-Bild erfolgreich geladen wird. Wirkt nur in Kombination
mit [`placeholder="blur"`](#placeholder).

Muss ein base64-kodiertes Bild sein. Es wird vergrößert und unscharf gemacht, daher wird ein sehr kleines Bild (10px oder
weniger) empfohlen. Die Verwendung größerer Bilder als Platzhalter kann die Leistung Ihrer Anwendung beeinträchtigen.

Probieren Sie es aus:

- [Demo der standardmäßigen `blurDataURL`-Prop](https://image-legacy-component.nextjs.gallery/placeholder)
- [Demo des Schimmer-Effekts mit `blurDataURL`-Prop](https://image-legacy-component.nextjs.gallery/shimmer)
- [Demo des Farb-Effekts mit `blurDataURL`-Prop](https://image-legacy-component.nextjs.gallery/color)
...
Eine Zeichenkette (mit ähnlicher Syntax wie die margin-Eigenschaft), die als Begrenzungsrahmen dient, um die Überschneidung des Viewports mit dem Bild zu erkennen und das Lazy [Loading](#loading) auszulösen. Standardwert ist `"200px"`.

Wenn das Bild in einem scrollbaren Elternelement außerhalb des Root-Dokuments verschachtelt ist, müssen Sie auch das [lazyRoot](#lazyroot)-Prop zuweisen.

[Mehr erfahren](https://developer.mozilla.org/docs/Web/API/IntersectionObserver/rootMargin)

### lazyRoot

Ein React [Ref](https://react.dev/learn/referencing-values-with-refs), der auf das scrollbare Elternelement zeigt. Standardwert ist `null` (der Dokument-Viewport).

Die Ref muss auf ein DOM-Element oder eine React-Komponente zeigen, die [die Ref weiterleitet](https://react.dev/reference/react/forwardRef) an das zugrunde liegende DOM-Element.
...
[Mehr erfahren](https://developer.mozilla.org/docs/Web/API/IntersectionObserver/root)
...
> **Warnung**: Wir empfehlen, strikte [`remotePatterns`](#remote-patterns) anstelle von `domains` zu konfigurieren, um Ihre Anwendung vor böswilligen Nutzern zu schützen. Verwenden Sie `domains` nur, wenn Ihnen alle Inhalte der Domain gehören.
...
- Benutzerdefiniert: `loader: 'custom'` verwendet einen benutzerdefinierten Cloud-Anbieter durch Implementierung der [`loader`](#loader)-Eigenschaft in der `next/legacy/image`-Komponente

Falls Sie einen anderen Anbieter benötigen, können Sie die [`loader`](#loader)-Eigenschaft mit `next/legacy/image` verwenden.

> Bilder können nicht zur Build-Zeit mit [`output: 'export'`](/docs/pages/building-your-application/deploying/static-exports) optimiert werden, sondern nur bei Bedarf. Um `next/legacy/image` mit `output: 'export'` zu verwenden, müssen Sie einen anderen Loader als den Standard verwenden. [Mehr in der Diskussion.](https://github.com/vercel/next.js/discussions/19065)

> Der Standard-Loader der `next/legacy/image`-Komponente verwendet [`squoosh`](https://www.npmjs.com/package/@squoosh/lib), da es schnell installiert ist und für eine Entwicklungsumgebung geeignet ist. Wenn Sie `next start` in Ihrer Produktionsumgebung verwenden, wird dringend empfohlen, [`sharp`](https://www.npmjs.com/package/sharp) durch Ausführen von `npm i sharp` in Ihrem Projektverzeichnis zu installieren. Dies ist für Vercel-Bereitstellungen nicht erforderlich, da `sharp` automatisch installiert wird.
...
Sie können eine Liste von Bildbreiten mit der Eigenschaft `images.imageSizes` in Ihrer `next.config.js`-Datei angeben. Diese Breiten werden mit dem Array der [Gerätegrößen](#device-sizes) kombiniert, um das vollständige Array der Größen zu bilden, die zur Generierung der Bild-[srcset](https://developer.mozilla.org/docs/Web/API/HTMLImageElement/srcset)s verwendet werden.

Der Grund für die beiden separaten Listen ist, dass `imageSizes` nur für Bilder verwendet wird, die eine [`sizes`](#sizes)-Eigenschaft bereitstellen, was darauf hinweist, dass das Bild kleiner als die volle Bildschirmbreite ist. **Daher sollten die Größen in `imageSizes` alle kleiner sein als die kleinste Größe in `deviceSizes`.**
...
Die standardmäßige [Image Optimization API](#loader-configuration) erkennt automatisch die vom Browser unterstützten Bildformate über den `Accept`-Header der Anfrage.
...
Sie können die Time to Live (TTL) in Sekunden für zwischengespeicherte optimierte Bilder konfigurieren. In vielen Fällen ist es besser, ein [statisches Bildimport](/docs/pages/building-your-application/optimizing/images#local-images) zu verwenden, das den Dateiinhalt automatisch hashed und das Bild für immer mit einem `Cache-Control`-Header von `immutable` zwischenspeichert.
...
Wenn Sie das Caching-Verhalten pro Bild ändern müssen, können Sie [`headers`](/docs/pages/api-reference/next-config-js/headers) konfigurieren, um den `Cache-Control`-Header auf dem Upstream-Bild zu setzen (z.B. `/some-asset.jpg`, nicht `/_next/image` selbst).

Es gibt derzeit keinen Mechanismus zum Invalidieren des Caches, daher ist es am besten, `minimumCacheTTL` niedrig zu halten. Andernfalls müssen Sie möglicherweise manuell die [`src`](#src)-Eigenschaft ändern oder `<distDir>/cache/images` löschen.
...
Der Standard-[Loader](#loader) optimiert SVG-Bilder aus mehreren Gründen nicht. Erstens ist SVG ein Vektorformat, was bedeutet, dass es verlustfrei skaliert werden kann. Zweitens hat SVG viele der gleichen Funktionen wie HTML/CSS, was ohne entsprechende [Content Security Policy (CSP) headers](/docs/app/api-reference/next-config-js/headers) zu Sicherheitslücken führen kann.
