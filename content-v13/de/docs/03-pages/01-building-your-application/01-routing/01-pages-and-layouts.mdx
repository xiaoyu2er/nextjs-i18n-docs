...
translation-updated-at: 2025-06-06T17:56:29.786Z
...
In Next.js ist eine **Seite** eine [React-Komponente](https://react.dev/learn/your-first-component), die aus einer `.js`-, `.jsx`-, `.ts`- oder `.tsx`-Datei im `pages`-Verzeichnis exportiert wird. Jede Seite ist anhand ihres Dateinamens mit einer Route verknüpft.
...
Der Router leitet Dateien namens `index` automatisch an das Stammverzeichnis weiter.
...
Der Router unterstützt verschachtelte Dateien. Wenn Sie eine verschachtelte Ordnerstruktur erstellen, werden Dateien weiterhin automatisch entsprechend geroutet.
...
### Ein gemeinsames Layout mit Custom App
...
Wenn Sie mehrere Layouts benötigen, können Sie Ihren Seiten eine Eigenschaft `getLayout` hinzufügen, die es Ihnen ermöglicht, eine React-Komponente für das Layout zurückzugeben. Dadurch können Sie das Layout _pro Seite_ definieren. Da wir eine Funktion zurückgeben, können wir bei Bedarf auch komplexe verschachtelte Layouts erstellen.
...
Dieses Layout-Muster ermöglicht die Zustandserhaltung, da die React-Komponentenstruktur zwischen Seitenübergängen erhalten bleibt. Mit der Komponentenstruktur kann React erkennen, welche Elemente sich geändert haben, um den Zustand zu erhalten.
...
Wenn Sie TypeScript verwenden, müssen Sie zunächst einen neuen Typ für Ihre Seiten erstellen, der eine `getLayout`-Funktion enthält. Anschließend müssen Sie einen neuen Typ für Ihre `AppProps` erstellen, der die `Component`-Eigenschaft überschreibt, um den zuvor erstellten Typ zu verwenden.
...
In Ihrem Layout können Sie clientseitig Daten mit `useEffect` oder einer Bibliothek wie [SWR](https://swr.vercel.app/) abrufen. Da diese Datei keine [Seite](/docs/pages/building-your-application/routing/pages-and-layouts) ist, können Sie derzeit nicht `getStaticProps` oder `getServerSideProps` verwenden.
