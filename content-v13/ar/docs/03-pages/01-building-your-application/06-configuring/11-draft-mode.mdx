...
translation-updated-at: 2025-06-06T17:53:31.184Z
...
يكون التوليد الثابت مفيدًا عندما تستخدم صفحاتك نظام إدارة محتوى (CMS) بدون واجهة أمامية. لكنه ليس الخيار الأمثل عندما تكتب مسودة في نظام إدارة المحتوى الخاص بك وتريد معاينة المسودة فورًا على صفحتك. سترغب في أن يقوم Next.js بعرض هذه الصفحات في **وقت الطلب** بدلاً من وقت البناء وجلب محتوى المسودة بدلاً من المحتوى المنشور. سترغب في أن يتجاوز Next.js التوليد الثابت لهذه الحالة المحددة فقط.

يوفر Next.js ميزة تسمى **وضع المسودة (Draft Mode)** التي تحل هذه المشكلة. إليك التعليمات حول كيفية استخدامها.
...
يمكنك اختبار هذا يدويًا عن طريق إنشاء مسار API كما يلي والوصول إليه يدويًا من متصفحك:

```ts filename="pages/api/draft.ts"
// مثال بسيط لاختباره يدويًا من متصفحك.
export default function handler(req, res) {
  res.setDraftMode({ enable: true })
  res.end('تم تفعيل وضع المسودة')
}
```

إذا فتحت أدوات المطور في متصفحك وزرت `/api/draft`، ستلاحظ رأس استجابة `Set-Cookie` يحتوي على كوكي باسم `__prerender_bypass`.

### الوصول إليه بأمان من نظام إدارة المحتوى الخاص بك

عمليًا، سترغب في استدعاء مسار API هذا _بأمان_ من نظام إدارة المحتوى الخاص بك. الخطوات المحددة ستختلف حسب نظام إدارة المحتوى الذي تستخدمه، ولكن إليك بعض الخطوات الشائعة التي يمكنك اتخاذها.
...
**أولاً**، يجب إنشاء **رمز سري** باستخدام أداة توليد الرموز التي تختارها. سيعرف هذا السر فقط تطبيق Next.js الخاص بك ونظام إدارة المحتوى. يمنع هذا السر الأشخاص الذين لا يمكنهم الوصول إلى نظام إدارة المحتوى الخاص بك من الوصول إلى عناوين URL للمسودات.
...
export default async (req, res) => {
  ...
  // اتصل بنظام إدارة المحتوى للتحقق من وجود `slug` المقدم
  // ستنفذ getPostBySlug منطق الجلب المطلوب لنظام إدارة المحتوى
  const post = await getPostBySlug(req.query.slug)

  // إذا لم يكن slug موجودًا، امنع تفعيل وضع المسودة
  if (!post) {
    return res.status(401).json({ message: 'slug غير صالح' })
  }

  // فعّل وضع المسودة عن طريق تعيين الكوكي
  res.setDraftMode({ enable: true })

  // أعد التوجيه إلى المسار من المنشور الذي تم جلب بياناته
...
استخدمنا `res.setDraftMode` في مسار API للمسودة، لذا `context.draftMode` سيكون `true`.
...
قم بتعيين هذا كعنوان URL للمسودة في نظام إدارة المحتوى الخاص بك أو قم بالوصول يدويًا، ويجب أن تكون قادرًا على رؤية المسودة.
...
ثم أرسل طلبًا إلى `/api/disable-draft` لاستدعاء مسار API. إذا كنت تستدعي هذا المسار باستخدام [`next/link`](/docs/pages/api-reference/components/link)، فيجب تمرير `prefetch={false}` لمنع حذف الكوكي عن طريق الخطأ أثناء الجلب المسبق.
...
> **ملاحظة جيدة**: لا يجب تعيين رأس `Cache-Control` عند استخدام وضع المسودة لأنه لا يمكن تجاوزه. بدلاً من ذلك، نوصي باستخدام [ISR](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration).

### يعمل مع مسارات API

سيكون لدى مسارات API إمكانية الوصول إلى `draftMode` على كائن الطلب. على سبيل المثال:
...
سيتم إنشاء قيمة جديدة لكوكي التجاوز في كل مرة تقوم فيها بتشغيل `next build`.

هذا يضمن عدم إمكانية تخمين كوكي التجاوز.

> **ملاحظة جيدة**: لاختبار وضع المسودة محليًا عبر HTTP، سيحتاج متصفحك إلى السماح بملفات تعريف الارتباط من أطراف ثالثة والوصول إلى التخزين المحلي.