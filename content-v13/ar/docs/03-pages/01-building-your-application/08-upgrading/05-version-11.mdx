...
translation-updated-at: 2025-06-06T17:51:57.556Z
title: الإصدار 11
...
دليل إخراج البناء (الافتراضي هو `.next`) يتم تنظيفه الآن افتراضيًا باستثناء ذاكرة التخزين المؤقت لـ Next.js. يمكنك الرجوع إلى [RFC الخاص بتنظيف `distDir`](https://github.com/vercel/next.js/discussions/6009) لمزيد من المعلومات.
...
يدعم Next.js 11 متغير البيئة `PORT` لتعيين المنفذ الذي يعمل عليه التطبيق. لا يزال يوصى باستخدام `-p`/`--port` ولكن إذا كنت محظورًا من استخدام `-p` بأي طريقة، يمكنك الآن استخدام `PORT` كبديل:
...
تم إهمال `componentDidCatch` لمكون `next/app` في Next.js 9 لأنه لم يعد هناك حاجة إليه وأصبح منذ ذلك الحين بلا تأثير. في Next.js 11، تمت إزالته.
...
تم إهمال هذا التصدير في Next.js 9 لأنه لم يعد هناك حاجة إليه وأصبح منذ ذلك الحين بلا تأثير مع ظهور تحذير أثناء التطوير. في Next.js 11، تمت إزالته.
...
تم إهمال هذه الخاصية في Next.js 4 وكانت تظهر تحذيرًا أثناء التطوير منذ ذلك الحين. مع إدخال `getStaticProps` / `getServerSideProps`، كانت هذه الطرق تمنع بالفعل استخدام `props.url`. في Next.js 11، تمت إزالته تمامًا.

يمكنك التعلم أكثر في [التوثيق](/docs/messages/url-deprecated).

### إزالة الخاصية `unsized` من `next/image`
...
### إزالة الخاصية `modules` من `next/dynamic`
...
يمكنك إلغاء تفعيل هذا الافتراضي الجديد عن طريق إضافة `excludeDefaultMomentLocales: false` إلى `next.config.js` إذا كنت لا تريد السلوك الجديد، لاحظ أنه يوصى بشدة بعدم تعطيل هذا التحسين الجديد لأنه يقلل بشكل كبير من حجم Moment.js.
...
إذا كان تطبيقك يستخدم `router.router.events` الذي كان خاصية داخلية ولم تكن عامة، تأكد من استخدام `router.events` أيضًا.

## الترقية من React 16 إلى 17

أدخل React 17 [تحويل JSX جديد](https://reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) الذي يجلب ميزة كانت موجودة منذ فترة طويلة في Next.js إلى مجتمع React الأوسع: عدم الحاجة إلى `import React from 'react'` عند استخدام JSX. عند استخدام React 17، سيقوم Next.js تلقائيًا باستخدام التحويل الجديد. هذا التحويل لا يجعل المتغير `React` عامًا، وهو ما كان تأثيرًا جانبيًا غير مقصود للتنفيذ السابق لـ Next.js. يتوفر [أداة تعديل تلقائي](/docs/pages/building-your-application/upgrading/codemods#add-missing-react-import) لإصلاح الحالات التي استخدمت فيها `React` دون استيراده عن طريق الخطأ.
