...
translation-updated-at: 2025-05-21T20:56:24.990Z
...
`exportPathMap` 允许您指定请求路径到页面目标的映射关系，用于导出过程。在 [`next dev`](/docs/app/api-reference/next-cli#development) 中使用时，`exportPathMap` 中定义的路径同样有效。
...
- `dir` - 项目目录的绝对路径
- `outDir` - `out/` 目录的绝对路径（可通过 `-o` [自定义输出目录](#自定义输出目录)）。当 `dev` 为 `true` 时，`outDir` 的值为 `null`。
...
[`next export`](/docs/app/building-your-application/deploying/static-exports) 默认使用 `out` 作为输出目录，您可以使用 `-o` 参数自定义输出目录，如下所示：

</AppOnly>

<PagesOnly>

[`next export`](/docs/pages/building-your-application/deploying/static-exports) 默认使用 `out` 作为输出目录，您可以使用 `-o` 参数自定义输出目录，如下所示：
