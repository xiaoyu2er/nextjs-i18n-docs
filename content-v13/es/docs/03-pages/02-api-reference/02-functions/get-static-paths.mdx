...
translation-updated-at: 2025-06-06T17:08:19.625Z
...
La función `getStaticPaths` debe devolver un objeto con las siguientes propiedades **requeridas**:

### `paths`

La clave `paths` determina qué rutas serán pre-renderizadas. Por ejemplo, supongamos que tienes una página que usa [Rutas Dinámicas](/docs/pages/building-your-application/routing/dynamic-routes) llamada `pages/posts/[id].js`. Si exportas `getStaticPaths` desde esta página y devuelves lo siguiente para `paths`:
...
Las cadenas de `params` son **sensibles a mayúsculas/minúsculas** y idealmente deberían normalizarse para garantizar que las rutas se generen correctamente. Por ejemplo, si se devuelve `WoRLD` para un parámetro, solo coincidirá si `WoRLD` es la ruta visitada realmente, no `world` o `World`.

Además del objeto `params`, se puede devolver un campo `locale` cuando [i18n está configurado](/docs/pages/building-your-application/routing/internationalization), que configura el locale para la ruta que se está generando.

### `fallback: false`

Si `fallback` es `false`, cualquier ruta no devuelta por `getStaticPaths` resultará en una **página 404**.

Cuando se ejecuta `next build`, Next.js verificará si `getStaticPaths` devolvió `fallback: false`, luego construirá **solo** las rutas devueltas por `getStaticPaths`. Esta opción es útil si tienes un número pequeño de rutas para crear, o si los datos de la página no se añaden con frecuencia. Si necesitas añadir más rutas y tienes `fallback: false`, deberás ejecutar `next build` nuevamente para que las nuevas rutas puedan generarse.

El siguiente ejemplo pre-renderiza un post de blog por página llamado `pages/posts/[id].js`. La lista de posts se obtendrá de un CMS y será devuelta por `getStaticPaths`. Luego, para cada página, obtiene los datos del post desde un CMS usando [`getStaticProps`](/docs/pages/building-your-application/data-fetching/get-static-props).
...
> **Nota importante**: `fallback: true` no es compatible cuando se usa [`output: 'export'`](/docs/pages/building-your-application/deploying/static-exports).
...
En su lugar, puedes generar estáticamente un subconjunto pequeño de páginas y usar `fallback: true` para el resto. Cuando alguien solicite una página que no se ha generado aún, el usuario verá la página con un indicador de carga o un componente esqueleto.
...
`fallback: true` no _actualizará_ las páginas generadas, para eso mira [Regeneración Estática Incremental](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration).

### `fallback: 'blocking'`

Si `fallback` es `'blocking'`, las nuevas rutas no devueltas por `getStaticPaths` esperarán a que se genere el `HTML`, idéntico a SSR (de ahí el nombre _blocking_), y luego se almacenarán en caché para solicitudes futuras, por lo que solo ocurre una vez por ruta.

`getStaticProps` se comportará de la siguiente manera:

- Las rutas devueltas por `getStaticPaths` se renderizarán a `HTML` en tiempo de construcción por `getStaticProps`.
- Las rutas que no se hayan generado en tiempo de construcción **no** resultarán en una página 404. En su lugar, Next.js hará SSR en la primera solicitud y devolverá el `HTML` generado.
...
`fallback: 'blocking'` no _actualizará_ las páginas generadas por defecto. Para actualizar páginas generadas, usa [Regeneración Estática Incremental](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration) junto con `fallback: 'blocking'`.

> **Nota importante**: `fallback: 'blocking'` no es compatible cuando se usa [`output: 'export'`](/docs/pages/building-your-application/deploying/static-exports).
...
| Versión   | Cambios                                                                                                                                                                                                    |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](/docs/app/building-your-application/data-fetching) ahora es estable con una obtención de datos simplificada, incluyendo [`generateStaticParams()`](/docs/app/api-reference/functions/generate-static-params) |
...
| `v12.1.0` | [Regeneración Estática Incremental bajo demanda](/docs/pages/building-your-application/data-fetching/incremental-static-regeneration#on-demand-revalidation) añadida (beta).                                      |
