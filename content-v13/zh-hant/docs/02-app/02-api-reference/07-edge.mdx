...
translation-updated-at: 2025-05-25T20:46:48.333Z
title: Edge Runtime
description: Edge Runtime 的 API 參考文檔。
---

{/* 本文件內容在 app 和 pages 路由之間共享。您可以使用 `<PagesOnly>內容</PagesOnly>` 元件來新增專屬於 Pages Router 的內容。任何共享內容不應包裹在元件中。 */}
...
| API                                                                       | 描述                                                                                         |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| [`crypto`](https://developer.mozilla.org/docs/Web/API/Window/crypto)      | 提供對平台加密功能的存取                                                                    |
| [`CryptoKey`](https://developer.mozilla.org/docs/Web/API/CryptoKey)       | 表示一個加密金鑰                                                                           |
| [`SubtleCrypto`](https://developer.mozilla.org/docs/Web/API/SubtleCrypto) | 提供對常見加密原語的存取，如雜湊、簽章、加密或解密                                         |
...
| [`setInterval`](https://developer.mozilla.org/docs/Web/API/setInterval)                                               | 以固定的時間間隔重複呼叫函式                                                                                                                               |
...
| [`TypeError`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/TypeError)                   | 表示值的類型不符合預期時發生的錯誤                                                                                                                                         |
...
| [`URLSearchParams`](https://developer.mozilla.org/docs/Web/API/URLSearchParams)                                       | 表示鍵值對的集合                                                                                                                                                           |
| [`WeakMap`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/WeakMap)                       | 表示鍵值對的集合，其中鍵是弱引用                                                                                                                   |
...
在極少數情況下，你的程式碼可能包含（或引入）某些動態程式碼評估語句，這些語句_在執行時無法觸達_且無法透過 tree shaking 移除。

你可以透過中介軟體或 Edge API 路由的導出配置來放寬檢查，允許特定檔案：

```javascript
export const config = {
  runtime: 'edge', // 僅適用於 Edge API 路由
...
`unstable_allowDynamic` 是一個 [glob](https://github.com/micromatch/micromatch#matching-features) 或 glob 陣列，用於忽略特定檔案的動態程式碼評估。這些 glob 是相對於應用程式根目錄的路徑。

請注意，如果這些語句在 Edge 環境中執行，_它們會拋出錯誤並導致執行時錯誤_。
