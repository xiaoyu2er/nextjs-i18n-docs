...
translation-updated-at: 2025-05-25T21:35:59.869Z
title: 資料獲取
description: Next.js 提供了多種資料獲取方式，包含預先渲染 (pre-rendering)、伺服器渲染 (SSR) 或靜態網站生成 (SSG)，以及增量靜態再生 (ISR)。了解如何在 Next.js 中管理應用程式資料。
---

Next.js 中的資料獲取功能讓您可以根據應用程式的使用情境，以不同方式呈現內容。這些方式包含使用 **伺服器渲染 (SSR)** 或 **靜態生成 (SSG)** 進行預先渲染，以及透過 **增量靜態再生 (ISR)** 在運行時更新或建立內容。
