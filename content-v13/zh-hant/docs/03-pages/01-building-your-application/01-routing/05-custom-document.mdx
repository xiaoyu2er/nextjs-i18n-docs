...
translation-updated-at: 2025-05-25T21:37:46.394Z
title: 自訂文件
description: 擴充 Next.js 預設加入的文件標記。
---

自訂 `Document` 可用來更新渲染 [頁面](/docs/pages/building-your-application/routing/pages-and-layouts) 時使用的 `<html>` 和 `<body>` 標籤。

要覆寫預設的 `Document`，請按照以下方式建立 `pages/_document` 檔案：
...
> - 頁面要正確渲染必須包含 `<Html>`、`<Head />`、`<Main />` 和 `<NextScript />`。
...
自訂 `renderPage` 屬於進階用法，僅在需要支援伺服器端渲染的函式庫（如 CSS-in-JS）時使用。內建的 `styled-jsx` 支援不需要此設定。

**我們不建議使用此模式。** 相反地，請考慮 [逐步採用](/docs/app/building-your-application/upgrading/app-router-migration) App Router，它能更輕鬆地為 [頁面和版面配置](/docs/app/building-your-application/routing/pages-and-layouts) 獲取資料。
...
class MyDocument extends Document {
  static async getInitialProps(
    ...
    // 執行父層的 `getInitialProps`，現在包含自訂的 `renderPage`
...
> **須知事項**
>
> - `_document` 中的 `getInitialProps` 不會在客戶端轉換時呼叫。
> - `_document` 的 `ctx` 物件等同於 [`getInitialProps`](/docs/pages/api-reference/functions/get-initial-props#context-object) 接收的物件，並增加了 `renderPage` 屬性。