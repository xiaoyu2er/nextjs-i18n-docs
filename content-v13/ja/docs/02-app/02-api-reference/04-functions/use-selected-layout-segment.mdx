...
translation-updated-at: 2025-06-06T17:16:18.105Z
title: useSelectedLayoutSegment
description: useSelectedLayoutSegment フックの API リファレンス
---

`useSelectedLayoutSegment` は、呼び出し元のレイアウトから **1階層下** のアクティブなルートセグメントを読み取ることができる **クライアントコンポーネント** 用フックです。
...
以下のレイアウトとURLの例では、返されるセグメントは次のようになります:
...
`useSelectedLayoutSegment` を使用して、アクティブなセグメントに応じてスタイルを変更するリンクコンポーネントを作成できます。例えば、ブログのサイドバーに表示される特集記事リスト:
